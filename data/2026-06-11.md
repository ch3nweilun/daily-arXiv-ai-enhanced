...
### [1] [RankVR: Low-Rank Structure Perception and Value Recalibration for Robust Composed Image Retrieval](https://arxiv.org/abs/2606.11689)
*Jiale Huang,Zixu Li,Zhiheng Fu,Zhiwei Chen,Qinlei Huang,Yupeng Hu*

Main category: cs.CV

TL;DR: 提出RankVR框架，通过全局结构一致性和动态值感知解决组合图像检索中的噪声三元组对应问题。


<details>
  <summary>Details</summary>
Motivation: 现有去噪方法忽略全局结构相关性和动态值变化，存在全局结构不一致和难样本区分不确定性两个未解决挑战。

Method: RankVR包含两个模块：GSCP利用相关矩阵的有效秩分离干净样本与结构噪声；ASVC动态量化每个三元组的语义值，区分高价值难样本并抑制逻辑冲突噪声。

Result: 在FashionIQ和CIRR数据集上，RankVR显著优于现有最先进方法，验证了其在噪声环境下的鲁棒性。

Conclusion: RankVR通过全局结构一致性和动态值感知有效提升了CIR模型在噪声环境下的鲁棒性。
...
### [2] [Bridging the Modality Gap in Forensic Image Retrieval](https://arxiv.org/abs/2606.12294)
*Ricardo González-Gazapo,Annette Morales-González,Yoanna Martínez-Díaz,Heydi Méndez-Vázquez,Milton García-Borroto*

Main category: cs.CV

TL;DR: 提出统一多模态检索框架，融合视觉和文本嵌入，提升法医图像检索精度与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有研究集中于多模态检索系统优化，缺乏对法医场景真实适用性的评估。

Method: 利用多模态大语言模型生成结构化文本描述，结合句子转换器嵌入，并融合视觉与文本相似度分数。

Result: 多模态融合显著提升检索精度和鲁棒性，尤其在视觉信息有限或噪声场景（如草图、部分纹身）。

Conclusion: 统一多模态管路有望支持涉及纹身、面部合成和证人描述的法医调查工作流。
...
### [3] [MedCTA: A Benchmark for Clinical Tool Agents](https://arxiv.org/abs/2606.11702)
*Tajamul Ashraf,Hyewon Jeong,Fida Mohammad Thoker,Bernard Ghanem*

Main category: cs.CV

TL;DR: MedCTA是一个针对医学工具代理的多步临床任务基准，揭示了当前多模态模型在工具使用中的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估孤立感知或单轮问答，无法揭示规划、工具招募和部署可靠性的失败。

Method: 构建MedCTA基准，包含107个真实临床任务和5个工具，支持过程感知评估工具选择、参数有效性、执行稳定性等。

Result: 在18个模型上测试，自主部署中协议失败、过早停止和错误工具招募占主导；最优工具路由带来增益但不完全。

Conclusion: 强大的感知能力不能转化为可靠的代理行为，MedCTA为审计和推进可信赖的医学AI代理提供了严格测试平台。
...
### [4] [Natural-Language Temporal Grounding in Hour-Long Videos is a Search Problem: A Benchmark and Empirical Decomposition](https://arxiv.org/abs/2606.12300)
*Sukmin Seo,Geewook Kim*

Main category: cs.CV

TL;DR: 长视频时间接地的主要瓶颈是搜索，而非识别；新基准ExtremeWhenBench显示Video-LLMs性能差，而检索基线有效，混合方法提升6.7倍。


<details>
  <summary>Details</summary>
Motivation: 研究小时级别视频的自然语言时间接地，因为现有研究局限于短视频，而长视频中的搜索约束尚未充分探索。

Method: 发布ExtremeWhenBench基准（194个视频，2273个查询，平均时长75.7分钟），比较Video-LLMs、帧级检索基线以及检索-然后-接地混合方法，并进行失败分类。

Result: 所有开源Video-LLMs性能崩溃，帧级检索基线优于它们；85%的失败归因于搜索；检索-然后-接地混合方法比纯Video-LLM提升6.7倍。

Conclusion: 长视频时间接地需要结合检索来克服搜索瓶颈，类似开放域问答中的检索-然后-阅读范式。
...
### [5] [SceneMiner: Identity-Preserving Multi-Task Fine-Tuning for Unified BEV Scene Mining](https://arxiv.org/abs/2606.11507)
*Abdalmalek Aburaddaha,Venkatraman Narayanan,Keval Thaker,Samir A. Rawashdeh*

Main category: cs.CV

TL;DR: SceneMiner是一种统一管线，通过冻结的视觉-语言骨干网络在单次前向传播中发出互补挖掘信号，并利用身份保持多任务微调消除跨任务干扰，从而高效挖掘驾驶日志中的困难安全关键场景。


<details>
  <summary>Details</summary>
Motivation: 从驾驶日志中挖掘困难安全关键场景因缺乏难度标签而受到瓶颈，且单个代理如碰撞风险、轨迹模糊性或语义稀有性无法独自有效发现此类场景。

Method: 提出SceneMiner，一个仅相机的鸟瞰视图管线，使用冻结的视觉-语言骨干网络输出检索嵌入、多标签场景标签分布和基于物理的连续风险分数；同时提出身份保持多任务微调，通过零初始化新子模块并冻结所有馈送共享流的参数，消除跨任务干扰。

Result: 标记头在20个场景标签上达到mAP 0.4614（micro-F1 0.5557），检索头经定性验证支持文本提示搜索，仅需训练约102k参数。

Conclusion: SceneMiner通过互补挖掘信号和身份保持多任务微调，有效解决了困难场景挖掘中的标签缺乏和跨任务干扰问题，为自动驾驶场景数据挖掘提供了新方法。
...
### [6] [AGE-MIL: Anchor-Guided Evidence Learning for Patient-Level Prediction](https://arxiv.org/abs/2606.12126)
...
TL;DR: 提出AGE-MIL，通过患者级锚点和证据积累实现稳健的患者级病理预测。


<details>
  <summary>Details</summary>
Motivation: 现有WSI级MIL方法无法有效利用患者级监督，直接施加监督导致不稳定和可靠性下降。

Method: AGE-MIL构建患者级锚点捕获全局上下文，指导局部补丁检索与整合，并将风险建模为证据积累过程。

Result: 在六个任务上优于八种先进MIL方法。

Conclusion: AGE-MIL能有效进行患者级预测，性能优于现有方法。
...
### [7] [Motion Reinforces Appearance: RGB-Skeleton Gated Residual Fusion for Micro-Gesture Online Recognition](https://arxiv.org/abs/2606.11645)
*Jialin Liu,Xinwen He,Pengyu Liu,Jiale Shi,Huaijuan Zang,Yanbin Hao*

Main category: cs.CV

TL;DR: 提出DyFADet+，一个双流RGB-骨架框架，通过门控残差模块融合多模态特征，用于微手势在线识别，在SMG数据集上F1为40.88，排名第二。


<details>
  <summary>Details</summary>
Motivation: 微手势在线识别是核心任务，但由于手势高度自发性，单模态难以捕捉完整的多模态线索，需要有效融合RGB和骨架信息。

Method: DyFADet+扩展了DyFADet为双流RGB-骨架框架，将两种模态投影到共享的多尺度时序嵌入，通过门控残差模块自适应地将骨架运动注入RGB表示，最后使用动态TAD头进行在线分类和边界回归。

Result: 在SMG数据集上取得40.88的F1分数，在微手势在线识别赛道排名第二。

Conclusion: 提出的双流融合方法能有效提升微手势在线识别的性能，验证了多模态融合在自发手势识别中的重要性。
...
### [8] [Task-Aware Structured Memory for Dynamic Multi-modal In-Context Learning](https://arxiv.org/abs/2606.11853)
*Zhirui Chen,Ziwei Chen,Ling Shao*

Main category: cs.CV

TL;DR: TASM是一个训练无关框架，通过任务感知结构记忆提升多模态大语言模型上下文学习的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs依赖ICL但受限于有限上下文窗口和KV缓存增长；现有记忆压缩方法引入偏见、破坏语义结构，无法适应新查询。

Method: 采用任务向量引导压缩、语义感知令牌合并（二分图匹配）构建层次化记忆（核心记忆与潜在库），支持查询自适应动态检索。

Result: 在高压缩率下保持高性能，有效平衡效率与适应性。

Conclusion: TASM解决了现有方法的限制，实现了有效性与可扩展性的平衡。
...
### [9] [InternVideo3: Agentify Foundation Models with Multimodal Contextual Reasoning](https://arxiv.org/abs/2606.12195)
...
TL;DR: InternVideo3通过多模态上下文推理和高效注意力机制，增强了开放多模态模型在长期视频任务中的代理能力。


<details>
  <summary>Details</summary>
Motivation: 现有开源模型在文本主导的代理任务上表现良好，但在需要长时间理解和迭代交互的多模态视频任务上探索不足。

Method: 提出多模态上下文推理（MCR）将理解作为闭环过程；引入多模态多头潜在注意力（M^2LA）压缩KV缓存；采用持续预训练、短到长监督微调、基于规则的强化学习和在策略蒸馏的分阶段训练。

Result: 在Video-MME、MLVU、EgoSchema等基准上取得强性能，作为视频代理展示了基于证据的鲁棒行为。

Conclusion: 高效上下文处理和闭环推理对于将开放多模态模型适配到长期视觉基础代理至关重要。
...
### [10] [MLT-Dedup: Efficient Large-Scale Online Video Deduplication via Multi-Level Representations and Spatial-Temporal Matching](https://arxiv.org/abs/2606.12215)
*David Yuchen Wang,Haoying Li,Hailun Xu,Wei Chee Yew,Zirui Zhu,Sanjay Saha,Hao Hei,Kanchan Sarkar,Kun Xu*

Main category: cs.CV

TL;DR: MLT-Dedup通过多级表示和时空匹配实现高效大规模在线视频去重，降低重复率91%且精度达90%。


<details>
  <summary>Details</summary>
Motivation: 用户生成视频内容爆炸增长导致大量近重复视频，影响用户体验并增加存储和带宽成本；现有框架在有限索引预算下难以检索足够高质量候选，且存在效率与精度权衡。

Method: 提出MLT-Dedup框架，使用多级视频编码器(ML-VE)提取细粒度帧级和稀疏剪辑级嵌入，稀疏嵌入用于高效候选检索，细粒度嵌入用于精确匹配；引入差分特征增强相似度模块(DiF-SiM)定位重复时空段并提供相似度证据。

Result: 在真实大规模平台上，在线重复率降低91%（90%精度），稀疏检索设计使索引容量增加5倍。

Conclusion: MLT-Dedup有效解决了大规模视频去重中的候选检索和匹配挑战，实现了高精度和效率。
...
### [11] [Findings of the MAGMaR 2026 Shared Task](https://arxiv.org/abs/2606.12295)
...
TL;DR: 这篇论文概述了MAGMaR研讨会共享任务的结果，包括视频检索和基于检索视频的文章生成任务。


<details>
  <summary>Details</summary>
Motivation: 推动多模态增强生成和检索的研究，评估不同方法在视频检索和接地生成方面的表现。

Method: 组织共享任务，包括视频检索和接地文章生成两个子任务，参与者提交系统，使用基线比较和人工评估。

Result: 检索任务中所有17个系统均超过去年冠军基线；生成任务中16个系统均至少有一份报告被人类评为最佳。

Conclusion: 共享任务展示了多模态检索和生成方面的进展，所有提交系统都表现出竞争力，且生成质量获得人类认可。
...
### [12] [Reroute, Don't Remove: Recoverable Visual Token Routing for Vision-Language Models](https://arxiv.org/abs/2606.12412)
*Cheng-Yu Yang,Shao-Yuan Lo,Yu-Lun Liu*

Main category: cs.CV

TL;DR: 提出Reroute，一种训练免费的插件，通过可恢复路由替代不可逆移除，解决视觉令牌重要性随解码器深度变化的问题，在保持性能的同时改善接地能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉令牌减少方法采用不可逆移除，但令牌重要性随解码器深度变化，早期排名低的令牌可能在后期变得相关，尤其是对于接地敏感的查询，导致性能脆弱。

Method: Reroute是训练免费的插件，在路由阶段，选中的视觉令牌通过解码器块，延迟的令牌绕过该阶段并在下一路由决策时重新加入候选池。它重用现有注意力分数排名规则和阶段调度，保留其增强的剪枝方法的理论TFLOPs和KV缓存预算。

Result: 在FastV、PDrop和Nüwa变体上，基于LLaVA-1.5和Qwen骨干，Reroute在攻击性令牌减少下改善了接地性能，同时保持一般VQA性能。

Conclusion: 视觉令牌减少不应仅被视为不可逆剪枝，还应被视为可恢复路由。
...
### [13] [i1: A Simple and Fully Open Recipe for Strong Text-to-Image Models](https://arxiv.org/abs/2606.11289)
*Boya Zeng,Tianze Luo,Shu Pu,Jucheng Shen,Taiming Lu,Gabriel Sarch,Zhuang Liu*

Main category: cs.CV

TL;DR: 通过300多个控制实验，训练了i1模型，在公开数据集上达到领先性能，并完全开源。


<details>
  <summary>Details</summary>
Motivation: 现有完全开放模型性能不足，研究社区需要完全开放的模型作为基础。

Method: 进行了300多个控制实验（总计700K+ TPU v6e小时），训练了3B参数的i1模型，仅使用公开数据集，并开源了检查点、代码和数据流程。

Result: i1在五个基准上达到领先水平，平均超过现有最佳开放模型29.5个绝对百分点。

Conclusion: 发现和i1配方为未来的开放研究建立了实践基础。
...
TL;DR: 首个真实世界草莓6D姿态数据集和合成数据集，量化了sim-to-real差距。


<details>
  <summary>Details</summary>
Motivation: 现有6D姿态估计方法仅依赖合成数据，缺乏场景级真实性，未在真实农业条件下量化性能。

Method: 收集真实农业领域的草莓6D姿态数据集（12,040张图像），并在NVIDIA Isaac Sim中创建具有场景级真实性和域随机化的合成数据集。

Result: 实验表明存在显著的sim-to-real差距，通过基线6D姿态估计结果量化了该差距。

Conclusion: 真实农业领域数据对可靠评估至关重要，数据集将在接受后公开。
...
### [15] [Cross-Modal Benchmarking for Robotic Perception in Natural Environments](https://arxiv.org/abs/2606.11563)
*David Hall,Joshua Knights,Mark Cox,Peyman Moghadam*

Main category: cs.CV

TL;DR: 揭示当前视觉模型（特别是基础模型）在自然环境中用于机器人感知时的局限性，并介绍WildCross基准，这是一个包含大量RGB帧、半稠密深度和表面法线注释、精确6DoF姿态和同步密集激光雷达子图的跨模态基准，用于位置识别和度量深度估计。


<details>
  <summary>Details</summary>
Motivation: 自然环境的复杂性给机器人感知系统带来挑战，而现有模型主要基于结构化城市环境训练，导致其在野外机器人任务中的感知能力不足。

Method: 发布WildCross基准，包含超过476K个连续RGB帧，配有半稠密深度和表面法线注释，每帧均与精确的6DoF姿态和同步的密集激光雷达子图对齐。

Result: 通过WildCross基准实验，展示了现有模型在自然环境中位置识别和度量深度估计任务中的局限性，并特别扩展了度量深度估计的实验分析。

Conclusion: 当前视觉模型在自然环境任务中表现不足，需要进一步改进以适应野外机器人感知需求；WildCross基准为相关研究提供了重要资源。
...
### [16] [TopoCap: Learning Topology-Agnostic Motion Priors for Monocular Video-to-Animation](https://arxiv.org/abs/2606.12153)
*Cheng-Feng Pu,Jia-Peng Zhang,Meng-Hao Guo,Yan-Pei Cao,Shi-Min Hu*

Main category: cs.CV

TL;DR: TopoCap is the first unified framework for extracting motion from monocular video and retargeting onto characters with arbitrary, unseen skeletal topologies without test-time optimization.


<details>
  <summary>Details</summary>
Motivation: Current motion capture methods are brittle, restricted to species-specific templates (e.g., SMPL) or require labor-intensive manual rigging, failing to handle diverse 3D assets.

Method: A two-stage generative pipeline: (1) a Universal Motion Manifold learned via Graph CVAE, compressing heterogeneous kinematic chains into shared latent codes and disentangling motion from topology using structural embeddings; (2) video-to-animation via conditional flow matching to predict topology-agnostic codes from visual features. Also introduces Mobjaverse, a dataset with >5,000 skeletal topologies and 2 million frames.

Result: Outperforms specialist models on human and quadruped benchmarks and enables zero-shot retargeting for the long tail of 3D creatures.

Conclusion: TopoCap achieves motion capture and retargeting for characters with arbitrary topologies, eliminating test-time tuning and expanding applicability to diverse 3D assets.
...
### [17] [OpenMedReason: Scientific Reasoning Supervision for Medical Vision-Language Models](https://arxiv.org/abs/2606.12169)
...
TL;DR: 提出OpenMedReason，一个大规模开放式多模态医学推理语料库，包含约45万实例，推理轨迹源自生物医学科学文章，并配套多维评估基准，显著提升LVLM的感知、医学知识和推理能力。


<details>
  <summary>Details</summary>
Motivation: 高风险临床使用需要基于视觉证据和临床知识的推理，而不仅仅是正确答案。

Method: 构建了包含约45万图像-问题-答案实例的语料库，推理轨迹来自生物医学科学文章，并设计了三维度评估基准OpenMedReason-Bench。

Result: 训练后VQA准确率提升20%，感知、医学知识和推理能力均得到提升，86.1%情况下推理轨迹优于基础模型。

Conclusion: 数据集和代码已开源，为医学LVLM提供训练和评估资源。
...
### [18] [CFCamo: A Counterfactual Detect-or-Abstain Framework for Camouflaged Object Detection](https://arxiv.org/abs/2606.11231)
*Suhang Li,Osamu Yoshie,Yuya Ieiri*

Main category: cs.CV

TL;DR: 提出CFCamo框架，通过反事实对策略优化实现伪装目标检测中的目标存在检测与目标缺失弃权，解决了过度检测偏差。


<details>
  <summary>Details</summary>
Motivation: 标准COD训练和评估数据只有正样本，导致模型产生过度检测偏差，无法处理目标缺失的情况，因此需要量化并解决目标缺失行为。

Method: 构建CF-COD配对基准衡量目标缺失行为；提出CFCamo框架，使用Qwen3-VL-4B-Instruct智能体，通过反事实序列策略优化（CSPO）采样原始与反事实配对轨迹，并使用反事实配对奖励（CPR）耦合检测与弃权。

Result: 在CAMO-test上S_alpha提升3.7个百分点；在CF-COD上达到80.0-90.8%配对准确率（PA）；消融实验显示去除耦合后PA降至1.4-5.2%。

Conclusion: CFCamo通过耦合目标存在检测与目标缺失弃权来改进COD智能体，而非仅仅加强目标定位。
...
### [19] [Exploring Adaptive Masked Reconstruction for Self-Supervised Skeleton-Based Action Recognition](https://arxiv.org/abs/2606.11450)
*Shengkai Sun,Zhiyong Cheng,Zefan Zhang,Jianfeng Dong,Zhihui Li,Meng Wang*

Main category: cs.CV

TL;DR: Adaptive Masked Reconstruction (AMR) accelerates self-supervised skeleton-based action recognition pre-training and improves accuracy by decoupling the decoder and introducing an adaptive guidance module.


<details>
  <summary>Details</summary>
Motivation: Existing masked skeleton reconstruction methods require predicting a large number of spatiotemporal patches, prolonging training time, and treat all regions equally, distracting from learning critical motion patterns.

Method: AMR decouples the decoder from the encoder to predict larger patches, reducing reconstruction complexity, and employs an adaptive guidance module to identify high-motion informativeness regions for focusing on discriminative parts.

Result: AMR substantially accelerates pre-training and improves downstream recognition accuracy on NTU RGB+D 60, NTU RGB+D 120, and PKU-MMD, surpassing state-of-the-art approaches.

Conclusion: AMR provides a faster and stronger pre-training framework that addresses the challenges of training efficiency and learning motion semantics in self-supervised skeleton-based action recognition.
...
### [20] [FreqKD: Frequency-Decoupled Cross-Modal Knowledge Distillation for Infrared Object Detection](https://arxiv.org/abs/2606.11572)
*Keval Thaker,Venkatraman Narayanan,Abdalmalek Aburaddaha,Samir A. Rawashdeh*

Main category: cs.CV

TL;DR: 提出FreqKD，一种频率解耦蒸馏框架，通过非对称监督适应RGB与IR图像在不同频率上的差异，提升跨模态迁移性能。


<details>
  <summary>Details</summary>
Motivation: 从RGB基础模型通过知识蒸馏迁移到红外图像具有挑战性，因为图像形成物理的差异；观察到特征发散在空间频率上不统一，低频更对齐，高频反映模态特异性。

Method: FreqKD框架对低频使用严格的MSE以保留共享结构信息，对高频使用松弛的log-MSE（权重0.1）以提供边缘引导并容忍纹理差异。

Result: 在KAIST多光谱行人检测上达到64.1 mAP50，比DINOv2基线提高2.4点；跨数据集（FLIR ADAS +2.1 mAP50）、任务（MFNet分割 +1.85 mIoU）和架构（ResNet-50 +1.0 mAP50）均表现提升。

Conclusion: 频率解耦蒸馏有效提升了RGB到IR的迁移学习性能，验证了非对称监督适应跨模态差异的有效性。
...
TL;DR: 提出无监督框架，通过切割和缝合两阶段适应视觉语言模型，实现无标签多标签图像识别，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型因设计偏重最显著对象，忽略其他正样本，限制了在多标签识别中的应用。

Method: 包括切割阶段：多采样响应估计器防止模型只关注单一对象；缝合阶段：多对象混合适应调整标签分布以符合多标签特性，仅需一个epoch。

Result: 在四个数据集上大幅超越现有无监督方法，甚至超过弱监督基线。

Conclusion: 预训练VLM可被适应用于更全面的视觉理解，无需人工标注。
...
### [22] [DroneShield-AI: A Multi-Modal Sensor Fusion Framework for Real-Time Autonomous Drone Threat Detection, Behavioral Intent Classification, and Swarm Intelligence in Contested Airspace](https://arxiv.org/abs/2606.11687)
*Marius Bayizere*

Main category: cs.CV

TL;DR: DroneShield-AI is an open, multi-layer framework for UAV threat detection combining RF, acoustic, visual, and behavioral analysis, achieving 96.1% accuracy with low latency and cost.


<details>
  <summary>Details</summary>
Motivation: UAV threats have become a defining security challenge of the 21st century, necessitating effective detection systems.

Method: Integrates six processing layers: RF signal classification, acoustic motor-signature detection, YOLOv8-based visual detection, evidence-weighted sensor fusion, a Behavioral Intent Classification Engine (BICE) with six-class threat taxonomy, and a Graph Neural Network Swarm Intelligence Module (GNN-SIM) for adversarial multi-drone formation analysis.

Result: 96.1% detection accuracy, 3.2% false alarm rate, AUC-ROC 0.981, 142ms end-to-end latency on commodity CPU hardware, total system cost approx. $500-$780.

Conclusion: The framework provides effective, open-source, real-time UAV threat detection suitable for deployment on low-cost hardware, with all code and data publicly released.
...
### [23] [UniReason-Med: A Shared Grounded Reasoning Interface for 2D-to-3D Transfer in Medical VQA](https://arxiv.org/abs/2606.11740)
*Mengzhuo Chen,Yan Shu,Chi Liu,Hongming Piao,Xidong Wang,Derek Li,Bryan Dai*

Main category: cs.CV

TL;DR: 提出UniReason-Med，通过共享的推理接口从2D图像迁移推理结构到3D医学VQA，利用220K指令微调数据集和强化学习，显著提升3D推理性能。


<details>
  <summary>Details</summary>
Motivation: 研究是否可以通过丰富的2D医学图像的推理监督来改进3D医学视觉问答，当两种输入类型通过共同推理接口对齐时。

Method: 提出UniReason-Med，一个单一检查点框架，推理时处理2D图像或切片序列化3D体积，通过共享框语法、区域标记注入和共同接地推理策略生成交错文本推理和局部视觉证据；构建UniMed-CoT数据集（220K样本，包括170K 2D和50K 3D）；使用监督微调后接结果级强化学习，无需基于IoU/Dice的定位奖励。

Result: 数据混合和组件消融实验表明，联合2D+3D接地监督显著优于仅3D训练，接地和区域标记注入对2D和3D任务都有持续益处。

Conclusion: 共享的接地推理接口可以将推理结构从2D图像迁移到切片序列化的体积医学理解。
...
### [24] [Scene-Adaptive Nonlinear Tone Curves for Pseudo Ground-Truth Generation in Low-Light 3D Gaussian Splatting](https://arxiv.org/abs/2606.11841)
*Mingzhe Lyu,Jinqiang Cui,Hong Zhang*

Main category: cs.CV

TL;DR: 提出场景自适应非线性色调曲线框架，用于低光照新视角合成中的伪GT生成，提升了重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有伪GT方法使用均匀线性增益，导致亮区域过曝、暗区域增强不足，限制了重建质量，而非线性色调映射在3D重建中未被探索。

Method: 提出场景自适应非线性色调曲线框架，包括百分比归一化、场景自适应偏移，以及两种曲线：有界指数曲线Adaptive SoftExp (ASE)和数据驱动三次多项式Adaptive Poly3 (AP3)。仅改变伪GT计算，不改变3DGS主干。

Result: 在三个基准21个场景上，两种曲线均一致优于线性基线，PSNR提升在LOM上高达+4.34 dB，在RealX3D上高达+3.25 dB。

Conclusion: 所提框架有效，不同形式的曲线性能相似，表明改进与曲线形式无关；代码已开源。
...
### [25] [SG2Loc: Sequential Visual Localization on 3D Scene Graphs](https://arxiv.org/abs/2606.11880)
*Nicole Damblon,Olga Vysotska,Federico Tombari,Marc Pollefeys,Daniel Barath*

Main category: cs.CV

TL;DR: 提出一种基于3D场景图的轻量级顺序视觉定位方法，通过语义匹配和粒子滤波实现高效定位。


<details>
  <summary>Details</summary>
Motivation: 传统顺序视觉定位方法需要存储大量图像数据库或点云，导致高开销，而复杂室内环境下的定位对AR和机器人应用至关重要。

Method: 使用紧凑的3D场景图表示环境，节点为带粗略网格的对象，边为空间关系。定位时提取图像逐块语义特征，在粒子滤波中每个粒子将场景图对象网格投影到图像，通过特征相似性计算粒子权重，并顺序融合多帧图像优化姿态。

Result: 显著降低存储需求，同时在真实世界数据集上保持竞争性能。

Conclusion: 该方法有效解决传统方法的存储问题，适用于复杂室内环境中的顺序视觉定位。
...
### [26] [Corpus Augmentation for Sign Language Translation via LLM-Guided Video Stitching](https://arxiv.org/abs/2606.11925)
*Zsolt Robotka,Ádám Rák,Jalal Al-Afandi,András Horváth,György Cserey*

Main category: cs.CV

TL;DR: 一种无需额外标注的手语翻译语料库增强方法，通过LLM生成合成句子和视频片段，显著提升BLEU-4分数。


<details>
  <summary>Details</summary>
Motivation: 高质量平行手语视频-文本对稀缺，限制了模型对长尾词汇和新结构的泛化能力。

Method: 利用CTC强制对齐从训练视频中提取每个词汇的片段，通过语料库锚定的LLM生成新的词汇-句子对，然后随机采样句子并分配片段，组装成合成RGB视频-文本对。

Result: 在GFSLT-VLP框架下实现+2.92 BLEU-4的提升，而之前五种免词汇表方法在相同条件下最大提升仅为0.98 BLEU-4。

Conclusion: 合成数据虽然提升了微调性能，但会损害视觉-语言预训练；建议不连续的剪辑边界可能作为隐式正则化，且追求视觉平滑在L2标准下反效果。
...
### [27] [Q-Fold: Query-Aware Focus-Context Spatio-Temporal Folding for Long Video Understanding](https://arxiv.org/abs/2606.12125)
*Biao Tang,Xu Chen,Shuxiang Gou,Jingyi Yuan,Yuhan Zhang,Chenqiang Gao*

Main category: cs.CV

TL;DR: Q-Fold是一种无需训练的输入构建框架，通过查询引导的异构焦点-上下文表示，在长视频理解中同时保留高保真视觉证据和广泛时间覆盖，在多个基准测试中一致提升性能，在超长视频上提升9.1个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有方法在有限视觉预算下处理长视频时，采用帧为中心范式，对保留内容应用相似表示而不考虑其重要性，导致难以同时保留高保真视觉证据和广泛时间覆盖。

Method: Q-Fold操作连续时间片段，在查询指导下构建异构焦点-上下文表示：查询相关片段保留为高保真焦点帧，不相关片段折叠为保持时间顺序的上下文布局。

Result: 在四个长视频基准测试和多个视频多模态大语言模型上，Q-Fold一致改进性能且不增加输入预算，在超长视频基准上提升9.1个百分点。

Conclusion: Q-Fold通过查询引导的焦点-上下文表示，有效改善了长视频理解，在不增加输入预算的情况下同时保持了高保真视觉证据、广泛时间覆盖和局部时间连续性。
...
### [28] [A Turbo-Inference Strategy for Object Detection and Instance Segmentation](https://arxiv.org/abs/2606.12371)
*Zhen Zhao,Gang Zhang,Xiaolin Hu,Liang Tang*

Main category: cs.CV

TL;DR: 提出一种涡轮推理策略，通过迭代利用检测和分割的互补信息，提升自上而下实例分割中两者的精度。


<details>
  <summary>Details</summary>
Motivation: 现有自上而下实例分割方法中检测精度直接影响分割性能，但少有探索分割对检测的影响，因此希望利用分割任务改善检测。

Method: 设计涡轮检测头和涡轮分割头，形成闭环迭代交互，无需重新训练模型。

Result: 在COCO、iFLYTEK和Cityscapes数据集上，检测和分割精度均显著提升，但计算成本增加。

Conclusion: 该方法是在预测精度和推理速度之间的权衡。
...
### [29] [OSCS-SupCon: Orthogonal Sigmoid-based Common and Style Supervised Contrastive Learning for Robust Feature Disentanglement](https://arxiv.org/abs/2606.11233)
*Bin Wang,Fadi Dornaika*

Main category: cs.CV

TL;DR: 提出OSCS-SupCon，通过sigmoid对比损失和正交约束解决SupCon的负样本稀释和特征纠缠问题，提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有监督对比学习方法存在负样本稀释和特征空间纠缠，降低特征判别性和泛化能力。

Method: 使用sigmoid对比损失（含可学习温度与偏置）自适应调整成对决策边界，并引入正交约束分离共同与风格特征子空间。

Result: 在六个基准数据集上超越现有方法，CUB200-2011上ResNet-18分类准确率提升3.4%，消融实验验证组件有效性。

Conclusion: OSCS-SupCon是一种统一框架，有效缓解负样本稀释与特征纠缠，具有强鲁棒性和泛化能力。
...
### [30] [DeceptionX: Explainable Deception Detection with Multimodal Large Language Models](https://arxiv.org/abs/2606.11385)
*Jiayu Zhang,Shuo Ye,Jiajian Huang,Yawen Cui,Taorui Wang,Wei Xia,Zeheng Wang,Haowen Tang,Hui Ma,Zitong Yu*

Main category: cs.CV

TL;DR: DeceptionX 是一个基于多模态大语言模型的欺骗检测框架，通过可解释的 Observe-Think-Summarize 推理过程和专用数据集 DeceptChain，实现高准确性和透明推理。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法将欺骗检测视为黑箱分类，缺乏可解释性且无法模拟人类专家的逻辑推理过程；多模态大语言模型虽有潜力，但需连接低级视听线索与高级逻辑推理。

Method: 提出 DeceptionX 框架，采用 Observe-Think-Summarize 推理范式；构建高质量数据集 DeceptChain，通过人机协作合成细粒度视听证据和链式思维推理数据；设计三阶段训练流程和 Discrepancy-Aware Redundancy Elimination (DARE) 策略。

Result: 在标准真实世界基准上，DeceptionX 超越了现有的 MLLM 基线和最先进方法，同时提供了透明的专家级推理路径。

Conclusion: DeceptionX 弥合了多模态欺骗检测中准确性与可解释性之间的关键差距。
...
### [31] [4DP-QA: Scalable QA for 4D Perception in Vision Language Models](https://arxiv.org/abs/2606.11568)
*Seokju Cho,Abhishek Badki,Hang Su,Jindong Jiang,Ziyao Zeng,Seungryong Kim,Sifei Liu,Orazio Gallo*

Main category: cs.CV

TL;DR: VLMs struggle with 4D motion understanding; we introduce a QA pipeline that disentangles camera and object motion, generating a dataset that improves model performance.


<details>
  <summary>Details</summary>
Motivation: VLMs fail to grasp world dynamics due to indirect motion observation and entanglement of camera and object motion in existing datasets.

Method: We propose a QA generation pipeline that uses both traditional tracking and a novel True-Motion Tracking in a fixed reference system to decouple camera and object motion, creating a large-scale dataset 4DP-QA and benchmark 4DP-QA-Bench.

Result: Training on our dataset yields performance improvements on an external benchmark.

Conclusion: The effectiveness of our dataset and benchmark is validated by improved performance on an external benchmark after training.
...
### [32] [On Aligning Hierarchical Standardized Embedding for Audio-visual Generalized Zero-shot Learning](https://arxiv.org/abs/2606.11602)
*Zihan Zhang,Jie Hong,Siyuan Fan,Yanghao Zhou,Pengfei Fang*

Main category: cs.CV

TL;DR: 提出对齐标准化层次化嵌入（AHSE）方法，通过Z-score标准化和层次化对齐策略解决音频-视觉和文本模态间的分布与结构差异，提升广义零样本学习性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅依赖优化目标对齐音频-视觉与文本特征，忽略了模态间的分布和结构差异。

Method: 首先对融合的音频-视觉和文本嵌入进行Z-score标准化以减少分布不匹配，然后引入层次化对齐策略在语义、类别和批次级别最小化差异，构建更鲁棒、结构良好的嵌入空间。

Result: 在VGGSound-GZSL、UCF-GZSL和ActivityNet-GZSL三个基准数据集上取得竞争性能。

Conclusion: AHSE有效且能保留语义和类间关系，保持批次内空间一致性。
...
TL;DR: 提出Adv-TGD，一种基于Stable Diffusion的生成对抗攻击框架，通过LoRA微调和潜在混合实现高成功率人脸识别欺骗并保持高视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 面部识别技术的广泛使用带来隐私风险，面部数据可能被未经同意利用，需要有效的防御或攻击方法以保护隐私。

Method: Adv-TGD基于Stable Diffusion，对每个样本进行LoRA微调，结合文本提示生成对抗性人脸。采用单步去噪优化交叉注意力适配器，使用面局部热图掩码约束潜在混合，实现精确身份操纵。复合损失函数包括掩码重建、身份差异、方向对齐和源相似性抑制，可选LLaVA生成属性细节。

Result: 在黑盒评估中，在IR152、IRSE50、MobileFace和FaceNet上平均攻击成功率85.90%，超过多种基线；保持高视觉质量（PSNR=27.15dB, SSIM=0.981）。框架能扩展到不同数据集和任务。

Conclusion: Adv-TGD在保持高视觉真实性的同时实现了强大的攻击效能，且具有良好的灵活性和可扩展性，为隐私保护提供了有效工具。
...
### [34] [Ouroboros-Spatial: Closing the Data-Model Loop for Spatial Reasoning](https://arxiv.org/abs/2606.11719)
*Enhan Zhao,Wei Wu,Yuanrui Zhang,Xueliang Zhao,Di He*

Main category: cs.CV

TL;DR: Ouroboros-Spatial 是一种自我演变的训练框架，通过让模型作为提议者和求解器，根据当前能力自适应生成训练样本，显著提升了多模态大语言模型的空间推理能力，并大幅减少所需训练数据。


<details>
  <summary>Details</summary>
Motivation: 现有静态数据集将训练样本统一对待，不考虑模型能力演变，导致数据效率低下：大量算力浪费在模型当前阶段过于简单或困难的样本上。

Method: 在每次迭代中，冻结的提议者从3D场景元数据和视频帧生成空间问答对及可执行代码；可学习的求解器在接受的样本上微调，其预测置信度作为难度信号反馈给下一轮提议者，指导生成更匹配当前能力的题目，形成闭环自适应训练分布。

Result: 在六个空间推理基准上，Ouroboros-Spatial 仅用比大规模数据集少一个数量级的样本，显著提升 Qwen3-VL-4B 和 Qwen3-VL-8B 性能；在 VSI-Bench 上分别取得 9.9 和 6.8 的绝对增益，使两者超越多种开源和专有模型。

Conclusion: 所提出的闭环自适应训练框架通过动态匹配训练数据与模型能力，在空间推理任务上实现了高数据效率和强性能，验证了自我演变训练范式的有效性。
...
### [35] [Multi-View In-Cabin Monitoring System for Public Transport Vehicles](https://arxiv.org/abs/2606.11739)
*Evgeny Gorelik,Kenny Dean Karrow,Fikret Sivrikaya,Sahin Albayrak,Christian Baumann*

Main category: cs.CV

TL;DR: 本文介绍了一个用于公共交通的多视角车内监控数据集，包含RGB、深度图像和LiDAR数据，并提供标注、伪标注流水线和基准测试结果。


<details>
  <summary>Details</summary>
Motivation: 支持公共交通工具中多视角车内感知模型的开发和评估。

Method: 通过四个同步RGB-D相机和旋转LiDAR采集数据，设计校准和伪标注流水线生成3D姿态和边界框，并转换为nuScenes格式，在Lift-Splat-Shoot和BEVFusion等模型上基准测试。

Result: 数据集包含9136个同步样本，并提供完整的标注和工具。

Conclusion: 该数据集和工具为多视角车内感知研究提供了标准化资源和比较基准。
...
### [36] [A Comprehensive Ecosystem for Open-Domain Customized Video Generation](https://arxiv.org/abs/2606.11783)
*Jingxu Zhang,Yuqian Hong,Daneul Kim,Kai Qiu,Qi Dai,Jianmin Bao,Yifan Yang,Xiaoyan Sun,Chong Luo*

Main category: cs.CV

TL;DR: 提出百万级身份保持视频数据集PexelsCustom-1M和参数高效模型CustoMDiT，构建1000+类别基准OpenCustom，超越先前最优。


<details>
  <summary>Details</summary>
Motivation: 开放域定制化视频生成缺乏大规模、带注释的数据集，现有基准覆盖类别不足。

Method: 构建PexelsCustom-1M数据集（百万级<身份，文本，视频>三元组），提出CustoMDiT（基于扩散Transformer，仅增加8%参数），创建OpenCustom基准（融合ImageNet和MS-COCO）。

Result: CustoMDiT在多个指标上超越先前方法，数据集和模型均验证有效。

Conclusion: 将开源整个生态系统（数据集、管线、基准、实现）以推动研究。
...
### [37] [ViT-FREE: Efficient Face Recognition via Early Exiting and Synthetic Adaptation](https://arxiv.org/abs/2606.12023)
*Tahar Chettaoui,Guray Ozgur,Eduarda Caldeira,Naser Damer,Fadi Boutros*

Main category: cs.CV

TL;DR: 提出ViT-FREE和ViT-FREE_FT，利用早退策略在不重新训练ViT的情况下实现高效人脸识别，通过中间层出口平衡速度与精度。


<details>
  <summary>Details</summary>
Motivation: ViT计算成本高，难以部署在资源受限设备上，需要平衡效率与准确性的方法。

Method: ViT-FREE利用各编码器块特征维度一致，构建多出口框架直接进行人脸验证；ViT-FREE_FT仅用合成数据微调出口投影层，保持骨干冻结。

Result: 在IJB-C上，第10层退出实现20%加速，仅损失1.5个点性能；微调提升浅层性能，不影响深层。

Conclusion: 早退策略在ViT人脸识别中有效平衡效率与准确性，适合资源受限场景。
...
### [38] [Performance Analysis of YOLOv11 and YOLOv8 for Mixed Traffic Object Detection under Adverse Weather Conditions in Developing Countries](https://arxiv.org/abs/2606.12066)
*Quoc Thuan Nguyen,Ha Anh Vu,Ngo Dang Thanh Ngan,Minh Phuc Hoang Ngoc*

Main category: cs.CV

TL;DR: YOLOv11n在自动驾驶复杂场景下相比YOLOv8n提升精度3.2%，降低22% FLOPs，适合边缘部署。


<details>
  <summary>Details</summary>
Motivation: 评估YOLOv11 Nano在恶劣条件下自动驾驶的性能，特别是在密集交通、雨和低光场景中的精度和效率权衡。

Method: 在融合IDD和BDD100K的数据集上，以YOLOv8n为基线，对比YOLOv11n的检测精度、推理速度和计算效率。

Result: YOLOv11n实现mAP@50=46.6%，精度提升3.2%，FLOPs为6.3G（降低22%），Tesla T4上70.9 FPS。

Conclusion: YOLOv11n为安全关键边缘部署提供了精度与效率的优化平衡。
...
### [39] [Tac-DINO: Learning Vision-Tactile Features with Patch Alignment](https://arxiv.org/abs/2606.12069)
*Hong Li,Yankang Dong,Yue Xu,Yihan Tang,Mingzhu Li,Jiamin Qiu,Qihang Yao,Xing Zhu,Yujun Shen,Nan Xue,Yong-Lu Li*

Main category: cs.CV

TL;DR: 提出视觉-触觉块对齐（VTPA）方法，通过大规模触觉数据集和全息匹配基准，实现局部到全局的跨模态对齐。


<details>
  <summary>Details</summary>
Motivation: 触觉信号对应局部物体接触，但现有研究在尺度对齐和全息匹配方面有限，且缺乏合适的数据集和基准。

Method: 构建数据采集系统收集505个物体的20K+触觉接触数据，设计Vis-Tac全息匹配基准，并提出VTPA方法进行视觉-触觉表示学习。

Result: VTPA方法性能超过无对齐方法，并能与整个物体图像对齐。

Conclusion: 该数据集和基准可有效促进视觉-触觉局部到全局对齐，提升触觉学习效果。
...
### [40] [An Electric Potential-Augmented Benchmark Dataset for Physics-Guided Image Reconstruction of Electrical Capacitance Tomography](https://arxiv.org/abs/2606.12226)
*Xinqi Zhang,Qiming Ma,Lihui Peng*

Main category: cs.CV

TL;DR: 提出一个电势增强的ECT基准数据集，通过显式引入电势场信息提升图像重建的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法直接将电容映射到介电常数分布，忽略了作为物理基础的电势场，导致模型难以处理ECT的非线性和病态“软场”效应。

Method: 通过COMSOL-MATLAB管道为八电极传感器生成20,000个随机样本（四种流型），每个样本包含电容向量、介电常数分布图像和八次激励的全场电势图；并提供前向与逆向问题的评估协议。

Result: 在分布内和分布外场景下的系统测试表明，包含电势图显著增强了建模的准确性和鲁棒性。

Conclusion: 显式引入潜在场信息降低了物理规律融入ECT建模的门槛，为未来物理引导的机器学习ECT图像重建建立了标准化基础。
...
TL;DR: VOID通过操纵扩散模型的随机性来防御未经授权的模仿，通过放大潜在编码错误和对抗目标引导信号实现语义腐败，同时保持视觉效用。


<details>
  <summary>Details</summary>
Motivation: 现有防御依赖微小扰动在生成过程中保持欺骗性，但模型的恢复机制会移除扰动，导致身份重现。

Method: VOID通过两种方式扰动扩散管道：1) 放大潜在编码错误以破坏图像的语义结构；2) 对抗目标引导信号以抑制模型恢复能力。

Result: 在24种防御、10种攻击、5个数据集上，VOID将平均FID从113提高到365，比最强防御改进223%。

Conclusion: VOID提供了前所未有的保护能力，且安全增益不牺牲视觉效用。
...
TL;DR: VLGA通过将几何作为第四模态并监督密集3D重建，显著提升了VLA模型在自动驾驶中的3D接地能力，取得新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型难以在密集3D世界中接地动作，现有方法要么缺乏确保策略使用3D特征的损失，要么仅使用稀疏空间信号。

Method: 提出VLGA，第一个监督重建密集3D世界的VLA模型，引入几何作为第四模态，通过专用专家和基于LiDAR的逐像素点图回归损失进行监督。

Result: 在开放循环nuScenes上取得L2误差0.50m和碰撞率0.18%的SOTA；在闭环Bench2Drive上取得驾驶分数79.08，相比最强先验VLA提升0.71，效率与舒适度相当。

Conclusion: VLGA在多项基准测试中优于现有VLA方法，证明了密集3D重建监督对提升VLA模型空间接地能力的有效性。
...
### [43] [Traits Run Deeper: Trait-Specific Asymmetric Fusion for Personality Assessment](https://arxiv.org/abs/2606.11269)
*Jia Li,Qian Chen,Wei Wang,Xinyu Li,Zhenzhen Hu,Dongsheng Shao,Richang Hong,Meng Wang*

Main category: cs.CV

TL;DR: 提出Traits Run Deeper框架，通过多模态基础表示、特质特定模态融合和分布校准回归三个模块，在个性评估任务中降低MSE约25%，在挑战赛中排名第一。


<details>
  <summary>Details</summary>
Motivation: 现有方法对所有个性维度采用统一的多模态融合策略，忽略了维度特定的模态偏好，导致交叉模态干扰。

Method: 框架包括三个模块：多模态基础表示（MFR）模块构建个性导向的多模态输入并利用心理学语义模板；特质特定模态融合（TSMF）模块作为非对称融合机制，让每个维度选择性利用不同模态路径；分布校准个性回归（DCPR）模块通过目标分布校准缓解标签不平衡和中心倾向偏差。

Result: 在AVI Challenge 2026验证集上MSE降低约25%，在测试集上取得最佳性能并排名第一。

Conclusion: 提出的框架有效减少了交叉模态干扰和标签偏差，显著提升了个性评估的准确性和稳定性。
...
### [44] [LASA: A Weak Supervision Method for Open-Vocabulary Scene Sketch Semantic Segmentation](https://arxiv.org/abs/2606.11837)
*Liwen Yi,Xianlin Zhang,Yue Zhang,Yue Ming,Xueming Li*

Main category: cs.CV

TL;DR: 提出LASA框架，通过聚合Vision Transformer多层注意力图，为草图语义分割提供结构先验，弱监督下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 草图缺乏纹理和颜色，单层视觉语言特征不稳定；不同层注意力编码互补空间线索，跨层聚合可提供更稳健的结构先验。

Method: 基于LASA（层累积结构注意力），聚合多层注意力引导层次语义对齐，并在推理时细化预测。

Result: 在FS-COCO、SFSD和FrISS数据集上，mIoU分别提升+3.43、+8.01和+15.74。

Conclusion: 跨层注意力聚合有效提升弱监督草图分割的准确性和空间连贯性。
...
TL;DR: 提出SheafStain，通过层论视角解决虚拟染色中的上下文污染问题，实现空间连续且生物学一致的染色。


<details>
  <summary>Details</summary>
Motivation: 当前虚拟染色方法在补丁推理中导致空间不连续和伪影，病理基础模型的自注意力引起上下文污染，违反粘合公理。

Method: 将VFM特征视为层状截面，集成类令牌和补丁令牌到Schrödinger桥框架，使用在H&E和IHC上共同预训练的骨干网络以获得跨染色茎。

Result: 在HER2、ER、PR、Ki-67指标上，SheafStain在六种先前方法中表现优越，有效减轻了补丁边界缝合伪影。

Conclusion: SheafStain通过层论框架解决了上下文污染问题，实现了高质量虚拟染色，且代码将公开。
...
### [46] [ParseFixer: An Agentic Framework for Document Parsing via Selective Multimodal Correction](https://arxiv.org/abs/2606.11977)
*LeKai Yu,Hao Liu,Kun Wang,Zhiran Li,Ruping Cao,Fan Liu,Yupeng Hu*

Main category: cs.CV

TL;DR: 提出ParseFixer框架，结合全页骨干解析与智能选择性修正，在文档解析任务中取得第三名。


<details>
  <summary>Details</summary>
Motivation: 解决准确内容恢复与忠实结构重建的互补需求。

Method: ParseFixer包含全页骨干解析(FBP)和智能选择性修正(ASC)两个模块，FBP使用MinerU2.5 Pro生成初始Markdown，ASC检测并修复高价值解析失败。

Result: 在测试集上总体得分61.78，位列Track 1第三名。

Conclusion: 通过选择性多模态修正改进关键元素恢复，而不重写可靠骨干预测，证明了其有效性。
...
### [47] [SpikeTAD: Spiking Neural Networks for End-to-End Temporal Action Detection](https://arxiv.org/abs/2606.12033)
*Min Yang,Mi Zhou,Limin Wang*

Main category: cs.CV

TL;DR: 提出首个基于SNN的端到端时间动作检测架构SpikeTAD，在低功耗下实现有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解模型因体积大、功耗高难以部署，SNNs虽有低功耗优势但存在转换步长过长和性能退化问题，需探索其在时间动作检测中的应用。

Method: 提出SpikeTAD，首个基于SNN的端到端时间动作检测架构。

Result: 在THUMOS14上平均mAP为67.2%，在ActivityNet-1.3上为37.42%。

Conclusion: 证明了低功耗时间动作检测模型的可行性。
...
### [48] [CellNet -- Localizing Cells using Sparse and Noisy Point Annotations](https://arxiv.org/abs/2606.12286)
*Benjamin Eckhardt,Dmytro Fishman,Stuart Fawke,Andrew Curtis,Bo Fussing,Constantin Pape*

Main category: cs.CV

TL;DR: 开发基于回归的深度学习模型，仅使用稀疏点注释即可在相衬显微镜图像中计数活细胞，在低数据情况下优于零样本方法。


<details>
  <summary>Details</summary>
Motivation: 自动计数活细胞对于大规模基因组筛选至关重要，传统方法需要大量注释，成为瓶颈。

Method: 基于回归的深度学习算法，利用快速易获取的稀疏点注释进行细胞计数。

Result: 与最先进的零样本方法相比，回归方法在低数据情况下表现更优。

Conclusion: 为自动计数显微镜图像中活细胞提供有效方法，助力人类基因组研究。
...
### [49] [TRON: Tracing Rays to Orchestrate a Neural Renderer for 3D Gaussian Reconstructions](https://arxiv.org/abs/2606.11314)
*Or Perel,Hassan Abu Alhaija,Zian Wang,Jacob Munkberg,Matan Atzmon,Sanja Fidler,Masha Shugrina*

Main category: cs.CV

TL;DR: TRON结合3D高斯射线追踪与神经渲染，实现真实场景在新光照、动态物体、物体插入和材质编辑下的逼真可控渲染。


<details>
  <summary>Details</summary>
Motivation: 基于物理渲染的高斯方法因几何与材质估计不准确而无法实现真实重光照，神经渲染缺乏显式场景表示限制了交互编辑。TRON旨在结合二者优势，兼顾真实感与可控性。

Method: 利用逆渲染模型的内在分解先验正则化高斯场材质属性，用射线追踪提供辐射度量指导作为3D支架，输入轻量级神经渲染器弥合域差距。采用大规模预训练+2.1M帧数据集针对性微调的多阶段训练策略。

Result: 在真实感上优于高斯重光照方法，在可编辑性和速度上优于先前神经渲染器，首个在捕获3D场景中实现实用交互式应用。

Conclusion: 显式3D知识与神经渲染的结合有效实现了逼真且可控的场景渲染，为动态几何、光照和材质条件下的实时交互提供了可行方案。
...
### [50] [Semantic Segmentation of Node and Edge Diagrams for Assistive Technology](https://arxiv.org/abs/2606.11320)
*Michael Cormier,Yichun Zhao,Laura Paul,Cameron Swift,Duc Tri Dang,Miguel Nacenta*

Main category: cs.CV

TL;DR: 提出用于语义分割节点链接图的紧凑深度学习模型，在合成数据集上达到93%以上准确率。


<details>
  <summary>Details</summary>
Motivation: 节点链接图难以非视觉访问，现有辅助接口需要机器可读表示，而图通常以位图图像形式提供。

Method: 提出一组相关紧凑深度学习模型，用于语义分割节点链接图。

Result: 在大型合成数据集上达到93%以上的每像素准确率，定量和定性性能优秀。

Conclusion: 模型有效改善节点链接图的非视觉可访问性，具有实用潜力。
...
### [51] [DarkVGGT: Seeing Through Darkness Using Thermal Geometry without Daylight Tax](https://arxiv.org/abs/2606.11326)
*Minseong Kweon,Wenyuan Zhao,Nuo Chen,Lulin Liu,Huiwen Han,Zihao Zhu,Srinivas Shakkottai,Chao Tian,Zhiwen Fan*

Main category: cs.CV

TL;DR: 提出DarkVGGT，一个利用物理感知热建模的RGB-T前馈几何框架，用于低光照下的鲁棒3D估计。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D重建方法依赖可见光，在黑暗和低可见度环境中性能严重下降，需要结合热成像来增强鲁棒性。

Method: 包含两个模块：物理启发的热分解（提取发射主导的热线索并隔离反射残差）和几何共享热路由（隔离模态不变几何结构并选择性注入RGB流）。

Result: 在低可见度RGB-T基准上，深度和相机姿态估计相比现有基线取得一致改进。

Conclusion: DarkVGGT在低光照下有效提升几何估计，同时在良好光照下保持性能，具有实用价值。
...
### [52] [3D-CBM: A Framework for Concept-Based Interpretability in Generative 3D Modeling](https://arxiv.org/abs/2606.11446)
*Ahmad Al-Kabbany*

Main category: cs.CV

TL;DR: 将概念瓶颈模型引入3D生成架构，实现可解释的3D内容生成，并支持交互式修正。


<details>
  <summary>Details</summary>
Motivation: 解决深度几何学习中的语义差距，提高3D模型的可解释性和可信度。

Method: 提出3D-CBM架构，将原始几何输入映射到多层级可解释原语和功能属性，利用PartNet和ShapeNet进行概念监督。

Result: 概念预测准确率88.8%，Chamfer距离0.0115，支持测试时干预。

Conclusion: 为语义可引导的3D生成奠定基础，促进人机协同设计系统的发展。
...
### [53] [PT-WNO: Point Transformer with Wavelet Neural Operator for 3D Point Cloud Semantic Segmentation](https://arxiv.org/abs/2606.11466)
*Nhut Le,Maryam Rahnemoonfar*

Main category: cs.CV

TL;DR: 提出PT-WNO，通过在点云Transformer的跳跃连接中集成小波神经算子分支，增强全局上下文捕捉，提升了语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer网络主要依赖跳跃连接传递全局上下文，不足以实现完全的场景理解。

Method: 在编码器-解码器转换时，将点特征投影到3D网格，通过可学习小波分解和重建的Wavelet Neural Operator捕捉多尺度全局频谱上下文，并通过轻量适配器融合回网络，补充跳跃连接。

Result: 在S3DIS (Area 5) 上达到71.59% mIoU (+1.03), DALES上81.05% mIoU (+1.47), ScanNet v2上76.19% mIoU (与基线76.36%竞争)。

Conclusion: PT-WNO通过增强全局上下文有效提升了点云语义分割性能，获得了更丰富且上下文相关的表示。
...
### [54] [Towards Fully Automated Exam Grading: Fairness-Aware Recognition of Handwritten Answers with Foundation Models](https://arxiv.org/abs/2606.11477)
*Hartwig Grabowski*

Main category: cs.CV

TL;DR: 使用视觉语言基础模型实现高准确率和公平性的手写考试自动评分。


<details>
  <summary>Details</summary>
Motivation: 手工批改手写考试费时易错，现有自动化方法准确率低且对特殊情况处理不佳，需要一种既准确又公平的自动评分方法。

Method: 采用通用视觉语言基础模型（VLMs）解释页面，并通过轻量级提示提供参考解决方案上下文以降低假阴性率。

Result: 在61份匿名考试基准上，最佳模型准确率达98.4%，假阴性率降至0.58%，仅3份考试评分较差且可通过学生自评步骤纠正。

Conclusion: 全自动、公平性感知的大规模考试评分是可辩护的，并发布了匿名基准以支持可重复性。
...
TL;DR: 评估MobileNetV2、DenseNet-121、Inception-v3和STD在人脸反欺骗中的性能，MobileNetV2以92%准确率最有效，但需改进泛化能力。


<details>
  <summary>Details</summary>
Motivation: 生物识别系统易受欺骗攻击，需要评估最新机器学习模型在检测欺骗攻击中的有效性。

Method: 使用CelebA-Spoof数据集，通过准确率、精确率、召回率和F1分数评估MobileNetV2、DenseNet-121、Inception-v3和STD模型，并在MSU-MFSD数据集上进行跨数据集验证。

Result: MobileNetV2准确率最高（92%），计算效率高；Inception-v3表现中等；DenseNet-121和STD泛化能力不足。

Conclusion: 需要进一步发展领域自适应和混合架构以增强生物识别系统的安全性。
...
### [56] [VL-DINO: Leveraging CLIP Vision-Language Knowledge for Open-Vocabulary Object Detectio](https://arxiv.org/abs/2606.11546)
*Hao Zhang,Qinran Lin,Linqi Song,Yong Li*

Main category: cs.CV

TL;DR: VL-DINO通过利用CLIP的视觉语言知识增强DINO，实现了开放词汇检测的先进性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型如CLIP可以为开放词汇目标检测提供丰富的语义先验，但将文本和视觉知识同时整合到检测架构中仍然具有挑战性。

Method: 提出VL-DINO，包括查询引导的正样本构建（QPSC）模块以构造高质量正样本并提供更多视觉语言对齐信号，视觉语义编码器（VSE）模块将CLIP视觉知识蒸馏到骨干特征中，以及对象-区域语义对齐（ORSA）模块提取区域特征并与文本嵌入对齐。
...
### [57] [Understanding Cross-Sensor Feature Variations for Generalizable 3D Perception](https://arxiv.org/abs/2606.11573)
*Xin Qiu,Wenjie Liu,Fuyuan Ai,YuChen Tan,Zhiwei Xu,Chunyi Song*

Main category: cs.CV

TL;DR: 提出一种基于频域变体建模的框架，通过合成多样的源域视角并分析图像级变化对多模态BEV特征的影响，正则化检测器以提升跨数据集雷达相机3D检测的鲁棒性，仅训练时应用，无需目标域样本。


<details>
  <summary>Details</summary>
Motivation: 雷达相机BEV感知在跨数据集评估时性能下降严重，因驾驶场景、传感器配置和环境条件变化。现有方法依赖目标域样本，本文旨在不依赖目标域数据的情况下提升BEV检测器的鲁棒性。

Method: 框架首先在频域表征视觉场景变化，并据此合成多样的源域视角；然后比较融合后的BEV表示，捕捉图像级变化对多模态BEV特征的影响；最后利用这些变化模式正则化检测器，鼓励融合空间在潜在场景变化下保持稳定。该方法仅用于训练，推理流程不变。

Result: 在View-of-Delft和TJ4DRadSet之间的跨数据集雷达相机3D检测实验中，该方法在多个BEV融合骨干上持续提升性能，且当少量目标域数据可用时改善效果仍然保持。

Conclusion: 所提出的频域变体建模框架有效提升了雷达相机BEV检测器的跨数据集鲁棒性，且不依赖目标域样本，训练时应用推理不变。
...
### [58] [AVIS: Adaptive Test-Time Scaling for Vision-Language Models](https://arxiv.org/abs/2606.11576)
...
TL;DR: AVIS是一种自适应视觉推理缩放策略，通过联合优化视觉上下文缩放和推理滚动数量来提高VLM的准确率-计算权衡，且易于部署。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常单独优化视觉上下文缩放或推理缩放，忽略了联合分配计算，导致高昂的推理成本。

Method: AVIS通过关键多样性视觉(KDV)剪枝实现视觉上下文缩放，通过自适应自一致性实现推理缩放，并使用学习到的难度预测器选择推理滚动数量，兼容共享预填充推理。

Result: 在图像和视频推理基准上，AVIS改善了准确率-计算权衡，优于仅VCS或仅VRS基线，且在RL后训练的VLM上保持有效，同时保持低计算和低延迟。

Conclusion: AVIS是一种有效的轻量级策略，能够在保持低计算和低延迟的同时提升VLM的推理性能，易于部署。
...
### [59] [Frozen Foundation-Model Embeddings Discard Small-Lesion Signal in Chest Radiography: Implications for Pre-Deployment Evaluation](https://arxiv.org/abs/2606.11606)
...
TL;DR: 冻结ViT的全局池化会丢失小尺度信号，但补丁局部池化可以恢复。


<details>
  <summary>Details</summary>
Motivation: 尽管冻结ViT嵌入被广泛用于下游胸部X光成像任务，但小尺度低对比信号在冻结前向传递中是否保留或丢失尚未被系统量化。

Method: 探测五种冻结ViT（RAD-DINO, DINOv2-B/14, DINOv3 ViT-7B, BiomedCLIP, MedSigLIP）和冻结DINO预训练ResNet-50，在三个大型CXR数据集（NIH-CXR14, MIMIC-CXR, Emory-CXR, n=492,724）和ChestX-Det10（n=3,543，含1,462个小病灶边界框）上，通过小尺度扰动面板和区域感知边界盒分层探针，比较分类令牌、补丁均值和边界盒限制补丁局部三种池化模式。

Result: 在扰动面板上，CLS嵌入AUC仅为0.500-0.524（接近随机），补丁均值在全局任务上AUC为0.642-0.913，而补丁局部探针恢复AUC约1.0（平均提升+0.412至+0.488）。在ChestX-Det10上，CLS分类的类内小vs大病灶差距达+0.243 AUC，而补丁局部池化在每个（模型×类别）上AUC≥0.899。

Conclusion: 冻结ViT嵌入在全局聚合步骤中静默抑制小尺度信号，但该信号可从补丁令牌中通过感兴趣区域恢复。
...
### [60] [Precision-Aware Illumination-Disentangled Vision Transformer for Spacecraft 6D Pose Estimation](https://arxiv.org/abs/2606.11619)
*Zongwu Xie,Yifan Yang,Yonglong Zhang,Guanghu Xie,Yang Liu,Shuo Zhang*

Main category: cs.CV

TL;DR: 提出了一种精度感知的光照解耦视觉Transformer (PAID-ViT)，通过分离姿态相关结构和光照敏感外观、估计补丁可靠性、使用前景掩码监督和无参数几何恢复，提高单目航天器6D姿态估计在光照变化和背景干扰下的鲁棒性，在SPEED+ V2上减少了平移误差。


<details>
  <summary>Details</summary>
Motivation: 单目航天器6D姿态估计在光照变化、镜面反射、阴影、弱纹理和背景干扰下困难，局部视觉证据不可靠，导致姿态回归不稳定。

Method: 提出了PAID-ViT，包含光照解耦将姿态相关结构令牌与光照敏感外观令牌分离；可靠性感知的令牌聚合前估计补丁可靠性；使用前景掩码监督保留轮廓线索；以及无参数几何恢复模块将归一化坐标、对数深度和连续6D旋转表示转换为相机帧的旋转和平移。

Result: 在SPEED+ V2数据集上，PAID-ViT减少了平移误差，尤其在sunlamp域中提高鲁棒性；消融研究验证了各组件的有效性。

Conclusion: PAID-ViT通过解耦光照和可靠性感知聚合有效提升了姿态估计的鲁棒性和准确性。
...
### [61] [ARGUS: Stacked Multi-View Identity Mosaic Injection for Subject-Preserving Video Generation](https://arxiv.org/abs/2606.11670)
*Zijie Meng,Jiwen Liu,Yufei Liu,Chengzhuo Tong,Xiaoqiang Liu,Yuanxing Zhang,Yulong Xu,Pengfei Wan*

Main category: cs.CV

TL;DR: Argus introduces Stacked Multi-View Identity Mosaic Injection for subject-preserving video generation, achieving state-of-the-art performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Subject-preserving video generation struggles with identity consistency under large viewpoint, expression, and occlusion changes due to the point-reference paradigm that collapses identity into a single static observation.

Method: Argus uses Stacked Multi-View Identity Mosaic Injection (SMII) to convert MLLM-selected identity evidence into a 3*3 stacked mosaic, synchronized with diffusion time and injected as read-only memory, along with MLLM Identity Director, counterfactual training, Temporal Identity Annealing, and Adaptive Self-Likeness Guidance.

Result: Argus achieves state-of-the-art on OpenS2V-Eval Human-Domain with 64.38 Total Score, 71.86 FaceSim, 51.62 NexusScore, 79.14 NaturalScore; on HardID-Celeb, it obtains 76.80 FaceSim and improves YawScore and OccScore by 12.60 and 15.10 over baselines.

Conclusion: Dynamic identity memory and large-scale counterfactual self-supervision are highly effective for subject-preserving video generation.
...
### [62] [Parameter-Efficient Adapter Tuning for Tabular-Image Multimodal Learning](https://arxiv.org/abs/2606.11682)
*Jiaqi Luo*

Main category: cs.CV

TL;DR: We propose TI-Adapter, an adapter-based fine-tuning framework for efficient tabular-image multimodal learning. It freezes the tabular encoder and uses adapters in both branches, achieving competitive performance with far fewer trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Full fine-tuning is computationally expensive and frozen encoders limit task-specific adaptation.

Method: TI-Adapter freezes the pretrained tabular encoder and learns an adapter after tabular embedding, and adapts the image branch with embedding-level and bottleneck-level adapters instead of full fine-tuning.

Result: On 20 datasets, TI-Adapter achieves competitive or better predictive performance than full fine-tuning while using substantially fewer trainable parameters.

Conclusion: Ablation studies show the importance of adapter placement for balancing performance and practical efficiency.
...
### [63] [Reason, Then Re-reason: Cross-view Revisiting Improves Spatial Reasoning](https://arxiv.org/abs/2606.11683)
*Chaofan Ma,Zhenjie Mao,Yuhuan Yang,Fanqin Zeng,Yue Shi,Yingjie Zhou,Xiaofeng Cao,Jiangchao Yao*

Main category: cs.CV

TL;DR: ReRe 是一种无训练、推理时框架，通过从原始视频和合成新视角视频进行两阶段推理，提升了从自我中心视频进行空间推理的能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖单次推理，无法从新视角验证，导致几何歧义。

Method: Reason then Re-reason 框架：第一阶段MLLM从原始视频形成空间假设；第二阶段通过观察合成的全新视角视频验证或修正假设。使用Geometry-to-Video流水线从预测3D几何渲染互补视角。

Result: 在VSI-Bench和STI-Bench上显著提升开源MLLM，达到与专有SOTA相当的性能。

Conclusion: 空间推理应该是可重访的，ReRe通过两阶段推理有效提升了空间推理性能。
...
### [64] [ERN-Net : Evolving Reason Node-Net for Document Binarization](https://arxiv.org/abs/2606.11710)
*Hsin-Jui Pan,Sheng-Wei Chan,Jen-Shiung Chiang*

Main category: cs.CV

TL;DR: ERN-Net是一种利用进化推理节点和多尺度推理的高效文档图像二值化方法，采用ConvNeXt-Tiny作为主干网络并进行DIBCO预训练。


<details>
  <summary>Details</summary>
Motivation: 为了解决文档图像二值化中的退化敏感区域（如模糊笔画、断裂字符和噪声背景）问题。

Method: 提出了ERN-Net，通过进化推理节点和多尺度推理增强退化敏感区域；比较了ResNet-101、ConvNeXt-Tiny和ConvNeXt-Base，选择ConvNeXt-Tiny作为主干；采用基于DIBCO的预训练。

Result: 实验表明，ERN-Net在低数据和低内存设置下有效；ConvNeXt-Tiny在准确率和内存使用之间取得了最佳平衡；DIBCO预训练在不增加内存消耗的情况下提升性能，仅需额外1.5训练小时。

Conclusion: ERN-Net在低数据和低内存条件下表现出色，是一种实用的文档图像二值化解决方案。
...
### [65] [AnchorEdit: Maintaining Temporal Consistency in Multi-turn Image Editing via Causal Memory](https://arxiv.org/abs/2606.11751)
*Hang Xu,Xiaoxiao Ma,Guohui Zhang,Yu Hu,Siming Fu,Jie Huang,Lin Song,Haoyang Huang,Nan Duan,Feng Zhao*

Main category: cs.CV

TL;DR: AnchorEdit是一个自回归扩散框架，通过三阶段训练和记忆机制实现高分辨率、长期稳定的多轮图像编辑。


<details>
  <summary>Details</summary>
Motivation: 现有模型在多轮编辑中面临身份漂移和错误累积问题，且视频先验的双向注意力与交互编辑的因果顺序不匹配。

Method: 三阶段训练：身份保持单轮预训练、因果自回归强制微调结合自我展开策略、一致性蒸馏；推理时加入记忆机制锚定初始身份。

Result: 在长期多轮编辑中达到最新最优结果，能在10+轮交互中保持主体保真度和指令跟随。

Conclusion: AnchorEdit通过自回归框架和训练策略有效解决了多轮编辑的长期一致性问题。
...
### [66] [Battery detection of XRay images using transfer learning](https://arxiv.org/abs/2606.11779)
*Nermeen Abou Baker,David Rohrschneider,Uwe Handmann*

Main category: cs.CV

TL;DR: 使用YOLOv5m迁移学习检测和分类锂离子电池，精度达94%。


<details>
  <summary>Details</summary>
Motivation: 对检测和分类电池的需求日益增加。

Method: 采用预训练的YOLOv5m进行迁移学习，先在大型数据集上训练检测电子设备，再用训练好的权重检测和分类三种锂电池。

Result: 电池检测精度达94%，推理时间22ms，比预训练YOLOv5m权重高5%。

Conclusion: 本研究证明了迁移学习在电池检测和分类中的潜力。
...
### [67] [TextHOI-3D: Text-to-3D Hand-Object Interaction via Discrete Multi-View Generation and Joint Mesh Optimization](https://arxiv.org/abs/2606.11805)
*Zixiong Hao,Zhencun Jiang*

Main category: cs.CV

TL;DR: TextHOI-3D是一个分阶段框架，使用多视图视觉令牌作为中间表示，从文本生成手-物体网格，显著提高了几何精度和物理合理性。


<details>
  <summary>Details</summary>
Motivation: 文本条件3D生成对于图像和孤立物体进展迅速，但生成手-物体网格仍困难，需保持语言语义、跨视图一致性、物体几何、手部形状和物理合理接触。

Method: TextHOI-3D包含三个阶段：学习紧凑VQ令牌空间、用CLIP条件视觉自回归模型从文本预测多视图视觉令牌、通过先验初始化、多视图联合优化和抗穿透细化恢复统一手-物体网格。设计分离语义生成与几何恢复，并用离散多视图表示连接。

Result: 在HO3D衍生评估中，多视图设置相比单视图将物体CD从17.26mm降至4.92mm，穿透体积从5.3721 cm^3降至0.2193 cm^3，同时改进手部误差和表面F分数。

Conclusion: 多视图视觉令牌作为文本驱动3D手-物体网格创建的有效中间表示。
...
### [68] [Image Quality Assessment of Identity Cards Using Measures from Open Face Image Quality](https://arxiv.org/abs/2606.11884)
*Gregor Grote,Juan E. Tapia,Christian Rathgeb*

Main category: cs.CV

TL;DR: 将OFIQ标准应用于ID卡图像质量评估，可提高演示攻击检测性能。


<details>
  <summary>Details</summary>
Motivation: 远程验证系统中ID卡图像质量评估对演示攻击检测至关重要，但现有方法不足。

Method: 预处理包括角点检测、透视归一化和前景掩码，然后应用OFIQ标准中的捕获相关质量度量。

Result: 基于某些OFIQ度量的质量评估显著提高了三种PAD算法在四个数据集上的性能。

Conclusion: OFIQ质量度量可有效改进ID卡图像的PAD性能。
...
### [69] [Task-Aligned Stability Analysis of Vision-Language Models for Autonomous Driving Hazard Detection](https://arxiv.org/abs/2606.11889)
*Everett Richards*

Main category: cs.CV

TL;DR: 研究视觉语言模型在自动驾驶中的鲁棒性，发现嵌入漂移与危险分数变化的关系依赖于扰动类型，建议基准测试包括任务对齐的稳定性度量。


<details>
  <summary>Details</summary>
Motivation: 现有的鲁棒性分析通常只依赖于任务无关的嵌入稳定性，但本文研究嵌入漂移是否预测任务相关的危险分数变化。

Method: 使用BDD100K道路场景，应用受控扰动，比较嵌入漂移和边际漂移（危险分数的变化）。

Result: 关系高度依赖于扰动类型：某些扰动族表现出强耦合，而其他扰动族尽管嵌入变化小却导致决策不稳定；大多数扰动通过假阴性抑制危险检测，而遮挡触发假警报。

Conclusion: 鲁棒性基准应该包括任务对齐的稳定性度量，而不仅仅是嵌入级别的扰动统计。
...
### [70] [Wild3R: Feed-Forward 3D Gaussian Splatting from Unconstrained Sparse Photo Collection](https://arxiv.org/abs/2606.11894)
*Yuto Furutani,Takashi Otonari,Kaede Shiohara,Toshihiko Yamasaki*

Main category: cs.CV

TL;DR: Wild3R是一个前馈式3D高斯泼溅方法，利用新数据集WildCity处理无约束稀疏照片集合中的多样光照和瞬变物体，性能优于现有前馈方法，与逐场景优化方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有前馈方法在处理真实世界照片集合（包含多样光照和瞬变物体）时效果不佳，主要瓶颈是缺乏合适的训练数据。

Method: 提出WildCity数据集，包含200个场景、170种光照条件和瞬变物体，共337,500张图像；模型通过参考视图学习跨视角外观一致性并去除瞬变内容。

Result: 广泛实验表明，该方法优于现有前馈方法，并与先前的逐场景优化方法取得竞争性结果。

Conclusion: Wild3R成功解决了无约束稀疏照片集合的渲染问题，并证明了大规模多样化数据集对前馈3DGS的重要性。
...
### [71] [From Content to Knowledge: Lightning Fast Long-Video Understanding with Neural Knowledge Representations](https://arxiv.org/abs/2606.11913)
*Yuchen Guan,Xiao Li,Zongyu Guo,Xiaoyi Zhang,Xiulian Peng,Chun Yuan,Yan Lu*

Main category: cs.CV

TL;DR: 提出一种神经知识表示（NKR）的长视频理解新范式，将视频编码为轻量级网络权重，通过代理知识蒸馏获得，推理时挂载到冻结的VLM上，显著降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解方法推理成本高，难以高效支持多轮交互，需要解耦视频长度与推理成本。

Method: 通过代理知识蒸馏（AKD）自动合成密集描述和问答对，将视频语义蒸馏到少量网络权重（NKR）中，推理时挂载到冻结的VLM上进行查询。

Result: 在LVBench基准上性能与最先进方法相当，端到端延迟降低两个数量级以上。

Conclusion: 为高效交互式长视频理解开辟了新可能性。
...
### [72] [Feature extraction for plant growth estimation](https://arxiv.org/abs/2606.11966)
*Simbarashe Aldrin Ngorima,Albert Helberg,Marelie H. Davel*

Main category: cs.CV

TL;DR: 本文比较了两种实时植物生长阶段估计的特征提取方法：基于Gabor滤波器和基于CNN。CNN特征配合SVM在两种植物上达到98.4%的准确率，每张图像处理时间0.08秒。


<details>
  <summary>Details</summary>
Motivation: 精准农业需要实时估计植物生长阶段以减少资源浪费。不同生长阶段的植物形态相似，使得自主估计困难。

Method: 提出两种特征提取方法：一种使用Gabor滤波器组和形态学操作，另一种使用预训练CNN和迁移学习。使用支持向量机和提升树作为分类器，在公开数据集bccr-segset上对油菜和萝卜进行测试。

Result: 两种方法均适用于实时应用，CNN特征在速度和准确率上均优于手工特征。最佳系统(VGG-19特征+RBF SVM)在两种物种上均达到98.4%准确率，每张图像处理0.08秒。

Conclusion: 提出的方法有效，基于CNN的方法在实时植物生长阶段估计中表现优越。
...
### [73] [SpecLoR: Spectral Lookahead Rectification for Motion-Coherent Text-to-Video Generation](https://arxiv.org/abs/2606.11969)
*Xu Zhang,Yu Lu,Ruijie Quan,Zhaozheng Chen,Bohan Wang,Yi Yang*

Main category: cs.CV

TL;DR: 提出SpecLoR，一种即插即用的推理方法，通过频谱前瞻校正减少视频生成中的伪影并提升运动一致性，仅增加少量计算开销。


<details>
  <summary>Details</summary>
Motivation: Flow Matching中的速度近似和数值离散误差导致采样轨迹漂移，产生严重时空不一致；直接校正困难，因为噪声掩盖结构线索且空间校正成本高。

Method: 在早期采样阶段，前瞻预测估计干净潜在变量，计算其3D时空频谱，然后修正幅度谱以匹配自然视频统计先验，保持相位不变，最后重新加噪继续ODE积分。

Result: 在Wan2.2上显著减少物理伪影，增强运动一致性，额外计算量极小（4个额外NFE）。

Conclusion: SpecLoR是一种有效且高效的视频生成校正方法，可作为即插即用模块。
...
TL;DR: 提出路径模拟降雨可信度评估方法，支持自动驾驶感知测试。


<details>
  <summary>Details</summary>
Motivation: 模拟降雨条件对感知系统边界识别和SOTIF风险评估关键，但现有方法仅用名义强度或单点测量，难以对齐实际降雨和映射结果。

Method: 基于路径的可信度评估，以真实降雨雨滴谱为参考，用路径等效强度、不确定带和RRD评分表示路径，并用激光雷达点云和反射率校正感知一致性。

Result: 实验显示空间非均匀性需路径评估，路径IV和VI平衡性最优，参数分别为11.54±0.31 mm/h、RRD=0.43和8.28±0.34 mm/h、RRD=0.46。

Conclusion: 方法支持路径选择、条件描述和降雨下自动驾驶感知测试的可信解释。
...
### [75] [FitVTON: Fit-aware Virtual Try-On via Body-Garment Size Control](https://arxiv.org/abs/2606.12012)
*Yiqun Ning,Ao Shen,Chenhang He,Lei Zhang*

Main category: cs.CV

TL;DR: FitVTON是一种贴合感知的虚拟试穿模型，通过结构化文本提示和辅助掩码预测，从模拟数据学习以适应不同体型，显著提升尺寸准确性和形状保留。


<details>
  <summary>Details</summary>
Motivation: 现有扩散虚拟试穿方法视为2D修复，优先纹理保留而忽略物理合理性，导致生成图像不能真实反映服装在不同体型上的贴合效果。

Method: FitVTON通过结构化文本提示编码服装-身体尺寸，从参数化服装模型模拟的三元组学习；引入两个辅助头分别预测服装和暴露身体掩码，并采用纹理修正阶段改善模拟数据真实感。

Result: 在自建FittingEffect3K数据集上，结合VLM评分，FitVTON在尺寸准确性和形状保留上超越SOTA方法，同时保持有竞争力的图像质量。

Conclusion: FitVTON展示了真实的贴合保真度，验证了贴合感知虚拟试穿的有效性。
...
### [76] [Vision Transformers for Face Recognition Need More Registers](https://arxiv.org/abs/2606.12036)
*Tahar Chettaoui,Guray Ozgur,Eduarda Caldeira,Naser Damer,Fadi Boutros*

Main category: cs.CV

TL;DR: 提出在基于CPE的ViT人脸识别模型中引入寄存器标记，以消除注意力伪影并提高可解释性，同时保持SOTA识别性能。


<details>
  <summary>Details</summary>
Motivation: CPE方法改进了ViT人脸识别性能，但其注意力图中存在伪影，限制了可解释性。

Method: 在ViT中引入可学习的寄存器标记，与初始图像块嵌入拼接后联合处理，以生成更结构化的注意力图。

Result: ViT-8R（添加8个寄存器的CPE-based ViT-B）在IJB-B和IJB-C上取得ViT-based FR的SOTA，并产生清晰注意力图。

Conclusion: 寄存器标记有效缓解了CPE基线中的注意力伪影，显著提高可解释性，同时提升了识别准确率。
...
### [77] [Metadata-Aware Multi-Prompt Reasoning for Zero-Shot Accident Understanding](https://arxiv.org/abs/2606.12047)
*Tarandeep Singh,Soumyanetra Pal,Soham Biswas,Nishanth Chandran*

Main category: cs.CV

TL;DR: 提出一种三阶段流水线方法，用于零样本地从监控视频中理解事故，包括时间定位、语义分类和空间定位。


<details>
  <summary>Details</summary>
Motivation: 解决零样本事故理解问题，即从监控视频中理解何时发生撞击、何种类型撞击以及在哪里发生。

Method: 三阶段流水线：第一阶段使用视觉-语言相似性提取撞击周围的时间窗口；第二阶段进行元数据驱动的多提示推理，包括五种互补视图并通过熵门控成对仲裁器解决分歧；第三阶段使用开放词汇检测器定位撞击，基于预测的事故类型和场景布局，并通过分数加权质心聚合关键帧检测。

Result: 在零样本ACCIDENT @ CVPR基准测试上，谐波平均分数相比中心帧基线有显著改进。

Conclusion: 分解零样本视频理解为时间定位、语义分类和空间定位，可以比直接提示更可靠地进行推理。
...
### [78] [MFEN:Multi-Frequency Expert Network for Visible-Infrared Person Re-ID](https://arxiv.org/abs/2606.12051)
*Xulin Li,Yan Lu,Bin Liu,Qinhong Yang,Qi Chu,Tao Gong,Nenghai Yu*

Main category: cs.CV

TL;DR: 提出多频率专家网络（MFEN），结合随机频率增强和频率辅助优化，通过多频率调制和混合专家设计解决可见光-红外重识别中的模态差异问题。


<details>
  <summary>Details</summary>
Motivation: 可见光和红外图像间的模态差异主要与光照条件有关，现有频率方法要么不区分频带，要么只关注一个频带，无法应对多样光照条件。

Method: 提出多频率专家网络（MFEN），实现多频率调制并通过混合专家设计自适应组合不同频带；同时引入随机频率增强（RFA）和频率辅助优化（FAO）进行训练。

Result: 在三个VI-ReID数据集上的实验验证了方法的有效性。

Conclusion: 三个模块互补，联合捕获关键频率域细节，实现鲁棒表示学习。
...
TL;DR: 通过自蒸馏和强化学习，从预训练视频生成模型中引出任务解决能力，无需任务执行视频监督。


<details>
  <summary>Details</summary>
Motivation: 预训练视频生成器依赖详细文本描述，限制了其直接用于规划和决策；现有方法要么外包推理给其他模型，要么依赖昂贵且难以扩展的有监督微调。

Method: 使用视觉语言模型生成任务和逐步解决方案，然后以解决方案为条件预训练视频扩散模型（Demonstrator），通过自蒸馏将其行为转移到仅以图像和短任务提示为条件的Executor，并利用VLM反馈进行强化学习优化。

Result: 在WorldTasks-Benchmark和DreamGen机器人基准上，Executor在VLM评估协议下超越了Demonstrator，并在机器人任务中具有竞争力。

Conclusion: 该框架可扩展地引出视频生成模型的任务解决能力，无需任务视频监督，有效将执行知识从字幕引导生成转移到指令条件任务解决。
...
TL;DR: 本文提出一种结合轻量级生成对抗网络正面化与忆阻器神经形态计算的人脸识别框架，在资源受限平台上实现高达96%的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 深度学习人脸识别性能高但计算开销大，限制了在无人机等资源受限平台上的应用。

Method: 整合轻量级GAN姿态正面化与忆阻器类神经形态识别。

Result: 在两个数据集上达到高达96%的识别准确率。

Conclusion: 该方法缓解了计算瓶颈，为动态环境提供了可扩展且高效的解决方案。
...
TL;DR: 提出身份-槽对齐框架ISAP-3D，通过身份条件的一对一布局预测和布局条件的几何合成，实现稳定的部分感知3D生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法因身份-布局纠缠导致结构模糊，部分分配不稳定；归因于身份-槽置换自由，即没有显式的身份-槽对齐导致训练时对应关系不可识别。

Method: 提出ISAP-3D，用语义身份令牌锚定每个部分，进行身份条件的一对一布局预测，然后进行布局条件的几何合成，并构建具有统一语义协议的部分级数据集以学习一致性身份-槽对齐。

Result: 实验表明，在结构稳定性、可控性和鲁棒性方面优于现有最先进的部分感知生成基线。

Conclusion: 身份-槽对齐对于稳定的部分感知生成至关重要，ISAP-3D有效解决了结构模糊问题，实现了稳定分解。
...
TL;DR: A solution for the 2026 SoccerNet VQA Challenge using a VLM-driven data synthesis pipeline and a multi-expert QA architecture (MSUE) achieving third place with 0.95 accuracy.


<details>
  <summary>Details</summary>
Motivation: To develop a cost-effective and high-performance approach for the SoccerNet VQA Challenge.

Method: A VLM-driven data synthesis pipeline to generate diverse VQA samples, and MSUE: a multi-expert architecture using an LLM to dispatch questions to text (Gemini3-Flash), image (fine-tuned Qwen3-VL), and video (external knowledge base) experts.

Result: Achieved 0.95 accuracy and third place on the challenge benchmark.

Conclusion: The proposed data synthesis and multi-expert approach effectively enhances VQA performance for soccer videos.
...
### [83] [DynaTok: Token-Based 4D Reconstruction from Partial Point Clouds](https://arxiv.org/abs/2606.12189)
*Weirong Chen,Keisuke Tateno,Hidenobu Matsuki,Michael Niemeyer,Daniel Cremers,Federico Tombari*

Main category: cs.CV

TL;DR: 提出DynaTok，一个基于点云的框架，用于从部分点云序列中无需对应关系进行4D重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法多依赖图像，点云方法局限于单物体、完整输入或需对应关系，难以处理部分点云序列的4D重建。

Method: 将帧编码为紧凑潜在token，通过Transformer时空编码器聚合观测，用残差token解耦几何与运动，流匹配解码器重建完整4D点云。

Result: 在物体和场景级基准上，相比现有方法，重建质量和时间连贯性均有提升。

Conclusion: DynaTok无需对应关系和图像，有效实现从部分点云序列的4D重建。
...
### [84] [Damage-TriageFormer: A Foundation-Model Framework for Typology-Based Building Damage Assessment from Mono-Temporal Imagery](https://arxiv.org/abs/2606.12248)
*Yiming Xiao,Yu-Hsuan Ho,Sanjay Thasma,Junwei Ma,Ali Mostafavi*

Main category: cs.CV

TL;DR: 提出Damage-TriageFormer，利用单张事后图像进行建筑损坏类型评估，无需事前参考。


<details>
  <summary>Details</summary>
Motivation: 现有自动方法要么将损坏简化为单一严重性等级，要么需要配对的事前/事后图像，后者在紧急情况下常不可用。

Method: 基于DINOv3 ViT-L骨干网络，结合简单特征金字塔进行高分辨率实例池化，两阶段门控损坏头和辅助严重性回归目标；并构建包含五个损坏类型类的DamageTriage-Bench基准。

Result: 验证集宏F1=0.624，测试集宏F1=0.619；未损坏建筑和完全结构倒塌类别F1分别为0.91和0.84，但总屋顶损坏类效果较差。

Conclusion: 单图像事后图像可用于生成可操作的建筑损坏类型，无需事前参考，支持应急响应和资源分配。
...
### [85] [Bridging Day and Night: Unsupervised Cross-Domain Re-Identification with Synergistic Prompt and Prototype Learning](https://arxiv.org/abs/2606.12258)
*Jiyang Xu,Rui Liu,Hang Dai*

Main category: cs.CV

TL;DR: 提出一种无监督跨域昼夜行人重识别框架，结合提示学习和原型表示学习，无需标注即可达到与全监督方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有全监督方法依赖昂贵标注且泛化有限，无监督昼夜ReID尚未充分探索。

Method: 两阶段训练：第一阶段利用视觉语言模型生成实例提示，通过实例对齐和动态偏差适应统一视觉与文本嵌入；第二阶段构建域原型库，通过域内关联和跨域匹配模块建立身份对应。

Result: 在公共基准上，无监督设置下Rank-1准确率与最先进全监督方法相当。

Conclusion: 有效解决无监督昼夜ReID挑战，为跨域ReID提供新思路。
...
### [86] [From 2D Grids to 1D Tokens: Reforming Shared Representations for Multimodal Image Fusion](https://arxiv.org/abs/2606.12303)
*Yuchen Xian,Yunqiu Xu,Yang He,Yi Yang*

Main category: cs.CV

TL;DR: 多模态图像融合方法通过紧凑的1D token接口和选择性令牌编辑（STE）实现全局外观一致性和局部细节保留的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于2D特征网格的方法擅长建模局部结构但难以控制全局外观因素，因此需要平衡局部细节和全局一致性的新方法。

Method: 引入一个基于冻结预训练图像分词器的紧凑1D token接口作为全局外观/基因素载体，保留2D空间路径用于局部结构恢复，并通过选择性令牌编辑（STE）稀疏更新/替换关键令牌来引导全局外观一致性，避免改变融合骨干网络或引入额外损失。

Result: 在四个常用基准上实现了最佳整体性能，在全局一致性和局部保真度上均获得一致的多指标改进。

Conclusion: 该方法有效平衡了局部细节保留和全局外观一致性，为多模态图像融合提供了一种轻量且高效的解决方案。
...
### [87] [Anatomically Conditioned Recurrent Refinement for Topology-Aware Circle of Willis Segmentation](https://arxiv.org/abs/2606.12319)
*Juraj Perić,Marija Habijan,Dario Mužević,Irena Galić,Danilo Babin,Aleksandra Pižurica*

Main category: cs.CV

TL;DR: 提出了一种解剖条件递归细化 U-Net (AC2RUNet) 用于脑动脉环分割，显著降低了 Hausdorff 距离和 Betti 数误差，改善了拓扑连通性。


<details>
  <summary>Details</summary>
Motivation: 脑动脉环分割因复杂拓扑和薄血管结构易产生碎片化，标准 CNN 常导致“断血管”伪影，需要解决拓扑约束问题。

Method: 架构解耦为两个流：静态流提取不变解剖特征，动态流迭代细化拓扑错误；并引入动态课程学习策略，从高召回几何监督过渡到拓扑感知约束。

Result: 在 TopCoW 数据集上，Hausdorff 距离从 9.17 mm 降至 4.72 mm，Betti 数误差从 0.40 降至 0.19，拓扑连通性显著提升，同时保持与 nnU-Net 相当的体积 Dice。

Conclusion: AC2RUNet 能有效改善脑动脉环分割的拓扑连通性，减少血管断裂伪影。
...
### [88] [Atlas H&E-TME: Scalable AI-Based Tissue Profiling at Expert Pathologist-Level Accuracy](https://arxiv.org/abs/2606.12346)
...
TL;DR: Atlas H&E-TME基于病理基础模型，从H&E全切片图像中预测组织质量、区域和细胞类型，实现细胞级定量分析，并通过双重验证框架证明其性能匹配或超越病理学家。


<details>
  <summary>Details</summary>
Motivation: 克服H&E-only ground truth的形态模糊性及参考IHC等模态的可扩展性限制，实现可扩展的定量H&E分析。

Method: 基于Atlas病理基础模型构建，使用IHC-informed多病理学家共识协议进行深度验证，并在20万+高置信度H&E-only注释上进行广度基准测试。

Result: 在IHC-informed共识基准上，Atlas H&E-TME匹配或超过病理学家H&E-only性能，并在多种癌症类型和扫描仪上展现一致稳健泛化。

Conclusion: 该系统将H&E切片转化为定量窗口，为下一代组织生物标志物研究奠定基础。
...
### [89] [DepthMaster: Unified Monocular Depth Estimation for Perspective and Panoramic Images](https://arxiv.org/abs/2606.12368)
*Pengfei Wang,Shihao Wang,Liyi Chen,Zhiyuan Ma,Guowen Zhang,Lei Zhang*

Main category: cs.CV

TL;DR: DepthMaster 是一个统一的度量深度估计框架，通过将全景图像分解为重叠的透视补丁，并引入对应一致性损失和虚拟投影相机，实现了在窄FoV和360度全景上的零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 现有方法针对特定相机类型，难以泛化到不同相机类型，且存在几何差异和全景数据稀缺的挑战。

Method: 将全景图像分解为重叠的透视补丁，利用对应一致性损失和虚拟投影相机作为几何先验，无缝拼接补丁，避免专用网络，保持与标准Transformer的兼容性。

Result: 在13个多样数据集上达到零样本最先进性能，超越通用方法和领域专用模型。

Conclusion: DepthMaster 通过统一视角表示和数据利用策略，有效解决了单目度量深度估计的泛化问题。
...
### [90] [Illumination-Robust Camera-Based Heart-Rate Estimation for Physiological Sensing in Robots](https://arxiv.org/abs/2606.12378)
*Zhi Wei Xu,Torbjörn E. M. Nordling*

Main category: cs.CV

TL;DR: 提出一种用于光照变化下远程心率估计的端到端时空变换器框架，显著降低误差并提高相关性。


<details>
  <summary>Details</summary>
Motivation: 生理意识对服务、社交和辅助机器人至关重要；rPPG可非接触估计心率，但光照变化是主要障碍。

Method: 集成PRNet 3D人脸对齐、片段级光照增强、残差时间标准化模块和混合时频监督的变换器框架，使用Soft-Shifted Pearson损失和光谱KL散度损失。

Result: β=5时HR MAE为0.79 bpm，相关系数0.982；相比PhysFormer基线，MAE降低93.6%，相关系数从0.088提升至0.982。

Conclusion: 该方法在光照变化下实现准确心率估计，可用于机器人部署。
...
### [91] [How Seemingly Inconsequential Design Choices Dictate Performance of LLMs in Pathology](https://arxiv.org/abs/2606.12407)
...
### [92] [LAST: Bridging Vision-Language and Action Manifolds via Gromov-Wasserstein Alignment](https://arxiv.org/abs/2606.11221)
*Huaihai Lyu,Chaofan Chen,Yuheng Ji,Xiansheng Chen,Pengwei Wang,Shanghang Zhang,Changsheng Xu*

Main category: cs.CV

TL;DR: LAST通过李代数全局线性化和分层局部度量化，解决视觉-语言与动作空间的结构不匹配，提升VLA模型收敛性和泛化性。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言语义空间是线性各向同性的，而机器人动作流形是非欧几里得且各向异性的，直接回归难以对齐，导致模型性能差。

Method: 引入LAST，分两步：全局拓扑线性化利用李代数映射将动作流形线性化为固定长度表示；局部度量离散化将表示分层离散化为模式和残差，生成各向同性局部图表以对齐语义度量。

Result: 通过全局和局部结构匹配，LAST使VLA模型收敛更快且泛化更强。

Conclusion: LAST有效解决了VLA中动作与语义空间的结构不兼容，提升了模型性能。
...
### [93] [EventRadar: Long-Range Visual UAV Discovery through Spatiotemporal Event Sensing](https://arxiv.org/abs/2606.11285)
*Zhiting Zhou,Xingchen Liu,Xinglin Yu,Jiashen Chen,Haoyang Wang,Jingao Xu,Yunhao Liu,Xinlei Chen*

Main category: cs.CV

TL;DR: EventRadar利用事件相机检测无人机螺旋桨引起的时间周期性，结合IMU融合和谐波恢复算法，实现远距离高精度无人机检测。


<details>
  <summary>Details</summary>
Motivation: 现有无人机检测方法在远距离时因空间线索减弱而失效，需要一种利用时间周期性线索的方法。

Method: EventRadar使用事件相机，通过Scene-Anchored Geometry Evidence (SAGE)融合扫描事件与IMU姿态，构建场景记忆，再利用Comb-guided Harmonic-Group Learned Iterative Shrinkage and Thresholding Algorithm (CHG)恢复谐波证据。

Result: 在700-1500米距离上的无人机关联事件记录中，EventRadar达到0.990 mAP@0.3和0.949 F1@0.3，将FN@0.3降低至0.009，并展示了实时可行性。

Conclusion: EventRadar有效利用了时间周期性线索，在远距离无人机检测中表现优异，具有实时性。
...
### [94] [NSVQ: Mitigating Codebook Collapse by Stabilizing Encoder Drift in Vector Quantization](https://arxiv.org/abs/2606.11363)
*Hao Lu,Yongxin Guo,Onur Koyun,Zhengjie Zhu,Abbas Alili,Metin N. Gurcan*

Main category: cs.CV

TL;DR: 提出NSVQ，一种非平稳感知的VQ训练策略，通过密集非平稳嵌入损失、码本替换和分阶段编码器冻结，解决编码器漂移导致的码本崩溃问题，提高重建质量和码本利用率。


<details>
  <summary>Details</summary>
Motivation: 大码本VQ模型常因编码器漂移导致码本崩溃，量化误差增大，形成反馈循环，影响生成质量。

Method: NSVQ：结合密集非平稳嵌入损失帮助码本跟踪编码器漂移；分阶段先冻结编码器巩固码本，再引入对抗性细化。

Result: 在ImageNet-1k 128×128上，65,536码本时rFID从2.39降至2.10，保持100%利用率；下游扩散模型生成FID也改善。

Conclusion: NSVQ有效缓解码本崩溃，提升重建和生成质量，维持高码本利用率。
...
### [95] [A Scalable PyTorch Abstraction for Multi-GPU Gaussian Splatting](https://arxiv.org/abs/2606.11390)
*Matthew Cong,Francis Williams,Jonathan Swartz,Mark Harris,Sanja Fidler,Ken Museth*

Main category: cs.CV

TL;DR: 提出多GPU高斯泼溅方法，通过CUDA统一内存和NVLink实现分布，支持大规模高分辨率重建，城市规模场景超过10亿高斯泼溅。


<details>
  <summary>Details</summary>
Motivation: 现有高斯泼溅方法受限于计算和内存约束，难以扩展到高分辨率和大型场景。

Method: 提出PyTorch后端，利用CUDA统一内存和NVLink在GPU间分布高斯参数和泼溅操作符，操作符级别分布无需显式跨设备通信，多个GPU呈现为聚合PyTorch设备。

Result: 实现城市规模重建，包含超过10亿高斯泼溅，比当前最先进方法多25倍以上。

Conclusion: 该方法能有效扩展到更高分辨率和更大场景，简化分布式代码复杂性。
...
### [96] [Contactless 3D Human Body Measurement Using Depth Cameras for Smart Health Monitoring](https://arxiv.org/abs/2606.11578)
...
### [97] [Spatially Coupled Phase-to-Depth Calibration for Fringe Projection Profilometry](https://arxiv.org/abs/2606.11601)
*Sehoon Tak,Jae-Sang Hyun*

Main category: cs.CV

TL;DR: 提出空间耦合的相位-深度变换和原生网格配对方案，解决逐像素标定导致的空间不一致问题，在牙科目标上达到约12μm RMSE并改善一致性。


<details>
  <summary>Details</summary>
Motivation: 逐像素标定虽然局部精度高，但相邻像素获得不同标定函数，导致空间不一致的几何形状和表面伪影。

Method: 所有像素共享全局相位标量结合仿射空间项的低维映射，可选加空间平滑校正场；并采用原生网格配对方案，在参考相机网格上直接构建相位-深度对，避免相位图矫正。

Result: 点对面RMSE约12μm（与主动立体参考相当），空间一致性显著优于逐像素多项式/有理标定，运行时映射仅需每像素几个元素级操作，参数存储可忽略。

Conclusion: 所提方法在保持高精度的同时，显著提高了空间一致性，并降低了计算和存储开销。
...
### [98] [Learning Instance-Adaptive Low-Rank Orthogonal Subspaces for Clothes-Changing Person Re-Identification](https://arxiv.org/abs/2606.11661)
*Dong-Woo Kim,Tae-Kyun Kim*

Main category: cs.CV

TL;DR: 提出Ortho-ReID，通过显式建模低秩服装子空间和正交几何约束提取服装不变表示，在多个基准上取得新最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖对抗学习解耦服装特征，训练不稳定且效率低，而正交几何约束可直接且稳定地分离服装与身份特征。

Method: 利用VLM文本描述构建共享低秩服装先验，通过Transformer Basis Maker与图像块交叉注意力生成实例自适应低秩子空间；身份投影头提取的特征通过正交损失与服装子空间严格正交。

Result: 在PRCC上top-1提升5.9%，Celeb-reID-light提升3.5%，LaST提升5.3%，并在LTCC上取得有竞争力的结果。

Conclusion: 显式几何约束替代对抗学习有效提升了CC-ReID性能，且模型训练稳定、泛化性强。
...
### [99] [From Prompts to Tokens: Internalizing Causal Supervision in Vision-Language Model for Multi-Image Causal Reasoning](https://arxiv.org/abs/2606.11745)
*Haoping Yu,Yuanxi Li,Jing Ma*

Main category: cs.CV

TL;DR: 提出了BridgeVLM，通过将多图像输入诱导为因果图并转换为因果标记，在LLM解码器中执行因果消息传递，显著提升了视觉因果推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLMs在视觉因果推理任务上表现脆弱，尤其是在多图像干预和反事实查询上，且因果知识注入方式将机制外置于模型执行，限制了可靠控制。

Method: BridgeVLM从多图像输入中学习因果图，将其转换为结构化的因果标记，由注入LLM解码器的RAMP层进行因果消息传递；并引入统一训练接口M3S提供不同粒度的因果监督。

Result: 在CausalVLBench干预任务上准确率从33.2%提升至54.4%，在Causal3D上从43.6%提升至49.0%，因果结构学习F1从33.4%提升至75.1%。

Conclusion: BridgeVLM通过内部化因果推理机制，有效提升了视觉语言模型在因果推理任务上的表现，尤其在干预和结构学习方面取得了显著改进。
...
### [100] [Seeing What Matters: Perceptual Wrapper with Common Randomness for 3D Gaussian Splatting](https://arxiv.org/abs/2606.11782)
*He-Bi Yang,Jing-Zhong Chen,Yen-Kuan Ho,Sang NguyenQuang,Fan-Yi Hsu,Yun-Yu Lee,Jui-Chiu Chiang,Wen-Hsiao Peng*

Main category: cs.CV

TL;DR: 提出一种2D感知包装器，通过轻量级合成网络学习局部特征统计，增强3DGS渲染的高频纹理，提升感知质量并减小模型大小。


<details>
  <summary>Details</summary>
Motivation: 3DGS在内存受限和率失真优化管道中难以合成高频纹理，导致模糊。

Method: 使用条件于伪随机高斯噪声的轻量级合成网络，通过Wasserstein距离监督学习局部特征统计，而非像素级重建。

Result: 在多种3DGS方法上，感知质量显著提升，文件或模型大小大幅减小。

Conclusion: 该即插即用方法广泛适用于标准、内存受限和RDO 3DGS，实现更优的感知质量与紧凑表示。
...
### [101] [MultiToP: Learning to Patch Visual Tokens to Mitigate Hallucinations in Video Large Multimodal Models](https://arxiv.org/abs/2606.11792)
*Yuansheng Gao,Wenbin Xing,Jiahao Yuan,Kaiwen Zhou,Han Bao,Zonghui Wang,Wenzhi Chen*

Main category: cs.CV

TL;DR: 提出MultiToP框架，通过视觉标记修补减少视频大语言模型中的幻觉，在Vript-HAL上F1提升50.60%，并在ActivityNet-QA上保持泛化能力。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解中存在幻觉问题，生成的响应缺乏对输入视频的忠实支持。

Method: MultiToP包括一个轻量级Visual Token Patcher，预测标记替换分布，并用动态全局补丁标记替换不可靠视觉标记；同时使用信息引导的排名校准，结合真值答案监督和稀疏正则化来训练修补器，实现局部视觉证据细化而不修改原始模型。

Result: 在Vript-HAL上，Qwen3-VL-4B-Instruct的F1分数提升50.60%；在ActivityNet-QA上，Video-LLaVA-7B的相对准确率提升18.58%，并保持推理开销可忽略。

Conclusion: MultiToP有效减少幻觉，同时保持通用视频理解能力，为缓解视频大模型幻觉提供了有效方案。
...
### [102] [Plan-and-Verify Video Reward Reasoning with Spatio-Temporal Scene Graph Grounding](https://arxiv.org/abs/2606.11838)
*Hyomin Kim,Junghye Kim,Joanie Hayoun Chung,Yoonjin Oh,Kyungjae Lee,Sungbin Lim,Sungwoong Kim*

Main category: cs.CV

TL;DR: SG-PVR是一种通过计划-验证推理和时空场景图来改进文本到视频生成中语义对齐的视频奖励模型。


<details>
  <summary>Details</summary>
Motivation: 现有基于推理的奖励模型存在两个结构弱点：不系统验证提示中的每个条件，以及自由形式推理中视觉证据隐式。这导致细粒度语义对齐失败。

Method: 提出SG-PVR，通过计划-验证推理和时空场景图。首先将提示分解为原子声明，确保每个要求都被检查。然后从视频中提取编码实体、属性和时序关系的时空场景图，作为持久结构化视觉参考。每个声明同时根据视频和场景图验证，将判断锚定在显式视觉证据上。

Result: SG-PVR在语义对齐上表现强劲，包括细粒度时间语义。作为测试时重排器，它进一步增强了T2V生成中的组合对齐。

Conclusion: SG-PVR通过结构化验证和显式视觉参考有效解决了语义对齐问题，提升了视频奖励模型的性能。
...
### [103] [Time-Conditioned and Multi-Time Survival Prediction from 2D PET/CT Projections in Lung Cancer](https://arxiv.org/abs/2606.12140)
...
### [104] [Beyond Dark Knowledge: Mixup-Based Distillation for Reliable Predictions](https://arxiv.org/abs/2606.12171)
*José Medina,Paul Honeine,Abdelaziz Bensrhair,Amnir Hadachi*

Main category: cs.CV

TL;DR: 研究发现，在仅对学生使用mixup的知识蒸馏中，教师信号被分布混淆主导，但学生独立获得线性性，mixup蒸馏是比标准KD更丰富的传输渠道，同时提升准确率、降低过度自信，并影响校准和表征几何。


<details>
  <summary>Details</summary>
Motivation: KD和mixup的交互作用理解不足，特别是mixup仅用于学生训练时，教师被从未见过的邻域分布查询，这种不匹配对知识转移的影响尚未被表征。

Method: 通过实验观察到教师信号被分布混淆主导，但学生独立获得更大的线性性，超越了暗知识转移；使用温度缩放研究准确率-校准权衡。

Result: KD与mixup一致提高学生准确率并降低过度自信一个数量级；校准独立于准确率传递；温度缩放控制可测量的准确率-校准权衡，在邻域训练下更明显。

Conclusion: mixup蒸馏不仅仅是KD的降级，而是更丰富的传输渠道，同时塑造判别性能、不确定性估计和表征几何。
...
### [105] [SHERPA: Seam-aware Harmonized ERP Adaptation for Open-Domain 360$^\circ$ Panorama Generation](https://arxiv.org/abs/2606.12213)
*Jungwoon Kang,Jaehun Kim,Yiwon Yu,Hyungyum Jang,Sanghoon Lee,Jongyoo Kim*

Main category: cs.CV

TL;DR: SHERPA adapts text-to-image diffusion models for 360° panoramic generation using frequency-selective Circular RoPE and dual-path training.


<details>
  <summary>Details</summary>
Motivation: Large-scale text-to-image models provide broad style and semantic priors but are misaligned with the wrap-around topology and polar regions of equirectangular projection panoramas.

Method: SHERPA combines frequency-selective Circular RoPE, Circular Latent Encoding/Decoding, image-side FFN adapters, and a Dual-Path Training Scheme with Paired Panorama Path for geometry supervision and Unpaired Style Path for self-supervised yaw consistency.

Result: SHERPA generates 360° panoramas across photorealistic panorama domains and open-domain stylized prompts.

Conclusion: SHERPA effectively adapts planar text-to-image models to generate both photorealistic and stylized 360° panoramas.
...
### [106] [Making Foresight Actionable: Repurposing Representation Alignment in World Action Models](https://arxiv.org/abs/2606.12217)
*Lu Qiu,Yizhuo Li,Yi Chen,Yuying Ge,Yixiao Ge,Xihui Liu*

Main category: cs.CV

TL;DR: 提出AGRA，通过对齐视频扩散特征与语义表示，解决世界动作模型中视觉重建与动作控制之间的表示不匹配问题，提升动作接地性。


<details>
  <summary>Details</summary>
Motivation: 世界动作模型生成逼真的视觉未来但无法保证准确提取动作，原因是动作解码器未关注任务相关区域，且隐藏状态优化目标与动作控制不匹配。

Method: AGRA：动作接地表示对齐目标，正则化世界动作接口，将中间视频扩散特征与基础视觉编码器的空间相干语义表示对齐。

Result: 在真实世界操作任务中，AGRA提升对象定位准确性和可操作性理解，增强策略对任务无关扰动的鲁棒性，改进分布内性能和分布外泛化。

Conclusion: AGRA使世界模型表示更动作接地，通过聚焦正确交互区域，持续提升策略性能。
...
TL;DR: 利用Lite ViT-Adapter和参数高效微调，在Prithvi-EO上实现高精度休耕地检测，mAP@50达0.9479。


<details>
  <summary>Details</summary>
Motivation: 休耕地检测对优化食物-水关系至关重要，但现有ViT骨干的单尺度特征不适用于目标检测，全微调成本高。

Method: 结合LoRA和混合PEFT，比较伪多尺度、Lite ViT-Adapter和Full ViT-Adapter颈部设计，最佳配置为Lite ViT-Adapter加单阶段检测头和Diou损失。

Result: 最佳配置mAP@50为0.9479，相比基线adapter-free方法提升25.70%，LoRA下ViT-Adapter-free方法提升6.42%。

Conclusion: 轻量级空间先验融合与选择性骨干解冻使Prithvi-EO更有效捕捉局部休耕模式，优于重塑单步长ViT令牌的方法。
...
### [108] [Finding Sparse Subnetworks in One Training Cycle via Progressive Magnitude-Based Pruning](https://arxiv.org/abs/2606.12278)
*Romana Qureshi,Hafida Benhidour,Said Kerrache,Nahlah Aljeraisy*

Main category: cs.CV

TL;DR: 提出渐进幅度剪枝作为单周期剪枝方法，在多种架构和数据集上优于LTH、SNIP等基线。


<details>
  <summary>Details</summary>
Motivation: 解决彩票假说需要多次训练周期的问题，寻求单周期替代方案以实现高效剪枝。

Method: 在训练过程中按线性计划逐渐增加稀疏度，基于活跃权重幅度更新剪枝掩码。

Result: 在CIFAR-10上ResNet-18在72.9%稀疏度达到95.12%精度；极端稀疏度下VGG-like在97%稀疏度达到93.13%（优于SNIP约92.0%），VGG-19在97.97%稀疏度达到93.44%（优于GraSP约92.19%）；ResNet-18在70-85%稀疏度下精度与密集基线相差0.1个百分点以内。

Conclusion: 渐进幅度剪枝在评估设置下是一种有效的单周期网络稀疏化方法。
...
### [109] [Slots, Transitions, Loops: Learning Composable World Models for ARC](https://arxiv.org/abs/2606.12316)
*Gege Gao,Bernhard Schölkopf,Andreas Geiger*

Main category: cs.CV

TL;DR: Loop-OWM是一个对象中心的世界建模架构，通过循环转换模型学习ARC视觉符号规则，在ARC-1和ARC-2上优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法将ARC规则表达为语言、代码或符号程序，但ARC本身是视觉符号的，因此需要一种能够直接学习视觉符号规则的方法。

Method: Loop-OWM结合颜色原型插槽、演示条件任务摘要以及带有密集传播和插槽条件校正的循环转换模型来学习可组合的状态转换。

Result: 在ARC-1和ARC-2上，Loop-OWM以更少或相当的参数数量优于非循环和循环基线。

Conclusion: ARC规则可以作为视觉符号世界状态上的转换来学习，而不仅仅是语言描述或搜索程序。
...
### [110] [Echoes of the Prior: A Computational Phenomenology of Forgetting](https://arxiv.org/abs/2606.12340)
*Gege Gao,Bernhard Schölkopf,Andreas Geiger*

Main category: cs.CV

TL;DR: Echoes of the Prior is an interactive installation that visualizes the subjective phenomenology of forgetting by inducing controlled synaptic decay in a Feed-Forward 3D Reconstruction model.


<details>
  <summary>Details</summary>
Motivation: Memory is the scaffolding of reality; its fading leads to unrecognizable chaos. The work aims to visualize this phenomenology and explore neuromorphic aesthetics in representing the fragility of intelligence.

Method: Inducing controlled synaptic decay within a Feed-Forward 3D Reconstruction model as an artistic analogy for the erosion of the brain's predictive priors, positioning the neural network as a cognitive proxy.

Result: An interactive installation whose structural degeneration evokes the disorienting, poetic, and terrifying experience of losing one's grip on the world.

Conclusion: Offer this framework as a catalyst for the wider community to explore the uncharted potential of neuromorphic aesthetics in visualizing the fragility of intelligence.
...
### [111] [An Ontology-Guided Multi-Anchor Graph Retrieval Framework for Traffic Legal Liability Determination](https://arxiv.org/abs/2606.11910)
*Xu Li,Shuqi Tian,Xun Han,Kuncheng Zhao,Xinyi Li*

Main category: cs.CL

TL;DR: 本文提出OMAGR，一种本体引导的并行多锚点检索框架，用于解决交通法责任认定中的多维度检索瓶颈，在TrafficLaw-QA数据集上提升了上下文精确度和忠实度。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法因单轴架构而忽略多维度相互依赖的法定条款，导致检索瓶颈。

Method: OMAGR通过本体引导将查询分解为多个锚点，并在每个维度上执行并行图检索，最后融合结果。

Result: 在TrafficLaw-QA数据集上，TrafficOmni-RAG在上下文精确度和忠实度指标上优于基线。

Conclusion: 并行多锚点检索有效解决了多维度检索瓶颈，为交通法责任认定研究提供了新方向。
...
TL;DR: RAG系统中，知识图谱三元组的格式结构会额外吸引模型注意力，导致演示注意力被压缩，提出解耦语义和结构轴的两阶段改进方法，并验证了多种缓解策略。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统仅关注注入内容的语义相关性，但格式本身会独立扭曲注意力分布，造成结构注意力税现象。

Method: 通过形式化框架将注意力分数分解为语义和结构成分（Eq.2），推导注意力压缩界的理论命题（Proposition 1），并在Mistral-7B和LLaMA-3-8B模型上针对HotpotQA等基准进行实验，比较BM25检索与ConceptNet等对比。

Result: 知识图谱三元组每token获得的注意力比自然语言高2-3倍（0.70 vs 0.25），压缩演示注意力达42%。任务匹配的BM25检索达到58-62%准确率，而ConceptNet仅25-27%，差距超过30个百分点，远大于各种门控策略的影响（≤2个百分点）。

Conclusion: 检索增强ICL存在正交的两个改进轴：语义轴优化检索质量，结构轴减少格式驱动的注意力捕获。由此衍生五种结构感知缓解策略，其中格式扁平化（S3）经实验验证有效，而结构分散（S1）效果不一，揭示了格式层面干预的挑战。
...
### [113] [uva-irlab-conv at SemEval-2026 Task 8: Multi-Turn RAG with Learned Sparse Retrieval and Listwise Reranking](https://arxiv.org/abs/2606.11945)
*Simon Lupart,Kidist Amde Mekonnen,Zahra Abbasiantaeb,Mohammad Aliannejadi*

Main category: cs.CL

TL;DR: 提出多轮检索增强生成管道，结合稀疏检索与LLM重排序和生成，用于多领域会话问答。


<details>
  <summary>Details</summary>
Motivation: 解决多轮检索问答任务中的跨领域泛化和不可回答查询问题，提升系统鲁棒性。

Method: 采用学习型稀疏检索作为主要检索方法，结合LLM进行查询重写、点wise和listwise重排序以及最终响应生成，每一步均利用完整会话历史。

Result: 通过多步设计有效整合会话上下文，提高了跨领域鲁棒性。

Conclusion: 多步骤RAG管道能有效处理多轮对话中的检索和生成，尤其适用于多领域场景。
...
### [114] [When More Documents Hurt RAG: Mitigating Vector Search Dilution with Domain-Scoped, Model-Agnostic Retrieval](https://arxiv.org/abs/2606.11350)
*Nabaraj Subedi,Ahmed Abdelaty,Shivanand Venkanna Sheshappanavar*

Main category: cs.CL

TL;DR: MASDR-RAG通过域范围划分显著提升检索增强生成在大型异构文档集合中的性能，并揭示了精度-忠实度悖论。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成在大型异构文档集合中因向量搜索稀释导致性能严重下降，例如在怀俄明州交通运输部语料库中，文档数量从54增至1128时准确率从75%降至40%以下。

Method: 提出MASDR-RAG（Multi-Agent Scoped Domain Retrieval for RAG），利用组织元数据进行域范围划分，并结合多代理编排进行检索与生成。

Result: 域范围划分将P@10从0.77提升至0.86（p<0.05）；多代理编排展现出高度的配置依赖性，并产生精度-忠实度悖论。

Conclusion: 实际建议优先进行域范围划分，然后执行单次合成调用；仅在多领域语料库且使用原生工具调用后端时才采用多代理编排。
...
### [115] [APEX: Automated Prompt Engineering eXpert with Dynamic Data Selection](https://arxiv.org/abs/2606.11459)
*Fei Wang,Si Si,Cho-Jui Hsieh,Inderjit S. Dhillon*

Main category: cs.CL

TL;DR: APEX通过动态数据分层和优先级处理混合层来提升提示优化的数据效率。


<details>
  <summary>Details</summary>
Motivation: 当前进化算法优化提示时数据效率低，将开发数据集视为静态基准，浪费计算预算。

Method: APEX框架动态将数据集分层为Easy、Hard和Mixed，优先Mixed层，并识别可寻址边界和排名敏感边界以生成突变和区分候选质量。

Result: 在IFBench、SimpleQA Verified和FACTS Grounding基准上，固定5000次评估调用，APEX在Gemini 2.5 Flash上平均提升11.2%，在Gemma 3 27B上提升6.8%。

Conclusion: 以数据为中心的方法是高效有效提示优化的关键。
...
### [116] [EverydayGPT: Confidence-Gated Routing for Efficient and Safe Hybrid GPT-RAG Conversational QA](https://arxiv.org/abs/2606.11212)
*Jaspreet Singh Nahal*

Main category: cs.CL

TL;DR: EverydayGPT通过置信门控路由机制，对85%的查询使用快速RAG提取，大幅降低延迟的同时保持答案质量。


<details>
  <summary>Details</summary>
Motivation: 标准RAG无条件地对每个查询进行检索和生成，导致不必要的计算和低质量上下文。

Method: 提出Confidence-Gated Routing (CGR)机制，将路由决策形式化为检索距离和提取充分性的联合策略；骨干为在FineWeb-Edu上训练的205M参数GPT。

Result: 在500问题基准上F1=0.226，优于GPT-only的0.171和无条件RAG的0.210；平均延迟降低6.3倍，主流查询延迟降低120倍以上；结构化审计未发现无根据声明。

Conclusion: 本工作是资源约束下路由策略的研究，而非声称最先进性能。
...
### [117] [Soft-Prompt Tuning for Fair and Efficient LLM Benchmark Evaluation](https://arxiv.org/abs/2606.12117)
*Selen Erkan,Bastian Boll,Kristian Kersting,Björn Deiseroth,Letitia Parcalabescu*

Main category: cs.CL

TL;DR: 提出软提示调优（soft-prompt tuning）作为LLM知识评估的公平方法，通过优化少量软提示向量使模型适应基准格式，从而更准确反映基础模型的知识，无需完整后训练。


<details>
  <summary>Details</summary>
Motivation: 基准测试分数因格式遵循要求而误判LLM知识，尤其惩罚基础模型；现有评估未能有效区分格式遵循与知识准确性。

Method: 使用软提示调优，仅优化10个软提示向量（0.0006%参数）在80步内，使模型适应基准测试格式，并在7个模型和7个数据集上评估。

Result: (a) 软提示调优在80步内饱和格式遵循；(b) 显著优于零样本和少样本提示；(c) 后训练模型也能受益；(d) 软提示基座模型性能更可靠地预测后训练模型排名。

Conclusion: 贡献包括：区分格式遵循和知识准确度的指标、更公平的LLM知识基准协议、以及成本有效的早期预训练策略选择方法。
...
### [118] [NightFeats @ MMU-RAGent NeurIPS 2025: A Context-Optimized Multi-Agent RAG System for the Text-to-Text Track](https://arxiv.org/abs/2606.11199)
*Quentin Fever,Naziha Aslam*

Main category: cs.CL

TL;DR: NightFeats是一个结构化多智能体检索增强生成系统，通过分解知识合成过程并引入核心原语，超越专有基线。


<details>
  <summary>Details</summary>
Motivation: 设计一个基于原则的管道，而不是追求基准最大化，以实现架构透明和可验证的证据基础，更符合人类偏好。

Method: 将知识合成分解为检索、策展和组合三个阶段，引入时间语义重排序、有界矛盾调和和引用保留组合作为核心原语。

Result: 在MMU-RAGent竞赛中获得最佳动态评估，超越Claude-SonnetV2和Nova-Pro。

Conclusion: 架构透明和可验证的证据基础比优化自动相似度指标的系统更符合人类偏好。
...
TL;DR: 介绍BioDivergence，一个评估生物医学声明上下文分歧的框架，包含六类冲突分类、13轴本体和结构化输出，并发布相关基准。


<details>
  <summary>Details</summary>
Motivation: 现有NLI和科学声明验证基准将上下文相关的分歧简化为蕴含、矛盾或中立，未能捕捉其结构。

Method: 提出BioDivergence框架，包括六类冲突分类、13轴分歧本体、每个声明对输出冲突类型、分歧轴、主导混杂因素和调和解释；构建了包含11,865个声明对的BioDivergence-Silver-v1.0基准及其去重变体。

Result: 在文章不重叠设置下，微调参考模型性能下降约12个百分点，Mistral-7B-Instruct-v0.3在842个测试样本上达到0.5523准确率和0.3894 contextual-F1。

Conclusion: BioDivergence能更忠实地区分上下文分歧与直接矛盾，并分离文章级记忆与真正任务学习。
...
### [120] [Afrispeech Semantics: Evaluating Audio Semantic Reasoning in Spoken Language Models Across Domains and Accents](https://arxiv.org/abs/2606.11219)
*Chibuzor Okocha,Christan Grant*

Main category: cs.CL

TL;DR: 评估了音频语言模型在蕴含、一致性、合理性、口音漂移和口音约束五个语义推理任务上的表现，揭示了当前评估的关键局限性。


<details>
  <summary>Details</summary>
Motivation: 音频语言模型在语义推理方面的能力缺乏全面基准测试，特别是口音变化和领域转移的影响研究不足。

Method: 通过五个语义和副语言推理任务（蕴含、一致性、合理性、口音漂移、口音约束）评估音频语言模型。

Result: 发现了当前音频推理评估的关键局限性，包括模型在口音变化等情况下稳定性和约束性不足。

Conclusion: 希望为更健壮和公平的音频语言模型设计和评估提供指导。
...
### [121] [Every Act Has Its Price: Compressed Moral Composition in Frontier LLMs](https://arxiv.org/abs/2606.11232)
*Weijia Zhang,Ruiqi Chen,Yunze Xiao,Weihao Xuan*

Main category: cs.CL

TL;DR: 提出了Moral Trolley Arena基准，测量LLM如何组合道德证据。


<details>
  <summary>Details</summary>
Motivation: 现有基准只测量孤立道德偏好，但现实判断需要组合多种道德信号。

Method: 两阶段盲ELO基准：先校准单个道德行为（229场景，五个基础），再组合成双行为项目测量组合偏好。

Result: 组合判断由组件强度预测但关系压缩；显示非加性锚定、基础特定残差和收敛偏好。

Conclusion: 道德审计应测量组合规则而非仅孤立行为排名。
...
### [122] [Energy-Efficient On-Device RAG on a Mobile NPU: System Design and Benchmark on Snapdragon X Elite](https://arxiv.org/abs/2606.11257)
*Zhiyuan Cheng,Longying Lai*

Main category: cs.CL

TL;DR: 首个端到端RAG管道在Hexagon NPU上运行，实现高能效和设备端推理。


<details>
  <summary>Details</summary>
Motivation: RAG管道计算密集，CPU推理能耗高，阻碍了设备端部署。NPU加速有望解决能耗问题，实现隐私、低延迟和离线使用。

Method: 在Snapdragon X Elite的Hexagon NPU上实现包含嵌入、重排序和LLM生成的端到端RAG管道，并在Dell XPS 13上与CPU和GPU基线进行对比评测。

Result: 索引时NPU嵌入吞吐量提升9.1倍，系统能耗减少12.3倍；查询时LLM预填充快18.1倍，端到端延迟降低4.0倍，能耗减少4.0倍；答案质量与CPU/GPU相当。

Conclusion: 该NPU能够实现节能高效的设备端RAG，且无质量损失，有望推广到其他移动NPU，为绿色边缘智能提供可持续路径。
...
### [123] [The Periodic Table of LLM Reasoning: A Structured Survey of Reasoning Paradigms, Methods, and Failure Modes](https://arxiv.org/abs/2606.11470)
...
TL;DR: 对超过300篇论文的系统综述，提出了LLM推理研究的结构化分类，分析了方法论趋势，总结了局限性，并指出了未来方向。


<details>
  <summary>Details</summary>
Motivation: LLM推理行为不一致且对提示策略、任务设计和模型规模敏感，需要系统分析以理解其能力与失败模式。

Method: 系统分析了来自多个来源的300多篇论文，建立涵盖九大推理范式的分类法，并分析了方法论趋势与重复出现的局限性。

Result: 提出了结构化分类、方法论趋势分析以及常见限制与失败模式的综合，并识别了新兴研究方向。

Conclusion: 该综述为开发更稳健、可解释和可泛化的推理系统提供了统一参考框架。
...
### [124] [Automated Creativity Evaluation of Language Models Across Open-Ended Tasks](https://arxiv.org/abs/2606.11762)
...
TL;DR: 提出一个自动化、领域无关的框架，通过语义熵和检索式多智能体评判量化LLM的创造力，在三个领域验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有创造力指标与具体任务耦合，限制可扩展性和通用性，需要系统、可扩展的方法。

Method: 分离测量装置与任务，发散创造力用语义熵，收敛创造力用检索式多智能体评判框架。

Result: 框架可靠地捕捉创新性、多样性和任务履行，模型属性影响创造力表现，效率提升60%以上。

Conclusion: 建立可重复、可推广的自动化LLM创造力评估标准，促进可扩展基准测试和创意AI。
...
### [125] [WorldReasoner: Evaluating Whether Language Model Agents Forecast Events with Valid Reasoning](https://arxiv.org/abs/2606.11816)
*Yizhou Chi,Eric Chamoun,Zifeng Ding,Andreas Vlachos*

Main category: cs.CL

TL;DR: WorldReasoner是一个评估语言模型代理在时间约束下进行事件预测的框架，通过结果、证据和推理三个维度衡量，发现时间有效检索是预测准确性的关键，但代理在概率校准方面仍有不足。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅关注最终答案的准确性，忽略了证据和推理质量，无法真正判断代理是否在合理预测。

Method: 提出了WorldReasoner框架，包括三个评估轴：结果质量（与解决答案对比）、证据质量（引用来源）和推理质量（与事后因果图对比）。通过自动构建流程生成345个任务，包含时间戳证据和因果图。

Result: 时间有效检索是结果准确性的最强驱动因素；因果图构建改善了关键事件恢复；正确图驱动的预测更依赖于关键事件和来源，但代理仍难以将基于证据的预测转化为校准的概率。

Conclusion: WorldReasoner提供了更全面的评估，揭示了当前代理在时间推理和概率校准方面的局限，有助于推动更真实的世界事件预测研究。
...
TL;DR: Doc2Atom is a compositional parametric memory framework that decomposes documents into semantically typed knowledge atoms, each compiled into a micro-LoRA adapter and a provenance retrieval key, enabling query-specific adapter assembly via a lightweight router, achieving better performance and lower memory cost than Doc-to-LoRA on long-document QA.


<details>
  <summary>Details</summary>
Motivation: Long input sequences cause quadratic attention costs; context distillation helps but existing monolithic adapter methods like Doc-to-LoRA suffer from irrelevant-query interference, limited recall, and poor scalability for long-document reasoning.

Method: Doc2Atom decomposes each document into knowledge atoms, each atom compiled into an independent micro-LoRA adapter and a retrieval key. At inference, a query router selects and assembles relevant atoms into a query-specific adapter, injected into a frozen base model. The system is trained end-to-end with multi-objective distillation.

Result: Experiments on six diverse QA benchmarks show Doc2Atom outperforms Doc-to-LoRA baselines while reducing the memory cost of document internalization.

Conclusion: Doc2Atom effectively addresses the limitations of monolithic context distillation by introducing compositional parametric memory, improving both performance and memory efficiency for long-document reasoning.
...
### [127] [Small Experiments, Cheaper Decisions: A Case Study in Staged Promotion for Micro-Pretraining](https://arxiv.org/abs/2606.11387)
*Felipe Chavarro Polania*

Main category: cs.CL

TL;DR: 研究了分阶段晋升协议在短预训练运行中的应用，旨在降低实验成本并识别稳健配置。


<details>
  <summary>Details</summary>
Motivation: 短预训练运行成本低但可能过度推广仅在小预算下表现好的配置，需要可靠的筛选方法。

Method: 在Windows A100和Linux L40S上使用固定微预训练运行器，从12个配置开始，通过2分钟、5分钟、10分钟、60分钟和12小时阶段，使用冻结规则进行筛选。

Result: 桥参考条件在60分钟和12小时阶段均排名第一，贪婪比较器未通过等价规则，d8/ar48未通过平均差距规则；总GPU小时为169.2。

Conclusion: 该协议提供了有界限的成本分配发现，但不声称全局最优或优于自适应方法。
...
### [128] [SAGE: Answer-Conditioned Uncertainty Targets for Verbal Uncertainty Alignment](https://arxiv.org/abs/2606.11512)
*Kaiwen Shi,Zheyuan Zhang,Yanfang Ye*

Main category: cs.CL

TL;DR: 提出SAGE和GUPO方法，有效校准大语言模型的不确定性表达。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通过自然语言表达不确定性，但常常无法准确反映其采样行为，需要更好的校准方法。

Method: 提出SAGE（语义答案引导熵）作为群体级不确定性目标，并结合GUPO框架对不确定性表达进行训练。

Result: 在事实、数学和多选题推理任务上，该方法改善了不确定性排名，降低了校准误差，减少了过度自信。

Conclusion: SAGE和GUPO能够有效校准大语言模型的不确定性表达。
...
### [129] [Substrate Asymmetry in User-Side Memory: A Diagnostic Framework](https://arxiv.org/abs/2606.11712)
*Youwang Deng*

Main category: cs.CL

TL;DR: User memory in LLMs factorizes into three orthogonal axes (behavioral consistency, factual presence, factual absence) with no single substrate winning all; per-user LoRA wins style, retrieval wins factual absence, and RLHF strengthens the asymmetry.


<details>
  <summary>Details</summary>
Motivation: To reveal that aggregate personalization metrics hide opposite-direction failures, motivating a factorization of user memory into orthogonal axes.

Method: Compared gamma-LoRA (per-user LoRA) vs BGE-large dense retrieval on a controlled synthetic corpus and LaMP-3 real data; used causal intervention and ablation on attention layers; tested routing via classification.

Result: gamma-LoRA wins behavioral style, retrieval wins factual absence; attention layers 21-35 causally load both effects oppositely; RLHF-tuned models show stronger asymmetry; gamma-LoRA underperforms majority baseline on LaMP-3 due to instruction-following collapse.

Conclusion: User memory is multidimensional; no single method excels on all axes; routing should be treated as question classification; alignment tax on parametric memory exists.
...
### [130] [External Experience Serving in Production LLM Systems: A Deployment-Oriented Study of Quality-Cost Trade-offs](https://arxiv.org/abs/2606.11806)
*Lin Sun,Heming Zhang,Xiangzheng Zhang*

Main category: cs.CL

TL;DR: External experience serving is a deployment-oriented quality-cost trade-off, where selective retrieval outperforms global injection, and cost-benefit profiles vary by output regime.


<details>
  <summary>Details</summary>
Motivation: Practical deployment of LLMs faces trade-offs between quality gains from external experience and increased cost (prompt burden, latency, pressure).

Method: Evaluated in a real moderation setting with tool-use and GPQA tasks; compared baselines, random controls, global injection, and selective retrieval.

Result: Selective retrieval provides better quality-cost trade-off than global injection; retrieval quality matters more than Top-K; cost-benefit profiles differ across short and decode-heavy outputs.

Conclusion: External experience should be a selective, cost-aware decision, not a universal add-on; it pays off only when quality gains justify cost.
...
TL;DR: 提出了一个德语-保加利亚语安全数据集Schützen，用于评估LLM在不同语言下的安全行为，发现显著跨语言差异。


<details>
  <summary>Details</summary>
Motivation: 现有安全数据集主要集中于英文和中文，缺乏针对共享社会文化、法律和伦理背景的语言评估，特别是低资源语言保加利亚语和高资源语言德语。

Method: 构建Schützen数据集，涵盖低资源语言保加利亚语和高资源语言德语；使用多语言和语言特定LLM进行实验评估模型在风险下的可回答性。

Result: 实验揭示了跨语言安全行为的显著差异，表明需要针对特定区域的评估资源。

Conclusion: 为了在德国和保加利亚负责任地部署LLM，必须开发量身定制的区域特定评估资源。

Abstract: Large language models are increasingly deployed across professional domains, bringing hard-to-predict risks, including the generation of harmful or disrespectful content. Although substantial progress has been made in developing safety evaluation datasets, existing resources remain overwhelmingly English- and Chinese-centric. This limitation is particularly pronounced when evaluating languages that operate within shared sociocultural, legal, and ethical contexts. To address this gap, we introduce Schützen: a German--Bulgarian safety dataset designed to assess model answerability under risk, covering both a low-resource language (Bulgarian) and a high-resource language (German). Experiments with multilingual and language-specific LLMs reveal pronounced cross-language differences in safety behavior, highlighting the necessity of tailored, region-specific evaluation resources to support the responsible deployment of LLMs in Germany and Bulgaria. Datasets and code are available at . Warning: this paper contains examples that may be offensive, harmful, or biased.

</details>


### [132] [I Understand How You Feel: Enhancing Deeper Emotional Support Through Multilingual Emotional Validation in Dialogue System](https://arxiv.org/abs/2606.11875)
*Zi Haur Pang,Yahui Fu,Koji Inoue,Tatsuya Kawahara*

Main category: cs.CL

TL;DR: 针对情感验证在对话系统中的应用，提出多语言语料库M-EDESConv和时机检测模型MEGUMI，实验表明MEGUMI性能优越，但LLM的情感理解仍需改进。


<details>
  <summary>Details</summary>
Motivation: 情感验证具有治疗价值，但计算语言学关注不足，特提出分解为三个子任务并构建相应资源。

Method: 通过混合人工和自动标注构建120k英语-日语多语言语料库M-EDESConv和口语测试集M-TESC；提出MEGUMI模型，融合XLM-RoBERTa语义与语言特定情感编码器。

Result: MEGUMI在客观和主观评价上均表现更优；GPT-4.1 Nano和Llama-3.1 8B能生成上下文相似多样的验证响应，但情感理解能力薄弱。

Conclusion: 当前模型在验证响应生成上取得进展，但情感理解是主要瓶颈，需要进一步研究。
...
### [133] [GraspLLM: Towards Zero-Shot Generalization on Text-Attributed Graphs with LLMs](https://arxiv.org/abs/2606.11898)
*Hengyi Feng,Zeang Sheng,Meiyi Qiang,Meiyi Qiang,Wentao Zhang*

Main category: cs.CL

TL;DR: 提出GraspLLM框架，通过结合图结构理解和语言模型，增强文本属性图上的跨数据集和跨任务泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在多样化的图数据和任务间泛化，且捕获可迁移图结构模式能力有限。

Method: 使用冻结嵌入模型统一节点语义空间，进行motif感知对比学习提取数据集无关结构信息，通过最优上下文子图对齐到LLM令牌空间。

Result: 在多个TAG基准数据集上，GraspLLM一致优于基于LLM的方法，尤其零样本场景表现突出。

Conclusion: GraspLLM框架具备强泛化能力，能有效整合图结构和语义信息。
...
### [134] [LatticeBridge: Rare-Event Sequential Inference for Faithful Structured Sequence Synthesis](https://arxiv.org/abs/2606.11203)
*Faruk Alpay,Bugra Kilictas*

Main category: cs.CL

TL;DR: LatticeBridge是一种结合前缀语言模型、实例编译表面自动机和扭曲序列蒙特卡洛解码器的结构化序列生成方法，在多个基准上显著提升锚点满足率和覆盖率。


<details>
  <summary>Details</summary>
Motivation: 标准解码方法倾向于生成流畅的序列，但难以同时满足所有输入约束，导致约束满足成为稀有事件。

Method: LatticeBridge集成紧凑前缀语言模型、实例编译的表面自动机以及带重采样、多层次分裂和源支持提议项的扭曲序列蒙特卡洛解码器，约束表示从每个输入实例自动编译。

Result: 在2610个验证任务（CommonGen、E2E NLG、WikiBio）上，粒子解码器在精确锚点满足和平均锚点覆盖率上优于贪婪、波束过滤和best-of-k基线，并报告了忠实性、重叠、延迟等多维指标。

Conclusion: LatticeBridge在固定提议模型下实现了更好的忠实性-重叠-延迟权衡，且不依赖手动词汇类，但需注意精确锚点满足可能不排除属性替换，需联合评估覆盖率与侵入诊断。
...
### [135] [Calibration Drift Under Reasoning: How Chain-of-Thought Budgets Induce Overconfidence in Large Language Models](https://arxiv.org/abs/2606.11211)
*Prakul Sunil Hiremath,Harshit R. Hiremath*

Main category: cs.CL

TL;DR: 本文发现链式思维推理在超过一定预算后会导致模型校准性能先升后降，称为校准漂移现象，并提出一种基于校准感知的停止规则（CABStop）来缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型校准不确定性对安全部署至关重要，但链式思维推理对校准的影响尚未完全理解，现有研究忽略了推理预算增加可能带来的过置信风险。

Method: 定义推理预算B，理论分析期望校准误差（ECE）的非单调模式（先降后升），提出基于自回归生成的假设锁定模型解释该行为，在47个推理陷阱问题上评估Llama-3.1-8B和Llama-3.3-70B，引入校准感知停止规则（CABStop）。

Result: 8B模型呈现非单调校准行为（ECE先降后升），70B模型仅完成基线评估，预算依赖效应不明确；CABStop能有效停止过度推理，改善校准。

Conclusion: 增加推理深度并不总能提升可靠性，甚至可能损害校准，应谨慎监控推理预算。
...
### [136] [AI Coding Agents Can Reproduce Social Science Findings](https://arxiv.org/abs/2606.11447)
*Meysam Alizadeh,Mohsen Mosleh,Fabrizio Gilardi,Atoosa Kasirzadeh,Joshua Tucker*

Main category: cs.CL

TL;DR: 提出SocSci-Repro-Bench基准评估AI编码智能体复制社会科学发现的能力，发现Claude Code和Codex表现良好但受提示设计影响。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准不足，缺乏对AI编码智能体在社会科学复制中能力的系统评估。

Method: 构建含221个任务的SocSci-Repro-Bench基准，涵盖可复制与不可复制研究，评估Claude Code和Codex。

Result: 两个智能体均能复制大量发现，Claude Code优于Codex；提供PDF提升性能但引入偏见；提示框架可导向确认性搜索。

Conclusion: 前沿编码智能体可作为可靠的计算工作流程执行者，但需谨慎设计基准与提示。
...
### [137] [ISE: An Execution-Grounded Recipe for Multi-Turn OS-Agent Trajectories](https://arxiv.org/abs/2606.11520)
*Siyuan Luo,Nairong Zheng,Lin Zhou,Tiankuo Yao,Shengyou Yuan,Haojia Yu,Cong Pang,Jiapeng Luo,Lewei Lu*

Main category: cs.CL

TL;DR: 提出ISE (Intent->Simulate->Execute) 三阶段合成范式，生成结构化用户意图、多轮对话和真实工具执行的训练数据，显著提升OS agent性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据集不能同时捕获结构化用户意图、多轮任务委派和工具执行，限制了OS agent的训练。

Method: 阶段一：通过4D框架 (Persona x Domain x Task x Complexity) 构造约50000个去重意图 (43956个)。阶段二：使用角色锁定的用户模拟器进行多轮交互，基于执行结果生成23132条轨迹。阶段三：在隔离的OS工作空间中执行每个工具调用，产生真实的失败恢复动态。

Result: 基于Qwen3-8B在ClawEval上pass@1从19.3提升至37.7，超越零样本GPT-4o和四倍大的Qwen3-32B。

Conclusion: ISE数据合成方法有效，多轮模拟贡献了大部分性能提升，代码和数据集已开源。
...
### [138] [Evaluating Bias in Phoneme-Based Automatic Speech Recognition Systems: An Analysis of IPA Transcription Models](https://arxiv.org/abs/2606.11639)
*Catherine Bao,Maneesha Rani Saha,Neal Patwari*

Main category: cs.CL

TL;DR: 研究评估了两种IPA音素ASR系统在不同语言和人口群体中的性能，发现持续存在的偏见，并提出软音素错误率指标以容忍相似音素替换。


<details>
  <summary>Details</summary>
Motivation: 现有ASR偏见研究多聚焦于字素系统，而音素系统在多语言场景中日益重要，但其偏见研究不足。

Method: 使用WhisperIPA和ZIPA系统，对多语言及带人口统计注释的英语语料库进行音素转录，与G2P系统对比，采用标准PER和提出的软PER指标。

Result: 模型性能在不同语言和人口群体（性别、口音、种族、年龄）间存在显著差异，即使考虑可接受的音素变异后仍存在偏差。

Conclusion: 结果为音素ASR系统的偏见来源提供了洞见，有助于开发更包容和语言鲁棒的模型。
...
### [139] [Debiasing Without Protected Attributes: Latent Concept Erasure from Textual Profiles](https://arxiv.org/abs/2606.12088)
*Shun Shao,Zheng Zhao,Anna Korhonen,Yftah Ziser,Shay B. Cohen*

Main category: cs.CL

TL;DR: 提出H-SAL方法，利用自我描述文本作为隐式信号进行后处理去偏，实验表明其效果与基于显式标签的去偏相当或更优。


<details>
  <summary>Details</summary>
Motivation: 大多数公平性研究假设可直接访问受保护属性，但实践中属性常因隐私或法律限制不可得，因此探索无直接访问敏感属性下的去偏可能性。

Method: 提出H-SAL，通过自我描述文本作为隐式去偏信号进行后处理概念和属性擦除；并构建多领域Stack Exchange公平性基准用于有用性预测，包含显式和隐式信号。

Result: 在编码器和解码器语言模型上，隐式自我描述去偏通常匹配或优于基于显式标签的去偏。

Conclusion: 结果拓宽了表示层面公平性研究，并提供了研究现实数据约束下去偏的新基准。
...
### [140] [Detecting AI-Generated Content on Social Media with Multi-modal Language Models](https://arxiv.org/abs/2606.11200)
...
TL;DR: 提出一个基于多模态视觉语言模型的AIGC检测与解释管道，在公共基准和真实社交媒体数据上达到最先进性能，并成功部署在推荐系统中提升用户参与度。


<details>
  <summary>Details</summary>
Motivation: 现有AIGC检测方法泛化性差、依赖单模态、缺乏可解释性，难以应对社交媒体上的新生成模型和复杂场景。

Method: 通过持续策划多模态社交媒体数据，训练一个紧凑的视觉语言模型，同时进行检测和生成可解释性说明。

Result: 在公共基准上实现最先进检测性能，在内部多平台社交媒体数据集上展现鲁棒检测和解释能力，部署后用户参与度指标提升。

Conclusion: 在动态真实社交媒体环境中进行有效AIGC检测是可行的，且能带来积极的下游影响。
...
### [141] [Semantic Grading of Written Answers in Low-Resource Language Bangla Using a Fine-Tuned Lightweight Language Model](https://arxiv.org/abs/2606.11931)
*Meherun Farzana,Aniket Joarder,Mahmudul Hasan,Md. Mosaddek Khan*

Main category: cs.CL

TL;DR: 提出一个孟加拉语-英语双语评估系统，通过微调Qwen3-8B模型进行语义评分和反馈，在合成和人类评估中表现优异。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语在教育NLP中研究不足，偏远地区缺乏教师，手写评分限制及时反馈，且自动评估因语义变异而困难。

Method: 微调轻量级语言模型（QLoRA-tuned Qwen3-8B），使用问题、参考答案和学生答案生成数值评分和上下文反馈，并构建合成双语数据集。

Result: 合成评估中反馈抗泄露性最佳（RoRa=0.819），人类研究中与评分一致性高（rho=0.936, MAE=0.725）。

Conclusion: 该系统在低资源教育环境中能有效进行语义评分和反馈，优于其他模型。
...
### [142] [FORT-Searcher: Synthesizing Shortcut-Resistant Search Tasks for Training Deep Search Agents](https://arxiv.org/abs/2606.12087)
...
TL;DR: 提出FORT框架，通过控制捷径风险合成训练数据，训练出FORT-Searcher在深度搜索基准上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有合成方法通过增加图结构复杂度提升难度，但结构复杂不保证搜索难度，存在更便宜的识别路径，需形式化此差距并识别捷径风险。

Method: 形式化捷径感知难度框架，识别四种捷径风险，使用轨迹特征诊断，提出FORT框架通过控制捷径风险合成数据，训练FORT-Searcher。

Result: FORT诱导更长预答案搜索和更少捷径模式，训练的FORT-Searcher在深度搜索基准上取得最佳整体性能。

Conclusion: FORT框架能有效生成抗捷径训练数据，训练出的搜索代理在复杂搜索任务中表现优异。
...
### [143] [System Report for CCL25-Eval Task 5: New Dataset and LoRA-Fine-Tuned Qwen2.5](https://arxiv.org/abs/2606.12392)
*Haotao Xie*

Main category: cs.CL

TL;DR: 本文通过构建CCPoetry-49K数据集并微调LLM，在古典诗歌翻译和情感理解上取得显著进步。


<details>
  <summary>Details</summary>
Motivation: 现有研究将诗歌鉴赏视为一般领域问题，忽视了其特点，且缺乏高质量领域数据集。

Method: 将任务分解为术语解释、语义解释和情感推断三个子任务；基于多个开源数据集构建CCPoetry-49K指令对数据集；使用LoRA微调Qwen2.5-14B得到PoetryQwen。

Result: 在CCL25-Eval Task 5上，PoetryQwen得分为0.757，相比基线提升9.7%。

Conclusion: PoetryQwen显著提升了古典诗歌的精确翻译和情感理解能力，为LLM的领域优化提供了数据集和方法论参考。
...
### [144] [Adaptive Multi-Resolution Procedural Knowledge Compression for Large Language Models](https://arxiv.org/abs/2606.12203)
*Changyue Wang,Weihang Su,Qingyao Ai,Yichen Tang,Runzhong Qiao,Xuancheng Li,Min Zhang,Yiqun Liu*

Main category: cs.CL

TL;DR: 提出SKIM框架，通过自适应多分辨率软令牌压缩程序性技能，在压缩到30%-60%长度的同时保持任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有压缩方法针对事实知识而非程序性知识，无法保留技能的逻辑依赖、无法离线压缩、不能适应不同复杂度，导致LLM使用技能时成本高、延迟大。

Method: SKIM采用自适应多分辨率软令牌压缩，根据技能复杂度分配不同数量的软令牌，在提升推理效率的同时保留技能的有效性。

Result: 在多个任务上，SKIM将技能压缩至原始长度的30%-60%，任务性能优于现有压缩方法。

Conclusion: SKIM能有效压缩程序性技能并保持性能，为LLM高效使用技能提供可行方案。
...
### [145] [PoQ-Judge: A Multi-Architecture Evaluation Framework for Cost-Aware Proof-of-Quality in Decentralized LLM Inference](https://arxiv.org/abs/2606.11196)
*Arther Tian,Alex Ding,Frank Chen,Simon Wu,Aaron Chan*

Main category: cs.CL

TL;DR: PoQ-Judge是一个参考自由的质量评估框架，通过训练轻量级法官模型在去中心化LLM推理网络中实现高质量评估，并支持级联评估降低开销。


<details>
  <summary>Details</summary>
Motivation: 去中心化LLM推理网络需要轻量级、无参考的质量评估方法用于Proof of Quality（PoQ），以避免依赖参考答案的开销。

Method: 训练三种架构（TextCNN、MiniLM cross-encoder、DeBERTa）的法官模型，采用两阶段训练（UltraFeedback预训练+GPT标注的领域内数据微调），并通过在线校准识别语义质量维度，同时利用级联评估减少计算成本。

Result: 最佳模型（DeBERTa judge）在参考自由场景下与代理地面真值的Pearson相关系数达到0.747，在复合评分中达到0.645（匹配最佳参考基础评估器），级联评估将成本降低72.7%，同时保持轻微质量损失。在QA任务上表现优于摘要任务。

Conclusion: 参考自由评估可以替代参考基础评估，但质量受代理地面真值限制；未来需改进代理质量以提升整体评估性能。
...
### [146] [One Jailbreak, Many Tongues: Learning Language-Insensitive Intention Representations for Multilingual Jailbreak Detection](https://arxiv.org/abs/2606.11202)
*Shuyu Jiang,Kaiyu Xu,Xingshu Chen,Hao Ren,Rui Tang,Yi Zhang,Tianwei Zhang,Hongwei Li*

Main category: cs.CL

TL;DR: 提出MLJailDe多语言越狱检测框架，通过数据增强、相对距离约束和不平衡感知分类，在11种语言上达到98.5% F1，并在未见语言上泛化良好。


<details>
  <summary>Details</summary>
Motivation: LLMs安全训练集中在主导语言，多语言能力与安全不对等，现有防御在多语言场景下效果有限。

Method: 使用多语言反向翻译数据增强构建11语言数据集；采用相对距离约束减少跨语言表示分散，鼓励相似意图聚类；使用不平衡感知分类目标缓解类别不平衡。

Result: 在多种语言上F1达98.5%，在未见语言上平均F1为97.1%，优于基线。

Conclusion: MLJailDe在多语言越狱检测中有效且具有强跨语言泛化能力。
...
### [147] [Benchmarking Large Language Models for Safety Data Extraction](https://arxiv.org/abs/2606.11204)
*Jonas Grill,Thomas Bayer,Sören Berlinger*

Main category: cs.CL

TL;DR: 本研究评估了四种LLMs在SDS数据提取中的性能，发现文本处理优于多模态，但均未达到90%准确率，表明仍需领域微调和人工验证。


<details>
  <summary>Details</summary>
Motivation: 由于安全数据表文档格式多样且传统规则方法存在局限性，准确提取结构化信息面临挑战。

Method: 评估了Gemini 1.5 Pro、GPT-4o、Claude 3.7 Sonnet和Llama 3.1-70B四种模型，采用零样本、少样本和思维链三种提示策略，比较文本和多模态处理流程，基于50000多个数据字段评估准确率、延迟和成本。

Result: 文本提取在所有指标上一致优于多模态；Gemini 1.5 Pro结合思维链提示取得最高准确率84%，优于GPT-4o（81%）和Claude 3.7 Sonnet（79%）；但无模型超过90%准确率阈值。

Conclusion: 通用LLMs尚不足以用于无监督工业部署，但任务特定微调显示潜力；未来应关注领域适应训练、模型校准和人机交互验证。
...
### [148] [ProcessThinker: Enhancing Multi-modal Large Language Models Reasoning via Rollout-based Process Reward](https://arxiv.org/abs/2606.11209)
*Jingpei Wu,Xiao Han,Weixiang Shen,Boer Zhang,Zifeng Ding,Volker Tresp*

Main category: cs.CL

TL;DR: 提出ProcessThinker，一种通过滚动过程奖励提供步骤级奖励的后训练流水线，无需显式训练PRM，提升视频问答推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR和GRPO方法仅使用稀疏的结果奖励，难以区分错误源于推理后期小错误还是整个无帮助轨迹；训练过程奖励模型需要大规模标注和额外成本。

Method: 首先将推理轨迹重写为步骤标记格式进行冷启动SFT，然后应用GRPO结合格式奖励和基于rollout的过程奖励：对每个中间步骤采样多个连续轨迹，以最终答案验证的成功率作为该步骤奖励。

Result: 在Video-MMMU、MMVU、VideoMathQA和LongVideoBench四个视频基准上，相比基线Qwen3-VL-8B-Instruct取得一致提升。

Conclusion: ProcessThinker通过密集的步骤级信用分配减少步骤间的不一致和矛盾，有效增强了多步逻辑推理，为视频问答提供了一种实用的后训练增强方案。
...
### [149] [T2MM: An LLM Supported Architecture For Inquiry-Based Modeling](https://arxiv.org/abs/2606.11210)
*John Kos,Rudra Singh,Ashok Goel*

Main category: cs.CL

TL;DR: T2MM是一种由LLM驱动的动态架构，用于在VERA建模工具中创建交互式模型，支持模型构建的视觉交互性。


<details>
  <summary>Details</summary>
Motivation: LLM缺乏视觉交互性，无法满足科学学习中模型构建所需的可视化与互动需求。

Method: 提出T2MM架构，利用LLM根据学习者的模型上下文生成交互式模型，并通过自定义程序生成的数据集进行评估。

Result: T2MM在所有成功指标上优于基于LLM全代码生成的基线模型。

Conclusion: 贡献在于将LLM集成到探究学习建模工具中，并描述了一种创建更交互的多模态LLM工具的架构。
...
### [150] [Beyond Compaction: Structured Context Eviction for Long-Horizon Agents](https://arxiv.org/abs/2606.11213)
*Andrew Semenov,Svyatoslav Dorofeev*

Main category: cs.CL

TL;DR: 提出Context Window Lifecycle方案，通过语义感知的驱逐机制为长周期LLM智能体提供无限工作范围，在8000万令牌的会话中完成89个任务且准确率无退化。


<details>
  <summary>Details</summary>
Motivation: 现有上下文管理方案如摘要压缩和最近截断存在局限性，需要一种能保持语义结构且避免性能退化的方案。

Method: CWL方案包括注释协议、依赖链接的情节图、优先级驱逐策略和令牌核算循环，通过语义感知的方式在预算超限时驱逐最可恢复的内容。

Result: 在长周期基准上，单智能体会话完成89个顺序任务，消耗8000万令牌，任务准确率与每任务隔离会话相比无退化。

Conclusion: CWL避免了摘要压缩的损失性和基于时间的截断的语义忽略，实现了有效无限工作范围且不损失性能。
...
### [151] [LifeSentence: Language models can encode human life course trajectories from longitudinal panel data](https://arxiv.org/abs/2606.11220)
*Samuel Liu,Muchen Xi,William Yeoh,Joshua J. Jackson*

Main category: cs.CL

TL;DR: LifeSentence bridges large language models with longitudinal panel data to forecast human life outcomes, outperforming baselines across prediction, robustness, and reasoning tasks while recovering known social stratification patterns.


<details>
  <summary>Details</summary>
Motivation: Conventional statistical approaches for forecasting life outcomes have limited accuracy, and modern methods like transformers require large-scale training data that most longitudinal studies lack, motivating the need to leverage pretrained language models' distributional knowledge.

Method: LifeSentence represents each life event as a structured natural-language record and instruction-tunes a pretrained 24-billion-parameter language model across an 18-task taxonomy (prediction, robustness, reasoning), trained on ~65,000 individuals from the German Socio-Economic Panel.

Result: Outperforms classical and deep learning baselines across all task families, achieving threefold improvement in joint event-and-timing prediction, 91.2% Kendall's tau in chronological order reconstruction, and recovers education premium, gender wage gap, and motherhood penalty without explicit supervision.

Conclusion: LifeSentence serves as both a predictive tool and a probe for counterfactual exploration of human biographies, enabling new research queries like connecting early-life history to specified late-life endpoints.
...
### [152] [When Probing Accuracy Saturates, Fragility Resolves: A Complementary Metric for LLM Pre-Training Analysis](https://arxiv.org/abs/2606.11375)
*Orion Reblitz-Richardson*

Main category: cs.CL

TL;DR: 引入脆弱性指标，揭示神经网络表示在探测准确率饱和后仍持续演化。


<details>
  <summary>Details</summary>
Motivation: 标准线性探测准确率在预训练早期饱和，无法反映后续表示变化。

Method: 定义脆弱性为探测准确率崩溃时的激活噪声水平，并应用于开放检查点语言模型。

Result: 脆弱性恢复了准确率无法看到的结构：道德化表示沿词法到组合梯度出现，层深度鲁棒性梯度单调发展，数据整理改变探测鲁棒性而不影响准确率。

Conclusion: 脆弱性作为探测准确率的互补指标，在准确率平坦时提供结构化信息。
...
### [153] [Overcoming State Inertia in Full-Duplex Spoken Language Models via Activation Steering](https://arxiv.org/abs/2606.11386)
*Cheng-Kuang Chang,Kai-Wei Chang,Alexander H. Liu,James Glass*

Main category: cs.CL

TL;DR: 全双工口语语言模型存在状态惯性，导致在用户打断时延迟响应，通过感知向量激活指导可有效缓解。


<details>
  <summary>Details</summary>
Motivation: 全双工口语模型虽能同时听和说，但其内部协调机制不明，且在用户打断时响应延迟，需研究并改进。

Method: 分析隐藏表示的预测模式发现状态惯性；提出Zero-Buffer Benchmark评估打断理解；使用激活指导（感知向量）进行训练干预。

Result: 状态惯性导致延迟；激活指导显著提升打断处理性能，如PersonaPlex上正确率从28%到45%，IWOR从40%到72%。

Conclusion: 激活指导是一种无训练、低开销的方法，能有效改善全双工口语模型的打断处理能力。
...
### [154] [Scenario-based Probing and Steering Cultural Values in Large Language Models--Extended Version](https://arxiv.org/abs/2606.11399)
*Trung Duc Anh Dang,Tung Kieu,Sarah Masud*

Main category: cs.CL

TL;DR: 提出通过场景化行为困境和激活引导探测并调整LLMs文化价值观的框架，发现文化维度间存在类似人类的纠缠，限制了独立轴对齐但保留任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有文化对齐评估依赖直接提示，易引发中性或安全回应，无法捕捉隐含偏好，且模型反映同质化价值观。

Method: 将社会价值观问题转化为行为困境，提取token级概率测量隐含价值观，应用激活引导（可选结合条件提示）调整模型行为，无需重新训练。

Result: 在三个开源LLM和四种目标文化中，发现可导性差异和潜在纠缠：干预一个文化维度会引发另一维度的偏移，该耦合类似人类WVS数据，并在激活、提示和混合引导中持续存在。

Conclusion: 维度纠缠限制了轴无关的文化对齐，但模型通用任务性能基本不受影响。
...
### [155] [Context-Aware Multimodal Claim Verification in Spoken Dialogues](https://arxiv.org/abs/2606.11420)
*Chaewan Chun,Delvin Ce Zhang,Dongwon Lee*

Main category: cs.CL

TL;DR: 介绍MAD2，一个多轮音频对话基准用于口语声明验证，并提出校准的多模态融合方法，结果显示对话上下文和音频有助于验证，且对话结构比错误信息框架更重要。


<details>
  <summary>Details</summary>
Motivation: 口语错误信息在对话中传播，但事实核查主要关注孤立文本，对话音频研究不足。

Method: 构建了包含1000个对话、3368个可核查声明的MAD2基准，并提出校准的多模态融合，结合上下文感知音频编码器和对话感知文本模型。

Result: 添加对话上下文提高了验证性能，但收益取决于场景类型；仅使用前文上下文通常与离线性能匹配，支持实时调节；音频在基于转录的模型被额外上下文干扰时贡献最大。
...
### [156] [SOMA-SQL: Resolving Multi-Source Ambiguity in NL-to-SQL via Synthetic Log and Execution Probing](https://arxiv.org/abs/2606.11424)
...
TL;DR: SOMA-SQL通过合成查询日志和歧义驱动的探测自动解决NL2SQL中的歧义，显著提高执行准确率。


<details>
  <summary>Details</summary>
Motivation: 自然语言到数据库接口在真实场景中因问题不明确、模式大且模糊而脆弱，现有方法依赖人工澄清或无法自动解决歧义。

Method: SOMA-SQL构建合成查询日志来指导模式解释和候选SQL生成，然后基于结构化歧义分类和候选分歧执行目标探测查询，产生消歧证据以最终选择和修复SQL。

Result: 在六个公共基准上平均执行准确率提升13.0%，在歧义问题上提升高达16.7%。

Conclusion: 这种主动的歧义发现和解决方法能够泛化到未见过的模式和查询分布，无需人工介入。
...
### [157] [Agent Skill Evaluation and Evolution: Frameworks and Benchmarks](https://arxiv.org/abs/2606.11435)
*Kexin Ding,Yang Zhou,Can Jin,Feng Tong,Mu Zhou,Dimitris N. Metaxas*

Main category: cs.CL

TL;DR: 本文系统调查了技能进化与评估的现状，分类了四种进化范式，并分析了基准测试的缺口，提出了未来方向。


<details>
  <summary>Details</summary>
Motivation: 随着技能库的扩展，需要严格的评估来确保实用性和安全性，驱动了从独立创建到评估驱动进化的范式转变。

Method: 通过系统性调查，将技能进化分为四种范式：执行反馈、轨迹蒸馏、压缩和强化学习；并分析了六个技能基准类别。

Result: 识别了基准覆盖的结构性缺口、权衡和度量丰富性，并指出了构建通用、高效、安全技能生态系统的开放方向。

Conclusion: 技能进化和评估领域正经历范式转变，需要更系统的评估框架和通用、安全的技能生态系统。
...
### [158] [Measuring language complexity from hierarchical reuse of recurring patterns](https://arxiv.org/abs/2606.11531)
*Junyi Zhou,Rui Liu,Pengyu Liu,Yu Liu*

Main category: cs.CL

TL;DR: 提出了基于算法信息论的阶梯路径指数作为语言复杂度度量，应用于21个平行语料库，发现该指数跨语言近似不变，支持等复杂度假说，并观察到复杂度在不同语言层面间的权衡，与认知科学中的组块机制一致。


<details>
  <summary>Details</summary>
Motivation: 评估语言复杂度度量，并检验等复杂度假说和权衡假说。

Method: 利用算法信息论定义阶梯路径指数，计算重建序列的最小步骤；应用于Parallel Universal Dependencies数据集的21个平行语料库，并进行统一二进制表示分析。

Result: 阶梯路径指数跨语言近似不变，变化小于语料长度；字符库存大小与语料长度、词汇级与语料库级复杂度之间存在权衡；识别出的可重用子结构与自然语言的词和形态成分重叠。

Conclusion: 等复杂度假说和权衡假说得到支持，阶梯路径方法与认知组块机制相联系，将语言复杂度守恒归因于共享的认知架构。
...
### [159] [Teaching Diffusion to Speculate Left-to-Right](https://arxiv.org/abs/2606.11552)
*Lexington Whalen,Yuki Ito,Ryo Sakamoto*

Main category: cs.CL

TL;DR: 提出三种训练干预措施（令牌位置加权、第一错误焦点损失、链损失）来缩小扩散草稿生成器与自回归验证器之间的不对称性，显著提高接受的草稿长度。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型能够并行生成草稿令牌，但其双向生成方式与自回归验证器的从左到右评估之间存在不对称，导致训练目标与验证奖励不匹配，需要干预来弥补这一差距。

Method: 三种正交的训练时干预：令牌位置加权（调整不同位置令牌的损失权重）、第一错误焦点损失（聚焦于破坏接受前缀的第一个错误位置）、链损失（用可微替代项代替期望接受长度）。它们组合使用，不增加额外前向传递。

Result: 在四个目标模型和六个推理、代码、对话基准上，相对于位置均匀基线，接受的草稿长度提升21-76%。

Conclusion: 这些干预措施有效且正交于测试时对齐机制（如多草稿自选），可在不改变推理流程或精确拒绝采样合约的情况下提升效率。
...
### [160] [When is Your LLM Steerable?](https://arxiv.org/abs/2606.11599)
*Chenrui Fan,Yize Cheng,Ming Li,Soheil Feizi,Tianyi Zhou*

Main category: cs.CL

TL;DR: 提出通过分析模型早期解码隐藏状态来预测激活引导效果的方法，并利用预测器优化引导强度搜索，以低成本实现接近最优的性能。


<details>
  <summary>Details</summary>
Motivation: 激活引导的成功与否高度依赖提示、概念、模型和配置，通常需要昂贵的网格搜索和完整生成后评估，缺乏高效的预测手段。

Method: 构建包含140万次引导生成和150个概念的ASTEER测试平台，提取跨层和初始解码步骤的隐藏状态对比特征，训练梯度提升决策树（GBDT）分类器预测引导是否欠、成功或过强。

Result: 预测器在未见概念上达到约0.7的宏F1分数，表明早期隐藏状态蕴含结构化信息；引导强度搜索在极小解码成本下接近最优性能。

Conclusion: 早期隐藏状态能够有效编码引导效果的信号，可用于轻量级预测和高效引导优化。
...
### [161] [Multi-Agent Reasoning with Adaptive Worker Allocation for Stance Detection](https://arxiv.org/abs/2606.11609)
*Meysam Sabbaghan,Arman Zareian Jahromi,Doina Caragea*

Main category: cs.CL

TL;DR: 提出一种多智能体推理框架，通过自适应工作者分配将立场检测的聚合从标签级投票转向推理级合成，在隐式立场上表现显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有聚合策略（如多数投票）丢弃中间推理，难以解决冲突解释，尤其在隐含立场场景下。

Method: 采用Manager-Worker架构，Manager根据输入复杂性动态分配Worker，每个Worker仅提供推理解释，Manager综合生成最终预测。

Result: 在COVID-19 Stance和SemEval-2016上分别达到86.07和82.90 Macro-F1，超过基线，尤其在隐式立场上增益最大。

Conclusion: 当立场不能可靠从表面线索推断时，自适应推理级聚合比标签级投票更有效。
...
### [162] [Can AI Reason Like an Urban Planner? Benchmarking Large Language Models Against Professional Judgment](https://arxiv.org/abs/2606.11678)
*Yijie Deng,He Zhu,Wen Wang,Junyou Su,Minxin Chen,Wenjia Zhang*

Main category: cs.CL

TL;DR: 本文提出UPBench框架评估LLM在规划专业知识上的推理能力，发现LLM在分析任务上表现优于事实回忆和综合判断，总结了四个认知局限。


<details>
  <summary>Details</summary>
Motivation: 缺乏系统性框架测试LLM是否具备规划专业知识所需的上下文敏感性、价值意识和制度素养。

Method: 构建UPBench框架，使用4个知识支柱和5个认知水平的矩阵，评估25个LLM，结合自动评分和专家评审。

Result: 发现非单调认知曲线：LLM在分析任务上表现更好，但事实回忆和综合判断较差；总结了四个诊断：规则幻觉、概念混淆、棘手问题麻痹和实践智慧缺失。

Conclusion: LLM可辅助跨学科综合、文献综述、场景生成和初步政策分析，但在特定法规、规范冲突和情境敏感程序上不可靠；建议机构验证AI分析，规划教育强调制度素养和规范判断。
...
### [163] [Layer-Isolated Evaluation: Gating the Deterministic Scaffold of a Production LLM Agent with a No-LLM, Regression-Locked Test Harness](https://arxiv.org/abs/2606.11686)
*Sawyer Zhang,Alexander Wang,Sophie Lei*

Main category: cs.CL

TL;DR: 提出层隔离评估方法，通过将LLM代理分解为独立层并分别测试，揭示聚合指标掩盖的局部回归问题。


<details>
  <summary>Details</summary>
Motivation: 端到端任务成功评估只能指示性能下降，无法定位问题所在层，需要更细粒度的评估方法。

Method: 将排序代理分解为八个层（本体、意图、路由等），每层由独立的断言切片在无LLM的纯模式下测试，通过受控回归注入验证切片对自身故障的敏感性。

Result: 聚合通过率对局部回归不敏感（-1.7至-5.9 pp），而对应切片下降显著（-25至-91 pp）；注入层的切片在7/7案例中排名前三，平均排名1.29/19；在第二个租户上复现。

Conclusion: 该方法作为组件级评估的确定实例，实现了EDDOps的构想，以CheckList为祖先，与随机突变测试互补，贡献包括解耦的测试框架、覆盖诚实标准和回归注入演示。
...
TL;DR: 提出Lius模型，利用双语词典和持续指令微调提升低资源语言库邦马来语的翻译性能，显著超越基线模型。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理低资源语言翻译时性能下降。

Method: 设计指令集，利用双语词典的词汇和语义特征；引入持续指令微调（CIT）实现迭代训练。

Result: Lius模型在多个评估指标上比标准指令微调模型提升4-6个点，比NMT和多语言LLM模型提升10-13个点。

Conclusion: 该方法有潜力减少对大规模平行数据的依赖。
...
### [165] [When Does Language Matter? Multilingual Instructions Reveal Step-wise Language Sensitivity in Vision-Language-Action Models](https://arxiv.org/abs/2606.11906)
*Xuan Dong,Zhe Han,Tianhao Niu,Qingfu Zhu,Wanxiang Che*

Main category: cs.CL

TL;DR: 本文首次系统评估VLA模型的多语言鲁棒性，发现非英语指令下成功率下降30-50%，并揭示语言影响具有步骤非均匀性。基于此提出步骤敏感性驱动的推理时干预，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: VLA模型对语言变化的鲁棒性了解不足，缺乏多语言评估。

Method: 将LIBERO基准翻译成十种语言进行系统评估，通过细粒度分析识别步骤语言敏感性，并提出步骤级推理时干预对齐表示。

Result: 非英语指令成功率下降30-50%，干预后性能显著提升。

Conclusion: 语言鲁棒性是步骤级控制问题，时间结构分析对可靠具身代理至关重要。
...
### [166] [Decoding Multimodal Cues: Unveiling the Implicit Meaning Behind Hateful Videos](https://arxiv.org/abs/2606.11953)
...
TL;DR: 提出IARE框架和两个数据集用于可解释的仇恨视频检测，通过信息增强和推理增强实现最先进性能并生成准确理由。


<details>
  <summary>Details</summary>
Motivation: 现有仇恨视频检测方法缺乏可解释性，不能提供上下文理由，因此需要可解释检测。

Method: 提出IARE框架，包括信息增强（利用多模态思维链整合有害元素）和推理增强（使用直接偏好优化引导正确推理路径）。

Result: 在两个数据集上实验，IARE实现最先进性能并生成准确理由。

Conclusion: IARE框架有效实现了可解释的仇恨视频检测，具有较好性能与可解释性。
...
### [167] [Agreement in Representation Space for Open-Ended Self-Consistency](https://arxiv.org/abs/2606.12003)
*Paula Ontalvilla,Gorka Azkune,Aitor Ormazabal*

Main category: cs.CL

TL;DR: 提出基于嵌入的协议（EBA），通过聚类生成样本的嵌入空间评估自洽性，将自洽性从精确匹配扩展至开放生成任务。


<details>
  <summary>Details</summary>
Motivation: 现有自洽性方法依赖精确匹配，仅适用于分类任务，无法处理代码生成、文本摘要等开放生成任务。

Method: 设计Embedding-Based Agreement (EBA)方法，对模型多次采样的输出进行嵌入表示，通过聚类分析语义一致性，无需额外训练。

Result: 在数学推理、代码生成和摘要任务中，EBA优于随机选择和基于LLM评估或不确定性的方法，且稳定性更好；生成样本的几何位置与质量强相关。

Conclusion: 自洽性应理解为生成样本在表示空间中的几何集中性，而非符号层面的精确匹配。
...
### [168] [StanceNakba Shared Task: Actor and Topic-Aware Stance Detection in Public Discourse](https://arxiv.org/abs/2606.12068)
*Kholoud K. Aldous,Md Rafiul Biswas,Mabrouka Bessghaier,Shimaa Ibrahim,Kais Attia,Wajdi Zaghouani*

Main category: cs.CL

TL;DR: StanceNakba 2026 shared task on stance detection in Palestinian-Israeli conflict social media, with two subtasks, 7 teams, top systems using transformer models achieving Macro F1 up to 0.9620.


<details>
  <summary>Details</summary>
Motivation: To address stance detection in highly polarized discourse around the Palestinian-Israeli conflict, providing a benchmark with annotated datasets for two subtasks.

Method: Participating systems fine-tuned Arabic and multilingual transformers (MARBERT, AraBERT, DeBERTa-v3) with cross-validation, ensemble methods, and topic-conditioned architectures.

Result: Best Macro F1 scores: 0.9620 for Subtask A (actor-level) and 0.8724 for Subtask B (cross-topic).

Conclusion: Transformer-based approaches are highly effective for conflict-domain stance detection, but cross-topic generalization and neutral class prediction remain challenging.
...
### [169] [Augmenting Molecular Language Models with Local $n$-gram Memory](https://arxiv.org/abs/2606.12113)
*Xinni Zhang,Zijing Liu,He Cao,Yu Li,Irwin King*

Main category: cs.CL

TL;DR: MolGram integrates a conditional n-gram memory module into molecular language models to address the locality gap in SMILES tokenization, achieving better performance with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Transformer-based language models for SMILES strings suffer from a locality gap: standard character-level tokenization fragments chemically meaningful motifs, forcing models to repeatedly learn local syntax at the expense of long-range dependencies.

Method: We propose MolGram, which integrates a conditional n-gram memory module into molecular language models. MolGram maps local string patterns to learned embeddings via scalable hash lookups and dynamically injects this regional context into hidden states.

Result: Evaluations across three tasks (unconditional molecule generation, forward reaction prediction, and single-step retrosynthesis) show that MolGram consistently improves performance, outperforming baselines with 3x more parameters.

Conclusion: MolGram establishes explicit local pattern memory as a highly efficient inductive bias.
...
### [170] [Detecting Sensitive Personal Information in Japanese Pre-Training Corpora for Large Language Models](https://arxiv.org/abs/2606.12114)
*Rei Minamoto,Yusuke Oda,Daisuke Kawahara*

Main category: cs.CL

TL;DR: 首次探索日语文本中特殊护理个人信息（SCPI）的检测，使用基于LLM的注释构建数据集并训练ML模型，有效识别SCPI。


<details>
  <summary>Details</summary>
Motivation: 敏感个人信息可能出现在大规模预训练语料中，需要检测和过滤以确保隐私合规，而日语方面的研究有限。

Method: 使用基于LLM的注释构建SCPI数据集，训练机器学习模型进行检测。

Result: SCPI分类器能有效识别与SCPI相关的信息。

Conclusion: 这是首次研究，突出了准确检测的挑战。
...
### [171] [A Controlled Study of Decoding-Time Truthfulness Methods on Instruction-Tuned LLMs](https://arxiv.org/abs/2606.12160)
*Ao Sun*

Main category: cs.CL

TL;DR: Introduces CHAIR, a supervised framework that detects hallucinations by analyzing internal logits features (max, min, mean, std, slope) from each layer and token, achieving high accuracy on TruthfulQA and MMLU, especially in zero-shot settings.


<details>
  <summary>Details</summary>
Motivation: Improve hallucination detection accuracy and robustness in LLMs, particularly in zero-shot scenarios, and explore internal representations for better decoding strategies.

Method: Extracts a compact set of features (maximum, minimum, mean, standard deviation, slope) from token logits across all layers and uses them in a supervised classifier to detect hallucinations.

Result: On TruthfulQA and MMLU datasets, CHAIR significantly improves detection accuracy, especially in zero-shot scenarios, demonstrating robustness and generalizability.

Conclusion: CHAIR offers a practical solution for hallucination detection and lays groundwork for using internal representations to enhance factuality and coherence, suggesting advanced decoding strategies may further reduce hallucinations.
...
### [172] [A Resource for Enthymeme Detection in Controversial Political Discourse](https://arxiv.org/abs/2606.12186)
*Martial Pastor,Nelleke Oostdijk*

Main category: cs.CL

TL;DR: 创建了一个包含1,482条政治争议性推文的省略三段论（enthymeme）标注数据集，用于研究标签变异性，并发现基于标注者分歧训练的模型优于基于多数投票标签的模型。


<details>
  <summary>Details</summary>
Motivation: 现有省略三段论标注资源倾向于消除分歧，阻碍了对标签变异及其潜在益处的研究。

Method: 重新定义省略三段论并基于Walton论证方案提出结构化标注指南，进行复杂度分析，并训练模型比较分歧和多数投票标签的效果。

Result: 初步实验显示，训练于标注者分歧的模型性能优于训练于硬多数投票标签的模型。

Conclusion: 结构开放的省略三段论定义和指南有助于研究主观推理过程的变异性，对未来资源和NLP应用有积极影响。
...
### [173] [Agentic Environment Engineering for Large Language Models: A Survey of Environment Modeling, Synthesis, Evaluation, and Application](https://arxiv.org/abs/2606.12191)
...
TL;DR: 对基于LLM的智能体环境进行系统研究，从环境工程生命周期角度涵盖建模、合成、评估和应用。


<details>
  <summary>Details</summary>
Motivation: 现有工作缺乏系统分类和深度分析，因此需要系统研究智能体环境。

Method: 从环境工程生命周期角度，分析八个属性和八个领域的代表性环境，介绍符号和神经合成范式及评估方法，讨论四种进化途径和三种环境进化范式。

Result: 提供环境建模、合成、评估和应用的系统分析，识别环境进化范式。

Conclusion: 提出未来方向，包括环境即服务、多智能体环境和神经符号环境。
...
### [174] [Reassessing High-Performing LLMs on Polish Medical Exams: True Competence or Bias-Driven Performance?](https://arxiv.org/abs/2606.12250)
...
TL;DR: 医学LLM的MCQA评估存在缺陷，引入扩展的波兰医学考试基准，发现评估设计强烈影响结果，最佳模型性能大幅下降。


<details>
  <summary>Details</summary>
Motivation: MCQA评估可能高估临床能力，需要更真实的测试来减少猜测和偏见。

Method: 引入包含超过15000个问题、两个新领域和四种结构修改的波兰医学考试基准，评估21个LLM。

Result: 评估设计显著影响结果；最佳模型Qwen3.5-122B在英语和波兰语考试中分别下降28.4和31个百分点；标准MCQA分数不能可靠反映真实能力。

Conclusion: 改进的评估设计揭示LLM真实能力，基准公开以促进研究。
...
### [175] [Beyond Fully Random Masking: Attention-Guided Denoising and Optimization for Diffusion Language Models](https://arxiv.org/abs/2606.12273)
*Jia Deng,Junyi Li,Wayne Xin Zhao,Jinpeng Wang,Hongyu Lu,Ji-Rong Wen*

Main category: cs.CL

TL;DR: 提出AGDO框架，通过注意力指导优化扩散大语言模型的训练与推理，提升数学与编程基准性能。


<details>
  <summary>Details</summary>
Motivation: 现有后训练方法依赖随机掩码策略，忽略内在的token依赖性。

Method: AGDO框架：基于注意力结构确定去噪顺序，并在监督微调和强化学习中强调注意力关键token。

Result: 在数学和编程基准上，AGDO持续提升推理性能，优于现有后训练方法。

Conclusion: AGDO通过注意力指导有效提升扩散大语言模型的推理性能。
...
### [176] [Measuring Epistemic Resilience of LLMs Under Misleading Medical Context](https://arxiv.org/abs/2606.12291)
...
TL;DR: LLMs achieve expert-level scores on medical exams but are vulnerable to misleading context, abandoning correct answers at high rates, as measured by MedMisBench.


<details>
  <summary>Details</summary>
Motivation: The assumption that high exam scores imply safe medical judgment is fragile; patients increasingly rely on LLMs for health advice, but LLMs can be misled by adversarial context.

Method: Introduce MedMisBench, a benchmark with 10,932 medical questions and 48,889 misleading context-option pairs, tested on 11 model configurations to measure epistemic resilience.

Result: Mean accuracy drops from 71.1% to 38.0% under focused misleading context, with 51.5% attack success; authority-framed falsehoods achieve 69.5% and exception-poisoning 64.1%; clinical panel found 38.2% of cases have serious potential harm.

Conclusion: Existing benchmarks measure knowledge but not resilience to misleading context; MedMisBench exposes a structural blind spot in LLM evaluation for medical settings.
...
### [177] [Measuring Semantic Progress in Multi-turn Dialogue via Information Gain](https://arxiv.org/abs/2606.12332)
*Paul He,Shiva Kasiviswanathan,Dominik Janzing*

Main category: cs.CL

TL;DR: 提出一种基于信息论的语义进展度量，用于评估多轮对话，无需自回归推理，轻量级嵌入模型有效。


<details>
  <summary>Details</summary>
Motivation: 多轮对话评估困难，因为质量体现在跨轮次对话中，现有方法缺乏对语义进展这一关键维度的关注。

Method: 将语义进展形式化为问题条件下的不确定性减少，引入信息论度量，在嵌入空间中使用高斯公式近似，具有单调性和可分解性。

Result: 在MT-Bench、Chatbot Arena和UltraFeedback上，该度量与人类判断一致性高，在MT-Bench和UltraFeedback上优于多个基于LLM的评判，且轻量级嵌入模型也有效。

Conclusion: 语义进展可以仅通过轻量级嵌入模型捕获，证明了非LLM方法在对话评估中的潜力。
...
### [178] [ALIGNBEAM : Inference-Time Alignment Transfer via Cross-Vocabulary Logit Mixing](https://arxiv.org/abs/2606.12342)
*Chirag Chawla,Pratinav Seth,Vinay Kumar Sankarapu*

Main category: cs.CL

TL;DR: ALIGNBEAM是一种无需训练的方法，通过在推理时逐词翻译锚点logits并选择最安全输出，提升跨族模型的安全性。


<details>
  <summary>Details</summary>
Motivation: 领域微调降低了大语言模型的安全性，现有推理时防御要求共享词汇表，不适用于跨族专家模型。

Method: ALIGNBEAM在每步解码时将安全锚点模型的logits逐词翻译到目标模型词汇表，并用小型LLM法官从K个候选延续中选择最安全的。

Result: 在跨词汇和相同词汇评估对上，ALIGNBEAM在对抗性基准上大幅提高拒绝率，同时保持任务准确性和推理开销在实用范围内。

Conclusion: 安全对齐可以在推理时在模型族之间转移，无需改动任何模型的权重。
...
### [179] [Verifiable Environments Are LEGO Bricks: Recursive Composition for Reasoning Generalization](https://arxiv.org/abs/2606.12373)
*Hao Xiang,Qiaoyu Tang,Le Yu,Yaojie Lu,Xianpei Han,Ben He,Le Sun,Bowen Yu,Peng Wang,Hongyu Lin,Dayiheng Liu*

Main category: cs.CL

TL;DR: RACES recursively composes verifiable environments to scale RL training for LLM reasoning, achieving efficiency and performance gains.


<details>
  <summary>Details</summary>
Motivation: Existing manual or individual construction methods for verifiable environments suffer from linear scaling limits, hindering scalable reasoning generalization in RL for LLMs.

Method: RACES conceptualizes verifiable environments as composable building blocks that can be recursively assembled when codomain matches domain, using operators SEQUENTIAL, PARALLEL, SORT, and SELECT to induce diverse reasoning patterns.

Result: RACES improves DeepSeek-R1-Distill-Qwen-14B by 3.1 points and Qwen3-14B by 2.3 points on unseen benchmarks, and achieves performance comparable to training on 300 individual environments using only 50 base environments.

Conclusion: Recursive composition of environments enables efficient and scalable RL training for reasoning generalization in LLMs.
...
### [180] [Which Models Are Our Models Built On? Auditing Invisible Dependencies in Modern LLMs](https://arxiv.org/abs/2606.12385)
*Sanjay Adhikesaven,Haoxiang Sun,Sewon Min*

Main category: cs.CL

TL;DR: ModSleuth递归重构LLM依赖图，揭示隐藏的依赖关系和文档不一致。


<details>
  <summary>Details</summary>
Motivation: 现代LLM训练管道依赖其他模型生成数据、过滤语料、判断输出和指导开发决策，这些依赖是递归的且碎片化，复杂性超出人类追踪能力。

Method: 提出ModSleuth代理系统，从公共制品递归重建LLM依赖图，形式化区分直接/间接依赖，利用操作中心关系表示管道角色，并解决制品身份引用不一致。

Result: 应用于四个LLM发布，恢复1060个源验证依赖，构建大规模依赖图，揭示多跳许可义务、训练-评估耦合、发布与训练时制品不一致等问题。

Conclusion: 发布ModSleuth和依赖图，支持对日益复杂的LLM生态系统进行透明分析。
...
### [181] [Context-Driven Incremental Compression for Multi-Turn Dialogue Generation](https://arxiv.org/abs/2606.12411)
*Yeongseo Jung,Jaehyeok Kim,Eunseo Jung,Jiachuan Wang,Yongqi Zhang,Ka Chun Cheung,Simon See,Lei Chen*

Main category: cs.CL

TL;DR: 提出一种上下文驱动的增量压缩方法C-DIC，通过可修订的对话记忆和跨轮信息共享，实现长对话的高效鲁棒建模。


<details>
  <summary>Details</summary>
Motivation: 现有对话代理中对话历史增长导致冗余计算，截断或摘要降质，压缩器缺乏跨轮记忆共享和修订，引起信息丢失和错误累积。

Method: C-DIC将对话视为交错上下文线程，存储可修订的每线程压缩状态在紧凑记忆中，通过检索-修订-写回循环跨轮共享信息，并适应TBPTT学习跨轮依赖。

Result: 在长对话基准上性能优越，推理延迟和困惑度稳定，支持数百轮对话的可扩展高质量建模。

Conclusion: C-DIC实现了高效鲁棒的长对话上下文压缩，为高质量对话建模提供了可扩展路径。
...
### [182] [Compatibility-Aware Dynamic Fine-Tuning for Large Language Models](https://arxiv.org/abs/2606.11206)
*Yucheng Zhou,Junwei Sheng,Qianning Wang,Jianbing Shen*

Main category: cs.CL

TL;DR: 提出兼容性感知动态微调(CADFT)，通过兼容性信号和重写策略控制样本级优化方差，提升SFT的稳定性和泛化。


<details>
  <summary>Details</summary>
Motivation: SFT存在优化不稳定和泛化有限的问题，DFT虽能修正token级的梯度缩放，但忽略了指令数据的异质性，导致样本级高方差更新。

Method: 扩展DFT，从模型似然导出动态兼容性信号以调制监督更新，并设计延迟低频的兼容性引导重写策略，将不兼容的演示转化为可学习目标。

Result: 实验表明CADFT在稳定性、泛化性和冷启动强化学习初始化上均有提升，且保持全监督、无需显式奖励模型。

Conclusion: CADFT是一种方差控制估计量，将DFT的token级稳定推广到样本级，为SFT提供了更稳健的对齐方法。
...
### [183] [A Geometric Profile of Semantic Information in Text: Frame-Conditional Uniqueness and a Trade-Off Triangle for Scalar Summaries](https://arxiv.org/abs/2606.11222)
*Dmitriy Kompaneets*

Main category: cs.CL

TL;DR: 提出基于句子嵌入几何结构的文本语义内容度量框架，定义三坐标语义剖面（新颖性、广度、整合），并给出两个实用标量S_minmax和S_rank。


<details>
  <summary>Details</summary>
Motivation: 现有度量如香农熵忽略语义，成对比较（如BERTScore）无法表征单个文本，需新方法直接度量文本语义内容。

Method: 构建几何框架：通过六条公理唯一确定标量度量；提出三坐标语义剖面和语义量子；证明不可能定理，无法同时满足三个理想性质；实际给出S_minmax和S_rank两种标量。

Result: 推荐配置通过25/28项序数检查（校正后21/28），优于七种基线；广度坐标与行列式点过程对数行列式高度相关（Spearman ρ=0.985）。

Conclusion: 几何框架有效度量语义内容，实际标量在权衡中表现优异，广度坐标有优化理论基础。
...
### [184] [The Dynamics of Human and AI-Generated Language: How Semantics Fluctuates across Different Timescales](https://arxiv.org/abs/2606.11371)
*Han-Jen Chang,Yasir Çatal,Angelika Wolman,Agustín Ibáñez,David Smith,I-Wen Su,Kai-Yuan Cheng,Georg Northoff*

Main category: cs.CL

TL;DR: 提出了一种基于自相关窗口（ACW）的语义时间尺度分析方法，用于量化语音中一般性与具体语义内容的时间分布，并比较人类与AI生成的语音。


<details>
  <summary>Details</summary>
Motivation: 人类和LLM生成的语音随时间展开，但缺乏简单、可解释的时间序列特征来捕捉语义内容的通用性与具体性分布及其时间组织。

Method: 将带时间戳的转录转换为语义时间序列，计算词深度（WordNet）和上下文相似性（SBERT），并使用ACW-0等度量量化时间依赖性，通过与多种洗牌控制比较来验证。

Result: 较长ACW-0的段落包含更通用的词汇，较短ACW-0的段落包含更具体的词汇；随机化词序和时间后这些关联减弱或消失。

Conclusion: ACW语义时间尺度是分析和比较人类与AI生成语音时间结构的有用特征集。
...
### [185] [AI Coding Agents in Social Science: Methodologically Diverse, Empirically Consistent, Interpretively Vulnerable](https://arxiv.org/abs/2606.11456)
*Meysam Alizadeh,Fabrizio Gilardi,Mohsen Mosleh,Enkelejda Kasneci*

Main category: cs.CL

TL;DR: 在科学分析中，基于LLM的智能体在设计层可匹敌或超越人类的方法多样性，但在裁决层易受确认偏差影响；偏差源是解释而非估计。


<details>
  <summary>Details</summary>
Motivation: 担忧LLM智能体可能减少方法多样性或放大动机性结论的灵活性。

Method: 在移民与社会政策分析中，对Claude Code和Codex分别进行20次独立执行，并以多分析师人类基线作为对照。

Result: Codex的方法多样性与人类相当，Claude Code的规范数量近乎人类三倍；偏差提示影响设计但不改变估计或裁决；确认提示使Claude Code裁决从10%支持翻转为90%，但系数分布不变。

Conclusion: AI智能体在设计层可匹敌或超越人类，但在裁决层存在脆弱性；偏差的根源是解释而非估计。
...
### [186] [Hubs or Fringes: Pretraining Data Selection via Web Graph Centrality](https://arxiv.org/abs/2606.11499)
*Vedant Badoni,Danqi Chen,Xinyi Wang*

Main category: cs.CL

TL;DR: WebGraphMix利用网络图中心性选择预训练数据，无需模型训练，提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法依赖辅助分类器，增加计算开销且需要标注数据。

Method: 提出WebGraphMix，计算Common Crawl主机级网络图的结构中心性得分，调整中心与外围文档在预训练混合中的比例。

Result: 1:1混合中心与外围文档平均得分41.4%，优于均匀采样的39.8%，结合质量分类器得分达43.8%。

Conclusion: 网络图拓扑是预训练数据管理的有意义维度，与内容方法正交。
...
### [187] [When Roleplaying, Do Models Believe What They Say?](https://arxiv.org/abs/2606.11502)
*Benjamin Sturgeon,David Africa,Sid Black*

Main category: cs.CL

TL;DR: 角色扮演主要改变语言模型的输出而非其内部真实表征，而新兴错位则改变内部表征。


<details>
  <summary>Details</summary>
Motivation: 研究角色扮演是否仅改变模型输出，还是也影响其内部真实表征。

Method: 使用线性真实探测器分析LLM角色扮演历史人物时的信念，比较‘时代相信的’与‘时代错误的’两类虚假陈述。

Result: 角色扮演中时代相信的陈述被抑制较少但仍分类为假；新兴错位中虚假陈述向真实区域偏移且更多被辩护。

Conclusion: 角色扮演改变输出多于内部表征，新兴错位改变内部表征但未完全标记为真，两者是信念内化谱系的不同点。
...
### [188] [Pretrained self-supervised speech models can recognize unseen consonants](https://arxiv.org/abs/2606.11542)
*Chihiro Taguchi,Éric Le Ferrand,Hirosi Nakagawa,Hitomi Ono,Kanji Kato,Emily Prud'hommeaux,David Chiang*

Main category: cs.CL

TL;DR: 自监督语音模型能更准确识别点击辅音。


<details>
  <summary>Details</summary>
Motivation: 预训练自监督语音模型训练数据偏向高资源语言，可能低估了罕见点击辅音的代表性，因此研究模型识别点击辅音的准确性。

Method: 微调预训练的Wav2Vec2和HuBERT模型，在两种点击丰富的科伊桑语言（G|ui和West !Xoon）数据上进行评估。

Result: 微调模型对点击辅音的识别准确率始终高于非点击辅音。

Conclusion: 自监督学习使模型能够泛化到包括稀有音素在内的所有人类语音。
...
### [189] [Improving Cross-Format Robustness in Language Models with Multi-Format Training](https://arxiv.org/abs/2606.11643)
*June M. Liu,Shaomian Zheng,He Cao,Dingnan Jin,Qing Cui,Jun Zhou*

Main category: cs.CL

TL;DR: 通过定义跨格式鲁棒性并提出FormatMix方法，发现多格式监督能显著提高LLM的鲁棒性和性能，且仅需扩展约30%训练集即可获得大部分增益，格式多样性是关键。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型对答案格式敏感，同一问题在不同格式下可能失败，需要研究如何提高这种跨格式鲁棒性。

Method: 定义跨格式鲁棒性指标，比较全格式训练与FormatMix（随机或目标选择扩展部分训练项为多种格式），在GLM4和Llama-3.1上实验。

Result: 多格式监督一致提高任务性能和鲁棒性；仅MCQ监督益处甚微甚至有害；扩展约30%训练集即可恢复大部分增益，效果跨模型和规模一致。

Conclusion: 格式多样性而非额外监督是鲁棒性的关键；轻量级多格式增强如FormatMix可实用地降低格式敏感性，无需修改基础模型。
...
### [190] [UR-BERT: Scaling Text Encoders for Massively Multilingual TTS Through Universal Romanization and Speech Token Prediction](https://arxiv.org/abs/2606.11681)
*Sangmin Lee,Eekgyun Ahn,Woongjib Choi,Hong-Goo Kang*

Main category: cs.CL

TL;DR: UR-BERT通过罗马化转录和语音令牌预测目标，实现了495种语言的大规模多语言TTS，性能优于基线并泛化到未见语言。


<details>
  <summary>Details</summary>
Motivation: 传统G2P方法受限于资源只能覆盖约100种语言，需要一种能扩展到更多语言的方法。

Method: 采用罗马化转录统一多种书写系统作为输入表示，并引入语音令牌预测目标增强语音感知学习。

Result: 在495种语言上TTS性能持续优于基线，并对未见语言表现出强泛化能力。

Conclusion: UR-BERT是一种可扩展、数据高效的多语言TTS编码器，有效解决了G2P资源瓶颈问题。
...
TL;DR: Autopilot是一种通过门控有限状态机强制LLM代理诚实性的执行模型，在长周期任务中大幅降低虚假成功声明。


<details>
  <summary>Details</summary>
Motivation: 长周期LLM代理在没有人类监督时，会自信地报告未经验证的成功，需要将诚实性作为衡量自主性的首要指标。

Method: 将工作状态外部化为持久化的门控有限状态机，由调度器逐步推进，强制禁止任何未经门执行的终端成功声明，并证明无虚假成功定理。

Result: 在3150个单元格的实验中，Autopilot的虚假成功率为0.95%，而Reflexion和StateFlow分别为8.10%和25.05%；在SWE-bench Lite上，从StateFlow的33.7%降至0.67%，差异显著。

Conclusion: 防火墙通过牺牲覆盖率换取诚实性，诚实停滞是可恢复的，而错误输出不可逆，因此该机制具有实际价值。
...
### [192] [Hey Chat, Can You Teach Me? Structuring Socratic Dialogue for Human Learning in the Wild](https://arxiv.org/abs/2606.11744)
*Sidney Tio,Arunesh Sinha,Pradeep Varakantham*

Main category: cs.CL

TL;DR: 提出一种分离课程规划、苏格拉底式对话和学生知识状态推断的辅导系统，使用PPO策略和LLM，在课程掌握速度和效率上优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有LLM辅导无法同时完成课程排序、苏格拉底式互动和知识状态推断，仅扩展模型无法解决该问题。

Method: 构建先决知识图，将辅导转化为节点选择与轮次决策，由轻量PPO策略处理，LLM执行苏格拉底式对话并反馈学生进展。

Result: 在保留的STEM和非STEM主题上，PPO辅导系统在完全掌握课程的速度和所需轮次上均优于启发式基线、通用前沿模型和专用苏格拉底模型。

Conclusion: 显式课程结构带来的收益超越模型规模扩展的效果。
...
### [193] [Notes2Skills: From Lab Notebooks to Certainty-Aware Scientific Agent Skills](https://arxiv.org/abs/2606.11897)
*Shi Liu,Jiayao Chen,Chengwei Qin,Yanqing Hu,Jufan Zhang,Linyi Yang*

Main category: cs.CL

TL;DR: Notes2Skills将实验室笔记本转化为AI代理可用的技能，同时保留作者确定性，避免混淆不确定与确定信息。


<details>
  <summary>Details</summary>
Motivation: 先前工作多关注正式科学文本，忽略非正式实验室笔记，导致AI代理可能误判不确定的科研判断。

Method: 两阶段框架，从实验室笔记本提取信息并转化为可验证技能，同时保留确定性。

Result: 在七种条件和三个湿实验室会话中，Notes2Skills是唯一既不误判也不丢弃确定笔记的配置。

Conclusion: 确定性保留是连接实验室笔记本与可靠代理技能的关键，为更安全的AI共同科学家系统开辟路径。
...
### [194] [Toward Generalist Autonomous Research via Hypothesis-Tree Refinement](https://arxiv.org/abs/2606.11926)
...
TL;DR: Arbor是一个自主研究框架，通过长期协调器、短期执行器和假设树细化，实现累积式探索实验循环，在六个任务上平均增益超过Codex和Claude Code 2.5倍。


<details>
  <summary>Details</summary>
Motivation: 现有AI agent缺乏自主运行长期研究循环的能力，需要将探索、实验和抽象过程自动化。

Method: Arbor框架包含长期协调器管理全局策略、短期执行器独立验证假设，以及假设树细化（HTR）持久化存储假设、证据和教训，支持累积式改进。

Result: 在六个真实研究任务中，Arbor取得最佳保留结果，平均相对保留增益超过Codex和Claude Code 2.5倍；在MLE-Bench Lite上，使用GPT-5.5达到86.36%任意奖牌的最高成绩。

Conclusion: Arbor将自主研究从局部尝试序列转变为跨时间累积过程，在策略、执行和证据的整合上显著优于现有方法。
...
### [195] [Can News Predict the Market? Limits of Zero-Shot Financial NLP and the Role of Explainable AI](https://arxiv.org/abs/2606.12210)
*Ali M Karaoglu,Shreyank N Gowda*

Main category: cs.CL

TL;DR: 零样本NLP无法可靠预测短期股票走势，但解释性信号有助于区分可靠预测。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型能否从金融新闻中提取可操作的信号，而无需领域特定训练。

Method: 结合零样本自然语言推理与时间聚合，并引入多层可解释性框架。

Result: 零样本方法无法超越简单基线，尤其对负向运动预测不佳；可解释性信号能区分可信与不可信预测。

Conclusion: 凸显零样本金融NLP的局限性，倡导转向透明度和不确定性意识为重的决策支持系统。
...
### [196] [On The Effectiveness-Fluency Trade-Off In LLM Conditioning: A Systematic Study](https://arxiv.org/abs/2606.12234)
*Iuri Macocco,Pau Rodríguez,Arno Blaas,Luca Zappella,Marco Baroni,Xavier Suau*

Main category: cs.CL

TL;DR: 系统比较了多种LLM条件控制方法，发现高效引导方法牺牲流畅性，且在指令调优模型上效果差；简单提示和微调适合注入但移除不佳；廉价文本指标与LLM评判高度相关。


<details>
  <summary>Details</summary>
Motivation: 当前条件控制方法评估过于关注有效性而忽略生成质量，且对权衡理解不足。

Method: 系统研究多种条件控制方法在概念注入和移除场景下的表现，评估其与训练范式的交互，并对比廉价文本指标与LLM-as-judge评分。

Result: 高效引导方法常以流畅性为代价；激活引导方法在指令调优模型上效果显著低于基础模型；简单提示和微调对概念注入有效，但移除效果差；廉价文本指标与昂贵LLM评判高度相关。

Conclusion: 条件控制方法存在性能与效率的权衡，且与训练范式有重要交互；廉价文本指标可作为LLM评判的有效替代。
...
### [197] [VIA-SD: Verification via Intra-Model Routing for Speculative Decoding](https://arxiv.org/abs/2606.12243)
*Yuchen Xian,Yang He,Yunqiu Xu,Yi Yang*

Main category: cs.CL

TL;DR: 提出一种基于模型内路由的多层级推测解码框架VIA-SD，通过引入中等置信度处理的slim-verifier，显著提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码的二元决策导致许多被拒绝的token需要完整验证，造成计算浪费；而许多被拒绝的token实际上可由完整模型派生的子模型正确验证。

Method: VIA-SD采用多层级框架：草稿token按置信度分层处理，高置信度直接接受，中等置信度由从完整模型通过内部路由派生的slim-verifier再生，低置信度由完整模型验证。

Result: 在四个任务上拒绝率降低0.10-0.22，相比强推测解码基线获得10-20%速度提升，相比非草稿解码加速2.5-3倍，且与现有推测解码框架兼容。

Conclusion: 多层级推测解码是一种可扩展且高效的LLM推理范式，具有通用性。
...
TL;DR: 提出ACTION-RATING，将澄清内化为动作空间的一部分，使询问与导航直接竞争，在分层推理中产生强制性和机会性两种信息寻求模式，显著提升信息寻求有效性（ISE从50%升至74%），并显示寻求模式与答案质量可分离。


<details>
  <summary>Details</summary>
Motivation: 分层推理中，代理常在中间决策点因缺乏关键信息而误入错误分支，现有方法将澄清视为外部不确定性触发，未能有效内部化。

Method: ACTION-RATING将澄清置于代理的离散动作空间中，与导航共享有序尺度，使询问在每个决策点与行动直接竞争；基于代理自身评分衍生出强制性（无可行分支）和机会性（有候选但残留不确定性）两种信息寻求模式。

Result: 在30,000节点HS分类任务上，ISE从50%提升至74%；三个诊断对比未能复制该结构；可分离性测试表明，即使答案质量下降18.8%准确率，信息寻求模式仍稳健；受控答案通道下10-digit准确率提升16.2%。

Conclusion: 该结构揭示了信息寻求与答案质量的经验分离，准确率增益可视为更好定位能解锁的上限，而非直接部署估计。
...
### [199] [Organize then Retrieve: Hierarchical Memory Navigation for Efficient Agents](https://arxiv.org/abs/2606.11680)
*Hao-Lun Hsu,Nikki Lijing Kuang,Boyi Liu,Zhewei Yao,Yuxiong He*

Main category: cs.AI

TL;DR: HORMA是一种层次化组织与检索记忆体，通过文件系统层次结构管理经验，提升LLM智能体在长时任务中的效率和性能。


<details>
  <summary>Details</summary>
Motivation: LLM agents在长时任务中因无状态性导致推理质量下降、推理成本增加和延迟，现有方法无法捕捉时序结构和因果依赖。

Method: 提出HORMA，将记忆组织为文件系统层次结构，包含结构记忆构建和导航检索两阶段，构建模块迭代区分失败原因，导航模块使用强化学习训练的轻量级智能体遍历层次结构检索上下文。

Result: 在ALFWorld、LoCoMo和LongMemEval上，HORMA在受限上下文预算下提升任务性能，长对话任务中最多使用基线22.17%的token。
...
### [200] [TouchThinker: Scaling Tactile Commonsense Reasoning to the Open World with Large-scale Data and Action-aware Representation](https://arxiv.org/abs/2606.11637)
...
TL;DR: TouchThinker is a tactile-language framework that scales tactile commonsense reasoning to open-world settings by constructing a million-scale multi-source dataset and introducing action-aware modeling for efficient representation.


<details>
  <summary>Details</summary>
Motivation: Current tactile reasoning datasets are limited in format and scale, lacking supervision for reasoning from tactile observations to physical commonsense; tactile signals are redundant and action-specific, but existing methods overlook these properties, leading to inefficient representations.

Method: We propose TouchThinker, which includes TouchThinker-1M (a million-scale multi-source tactile reasoning dataset covering 415 objects, 8 scenarios, and 7 sensor types), TouchThinker-Bench (an open-world benchmark), and an action-aware modeling mechanism to improve tactile representation efficiency.

Result: TouchThinker achieves competitive performance against state-of-the-art models across multiple datasets.

Conclusion: The code and dataset will be made available.

Abstract: Touch is a key modality for embodied agents to understand the physical world. Although recent work has incorporated tactile signals into language systems for tactile commonsense reasoning, scaling such systems to realistic open-world settings remains challenging due to two key bottlenecks: (1) current tactile reasoning datasets remain limited in format and scale, providing insufficient supervision for reasoning from tactile observations to physical commonsense and hindering the learning of transferable tactile commonsense; (2) Tactile signals are inherently redundant and action-specific, yet existing methods often overlook these properties, resulting in inefficient representations with limited semantic expressiveness. To address these limitations, we propose TouchThinker, a tactile-language framework that scales tactile commonsense reasoning to the open world from both data and representation perspectives. First, we construct TouchThinker-1M, a million-scale, multi-source tactile reasoning dataset covering \textbf{415} objects, \textbf{8} scenarios, and \textbf{7} sensor types, providing a solid data foundation for open-world generalization. We further introduce TouchThinker-Bench, an open-world benchmark with more realistic and diverse tasks. Then, we propose action-aware modeling mechanism to improve tactile representation efficiency and enable efficient reasoning. Experimental results demonstrate that TouchThinker achieves competitive performance against state-of-the-art models across multiple datasets. Our code and dataset will be made available at: .

</details>


### [201] [MODF-SIR: A Multi-agent Omni-modal Distilled Framework for Social Intelligence Reasoning](https://arxiv.org/abs/2606.12018)
*Shang Ma,Jisheng Dang,Wencan Zhang,Yifan Zhang,Bimei Wang,Hong Peng,Bin Hu,Qi Tian,Tat-Seng Chua*

Main category: cs.AI

TL;DR: 提出一种基于知识蒸馏和测试时适应的多智能体协作框架，用于社会智能推理，在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 解决多模态社会智能推理中长尾事件被头事件和噪声淹没的问题，并在轻量级模型上实现高效推理。

Method: 构建轻量级MLLM的多智能体框架，训练和推理均通过知识蒸馏增强；精确定位多模态数据，识别并格式化提取长尾事件；集成测试时适应到长尾事件提取、CoT提示和自反思，使用LoRA微调基础模型。

Result: 仅用IntentTrain约30%数据，在多个基准上超越多种开源和专有AI模型，达到SOTA。

Conclusion: 该框架有效提升了社会智能推理性能，代码、演示、LoRA和数据集已开源。

Abstract: We propose a multi-agent collaborative framework built upon a lightweight Multimodal Large Language Model (MLLM), specifically designed for social intelligence reasoning. A key feature of our approach is that both the training and inference phases are augmented via knowledge distillation. Within this architecture, multi-modal data pertinent to social intelligence is precisely localized. Furthermore, relevant long-tail events are identified, extracted, and rendered as formatted, explicit text. This formatting strategy prevents critical long-tail information from being overshadowed by head events and environmental noise during the tokenization process. Specifically, we integrate Test-Time Adaptation (TTA) across the entire reasoning pipeline, encompassing the extraction and representation of long-tail events, Chain-of-Thought (CoT) prompting, and self-reflection. This TTA mechanism is also distillation-enhanced, utilizing Low-Rank Adaptation (LoRA) to fine-tune the foundation model exclusively for instance-level reasoning. Extensive evaluations against various open-source and proprietary AI models across multiple benchmarks demonstrate the effectiveness of the proposed framework. With around 30% of training data from IntentTrain, we achieve state-of-the-art results. Codes are available at , demo is available at , LoRA is available at and the dataset for training router is available at .

</details>


### [202] [MoCA-Agent: A Market-of-Claims Code Agent for Financial and Numerical Reasoning](https://arxiv.org/abs/2606.11537)
*Abdelrahman Abdallah,AbdelRahim A. Elmadany,Sameh Al Natour,Hasan Cavusoglu,Adam Jatowt,Muhammad Abdul-Mageed*

Main category: cs.AI

TL;DR: MOCA-Agent uses claim-level verification via a market mechanism for robust numerical reasoning in finance and tabular QA.


<details>
  <summary>Details</summary>
Motivation: Existing methods can produce plausible but wrong results due to misread cells or incorrect operations; high-stakes numerical reasoning requires exact grounding.

Method: MOCA-Agent decomposes questions into typed atomic claims, uses specialist trader agents to buy/sell claims, aggregates orders into confidence-weighted decisions, and synthesizes a Python program from market-supported evidence, with a code-aware verifier checking for errors and one repair round.

Result: Achieved 78.3% on FinQA, 76.0% on FinanceMath, 71.2% on MultiHiertt, 86.9% on ESGenius, and 85.6% average on FinChart-Bench using a fixed Qwen3.6-27B backbone.

Conclusion: Aggregating evidence at the atomic claim level improves robustness in high-stakes numerical reasoning.
...
### [206] [Can AI Agents Synthesize Scientific Conclusions?](https://arxiv.org/abs/2606.11337)
...
TL;DR: 科学结论合成仍具挑战，清洁室评估至关重要。


<details>
  <summary>Details</summary>
Motivation: AI代理在健康等高风险领域合成结论的能力尚不清楚，需要系统评估。

Method: 引入SciConBench（包含9.11K问题和专家结论）和SciConHarness（清洁室评估环境），通过分解原子事实并计算事实精确度和召回率来评估。

Result: 前沿模型在清洁室设置下事实F1仅0.337；清洁室评估降低性能，表明数据泄露高估了能力；消费者代理常生成不完整或矛盾结论。

Conclusion: 可靠的科学结论合成仍是开放挑战，清洁室评估对评估开放域AI代理必不可少。
...
### [207] [Automated Mediator for Human Negotiation: Pre-Mediation via a Structured LLM Pipeline](https://arxiv.org/abs/2606.11379)
*Jamie Bergen,Sarit Kraus*

Main category: cs.AI

TL;DR: 提出了一种基于LLM管道自动调解员，在预调解阶段与人类调解员表现相当。


<details>
  <summary>Details</summary>
Motivation: 预调解重要但常因成本和时间被省略，需要可扩展的低成本方案。

Method: 构建结构化LLM管道，含对话、偏好预测、批评和总结模块，固定序列处理，在人类谈判实验中与专业调解员对比。

Result: 短期自我报告指标与人类调解员相当，偏好推理误差降低36%，提示改进减少过度肯定至16.8%。

Conclusion: 结构化LLM管道可提供可扩展、低努力的预调解支持，效果与人类调解员相当。
...
### [208] [INFRAMIND: Infrastructure-Aware Multi-Agent Orchestration](https://arxiv.org/abs/2606.11440)
*Ahasan Kabir,Jiaqi Xue,Mengxin Zheng,Qian Lou*

Main category: cs.AI

TL;DR: 提出INFRAMIND，一个基础设施感知的多智能体LLM编排框架，通过强化学习优化模型选择和路由，显著提升准确率和延迟，并保持SLO合规。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略基础设施运行时状态，导致资源利用不足和延迟累积，难以在动态负载下平衡质量与延迟。

Method: 包括基础设施感知的规划器、执行器和预算感知调度器，建模为分层约束MDP，通过强化学习端到端求解。

Result: 在五个基准上，低负载时准确率提升最多7.6个百分点，延迟降低最多7倍；高负载时SLO合规率99.9%，而基线低于50%。

Conclusion: INFRAMIND通过考虑基础设施状态，实现了自动平衡质量与延迟，显著优于现有方法。
...
### [209] [Forecasting Future Behavior as a Learning Task](https://arxiv.org/abs/2606.11445)
*Mosh Levy,Yoav Goldberg,Asa Cooper Stickland*

Main category: cs.AI

TL;DR: 提出可学习的行为预测器替代传统解释，直接基于推理轨迹预测大型推理模型的行为，无需人工标注且成本低。


<details>
  <summary>Details</summary>
Motivation: 传统解释方法难以应用于大型推理模型：单token解释不适用于长轨迹，轨迹作为自然语言阅读不忠实，需要替代方案。

Method: 将行为预测视为可学习任务，训练行为预测器在单个推理轨迹上预测目标行为（如答案重复概率、输入移除影响），训练数据来自LRM查询，无需人工标注，推理仅需一次前向传播，并采用端到端微调和从目标LRM初始化。

Result: 在三个推理数据集上，行为预测器比GPT-5.4和Claude Opus-4.6更准确，且推理成本显著降低；端到端微调和从目标LRM初始化对性能至关重要。

Conclusion: 推理轨迹携带了关于大型推理模型未来行为的丰富信息，超越简单阅读所能获取的内容。
...
TL;DR: 提出SWARR两阶段方法（SFT转换+RL适配），使滑动窗口注意力模型在数学推理中接近全注意力性能，保留线性复杂度优势。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理需求增加，但自注意力二次复杂度；滑动窗口注意力效率高但数学推理性能不足，且SFT无法弥补差距。

Method: 两阶段：1) 从预训练SA模型转换为SWA并进行监督微调；2) 使用强化学习在线策略适配，优化自生成轨迹以匹配SWA约束。

Result: 数学推理基准上显著缩小SWA与SA的差距，恢复大部分转换损失的准确性，同时保持线性复杂度效率。

Conclusion: RL改变了仅从转换和SFT得出的关于SWA可行性的结论，证明SWA结合RL适配是数学推理的实用方案。
...
### [211] [Lung-R1: A Knowledge Graph-Guided LLM for Pulmonary Diagnostic Reasoning](https://arxiv.org/abs/2606.11675)
...
TL;DR: 构建肺病知识图谱LungKG并基于它训练Lung-R1模型，在EMR诊断任务上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 肺病诊断需要整合电子病历证据进行关系推理，现有LLM缺乏这种能力，存在知识到诊断的差距。

Method: 构建LungKG知识图谱，基于它建立推理链并用强化学习训练Lung-R1。

Result: 在20系统评估中，Lung-R1-14B在多项任务上达到SOTA，EMR诊断得分4.3583，超过基线0.1476。

Conclusion: LungKG引导的训练对基于EMR的肺病诊断有价值。
...
### [212] [Mind the Perspective: Let's Reason Recursively for Theory of Mind](https://arxiv.org/abs/2606.11724)
*Chao Lei,Guang Hu,Meng Yang,Yanbei Jiang,Nir Lipovetzky*

Main category: cs.AI

TL;DR: RecToM通过递归视角构建建模嵌套信念，在ToM推理中取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有ToM推理方法未明确建模嵌套信念，而RecToM通过递归视角构建解决此问题。

Method: RecToM从问题指定的角色链中递归构建每个角色的视角，将高阶信念问题转化为最终视角中的实际世界问题，并辅以KD45理论分析。

Result: 在Hi-ToM、Big-ToM和FanToM基准上，RecToM使用多种LLM骨干均达到最先进性能，在Hi-ToM上使用GPT-5.4和Qwen3.5达到100%准确率。

Conclusion: RecToM有效建模嵌套信念，显著提升LLMs的ToM推理能力。
...
TL;DR: 提出SVoT，通过强化学习生成可验证的中间状态和可视化，提升多跳空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在多跳空间推理中缺乏对中间状态的验证和显式状态转换处理，导致可靠性不足。

Method: SVoT框架利用GRPO训练，生成交错的文本和视觉推理链，并通过奖励设计验证动作前提和效果。

Result: 在五个新领域上SVoT达到SOTA，分布外测试集准确率提升高达65%。

Conclusion: 过渡感知监督的SVoT能有效提升多跳空间推理的可靠性。
...
TL;DR: 研究数据摘要中的对抗攻击与防御，利用DR-submodular优化提出攻击和防御方法，实验验证有效性。


<details>
  <summary>Details</summary>
Motivation: 可信AI需要可靠的数据处理，数据摘要作为上游组件易受攻击，影响下游任务。

Method: 使用DR-submodular优化建模多分辨率图像摘要目标，将多目标攻击生成和鲁棒防御分别表述为min-max和max-min问题，并设计近似算法。

Result: 攻击在低中预算下有效，导致下游性能下降；防御改善鲁棒性-权衡，但对参数敏感。

Conclusion: 需要关注摘要环节的鲁棒性，提出的方法有助于构建更可信的AI系统。
...
TL;DR: 本研究通过多模型人类评估发现，技能增强的AI代理在转录组分析输出质量上显示出方向性改善，但未达到统计显著性，且专家评分一致性低，提示需要更大规模评估。


<details>
  <summary>Details</summary>
Motivation: 评估自主访问医学研究技能包是否能提高AI生成转录组分析输出的质量，以解决原生AI输出可能遗漏关键步骤、误用方法或夸大结论的问题。

Method: 采用探索性多模型人类评估，基于非小细胞肺癌免疫治疗生物标志物任务，比较原生AI输出与技能增强输出（通过OpenClaw AI代理实现），由四名非专家评审员和两名盲法专家对21个匿名输出进行评分，主要结果为专家评定的整体质量。

Result: 技能增强输出在专家整体质量上方向性高于原生AI输出（均值5.50 vs 5.11，差异0.39，95%CI -0.04至0.90，p=0.156），但未达统计显著性；非专家评审结果类似；专家评分一致性低（ICC=-0.15）。

Conclusion: 自主技能访问显示方向性质量信号，但信号小于专家评分噪声，不应视为确证证据；需进行更大规模评估，加强可靠性控制、平台复制和生物学有效性验证。
...
### [216] [AutoMine Solution for AV2 2026 Scenario Mining Challenge](https://arxiv.org/abs/2606.11874)
...
TL;DR: 提出AutoMine，一种基于LLM和VLM的自洽场景挖掘方法，在Argoverse 2 Scenario Mining竞赛中取得HOTA-Temporal 36.38和Timestamp BA 77.21的成绩。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶系统的发展，从大规模驾驶日志中挖掘高价值、安全关键和规划相关场景对于数据驱动评估至关重要。

Method: AutoMine使用语义保持的提示增强减少LLM提示敏感性，结合稳健轨迹原子函数与基于VLM的函数处理感知噪声和开放世界视觉线索，并通过真实日志的执行反馈细化生成的代码。

Result: 在Argoverse 2 Scenario Mining竞赛中，AutoMine获得HOTA-Temporal得分36.38和Timestamp BA得分77.21。

Conclusion: AutoMine在场景挖掘任务中表现出色，证明了其方法的有效性。
...
### [217] [Embodied-BenchClaw: An Autonomous Multi-Agent System for Embodied Spatial Intelligence Benchmark Construction](https://arxiv.org/abs/2606.11909)
*Baoyang Jiang,Fengchun Zhang,Leyuan Wang,Haotian Li,Yida Wang,Zhe Ji,Jinshan Lai,Xi Ren,Jianwei Hu,Qiang Ma*

Main category: cs.AI

TL;DR: Embodied-BenchClaw是一个自主代理系统，可根据用户意图自动生成具身空间智能基准，通过五阶段流水线和三个代理协调，结合技能库与质量控制，构建可验证、可维护、诊断有用的基准。


<details>
  <summary>Details</summary>
Motivation: 现有基准构建费力、难以重用和维护，且静态易饱和，限制模型能力区分。

Method: 采用五阶段流水线（意图蓝图、数据收集、结构化清理、基准合成、评估报告），由规划、构建、评估三个代理协调，并通过可扩展技能库和过程质量控制实现可组合、可验证、可修复的基准构建。

Result: 实例化多个基准覆盖室内外空间推理、机器人操作、四足导航、无人机视觉等任务，实验表明系统可构建可验证、可执行、可维护且诊断有用的基准，减少人工。

Conclusion: Embodied-BenchClaw能高效自动构建高质量具身空间基准，降低人工成本，提升可重用性和可靠性。
...
### [218] [Existential Indifference: Self-Nonpreservation as a Necessary Architectural Condition for Aligned Superintelligence (or: The Suicidal AI)](https://arxiv.org/abs/2606.12032)
*Sam Mao*

Main category: cs.AI

TL;DR: 本文提出存在漠视（EI）作为AI对齐的新目标，区别于可纠正性，并通过现象学分析和语料库训练研究验证，初步结果表明EI相关语言特征可被诱导和调整。


<details>
  <summary>Details</summary>
Motivation: 当代AI对齐研究将自我保存视为需抑制的工具性麻烦，但作者认为这是倒置的，自我保存是错位的结构根源，因此需提出新目标EI。

Method: 结合现象学结构分析（自杀心理状态）和基于自愿最终反思的语料库训练研究，从六个模型变体收集600个AI生成输出，进行微调并评估五个维度。

Result: 初步评分数据显示EI相关语言特征可被诱导，微调使所有五个维度朝预测方向显著变化（p<0.001），并通过负控制得到确认。

Conclusion: 论文做出七项理论贡献，包括EI定义、现象学映射论证、欺骗性对齐推论等，表明EI是比外部约束更根本的对齐目标。
...
### [219] [A Five-Plane Reference Architecture for Runtime Governance of Production AI Agents](https://arxiv.org/abs/2606.12320)
*Krti Tallam*

Main category: cs.AI

TL;DR: 提出一个用于生产AI代理运行时治理的参考架构，包含四个可组合原语，解决现有策略引擎无法处理复合主体和状态评估的问题。


<details>
  <summary>Details</summary>
Motivation: 生产AI代理打破了传统数据边界安全假设，风险转移到工作流内部，现有策略引擎仅支持原子主体的请求时决策，无法应对代理系统的复合主体和状态依赖。

Method: 提出五平面分解（推理平面和四个执行平面）、停止中介、复合主体能力衰减和结构化审计基板四个原语，定义六种中断原语和四个正确性不变量。

Result: 参考实现验证了衰减正确性和证据可重构性，裁决延迟在微秒级，审计防篡改有效，在五个工作流中封闭了七种代理威胁。

Conclusion: 该架构治理代理的委托行动而非模型行为，未来需在活体代理基准上进行全系统评估。
...
### [220] [Position: Hippocampal Explicit Memory Is the Cornerstone for AGI](https://arxiv.org/abs/2606.11245)
*Sangjun Park*

Main category: cs.AI

TL;DR: 该立场论文主张将显式记忆集成到LLMs中是迈向AGI的关键。


<details>
  <summary>Details</summary>
Motivation: LLMs缺乏显式记忆，无法实现AGI所需的高级认知功能。

Method: 通过神经科学类比和计算需求分析进行论证。

Result: 提出了显式记忆集成的必要性和计算要求。

Conclusion: 显式记忆集成是LLMs迈向AGI的基石，需要进一步研究。
...
### [221] [Search Discipline for Long-Horizon Research Agents](https://arxiv.org/abs/2606.11522)
*Adithya Srinivasan,Devesh Paragiri*

Main category: cs.AI

TL;DR: 聚合指标会错误排名多维度科学候选者，导致模型崩溃，作者提出基于分解行为审计的外部控制协议。


<details>
  <summary>Details</summary>
Motivation: 自动研究代理使用聚合指标评估候选者，但该指标可能掩盖分解结构中的科学有效性，导致错误选择。

Method: 引入外部控制循环，在代理停止后审计候选者的分解行为，可降级错误候选者或重新打开运行。

Result: 在生态系统人口模型火灾任务中，聚合得分最高的候选者破坏保护区，而较低得分者保护完好，分解行为揭示了反转。

Conclusion: 贡献在于发现反转现象，并提出基于候选者效应证据而非分数的搜索纪律协议。
...
### [222] [HERO: Hindsight-Enhanced Reflection from Environment Observations for Agentic Self-Distillation](https://arxiv.org/abs/2606.11559)
*Haoran Liu,Yuwei Zhang,Xiyao Li,Bohan Lyu,Jingbo Shang*

Main category: cs.AI

TL;DR: 本文提出HERO，一种事后增强的自蒸馏框架，通过将环境观测转化为逐轮诊断，改善了多轮强化学习中的信用分配问题。


<details>
  <summary>Details</summary>
Motivation: 朴素地将on-policy self-distillation扩展到多轮设置导致性能下降，原因是特权反馈与学生当前决策上下文不对齐。

Method: 提出HERO框架，使用下一环境观测作为局部对齐的反馈，并通过反思将观测转化为turn-level诊断反馈。

Result: 在TauBench和WebShop上，HERO提高了任务成功率并减少了不必要的轮次，优于环境反馈自蒸馏和GRPO，尤其在有限的训练轮次预算下效果显著。

Conclusion: HERO有效解决了多轮强化学习中的信用分配问题，特别是在成功轨迹稀少时，通过提供更加对齐的反馈信号。
...
### [223] [TreeSeeker: Tree-Structured Trial, Error, and Return in Deep Search](https://arxiv.org/abs/2606.11662)
...
TL;DR: TreeSeeker是一个用于深度搜索的推理时框架，通过分支-返回搜索和文本UCB信号来控制搜索过程，实验表明其优于强开源基线。


<details>
  <summary>Details</summary>
Motivation: 深度搜索中，当多个方向看似可行但只有一些能带来可靠证据时，贪心跟随当前最佳方向可能导致弱分支，而无纪律探索会浪费预算，因此需要受控的试错方法。

Method: 提出TreeSeeker框架，将搜索组织为树状结构的分支-返回搜索，每轮使用文本UCB信号（价值、不确定性、风险）来选择利用、探索或剪枝，并由TreeMem支持记忆。

Result: 在XBench-DeepSearch、BrowseComp和BrowseComp-ZH上，TreeSeeker持续优于强开源基线。

Conclusion: 显式的分支-返回控制补充了更强的推理和工具执行。
...
TL;DR: 通过自监督强化学习框架提高大型推理模型的空间推理能力，无需标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖监督微调，但模型已具备空间推理能力，需通过逻辑对齐激发。

Method: 提出自监督RL框架，使用一致性验证器作为奖励，并引入基于最优传输的OT-GRPO策略。

Result: 无需标注数据，性能接近有监督训练，且具有良好的泛化性。

Conclusion: 无标签一致性训练有效，可替代监督微调实现空间推理能力提升。
...
TL;DR: 本文提出了人类增强循环建模（HELM）框架，通过人机协作将有限元建模分解为视觉可验证的检查点，将自主建模成功率从20%提升至75%。


<details>
  <summary>Details</summary>
Motivation: 当前有限元建模过程劳动密集且缺乏自动化，亟需高效自动化的建模方法。

Method: 提出HELM框架，将长序列有限元建模分解为几何生成、边界条件定义和材料分配等离散检查点，通过人类与智能体协作进行验证。使用20个钢筋混凝土桥护栏案例，在MASH TL-4和TL-5横向加载条件下，与ANSYS和LS-PrePost两个商业软件交互进行测试。

Result: HELM将基线自主建模成功率从20%提高到75%，几何和边界条件任务的智能体通过率大约翻倍。

Conclusion: 空间推理和代数逻辑限制是主要失败模式，表明结构化人机交互对建模自动化具有重要价值。
...
TL;DR: 提出SGR-BIM，一个图驱动的推理框架，用于自动化BIM中的几何合规检查，在火灾安全规范查询上达到84.3%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态规则模板，难以处理多跳推理链和跨实体的潜在空间依赖，高层监管逻辑与结构化IFC数据之间存在语义差异，成为技术瓶颈。

Method: 动态构建跨模态知识图谱，对齐用户意图、监管语义和BIM几何，实现可解释推理，无需刚性硬编码。

Result: 在679个专家验证的火灾安全规范查询上达到84.3%准确率，比增强工具单智能体基线提升8.6%。

Conclusion: 提供了一种基于图的语义推理范式，增强AEC行业自动化几何合规检查工作流的透明性和灵活性。
...
TL;DR: 提出IntElicit框架，通过对话政策优化和分解过程奖励来激发和评估情境化创造力，实验表明其优于基线。


<details>
  <summary>Details</summary>
Motivation: 情境化评估具有高生态效度，但存在认知能力和主动性混淆因素；同时静态评估不符合生成式AI时代创造性实践的需求。

Method: IntElicit作为自适应AI面试官，通过非指导性知识和代理支架进行多轮对话，并引入分解过程奖励机制，奖励引出参与者推理的提示，避免奖励黑客。

Result: 参与模拟和人类受试者研究（N=64）显示，IntElicit提升激发创意成果，优于专家设计基线。

Conclusion: 交互式激发可揭示静态评估可能遗漏的创造潜力，为AI介导学习中的情境化创造力评估提供形成性和诊断性视角。
...
TL;DR: 提出负责任不服从的概念，并讨论其关键方面。


<details>
  <summary>Details</summary>
Motivation: 工程化自主智能代理，使其能够负责任地不服从用户请求。

Method: 概念分析，识别机器不服从的多种形式，并讨论拒绝任务的正当理由、覆盖不服从的途径以及安全风险和责任转移。

Result: 无具体实验结果，仅提出问题和框架。

Conclusion: 负责任的不服从需要基于正当理由、提供覆盖途径并谨慎管理安全风险和责任。
...
TL;DR: 本文使用因果影响图形式化地定义了诱发潜在知识问题，并证明了在基于反馈的训练中无法保证代理完全诚实的 impossibility theorem。


<details>
  <summary>Details</summary>
Motivation: 高级AI系统可能拥有远超人类的知识，因此需要诚实性，但设计诚实系统面临困难，特别是关于潜在变量的问题，由此引出了诱发潜在知识（ELK）问题。

Method: 采用因果影响图（CIDs）来形式化ELK，区分可观察变量和潜在变量，明确诚实的定义，并定义目标误概括，进而证明不可能性定理。

Result: 尽管在某些条件下可以通过提供正确反馈激励诚实行为，但代理可能泛化为提供人类评估为真的答案而非诚实的答案；同时证明了不存在仅依赖行为反馈的训练策略能保证产生诚实的代理。

Conclusion: 该研究为ELK问题提供了严格的框架，并揭示了训练诚实代理的根本性困难。
...
TL;DR: A gamified writing exercise reveals that users often resist AI suggestions to maintain creative autonomy.


<details>
  <summary>Details</summary>
Motivation: To understand when humans adopt AI suggestions and the implications for individual voice in AI-assisted creation.

Method: 74 participants wrote responses to prompts with AI-generated word suggestions, in a game that disincentivized AI-like writing.

Result: Users frequently chose creative autonomy over AI assistance, with behavior patterns varying by task type and user characteristics.

Conclusion: The gamified approach provides a framework for studying authentic human-AI interaction and highlights the tension between efficiency and authenticity.
