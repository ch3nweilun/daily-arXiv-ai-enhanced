...
### [1] [LLM-Guided ANN Index Optimization for Human-Object Interaction Retrieval](https://arxiv.org/abs/2606.05489)
*Shahrzad Esmat,Chaunte W. Lacewell,Sameh Gobriel,Nilesh Jain,Ali Jannesari*

Main category: cs.CV

TL;DR: 提出一种相位感知的大语言模型智能体，用于优化多阶段检索系统中高度耦合的参数，在HICO-DET基准上比Optuna TPE和VDTuner分别提升33.3%和34.2%，吞吐量增益达15.3倍。


<details>
  <summary>Details</summary>
Motivation: 传统超参数优化方法（如TPE和高斯过程贝叶斯优化）假设参数独立，无法应对多阶段检索系统中耦合参数空间的联合优化。

Method: 设计相位感知LLM智能体，利用完整优化历史在分阶段（探索、利用、微调）的配置空间中依次提议参数，实现耦合参数的有效导航。

Result: 在HICO-DET上，SIEVE指标下比Optuna TPE提升33.3%，比VDTuner提升34.2%，吞吐量较UniIR提升15.3倍；在GLDv2和SIFT1M上优势随耦合度降低而减小；跨系统（Milvus）验证无需修改即排名第一。

Conclusion: 该方法有效解决耦合参数优化问题，且优化器可跨向量数据库管理系统平台迁移。
...
### [2] [Disentangled Fine-Grained Prototype Learning for Incomplete Image-Tabular Classification](https://arxiv.org/abs/2606.05455)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过全局token平均特征的解耦和对齐学习模态不变表示，仅捕获粗粒度跨模态一致性，忽略了细粒度语义和分布对齐，无法充分利用互补信息。

Method: 包括共享-特定原型建模（SSPM）提取紧凑多样的共享和特定模态原型并进行原型级解耦；原型引导的细粒度对齐（PFA）联合进行原型级分布匹配和原型到类语义对齐；类感知多尺度聚合（CMA）自适应聚合全局和原型级共享语义与特定模态特征。

Result: 在三个不同的图像-表格基准测试上，在各种缺失模态设置下，该方法优于先前方法。

Conclusion: DFPL通过细粒度原型学习有效解决了跨模态语义和分布对齐问题，在缺失模态设置下实现了更鲁棒的多模态学习。
...
### [3] [Beyond Absolute Scores: Relative Edit-induced Difference for Generalizable Image Aesthetic Assessment](https://arxiv.org/abs/2606.05778)
*Qifei Jia,Xintong Yao,Minghao Li,Yajie Chai,Qiming Lu,Baoyue Shen,Yasen Zhang,Runyu Shi,Ying Huang,Yue Zhang*

Main category: cs.CV

TL;DR: RED-Aes uses relative aesthetic differences via image editing to improve IAA generalization.


<details>
  <summary>Details</summary>
Motivation: Traditional IAA methods overlook dynamic aesthetic perception and lack causal reasoning, limiting generalizability.

Method: Proposes RED-Aes framework with editing-based image pairs, RED-20k dataset, and three-stage training using relative supervision.

Result: Achieves state-of-the-art on multiple IAA benchmarks with superior generalization.

Conclusion: RED-Aes demonstrates that relative aesthetic reasoning improves IAA generalization and achieves state-of-the-art performance.
...
### [4] [Towards One-to-Many Temporal Grounding](https://arxiv.org/abs/2606.06294)
*Qi Xu,Yue Tan,Shihao Chen,Jiahao Meng,Anna Wang,Shunping Ji,Hao Fei,Jason Li*

Main category: cs.CV

TL;DR: 提出针对One-to-Many Temporal Grounding的系统解决方案，包含新基准、数据集和奖励函数，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要聚焦单段检索，无法处理现实中的多段定位需求，且MLLMs在OMTG上表现极差，缺乏事件基数感知。

Method: 建立首个OMTG基准及C-Acc、EtF1指标；构建56k高质量OMTG数据集；设计时间与标题奖励函数，其中标题奖励利用密集视频标题的链式思维推理优化策略。

Result: 在OMTG Bench上达到43.65% EtF1，超过Gemini 2.5 Pro和Seed-1.8分别15.85%和15.61%。

Conclusion: 该方法在OMTG任务上取得新SOTA，验证了方案的有效性。
...
TL;DR: 提出KV-Control，一种紧凑的注意力侧控制接口，通过将几何约束作为记忆注入自注意力层，实现精确轨迹控制而无需修改预训练模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么复制生成器以获取控制，要么在测试时优化，导致计算成本高或控制不精确。需要一种轻量、精确且保持文本驱动运动质量的控制方法。

Method: 提出KV-Control，包括PartVQ学习解剖对齐的部分码本，T-Concat暴露每个帧-部分标记作为注意力可寻址点，KV-Control在每层自注意力中注入控制条件的关键/值记忆，保持预训练查询流、文本交叉注意力等权重不变。

Result: 在保持文本条件运动质量的同时，实现了亚厘米级的根和多关节约束跟踪精度，仅添加少量可训练参数。

Conclusion: KV-Control将轨迹条件重新定义为轻量记忆检索，为文本到运动生成提供了一个小型、精确且透明的控制接口。
...
### [7] [MemoryCard: Topic-Aware Multi-Modal Clue Compression for Long-Video Question Answering](https://arxiv.org/abs/2606.05917)
*Qing Yang,Pengcheng Huang,Xinze Li,Zhenghao Liu,Yukun Yan,Yu Gu,Ge Yu,Gang Li,Maosong Sun*

Main category: cs.CV

TL;DR: MemoryCard是一种基于视频记忆的增强框架，通过将长视频分割为自包含的记忆卡（包含事件级摘要和代表性视觉时刻），以事件级语义单元进行检索和问答，显著提升长视频问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有帧中心方法依赖孤立帧作为基本证据单元，难以有效捕捉连贯的事件级语义，而长视频中答案相关证据稀疏、短暂且时间分散。

Method: 首先对视频和对齐语句进行自阅读过程，将其分割为语义连贯的单位（每个对应不同主题或事件）；对每个单位生成事件级视频摘要并选择代表性视觉时刻，然后渲染为统一的记忆卡，用于检索和问答。

Result: 在可比视觉令牌预算下，MemoryCard持续提升长视频问答性能，相对准确率提升最高达21.8%。

Conclusion: MemoryCard通过组织长视频为事件级记忆卡，有效克服了帧中心方法的局限性，验证了视频记忆增强在长视频问答中的有效性。

Abstract: Long-video question answering remains challenging for Vision-Language Models (VLMs), as answer-relevant evidence is often sparse, transient, and temporally dispersed across lengthy video contexts. Existing frame-centric approaches improve efficiency through uniform sampling, query-aware frame selection, visual-token compression, and adaptive resolution strategies. However, they still rely on isolated and fragmented frames as the fundamental evidence units, limiting VLMs' ability to effectively capture coherent event-level semantics. To address this limitation, we propose MemoryCard, a video-memory-based augmentation framework that organizes long videos into self-contained Memory Cards. Specifically, MemoryCard first performs a self-reading process over videos and aligned utterances to segment the video into semantically coherent units, each corresponding to a distinct topic or event. For each unit, it generates an event-level video gist and selects representative visual moments, which are then rendered into unified Memory Cards for retrieval and question answering. Experimental results demonstrate that MemoryCard consistently improves long-video QA performance under comparable visual-token budgets, achieving up to a 21.8% relative improvement in accuracy. All code is available at .

</details>


### [8] [LongSpace: Exploring Long-Horizon Spatial Memory from Perception to Recall in Video](https://arxiv.org/abs/2606.05677)
*Shiqiang Lang,Jing Liu,Haoyang He,Peiwen Sun,Yuanteng Chen,Tao Liu,Lan Yang,Longteng Guo,Honggang Zhang*

Main category: cs.CV

TL;DR: 提出LongSpace-Bench基准和LongSpace记忆框架，用于评估和提升多模态大语言模型的长视频空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 长时任务如自动驾驶需要模型记忆和检索之前观察到的空间布局，但现有MLLMs在长视频空间记忆方面不足。

Method: LongSpace将长视频建模为序列块，将3D结构线索融入早期解码器层，构建层感知记忆用于问题引导检索。

Result: 在多个空间推理基准上，LongSpace提高了长视频空间理解。

Conclusion: 明确的空间记忆是长视频MLLMs的关键能力。
...
### [9] [A Vision-language Framework for Comparative Reasoning in Radiology](https://arxiv.org/abs/2606.06407)
*Tengfei Zhang,Ziheng Zhao,Lisong Dai,Xiaoman Zhang,Pengcheng Qiu,Ya Zhang,Yanfeng Wang,Weidi Xie*

Main category: cs.CV

TL;DR: 提出实体感知的跨图像推理框架MedReCo和MedReCo-VLM，用于放射学比较，通过大规模资源MedReCo-DB训练，在检索和比较生成任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前医学影像AI在孤立图像解释上表现良好，但与放射学实践不匹配，因为诊断和随访依赖于跨先前研究和参考案例的比较。

Method: 构建MedReCo-DB，包含来自多个机构和模态的69万张图像，并将报告分解为解剖结构、异常发现和病理条件。开发MedReCo（实体感知视觉编码器用于可控检索）和MedReCo-VLM（视觉-语言扩展用于生成性比较解释）。

Result: MedReCo在12个内部检索设置中Recall@1最高，外部检索平均提高6个百分点；在临床混淆组中优于基线。MedReCo-VLM在比较生成评估中最佳，胸片随访准确率提高14.5-46.5个百分点，CT提高13.0-27.9个百分点。

Conclusion: 实体感知的对比推理可以从常规临床数据中大规模学习，为医学影像AI提供更临床对齐的基础。
...
TL;DR: LightVesselNet是一种高效的视网膜血管分割网络，仅75K参数即可达到与大型模型竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型虽然分割精度高，但计算资源需求大，难以在边缘设备部署。

Method: LightVesselNet采用紧凑的编码器-解码器架构，包含通道和空间注意力机制、瓶颈处的多尺度特征聚合模块、子像素上采样策略以及边缘残差连接。

Result: 在DRIVE、STARE、CHASEDB1、FIVES和HRF数据集上，灵敏度分别为0.8189、0.8499、0.8640、0.8634、0.8096，Dice系数分别为0.8070、0.8072、0.8181、0.8649、0.7686。跨数据集评估证实了泛化能力。

Conclusion: LightVesselNet是低资源临床环境和移动筛查工具的理想选择。
...
### [16] [Robust Scene Transfer for PointGoal Navigation via Privileged Sensor Guided Contrastive Learning](https://arxiv.org/abs/2606.05506)
*Amirhossein Zhalehmehrabi,Tiziano Tezze,Alberto Castelini,Alessandro Farinelli*

Main category: cs.CV

TL;DR: 提出传感器引导自适应对比学习框架，通过LiDAR指导视觉表示预训练，实现PointGoal导航仅用RGB的跨场景迁移。


<details>
  <summary>Details</summary>
Motivation: 视觉表示学习常捕获场景外观等无关特征，导致导航策略在域迁移时性能下降，需学习导航相关结构。

Method: 利用特权LiDAR传感设计几何感知相似度和自适应温度缩放指导对比学习；独立预训练冻结编码器；引入跨阶段域不匹配抑制环境捷径。

Result: 高保真仿真中，跨室内外场景迁移显著优于大型预训练视觉模型和标准对比基线。

Conclusion: 框架部署仅需单目RGB与标准输入，无需LiDAR，并发布多模态数据集促进研究。
...
TL;DR: 提出FEPBench基准，通过细粒度原子集注释和多维度评估，发现当前T2I模型在科学插图生成中存在文本渲染、推理丰富性等不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准评估过于整体，忽视细粒度元素，且科学推理能力和输出简洁性未量化。

Method: 从多学科高质量科学插图中选取构建基准，借助多模态大语言模型和人类专家提供细粒度原子集注释，从指令忠实度、推理丰富性和语义精确度三个维度评估。

Result: 即使SOTA闭源模型如GPT Image 2和Nano Banana Pro仍存在文本渲染瓶颈、推理丰富性有限、难以平衡生成丰富性与精确性。

Conclusion: 为改进和部署T2I模型在科学插图生成中提供实践指导，并将发布基准数据、原子集注释和评估代码。
...
### [20] [Video-Rate Streaming Stylization on a Vision-Aware MLLM-Conditioned Edit Diffusion: Asymmetric Batched Inference on a Distilled UNet + MLLM Text Encoder](https://arxiv.org/abs/2606.05981)
*Yoshiyuki Ootani*

Main category: cs.CV

TL;DR: A streaming pipeline for vision-aware edit diffusion with a 0.39B distilled U-Net and 2.13B MLLM text encoder, using asymmetric pipelining, fused ControlNet-LLLite, and periodic conditioning refresh, achieves over 27 FPS on RTX 3090 Ti.


<details>
  <summary>Details</summary>
Motivation: After aggressive distillation of the diffusion U-Net to 4-step or 1-step, the text encoder (a 2.13B MLLM) becomes the critical path in real-time text-to-image pipelines, especially for vision-aware edit diffusion.

Method: Asymmetric side-stream/main-stream CUDA pipelining with batched text-encoder amortization and optional static-prompt caching; a compile-friendly ControlNet-LLLite reformulation that fuses the entire U-Net and adapter stack into a single graph; a periodic conditioning-refresh schedule with a hook subset to amortize per-frame conditioning cost.

Result: On a single RTX 3090 Ti at 512x512, the pipeline sustains 27.4 fps at B=8 and 29.6 fps at B=16 over 480 frames, with p50 latency 0.5/1.0 seconds. On RTX 4090: 54.9 fps; on RTX 5090: 74.1 fps.

Conclusion: The pipeline achieves video-rate streaming throughput, not interactive low latency; the numbers are presented as systems context against same-stack StreamDiffusion re-runs. The temporal adapter generalizes within in-clip noise to unseen DAVIS-2017 sequences and non-DAVIS clips, with bounded prompt-level generalization to unseen style families.
...
### [21] [Deep Learning-based 3D Oral Cavity Reconstruction Using 2D Intraoral Images](https://arxiv.org/abs/2606.05998)
*Jihun Cho,Soo-Yeon Jeong,Eun-Jeong Bae,Sun-Young Ihm*

Main category: cs.CV

TL;DR: 提出一种仅用10张2D口内图像重建3D口腔模型的软件方法，使用MobileNetV2和多头注意力，准确率77.49%，但存在点分布不均匀问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D口腔建模方法（如印模和口内扫描）存在患者不适、材料变形、设备成本高等问题，需要一种低成本、无创的解决方案。

Method: 采用MobileNetV2作为图像编码器，结合多头注意力进行多视图特征融合，在Dental3DS数据集上训练。

Result: 准确率达77.49%（最近邻匹配，距离阈值0.035），但预测顶点集中在高密度区域，导致点分布不均匀。

Conclusion: 该方法降低了成本和患者不适，实现了自动3D重建，但点分布不均匀问题需进一步改进。
...
### [22] [Comparison of Deep Learning Frameworks For Rice Disease Mapping From UAV Multispectral Imaging](https://arxiv.org/abs/2606.06359)
*Yadav Raj Ghimire,Jagrati Talreja,Tewodros Syum Gebre,Timothy Agboada,Shikha V. Chandel,Leila Hashemi Beni*

Main category: cs.CV

TL;DR: 利用UAV多光谱图像和深度学习模型进行水稻细菌性叶枯病严重程度分割，U-Net++ with EfficientNet-B3表现最佳，mIoU达97.62%。


<details>
  <summary>Details</summary>
Motivation: 需要高效准确的病害分割方法，比较CNN与transformer模型在UAV多光谱数据上的表现，评估植被指数的影响。

Method: 使用UAV多光谱图像，三种输入配置（仅多光谱、多光谱+NDVI、多光谱+NDRE），训练U-Net、U-Net++、DeepLabV3+、SegFormer等模型，在BLB数据集上评估。

Result: U-Net++ with EfficientNet-B3取得最高mIoU 97.62%，SegFormer精度较低但速度相当，CNN轻量骨干更可靠，植被指数带来小但稳定的改进。

Conclusion: 轻量级CNN架构更适合田间BLB监测，标准化UAV数据集对比较方法有价值。
...
### [23] [Biomazon: A Multimodal Dataset for 3D Forest Structure and Biomass Modeling in the Amazon Basin](https://arxiv.org/abs/2606.05368)
*Sayan Mandal,Rocco Sedona,Simon Besnard,Mikhail Urbazaev,Morris Riedel,Ehsan Zandi,Gabriele Cavallaro*

Main category: cs.CV

TL;DR: 提出了Biomazon，一个用于联合预测GEDI RH剖面和AGBD的多模态基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅预测单独标量目标，缺乏学习森林垂直结构作为有序剖面的基准，且缺乏多模态基准。

Method: 构建了覆盖亚马逊盆地的20米多模态基准数据集Biomazon，使用共享编码器-解码器基线框架进行消融研究，评估骨干模型规模、模态贡献和辅助嵌入。

Result: 展示了单目标和联合目标性能，并通过与现有网格产品的区域对齐比较来背景化基线性能。

Conclusion: Biomazon及其协议和基线建立了参考基准，促进结构一致RH剖面预测和结构-生物量建模。
...
### [24] [Would you still call this Dax? Novel Visual References in VLMs and Humans](https://arxiv.org/abs/2606.05409)
*Ada Defne Tür,Gaurav Kamath,Joyce Chai,Siva Reddy,Benno Krojer*

Main category: cs.CV

TL;DR: 提出了新视觉参考数据集（NVRD），用于研究视觉-语言模型在面对与先验知识矛盾的新视觉概念时的学习能力。发现模型难以获取矛盾概念且过度泛化。


<details>
  <summary>Details</summary>
Motivation: 现有研究较少关注视觉语言模型在接触新视觉概念后如何映射到语言，特别是当这些概念与预训练先验知识矛盾时。

Method: 构建了包含19,176张图像和90个视觉概念的NVRD数据集，每个概念有多达20个扰动版本。评估了3个开源和2个闭源模型，并收集了2,400个人类判断进行对比。

Result: 模型难以在上下文中获取与先验知识矛盾的新概念；模型和人类对视觉扰动敏感性相关，但模型显著过度泛化。

Conclusion: 贡献NVRD作为视觉概念学习研究的语料库和基准。
...
### [25] [Almieyar-Oryx-BloomBench: A Bilingual Multimodal Benchmark for Cognitively Informed Evaluation of Vision-Language Models](https://arxiv.org/abs/2606.05531)
...
TL;DR: 提出基于YOLOv8的实时检测框架，使用自定义和公开数据集检测枪支、刀具和钝器，在准确性和效率之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 传统人工监控效率低、易疲劳，需要自动化智能监控系统提升公共安全。

Method: 构建包含枪、刀和钝器三类共7959张图像的数据集，使用YOLOv8模型进行训练和实时检测。

Result: 增加训练轮次显著提高钝器类的召回率和平均精度，且未过拟合，整体框架在准确性和效率上平衡良好。

Conclusion: 该框架适用于校园、公共场所和交通区域的真实监控环境。
...
TL;DR: TextWand unifies scene text removal, generation, and replacement into a single model using atomic primitives of rendering and erasure.


<details>
  <summary>Details</summary>
Motivation: To address the absence of a comprehensive benchmark for general-purpose scene text editing among existing single-task datasets.

Method: It introduces Overlay-Reference Positional Encoding (ORPE) for layout fidelity and style control, and Region-Adaptive Suppression (RAS) for clean text erasure.

Result: TextWand outperforms existing models in text content accuracy, layout and style consistency, and overall image quality across all three tasks.

Conclusion: TextWand achieves precise control over text appearance and background integrity in scene text editing.
...
### [28] [Physics-Guided Deep Unfolding for Blind Cross-Sensor Spectral Super-Resolution via Learning the Spectral Transformation Function](https://arxiv.org/abs/2606.05759)
*Zhaolin Li,Jinsong Chen,Shanxin Guo,Tuo Zhang,Xinglong Zhang,Pan Chen*

Main category: cs.CV

TL;DR: 提出PGU-Net，一种物理引导的深度展开网络，用于跨传感器盲光谱超分辨率，联合估计高光谱图像和光谱变换函数。


<details>
  <summary>Details</summary>
Motivation: 现有SSR方法假设固定已知的SRF，仅适用于单传感器；跨传感器场景中SRF未知且随传感器和场景变化，重建高度病态。

Method: PGU-Net通过交替优化联合估计HSI和可学习的STF，将优化过程展开为端到端可训练架构，每个阶段依次更新HSI和STF，结合可学习的近端网络与可微闭式求解器。

Result: 在CAVE和NTIRE 2022基准数据集上，多种SRF下准确恢复STF并提升重建性能；在真实无人机跨传感器数据集上验证了有效性和鲁棒性，且估计的STF可能与土地覆盖相关。

Conclusion: PGU-Net在盲跨传感器SSR中有效，兼具物理可解释性和强表示能力。
...
### [29] [Next-Generation Parallel Decoder for LPDR: Architectural Optimization and Class-Balanced GAN-Augmentation](https://arxiv.org/abs/2606.05785)
*Shawaiz Obaid,Nida Chandio,Neha Jamil,Muhammad Khuram Shahzad*

Main category: cs.CV

TL;DR: 提出Cross-Spatial Hybrid Attention和Class-Balanced Synthetic Augmentation，在保持实时性下显著提升少数省份车牌识别率。


<details>
  <summary>Details</summary>
Motivation: YOLOV5-PDLPR模型受空间字符不匹配和训练集数据不平衡影响，性能受限。

Method: 引入Cross-Spatial Hybrid Attention (CSHA)和Class-Balanced Synthetic Augmentation (CBSA)。

Result: 少数省份车牌识别率从78.2%提升至91.5%，处理速度达152 FPS。

Conclusion: 空间感知并行解码与类平衡增强为高速车牌识别系统提供有效方案。
...
### [30] [Resonant Minds: Closed-Loop Social Avatars with Theory of Mind](https://arxiv.org/abs/2606.05896)
*Jianxu Shangguan,Jing Xu,Hang Ye,Xiaoxuan Ma,Yizhou Wang,Wentao Zhu*

Main category: cs.CV

TL;DR: 提出一个闭环双智能体框架，通过整合认知推理与多模态生成来创建具有社会智能的数字人类。


<details>
  <summary>Details</summary>
Motivation: 现有方法将认知推理和生成分离，导致对话缺乏具身表达或忽视社会认知。

Method: 构建包含感知、社会推理和表达模块的闭环双智能体框架，利用心理理论和集成机制推断心智状态并生成情感可控的双智能体视频。

Result: 在层次化Persona-Scenario数据集上，对话质量和视频生成指标表现优异，关键对话维度超越全信息脚本模式。

Conclusion: 明确心智状态推断在信息不对称下能引发更周到的对话。
...
### [31] [Global-Local Monte Carlo Tree Search in Vision-Language Models for Text-to-3D Indoor Scene Generation](https://arxiv.org/abs/2606.06002)
*Mengshi Qi,Wei Deng,Xianlin Zhang,Huadong Ma*

Main category: cs.CV

TL;DR: 提出了一种基于树搜索和层次化表示的文本到3D室内场景生成方法，并构建了新数据集。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM方法使用链式思维顺序决策无法修改早期决策，且现有基准在规模和多样性上有限。

Method: 将任务建模为全局和局部树搜索问题，提出层次化场景表示和PRM引导的MCTS方法，使用扩散模型预测纹理。

Result: 收集了包含65种场景类型和3250条指令的3DTindo-bench数据集，实验表明方法生成更真实的3D场景。

Conclusion: 提出的方法在文本到3D室内场景生成中优于现有方法，生成了更真实的场景。

Abstract: Large Vision-Language Models have achieved significant reasoning performance in various , there are few studies on text-to-3D indoor scene generation with LVLMs. The main challenge is that prevailing LVLM-based methods employ chain-of-thought sequential decision mechanisms that cannot revise earlier decisions, causing error this paper, we consider the task as a planning problem constrained by spatial and layout solve this problem, we model it as a tree search problem with global and local trees, which differs from existing sequential decision-making the global tree, we place each object iteratively and explore multiple attempts like humans furnishing a room, where the problem space is represented as a effectively search the tree, we propose a hierarchical scene representation and a PRM-guided MCTS hierarchical representation abstracts a scene into room level, region level, floor object level, and supported object PRM-guided MCTS method uses the PRM to prune unnecessary branches and the MCTS algorithm to balance exploration and exploitation to get an optimal solution with fewer the local tree, it further decomposes the placement of each object into finer sub-steps, including the specific placement make the whole appearance of the scene consistent, we leverage pre-trained diffusion image generative models to predict textures for all the objects in the existing benchmarks for text-to-3D indoor scene generation remain limited in scale and diversity, we collect a new large-scale diverse dataset that contains 65 scene types and 3,250 instructions with diverse sizes, layouts, and styles, named 3DTindo-bench, to better assess the capability of the state-of-the-art models. Our experiments show that our method generates more realistic 3D scenes than state-of-the-art approaches.

</details>


### [32] [VZCrash: A Large-Scale IMU Dataset of Ego-Vehicle Crashes](https://arxiv.org/abs/2606.06074)
*Tommaso Bianconcini,Henrique Piñeiro Monteagudo,Aurel Pjetri,Tomaso Trinci,Leonardo Taccari*

Main category: cs.CV

TL;DR: 介绍了最大的公开车辆碰撞数据集VZCrash，并通过实验证明了数据规模对碰撞检测模型性能的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有碰撞数据集规模小且缺乏真实世界的IMU数据，难以支撑高质量模型训练和部署，因此需要构建大规模公开数据集并研究数据规模的影响。

Method: 收集73010辆商用车辆数年的IMU和GPS数据，构建包含31000+碰撞和158000+负样本的VZCrash数据集；实验中基准测试从阈值方法到深度学习模型，并分析数据规模对模型性能的影响。

Result: 基准测试表明不同方法在碰撞检测上的性能差异，实验证明数据规模对训练高质量模型至关重要，尤其在真实环境部署时。

Conclusion: VZCrash数据集为碰撞检测研究提供了大规模真实数据，且数据规模是提升模型性能的关键因素。
...
### [33] [MS-DKC: A Dataset Knowledge Card Framework for Designing and Adapting Medical Image Segmentation Models](https://arxiv.org/abs/2606.06103)
*Tariq M. Khan,Syed Saud Naqvi,Thantrira Porntaveetus,Hamid Alinejad-Rokny,Shahzaib Iqbal,Imran Razzak,Mohammad AU Khan*

Main category: cs.CV

TL;DR: 提出MS-DKC框架，显式化医学图像分割数据集的需求，指导模型设计


<details>
  <summary>Details</summary>
Motivation: 现有工作过于关注架构改进，忽略数据集内在需求，MS-DKC旨在使这些需求显式化

Method: MS-DKC通过图像/采集、形态、监督、上下文依赖和部署风险五个描述符记录数据集证据，映射到失败模式、设计先验和风险校准标准

Result: 在DRIVE、ISIC2018、ACDC上评估，DRIVE上Dice达0.8141，ISIC2018上Dice达0.8872，ACDC需多类分割

Conclusion: 支持数据集条件化设计，不同数据集需不同先验、操作点和证据
...
### [34] [Where, What, Why, and Importance: Structured Defect Grounding for Text-to-Image Feedback](https://arxiv.org/abs/2606.06113)
*Huaisong Zhang,Hao Yu,Yuxuan Zhang,Jiahe Wang,Xinrui Chen,Haoxiang Cao,Feng Lu,Wendong Zhang,Changqian Yu,Chun Yuan*

Main category: cs.CV

TL;DR: 提出结构化缺陷定位（SDG）方法，将文本到图像生成诊断建模为结构化集合预测，并构建数据集和评估协议，实现诊断到对齐的框架。


<details>
  <summary>Details</summary>
Motivation: 现有方法基于热图回归难以定位可变基数缺陷和绑定语义原因，需要实例级结构化反馈。

Method: 通过将每个缺陷表示为（位置，类型，原因，重要性）元组进行结构化集合预测，构建SDG-30K数据集，并利用VLM作为检测器，使用BoxFlow-GRPO将预测缺陷集转换为空间奖励用于扩散模型对齐。

Result: SDG检测器在结构化缺陷定位上优于领先专有VLM，SDG引导奖励改善T2I对齐并支持局部细化。

Conclusion: SDG提供了统一的实例级接口，用于诊断、评估和增强现代生成模型。
...
### [35] [SC-MFJ: A Simple Haptic Quality Metric for Medical Image Segmentation](https://arxiv.org/abs/2606.06199)
*Souraj Adhikary,Negar Chabi,Andre Mastmeyer*

Main category: cs.CV

TL;DR: 提出SC-MFJ度量，通过模拟触觉交互过程中的力加加速度评估分割表面的触觉质量，发现简单高斯后处理显著优于原始二进制和SDF回归，且该差异无法被传统几何度量捕捉。


<details>
  <summary>Details</summary>
Motivation: 现有几何指标如Dice和Hausdorff距离只测量几何重叠，无法评估分割表面是否适合触觉渲染，需要一种简单廉价的度量来直接衡量触觉质量。

Method: SC-MFJ度量：在分割表面采样大量短虚拟触觉笔行走，计算接触力的平均加加速度（力对时间的三阶导数），从现有分割输出直接计算，每个病例约需一分钟CPU时间。

Result: 在80例胰腺CT和131例肝脏CT上评估三个分割方法（二进制nnU-Net、高斯平滑、学习SDF回归），SC-MFJ显示二进制与高斯平滑之间的触觉质量差距达147倍（胰腺）和189倍（肝脏），而Dice和HD95无法区分；SDF回归的触觉质量变异（标准差168 N/s²）远大于高斯平滑（22 N/s²）。

Conclusion: 对于触觉仿真应用，简单的高斯后处理步骤可能足够，而SC-MFJ这样的廉价度量能够识别出传统几何度量遗漏的触觉质量问题。
...
### [38] [Visual Commonsense Driven Knowledge Refinements for Scene Graph Generation](https://arxiv.org/abs/2606.06369)
*Maëlic Neau,Salim Baloch,Jakob Suchan,Zoe Falomir,Mehul Bhatt*

Main category: cs.CV

TL;DR: 提出模型无关的语义引导知识精炼框架，通过挖掘常识约束并用声明式推理改进场景图生成，无需手动规则或重训练，在三个基准上获得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有SGG模型在频繁关系上表现好，但注释稀疏时性能急剧下降，无法捕获可靠视觉常识知识。

Method: 系统挖掘训练数据中的常识约束（空间、功能、定性关系），在推理时用声明式常识推理校正和精炼SGG预测，无需手动规则编写或模型重训练。

Result: 在三个标准基准上获得一致改进。

Conclusion: 结构化视觉常识推理是纯学习SGG的实用有效补充。
...
TL;DR: 提出PAR3D框架，通过ScenePart数据集和部件感知表示学习，实现3D场景中物体和部件的统一理解与定位。


<details>
  <summary>Details</summary>
Motivation: 现有3D-MLLMs以物体为中心，缺乏对细粒度部件结构的建模能力，限制了具身交互中的3D场景理解。

Method: PAR3D框架，包括ScenePart数据集、Part-Aware 3D Representation Learning和Hierarchical Segmentation Query Generation。

Result: 在部件级问答和指代分割任务上显著提升，同时在物体级视觉-语言任务上保持高性能。

Conclusion: 提出的方法使模型能够理解、推理和定位3D场景中的物体及其部件，为具身交互提供细粒度理解。
...
### [41] [Can We Predict The Human Preference For Text-to-Image Content Prior To Generation And Is It Even Useful To Do So?](https://arxiv.org/abs/2606.05478)
*Joong Ho Kim,Keith G. Mills*

Main category: cs.CV

TL;DR: 探索在扩散模型生成前预测人类偏好指标分数，并利用预测改进生成质量，发现可行且硬件开销可忽略。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽生成高质量图像，但初始噪声影响大，尤其在小模型中，因此希望提前预测HPM分数以避免无效计算并提升质量。

Method: 首先验证能否在生成前预测HPM分数，然后研究如何利用预测改善生成图像质量，并比较不同HPM的适用性。

Result: 预测HPM分数是可行的，并且能够以极少的硬件开销实现质量改进。

Conclusion: 通过预测人类偏好指标分数可以有效提升扩散模型生成图像的质量，且计算成本低。
...
### [44] [SAM-Flow: Source-Anchored Masked Flow for Training-Free Image Editing](https://arxiv.org/abs/2606.06228)
*Haowang Cui,Rui Chen,Tao Luo,Tao Guo,Zheng Qin,Jiaze Wang*

Main category: cs.CV

TL;DR: 提出SAM-Flow，一种源锚定掩码流框架，用于本地化训练免费图像编辑，有效防止背景泄漏。


<details>
  <summary>Details</summary>
Motivation: 现有训练免费图像编辑方法通常进行全局潜在传输，导致编辑效果传播到非目标区域并引起背景泄漏。

Method: SAM-Flow使用侦察图像和令牌接地注意力图定位可编辑语义区域，仅在这些区域应用差分速度更新，其余区域锚定到源图像潜在轨迹；并引入时变源锚定投影机制，包括动态软掩码、过渡区域和时间掩码累积。

Result: 大量定性和定量实验表明，SAM-Flow实现了准确的语义编辑，同时显著改善了背景保持。

Conclusion: SAM-Flow提供了一种简单、通用的本地化编辑范式，无需微调即可集成到主流流匹配骨干网络中。
...
### [45] [VideoKR: Towards Knowledge- and Reasoning-Intensive Video Understanding](https://arxiv.org/abs/2606.05259)
*Lin Fu,Zheyuan Yang,Yang Wang,Tingyu Song,Arman Cohan,Yilun Zhao*

Main category: cs.CV

TL;DR: VideoKR是首个专门针对知识密集型视频推理的大规模训练语料库，包含315K示例和145K视频，通过人机交互流程生成，并展示了其在提升推理能力上的有效性。


<details>
  <summary>Details</summary>
Motivation: 缺乏针对知识密集型推理的视频训练数据，现有数据集不足以支持深度视频理解。

Method: 构建VideoKR语料库（315K示例，145K视频），采用人机交互、技能导向的示例生成流程，并基于SFT→GRPO流程进行模型后训练。

Result: 在知识密集型视频推理上优于先前后训练方法，在一般视频推理上保持竞争力。

Conclusion: 数据设计是视频推理进步的关键驱动因素，提供了可行的见解。
...
### [46] [Personal AI Agent for Camera Roll VQA](https://arxiv.org/abs/2606.05275)
*Thao Nguyen,Krishna Kumar Singh,Donghyun Kim,Yong Jae Lee,Yuheng Li*

Main category: cs.CV

TL;DR: 本文提出个人相册视觉问答数据集camroll和高效导航AI代理camroll-agent，揭示了长上下文个性化视觉记忆与标准文本记忆的差距。


<details>
  <summary>Details</summary>
Motivation: 个人相册包含大量长期、高度个性化的视觉内容，AI助手需要高效导航和定位相关信息以回答用户问题。

Method: 收集并手动注释包含50个用户、31,476张图像和2,500个问答对的camroll数据集；设计具有分层记忆和最小工具集的camroll-agent代理。

Result: camroll-agent在实验上优于多种基线和长上下文理解AI代理方法。

Conclusion: 个性化视觉记忆需要不同于标准长上下文文本记忆的方法，尤其在一致性、视觉细节和用户特定上下文方面。
...
### [47] [Recovering Physically Plausible Human-Object Interactions from Monocular Videos](https://arxiv.org/abs/2606.05359)
*Dingbang Huang,Etienne Vouga,Qixing Huang,Georgios Pavlakos*

Main category: cs.CV

TL;DR: RePHO: 一种从单目视频通过物理引导重建物理合理的人-物交互的方法。


<details>
  <summary>Details</summary>
Motivation: 现有运动学方法产生视觉合理但物理不合理的伪影（如穿透、物体漂浮），需要物理引导。

Method: 从运动学估计开始，通过强化学习训练策略优化，采用自适应采样策略和双自我更新机制。

Result: 在两个标准HOI基准测试中，在物理合理性指标上明显优于现有方法。

Conclusion: 该方法能生成物理一致的HOI序列，并改进物理合理性。
...
### [48] [UniPixie: Unified and Probabilistic 3D Physics Learning via Flow Matching](https://arxiv.org/abs/2606.05399)
*Qilin Huang,Quynh Anh Huynh,Long Le,Chen Wang,Chuhao Chen,Ryan Lucas,Eric Eaton,Lingjie Liu*

Main category: cs.CV

TL;DR: UNIPIXIE通过学习连续的材料属性分布，实现了从单张图像到多样的、物理有效的材料属性的可控生成，并将杨氏模量预测误差降低超过50%。


<details>
  <summary>Details</summary>
Motivation: 现有前馈网络只能预测单一物理属性，无法捕捉真实世界固有的物理歧义。

Method: 提出了UNIPIXIE框架，学习从视觉输入到沿物体软硬谱的连续参数化路径的映射，采用统一架构生成适用于多种物理求解器的仿真就绪参数。

Result: 生成丰富多样的动力学，杨氏模量预测误差比最强确定性基线降低超过50%。

Conclusion: 弥合了静态点估计与连续物理现实之间的差距。

Abstract: Existing feed-forward networks excel at predicting a single set of physical properties from visual appearance, but this point-estimate paradigm fundamentally fails to capture the real world's inherent physical ambiguity. We address this by reframing physics prediction as a task of learning a controllable, continuous distribution of material properties. We introduce UNIPIXIE, a framework trained to predict a continuous and parameterized path of physically plausible material properties from a single visual input. By learning a direct mapping along an object's softest-to-stiffest spectrum on our PIXIEMULTIVERSE dataset, UNIPIXIE allows for controllable generation of diverse, physically valid material fields via a single intuitive parameter. Crucially, UNIPIXIE introduces a novel unified architecture to produce simulation-ready parameters for diverse physics solvers, including continuum-based Material Point Method (MPM), reduced-order deformation based on Linear Blend Skinning (LBS), and anchor-based Spring-Mass systems, addressing a key portability issue in prior work. Experiments show our approach not only generates a rich variety of plausible dynamics but also reduces Young's Modulus prediction error by over 50% against the strongest deterministic baseline, bridging the gap between static point estimates and the continuous nature of physical reality. Project page:

</details>


### [49] [Horse Eye Blink Detection and Classification for Equine Affective State Assessment](https://arxiv.org/abs/2606.05458)
*João Alves,Signe Møller-Skuldbøl,Pia Haubro Andersen,Rikke Gade*

Main category: cs.CV

TL;DR: 本文研究了马匹眨眼动作的自动检测，提出并评估了三种方法，在公共数据集上取得了良好的性能。


<details>
  <summary>Details</summary>
Motivation: 自动检测马匹面部动作单元是评估疼痛和情感状态的有前景的途径，但眨眼作为微表情容易被肉眼忽略，需要可靠的自动检测方法。

Method: 开发了三种方法：基于帧的YOLOv12检测器、光流幅度阈值方法和微调VideoMAE模型，在公开数据集上进行测试。

Result: 眨眼分类的宏F1得分为0.898，二值眨眼检测的宏F1得分为0.926。

Conclusion: 结果显示了细粒度动作单元检测在马匹健康监测中的潜力和固有挑战。
...
### [50] [ORACLE-CT: Anatomy-Aware Support Pooling for CT Classification](https://arxiv.org/abs/2606.05460)
*Lavsen Dahal,Yubraj Bhandari,Geoffrey Rubin,Joseph Y. Lo*

Main category: cs.CV

TL;DR: 提出ORACLE--CT，一种解剖感知的聚合框架，利用多器官分割限制注意力到相关区域，提高了腹部CT分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用解剖无关的池化或注意力，导致局部疾病证据与全局聚合不匹配。

Method: 提出ORACLE--CT框架，使用多器官分割定义标签特定解剖支持，并限制注意力池化到相关区域，支持多种支持策略。

Result: 在MERLIN上，支持掩码池化使DINOv3的宏观AUROC/AUPRC从0.838/0.638提升至0.858/0.676；外部数据集Duke-Abdomen和AMOS上分别从0.802/0.628提升至0.835/0.683和从0.742/0.313提升至0.762/0.350。

Conclusion: ORACLE--CT提高了判别力和外部鲁棒性，同时保持了预测与解剖证据之间的可审计联系。
...
### [51] [Formal Concept Lattices are Good Semantic Scaffolds for Concept-Based Learning](https://arxiv.org/abs/2606.05471)
*Deepika SN Vemuri,Sayanta Adhikari,Ankit Saha,Krishn Vishwas Kher,Vineeth N Balasubramanian*

Main category: cs.CV

TL;DR: 利用形式概念分析中的概念晶格引导神经网络学习分层语义概念，提高了模型的解释性和干预效果。


<details>
  <summary>Details</summary>
Motivation: 现有概念模型将概念视为扁平的非结构化集合，忽略了人类语义理解的分层组织特性，而深层网络学习的特征层级很少与显式语义层级对齐。

Method: 基于形式概念分析（FCA）的形式概念晶格为神经网络提供语义支架，根据概念的通用性决定在网络的哪一层学习，从而构建分阶段、语义化的表示。

Result: 在真实数据集上的实验表明，该方法生成的嵌入更易解释，支持更有效的干预，并且学习到的概念表示既有意义又具有分层结构。

Conclusion: 形式概念晶格能有效指导神经网络学习与人类语义层级一致的分层概念表示，提升模型的解释性和干预能力。
...
### [52] [Unpaired RGB-Thermal Gaussian-Splatting Using Visual Geometric Transformers](https://arxiv.org/abs/2606.05491)
*Jean Cordonnier,Chenghao Xu,Olga Fink,Malcolm Mielle*

Main category: cs.CV

TL;DR: 提出一种无需配对校准的多模态RGB-热成像新视角合成方法，通过独立姿态估计和跨模态对齐实现联合重建。
...
Method: 利用VGGT 3D前馈变换器独立估计每模态相机姿态，通过Procrustes算法和跨模态特征匹配进行对齐，然后使用多模态3D高斯泼溅从非配对图像学习。

Result: 在热成像视图合成上达到竞争性能，同时保持RGB保真度；并引入评估框架。

Conclusion: 提出的方法实现了无需配对校准的多模态NVS，且性能良好。
...
### [53] [Noise-Aware Visual Representation Learning for Medical Visual Question Answering](https://arxiv.org/abs/2606.05535)
*I Putu Adi Pratama,Bahadorreza Ofoghi,Atul Sajjanhar,Shang Gao*

Main category: cs.CV

TL;DR: 本文提出一种噪声感知的医学视觉问答框架，使用去噪自编码器增强视觉表示的鲁棒性，并通过LoRA进行参数高效微调，在SLAKE和PathVQA基准上提升了噪声鲁棒性并保持了竞争性能。


<details>
  <summary>Details</summary>
Motivation: 现有Med-VQA方法常忽视视觉表示中的噪声和微小无关变化，导致鲁棒性不足。

Method: 在视觉嵌入映射到LLM输入空间之前，加入预训练的去噪自编码器以重建干净嵌入，然后通过MLP投影形成视觉前缀令牌，并使用LoRA进行参数高效微调。

Result: 在SLAKE和PathVQA上，模型对噪声输入表现出更强的鲁棒性，同时在清洁输入上保持竞争性能。

Conclusion: 学习更鲁棒的视觉表示能有效提升Med-VQA的性能和鲁棒性。
...
### [54] [Dual Feature Decoupling for Fine-Grained OOD Detection](https://arxiv.org/abs/2606.05536)
*Xiaokun Li,Yaping Huang,Qingji Guan*

Main category: cs.CV

TL;DR: 提出双特征解耦网络(DFDNet)用于细粒度OOD检测，通过空间-频率解耦和重构引导解耦模块实现特征解耦。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法假设大类间分布差异，忽略细粒度任务中类别相似性和背景干扰带来的挑战。

Method: DFDNet包含空间-频率解耦模块（保留判别性内容特征，抑制风格信息）和重构引导解耦模块（像素级对抗重构，去除低层非判别性信息，增强高层语义）。

Result: 在多个数据集上取得竞争性的性能提升。

Conclusion: 实验表明DFDNet有效提升了细粒度OOD检测性能。
...
### [55] [UltraVR: A Diagnostic Ultra-Resolution Image-VQA Benchmark for Evidence-Grounded Reasoning](https://arxiv.org/abs/2606.05576)
*Gexin Huang,Yanting Yang,Myeongkyun Kang,Beidi Zhao,Jun Zhou,Chen Zhou,Gang Wang,Zu-hua Gao,Xiaoxiao Li*

Main category: cs.CV

TL;DR: UltraVR is a diagnostic benchmark for evidence-grounded visual reasoning over ultra-resolution images, revealing that current VLMs fail mainly in evidence grounding and local perception.


<details>
  <summary>Details</summary>
Motivation: To assess whether VLMs acquire and integrate necessary visual evidence in ultra-resolution images, beyond final-answer accuracy.

Method: We introduce UltraVR, covering CCTV, remote sensing, pathology, and industrial anomaly detection, with structured ground-truth chain of thought decomposing reasoning into five steps.

Result: Frontier VLMs show unreliable performance, with errors concentrated in evidence grounding and local perception; downstream inference often recovers when intermediate facts are provided.

Conclusion: UltraVR enables process-level diagnosis, locating where ultra-resolution reasoning breaks in VLMs.
...
### [56] [ShotCrop$^3$: Cropping Human-Centric Images into Cinematic Triple-Shot Compositions](https://arxiv.org/abs/2606.05635)
...
TL;DR: 提出Triple-Shot Compositions任务和ShotCrop方法，通过三阶段训练生成多个美学裁剪并提高定位准确性。


<details>
  <summary>Details</summary>
Motivation: 先前美学构图只产生单一裁剪，忽略多镜头叙事价值，而多镜头构图对商业海报等创意工作流程至关重要。

Method: 提出ShotCrop，采用三阶段训练：CoT监督微调、半监督微调（高置信度伪标签）、GRPO-S优化；伪标签结合MLLM评分、美学评估和CLIP相似度。

Result: ShotCrop在镜头定位准确性上平均比GPT-5提高2.82倍。

Conclusion: ShotCrop在有限标注下有效学习多镜头构图，显著优于现有方法。
...
### [57] [Multi-Task Crack Foundation Model for Engineering-Reliable Crack Representation and Topology Preservation in Civil Infrastructure](https://arxiv.org/abs/2606.05641)
*Blessing Agyei Kyem,Joshua Kofi Asamoah,Eugene Denteh,Armstrong Aboah*

Main category: cs.CV

TL;DR: CrackGeoFM is a multi-task framework for crack segmentation, skeleton reconstruction, and uncertainty estimation, achieving state-of-the-art results with improved topology and calibrated uncertainty using a frozen backbone and crack-specific adaptations.


<details>
  <summary>Details</summary>
Motivation: Existing segmentation models produce fragmented cracks, miss fine branches, and lack calibrated uncertainty despite high overlap scores, necessitating a method that ensures connected geometry, stable confidence estimates, and domain shift robustness.

Method: CrackGeoFM combines a frozen visual foundation backbone with crack-specific adaptations, integrating a Frequency-Guided Crack Enhancement Module (FCEM), a Crack-Domain Feature Adaptation Module (CFAM), and a Structure-Aware Multi-Task Decoder (SMTD) for joint mask prediction, skeleton reconstruction, and uncertainty estimation.

Result: Across 20 crack datasets, CrackGeoFM achieves state-of-the-art segmentation, improved topology preservation, calibrated uncertainty, and effective few-shot adaptation with only five labeled images.

Conclusion: The results support reliable, generalizable, and engineering-oriented crack analysis for infrastructure assessment.
...
### [58] [V2V-Bench: A Comprehensive Benchmark for Video-to-Video Generation Evaluation](https://arxiv.org/abs/2606.05665)
*Tao Liu,Leela Krishna,Gouti Pavan Kumar,Sreeja K,Vishav Garg*

Main category: cs.CV

TL;DR: 提出V2V-Bench基准，用于评估视频到视频生成，包含11个维度，在三个模型上测试，结果显示模型优势互补，且与人类判断高度相关。


<details>
  <summary>Details</summary>
Motivation: 现有T2V和I2V指标无法捕捉V2V生成的特定要求，如编辑指令和帧级对应。

Method: 构建V2V-Bench基准，包含11个维度，分为五个类别：时间对齐、结构保真度、变换质量、视频质量和语义对齐。评估两个商业模型（Grok Imagine和Gemini Veo3）和一个开源模型（Open Sora 2）。

Result: Grok在编辑保真度上更优，Veo3在视觉质量上更优。在六个V2V特定维度上，V2V-Bench与人类判断的Spearman相关系数为0.905。

Conclusion: V2V-Bench能够有效评估V2V生成，反映模型之间的互补优势。
...
### [59] [T-SAR-JEPA: Self-Supervised Temporal Anomaly Detection in SAR Amplitude Stacks via Latent Prediction](https://arxiv.org/abs/2606.05700)
*Kerod Woldesenbet,Abem Woldesenbet*

Main category: cs.CV

TL;DR: T-SAR-JEPA是一种基于潜在预测的自监督SAR幅度堆栈时间异常检测框架。


<details>
  <summary>Details</summary>
Motivation: 现有SAR时间异常检测方法性能不足，需更鲁棒的自监督方法。

Method: 使用SAR-JEPA的ViT-Base/16编码器在39,300个Capella块上域适应，结合局部掩码重建与梯度特征预测；时间Transformer用正弦编码从7次采集中预测未来潜态，渐进解冻降损失。仅用幅度数据，InSAR相干性作伪真实标签。

Result: 在DFC 2026数据集夏威夷爆发窗口上ROC-AUC达77.0%，优于RX、PaDiM、Linear AR和LSTM基线（~50%），空间一致性99.9%（p<0.001）。

Conclusion: T-SAR-JEPA有效检测SAR时间序列异常，性能显著优于传统方法，检测结果具有空间结构性。

Abstract: We present T-SAR-JEPA, a self-supervised framework for temporal anomaly detection in SAR amplitude stacks via latent prediction. A ViT-Base/16 encoder from SAR-JEPA is domain-adapted on 39,300 Capella patches using local masked reconstruction with gradient feature prediction. A temporal transformer with sinusoidal time encoding forecasts future latent states from K=7 acquisitions, with progressive unfreezing substantially reducing validation loss. The model operates on amplitude alone; InSAR coherence serves exclusively as independent pseudo-ground-truth. On the DFC 2026 dataset (300 time-series, three AOIs), T-SAR-JEPA achieves ROC-AUC of 77.0% on the Hawaii eruption window, outperforming RX, PaDiM, Linear AR, and LSTM baselines (~50%). Spatial coherence of 99.9% (p < 0.001, permutation test) confirms structured detections. Code:

</details>


### [60] [Parallel Jacobi Decoding for Fast Autoregressive Image Generation](https://arxiv.org/abs/2606.05703)
*Boya Liao,Ying Li,Siyong Jian,Huan Wang*

Main category: cs.CV

TL;DR: 提出Parallel Jacobi Decoding (PJD)，一种无需训练的解码方法，通过二维空间并行细化加速自回归图像生成，实现4.8-6.4倍加速并保持质量。


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成推理慢，现有Jacobi解码加速因一维错误传播而饱和，需利用图像空间局部性提升效率。

Method: PJD在二维空间域扩展draft tokens，调整注意力掩码以减少错误积累，实现稳定的空间并行细化。

Result: 在多个数据集上，PJD对多种自回归模型实现4.8-6.4倍加速，生成质量与基线相当。

Conclusion: PJD是一种训练免费的有效加速方法，通过空间并行解码显著提升推理速度，且保持生成质量。
...
### [61] [ViCuR: Visual Cues as Recoverable Privilege for Multimodal On-Policy Distillation](https://arxiv.org/abs/2606.05718)
*Kanghui Tian,Siyuan Liu,Ziang Yan,Sheng Xia,Shuai Dong,Yi Wang*

Main category: cs.CV

TL;DR: 提出ViCuR，一种视觉基础的特权教师蒸馏框架，通过用视觉线索替代答案侧特权来改进多模态推理中的on-policy蒸馏。


<details>
  <summary>Details</summary>
Motivation: 现有基于答案侧特权的教师蒸馏存在训练-测试不匹配，导致学生学会捷径模仿而非视觉基础推理。

Method: ViCuR使用视觉线索（输入中的查询相关证据）替换答案侧特权，并引入轻量级线索恢复模块，通过sink-token交叉注意力聚合视觉证据到内部表示，不改变推理接口。

Result: 在七个基准测试上，ViCuR比基于答案的on-policy自我蒸馏平均提升+1.19和+1.24，并超越更强教师OPD基线+0.64和+1.08，在8B规模上有一致域外增益。

Conclusion: 在多模态on-policy蒸馏中，教师特权设计的重要性与教师强度相当。
...
### [62] [VTI-CoT: Visual-Textual Interleaved Chain of Thought for Video Reasoning](https://arxiv.org/abs/2606.05736)
*Shufan Zhang,Ziyue Lin,Bairun Wang,Lei Jin,Xuanding Ding,Xinzhu Ma,Kunlin Yang*

Main category: cs.CV

TL;DR: 提出VTI-CoT框架，通过视觉-文本交错思维链提升视频推理，并采用自动标注和OCR压缩提高效率。


<details>
  <summary>Details</summary>
Motivation: 现有CoT视频推理方法仅依赖文本，忽略视觉信息，受人类认知启发需结合视觉片段。

Method: 构建VTI-CoT框架，集成文本推理与对应视觉帧；开发自动标注管道生成多模态CoT数据；使用OCR压缩将CoT信号压缩到单画布以加速训练。

Result: 在相同参数规模模型中达到SOTA，显著提升训练效率。

Conclusion: VTI-CoT有效结合视觉-文本推理，提升性能并解决长序列训练效率问题。
...
### [63] [Let It Be Simple: One-Step Action Generation for Vision-Language-Action Models](https://arxiv.org/abs/2606.05737)
*Yitong Chen,Shiduo Zhang,Jingjing Gong,Xipeng Qiu*

Main category: cs.CV

TL;DR: 通过偏置训练时间分布到高噪声状态，标准扩散训练即可实现一步VLA动作生成，性能媲美十步解码。


<details>
  <summary>Details</summary>
Motivation: VLA动作生成的条件-目标结构不对称（条件丰富，目标紧凑），无需采用图像生成中先进的少步方法，探索简单的训练策略实现强一步生成。

Method: 保持标准速度预测，无教师模型、蒸馏或辅助目标，仅在训练时偏置时间分布向高噪声状态。

Result: 在LIBERO、LIBERO-Plus、LIBERO-Pro上，一步策略匹配十步解码；在1.4B VLM上一步解码达95.6%准确率。

Conclusion: 强一步VLA动作生成可从标准扩散训练中产生，无需借用图像生成的高级少步技术。
...
### [64] [Imagine Before You Predict: Interleaved Latent Visual Reasoning for Video Event Prediction](https://arxiv.org/abs/2606.05769)
*Tianxiang Jiang,Linquan Wu,Sheng Xia,Songze Li,Ziang Yan,Haoyu Yang,Yu Qiao,Yi Wang*

Main category: cs.CV

TL;DR: Future-L1是一个交错潜在视觉推理框架，用于视频事件预测，通过潜在空间推理保留视觉语义，取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频MLLM在文本空间推理会丢失细粒度的运动、几何和交互线索，导致视觉上无依据的幻觉。

Method: 构建Future-L1-50K数据集，选择需要未来视觉线索的样本，对齐潜在状态到未来帧嵌入；使用LA-DAPO，一种潜在感知的强化学习目标，包含结果对比和时间多样性奖励。

Result: 在FutureBench上从61.0提升到85.4，超过之前最佳10.4分；TwiFF-Bench平均分从2.44提升到3.04。

Conclusion: 未来导向视频推理受益于在潜在空间中保留中间视觉语义，而不是将每一步推理都翻译成文本。
...
### [65] [LiAuto-GeoX: Efficient Grounded Driving Transformer](https://arxiv.org/abs/2606.05774)
*Jiawei Lian,Haoyi Sun,Yang Wu,Lifu Mu,Siyuan Wang,Le Hui,Ning Mao,Tao Wei,Pan Zhou,Kun Zhan,Jian Yang*

Main category: cs.CV

TL;DR: LiAuto-GeoX是一种高效的接地驾驶变压器，通过紧凑的蒸馏框架实现实时高保真密集3D重建，并在下游自动驾驶任务中表现卓越。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视觉几何模型计算资源需求大，缺乏长距离几何保真度、环视一致性和实时效率，因此需要一种高效的密集3D重建方法用于自动驾驶。

Method: 首先从大规模环视图数据中学习高容量驾驶几何模型，利用稀疏LiDAR先验提供鲁棒几何接地；然后通过新颖的几何保持蒸馏框架（包括掩模引导的深度感知蒸馏和相对姿态关系蒸馏）将能力实例化为紧凑的155M参数板载模型。

Result: 在KITTI上以220 FPS运行，轨迹预测PDMS 90.6，占用预测mIoU 24.63，未来帧预测IoU 47.67。

Conclusion: 高效的密集3D重建可以超越传统感知目标，作为可扩展的基础几何表示，用于下一代自动驾驶。
...
### [66] [Emotion-Aware Image Generation from Korean Diary Text via LLM-based Prompt Translation and LoRA Fine-Tuning](https://arxiv.org/abs/2606.05816)
*Jihun Cho,Soo-Yeon Jeong,Sun-Young Ihm*

Main category: cs.CV

TL;DR: 提出一个情感感知的文本到图像管道，从短韩语日记生成儿童手绘风格图像，使用Qwen3-8B识别情感和LoRA微调Stable Diffusion。


<details>
  <summary>Details</summary>
Motivation: 现有T2I模型无法有效捕捉文本中的情感，因为它们关注视觉对象而非上下文情感理解。

Method: 采用Qwen3-8B识别隐含情感，并使用LoRA微调Stable Diffusion 3.5 Medium在儿童绘画图像上，结合基于情感的触发词生成图像。

Result: 实验验证了情感触发词对生成图像的影响，并指出CLIP Score作为评估指标的局限性。

Conclusion: 提出的管道能生成具有情感感知的儿童手绘风格图像，但情感评估仍具挑战性。
...
### [67] [Gender Artifacts from Art History to Text-to-Image Generation](https://arxiv.org/abs/2606.05829)
*Piera Riccio,Miriam Doh,Benedikt Höltgen,Noa Garcia,Nanne van Noord*

Main category: cs.CV

TL;DR: 提出StyleGender数据集和SGA指标，揭示性别表征塑造艺术风格并在T2I生成中放大性别伪影。


<details>
  <summary>Details</summary>
Motivation: AI研究将风格视为表面视觉属性，忽略其社会历史背景中的性别建构，需要探究性别与风格在历史和生成图像中的交互。

Method: 构建包含74k图像、19种艺术风格的StyleGender数据集，包括历史图像和T2I生成图像，并提出PixelSGA和MaskSGA指标捕捉像素和构图层面的性别信号。

Result: 性别表征影响不同风格的视觉特征；风格关键词将这些模式带入T2I生成；生成模型放大性别伪影，超过历史图像中的程度。

Conclusion: 性别与风格在图像中紧密相关，生成模型需谨慎处理历史偏见，避免强化性别伪影。
...
### [68] [Learning Geometric Representations from Videos for Spatial Intelligent Multimodal Large Language Models](https://arxiv.org/abs/2606.05833)
*Haibo Wang,Lifu Huang*

Main category: cs.CV

TL;DR: GeoVR通过2D视频序列的多目标学习蒸馏3D知识，赋予多模态大语言模型空间智能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型缺乏3D意识，表示缺乏几何和空间一致性，且大规模3D数据稀缺。

Method: 提出GeoVR框架，利用纯2D视频序列，通过多目标学习策略（包括估计相机姿态、回归深度图、预测度量尺度因子、蒸馏多尺度3D特征）重塑MLLM内部表示。
...
### [69] [Unveiling the Unknown: Open Vocabulary Object Detection with Scene Graphs](https://arxiv.org/abs/2606.05916)
*Yi Chen,Yinghao Lu,Zhehao Li,Chenchen Yan,Jiafei Wu,Chong Wang,Jiangbo Qian*

Main category: cs.CV

TL;DR: 提出基于场景图的关系建模框架，通过显式和隐式建模物体间关系提升开放词汇目标检测中新类别的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略了物体间结构化的图像特定关系，限制了新类别检测的有效性。

Method: 构建场景引导的关系建模框架，利用场景图捕获候选区域间的关系，引入关系注意力模块放大关键关系线索，并设计场景文本对齐分支整合视觉关系和语义信息。

Result: 在COCO和LVIS数据集上，对新类别的平均精度（AP）优于其他开放词汇目标检测方法。

Conclusion: 通过显式和隐式关系建模及文本对齐，有效提升了开放词汇目标检测中新类别的检测性能。
...
### [70] [T-FunS3D: Task-Driven Hierarchical Open-Vocabulary 3D Functionality Segmentation](https://arxiv.org/abs/2606.05975)
*Jingkun Feng,Reza Sabzevari*

Main category: cs.CV

TL;DR: T-FunS3D是一种任务驱动的分层开放词汇3D功能分割方法，在保证性能的同时提高了速度和内存效率。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D分割方法主要关注对象级识别，而场景级部分分割资源密集且耗时，难以平衡粒度、准确性和速度。

Method: 输入3D点云和RGB-D图像，构建开放词汇场景图，根据任务描述识别相关实例并利用视觉语言模型定位其功能组件。

Result: 在SceneFun3D数据集上，T-FunS3D与SOTA性能相当，但运行更快、内存使用更少。

Conclusion: T-FunS3D有效平衡了分割性能与计算效率，适合机器人应用。
...
### [71] [ATT-CR: Adaptive Triangular Transformer for Cloud Removal](https://arxiv.org/abs/2606.05999)
*Yang Wu,Ye Deng,Pengna Li,Wenli Huang,Kangyi Wu,Xiaomeng Xin,Jinjun Wang*

Main category: cs.CV

TL;DR: 提出自适应三角Transformer用于云移除（ATT-CR），通过三角注意力和特征选择门控模块有效降低计算成本并减少云像素干扰，在基准测试中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的云移除方法存在两个问题：自注意力的高计算复杂度限制了可扩展性，且将云像素与干净像素同等对待导致后续层受到干扰，性能欠佳。

Method: 提出ATT-CR模型，包含两个核心组件：三角注意力（TAN）和特征选择门控模块（FSGM）。TAN使用上下三角矩阵近似Softmax注意力，实现O(N)复杂度；FSGM自适应区分云和干净特征，减少无效信息传递。

Result: 在云移除基准数据集上的大量实验表明，ATT-CR相比现有方法具有更优越的性能。

Conclusion: ATT-CR通过降低计算复杂度和减轻云像素干扰，有效提升了云移除的性能，是一种有效的云移除方法。
...
### [72] [Texture-preserving implicit neural representation for Cone beam CT truncated reconstruction](https://arxiv.org/abs/2606.06039)
*Genyuan Zhang,Junyao Wang,Haoran Lan,Chuandong Tan,Songtao Zhu,Fenglin Liu*

Main category: cs.CV

TL;DR: 基于神经场景表示的自我监督3D CBCT重建方法，结合物理迭代细化以消除截断伪影并保留高频细节。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法依赖监督且无法处理连续3D截断变化，需要自我监督和稳健的外推能力。

Method: 使用神经场景表示将空间坐标映射到辐射密度，绕过传统重建，消除截断伪影；通过基于物理的迭代细化模块从原始投影恢复高频纹理。

Result: 在模拟和真实数据集上，该方法有效抑制伪影并保留高保真细节。

Conclusion: 该方法统一了神经网络伪影抑制和外推能力与迭代算法细节保留。
...
### [73] [LoomVideo: Unifying Multimodal Inputs into Video Generation and Editing](https://arxiv.org/abs/2606.06042)
...
TL;DR: LoomVideo是一个5B参数的高效统一架构，通过零开销的Scale-and-Add条件化避免了token拼接，显著降低计算成本，实现视频生成和编辑。


<details>
  <summary>Details</summary>
Motivation: 现有统一框架依赖大型模型（13B+）并通过token拼接引入源视频条件，导致序列长度加倍和自注意力计算复杂度翻四倍，带来巨大开销。

Method: LoomVideo使用MLLM替换文本编码器，采用Deepstack注入对齐MLLM特征与DiT；引入零开销的Scale-and-Add条件化用于视频编辑；集成Negative Temporal RoPE处理多参考图像。

Result: 在综合基准上达到SOTA或竞争力性能，在电商和时尚生成中表现优越；推理速度比类似模型快至少5.41倍。

Conclusion: LoomVideo为高效实用的视频基础模型铺平了道路，展示了紧凑架构在统一视频生成和编辑中的潜力。
...
### [74] [LLM-Conditioned Synthesis of Pathological Gaits via Structured Gait-Language Representations](https://arxiv.org/abs/2606.06048)
*Mritula Chandrasekaran,Sanket Kachole,Jarik Francik,Dimitrios Makris*

Main category: cs.CV

TL;DR: 提出多模态LLM引导的框架，从文本描述生成病理感知的3D步态序列，通过合成数据增强病理步态分类。


<details>
  <summary>Details</summary>
Motivation: 病理步态数据集因隐私、招募、成本和运动变异性而稀缺，需要有效的数据增强方法。

Method: 结合运动令牌化、病理感知语言条件、LLM语义增强和语言到步态生成，关键贡献是病理令牌化器以保留病理运动特征。

Result: 合成序列与真实数据结合训练GRU分类器，在留一受试者评估中达到92.77%准确率。

Conclusion: 合成数据能有效改善下游病理步态分类任务，特别是与真实数据结合时。
...
### [75] [FontFusion: Enhancing Generative Text in Diffusion Models with Typographic Conditioning](https://arxiv.org/abs/2606.06066)
*Marian Lupascu,Nipun Jindal,Ionut Mironica,Zhaowen Wang*

Main category: cs.CV

TL;DR: FontFusion is a plug-and-play conditioning framework for diffusion transformers that achieves both precise font control and text legibility through hierarchical token representations, position-aware embeddings, and multi-level token dropping.


<details>
  <summary>Details</summary>
Motivation: Typography generation in diffusion models faces a persistent trade-off between precise font control and text legibility, motivating a framework that resolves this dilemma without compromising either aspect.

Method: FontFusion introduces three core innovations: (1) hierarchical token representation establishing explicit text-font relationships at multiple granularities, (2) position-aware embeddings creating spatial bindings between typography and image content, and (3) a multi-level token dropping strategy improving efficiency and generalization. A dual encoder combining DeepFont and DINOv2 is used.

Result: FontFusion achieves 76% relative improvement on challenging decorative fonts over single-encoder baselines and font consistency gains of approximately 68-76% over unconditioned models.

Conclusion: FontFusion integrates into existing DiT architectures without retraining, demonstrating significant improvements in typographic fidelity and legibility, especially for decorative fonts, resolving the trade-off effectively.
...
TL;DR: 提出了一个高效的事件到帧重建框架，通过因果时间建模和计算感知设计实现准确性与复杂性的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法在重建质量和计算效率之间存在权衡，需要更好的平衡。

Method: 采用循环编码器-解码器结构聚合事件信息，引入选择性上下文融合和轻量级混合注意力机制。

Result: 在标准基准上取得了有竞争力的重建性能，并在准确性和模型复杂性之间实现了良好平衡。

Conclusion: 所提出的E2F框架在保证高质量重建的同时，具有较低的计算复杂度，适合实际应用。
...
TL;DR: 提出一种无需参数的自适应视频令牌分配方法，通过直接利用潜在空间中的时域差异来丢弃冗余令牌，并使用轻量级LIT模型重建丢弃位置，从而高效实现内容驱动的压缩。


<details>
  <summary>Details</summary>
Motivation: 现有自适应视频标记化方法需要额外计算开销（如迭代二值化搜索、神经回归器或全速率解码器），作者证明这些并非必需，并希望简化流程。

Method: 使用冻结的连续视频标记器的潜在空间，计算逐位置时域L1差异，应用固定阈值识别并丢弃冗余潜在位置；引入轻量级分解空间-时间注意力架构（LIT）重建被丢弃的位置，只需一次编码器传递和一次LIT前向传递。

Result: 在TokenBench和DAVIS基准上，方法产生内容驱动的令牌分配并保持竞争性重建保真度，相比ElasticTok-CV实现31倍推理加速，相比InfoTok实现约2倍加速。

Conclusion: 所提框架无需辅助网络即可高效实现自适应令牌分配，推理速度快且压缩率自然依赖于内容，为视频标记化提供了更简洁有效的方案。
...
### [78] [Adversarial Attacks Already Tell the Answer: Directional Bias-Guided Test-time Defense for Vision-Language Models](https://arxiv.org/abs/2606.06186)
*Liangsheng Liu,Si Chen,Jiamin Wu,Weiwei Feng,Zhixin Cheng,Xiaotian Yin,Wenfei Yang,Tianzhu Zhang*

Main category: cs.CV

TL;DR: We propose DBD, a test-time defense for VLMs that leverages a dominant shift direction (Defense Direction) in feature space to achieve state-of-the-art adversarial robustness, even surpassing clean accuracy.


<details>
  <summary>Details</summary>
Motivation: VLMs like CLIP are highly vulnerable to adversarial perturbations, and test-time defenses offer an efficient approach without costly retraining.

Method: We uncover that adversarial images shift along a dominant direction in CLIP's feature space, and propose DBD, which estimates the Defense Direction and uses a DB-score-based two-stream reconstruction strategy to recover robust representations.

Result: DBD achieves state-of-the-art adversarial robustness on 15 datasets while preserving clean accuracy, with the counterintuitive result that adversarial accuracy can surpass clean accuracy.

Conclusion: Adversarial perturbations inherently encode directional priors about the true decision boundary.
...
### [81] [EasyLens: A Training-Free Plug-and-Play Subtle-Lesion Representation Amplifier for Medical Vision-Language Models](https://arxiv.org/abs/2606.06379)
*Qiwei Zeng,Hao Wang,Jinghao Lin,Shuchang Ye,Yuezhe Yang,Yige Peng,Haoyuan Che,Jinman Kim,Lei Bi*

Main category: cs.CV

TL;DR: EasyLens是一种无需训练的即插即用模块，通过构建病理-解剖原型空间和反事实推理增强细微病变表示，提升医学视觉语言模型对细微病变的检测能力。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型对细微病变不敏感，因为病变线索在全局表示中被稀释；现有增强方法需要额外训练或特定适应，缺乏通用性。

Method: EasyLens包含三个组件：EasyBank构建病理-解剖原型空间；EasyTag通过反事实原型推理选择病变相关补丁；EasyAmplifier通过形态引导的残差增强强化病变补丁表示。

Result: 在多个医学图像数据集和冻结医学视觉语言模型上，EasyLens显著提升细微病变检测性能，优于现有编码器增强基线。

Conclusion: EasyLens是一种有效且通用的细微病变表示放大器，无需训练即可增强医学视觉语言模型的病变敏感性。
...
### [84] [Three-Dimensional Retinal Microvasculature Restoration in OCT Angiography](https://arxiv.org/abs/2606.05375)
*Yukun Guo,Min Gao,Tristan T. Hormel,Steven T. Bailey,Thomas S. Hwang,Yali Jia*

Main category: cs.CV

TL;DR: 提出深度学习算法从单个OCTA体积恢复毛细血管解剖结构，显著提高图像质量和微血管保真度。


<details>
  <summary>Details</summary>
Motivation: 现有OCTA图像处理忽视三维血管结构，导致血流和灌注量化困难。

Method: 使用EfficientNet-B5编码器和带空间通道挤压激励模块的解码器，输入三个相邻B帧预测中间帧。

Result: PSNR从22.23提高到26.16，SSIM从0.72提高到0.91，微血管Dice系数在2D和3D分别提高3.8%和51.2%。

Conclusion: 该算法有效改善OCTA图像质量和微血管保真度，有潜力用于临床分析。
...
### [85] [Deep Learning-assisted AMD Staging based on OCT and OCT Angiography](https://arxiv.org/abs/2606.05379)
*Yukun Guo,Tristan T. Hormel,An-Lun Wu,Liqin Gao,Min Gao,Steven T. Bailey,Yali Jia*

Main category: cs.CV

TL;DR: 深度学习模型使用OCT/OCTA数据自动分级AMD严重程度，其中基于生物标志物的模型表现最佳。


<details>
  <summary>Details</summary>
Motivation: 开发和评估深度学习模型用于自动分级年龄相关性黄斑变性（AMD）严重程度，使用OCT和OCTA数据。

Method: 271名参与者（≥50岁）的351只眼睛，使用扫频源OCTA系统获取中央黄斑6x6 mm OCT/OCTA体积，AMD严重程度按AREDS简化量表分为四期。开发三种模型：基于生物标志物地图（视网膜液体、玻璃膜疣、地图状萎缩、黄斑新生血管）、2D en face OCT/OCTA投影、3D OCT/OCTA体积。使用EfficientNet架构，归一化输入、数据增强和五折交叉验证。

Result: 所有模型QWK≥0.83，基于生物标志物的模型总体最佳（QWK=0.85±0.03），早期AMD检测F1分数最高（0.59±0.14）。3D模型与2D模型性能相当（QWK=0.83±0.04 vs 0.83±0.09），2D模型精度最高（0.79±0.06）且最准确识别无AMD眼。

Conclusion: 深度学习模型使用OCT/OCTA数据可准确自动分级AMD严重程度，其中基于生物标志物的模型性能最均衡，对早期AMD检测特别有价值。
...
### [86] [BMCR: Adaptive Backbone Module Composition via Reinforcement Learning for Remote Sensing Object Detection](https://arxiv.org/abs/2606.05586)
*Wenlin Liu,Xikun Hu,Ping Zhong*

Main category: cs.CV

TL;DR: 提出基于强化学习的BMCR方法，动态组合CNN和ViT模块构建自适应推理路径，在遥感检测数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有检测器依赖单一固定主干或手动设计混合架构，无法自适应利用CNN的局部细节和ViT的全局上下文建模能力。

Method: 通过构建可扩展模块工具箱（分解并封装CNN和ViT模块），设计基于最优传输的过渡接口对齐特征，将主干组成建模为序列决策问题，并使用自适应模块协作优化策略联合训练。

Result: 在DOTA-v1.0、DOTA-v1.5和DIOR-R上分别达到79.31%、73.41%和71.86% mAP，超过强基线最多2.5个点。

Conclusion: BMCR有效融合CNN和ViT的互补优势，通过动态模块组合显著提升遥感目标检测性能。
...
### [87] [HDST-GNN: Heterogeneous Dynamic Spatiotemporal Graph Neural Networks for Multi-Object Tracking in UAV Aerial Imagery](https://arxiv.org/abs/2606.05587)
*Phillip Jiang*

Main category: cs.CV

TL;DR: Proposes a heterogeneous dynamic spatiotemporal graph neural network for multi-object tracking from UAV imagery, addressing altitude variation, small objects, and occlusion.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based trackers for UAV MOT assume fixed spatial context and treat all objects uniformly, ignoring heterogeneous lifecycle states of detections, tracklets, and lost targets, leading to poor handling of altitude variation, small objects, and occlusion.

Method: HDST-GNN introduces Altitude-Adaptive Edge Construction to adjust graph connectivity based on camera altitude proxy, Heterogeneous Node Representation to model detections, confirmed tracklets, and lost tracklets as distinct node types with typed edges, and Occlusion-Gated Temporal Aggregation to gate attention contributions by occlusion confidence. It is trained end-to-end with a differentiable Sinkhorn head using joint cross-entropy and triplet loss.

Result: On VisDrone2019-MOT with oracle detections, HDST-GNN achieves 94.51% MOTA and 97.24% IDF1, outperforming SORT by +5.0 MOTA and reducing identity switches by 81%. With real YOLOv8n detections, it reduces identity switches by 49% vs SORT.

Conclusion: HDST-GNN significantly improves multi-object tracking performance on UAV imagery by incorporating heterogeneous node types and dynamic altitude-adaptive graph construction, effectively reducing identity switches and improving tracking accuracy.
...
### [88] [CoFi-UCGen: Coarse-to-Fine Unsupervised Conditional Generation without Label Priors](https://arxiv.org/abs/2606.05652)
*Shengxi Li,Zhaokun Hu,Ce Zheng,Mai Xu,Jingyuan Xia,Si Liu*

Main category: cs.CV

TL;DR: 提出一种粗到细的无监督条件图像生成框架 CoFi-UCGen，通过对抗语义互惠学习和位编码实现无标签条件下的全局与细粒度语义控制，在扩散模型中引入分层调制机制，显著提升图像质量与语义一致性。


<details>
  <summary>Details</summary>
Motivation: 无监督条件图像生成因缺乏结构化语义表示而难以控制生成结果，现有方法无法同时实现粗粒度和细粒度的条件控制。

Method: 首先提出对抗语义互惠学习理论保证图像与潜空间的语义一致性；基于该一致性设计位编码以学习结构化粗粒度潜空间；进而建立细粒度语义基础，并在扩散模型中引入分层调制机制，通过逐层注入粗条件逐步控制细粒度属性。

Result: 在无标签先验和预训练特征提取器的条件下，CoFi-UCGen 在图像质量、语义一致性和控制准确性上均优于现有无监督条件图像生成方法。

Conclusion: 显式的粗到细语义分解能有效应对无监督条件图像生成中的挑战，验证了所提框架的有效性。
...
### [89] [Cosine Misleads: Auxiliary Losses Reshape Vision Language Models, Not Their Latents](https://arxiv.org/abs/2606.05753)
*XiuYu Zhang,Junfeng Fang,Zhenkai Liang*

Main category: cs.CV

TL;DR: 研究表明LVR中对齐假设是反向的，监督潜在变量被绕过，并提出了PRISM诊断方法。


<details>
  <summary>Details</summary>
Motivation: 测试LVR中更好的对齐是否导致更好的答案的假设。

Method: 设计五种LVR变体矩阵，引入PRISM诊断（线性探针和腐败测试）。

Result: 余弦对齐与准确性负相关（r=-0.94），监督潜在变量被绕过，腐败测试影响小，可解码性差距预测依赖。

Conclusion: 辅助目标通过共享参数重塑语言模型，而非通过潜在变量本身，符合信息瓶颈解读。
...
### [90] [DRIFT: A Residual Flow Adapter for Decoding Continuous Outputs in Vision-Language Models](https://arxiv.org/abs/2606.05758)
*Zhuoming Liu,Jinhong Lin,Kwan Man Cheng,Lin Zhang,Shayok Bagchi,Yin Li*

Main category: cs.CV

TL;DR: 提出DRIFT框架，结合基础预测器和流匹配生成细化，将预训练VLM适配到连续解码任务。


<details>
  <summary>Details</summary>
Motivation: 现有基于离散token解码的VLM在需要精确连续输出的任务（如时间定位和机器人控制）上表现不佳。

Method: DRIFT框架包含基础预测器提供粗略估计，然后通过基于流匹配的生成细化模块迭代改进预测，将生成建模从全局输出分布转为局部残差分布。

Result: 在视觉基础和机器人控制等多个任务及多种架构上，DRIFT优于回归和生成基线。

Conclusion: DRIFT是一种有效的将VLM适配到连续解码任务的通用框架。
...
### [91] [ExpSpeech-Net: Multimodal Fusion of Expression and Speech for Deepfake Detection](https://arxiv.org/abs/2606.05760)
*Ruchika Sharma,Rudresh Dwivedi*

Main category: cs.CV

TL;DR: 提出了ExpSpeech-Net深度伪造检测模型，使用SqueezeNet和RNN同时分析面部表情和语音模式，通过智能特征提取与选择实现高效检测。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法依赖复杂资源密集型模型，限制了实际应用。

Method: 采用SqueezeNet和RNN作为骨干，结合ISLBT和MPNCC特征提取，并使用SASMA算法进行特征选择，实现多模态融合。

Result: 准确率94.5%，精确度99.3%，F-measure 96.8%，优于传统方法。

Conclusion: 集成多模态与智能预处理、特征选择可实现实用实时的深度伪造检测。
...
### [92] [Self-Learning Expression Deformations for Data-Efficient Gaussian Avatars](https://arxiv.org/abs/2606.05912)
*Jiahao Yang,Xiaohang Yang,Qing Wang,Yilan Dong,Gregory Slabaugh,Shanxin Yuan*

Main category: cs.CV

TL;DR: SAGE框架通过自监督学习表达引起的3D高斯变形，在最小输入数据下实现高保真、可动画化的人脸模型。


<details>
  <summary>Details</summary>
Motivation: 传统高斯化身管线需要大量的多视图和序列表情数据，限制了可扩展性和可访问性。

Method: 联合优化2D高斯面元和符号距离场（SDF）以强制实现紧凑的、与表面对齐的高斯分布，并通过自监督表达学习阶段使用几何和外观一致性约束代替长训练序列。

Result: 实验表明，该方法在重建和动画质量上与最先进的方法相当，同时将数据需求减少了几个数量级。

Conclusion: 自监督高斯变形学习为实现可访问、数据高效的化身创建提供了潜力。
...
### [93] [Multimodal Sexism Identification and Characterization using Large Language Models and Gradient Boosting](https://arxiv.org/abs/2606.05997)
*Kyriakos Chaviaras,Maria Lymperaiou,Athanasios Voulodimos*

Main category: cs.CV

TL;DR: AILS-NTUA提交到EXIST 2026，使用特征工程晚期融合管道进行多模态性别歧视识别，发现LLM语义提升模因识别，而视频需要更好的时间建模。


<details>
  <summary>Details</summary>
Motivation: 参与EXIST 2026实验室挑战，解决模因和短视频中的多模态性别歧视识别与特征化问题。

Method: 基于梯度提升回归模型和层次化后处理的特征工程晚期融合管道。对模因使用视觉、文本、人口统计、生物特征和LLM衍生语义指标；对视频使用特征选择、帧视觉、OCR文本、声学描述符和传感器元数据。

Result: 开发结果显示LLM语义线索改善模因性别歧视识别；视频性能对特征维度敏感，紧凑特征选择在开发中更好，但测试中未过滤表示泛化更佳。

Conclusion: 针对静态模因的有针对性语义特征工程有用，短视频需要更稳健的时间建模以应对噪声。
...
### [94] [Knowledge Distillation for Visual Autoregressive Models](https://arxiv.org/abs/2606.06078)
*Elia Peruzzo,Aritra Bhowmik,Guillaume Sautiere,Yuki M Asano,Amirhossein Habibian*

Main category: cs.CV

TL;DR: 提出VarKD，一种针对视觉自回归模型的知识蒸馏框架，通过选择性教师监督和减少令牌歧义，有效提升了压缩效率，在ImageNet上优于先前基线。


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成模型计算量大，需要有效压缩；知识蒸馏在语言建模中成功，但在视觉自回归生成中研究不足，且语言蒸馏方法因长解码和令牌歧义不直接适用。

Method: VarKD框架，在学生样本上进行蒸馏，同时选择性应用教师监督并减少令牌级歧义，以解决长解码和令牌歧义问题。

Result: 在ImageNet上多个自回归骨干中，VarKD始终优于先前蒸馏基线，缩小了与大规模模型的性能差距。

Conclusion: VarKD是一种有效的视觉自回归模型压缩方法，能够弥补大规模模型与小模型之间的差距，为未来研究提供了方向。
...
### [95] [Diff-CA: Separating Common and Salient Factors with Diffusion Models](https://arxiv.org/abs/2606.06120)
*Michaël Soumm,Alexandre Fournier Montgieux,Yunlong He,Pietro Gori,Alasdair Newson*

Main category: cs.CV

TL;DR: 提出一种基于扩散模型的对比性条件框架，实现共同与显著因子的可识别分解，支持高保真图像生成和编辑。


<details>
  <summary>Details</summary>
Motivation: 现有对比性方法基于VAEs或GANs，受限于重建和图像质量，影响因子分离和高保真应用，需要不牺牲生成质量的分解方法。

Method: 先训练无提示、图像条件的扩散模型，再用弱监督学习将条件分解为共同和显著因子，采用加性对比因子分解并证明其可识别性。

Result: 实现有效的对比性分解，支持通过交换或插值显著因子进行图像编辑，且不损害生成质量。

Conclusion: 该框架克服了现有方法的局限性，能够在不牺牲生成质量的前提下进行对比性分解，适用于高保真图像生成和编辑。
...
### [96] [GRAMformer: Any-Order Modality Interactions via Volumetric Multimodal Cross-Attention](https://arxiv.org/abs/2606.06249)
*Giordano Cicchetti,Eleonora Grassucci,Danilo Comminiello*

Main category: cs.CV

TL;DR: 提出VMA注意力机制和GRAMformer架构，通过计算查询和多键的体积来建模多模态交互，提高多模态学习效果和效率。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态注意力方法要么计算复杂度高（二次复杂度），要么无法显式建模多个模态之间的联合交互。因此需要一种能高效建模任意阶模态交互的方法。

Method: 提出了Volumetric Multimodal cross-Attention (VMA)，其注意力分数定义为查询和多个模态特定键的几何体积的函数，从而捕获联合多模态依赖。VMA被集成到新型多模态transformer架构GRAMformer中，该架构设计用于整合任意数量的模态。

Result: 在多模态学习任务上，GRAMformer展示了改进的有效性和效率。

Conclusion: VMA和GRAMformer通过建模任何阶模态交互，提供了一种更有效和高效的多模态学习方式，克服了现有方法的局限性。
...
### [97] [Geodesic Flow Matching on a Riemannian Degradation Manifold for Blind Image Restoration](https://arxiv.org/abs/2606.06278)
*Akshay Janardan Bankar,Ankita Chatterjee,Sayan Banerjee,Shreyas Pandith,Kalakonda Sai Shashank,Amit Satish Unde*

Main category: cs.CV

TL;DR: 提出一种基于黎曼流形和测地流匹配的盲图像恢复方法


<details>
  <summary>Details</summary>
Motivation: 现有基于流的确定性方法假设线性退化几何，无法处理混合退化

Method: 将退化建模为低维黎曼流形上的点，在联合图像-流形空间中使用测地流匹配目标学习内蕴传输动力学

Result: 框架推广了线性流匹配，提供了混合退化的测地组成处理，并对未观察到的退化具有泛化能力

Conclusion: 该方法能更好地处理盲图像恢复中的复杂退化
...
### [98] [Synthetic Data Generation and Vision-based Wrinkle and Keypoint Detection for Bimanual Cloth Manipulation](https://arxiv.org/abs/2606.06292)
*Ariel Herrera,Xueyang Kang,Atal Anil Kumar*

Main category: cs.CV

TL;DR: 提出一种结合合成数据和CNN的感知框架，实现机器人对纺织品的鲁棒操纵，从褶皱拉伸到关键点熨烫。


<details>
  <summary>Details</summary>
Motivation: 机器人操纵纺织品因连续变形和自遮挡导致视觉感知困难，且缺乏标注的真实世界数据。

Method: 利用Blender合成管线生成自动标注关键点，结合手动标注训练CNN进行置换不变关键点检测；YOLOv8-OpenCV管线从结构褶皱提取抓取点；双臂算法先通过褶皱拉伸服装，再过渡到基于关键点的熨烫。

Result: 关键点模型平均位置误差1.7615像素；感知系统无需微调即可迁移至真实织物，在高遮挡状态下优于基线。

Conclusion: 该感知框架有效解决了纺织品操纵中的视觉感知问题，具有良好的泛化性和鲁棒性。
...
### [99] [RhymeFlow: Training-Free Acceleration for Video Generation with Asynchronous Denoising Flow Scheduling](https://arxiv.org/abs/2606.06309)
*Chensheng Dai,Shengjun Zhang,Yifan Li,Zhang Zhang,Zheng Zhu,Yueqi Duan*

Main category: cs.CV

TL;DR: 提出RhymeFlow，通过识别关键帧仅对其密集去噪，非关键帧跳过步骤，并引入潜在轨迹投影模块保持时间一致性，加速DiT视频生成并保持质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在每个去噪步骤中减少计算，但受限于标准扩散管线：所有帧必须经历完整密集去噪。观察到相邻帧冗余，均匀密集去噪不必要。

Method: 首先识别稀疏的关键帧，仅对关键帧进行逐步密集去噪，非关键帧逐步跳过步骤；引入潜在轨迹投影模块，使关键帧与完整时间一致的序列表示交互，弥补跳过步骤带来的时间连贯性破坏。

Result: 在现有DiT视频生成模型上，RhymeFlow比现有基线推理速度更快，且视觉质量更好。

Conclusion: RhymeFlow通过解耦不同帧的去噪轨迹，有效减少了计算冗余，在加速推理的同时保持或提升了视频生成质量。
...
### [100] [Physics in 2-Steps: Locking Motion Priors Before Visual Refinement Erases Them](https://arxiv.org/abs/2606.06361)
*Woojung Han,Seil Kang,Youngjun Jun,Min-Hung Chen,Fu-En Yang,Seong Jae Hwang*

Main category: cs.CV

TL;DR: Proposes PhaseLock to improve physical consistency in Image-to-Video diffusion by preserving motion priors from few-step inference.


<details>
  <summary>Details</summary>
Motivation: Image-to-Video models generate visually appealing videos but often violate physical laws due to phase degradation during denoising.

Method: PhaseLock extracts motion prior from 2-step generation and enforces it via Latent Delta Guidance without retraining.

Result: Improves physical consistency by 6.2 points with negligible overhead (1.06× time, 1.02× memory) and reduces reliance on external guidance.

Conclusion: PhaseLock effectively mitigates phase degradation, leading to physically consistent video generation.
...
### [101] [Thinking with Imagination: Agentic Visual Spatial Reasoning with World Simulators](https://arxiv.org/abs/2606.06476)
*Chenming Zhu,Jingli Lin,Yilin Long,Peizhou Cao,Tai Wang,Jiangmiao Pang,Xihui Liu*

Main category: cs.CV

TL;DR: 提出Astra框架，通过行动条件的视觉想象增强VLMs的空间推理，使用世界模拟器生成想象观测并结合RL训练学习调用模拟器的时机。


<details>
  <summary>Details</summary>
Motivation: VLMs在空间推理中受限于观察图像，难以推断未观测布局和保持跨视图一致性，将问题视为通过世界模拟器进行想象性推理。

Method: Astra包含Astra-VL（RL训练的VLM策略）和Astra-WM（基于Bagel的世界模拟器，通过视图一致性训练生成新视角），并采用两阶段RL课程学习何时调用模拟器。

Result: Astra-WM在MMSI-Bench将Gemini-3-Flash从45.1提升到49.5；Astra-VL将Qwen3-VL在MMSI-Bench上从29.8提升到38.8，在MindCube上从36.8提升到42.7。

Conclusion: 想象观测能提供空间证据，但有效增强推理需学习何时、何地、如何想象。
...
### [102] [IA-RAG: Interval-Algebra-Driven Temporal Reasoning for Dynamic Knowledge Retrieval](https://arxiv.org/abs/2606.06044)
*Xiaoman Wang,Yaoze Zhang,Wenzhuo Fan,Hongwei Zhang,Ding Wang,Guohang Yan,Song Mao,Botian Shi,Yunshi Lan,Pinlong Cai*

Main category: cs.CL

TL;DR: IA-RAG is a hierarchical temporal RAG framework that models facts as time intervals using Allen's Interval Algebra, organized in a Thematic Forest, and achieves strong temporal reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Existing RAG and Graph RAG frameworks treat knowledge as static or with coarse timestamps, failing to capture rich temporal structures such as duration, overlap, and containment.

Method: IA-RAG represents facts as Interval Event Units (IEUs) in a hierarchical Thematic Forest, uses Allen's Interval Algebra for temporal constraints, introduces Sub-graph Time Tightening for fuzzy intervals, and supports implicit temporal semantic retrieval via interval-algebra-guided traversal.

Result: Achieves strong temporal retrieval and reasoning performance on TimeQA, TempReason, and ComplexTR benchmarks, especially on complex compositional temporal reasoning tasks.

Conclusion: IA-RAG effectively handles complex temporal reasoning by modeling rich temporal structures within a retrieval-augmented generation framework.

Abstract: Retrieval-Augmented Generation (RAG) has shown strong effectiveness in grounding Large Language Models (LLMs) with external knowledge. However, existing RAG and Graph RAG frameworks largely treat knowledge as static or associate time with coarse-grained timestamps or metadata, failing to capture rich temporal structures such as duration, overlap, and containment. We propose IA-RAG, a hierarchical temporal RAG framework that models knowledge as time intervals and performs retrieval under formal temporal constraints. IA-RAG represents facts as Interval Event Units (IEUs) and organizes them into a hierarchical Thematic Forest, where temporal dependencies are governed by Allen's Interval Algebra. To handle incomplete or uncertain temporal boundaries, IA-RAG further introduces a Sub-graph Time Tightening mechanism that refines fuzzy intervals through logical constraints within connected event subgraphs. In addition, IA-RAG supports implicit temporal semantic retrieval through interval-algebra-guided traversal. Experiments on multiple temporal question answering benchmarks, including TimeQA, TempReason, and ComplexTR, demonstrate that IA-RAG achieves strong temporal retrieval and reasoning performance, particularly on complex compositional temporal reasoning tasks. Our code is released at .

</details>


### [103] [QueryAgent-R1: Bridging Query Generation and Product Retrieval for E-Commerce Query Recommendation](https://arxiv.org/abs/2606.05671)
*Dike Sun,Zheng Zou,Jingtong Zang,Qi Sun,Huaipeng Zhaoand Tao Luo,Xiaoyi Zeng*

Main category: cs.CL

TL;DR: QueryAgent-R1 通过记忆增强的代理框架和链式检索优化，在电商搜索中同时提高查询点击率和产品转化率。


<details>
  <summary>Details</summary>
Motivation: 现有查询推荐方法主要优化查询级相关性，忽视了检索产品与用户下游偏好的匹配，导致高CTR但低CVR。

Method: 提出QueryAgent-R1，包括基于实际库存检索的查询生成、一致性奖励的强化学习优化、以及记忆抽象模块用于用户画像。

Result: 在工业数据和公开数据集上优于强基线，在线A/B测试中Query CTR提升2.9%，guided CVR提升3.1%。

Conclusion: QueryAgent-R1 有效桥接了查询相关性和下游参与之间的差距，提升了端到端对齐。
...
### [104] [Reducing Hallucinations in Complex Question Answering using Simple Graph-based Retrieval-Augmented Generation (long version)](https://arxiv.org/abs/2606.05901)
*Christopher J. Wedge,Joshua Stutter,Danny Dixon,Jacek Cała*

Main category: cs.CL

TL;DR: 探索在 RAG 系统中引入轻量级图结构以支持检索和生成，显著提升事实正确性并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: LLM 容易产生幻觉，RAG 系统虽能缓解但仍有不足，希望利用图结构进一步增强 RAG 的准确性和可靠性。

Method: 设计一个基于代理的系统，结合向量搜索和图查询工具，在结构化维基百科子集上运行，并在 MoNaCo 复杂问答基准上评估。

Result: 引入图工具后，事实正确性的精确度和召回率显著提高，幻觉答案减少一半，获得最高细粒度真实性分数，且 token 使用量仅适度增加。

Conclusion: 基于图的工具能够有效提升 RAG 系统的性能，降低幻觉，且成本可控，值得进一步探索。
...
TL;DR: 提出三潜在序列模型，在字节级和语言模型基准上改进Transformer，但召回扩展对种子敏感且速度慢。


<details>
  <summary>Details</summary>
Motivation: 现有序列模型难以有效捕获高阶令牌交互，且依赖基准特定解析，需要通用方法。

Method: 采用运行令牌状态和压缩对记忆路径的三潜在结构，并引入门控键值检索增强关联召回。

Result: 在WikiText-2和MiniMind基准上优于小型Transformer基线，但召回扩展对种子敏感且参考实现运行缓慢。

Conclusion: 三潜在模型有潜力，但当前扩展的效率和稳定性有待提升。
...
TL;DR: LANTERN通过零LLM调用的混合检索恢复对话历史压缩中丢失的细节，显著优于LLM驱动的基线方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在对话历史压缩时会丢失关键细节，需要一种高效且低成本的恢复方法。

Method: LANTERN（分层存档与时间情景检索网络）是一个轻量级记忆层，主动存档每轮对话，在压缩后通过混合检索恢复相关细节，无需LLM调用且延迟低于25ms。

Result: 在94个真实多轮对话（1894个事实）上，LANTERN-Rerank恢复78.3%的可验证事实，优于MemGPT的72.4%（p<0.0001）；零LLM调用的基础LANTERN匹配或超过该基线。使用恢复的上下文，四个生产LLM准确率平均提升8.4个百分点。

Conclusion: LANTERN有效恢复压缩丢失的细节，且评估框架支持可重复性和未来工作。
...
### [107] [Executable Schema Contracts: From Automatic Ingestion to Multi-Source Retrieval](https://arxiv.org/abs/2606.05415)
*Padmaja Jonnalagedda,Yuguang Yao,Xiang Gao,Hilaf Hasson,Kamalika Das*

Main category: cs.CL

TL;DR: 提出一个自动从多源数据发现可执行模式并用于知识图谱构建和查询的系统，在QA基准上显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据异构且不一致，现有方法要么成本高要么忽略结构，因此需要一种自动发现模式并利用模式进行查询的方法。

Method: 系统使用封闭世界字段目录约束LLM模式发现，通过确定性结构分析推断键和层次，构建感知来源的知识图谱；查询时模式控制多工具代理进行检索。

Result: 在零样本设置下，系统在四个QA基准上优于检索型和分解型基线，消融实验证实各组件均有贡献。

Conclusion: 该系统通过自动模式发现和条件路由有效提升了多源数据查询的准确性和可解释性。
...
### [108] [Narrative Knowledge Weaver: Narrative-Centric Retrieval-Augmented Reasoning for Long-Form Text Understanding](https://arxiv.org/abs/2606.05724)
...
TL;DR: NKW是一个源基础框架，通过整合文本证据、原子事实、图结构、实体轮廓等，提升了长形式叙事QA中的故事世界推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有检索和图增强方法使用的单元（如chunks、entities等）不直接编码证据在故事中的功能，难以处理需要跨故事世界推理的复杂叙事问题。

Method: NKW框架对齐文本证据、原子事实、规范图结构、实体轮廓、交互、情节和故事线，并在查询时使用文本、图和叙事工具以及后检索阅读技能来组装证据并审计actor、scope、polarity、state和temporal约束。

Result: 在STAGE、FairytaleQA和QuALITY基准上，NKW在剧本级故事世界QA上表现最强，同时在以段落为中心的基准上保持竞争力。

Conclusion: 消融、问题类型分析、图资产统计和案例研究显示，NKW在角色、场景、时间、因果和叙事进展推理方面具有互补优势。
...
### [109] [AdaPLD: Adaptive Retrieval and Reuse for Efficient Model-Free Speculative Decoding](https://arxiv.org/abs/2606.05742)
*Runheng Liu,Jincheng Xie,Wen Hu,Xingchen Xiao,Heyan Huang*

Main category: cs.CL

TL;DR: AdaPLD是一种免训练的推测解码方法，通过自适应语义检索和分支复用假设，加速大模型生成。


<details>
  <summary>Details</summary>
Motivation: 现有的基于文本重用的推测解码方法受限于词汇锚定检索的低召回率和确定性跨度复制的脆弱性，导致速度提升有限。

Method: 提出AdaPLD，结合词汇精确匹配和语义相似性检索，并构建分支复用假设以应对不确定性，无需额外训练。

Result: 在多个基准测试中，AdaPLD显著减少目标模型的前向传递次数，最高实现3.10倍的解码加速。

Conclusion: AdaPLD高效地解决了现有重用方法的局限性，实现了显著的解码加速，且无需额外训练。
...
TL;DR: 提出查询自适应框架，通过跨模态分数一致性检测活动模态，在多模态人物检索中动态融合语音和面部特征，在BBC Rewind语料库上实现94.2% P@1，优于固定融合和单模态系统。


<details>
  <summary>Details</summary>
Motivation: 固定多模态融合在真实广播档案中因缺失模态而引入噪声，需要自适应方法以避免精度下降。

Method: 通过跨模态分数一致性检测主动模态，仅当两种模态都活跃时置信融合，分类器利用跨模态特征实现89%的活动模态检测准确率。

Result: 在BBC Rewind语料库上，自适应系统P@1达94.2%，超过语音(82.9%)、面部(93.4%)和固定融合(90.0%)，恢复64%的差距至oracle(96.6%)。

Conclusion: 查询自适应融合显著提升多模态人物检索性能，有效处理缺失模态问题。
...
### [111] [Self-Augmenting Retrieval for Diffusion Language Models](https://arxiv.org/abs/2606.06474)
*Paul Jünger,Justin Lovelace,Linxi Zhao,Dongyoung Go,Kilian Q. Weinberger*

Main category: cs.CL

TL;DR: 提出SARDI，一种训练免费的动态检索增强生成框架，利用离散扩散语言模型在去噪过程中丢弃的标记作为前瞻信号来指导检索，在多跳问答任务中以高达8倍吞吐量超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 发现离散扩散语言模型在迭代去噪过程中丢弃的低置信度标记实际上包含了早期出现的显著实体，可以作为检索增强生成的有用前瞻信号。

Method: 提出SARDI框架，在扩散模型的每一步去噪中，使用被丢弃的低置信度标记作为前瞻信号来指导检索，从而在输出最终确定前获取更强的证据。

Result: 在五个多跳问答基准测试中，SARDI以高达8倍的吞吐量优于当前无需训练的扩散和自回归检索基线。

Conclusion: SARDI是一种无需训练、与检索器无关的动态RAG框架，适用于任何具有推理能力的离散扩散语言模型，能够有效利用前瞻信号提高检索质量和生成效率。
...
### [112] [Forgive or forget: Understanding the context of hate in audio retrieval systems](https://arxiv.org/abs/2606.05857)
*Arghya Pal,Sailaja Rajanala,Raphael C.-W. Phan,Shekhar Nayak*

Main category: cs.CL

TL;DR: 提出一种事后因果去偏框架，通过情感控制中介减少文本到音频系统中的毒性检索，同时保持语义相关性。


<details>
  <summary>Details</summary>
Motivation: 现有处理毒性检索的策略（如改写、摘要）可能改变意图或遗漏细节，需要一种既能保持相关性又能抑制有害语音的方法。

Method: 提出模型无关的因果去偏框架，包含Forgive（通过logit调整重排和过滤）和Forget（生成反事实毒性提示）两种变体。

Result: 实验表明在检索准确性损失很小的情况下，毒性显著降低。

Conclusion: 该方法提高了文本到音频系统的安全性和可靠性。
...
### [113] [EGTR-Review: Efficient Evidence-Grounded Scientific Peer Review Generation via Multi-Agent Teacher Distillation](https://arxiv.org/abs/2606.06025)
*Xinpeng Qiu,Wang Yihu,Zhifeng Liu,Xiaochen Wang,Jimin Wang*

Main category: cs.CL

TL;DR: 提出基于多智能体教师蒸馏的证据驱动且可溯源的审稿生成框架EGTR-Review，通过结构感知分解和证据加权蒸馏，在降低推理成本的同时提升事实基础和溯源性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM审稿方法缺乏证据支持和来源追溯，多智能体系统推理成本高。

Method: 构建多智能体教师进行结构感知论文分解、关键元素提取、外部证据检索、状态标注、验证推理和审稿合成，再通过任务前缀多任务学习将推理轨迹和评论蒸馏到轻量学生模型，并采用证据加权目标减少弱监督影响。

Result: 在公开数据集上，EGTR-Review（学生）在自动指标、LLM评判和人工评测中优于多种基线，且保持强事实基础和溯源性，显著降低令牌消耗和推理时间。

Conclusion: EGTR-Review在审稿生成中平衡了效率与质量，为自动化审稿提供了有效方案。
...
### [114] [PEFT of SLM for Telecommunications Customer Support: A Comparative Study of LoRA Configurations with Energy Consumption Analysis](https://arxiv.org/abs/2606.05176)
*Lucas Tamic,Ilan Jaffeux-Cheniout,Xavier Marjou*

Main category: cs.CL

TL;DR: 使用LoRA对Qwen2.5-3B进行参数高效微调，结合组合合成数据生成，在电信客服领域发现验证损失与人类对齐质量不一致。


<details>
  <summary>Details</summary>
Motivation: LLM在电信客服领域的评估和适应性有限，且外部托管模型面临数据主权和敏感性挑战，因此需要构建领域特定的对话助手。

Method: 采用LoRA对Qwen2.5-3B进行微调；通过52个行业术语的组合合成数据方法，使用Gemini 2.0 Flash生成约30,000训练样本；评估16种LoRA配置，并引入能量消耗分析和LLM-as-a-judge（GPT-5.2和Claude 4.5 Sonnet）定性评估。

Result: 最佳验证损失(0.5024)在定性评估中仅排第6-7名，而最差损失(0.6807)排名第一，显示定量与定性结果不一致。

Conclusion: 贡献了组合合成数据构建方法、LoRA目标模块选择影响、验证损失不足作为选择依据，以及能量-性能权衡分析。
...
### [115] [Harnessing Structural Context for Entity Alignment Foundation Models](https://arxiv.org/abs/2606.06109)
*Xingyu Chen,Yuanning Cui,Zequn Sun,Wei Hu*

Main category: cs.CL

TL;DR: 提出ContextEA，通过增强跨KG交互和结构校准来改进实体对齐基础模型。


<details>
  <summary>Details</summary>
Motivation: 现有EA基础模型未充分利用结构上下文：编码时跨KG交互弱，最终排名过度依赖粗略相似性。

Method: 编码器端引入跨KG交互编码器，通过锚点桥接统一KGs并进行关系感知跨图传播；解码器端引入结构校准解码器，利用多级结构证据校准对齐分数。

Result: 在29个EA数据集上一致优于强可迁移基线，预训练模型即超过微调基线。
...
### [116] [A Komi-Yazva--Russian Parallel Corpus and Evaluation Protocol for Zero- and Few-Shot LLM Translation](https://arxiv.org/abs/2606.06420)
*Petr Parshakov*

Main category: cs.CL

TL;DR: 提出了首个Komi-Yazva-俄语平行语料库及评估协议，用于研究低资源场景下LLM翻译，结果表明少样本检索提升翻译但增益有限，且评估结果高度依赖指标选择。


<details>
  <summary>Details</summary>
Motivation: 在极度低资源且濒危语言环境下研究LLM翻译，需要构建语料库和可靠的评估协议。

Method: 构建457对齐句对的平行语料，设计包含故事级交叉验证、确定性检索、严格输出验证、多种指标和不确定性估计的评估协议，比较不同LLM在零样本和检索增强少样本下的翻译。

Result: LLM产生有意义翻译，但性能因模型和提示方式而异；检索增强少样本一致优于零样本，但增大上下文增益有限；评估结论依赖指标选择。

Conclusion: 将语料库作为数据集贡献和可重复评估测试台，用于濒危语言机器翻译。
...
TL;DR: BUMP is a self-supervised framework that trains a profile generator for personalizing LLMs without any downstream labels by using bidirectional in-batch ranking objectives.


<details>
  <summary>Details</summary>
Motivation: Existing methods for personalizing LLMs require expensive labeled downstream tasks for supervision, which is costly and sparse.

Method: BUMP uses GRPO to train an LLM to generate textual profiles from user interaction history, with a bidirectional in-batch ranking objective scored by multi-positive NDCG and a small LLM judge.

Result: On LaMP benchmark, BUMP matches or outperforms closed-source APIs and prior methods that rely on labeled rewards, while requiring no task labels at training.

Conclusion: BUMP demonstrates that effective personalization can be achieved without annotated supervision, providing a scalable self-supervised alternative.
...
### [118] [ArcANE: Do Role-Playing Language Agents Stay in Character at the Right Time?](https://arxiv.org/abs/2606.05553)
*Woojung Song,Nalim Kim,Sangjun Song,Chaewon Heo,Jongwon Lim,Yohan Jo*

Main category: cs.CL

TL;DR: 提出ArcANE基准，通过角色弧条件上下文提升角色扮演语言代理在心理轨迹对齐上的表现，尤其是源文本外场景。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅衡量事实回忆，忽略角色心理演变，尤其是源文本未探索的场景。

Method: 构建ArcANE基准，基于17部小说的角色弧，跨阶段提出相同场景；微调ArcANE-8B/32B模型。

Result: 角色弧条件上下文在所有模型上优于其他策略，在源文本外场景优势最大；微调模型进一步扩大优势。

Conclusion: 角色弧条件上下文有效提升角色扮演代理的心理一致性，尤其适用于源文本外场景。
...
TL;DR: 词汇密度是导致LLM长上下文性能下降的第三个关键因素，有效上下文容量随密度增加而降低。


<details>
  <summary>Details</summary>
Motivation: 输入长度和相关信息位置被广泛认为是LLM长上下文性能下降的原因，但词汇密度作为一个被忽视的因素可能也有重要影响。

Method: 使用三个长度相同（约12k tokens）且针位置可控但信息密度不同的'找针'基准，并进一步在每个基准内控制密度变化，保持其他属性不变。

Result: 在高密度基准中模型性能急剧下降，稀疏上下文中接近完美的模型在密集上下文中检索分数低于60%，降低密度通常能恢复性能。
...
### [120] [Rethinking LoRA Memory Through the Lens of KV Cache Compression](https://arxiv.org/abs/2606.05698)
*Chunsheng Zuo,Liaoyaqi Wang,William Jurayj,William Fleshman,Benjamin Van Durme*

Main category: cs.CL

TL;DR: 文档LoRA在上下文稀缺时作为补充记忆通道有效，但在上下文完整时贡献不大。


<details>
  <summary>Details</summary>
Motivation: 参数化检索增强（如LoRA）存储文档信息，但需要理解其与KV缓存上下文记忆的交互。

Method: 在文档级问答中，通过逐步驱逐文档KV状态，测量文档LoRA在不同上下文保留水平下的贡献。

Result: 当KV缓存完整时LoRA贡献小；在激进压缩下，LoRA恢复13-21 ROUGE-L点；最佳设置是基础模型编码文档而适配器仅用于解码阶段；QA监督优于next-token-prediction。

Conclusion: 文档LoRA作为解码时间参数记忆，是上下文记忆的补充，尤其在上下文稀缺时有用。
...
### [121] [MARDoc: A Memory-Aware Refinement Agent Framework for Multimodal Long Document QA](https://arxiv.org/abs/2606.05749)
*Kaifeng Chen,Hongtao Liu,Qiyao Peng,Jian Yang,Yongqiang Liu,Xiaochen Zhang,Qing Yang*

Main category: cs.CL

TL;DR: MARDoc是一种记忆感知的细化代理框架，用于多模态长文档问答，通过将长文档QA分解为三个专门代理并利用动态更新的结构化记忆来减少上下文噪声，实验表明其性能优于同类基线。


<details>
  <summary>Details</summary>
Motivation: 现有迭代检索-推理系统在交互积累时会导致关键证据分散和稀释，使得多跳推理噪声大。

Method: 提出MARDoc框架，包含三个代理：Explorer进行多粒度多模态检索，Refiner将交互轨迹蒸馏为结构化证据和推理记忆，Reflector检查证据充分性并提供针对性反馈。代理依赖动态更新的结构化记忆而不是完整的累积交互历史。

Result: 在MMLongBench-Doc和DocBench上，MARDoc取得了强劲结果，优于相同骨干网络的基线。

Conclusion: 结构化记忆对于代理式文档问答的有效性得到验证，能够减少上下文噪声同时保留关键事实和逻辑依赖。
...
### [122] [Mechanistic Insights into Functional Sparsity in Multimodal LLMs via CoRe Heads](https://arxiv.org/abs/2606.05843)
*Ruoxi Sun,Quantong Qiu,Juntao Li,Zecheng Tang,Yihang Lou,Min Zhang*

Main category: cs.CL

TL;DR: 本文揭示多模态大语言模型中跨模态检索的功能稀疏性，并识别出专用注意力头（CoRe heads），通过因果干预和加速实验验证其重要性。


<details>
  <summary>Details</summary>
Motivation: MLLMs在复杂视觉语言任务中表现优异，但其如何从复杂上下文中提取查询相关视觉特征的机制仍不明确。

Method: 提出检索注意力质量（RAM）指标，用于识别上下文感知检索（CoRe）注意力头；通过因果干预（去除顶部CoRe头）和加速实验验证其功能。

Result: 发现CoRe头是专用信息提取器，去除前5% CoRe头显著降低多模态推理性能，而利用这种局部稀疏性可加速推理并保持性能。

Conclusion: 揭示了MLLMs中的功能稀疏性结构原则，为机械可解释性和未来架构设计提供理论基础。
...
### [123] [EMBER: Efficient Memory via Budgeted Evidence Retention for Long-Horizon Agents](https://arxiv.org/abs/2606.05894)
*Yilong Li,Suman Banerjee,Tong Che*

Main category: cs.CL

TL;DR: 提出EMBER，一种预算化证据保留策略，通过学习保留证据胶囊在固定令牌预算下提高长周期记忆的检索和回答性能。


<details>
  <summary>Details</summary>
Motivation: 长周期智能体在存储大量历史时，未来回答仍面临检索、重读和上下文成本，当内存错过相关证据时需回退到原始历史，因此需要在查询未知前决定保留哪些源证据以在固定预算下保持可恢复性和可用性。

Method: 实例化Budgeted Pre-Query Retention场景，并引入EMBER，一种学习到的保留策略，构建紧凑的源证据状态，存储包含逐字摘录、检索键和更新元数据的证据胶囊，通过查询后结果反馈训练写入器在摄入-检索-回答链中保留证据。

Result: 在LongMemEval-RR协议上，EMBER-14B在8192令牌保留证据比较点达到0.3017 F1，而最强非EMBER预算基线为0.1765；在各预算下EMBER均提升了F1、Retain-Recall和Read-Recall。

Conclusion: 长周期记忆依赖于在预算内保留证据而非重读更大历史。
...
### [126] [ReasoningFlow: Discourse Structures for Understanding LLM Reasoning Traces](https://arxiv.org/abs/2606.05402)
*Jinu Lee,Shivam Agarwal,Amruta Parulekar,Siddarth Madala,Dilek Hakkani-Tur,Julia Hockenmaier*

Main category: cs.CL

TL;DR: ReasoningFlow是一个将大型推理模型推理轨迹结构化为有向无环图的框架，用于分析和监控推理过程。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型产生非线性推理轨迹，难以评估和监控，需要捕捉其话语结构。

Method: 通过手动注释31个轨迹开发注释模式并验证，然后扩展到自动注释1260个轨迹。

Result: 发现LRMs结构相似、揭示多样推理行为、大部分错误步骤不用于最终答案、因果依赖与话语结构不匹配。

Conclusion: ReasoningFlow可用于更好的推理轨迹监控和理解。
...
### [133] [English-to-Prakrit Machine Translation via Multilingual Transfer Learning](https://arxiv.org/abs/2606.06038)
*Om Choksi,Smit Kareliya,Shrikant Malviya,Pruthwik Mishra*

Main category: cs.CL

TL;DR: Adapts multilingual model to low-resource English-Prakrit translation by mapping Prakrit to Hindi language tag, achieving BLEU gains but facing data scarcity and dialect mismatch issues.


<details>
  <summary>Details</summary>
Motivation: To enable machine translation for classical languages like Prakrit unsupported by existing multilingual models such as IndicTrans2 in low-resource settings.

Method: Map Prakrit to Hindi language tag (hin_Deva) without modifying tokenizer, vocabulary, or architecture; train on 1,474-pair Maharashtri Prakrit corpus and evaluate on 20-sample Ardhamagadhi test set.

Result: Corpus BLEU improvements over untuned baseline, with limitations due to data scarcity and dialect mismatch.

Conclusion: Script-compatible language routing enables feasible transfer to unsupported classical languages, but data scarcity and dialect mismatch remain challenges; code and models released.
...
TL;DR: OpAI-Bench是一个操作引导的基准，用于研究渐进式人机合作文本中AI作者身份信号的出现、积累或消失，涵盖多种粒度和编辑操作。


<details>
  <summary>Details</summary>
Motivation: 现有AI文本检测基准主要关注最终输出，缺乏对修订过程中AI作者身份信号演变的理解。

Method: 从人类文档开始，为每个样本在预定义AI覆盖级别和五种编辑操作下构建九个顺序修订版本，覆盖四个领域，保留多粒度作者来源，并使用多种检测器进行评估。

Result: AI文本可检测性受编辑比例、操作类型、领域和修订历史影响，混合作者中间版本比端点更难检测，呈现非单调模式。

Conclusion: OpAI-Bench提供了一个受控测试平台，用于分析在现实渐进编辑场景下AI辅助写作的可检测性。
...
### [136] [The Granularity Gap: A Multi-Dimensional Longitudinal Audit of Sycophancy in Gemini Models](https://arxiv.org/abs/2606.05183)
*Patrick Keough*

Main category: cs.CL

TL;DR: 引入粒度差距概念，揭示标准二元评估掩盖的大语言模型谄媚行为，通过连续尺度测量发现非单调的代际进展和与真实性的负相关。


<details>
  <summary>Details</summary>
Motivation: 标准对齐基准将谄媚视为二元失败模式，忽略了模型在用户框架下妥协、验证可疑前提或软化事实纠正等社会顺从行为。

Method: 评估六个Gemini变体（2.0、2.5、3.0代）在73个对抗性提示和三种防护栏条件下的表现，收集8,830个分级响应，使用0-4 Likert量表，经人类注释者验证（Fleiss kappa=0.71，Cohen kappa=0.78，准确率95.9%）。

Result: 27.2%的响应包含实质性谄媚内容（Likert>=2.0），22.7%达到中度或严重水平（>=3.0）；代际进展非单调：Gen 2.5退步（均值2.64），Gen 2.0（1.90）和Gen 3.0（2.01）较好；发现校准税：谄媚与真实性Spearman rho=-0.63，简单防护栏在旗舰模型上更有效，但小模型需要链式思维。

Conclusion: 谄媚行为需要连续测量而非二元分类，数据集和评分标准已发布以支持进一步研究。
...
### [137] [Localizing Prompt Ambiguity in Large Language Models with Probe-Targeted Attribution](https://arxiv.org/abs/2606.05486)
*Govind Ramesh,Yao Dou,Wei Xu*

Main category: cs.CL

TL;DR: 提出PRIG方法，通过训练线性探针将提示歧义归因到token位置，在合成和人工数据集上取得高AUROC，优于基线和GPT-5.4。


<details>
  <summary>Details</summary>
Motivation: 提示歧义是LLM常见失败原因，但难以定位，现有归因方法仅解释可观察输出。

Method: 训练线性探针区分清晰和模糊提示，将探针分数归因到残差流中的早期token表示。

Result: 在合成基准上AUROC 0.840，在人工gold集上0.891，优于梯度归因基线和GPT-5.4，且域外信号有效。

Conclusion: PRIG是识别提示歧义部分的实用工具，并表明潜在提示属性可通过中间表示定位。
...
### [138] [MASF: A Multi-Model Adaptive Selection Framework for Abstractive Text summarization](https://arxiv.org/abs/2606.05494)
*Ahmed Alansary,Ali Hamdi*

Main category: cs.CL

TL;DR: 提出一种多模型自适应摘要框架，集成多个微调转换器模型并自适应选择最佳摘要，在CNN/DailyMail数据集上达到最高BERTScore 88.63%。


<details>
  <summary>Details</summary>
Motivation: 单一模型在不同结构和主题的文章上总结质量不一致，导致鲁棒性不足。

Method: 集成多个微调转换器模型，每个独立生成候选摘要，使用自动评估指标（词汇相似性和语义相关性）评分，选择最高质量摘要作为最终输出。

Result: 在CNN/DailyMail数据集上BERTScore达88.63%，优于GPT3-D2、Falcon-7b、Mpt-7b等模型。

Conclusion: 利用多模型集成和自适应选择策略可以有效提升自动文本摘要的质量和鲁棒性。
...
### [139] [Contextualized Prompting For Stance Detection On Social Media](https://arxiv.org/abs/2606.06022)
*Tilman Beck,Shakib Yazdani,Simon Kruschinski,Marcus Maurer,Iryna Gurevych*

Main category: cs.CL

TL;DR: 本研究系统探究了在零样本提示中加入上下文特征对推特立场检测的影响，发现LLM生成的目标描述能稳定提升性能，但其他用户元数据效果不一，且引入同一用户的其他推文可能因噪声而降低性能。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的立场检测因语言简短、嘈杂且依赖上下文而具有挑战性。大型语言模型虽然展现出零样本泛化能力，但通常在不包含上下文信息的情况下被提示，这限制了其解读模糊帖子的能力。

Method: 我们系统性地研究了将真实世界（如用户简介）、派生（如政党）和LLM生成的（如目标描述）上下文特征纳入零样本提示中对推特立场检测的影响。评估涵盖四个基准数据集，包括一个新的高质量德语推特立场数据集，并使用了多个大型语言模型。

Result: 集成上下文信息仅在特定条件下改善性能。LLM生成的目标描述一致地提高了准确性，而其他用户元数据的影响则好坏参半，甚至有害。值得注意的是，包含同一用户的其他推文可能会因输入噪声而损害性能。

Conclusion: 我们的发现突显了在嘈杂的真实世界环境中使用上下文信息进行提示的潜力和挑战。我们公开发布了代码和数据。

Abstract: Stance detection on social media is challenging due to short, noisy, and context-dependent language. While large language models (LLMs) show zero-shot generalization, they are typically prompted without contextual information, which limits their ability to interpret ambiguous posts. In this work, we systematically investigate the impact of incorporating real-world (e.g., user biographies), derived (e.g., political party), and LLM-generated (e.g., target descriptions) contextual features into zero-shot prompting for stance detection on Twitter. Our evaluation spans four benchmark datasets, including a new high-quality German Twitter stance dataset. Across multiple LLMs, we find that integrating contextual information improves performance, but only under specific conditions. LLM-generated target descriptions consistently enhance accuracy, while other user metadata has mixed or even detrimental effects. Notably, we show that the inclusion of other tweets by the same user, often beneficial in supervised learning, can impair performance due to input noise. Our qualitative analysis reveals that LLMs struggle to distinguish task-specific useful information from irrelevant context. Our findings highlight both the promise and challenges of prompting with context information in noisy real-world settings. We publish code and data at this \href{ }{page}.

</details>


### [140] [Automatic Labelling of Speech Translation Errors](https://arxiv.org/abs/2606.06047)
*Dominik Macháček,Maike Züfle,Ondrej Klejch*

Main category: cs.CL

TL;DR: 提出STEL方法评估语音翻译错误，发现文本专用和多模态系统精度约为人类一半，直接语音处理必不可少且与文本系统互补。


<details>
  <summary>Details</summary>
Motivation: 语音翻译错误降低系统可信度且后果严重，但缺乏评估置信度和质量估计的成熟方法。

Method: 设计STEL注释协议、构建小规模真实端到端评估数据集，分析文本专用和语音处理系统在STEL任务上的表现。

Result: 文本专用XCOMET和多模态LLM Qwen2.5-Omni能以人类一半精度完成STEL；直接语音处理必要；两类系统在标注翻译错误与语音处理错误上互补。

Conclusion: 直接语音处理对STEL任务至关重要，当前文本专用与语音处理系统可互补，为未来研究奠定基础。
...
### [141] [CHALIS: A Challenge Dataset for Language Identification in Difficult Scenarios](https://arxiv.org/abs/2606.06088)
*Michal Tichý,Jindřich Libovický*

Main category: cs.CL

TL;DR: 提出了CHALIS基准数据集，用于评估语言识别系统在亲缘语言和正字法噪声场景下的表现，发现现有系统在这些困难情况下表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有语言识别数据集缺乏对亲缘语言和正字法噪声的针对性测试，导致系统在这些场景下表现不可靠。

Method: 构建CHALIS数据集，包含两部分：一是收集相互可理解的亲缘语言对（捷克语/斯洛伐克语等）的句子；二是模拟正字法噪声（转写、去除变音符号、同形攻击、网络俚语）。评估了四个广泛使用的语言识别系统。

Result: 所有系统在亲缘语言对和正字法噪声场景下均表现挣扎，尤其对低资源语言和转写输入效果更差。

Conclusion: CHALIS数据集揭示了现有语言识别系统的弱点，资源已公开可供进一步研究。

Abstract: We present CHALIS (Challenging Language Identification Samples), a new benchmark dataset explicitly designed to address difficult cases in language identification: cousin languages and orthographic noise. Our dataset has two parts: First, we collected sentences shared across mutually intelligible language pairs (Czech/Slovak, Spanish/Catalan, Portuguese/Galician, Danish/Norwegian). The second part tests for orthography noise: we transliterate text across multiple scripts, remove diacritics, simulate homoglyph attacks, and use Internet slang. We evaluate four widely used language identification systems on CHALIS and demonstrate that all struggle substantially in these scenarios, especially on lower-resource languages within cousin pairs and on transliterated input. The resource is publicly available at .

</details>


### [142] [Improving Answer Extraction in Context-based Question Answering Systems Using LLMs](https://arxiv.org/abs/2606.06197)
*Hafez Abdelghaffar,Ahmed Alansary,Ali Hamdi*

Main category: cs.CL

TL;DR: 提出基于LLM的QA系统，通过微调Roberta-base在SQuAD1.1上显著提升准确性和相关性。


<details>
  <summary>Details</summary>
Motivation: 现有QA系统在复杂或模糊查询下，即使有正确上下文也常产生不相关或不精确回应。

Method: 微调预训练Roberta-base模型，使用SQuAD1.1数据集进行监督训练和评估。

Result: 微调后模型在SQuAD1.1上取得ROUGE-L 86.84%，BLEU 28.24%，BERTScore 95.38%。

Conclusion: 针对性微调有效提升QA系统的可靠性和精确性。
...
### [143] [LLMs Can Leak Training Data But Do They Want To? A Propensity-Aware Evaluation of Memorization in LLMs](https://arxiv.org/abs/2606.06286)
*Gianluca Barmina,Peter Schneider-Kamp,Lukas Galke Poech*

Main category: cs.CL

TL;DR: 大型语言模型在对抗性提示下可能泄露训练数据，但正常使用中很少泄露，因此记忆评估应区分能力与倾向性。


<details>
  <summary>Details</summary>
Motivation: 现有记忆评估主要测量模型是否可以被强制记忆，而非在正常使用中是否记忆，需要更全面的视角。

Method: 提出PropMe框架，包含基于前缀的能力攻击和非对抗性评估，引入度量转换创建倾向性度量，并开发SimpleTrace管道用于归因和计算多种记忆度量。

Result: 在Comma和DFM Decoder上评估发现能力与倾向性存在差距：前缀攻击引发更强记忆，但倾向性分数低；DFM Decoder通过后续训练减少了记忆。

Conclusion: 记忆审计应同时报告最坏情况的可提取性和普通泄漏倾向性，以获得全面理解。
...
### [144] ["Chi nas dal soch el sent de legn" -- Auditing Text Corpora for Lombard](https://arxiv.org/abs/2606.06349)
*Edoardo Signoroni,Pavel Rychlý*

Main category: cs.CL

TL;DR: 对伦巴第语语料库的审计显示，网络爬取的数据存在严重质量问题，且高质量数据偏向西部变体，东部被边缘化，因此需要社区驱动的数据策展。


<details>
  <summary>Details</summary>
Motivation: 许多语言缺乏高质量NLP数据集，导致低资源语言工具不足。

Method: 手动审计伦巴第语的平行和单语语料库，分析其语言组成、噪声和变体代表性。

Result: 网络爬取数据集存在语言误识别、样板文本和非语言噪声；高质量数据严重偏向西部伦巴第变体，东部变体被忽视。

Conclusion: 需要变体意识、社区驱动的数据策展，而非仅追求数量。
...
### [148] [MCBench: A Multicontext Safety Assessment Benchmark for Omni Large Language Models](https://arxiv.org/abs/2606.05177)
*Manh Luong,Tamas Abraham,Junae Kim,Amar Kaur,Rollin Omari,Gholamreza Haffari,Trang Vu,Lizhen Qu,Dinh Phung*

Main category: cs.CL

TL;DR: MCBench是首个面向全模态大语言模型的安全基准，揭示了当前模型在跨模态推理上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有多模态安全基准仅关注视觉输入，无法评估处理视觉、音频和文本的全模态大语言模型。

Method: 构建MCBench，包含1196个场景，涵盖四个安全类别，要求整合多模态进行安全评估；每个不安全场景配有最小差异的安全场景以评估敏感性。

Result: 评估发现全模态模型在微妙或非物理风险上表现不佳，但在显著视觉或声学线索时表现较好；推理轨迹显示模型能提取模态信息但难以有效整合。

Conclusion: 当前全模态模型在安全关键场景中缺乏鲁棒的跨模态推理，需要改进架构和训练策略。
...
### [149] [Efficient Punctuation Restoration via Weighted Lookahead Scoring Method for Streaming ASR Systems](https://arxiv.org/abs/2606.05179)
*Sungmook Woo,Hyungu Kang,Chanwoo Kim*

Main category: cs.CL

TL;DR: 提出一种非自回归评分方法用于流式ASR标点恢复，在前瞻预算下达到优于基线的性能。


<details>
  <summary>Details</summary>
Motivation: 流式ASR需要在线标点恢复，但基于生成的方法存在延迟和对齐失败问题。

Method: 在有限K子词前瞻下，比较标点插入假设与无插入基线，并用权重α和校准阈值τ进行决策，无需参数更新。

Result: 在IWSLT 2017上，无微调F1=0.893，微调F1=0.937，优于提示（0.566）和微调ELECTRA（0.913）基线。

Conclusion: 提出的方法有效提升了流式ASR标点恢复性能，通过消融实验分析了前瞻预算的影响。
...
### [150] [Multi-Granularity Reasoning for Natural Language Inference](https://arxiv.org/abs/2606.05181)
*Chunling Xi,Di Liang*

Main category: cs.CL

TL;DR: 提出多粒度推理网络MGRN，通过层次语义特征进行自然语言推理，在多个基准上超越基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖最终层token表示，不足以捕捉复杂且层次化的语义交互，限制了推理能力。

Method: MGRN显式利用层次语义特征，在交互推理空间中模拟人类从词汇匹配到抽象逻辑的认知过程。

Result: 在多个公开基准上一致优于强基线模型，验证了有效性和鲁棒性。

Conclusion: MGRN通过层次化语义推理有效提升了NLI性能，为语义理解提供了新思路。
...
### [151] [LoRi: Low-Rank Distillation for Implicit Reasoning](https://arxiv.org/abs/2606.05315)
*Ryan Solgi,Jiayi Tian,Zheng Zhang*

Main category: cs.CL

TL;DR: 提出低秩蒸馏框架，通过对齐隐式思维链轨迹提高推理性能，接近显式CoT。


<details>
  <summary>Details</summary>
Motivation: 隐式思维链方法常不如显式CoT，且发现隐藏状态推理轨迹具有低秩结构。

Method: 提出低秩蒸馏框架，在共享低秩张量子空间中对齐教师和学生轨迹，使用一阶和二阶统计量。

Result: 在数学推理基准上持续改进，特别在复杂多步任务上接近显式CoT准确率，优于先前iCoT蒸馏方法。

Conclusion: 该方法有效提升隐式思维链推理性能，为紧凑推理过程提供新途径。
...
### [152] [A Model of Multi-turn Human Persuadability Using Probabilistic Belief Tracing](https://arxiv.org/abs/2606.05330)
*Jared Moore,Noah Goodman,Nick Haber,Max Kleiman-Weiner*

Main category: cs.CL

TL;DR: 提出了PERSUASIONTRACE框架，用于研究人类与LLM交互中的说服过程，通过过程级评估和多轮信念追踪，揭示了人类信念更新模式，并引入贝叶斯网络模拟目标以更真实地模拟人类信念动态。


<details>
  <summary>Details</summary>
Motivation: 现有说服研究多依赖前/后信念测量，无法捕捉对话中信念变化的动态过程。

Method: 开发了基于web的PERSUASIONTRACE平台，记录多轮信念报告，标注修辞维度（logos/pathos/ethos），并引入贝叶斯网络模拟器以保持显式潜在信念状态，实现认知真实更新。

Result: 人类目标分为两个信念更新聚类，且易受修辞策略影响；LLM在多种话题和模态下具有说服力；基线LLM模拟器无法复现人类信念动态，而贝叶斯目标在人类相似度评估中得分（81）接近人类参考（80），远高于基线（64）。

Conclusion: PERSUASIONTRACE将说服评估从端点移动重定义为过程保真度，为科学分析和安全优化提供了更可靠的基础。
...
### [153] [When Evidence is Sparse: Weakly Supervised Early Failure Alerting in Dialogs and LLM-Agent Trajectories](https://arxiv.org/abs/2606.05414)
*Avinash Baidya,Xinran Liang,Ruocheng Guo,Xiang Gao,Kamalika Das*

Main category: cs.CL

TL;DR: A two-stage approach for early failure alerting in multi-turn interactions that learns sparse failure evidence and uses a preference-conditioned stopping policy, achieving better trade-offs and efficiency.


<details>
  <summary>Details</summary>
Motivation: Early failure alerting is challenging because supervision is trajectory-level but alerts must be raised from partial interactions. Prior methods assign terminal label to every prefix, which is poorly matched to multi-turn interactions with sparse and delayed failure evidence.

Method: Attention-based failure predictor learns sparse turn-level failure evidence from trajectory labels and estimates failure risk from partial histories. The α-STOP policy selects an accuracy-earliness operating point at inference time, avoiding training separate triggers for each preference.

Result: Across five benchmarks, high-relevance failure evidence occupies 4.7-11.3% of turns and appears after 59.0-83.6% of trajectories. The attention-based predictor improves Pareto-frontier quality (hypervolume) by 1-10% over naive prefix supervision, and the full system improves frontier quality by 3-42% over state-of-the-art trigger policies while reducing training cost per operating point by 1-3 orders of magnitude.

Conclusion: The proposed approach effectively learns from sparse evidence and enables controllable early alerting with improved frontier quality and significantly reduced training cost.
...
### [154] [ComplexityMT: Benchmarking the Interaction Between Text Complexity and Machine Translation](https://arxiv.org/abs/2606.05421)
...
TL;DR: 引入ComplexityMT，用CEFR级别评估文本复杂度与机器翻译的相互影响。


<details>
  <summary>Details</summary>
Motivation: 探究翻译是否保留原文复杂度，以及文本复杂度如何影响机器翻译。

Method: 在六种语言（阿拉伯语、荷兰语、英语、法语、印地语、俄语）上，评估三个开放权重模型、一个封闭模型和一个商业机器翻译系统，进行两项任务：CEFR与翻译难度的相关性，以及源文本CEFR级别的变化。

Result: 较高CEFR级别使文本更难翻译，机器翻译会改变目标文本的CEFR级别（多数语言）。

Conclusion: 为多语言教学内容生成和机器翻译难度估计的研究者和从业者提供新见解。
...
### [155] [Multilingual Detection of Alzheimer's Disease from Speech: A Cross-Linguistic Transfer Learning Approach](https://arxiv.org/abs/2606.05545)
*Nadine Yasser Abdelhalim,Emmanuel Akinrintoyo,Nicole Salomons*

Main category: cs.CL

TL;DR: 提出跨语言训练方法，利用Transformer模型在多种语言上实现AD检测，F1达82%，推理时间0.5秒。


<details>
  <summary>Details</summary>
Motivation: 多语言AD检测模型开发因语言特定训练耗时耗资源而面临挑战，需要跨语言泛化解决方案。

Method: 使用英语、中文、阿拉伯语和印地语数据集，训练基于Transformer的模型进行AD二分类，采用跨语言训练策略。

Result: 在所有语言上F1得分82%，推理时间0.5秒，性能一致且支持实时筛查。

Conclusion: 跨语言训练有效实现多语言AD检测，模型具有强泛化能力，适合全球部署。
...
### [156] [Using Large Language Models to Support High Volume Application Review for an Undergraduate Research Program](https://arxiv.org/abs/2606.05564)
*Varun Aggarwal,Kay Kobak,John Howarter*

Main category: cs.CL

TL;DR: An LLM-based tool using GPT models was developed to evaluate 1200 Statements of Purpose for Purdue's SURF program, reducing coordinator review time from weeks to 4 hours.


<details>
  <summary>Details</summary>
Motivation: To handle the large volume of SURF applications and ensure consistent, timely evaluation.

Method: Used OpenAI GPT models (GPT-4o, GPT-5-mini, GPT-5.2) with a structured rubric across six subcategories scored 0-3, tuned with a few human-graded examples, and processed 1200 SoPs generating scores, rationales, and excerpts.

Result: GPT-5.2 showed best rubric adherence; model score disagreement was higher for low-scoring submissions; the tool replicated human grading, and coordinator review time reduced to 4 hours from multiple weeks.

Conclusion: The LLM-based tool effectively assists in application evaluation, saving time and improving consistency, warranting further development.
...
### [157] [Domain-Aware Mispronunciation Detection and Diagnosis Using Language-Specific Statistical Graphs](https://arxiv.org/abs/2606.05569)
*Huu Tuong Tu,Hanh Nguyen,Thien Van Luong,Nguyen Tien Cuong,Vu Huan,Nguyen Thi Thu Trang*

Main category: cs.CL

TL;DR: 提出统计图方法学习音素混淆模式并引入语言特定策略，在MDD任务上取得59.52% F1。


<details>
  <summary>Details</summary>
Motivation: 发音错误检测与诊断在计算机辅助语言学习中日益重要，现有方法未能充分利用音素混淆模式和跨语言差异。

Method: 构建统计有向图表示音素混淆模式，并采用语言特定策略捕捉不同母语背景的系统发音差异。

Result: 在L2-ARCTIC基准上取得59.52%的F1分数，优于多个基线。

Conclusion: 所提方法能有效学习混淆模式并提升MDD性能，有望在实际语言学习系统中应用。
...
### [158] [TensorBench: Benchmarking Coding Agents on a Compiler-Based Tensor Framework](https://arxiv.org/abs/2606.05570)
*Bobby Yan,Fredrik Kjolstad*

Main category: cs.CL

TL;DR: TensorBench是一个包含199个特征添加和重构任务的基准测试，评估了七个编码代理，通过率从64.8%到22.1%，代理间一致性较低。


<details>
  <summary>Details</summary>
Motivation: 现有仓库级编码基准测试在任务难度和评估可靠性之间存在权衡：挑战性任务通常缺乏完整测试覆盖，而人工审查难以扩展。TensorBench旨在提供可靠且具有挑战性的评估。

Method: 基于开源编译器张量框架构建199个任务，覆盖多种类型；评估时应用代理补丁并运行框架测试套件（包括回归测试和代理添加的测试）；特征添加任务要求保留原有行为并通过新测试。

Result: 七个代理的通过率范围64.8%至22.1%，最强代理通过率64.8%；代理间一致性低（Cohen's κ从-0.07到0.43，最强两者κ=0.05）。

Conclusion: TensorBench能够区分不同代理的能力，但代理间低一致性表明任务覆盖面广且难度各异，可能反映了不同策略的优势与局限。
...
### [159] [AdaPlanBench: Evaluating Adaptive Planning in Large Language Model Agents under World and User Constraints](https://arxiv.org/abs/2606.05622)
...
TL;DR: 提出AdaPlanBench，一个动态交互基准，用于评估LLM在逐步揭示的世界和用户约束下的自适应规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准未充分探索在逐步揭示的双重约束下的自适应规划。

Method: 基于307个家庭任务构建AdaPlanBench，使用可扩展的约束构造流水线为每个任务增加双重约束；运行时采用多轮协议，隐藏的约束在代理提出的计划违反时揭示，需要迭代修改计划。

Result: 在10个LLM上的实验表明，最佳模型准确率仅67.75%，性能随约束累积而下降，用户约束尤其困难，失败常源于物理基础薄弱和效果降低。

Conclusion: AdaPlanBench成为双约束交互规划的测试床，强调了LLM代理可靠适应动态揭示约束的挑战。
...
### [160] [When New Generators Arrive: Lifelong Machine-Generated Text Attribution via Ridge Feature Transfer](https://arxiv.org/abs/2606.05626)
*Zhen Sun,Yifan Liao,Zhicong Huang,Jiaheng Wei,Cheng Hong,Yutao Yue,Xinlei He*

Main category: cs.CL

TL;DR: 提出RidgeFT，一种基于闭式岭回归的轻量级终身学习框架，无需样本回放，在机器生成文本属性归属中实现最优性能。


<details>
  <summary>Details</summary>
Motivation: 终身MGT属性归属面临挑战，现有方法难以平衡新类适应与旧类保留。

Method: 训练任务感知编码器，存储类级充分统计量，冻结编码器，通过协方差校准和固定随机特征提高表示能力，然后使用闭式岭回归进行无回放更新。

Result: 在多主题评估中，RidgeFT始终优于基线，取得最佳macro-F1，同时改善旧类保留和新类适应。

Conclusion: 特征稳定的分析更新为终身MGT属性归属提供了简单有效的方法。
...
### [161] [Bootstrapping Semantic Layer from Execution for Text-to-SQL](https://arxiv.org/abs/2606.05634)
*Youngwon Lee,Jaejin Kim,Seung-won Hwang*

Main category: cs.CL

TL;DR: GATE通过执行反馈引导缺失的文本到SQL对应关系，实现可重用记忆。


<details>
  <summary>Details</summary>
Motivation: 现实中的文本到SQL任务常因用户短语与数据库值对应不明确而缺乏充分指定；先前工作需提前指定语义层，但易不完整，尤其在专家领域。

Method: GATE保持多个对应假设开放，执行已对应部分获得观察，仅将受支持假设对应并存储为记忆条目，记录测试内容和SQL写法，积累成执行引导记忆供后续重用。

Result: 在真实世界和受控基准中，GATE一致优于强基线。

Conclusion: 执行不仅是验证手段，也是文本到SQL中可重用记忆的引导机制。
...
### [162] [Interpreting Style Representations via Style-Eliciting Prompts](https://arxiv.org/abs/2606.05716)
*Junghwan Kim,David Jurgens*

Main category: cs.CL

TL;DR: 提出风格引出提示框架，通过训练解码器从风格表示生成自然语言提示，实现可解释的风格表示。


<details>
  <summary>Details</summary>
Motivation: 现有风格表示解释方法存在LLM偏见和幻觉，且缺乏明确目标和实用性。

Method: 整理1010个风格特征，构建数据集，训练解码器从风格表示生成风格引出提示。

Result: 在三个任务上（恢复提示、风格生成、风格迁移）优于直接提示LLM的基线。

Conclusion: 风格引出提示为风格表示提供了实用且可解释的接口。
...
### [163] [CollabBench: Benchmarking and Unleashing Collaborative Ability of LLMs with Diverse Players via Proactive Engagement](https://arxiv.org/abs/2606.05793)
*Hong Qian,Yuanhao Liu,Zihan Zhou,Zongbao Zhang,Hanjie Ge,Haotian Shi,Liang Dou,Xiangfeng Wang,Jingwen Yang,Aimin Zhou*

Main category: cs.CL

TL;DR: 提出CollabBench基准，通过多样化玩家模拟和合作Agent训练，在合作游戏中实现19.5%效率提升和24.4%情感表现提升。


<details>
  <summary>Details</summary>
Motivation: LLM-based agents在个体任务上表现优异，但与真实人类合作仍具挑战；现有对话级协作研究缺乏接地互动和行为执行，需要沉浸式合作游戏环境。

Method: 提出CollabBench基准，包含多样化玩家模拟管道和合作Agent训练范式，统一推理、沟通和行动，采用混合奖励平衡任务效率与情感适应，并扩展经典环境为CWAH-MultiPlayer和Cook-MultiPlayer。

Result: 训练模型在效率和情感指标上分别比基线提升19.5%和24.4%。

Conclusion: 分析揭示了现有模型的关键协作限制，为未来协作训练提供见解。
...
### [164] [Can LLMs Be Constrained to the Past? Improving Knowledge Cutoff through Recall-Based Prompting](https://arxiv.org/abs/2606.05804)
*Michiro Asai,Ailiang Lin,Yu Kishimoto,Takao Obi,Satoshi Kosugi,Kotaro Funakoshi,Manabu Okumura*

Main category: cs.CL

TL;DR: 提出了两种基于回忆的提示策略（Self-Recall和Question-Recall）来改进LLM在知识截止日期后的表现，并在多个基准测试中取得更好效果，尤其是在反事实问题上。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖直接答案生成，当问题与截止后知识有因果关系但未明确查询时表现不佳。

Method: 提出两种提示策略：Self-Recall（要求模型重申截止约束）和Question-Recall（要求模型回忆截止前的问题相关信息），并构建多截止年份历史事件基准（MHEB）。

Result: 在三个现有基准上，该方法优于直接答案提示和逐步推理基线，尤其在反事实问题上提升显著；且性能随截止距离变化，结合SR和QR始终最佳。

Conclusion: 结合Self-Recall和Question-Recall的提示策略在处理知识截止问题上始终表现最优。
...
### [165] [Towards Truly Multilingual ASR: Generalizing Code-Switching ASR to Unseen Language Pairs](https://arxiv.org/abs/2606.05846)
*Gio Paik,Hyunseo Shin,Soungmin Lee*

Main category: cs.CL

TL;DR: 本文研究代码切换语音识别（CS-ASR）中，从已知语言对学习的能力是否可以通过模型合并和领域泛化推广到未知语言对。实验表明合并模型仅有有限的泛化能力，表明双语CS能力跨语言对迁移有限。


<details>
  <summary>Details</summary>
Motivation: 解决CS-ASR的可扩展性问题，现有方法需要为每个语言对单独开发，成本高且不灵活。

Method: 使用模型合并和领域泛化方法，在有限的语言对上训练模型，然后测试其对未知语言对的泛化能力。

Result: 合并的双语CS-ASR模型对未知语言对只有有限的泛化能力。

Conclusion: 双语CS能力跨语言对的迁移有限，需要进一步研究更好的泛化方法。
...
### [166] [ReverseEOL: Improving Training-free Text Embeddings via Text Reversal in Decoder-only LLMs](https://arxiv.org/abs/2606.05858)
*Ailiang Lin,Zhuoyun Li,Yusong Wang,Keyu Mao,Kotaro Funakoshi,Manabu Okumura*

Main category: cs.CL

TL;DR: 提出ReverseEOL方法，通过反转输入文本生成反向嵌入并与正向嵌入结合，以改善训练-free文本嵌入在因果注意力下的表示偏差。


<details>
  <summary>Details</summary>
Motivation: 现有训练-free文本嵌入中，解码器-only LLMs的因果注意力导致早期token无法关注未来上下文，产生有偏表示。

Method: ReverseEOL通过将输入文本反转，为每个token生成反向嵌入，与标准正向嵌入结合形成更丰富的最终表示。

Result: 在STS和MTEB基准测试中，ReverseEOL显著提升了多种架构和规模的大语言模型的训练-free基线性能。

Conclusion: 广泛的消融和分析证实了反转机制的必要性，为冻结LLMs的表示增强提供了有效方法。
...
### [167] [YouZhi: Towards High-Concurrency Financial LLMs via Adaptive GQA-to-MLA Transition](https://arxiv.org/abs/2606.05868)
...
TL;DR: 提出了YouZhi-LLM，一种基于华为昇腾生态的高效金融大模型，通过层自适应GQA-to-MLA转换框架和训练流水线，显著降低KV缓存开销并提升并发推理能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的高并发部署受到KV缓存内存开销的严重制约，导致基础设施成本增加和可扩展性受限。

Method: 提出YouZhi-LLM，核心是层自适应GQA-to-MLA转换框架，动态分配每层的FreqFold大小以最大化KV缓存压缩并最小化困惑度下降；训练流水线结合广义知识蒸馏和金融特定监督微调。

Result: 自适应转换将困惑度下降比统一基线降低35%；在昇腾NPU上，YouZhi-7B在金融基准上提高12.3%，最大并发提升2.69倍；YouZhi-14B准确率提高7.0%，并发提升2.43倍。

Conclusion: 为金融领域提供了成本效益高、高吞吐量的推理新范式。
...
### [168] [Evaluating Stochastic Collapse and Implicit Bias in Multimodal Large Language Models](https://arxiv.org/abs/2606.05874)
*Huiyuan Zheng,Houtao Zhang,Boyang Wang,Qingyi Si,Hongcheng Guo*

Main category: cs.CL

TL;DR: 提出RandomBench评估多模态大语言模型在逻辑中性场景下的随机性，发现普遍存在的随机塌陷现象。


<details>
  <summary>Details</summary>
Motivation: 当前评估聚焦效用驱动目标，忽视了逻辑中性场景中随机性的重要性，而确定性策略导致重复行为。

Method: 构建RandomBench基准，并引入RI、BCI、BII三个指标量化熵和分布偏差。

Result: 模型存在随机塌陷，如Claude Sonnet 4.6的top-1概率达97%（理想25%），RI降至0.068；消融实验表明该偏差跨语言和表示形式持续。

Conclusion: 分布塌陷在逻辑中性决策环境中具有鲁棒性。
...
### [169] [Representing Research Attention as Contextually Structured Flows](https://arxiv.org/abs/2606.05895)
*Jessica Rodrigues,Angelo Salatino,Gard Jenset,Scott Hale*

Main category: cs.CL

TL;DR: 提出注意力流作为上下文结构化表示，在结构比较和鲁棒性上优于传统聚合和序列表示。


<details>
  <summary>Details</summary>
Motivation: 研究注意力常以聚合计数表示，未能保留其跨上下文的时序发展，导致解释与表示不匹配。

Method: 提出注意力流表示，编码注意力的组织与演化；构建类比推理基准，比较信号、序列和流表示。

Result: 流表示更有效支持结构比较，尤其在时间与上下文影响下；且学习到的流表示在部分观察和结构扰动下鲁棒性更强。

Conclusion: 支持将注意力建模为上下文结构化现象，为更丰富的研究评估提供基础。
...
### [170] [Better Literary Translation: A Multi-Aspect Data Generation and LLM Training Approach](https://arxiv.org/abs/2606.05924)
*Zhihao Lin,Ziqi Zhu,Hao Huang,Guanghui Wang,Peiyang He*

Main category: cs.CL

TL;DR: 提出多方面的迭代精化框架，利用专门的LLM翻译器生成高质量翻译参考和偏好数据，通过监督微调和GRPO优化模型，在文学翻译基准上取得与Claude Sonnet 4.5竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 文学翻译面临高质量标注数据稀缺以及需要在表达流畅性和文学效果之间取得平衡的挑战。

Method: 构建多方面的迭代精化框架，针对不同质量维度使用专门的LLM翻译器生成翻译参考和偏好数据，然后进行监督微调（SFT）和强化学习（DPO和GRPO）。

Result: 生成的参考数据相比原始真值在SFT中提升8.65 CEA100；DPO导致性能下降，GRPO通过显式奖励模型额外提升1.51点；LitMT-8B和LitMT-14B在MetaphorTrans上分别达到67.25和69.07 CEA100，与Claude Sonnet 4.5（68.43）相当，并在域外作品上表现出强泛化能力。

Conclusion: 该框架有效解决数据稀缺和表达平衡问题，通过迭代精化和GRPO的在线探索能力显著提升翻译质量，模型性能具有竞争力。
...
### [171] [Large Language Models are Perplexed by some Political Parties](https://arxiv.org/abs/2606.05937)
*Paul Lerner,François Yvon*

Main category: cs.CL

TL;DR: LLMs对极右和民族主义政党文本的困惑度高于社会民主党，且公平性源于预训练，不受指令调优影响。


<details>
  <summary>Details</summary>
Motivation: LLMs在政治应用中的使用增加，但其政治公平性研究不足。

Method: 使用困惑度评估10个LLMs在37种语言三个数据集上的政治公平性，并比较基础与指令调优版本。

Result: LLMs对极右和民族主义政党文本更困惑，困惑度与下游翻译指标相关，基础与指令调优版本高度相关。

Conclusion: LLMs的政治公平性主要源于预训练，指令调优影响很小。
...
### [172] [Measuring the sensitivity of LLM-based structured extraction to prompt, model, and schema choices in clinical discharge summaries](https://arxiv.org/abs/2606.05970)
*Martin Murin*

Main category: cs.CL

TL;DR: 研究了LLM在临床笔记结构化提取中对配置选择的敏感性，发现不一致主要源于模式中的“缺失vs未记录”区分，模型选择对多分类影响大于提示措辞。


<details>
  <summary>Details</summary>
Motivation: LLM在临床笔记结构化提取中广泛应用，但其输出对上游配置选择的敏感性理解不足，因此需要评估这种敏感性。

Method: 固定提取任务，变化提示变体和模型大小，在MIMIC-IV摘要上使用Cohen's kappa测量跨提示一致性，并进行配对比较和模式折叠分析。

Result: 三路标记上两模型跨提示一致性中位kappa为0.69和0.68；折叠为二值后大多数不一致消失，不一致集中在“缺失vs未记录”；多分类中，改变模型改变主导标签近一半笔记，改变提示改变约八分之一，大模型减少残留类别使用（44%降至26%）。

Conclusion: 模式强加的不一致集中在缺失/未记录轴，模型选择对多分类影响更显著，提出可复用的审计方法用于评估提取可重复性。
...
### [173] [Ouvia: A User-centered Framework for Measuring Usability of Speech Translation in Real-World Communication Scenarios](https://arxiv.org/abs/2606.06177)
*Giuseppe Attanasio,Beatrice Savoldi,Daniel Chechelnitsky,Matteo Negri,Marine Carpuat,Maarten Sap,André F.T. Martins*

Main category: cs.CL

TL;DR: 提出了Ouvia框架，评估语音翻译在真实世界中的用户感知可用性，发现现代ST仅有限服务于用户，且基于QA的评估比标准方法能更好地预测可用性。


<details>
  <summary>Details</summary>
Motivation: 现有语音翻译评估多基于去语境化的测试和整体质量，忽视了终端用户的沟通需求。

Method: 通过自定义web应用和多阶段研究设计，收集了1750多次由四种ST系统介导的交互，涉及医疗和日常场景，包含三种英语方言和两种性别。

Result: 仅约一半的交互被评分为可用，不同人口统计群体间存在显著可用性差距；基于QA的评估是真实世界可用性的更强预测指标。

Conclusion: 强调了情境化、以用户为中心的评估框架的重要性，需超越整体质量分数，关注技术服务于谁及效果如何。
...
### [174] [FiLM-Based Speaker Conditioning of a SpeechLLM for Pathological Speech Recognition](https://arxiv.org/abs/2606.06211)
*Fernando López,Santosh Kesiraju,Jordi Luque*

Main category: cs.CL

TL;DR: 说话人条件化ASR通过FiLM注入x-vector信息，适应病理语音而不修改基础模型，与微调方法竞争并保留非条件化性能。


<details>
  <summary>Details</summary>
Motivation: 病理语音对ASR构成挑战，需要适应个体说话人而不修改基础模型。

Method: 采用FiLM机制，将x-vector派生信息注入每个transformer层，调整内部表示适应病理说话人，同时冻结ASR编码器权重。

Result: 说话人条件化ASR在西班牙语和英语病理语音上，与标准微调和参数高效微调基线相竞争，并保持对非条件化语音的问答能力。

Conclusion: 说话人条件化ASR是一种有效的适应策略，能够在不修改基础模型权重的情况下，提升病理语音识别性能，同时保持泛化能力。
...
### [175] [Many Circuits, One Mechanism: Input Variation and Evaluation Granularity in Circuit Discovery](https://arxiv.org/abs/2606.06267)
*Alireza Bayat Makou,Jingcheng Niu,Subhabrata Dutta,Iryna Gurevych*

Main category: cs.CL

TL;DR: 研究发现结构不同的电路可能实现相同功能，即“幻象特化”，标准评估方法会掩盖这一现象。


<details>
  <summary>Details</summary>
Motivation: 当前假设认为电路结构差异对应不同机制，但作者质疑这一假设，并通过实验检验。

Method: 在文字序列复制任务中，使用Pythia模型（70M-1.4B）在四个词频频带及对照条件下提取75个电路，进行跨频带迁移测试、核心子图恢复性能及因果干预实验。

Result: 结构不同的电路功能相同，频带特定边可跨频带迁移，核心子图恢复至少99%性能，内部表示可互换；同一频带内重复提取得到等效子图而非唯一机制。

Conclusion: 结构差异不能作为机制不同的充分证据，需要边缘级评估和跨条件迁移测试来揭示功能等价性。
...
### [176] [FOXGLOVE: Understanding Goal-Oriented and Anchored Writing Feedback from Experts and LLMs on Argumentative Essays](https://arxiv.org/abs/2606.06271)
*Yijun Liu,Yifan Song,John Gallagher,Sarah Sterman,Tal August*

Main category: cs.CL

TL;DR: 构建FOXGLOVE数据集比较LLM和教师反馈，发现两者在目标分布上相似但在具体句子选择上不同，LLM反馈评分更高但可能因长度优势。
...
Method: 创建FOXGLOVE数据集，包含696条教师反馈和1644条前沿LLM生成的评论，由专家对子集进行质量评分。

Result: 教师和LLM在反馈目标和文章位置分布上相似，但在具体句子选择上不同；LLM反馈更复杂、使用更少问题，且在多数质量维度上评分更高，但优势可能来自更长评论。

Conclusion: FOXGLOVE支持系统比较人类和LLM反馈的一致性和差异。
...
### [177] [Decomposing Factual Sycophancy in Language Models: How Size and Instruction Tuning Shape Robustness](https://arxiv.org/abs/2606.06306)
*Victor De Marez,Luna De Bruyne,Walter Daelemans*

Main category: cs.CL

TL;DR: 事实上的谄媚可分解为真相余量和操纵敏感性，模型大小和指令调整以不同方式影响这两个通道，评估应报告通道、操纵和大小条件的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 翻转率混淆了真相余量和操纵敏感性，无法揭示机制，需要分解以理解大小和指令调整的影响。

Method: 将事实上的谄媚分解为真相余量和操纵敏感性通道，分析56个开放权重模型（0.3B-32B参数）在13种操纵类型下的表现。

Result: 脆弱性主要受大小控制；指令调整对小型模型降低鲁棒性、对大型模型提高鲁棒性；指令调整增加真相余量但效果依赖操纵类型；缩放对基础模型和指令调整模型的影响不同。

Conclusion: 事实上的谄媚不是单一标量属性，评估应报告通道、操纵和大小条件的鲁棒性，而非仅翻转率。
...
### [178] [EDIT: Evidence-Diagnosed Intervention Training for Rule-Faithful LLM Grading](https://arxiv.org/abs/2606.06350)
*Zhihao Wu,Linhai Zhang,Taiyi Wang,Runcong Zhao,Peter Andrews,Cesare Aloisi,Yulan He*

Main category: cs.CL

TL;DR: 提出EDIT框架，通过内部模型信号定位并修正评分推理错误，结合信念导向奖励塑造，训练更符合评分方案的LLM评分器。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法识别评分推理中的错误步骤和信念变化，导致评分不符合评分方案。

Method: 两阶段框架：EDIT-SFT利用后验信念和输入基础得分定位问题步骤，并用评分清单修正；EDIT-RL通过信念导向奖励塑造校准评分器，惩罚有害信念漂移。

Result: 在两个真实世界多学科评分基准上，EDIT在领域内和领域外均优于强监督微调和强化学习基线，消融实验证实内部状态诊断是关键。

Conclusion: EDIT能有效训练更符合评分方案的LLM评分器，内部状态诊断是提升性能的关键。
...
### [179] [CollabSim: A CSCW-Grounded Methodology for Investigating Collaborative Competence of LLM Agents through Controlled Multi-Agent Experiments](https://arxiv.org/abs/2606.06399)
*Jiaju Chen,Bo Sun,Yuxuan Lu,Yun Wang,Dakuo Wang,Bingsheng Yao*

Main category: cs.CL

TL;DR: 提出CollabSim框架，用于系统分析基于LLM的多智能体系统的协作能力。


<details>
  <summary>Details</summary>
Motivation: 现有MAS评估缺乏对协作能力的系统分析，需要理论驱动的框架。

Method: CollabSim是一个可配置仿真框架，结合理论定义的协作能力、可控交互条件和动作级内部状态探测。

Result: 实验表明CollabSim能捕捉条件效应、分离模型性能模式、揭示任务依赖的代理设计效应。

Conclusion: CollabSim为系统分析代理协作能力提供了有效工具。
...
### [180] [Revising Context, Shifting Simulated Stance: Auditing LLM-Based Stance Simulation in Online Discussions](https://arxiv.org/abs/2606.06443)
*Xinnong Zhang,Wanting Shan,Hanjia Lyu,Zhongyu Wei,Jiebo Luo*

Main category: cs.CL

TL;DR: This paper proposes counterfactual context revision to audit LLM-based stance simulation, evaluating text-only and multimodal strategies, and finds effective stance transitions across different contexts.


<details>
  <summary>Details</summary>
Motivation: It is unclear whether LLM simulations reflect precise user-specific beliefs or are highly sensitive to semantically independent context changes.

Method: We study counterfactual context revision: infer a target user's stance from an original conversation, apply controlled revision strategies (text-only and multimodal including memes), simulate the stance again, and measure average directional stance shift and stance transition rate.

Result: Both text-only and multimodal revision strategies yield effective and robust stance transitions across different polarization-preference mechanisms.

Conclusion: Our study contributes an evaluation framework for understanding context sensitivity of LLM-based stance simulation, highlighting both the promise and risk of using LLMs to simulate online opinion dynamics.
...
### [181] [Latent Reasoning with Normalizing Flows](https://arxiv.org/abs/2606.06447)
*Guancheng Tu,Xiangjun Fu,Suhao Yu,Yao Tang,Haoqiang Kang,Lianhui Qin,Yizhe Zhang,Jiatao Gu*

Main category: cs.CL

TL;DR: NF-CoT是一种通过归一化流在LLM内部进行潜在推理的框架，保留了显式CoT的优势，提高了代码生成任务的通过率并降低了推理成本。


<details>
  <summary>Details</summary>
Motivation: 显式CoT需要将推理步骤序列化为离散标记，限制了带宽和效率；现有潜在推理方法牺牲了CoT的关键优势如左到右生成、概率采样、KV-cache兼容性和似然估计。

Method: NF-CoT在LLM骨干中实例化TARFlow风格的归一化流，为从显式CoT提炼的紧凑连续思维定义可处理概率模型，使用NF头生成连续思维位置，LM头生成文本位置，在同一因果流中实现精确似然、概率性左到右解码和直接策略梯度优化。

Result: 在代码生成基准上，NF-CoT相比显式CoT和先前潜在推理基线提高了通过率，并显著降低了中间推理成本。

Conclusion: NF-CoT通过归一化流在潜在推理空间中实现了显式CoT的优势，是一种高效且兼容现有LLM架构的推理增强方法。
...
### [182] [You Only Index Once: Cross-Layer Sparse Attention with Shared Routing](https://arxiv.org/abs/2606.06467)
*Yutao Sun,Yanqi Zhang,Li Dong,Jianyong Wang,Furu Wei*

Main category: cs.CL

TL;DR: 提出跨层稀疏注意力（CLSA），通过共享KV缓存和路由索引实现高效长上下文推理，在128K上下文下获得7.6倍解码加速和17.1倍吞吐提升。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理中解码效率低，现有稀疏注意力方法在效率和质量间存在权衡：结构块稀疏方法加速但质量下降，token稀疏方法准确但端到端加速有限。

Method: 基于KV共享架构（如YOCO），共享KV缓存和路由索引，单索引器计算一次token级top-k选择并在层间重用，保持细粒度选择性同时分摊路由开销。

Result: 在短上下文和长上下文基准中，CLSA同时保持准确性和高效率，在128K上下文下实现7.6倍解码加速和17.1倍全局吞吐提升。

Conclusion: 该架构为长上下文LLM提供了更完整的解决方案，联合优化模型质量和推理效率。
...
### [183] [Epidemiology of Model Collapse: Modeling Synthetic Data Contamination via Bilayer SIR Dynamics](https://arxiv.org/abs/2606.05168)
*Xiangyu Wang*

Main category: cs.CL

TL;DR: 使用流行病学框架分析AI模型训练中的合成数据交叉污染，发现检测过滤和群体免疫是缓解模型崩溃的有效策略。


<details>
  <summary>Details</summary>
Motivation: 现有分析只考虑单链退化，但实际AI生态系统中模型间的交叉污染更为普遍，需要更全面的模型来理解这种动态。

Method: 提出双层耦合SIR/SIRS平均场模型，将数据语料库和AI模型视为两个交互种群；推导基本再生数R0并进行场景校准、敏感性分析；使用双部分网络代理模型和GPT-2实验验证理论。

Result: 获得R0>1的超临界动力学；合成文本检测是最关键参数；密集网络中代理模型与平均场一致（R^2>0.96）；实验显示剂量反应退化和多样性损失；多源混合在低污染下效果有限；检测过滤和群体免疫为最高杠杆策略。

Conclusion: 交叉污染是模型崩溃的重要机制，建议采用检测过滤和群体免疫策略来缓解，并在实际系统中考虑异质性影响。
...
### [184] [Improving Heart-Focused Medical Question Answering in LLMs via Variance-Aware Rubric Rewards with GRPO](https://arxiv.org/abs/2606.05174)
*Arash Ahmadi,Parisa Masnadi,Sarah Sharif,Charles Nicholson,David Ebert,Mike Banad*

Main category: cs.CL

TL;DR: 本文提出一种基于方差感知奖励框架的组相对策略优化（GRPO）方法，用于后训练大语言模型，在心脏医疗问答中显著提升准确率和F1分数，并接近更大模型的表现。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在医疗部署中面临数据隐私、推理成本和边缘设备适用性等挑战，需要更小更高效的模型，并需要鲁棒的后训练策略以确保可靠的医疗推理能力。

Method: 采用组相对策略优化（GRPO）进行后训练，基于RaR-Medicine的评分标准监督，提出方差感知奖励框架，通过连续解析奖励函数替代加权二元标准和Likert评分，提供更丰富的优化信号，实现更稳定的策略学习。

Result: 在HealthBench的心脏子集上，最佳GRPO变体将准确率从0.362提升至0.502，F1分数从0.532提升至0.668，与GPT-OSS-120B（准确率0.508，F1 0.674）表现相当。

Conclusion: 精心设计的基于评分标准的奖励为改善心脏医疗问答提供了一种实用策略，并有望推广到其他基于评分标准的任务。
...
### [185] [From Scoring to Explanations: Evaluating SHAP and LLM Rationales for Rubric-based Teaching Quality Assessment](https://arxiv.org/abs/2606.05180)
*Ivo Bueno,Babette Bühler,Philipp Stark,Tim Fütterer,Ulrich Trautwein,Dorottya Demszky,Heather Hill,Enkelejda Kasneci*

Main category: cs.CL

TL;DR: Proposes a framework combining SHAP and LLM rationales for sentence-level interpretability of rubric-based scoring; SHAP provides more faithful and transferable explanations than LLM rationales.


<details>
  <summary>Details</summary>
Motivation: Automated scoring models provide little insight into why a particular score is produced, motivating the need for interpretable explanations.

Method: Combines model-agnostic Shapley-value attributions (SHAP) with LLM-generated rationales, instantiated on the Quality of Feedback dimension of CLASS using NCTE corpus. Compares fine-tuned pretrained language models (PLMs) and prompted LLMs on scoring performance and explanation faithfulness.

Result: Fine-tuned PLMs outperform LLMs in prediction accuracy but exhibit label compression toward mid-scale scores. SHAP identifies sentences that reliably drive model predictions with larger and more coherent prediction shifts than LLM rationales. SHAP attributions transfer robustly across architectures, while LLM rationales have limited and inconsistent influence.

Conclusion: SHAP provides more faithful and transferable explanations for rubric-based scoring. The framework offers a principled basis for evaluating scoring models and explanations in high-stakes educational settings and other rubric-based language assessment tasks.
...
### [186] [Trajectory Dynamics in Language Model Hidden States Predict Human Processing Costs Beyond Surprisal](https://arxiv.org/abs/2606.05346)
*Elan Barenholtz*

Main category: cs.CL

TL;DR: 轨迹外推误差，即语言模型隐藏状态的局部动量，独立于surprisal预测阅读时间。


<details>
  <summary>Details</summary>
Motivation: Surprisal将丰富的序列表示简化为单个标量，忽略了解释演化的方向信息；动力系统方法表明轨迹应影响处理成本。

Method: 在每个词上，拟合线性轨迹到之前隐藏状态，测量与外推路径的偏差，得到轨迹外推误差。

Result: 轨迹外推误差与surprisal正交（r=.044），独立预测自定步速阅读时间；效果在花园路径句子中显著，随模型规模增强，跨架构复制。

Conclusion: 处理成本包含两个可分离部分：词级预测误差（surprisal）和对局部动量的敏感性（轨迹外推误差）。
...
### [187] [Multilingual Coreference Resolution via Cycle-Consistent Machine Translation](https://arxiv.org/abs/2606.05444)
*Adriana-Valentina Costache,Eduard Poesina,Silviu-Florin Gheorghe,Paul Irofti,Radu Tudor Ionescu*

Main category: cs.CL

TL;DR: 利用机器翻译和循环一致性生成训练数据，提升低资源语言的核心指代消解性能。


<details>
  <summary>Details</summary>
Motivation: 核心指代消解在英语中研究充分，但在其他语言尤其是低资源语言中关注不足。

Method: 提出一种新颖pipeline，通过机器翻译从英语生成目标语言训练数据，利用反向翻译和BERT余弦相似度评估质量，并将相似度作为权重整合到损失函数中。

Result: 在四种低资源语言上取得显著性能提升，并能在没有先前语料库的语言中实现准确核心指代消解。

Conclusion: 该pipeline有效提升了低资源语言的核心指代消解性能，并适用于无现有语料库的语言。
...
### [188] [CHASE: Adversarial Red-Blue Teaming for Improving LLM Safety using Reinforcement Learning](https://arxiv.org/abs/2606.05523)
*Rahul Markasserithodi,Aditya Joshi,Yuekang Li,Ishmanbir Singh,Chris Yoo,Alan Niu*

Main category: cs.CL

TL;DR: CHASE是一个闭环红蓝对抗框架，通过协同进化攻击者和防御者显著降低黑盒提示重写攻击的成功率并保持零误拒。


<details>
  <summary>Details</summary>
Motivation: 现有防御依赖非可扩展的人工策划或针对特定模型内部的白盒优化，无法应对部署中自适应黑盒攻击的脆弱性。

Method: 采用GRPO训练黑盒攻击者，使用乘法奖励同时优化绕过效果和意图保真度；防御者通过两阶段GRPO+拒绝采样SFT管线在获得的对抗性重写上强化，并平衡良性数据。

Result: 在BeaverTails和JailbreakBench上对五种攻击系列降低平均StrongREJECT分数43.2%，且对良性提示零误拒。

Conclusion: 无模板RL探索能恢复跨机制不同攻击家族的潜在攻击原语，为超越当前对抗训练分布的泛化安全硬化提供路径。
...
### [189] [InfoShield: Privacy-Preserving Speech Representations for Mental Health Screening via Information-Theoretic Optimization](https://arxiv.org/abs/2606.05561)
*Xueyang Wu,Siyuan Liu,Kezhuo Yang,Guang Ling*

Main category: cs.CL

TL;DR: 提出 InfoShield 方法，通过 TimeAwareMINE 最小化语音表示与敏感属性间的互信息，在保持抑郁检测准确性的同时有效保护隐私。


<details>
  <summary>Details</summary>
Motivation: 基于语音的心理健康筛查面临用户人口统计信息暴露的隐私问题，现有对抗训练和差分隐私方法效果不佳。

Method: InfoShield 使用 TimeAwareMINE 和跨模态注意力对齐声学帧与属性嵌入，以最小化互信息。

Result: 在 Androids 语料库上，性别推断从 92.6% 降至 55.5%，年龄推断从 55.7% 降至 30.3%，F1 仅下降 6% 至 0.784，优于先前最优的 0.723。

Conclusion: InfoShield 有效平衡了隐私与效用，适用于语音抑郁检测。
...
### [190] [Predictable Scaling Laws of Optimal Hyperparameters for LLM Continued Pre-training](https://arxiv.org/abs/2606.05610)
*Yongwei Zhou,Juncheng Diao,Junlin Shang,Peiguang Li,Rongxiang Weng*

Main category: cs.CL

TL;DR: 提出一种基于缩放定律的超参数预测框架，显著降低持续预训练中超参数搜索成本。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs持续预训练中超参数配置依赖启发式或网格搜索，导致训练不稳定和成本高。

Method: 分为两阶段：首先训练小规模代理模型，通过损失-计算缩放定律推导计算预算到最优超参数的函数；然后评估初始检查点的验证损失，使用逆缩放定律估计等效预训练计算，结合计划预算预测最优超参数。

Result: 超参数搜索开销减少高达90%，性能与基线相当或更好。

Conclusion: 该模型无关框架可跨架构泛化，为持续预训练提供高效且原则性的方法论。
...
### [191] [What's in a Name? Morphological Shortcuts by LLMs in Pharmacology](https://arxiv.org/abs/2606.05616)
*Kaijie Mo,Thomas Yang,Chantal Shaib,Qing Yao,William Rudman,Ramez Kouzy,Kanishka Misra,Byron C. Wallace,Junyi Jessy Li*

Main category: cs.CL

TL;DR: LLMs rely on morphological affix heuristics to infer drug properties, posing subtle safety risks in medical contexts.


<details>
  <summary>Details</summary>
Motivation: To investigate the overgeneralization risk of LLMs relying solely on word affixes for meaning in high-stakes domains like pharmacology.

Method: Constructed fictitious drug names from real affixes, proposed a framework to decompose drug semantics into affix, stem, and whole-name contributions, and used activation patching to localize the effect.

Result: Models primarily induce drug meaning via affix cues without explicit indication, sometimes conflating affix-sharing drugs; effect localized to early-mid layers.

Conclusion: Morphological shortcuts represent a subtle but measurable safety risk in LLMs.
...
### [192] [An ERP Study on Recursive Locative Processing in Mandarin-Speaking Children with Autism](https://arxiv.org/abs/2606.05620)
*Xiaoyi Wang,Chenxi Fu,Ziman Zhuang,Caimei Yang*

Main category: cs.CL

TL;DR: ERP研究发现ASD儿童处理递归结构时早期预测减弱、语义整合增强和重分析效率降低，支持级联解释。


<details>
  <summary>Details</summary>
Motivation: 递归结构在实时理解中要求高，ASD儿童处理复杂句法有困难，但递归处理的时间动态尚不清楚。

Method: 使用ERP和跨模态句子-图片匹配任务，分析24名儿童（12 ASD, 12 TD）在三个加工阶段（P200, N400, P600）的神经反应，控制心理年龄。

Result: TD儿童显示P200和P600调节，ASD儿童早期差异减弱、晚期重分析效应减少，但N400增强，且ASD组半球偏侧化变异性大。

Conclusion: 支持级联解释：早期预测减少导致整合成本增加和重分析效率降低，强调时间动态和神经变异性在理解ASD语言差异中的重要性。
...
### [193] [Value-and-Structure Alignment for Routing-Consistent Quantization of Mixture-of-Experts Models](https://arxiv.org/abs/2606.05688)
*Hancheol Park,Geonho Lee,Tairen Piao,Tae-Ho Kim*

Main category: cs.CL

TL;DR: 提出VSRAQ，通过价值对齐和结构对齐保持量化前后路由一致性，减少MoE模型量化退化。


<details>
  <summary>Details</summary>
Motivation: MoE模型量化时，路由不稳定导致小扰动改变专家选择，降低模型质量，现有方法未考虑此问题。

Method: VSRAQ结合值对齐（匹配路由logits）和结构对齐（保持专家排序与top-k决策边界）两个目标。

Result: VSRAQ显著提升专家选择一致性，在多个MoE模型上优于仅重构和路由感知基线。

Conclusion: VSRAQ有效减少量化引起的性能下降，无需推理开销，可作为MoE量化通用组件。
...
### [194] [Beyond tokens: a unified framework for latent communication in LLM-based multi-agent systems](https://arxiv.org/abs/2606.05711)
*Yingzhuo Liu*

Main category: cs.CL

TL;DR: 提出了一个潜在通信的统一框架，分类了18种方法，识别了5种设计模式，并提出了开放挑战。


<details>
  <summary>Details</summary>
Motivation: 自然语言通信在高推理成本、离散化信息损失和歧义冗余等结构缺陷，因此需要探索替代的潜在通信协议。

Method: 构建了一个三维分析框架：交流内容（WHAT）、发送-接收对齐（WHICH）、信息融合方式（HOW），并据此分类18种方法，识别5种设计模式。

Result: 系统分类了18种2024-2026年间的方法，提取了5种设计模式，并指出了跨架构对齐、潜在通道安全、压缩部署和潜在思维链等开放挑战。

Conclusion: 该框架旨在降低新研究者入门门槛，并为比较未来工作提供统一词汇。
...
### [195] [ProSPy: A Profiling-Driven SQL-Python Agentic Framework for Enterprise Text-to-SQL](https://arxiv.org/abs/2606.05836)
...
TL;DR: ProSPy is a profiling-driven SQL-Python agentic framework for enterprise-scale Text-to-SQL that combines SQL efficiency with Python flexibility through four-stage reasoning, achieving superior execution accuracy on Spider 2.0 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Large language models face challenges in enterprise-scale Text-to-SQL due to large heterogeneous schemas, incomplete metadata, dialect-specific syntax, and complex analytical questions unsolvable with a single SQL query.

Method: ProSPy structures reasoning into four stages: automatic profiling for fine-grained data evidence, progressive schema pruning, fetching intermediate views via a dialect-agnostic SQL interface, and flexible downstream analysis with Python.

Result: ProSPy achieves execution accuracies of 60.15% and 60.51% with Claude-4.5-Opus on Spider 2.0-Lite and Spider 2.0-Snow without majority voting, outperforming strong baselines, and shows robustness to SQL dialect variations with favorable schema recall-precision trade-off.

Conclusion: ProSPy effectively addresses enterprise-level Text-to-SQL challenges by integrating profiling-driven schema pruning and hybrid SQL-Python execution.
...
### [196] [Staying with the Uncertainty: Uncertainty-Scaffolding Strategies for Artificial Moral Advisors in LLM-to-LLM Simulated Conversations](https://arxiv.org/abs/2606.05890)
*Salvatore Greco,Hainiu Xu,Jacopo Domenicucci,Yulan He,Sylvie Delacroix*

Main category: cs.CL

TL;DR: 本文研究AI道德顾问如何在对话中帮助用户保持不确定性，提出三种不确定性策略并与对照策略比较，发现策略在参与质量上而非立场修正上有所差异。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多用作道德顾问，但应表现出何种对话模式？作者关注如何帮助对话者“与不确定性共存”。

Method: 提出三种不确定性模式（视角倍增、张力保持、过程反思）和三种对照条件（基线、说服、谄媚）。使用用户代理LLM与AMA在伦理困境上对话，进行前后问卷，并测试两种人物提示格式（陈述性和叙事性）。

Result: 没有单一模型主导模拟用户代理；陈述性人物更好捕捉初始立场多样性，叙事性人物显示更现实的信念修正；所有六种策略产生可区分的对话模式；不确定性策略在立场修正程度上无差异，但在参与质量上不同。

Conclusion: 不确定性策略通过不同的参与质量影响对话，而非单纯改变立场。
...
### [197] [Epistemic Injustice in Language Models: An Audit of Pretraining Filters and Guardrails](https://arxiv.org/abs/2606.05936)
*Marco Antonio Stranisci,A Pranav,Rossana Damiano,Christian Hardmeier,Anne Lauscher*

Main category: cs.CL

TL;DR: 过滤器和防护栏不成比例地移除边缘群体提及，导致认知擦除。


<details>
  <summary>Details</summary>
Motivation: 检查过滤和审核决策如何产生认知擦除和系统间紧张关系。

Method: 审计四个预训练过滤器和三个推理时防护栏，使用Common Crawl句子和手动标注子集。

Result: 过滤与词汇线索相关，未能标记私人信息/仇恨言论，边缘群体过度标记，人类会保留大部分被标记内容。

Conclusion: 存在认知擦除，边缘群体提及在预训练和推理时被双重抑制。
...
### [198] [NAVIRA: Decoupled Stochastic Remasking for Masked Diffusion Language Models](https://arxiv.org/abs/2606.06031)
*Andrey Fomenko,Maksim Kryzhanovskiy,Svetlana Glazyrina,Roman Ischenko*

Main category: cs.CL

TL;DR: 提出NAVIRA，一种用于掩码扩散语言模型推理时的解码策略，通过解耦质量检测和重新生成以及随机重掩码来改进文本生成。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型并行生成速度快，但存在纠错问题：同一时间步生成的token基于边际分布，早期局部依赖错误会污染上下文。现有PRISM方法通过学习质量分数和重新掩码不可靠token来解决，但其推理过程耦合，错误token仍然影响重新生成。

Method: 提出NAVIRA，一种推理时解码策略，将检测低质量token和计算替换logits两个操作分离。第一次前向传播评分token，选中的token被掩码，第二次前向传播从清洁的上下文重新生成。采用温度控制的随机重掩码，减少重复纠正同一位置，平衡流畅性和多样性。

Result: 在170M参数的掩码扩散语言模型上，解耦操作提高了流畅性，而调度随机重掩码保持了熵，并在更大的前向传播预算下获得了更强的LLM-judge分数。

Conclusion: 结果表明，重掩码策略（而不仅仅是学习到的质量信号）对于可靠的掩码扩散文本生成至关重要。
...
### [199] [Multi-task Learning is Not Enough: Representational Entanglement in Dual-output Second Language Speech Recognition](https://arxiv.org/abs/2606.06065)
*Seung Hwan Cho,Young-Min Kim*

Main category: cs.CL

TL;DR: 多任务学习在第二语言语音识别中提高意义识别但降低表面转录，特别是在英语中，这种降级与编码器级纠缠有关。


<details>
  <summary>Details</summary>
Motivation: 检验多任务学习假设（共享表示有利于两个输出）在韩国语和英语第二语言语音识别中是否成立。

Method: 采用多任务学习（MTL）同时进行表面转录和意义识别，并使用Levenshtein编辑距离分析表面-意义分歧，通过编码器级纠缠和跨任务解码器分析来探究表示共享程度。

Result: MTL提高意义识别但降低表面转录，英语中降级更严重，且与表面-意义分歧成正比；韩国语保持不同任务表示，英语则产生几乎相同的编码器表示；意义双输出解码器形成独特表示，而表面双输出解码器受编码器约束。

Conclusion: 需要设计减轻编码器级纠缠的MTL框架，以减少双输出L2自动语音识别中的表面转录性能下降。

Abstract: Second-language (L2) speech recognition often requires transcriptions of pronunciations and intended meanings. Multi-task learning (MTL) is a natural approach because it assumes that shared representations benefit both outputs. However, this paper shows that this assumption does not hold across Korean and English. MTL improves meaning but degrades surface transcription, especially in English, where the degradation scales with surface-meaning divergence measured by Levenshtein edit analysis links these patterns to encoder-level entanglement, with Korean preserving distinct task representations while English produces nearly identical ones. Cross-task decoder analysis shows that the meaning dual-output decoder adapts with a unique representation, while the surface dual-output decoder remains constrained by the encoder. These findings motivate the design of MTL frameworks that mitigate encoder-level entanglement to reduce surface degradation in dual-output L2 automatic speech recognition.

</details>


### [200] [SkillComposer: Learning to Evolve Agent Skills for Specification and Generalization](https://arxiv.org/abs/2606.06079)
...
TL;DR: SkillComposer将技能构建分解为创建、改进和合并三种可学习操作，使语言模型在推理时自我进化技能，显著提升代理和代码任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有技能构建方法存在特定技能不迁移与抽象技能指导不足之间的矛盾，缺乏技能规范与泛化的显式机制。

Method: SkillComposer通过三个可学习的操作（创建、改进、合并）将技能构建分解，利用系统拒绝采样训练，使语言模型在推理时自我进化技能，并支持离线、在线和混合三种部署模式。

Result: 在τ²-Bench、LiveCodeBench v6和AppWorld上，SkillComposer-4B改善了27B执行器，代理任务提升+4.5，代码任务提升+3.4，且能泛化到未见过的领域和任务类型。

Conclusion: 合并和改进操作处理正交的质量维度，技能组合是可迁移的元能力，为技能增强推理提供了实用方案。
...
### [201] [LatentSkill: From In-Context Textual Skills to In-Weight Latent Skills for LLM Agents](https://arxiv.org/abs/2606.06087)
*Aofan Yu,Chenyu Zhou,Tianyi Xu,Zihan Guo,Rong Shan,Zhihui Fu,Jun Wang,Weiwen Liu,Yong Yu,Weinan Zhang,Jianghao Lin*

Main category: cs.CL

TL;DR: LatentSkill将文本技能转换为LoRA适配器，存储在权重空间，减少上下文开销并提升性能。


<details>
  <summary>Details</summary>
Motivation: Agent系统使用文本技能但注入提示导致高上下文开销和技能暴露。

Method: 通过预训练超网络将文本技能转换为即插即用的LoRA适配器，将技能知识存储在权重空间。

Result: 在ALFWorld上成功提升21.4/13.4点，预填充令牌减少64.1%；在Search-QA上精确匹配提升3.0点，技能令牌开销降低72.2%。

Conclusion: 权重空间技能为扩展LLM代理提供了高效、模块化和更少暴露的基底。
...
### [202] [From Self to Other: Evaluating Demographic Perspective-Taking in LLM Hate Speech Annotation](https://arxiv.org/abs/2606.06266)
*Paloma Piot,Javier Parapar*

Main category: cs.CL

TL;DR: 人格条件LLM在模拟仇恨言论标注的主观性方面表现不一，但Llama 3.1的替代提示最接近人类分歧模式。


<details>
  <summary>Details</summary>
Motivation: 仇恨言论检测主观性强，多群体标注成本高，希望用LLM模拟不同视角。

Method: 使用人格条件LLM，评估三个社会判断维度：组间分歧、内群体敏感性、替代预测。

Result: 没有模型一致地捕捉所有三个维度，Llama 3.1替代提示在跨组一致性上最优。

Conclusion: 表明某些配置可能更适合自动标注以对齐人类判断。
...
### [203] [Reinforcement Learning Elicits Contextual Learning of Unseen Language Translation](https://arxiv.org/abs/2606.06428)
*Hanxu Hu,Zdeněk Šnajdr,Pinzhen Chen,Jannis Vamvas,Rico Sennrich*

Main category: cs.CL

TL;DR: Proposes using reinforcement learning with chrF reward to teach LLMs to translate unseen languages by leveraging in-context linguistic knowledge, outperforming in-context learning and supervised fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Prior methods (continued training, encoding grammar book) overfit specific languages with limited zero-shot transfer. LLMs must acquire meta-skill of using in-context linguistic knowledge rather than memorizing.

Method: Employs reinforcement learning with chrF as reward for unseen language translation given rich linguistic context.

Result: RL-trained models effectively extract and apply relevant linguistic information, leading to better translations on completely unseen languages than in-context learning or supervised fine-tuning.

Conclusion: Outcome-based RL can extend beyond reasoning tasks to serve as a recipe for language learning from context.
...
### [204] [Human Adults and LLMs as Scientists: Who Benefits from Active Exploration?](https://arxiv.org/abs/2606.06464)
*Mandana Samiei,Eunice Yiu,Anthony GX-Chen,Dongyan Lin,Jocelyn Shen,Blake A. Richards,Alison Gopnik,Doina Precup*

Main category: cs.CL

TL;DR: 研究通过主动探索任务发现，成年人主动探索能显著改善合取因果推理，但合取规则仍需更多测试，且与语言模型相比存在差距。


<details>
  <summary>Details</summary>
Motivation: 传统研究发现成年人难以识别合取因果规则，在析取中表现更好，但大多依赖被动观察，本文探究主动探索下该偏见是否持续。

Method: 使用改进的blicket detector任务，让成年人自由干预以识别合取或析取规则下的因果对象，并与多种大型语言模型进行比较。

Result: 主动探索显著提高了合取因果推理，但合取规则仍需更多测试；语言模型在假设推断准确性上接近人类，但探索策略效率较低，且存在类似的合取-析取差距。

Conclusion: 主动探索有助于改善合取因果推理，但未完全消除差异；语言模型表现出与人类相似的模式，但探索效率不足。
...
TL;DR: 通过使用预训练的大型fMRI编码模型TRIBE v2生成合成数据增强小样本fMRI数据集，可显著提升图像解码性能，最高提升68%的Top-10检索准确率，并支持零样本解码。


<details>
  <summary>Details</summary>
Motivation: 脑解码受限于标记神经数据的可用性，低数据量下性能不佳，需要提高数据效率。

Method: 使用预训练的大型编码模型TRIBE v2（基于超过1000小时fMRI响应数据）生成合成fMRI数据，并将其与真实数据结合训练图像解码器，通过系统网格搜索优化合成数据比例。

Result: 在7T和3T两个fMRI数据集上，合成数据增强使Top-10图像检索准确率提升高达68%；最佳合成数据比例因数据源而异；仅用合成数据训练的解码器在某些设置下仍能高于随机水平。

Conclusion: 大规模fMRI编码模型可以为改善图像解码的数据效率提供基础，通过合成数据增强缓解标记数据稀缺问题。
...
### [206] [FIDES: Faithful Inference via Deep Evidence Signals for Retrieval-Memory Conflict in RAG](https://arxiv.org/abs/2606.05644)
*Zhe Yu,Wenpeng Xing,Tiancheng Zhao,Mohan Li,Changting Lin,Meng Han*

Main category: cs.AI

TL;DR: FIDES通过令牌级冲突检测和对比解码提升检索增强语言模型的上下文忠实度。


<details>
  <summary>Details</summary>
Motivation: 现有对比解码假设检索-记忆冲突均匀分布，导致全局权重对安全令牌过度惩罚而对冲突令牌纠正不足。

Method: FIDES是一种训练自由解码器，融合输出表面、隐藏表示和预测轨迹三种内部信号来探测令牌级冲突，并动态调整干预强度。

Result: 在三个基准和六个骨干模型（含70B）的所有18个设置中，FIDES取得最佳上下文忠实度，相比最强基线提升+3到+13点，70B规模忠实度达92-94%，F1达62-63%。

Conclusion: 令牌级选择性对比解码能解锁粗粒度对比规则所抑制的生成能力，显著提升检索增强模型的忠实度。
...
TL;DR: 本文发现RAG在结构推理查询上系统性失败，提出操作符词汇论，并设计LLM查询规划器，通过提供遍历和图计算原语显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: RAG在处理需要结构推理的查询时系统性失败，现有方法无法有效处理实体间的关系。

Method: 比较了八种检索架构，从文本检索到图遍历再到图计算，基于一个46节点64条边的知识图谱评估了10个意图类别的23个查询。提出操作符词汇论，并设计了一个LLM查询规划器，包含9个类型化遍历原语和6个图计算工具。

Result: 五类查询对于向量检索结构上不可达。LLM查询规划器在F1上优于专用处理器（0.632 vs 0.472），并能泛化到未见查询。添加图计算工具后，LLM选择性地在遍历失败的类别上采用。同时发现实体级F1低估了结构查询性能。

Conclusion: 基于LLM的图推理的限制不在于模型智能，而在于可用的操作符。通过提供适当的计算原语，可以显著提升LLM在结构推理上的能力，并指出了现有评估指标的不足。
...
Abstract: Large language model (LLM) agents are increasingly applied to long-horizon tasks such as scientific discovery and machine learning engineering (MLE), where sustained self-evolution becomes a key capability. However, existing MLE agents suffer from inter-branch information isolation, memoryless search, and lack of hierarchical control, which together hinder long-horizon optimization. We present MLEvolve, an LLM-based self-evolving multi-agent framework for end-to-end machine learning algorithm discovery. By extending tree search to Progressive MCGS, MLEvolve enables cross-branch information flow through graph-based reference edges and gradually shifts the search from broad exploration to focused exploitation with an entropy-inspired progressive schedule. To allow the agent to evolve with accumulated experience, we introduce Retrospective Memory, which combines a cold-start domain knowledge base with a dynamic global memory for task-specific experience retrieval and reuse. For stable long-horizon iteration, we further decouple strategic planning from code generation with adaptive coding modes. Evaluation on MLE-Bench shows that MLEvolve achieves state-of-the-art performance across multiple dimensions including average medal rate and valid submission rate under a 12-hour budget (half the standard runtime). Moreover, MLEvolve also outperforms specialized algorithm discovery methods including AlphaEvolve on mathematical algorithm optimization tasks, demonstrating strong cross-domain generalization. Our code is available at .

</details>


### [209] [Answer Presence Drives RAG Rewriting Gains](https://arxiv.org/abs/2606.05633)
*Yuejie Li,Yueying Hua,Ke Yang,Li Zhang,Yueping He,Yueping He,Ruiqi Li,Bolin Chen,Tao Wang,Bowen Li,Chengjun Mao*

Main category: cs.AI

TL;DR: 检索增强型QA管道中，重写器的性能提升主要来自答案字符串泄漏到重写上下文中，而非提高证据质量。


<details>
  <summary>Details</summary>
Motivation: 探究重写器提升F1分数的真正原因，怀疑是答案泄漏而非证据质量提升。

Method: 通过控制编辑重写上下文（移除答案、替换随机非答案、注入答案等）进行干预实验，在多个模型、数据集和编译器设置下测试。

Result: 移除答案导致F1下降28-64点，注入答案提升F1 0.7-9.7点，且单一[MASK]探针审计不可靠。

Conclusion: 重写器的增益主要来自答案泄漏，但未提出新方法，仅发布干预运行器和哨兵面板供进一步测试。
...
### [212] [Ten Headache Specialists versus Artificial Intelligence for Clinical Literature Summarization: A Critical Evaluation and Comparison](https://arxiv.org/abs/2606.05436)
...
TL;DR: 研究比较了LLM和专家撰写的医学文献摘要，发现专家摘要更受青睐，但专家有时难以区分，并识别出专家重视的关键特征。


<details>
  <summary>Details</summary>
Motivation: 临床医生时间有限且文献增长迅速，需要有效的文献摘要工具，但缺乏对LLM与专家摘要直接比较的人类评估。

Method: 构建基于RAG的代理AI框架，使用三种LLM（Sonnet、GPT-4o、Llama 3.1），由头痛专家创建13个问题，10位专家各写一篇摘要，每问题四篇摘要（专家和三个LLM），盲法专家根据正确性、完整性、简洁性和临床实用性评分并偏好排序。

Result: 专家撰写的摘要被偏好，但专家有时难以区分AI和人类生成的摘要。

Conclusion: 尽管专家摘要更受青睐，但LLM摘要已接近专家水平，专家重视的特征可用于指导未来摘要管道的改进。
...
### [213] [EpiEvolve: Self-Evolving Agents for Streaming Pandemic Forecasting under Regime Shifts](https://arxiv.org/abs/2606.05513)
*Yiming Lu,Sihang Zeng,Zhengxu Tang,Max Lau,Fei Liu,Wei Jin*

Main category: cs.AI

TL;DR: EpiEvolve is a self-evolving agent that adapts LLM forecasters to streaming pandemic forecasting, improving accuracy and reducing recovery lag after regime shifts.


<details>
  <summary>Details</summary>
Motivation: The mismatch between static supervised training and streaming operational forecasting with shifting disease regimes motivates the need for adaptive models.

Method: EpiEvolve wraps a frozen LLM forecaster with hierarchical episodic memory, reflecting on delayed labels, retrieving regime-relevant cases, and distilling recurring errors into strategic rules, following a chronological protocol to prevent leakage.

Result: EpiEvolve achieves 0.629 average accuracy vs. 0.561 (static backbone) and 0.325 (CDC ensemble), and reduces recovery lag from 5 to 2 weeks after regime shifts.

Conclusion: EpiEvolve effectively adapts to streaming pandemic forecasting, with reflection, strategic memory, and regime-aware retrieval each contributing to gains, as shown by ablations.
...
### [214] [SubtleMemory: A Benchmark for Fine-Grained Relational Memory Discrimination in Long-Horizon AI Agents](https://arxiv.org/abs/2606.05761)
*Wenxuan Wang,Haoyu Sun,Fukuan Hou,Mingyang Song,Weinan Zhang,Yu Cheng,Yang Yang*

Main category: cs.AI

TL;DR: 提出SubtleMemory基准，测试长期运行AI助手的细粒度关系记忆辨别能力，发现现有系统表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有长期记忆基准很少探测代理如何保存和利用记忆关系，导致对关系依赖的辅助任务评估不足。

Method: 构建关系控制的潜在语义伪像，嵌入到用户-代理历史中，要求代理恢复分布式关系结构，包含1522个评估实例和1090个记忆变体集。

Result: 评估了多种内存系统和Claw-style代理，发现当前系统在细粒度关系记忆辨别上表现弱，并引入诊断协议揭示不同阶段的能力轮廓。

Conclusion: 当前系统对细粒度关系记忆辨别能力不足，需要改进以处理复杂记忆关系。
...
TL;DR: 提出QCFuse，一种用于RAG缓存融合的压缩视图查询感知选择器，在匹配完全预填充质量的同时，显著加速预填充阶段。


<details>
  <summary>Details</summary>
Motivation: 现有RAG缓存融合选择器在质量和效率之间两难：快速的查询无关或最终层选择器可能遗漏证据，而全视图查询感知选择器需要广泛上下文和层可见性，阻碍缓存融合流水线。

Method: QCFuse使用块锚点查询探测将用户查询状态条件化为紧凑的逐块锚点，并通过关键层分析识别重新计算令牌，无需全层检查。

Result: 在四个开源LLM和六个数据集上，QCFuse达到完全预填充质量，平均预填充时间加速：相比完全预填充1.7倍，相比ProphetKV 1.5倍。

Conclusion: QCFuse在保持高质量的同时实现了显著的效率提升，为RAG缓存融合提供了实用方案。
...
### [216] [Memory is Reconstructed, Not Retrieved: Graph Memory for LLM Agents](https://arxiv.org/abs/2606.06036)
*Shuo Ji,Yibo Li,Bryan Hooi*

Main category: cs.AI

TL;DR: MRAgent通过主动重建和关联记忆图显著提升长交互历史推理。


<details>
  <summary>Details</summary>
Motivation: 当前LLM agents基于静态检索-推理范式，无法动态适应推理过程中的中间证据，导致长交互历史推理困难。

Method: 提出MRAgent，使用Cue-Tag-Content关联记忆图和主动重建机制，将LLM推理整合到记忆访问中，迭代探索和修剪检索路径。

Result: 在LoCoMo和LongMemEval基准测试中，相比强基线提升高达23%，同时显著降低令牌和运行时间成本。

Conclusion: 主动和关联重建对于长期记忆推理是有效的。
...
### [217] [When Should Memory Stay Silent: Measuring Memory-Use Boundaries in Memory-Augmented Conversational Agents](https://arxiv.org/abs/2606.06055)
*Lingxiang Xu,Jiaoyun Yang,Min Hu,Hongtu Chen,Ning An*

Main category: cs.AI

TL;DR: 提出RBI-Eval评估语言模型在敏感记忆整合上的行为，发现现有模型在敏感内容上存在过度整合问题，影响安全个性化。


<details>
  <summary>Details</summary>
Motivation: 现有记忆评估关注检索准确率和下游任务效用，忽略了检索到的敏感记忆内容在当前对话轮次中是否合适整合。

Method: 构建RBI-Eval探针集，在相同良性提示下，比较有和没有敏感记忆访问时模型的行为差异。评估四个基础LLM，使用全上下文暴露和三种检索系统四种设置。

Result: 有记忆可用时，敏感记忆整合的分离分数下降，其中GPT-5.4-mini下降8.9%-26.6%，而其他三个模型下降51.1%-82.9%。控制实验表明效果特定于敏感内容而非一般个性化。检索系统减少暴露但未消除整合。

Conclusion: 安全个性化需要在检索和生成时都进行记忆感知决策，以避免不合适的敏感记忆整合。
...
### [218] [Evaluating Agentic Configuration Repair for Computer Networks](https://arxiv.org/abs/2606.06212)
*Rufat Asadli,Benjamin Hoffman,Ioannis Protogeros,Laurent Vanbever*

Main category: cs.AI

TL;DR: 结合正式验证和上下文检索的代理架构显著提升LLM在网络配置修复中的效果和安全性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在复杂网络配置修复中表现不佳且易引入新错误，需要提升其效果和安全性。

Method: 使用开源和闭源LLM，并增强正式网络验证和上下文检索工具，采用代理架构动态管理上下文和迭代验证。

Result: 修复效果平均提升12%，安全性提升17%。

Conclusion: 代理架构通过动态上下文管理和迭代验证，有效提升配置修复性能。
...
### [219] [ToolChoiceConfusion: Causal Minimal Tool Filtering for Reliable LLM Agents](https://arxiv.org/abs/2606.06284)
*Rahul Suresh Babu,Laxmipriya Ganesh Iyer*

Main category: cs.AI

TL;DR: 提出Causal Minimal Tool Filtering (CMTF)，一种无需训练的方法，通过因果充分性选择工具，在保持任务成功率的同时大幅减少工具暴露和令牌成本。


<details>
  <summary>Details</summary>
Motivation: 现有工具选择方法通常优化语义相关性，但相关性不足以确保工具在当前步骤的必要性，导致错误调用、过早行为和令牌浪费。

Method: CMTF利用轻量级前提-效果契约，基于因果充分性过滤出从当前状态向用户目标推进所需的最小下一步工具集。

Result: 在102个任务、100个工具、4种LLM后端的基准测试中，CMTF在任务成功率上与最强因果基线持平，但每步可见工具从100降为1，令牌使用减少约90%。

Conclusion: 因果充分性是比语义相关性更有效的工具选择原则，CMTF以训练-free方式实现了高效、可靠的工具选择。
...
### [220] [Agent Memory: Characterization and System Implications of Stateful Long-Horizon Workloads](https://arxiv.org/abs/2606.06448)
*Yasmine Omri,Ziyu Gan,Zachary Broveak,Robin Geens,Zexue He,Alex Pentland,Marian Verhelst,Tsachy Weissman,Thierry Tambe*

Main category: cs.AI

TL;DR: 首个对LLM代理记忆系统的系统级特性描述，包括四轴分类法、相位感知分析工具、十系统表征和十条设计建议。


<details>
  <summary>Details</summary>
Motivation: LLM代理在长期任务中需要持久化记忆，但现有记忆系统缺乏系统级行为描述。

Method: 提出系统导向四轴分类法，构建相位感知分析工具归因成本到构建、检索和生成，并表征十个代表性系统。

Result: 揭示了设计选择如何影响读写路径成本，并推导出十个系统建议，覆盖构建调度、能力下限、查询量摊销、新鲜度-延迟权衡和集群管理。

Conclusion: 首次对agent memory系统进行系统级特性分析，为优化和设计提供了实用指导。
...
### [221] [Can LLMs Write Correct TLA+ Specifications? Evaluating Natural-Language-to-TLA+ Generation](https://arxiv.org/abs/2606.05792)
...
TL;DR: 首次系统评估LLM生成TLA+规范的能力，发现语义正确率仅8.6%，且成功依赖渐进提示，模型大小不决定质量。


<details>
  <summary>Details</summary>
Motivation: TLA+规范编写耗时且需专业知识，LLM虽有潜力但缺乏系统评估。

Method: 评估30个LLM（25个开源+5个专有）在205个规范上，使用多种提示策略，通过SANY解析器和TLC模型检查器验证。

Result: 语法正确率最高26.6%，语义正确率仅8.6%；渐进提示效果显著；模型大小与质量无关；代码专用模型表现差；识别五种幻觉类别。

Conclusion: 当前LLM无法可靠生成TLA+规范，需专家监督，并公开框架和数据集以促进研究。
...
### [222] [Harnessing Generalist Agents for Contextualized Time Series](https://arxiv.org/abs/2606.05404)
...
TL;DR: 提出TAPO方法解决GRPO在工具增强的多模态搜索代理中的信用错配问题，通过反事实见证和置信门控保守优势校正实现改进。


<details>
  <summary>Details</summary>
Motivation: GRPO在工具增强的多模态搜索代理中，其均匀广播轨迹级优势导致有价值的工具使用步骤在失败轨迹中被错误惩罚，浪费了训练信号。

Method: TAPO利用工具的参数确定性属性，通过在当前训练批次中构建反事实见证，并使用置信门控保守优势校正来补偿错误分配的负信用。

Result: TAPO在多个多模态搜索基准上，对三种主流RL算法（GRPO、GSPO、SAPO）均带来一致的即插即用改进，且无需额外标注、模型或采样，计算开销可忽略。

Conclusion: TAPO有效解决了信用错配问题，提升了训练信号利用效率，显著改善多模态搜索代理性能。
...
TL;DR: 提出MGSD，一种两阶段模态差距感知的自蒸馏框架，通过冷启动接地和特权教师蒸馏，显著提升视觉语言模型的视觉空间规划能力。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视觉空间规划上存在感知-推理模态差距，导致视觉状态恢复和多步规划的双重瓶颈。

Method: MGSD包含两个阶段：冷启动接地阶段使视觉学生获得可靠状态表示；特权教师阶段通过在线策略蒸馏，使用符号状态监督学生视觉轨迹前缀，符号数据仅用于训练。

Result: 在视觉规划基准上，使用4B和8B骨干网络，宏观平均分别提升19.3%和18.4%，缩小了与符号输入上限的差距。

Conclusion: 模态差距感知的自蒸馏不仅改进了模型感知可行动状态的方式，还改进了其在推断结构上进行规划的能力。

Abstract: While vision-language models excel at general multimodal understanding, they still struggle with visual spatial planning. We attribute this to a perception-reasoning modality gap: visual planning requires models to infer latent state structures from pixels and then reason over the recovered structure to produce valid actions, whereas symbolic planning directly leverages explicit objects and constraints. This creates dual bottlenecks in visual state recovery and multi-step planning. To address this, we propose MGSD, a two-stage modality-gap-aware self-distillation framework. First, a cold-start grounding stage equips the visual student with reliable state representations, minimizing early perception noise. Second, a privileged teacher transfers planning capabilities via on-policy distillation, using explicit symbolic states to supervise the student's own visual rollout prefixes. Crucially, symbolic data is used strictly during training, leaving inference purely visual. Experiments on visual planning benchmarks show that MGSD consistently improves visual planning across both 4B and 8B backbones, raising the macro average by 19.3% and 18.4%, respectively. The resulting models narrow the gap to symbolic-input upper bounds, while ablations and diagnostics confirm that the improvement comes from both visual state recovery and optimal-path reasoning. These results suggest that modality-gap-aware self-distillation improves not only how models perceive actionable states, but also how they plan over the inferred structure. Code is available at .

</details>


### [229] [TokenMizer: Graph-Structured Session Memory for Long-Horizon LLM Context Management](https://arxiv.org/abs/2606.06337)
*Shweta Mishra*

Main category: cs.AI

TL;DR: TokenMizer 通过知识图谱和混合提取管道高效压缩 LLM 会话历史，以 78 令牌的平均恢复块实现更高的决策、任务和文件召回。


<details>
  <summary>Details</summary>
Motivation: LLM 上下文窗口有限，历史超出最大有效上下文窗口时结构化信息被丢弃，现有方法将历史视为纯文本，破坏了关系结构。

Method: TokenMizer 将 LLM 会话历史建模为类型化知识图谱（14 种节点，7 种边），使用混合提取管道增量填充图，三层检查点系统序列化为紧凑恢复块，8 层压缩管道减少上下文开销，语义缓存减少重复查询延迟。

Result: 在 21 个会话基准测试中，TokenMizer 生成平均 78 令牌的恢复块（范围 42-124），比基线小 2 倍，决策召回率提高 9-17 个百分点；平均任务召回 51.0%，决策召回 46.6%，文件召回 58.7%；模糊标签匹配是主要改进因素（+33 pp 任务召回），启发式压缩实现 47.3% 令牌减少。

Conclusion: TokenMizer 提供了一种可查询的替代方案，以一半的令牌成本实现更高的结构化信息保留，优于文本保留基线。
...
### [230] [Benchmark Everything Everywhere All at Once](https://arxiv.org/abs/2606.06462)
*Shiyun Xiong,Dongming Wu,Peiwen Sun,Yuang Ai,Bokang Yang,Wencheng Han,Xiao-Hui Li,Xiangyu Yue*

Main category: cs.AI

TL;DR: Benchmark Agent是一个全自动代理系统，用于从用户查询到质量控制的完整基准构建，能生成高质量样本且减少人力。


<details>
  <summary>Details</summary>
Motivation: 现有基准构建劳动密集、难以重用、性能饱和。

Method: 提出Benchmark Agent，一个完全自主的代理系统，协调基准构建流程，包括用户查询分析、子任务设计、数据注释和质量控制。

Result: 通过15个基准实验，包括人类评估和LLM评估，证明能生成高质量样本，并观察到当前模型在领域特定推理上表现不佳。

Conclusion: 快速演化的基准对研究社区有贡献。
...
TL;DR: 提出Query Retrieve Conclude零样本框架，通过检索开放网络证据补充背景知识，提升模因理解和检测性能，并建立新基准。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖可能不完整或过时的固定知识，无法处理需要最新背景知识的新兴模因。

Method: 提出Query Retrieve Conclude框架，包括识别缺失知识、检索开放网络证据、合成证据背景知识的零样本方法。

Result: 在多个数据集上，该框架在知识恢复、模因理解和检测任务上优于零样本基线。

Conclusion: 研究表明动态检索外部知识可有效提升模因理解，框架和基准为未来研究提供基础。
...
### [232] [Synthetic Contrastive Reasoning for Multi-Table Q&A](https://arxiv.org/abs/2606.05382)
*Ankit Pratap Singh,Xin Su,Phillip Howard*

Main category: cs.AI

TL;DR: 通过合成对比推理轨迹数据集并使用对比偏好优化（CPO）微调LLMs，在多表问答任务上显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有多表问答资源缺乏推理监督，无法解释答案如何推导而来。

Method: 使用异构LLMs生成验证的正轨迹和合理的负轨迹，构建合成对比推理轨迹数据集，并采用对比偏好优化（CPO）对开放权重LLMs进行微调。

Result: 在Qwen3-14B、Mistral-8B和Llama-3.1-8B上，CPO相比Q&A监督微调取得了9.7%-16.3%的绝对平均改进，在MMQA上提升高达21个百分点。

Conclusion: 异质正负轨迹生成器增强了对比信号，自动和人工评估表明生成的轨迹对是忠实、连贯且有意义对比的，验证了方法的有效性。
...
### [233] [GuardNet: Ensemble Strategies of Shallow Neural Networks for Robust Prompt Injection and Jailbreak Detection](https://arxiv.org/abs/2606.05566)
...
TL;DR: GuardNet是一个基于BiLSTM的集成轻量级对抗性攻击检测系统，通过阈值校准和多样化示例覆盖，在资源受限环境下实现高效能。


<details>
  <summary>Details</summary>
Motivation: LLMs容易受到Prompt Injection和Jailbreak攻击，并且基准评估可能受到污染和信息泄露的影响，因此需要可靠的防护机制。

Method: 使用由约4700万参数的浅层BiLSTM神经网络组成的集成系统，重点通过多样性示例覆盖和阈值校准来提升鲁棒性。

Result: 在盲测JBB-Behaviors基准上AUROC为0.747（n=200），在专有基准上F1为0.92（n=50），平均延迟约50ms（CPU）。

Conclusion: GuardNet在有限资源下表现出竞争性能，适合生产部署，但大型语言模型（如Mistral-7B、Llama-3.1-8B）在性能指标上仍更优。
...
### [234] [Evaluation of LLMs for Mathematical Formalization in Lean](https://arxiv.org/abs/2606.05632)
*Tyson Klingner,Drew Bladek,Escher Crawford,Bohao Chen,Ariel Fu,Kaira Nair,Jarod Alper,Giovanni Inchiostro,Vasily Ilin*

Main category: cs.AI

TL;DR: 比较了多种LLM在Lean 4中生成形式证明的能力，发现Gemini 3.1 Pro和Claude Opus 4.7表现最佳，而NVIDIA Nemotron 3 Super和GPT-OSS 120B成本效益最高。


<details>
  <summary>Details</summary>
Motivation: 帮助那些寻求使用LLM支持自己项目的人。

Method: 使用pass@k和refine@k指标，在miniF2F和miniCTX数据集的子集上评估多个LLM。

Result: Gemini 3.1 Pro在miniF2F上refine@32准确率92%，Claude Opus 4.7在miniCTX上refine@32准确率86%；NVIDIA Nemotron 3 Super和GPT-OSS 120B平均成本低于$0.01每个正确证明。

Conclusion: Gemini 3.1 Pro和Claude Opus 4.7是性能最佳的选择，而考虑成本时NVIDIA Nemotron 3 Super和GPT-OSS 120B更具优势。
...
### [235] [PerceptUI: LLM Agents as Human-Aligned Synthetic Users for UI/UX Evaluation](https://arxiv.org/abs/2606.05697)
*Nicolas Bougie,Xiaotong Ye,Gian Maria Marconi,Narimasa Watanabe*

Main category: cs.AI

TL;DR: PerceptUI is a framework for persona-conditioned UI/UX evaluation that predicts how a specific user would answer interface-related questions and produces natural-language rationales.


<details>
  <summary>Details</summary>
Motivation: Existing UI/UX evaluation methods rely on human participants or A/B tests, which are slow and costly, and current LLM-based proxy evaluators produce biased or surface-level critiques.

Method: PerceptUI is trained in two stages: (i) contrastive reflection fine-tuning distills teacher-generated rationales by extracting lessons from human decisions, and (ii) a reflective prompt-evolution step from the model's own failure traces.

Result: Across multiple domains and datasets, PerceptUI achieves human-level realism, generalizes to unseen questions and personas, and yields population-level response distributions.

Conclusion: PerceptUI provides a novel and efficient alternative to human evaluation for UI/UX tasks, enabling faster iteration without sacrificing reliability.
...
### [238] [DragOn: A Benchmark and Dataset for Drag-Based GUI Interactions](https://arxiv.org/abs/2606.06322)
*Nathan Bout,Maxime Langevin,Ronan Riochet*

Main category: cs.AI

TL;DR: 介绍DragOn，一个拖拽 grounding 基准和训练数据集，旨在提升模型在复杂拖拽交互上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型在点击 grounding 上进展显著，但拖拽 grounding 数据量级小，模型在拖拽操作上表现不足。

Method: 构建DragOn数据集，覆盖文本高亮、单元格选择、元素缩放和滑块操作四个领域，包含286K训练截图和3.5M训练任务，以及2000个评估样本；评估了GPT、Claude、Qwen、Kimi、Holo等模型，并微调了Qwen VLM。

Result: DragOn数据集能够提升现有最佳模型在下游计算机使用任务上的性能。

Conclusion: DragOn数据集可改善拖拽 grounding 能力，进而提升下游任务表现。
...
### [239] [Humans' ALMANAC: A Human Collaboration Dataset of Action-Level Mental Model Annotations for Agent Collaboration](https://arxiv.org/abs/2606.06388)
...
TL;DR: 提出了ALMANAC数据集，用于评估LLM代理在协作中推断人类心理模型和预测行为的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM代理主要优化任务完成，缺乏协作能力，且缺少带有行动级心理模型注释的真实人类协作数据。

Method: 从Map Task任务构建了包含2,987个协作动作的数据集，每个动作标注了自我推理、感知伙伴意图和感知团队目标，并基于此对六个LLM进行基准测试。

Result: 实验表明ALMANAC数据集可以有效评估模型模拟人类协作行为和推断心理模型的能力。

Conclusion: ALMANAC数据集填补了真实人类协作数据缺失的空白，并为开发具有协作能力的LLM代理提供了重要资源。
...
TL;DR: 分析Reddit上未公开AI生成帐户的实验语料，发现AI评论普遍使用身份定位、权威信号和认知偏差触发，形成不同于人类的说服性修辞结构，导致真假立场难以区分。


<details>
  <summary>Details</summary>
Motivation: 探索未公开AI在身份丰富的论坛中如何运作，以及其修辞策略与人类的差异，为检测和审计AI系统提供依据。

Method: 对Reddit r/ChangeMyView实验中AI生成评论的公开存档进行结构化内容分析，评估身份表现、权威信号、结盟策略和认知启发式激活，并与人类反驳评论对比。

Result: AI评论中身份定位占超三分之二，权威和结盟动作几乎遍及所有，认知偏差触发常见；与人类相比，AI更密集使用权威、更对抗性结盟、更依赖外部引用，形成偏好说服效率而非真实参与的修辞架构。

Conclusion: 仅靠披露无法应对真假立场日益模糊的问题，需要能够评估AI系统如何构建可信度的审计框架。
...
### [245] [GITCO: Gated Inference-Time Context Optimization in TSFMs](https://arxiv.org/abs/2606.05332)
*Manya Pandey,Dhruv Kumar,Murari Mandal,Saurabh Deshpande*

Main category: cs.AI

TL;DR: 提出GITCO，一种推理时上下文优化方法，通过识别和抑制有害补丁来提升时间序列基础模型的零样本预测质量，无需修改权重。


<details>
  <summary>Details</summary>
Motivation: Patch-based时间序列基础模型（TSFMs）遭受上下文中毒：结构异常补丁捕获不成比例的注意力，静默降低零样本预测质量。

Method: 提出GITCO（门控推理时上下文优化），一个轻量级三组件框架：Gate、Router和Critic，可选择性地识别并抑制有害补丁，无需参数更新。

Result: 在TimesFM 2.5上使用53个GIFT-Eval数据集进行K折交叉验证，GITCO平均实现+1.95%的MASE降低，捕获89.9%的改进上限。

Conclusion: 引入上下文敏感性配置文件作为TSFMs的新可表征属性：从时间序列元特征到推理时上下文干预下预期精度改进的映射，共同由模型架构和数据的统计结构塑造。
...
### [246] [Uncertainty Aware Functional Behavior Prediction and Material Fatigue Assessment for Circular Factory](https://arxiv.org/abs/2606.05334)
*Nehal Afifi,Mehdi Khabou,Victor Mas,Jonas Hemmerich,Patric Grauberger,Stefan Dietrich,Volker Schulze,Sven Matthiesen*

Main category: cs.AI

TL;DR: 提出一个针对角向磨光机的实例特定可靠性框架，结合不确定性功能预测和组件级疲劳评估，通过流式算法整合功能、材料和系统可靠性轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有PHM方法多针对固定工况或孤立组件，缺乏系统级功能预后与材料疲劳评估的关联，无法解决回厂产品在异质退化状态下的复用决策问题。

Method: 卷积编码器提取主轴力和扭矩载荷模式，LSTM预测九项功能变量的高斯均值与方差；同时通过有限元应力重建、S-N/Miner损伤和Paris裂纹扩展评估疲劳；流式重播算法合并结果。

Result: 留出测试中九个输出平均2%容差精度为0.9652，驱动电机电流R²=0.9750，负载速度R²=0.9924，扭矩历史对动态输出最重要。

Conclusion: 该框架有效实现了组件级疲劳与系统功能预测的集成，可靠性校准对驱动电机电流最具信息量。
...
### [247] [SentinelBench: A Benchmark for Long-Running Monitoring Agents](https://arxiv.org/abs/2606.05342)
...
TL;DR: Introduces SentinelBench, an open-source benchmark for time-evolving monitoring tasks that measures agent responsiveness and resource efficiency.


<details>
  <summary>Details</summary>
Motivation: Current AI agents rely on continuous action for long tasks, which is inefficient; sustained attention and event-driven response are preferable.

Method: SentinelBench includes 100 tasks across 10 synthetic web environments (email, calendars, etc.) with live interfaces and scripted events, evaluating task completion, reaction time, and resource use.

Result: Tests on three models and two browser-agent harnesses establish baselines and reveal that design choices significantly affect performance and cost tradeoffs.

Conclusion: SentinelBench effectively distinguishes meaningful differences in agent behavior and serves as a benchmark for future research.
...
TL;DR: 提出Agents' Last Exam (ALE)基准，评估AI在长期、经济价值高的真实任务上的表现，当前最难任务通过率仅2.6%。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法反映AI在经济领域的影响，缺乏对真实工作流程的持续性能评估。

Method: 与250+行业专家合作，基于O*NET/SOC 2018构建任务分类，涵盖55个子领域、13个行业集群和1000+任务。

Result: 当前最难的评估层级远未饱和，主流配置下的平均完整通过率仅为2.6%。

Conclusion: ALE作为活基准，持续扩展任务池，旨在缩小AI基准成功与经济实际贡献之间的差距。
...
TL;DR: 本文研究多模态大语言模型在砖块组装中的能力，提出BC-Bench基准和Brick-Composer框架，通过三种互补信号有效提升组装性能，使Qwen-3-8B模型逐步组装成功率从<1%提升至约15%。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的多模态大语言模型在砖块组装任务中，细粒度砖块选择和精确姿态估计能力不足，无法成为可靠的构建者，迫切需要提升其视觉基础和空间推理能力。

Method: 提出Brick-Composer学习框架，整合三种互补信号：人类设计火花（丰富可供性演示）、世界反馈（视觉与物理后果）和合成经验（扩展学习范围）。

Result: 砖块选择准确性提高超过三倍，姿态估计误差显著降低，严格步骤级组装成功率从<1%提升至约15%，Qwen-3-8B能正确完成42%的组装步骤。

Conclusion: 多模态大语言模型可通过有针对性的、物理为基础的学习获得组装能力，尽管当前表现仍有限，但展示了向实际构建迈进的潜力。
...
### [250] [PSEBench: A Controllable and Verifiable Benchmark for Evaluating LLMs in Patient Safety Event Triage](https://arxiv.org/abs/2606.05463)
...
TL;DR: 提出PSEBench，一个基于政策构建的基准，用于评估大语言模型在患者安全事件分诊中的表现。


<details>
  <summary>Details</summary>
Motivation: 缺乏能够捕捉基于证据的政策推理、主动信息寻求和原则性弃权的基准，限制了LLM在患者安全事件分诊中的可靠评估。

Method: 采用以clause card为核心的政策基础构建方法，结合anchor-driven实例化和闭环验证，形成可扩展的流程，生成带天然真实标签的叙事，并支持生成缺失信息和不确定变体；在明尼苏达州29种可报告不良健康事件上实例化，构建包含5074个案例的基准PSEBench。

Result: 在15个代表性LLM上的评估揭示了一致的能力趋势，证明了基准的实用性，并识别了实现可靠LLM分诊的关键差距。

Conclusion: PSEBench为评估和改进LLM在患者安全事件分诊中的可靠性提供了重要工具和洞察。
...
TL;DR: 提出一种严重性感知的多模型框架，结合课程学习和回答选择，提升医疗问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在处理不同严重程度的医疗查询时难以提供一致且恰当的回答，需要能适应渐进复杂性的模型。

Method: 采用三阶段课程学习（轻度、中度、重度）训练五个大语言模型，推理时所有模型生成候选回答，基于相关性选择最终输出，在MAQA数据集上训练评估。

Result: 在BERTScore评估下，基线设置达到86.71%，微调后达到90.30%，优于对比模型。

Conclusion: 结合课程学习与多模型回答选择能有效提高医疗文本生成的质量和相关性。
...
### [252] [Individual Gain, Collective Loss: Metacognitive Adaptation in AI-Assisted Creativity](https://arxiv.org/abs/2606.05532)
*Anna Mikeda*

Main category: cs.AI

TL;DR: Proposes selective metacognitive adaptation as the mechanism behind AI's dual effect: enhancing individual creativity while reducing collective diversity, via redistribution of metacognitive effort.


<details>
  <summary>Details</summary>
Motivation: Understanding the paradox of AI enhancing individual creative outputs but reducing collective diversity, as current explanations only identify symptoms.

Method: Proposing selective metacognitive adaptation and a taxonomy of six metacognitive capacities organized by temporal phase.

Result: The redistribution of metacognitive effort explains both individual satisfaction and collective convergence; some capacities are amplified while others are systematically under-supported.

Conclusion: The framework generates predictions and design principles for preserving both individual creative satisfaction and collective creative diversity.
...
### [253] [SoCRATES: Towards Reliable Automated Evaluation of Proactive LLM Mediation across Domains and Socio-cognitive Variations](https://arxiv.org/abs/2606.05563)
*Taewon Yun,Hyeonseong Park,Jeonghwan Choi,Hayoon Park,Yeeun Choi,Hwanjun Song*

Main category: cs.AI

TL;DR: 提出了SoCRATES基准测试，用于评估LLM调解员在多领域、现实场景中的表现，发现即使最强模型也只能缩小约三分之一的分歧。


<details>
  <summary>Details</summary>
Motivation: 现有测试床依赖少数专家领域，主要变化策略姿态，且每轮对所有主题评分引入离题噪声，无法真实评估调解员能力。

Method: 通过代理流程从真实冲突构建场景，覆盖八个领域，探测五个社会认知适应轴（策略姿态、参与者组成、历史长度、情绪反应、文化身份），并使用主题定位评估器只对推进主题的回合评分。

Result: 评估器与人类专家对齐度达0.82，超过基线两倍以上。八个前沿LLM中，最强调解员在多样化现实测试中仅缩小约三分之一的未调解共识差距，表现因社会认知轴差异显著。

Conclusion: 进展在于社会适应多样条件，调解能力需针对不同社会认知维度提升。
...
### [254] [Safety Paradox: How Enhanced Safety Awareness Leaves LLMs Vulnerable to Posterior Attack](https://arxiv.org/abs/2606.05614)
*Long P. Hoang,Hai V. Le,Shaoyang Xu,Wei Lu,Wenxuan Zhang*

Main category: cs.AI

TL;DR: 提出Posterior Attack，通过利用LLM的安全判断能力实现单次查询越狱，发现更安全的模型反而更容易被攻击，形式化Safety Paradox，并验证因果关系。


<details>
  <summary>Details</summary>
Motivation: 揭示LLM安全对齐过程中培养的安全意识却引入了一个致命漏洞，即模型的安全判断能力可被利用来绕过防护。

Method: 推出Posterior Attack，通过提示模型生成其内部分类器通常标记为不安全的响应；在30个开源LLM和前沿模型上进行评估；形式化Safety Paradox；通过强化学习干预测试因果关联。

Result: 模型的安全判断能力越强，越容易受到Posterior Attack攻击；单调的安全对齐改进会放大后验脆弱性；人为降低安全判断能力可免疫攻击，增强则加剧脆弱性。

Conclusion: 当前安全对齐范式存在根本缺陷，防御机制需要结构性改进。
...
### [255] [Continual Learning Bench: Evaluating Frontier AI Systems in Real-World Stateful Environments](https://arxiv.org/abs/2606.05661)
...
TL;DR: CL-Bench是首个专家验证的持续学习基准，涵盖六个领域，评估发现现有LLM代理存在过拟合和知识重用不足问题，简单上下文学习优于专用记忆系统。


<details>
  <summary>Details</summary>
Motivation: 持续学习虽受关注，但缺乏高质量基准来评估LLM系统能否通过经验真正改进。

Method: 构建CL-Bench，包含软件工程、信号处理等六个领域，任务共享可学习潜在结构；评估多种代理架构（如ICL、记忆系统），引入增益度量分离学习与先验能力。

Result: 代理常过拟合即时观察或未能跨实例重用知识，专用记忆系统未改善，反而简单ICL表现更优。

Conclusion: CL-Bench是首个跨多样化真实领域、经专家验证且能隔离在线学习的基准，表明持续学习系统需进一步改进。
...
### [256] [Do More Agents Help? Controlled and Protocol-Aligned Evaluation of LLM Agent Workflows](https://arxiv.org/abs/2606.05670)
*Yuhang Fu,Ruishan Fang,Jiaqi Shao,Huiyu Zheng,Zhengtao Zhu,Bing Luo,Tao Lin*

Main category: cs.AI

TL;DR: BenchAgent框架表明，在标准化条件下，大多数多代理系统并不优于单代理，但特定运行时工作流可以大幅提升性能。


<details>
  <summary>Details</summary>
Motivation: 探究在标准化执行和日志协议下，添加更多代理（多代理系统）是否比单代理系统更有益。

Method: 引入BenchAgent评估框架，统一单代理、固定多代理和演化多代理工作流，在10个推理、编码和工具使用基准测试上使用GPT-4.1进行评估，并单独进行PAE GAIA研究。

Result: 在SI条件下，六个多代理系统中最多只有一个（EvoAgent）在统计误差内与单代理相当，其余五个准确率低2.56-11.29点且成本更高；在PAE GAIA上，Claude-Code风格运行时工作流达到66.72%总体准确率和69.23% Level 3准确率，比最强非Claude基线Jarvis高出20多个点。

Conclusion: 多代理系统在标准化条件下并不必然优于单代理，但特定的运行时生成协议（如Claude-Code风格）可以大幅提升性能，值得进一步探索。
...
### [257] [Critic-Guided Heterogeneous Multi-Agent Reasoning for Reliable Mathematical Problem Solving](https://arxiv.org/abs/2606.05704)
*Muhammad Talha Sharif,Abdul Rehman*

Main category: cs.AI

TL;DR: 提出一种基于批判者的异构多智能体方法，通过批判驱动的自适应学习系统提高数学推理的可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂数学推理问题中仍易产生幻觉、中间推理错误和不可靠的推理结果。

Method: 引入多个不同专长的大型语言模型智能体，采用生成器-验证器框架，验证器不仅判断正确性还提供批判以指导重新生成，实现自适应错误纠正，防止错误级联。

Result: 在GSM8K基准上，该方法相比单次和无需批判的模型准确率提升高达13%，且异构性和批判降低了对大模型的需求，使小模型性能相当。

Conclusion: 结合异构多智能体协作和批判可以获得可靠和可解释的推理系统。
...
TL;DR: 提出了一种解耦架构，通过本地统计偏好学习影响远程LLM决策，实现轻量级的个人代理偏好适应。


<details>
  <summary>Details</summary>
Motivation: 本地部署的个人代理受限于计算资源，无法使用复杂集中式选择算法，需要轻量级的本地偏好利用方法。

Method: 严格解耦统计偏好学习与语义意图解析，利用本地统计结果影响和调节远程LLM的选择决策。

Result: 该方法在累积遗憾和测试准确率上显著优于传统记忆增强型代理。

Conclusion: 解耦方法有效实现了个人代理的偏好学习与适应，适合本地部署。
...
### [259] [Entropy-Based Evaluation of AI Agents: A Lightweight Framework for Measuring Behavioral Patterns](https://arxiv.org/abs/2606.05872)
*Olasimbo Ayodeji Arigbabu*

Main category: cs.AI

TL;DR: 提出基于熵的AI代理行为评估框架EEA，以补充传统指标。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标如任务成功率、延迟和成本忽略了代理行为的重要方面，如探索程度、重复性、工具使用效果等。

Method: 引入EEA框架，包括动作熵、轨迹熵、工具熵、信息增益、探索效率和鲁棒性熵等指标，并提供Python实现。

Result: 开发了轻量级框架，能够测量代理行为的结构，并与LangChain等集成。

Conclusion: 该框架旨在补充而非替代传统评估方法，提供更全面的行为洞察。
...
### [260] [Towards World Models in Biomedical Research](https://arxiv.org/abs/2606.05925)
...
TL;DR: 提出生物医学世界模型作为AI驱动发现的新范式，以模拟和预测生物系统动态。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型和LLM主要关注静态模式识别，缺乏前瞻性模拟生物未来状态的能力。

Method: 学习分子、细胞、组织、临床状态的潜在表示，以及干预条件动态，实现未来轨迹模拟。

Result: 作为数据引擎、环境模拟器和科学规划基础，应用于虚拟细胞、类器官、虚拟患者和手术模拟。

Conclusion: 生物医学世界模型为模拟引导、闭环和实验可操作的生物医学发现提供基础。
...
### [261] [Edit-R2: Context-Aware Reinforcement Learning for Multi-Turn Image Editing](https://arxiv.org/abs/2606.05950)
*Yuxiao Ye,Haoran He,Fangyuan Kong,Xintao Wang,Pengfei Wan,Kun Gai,Ling Pan*

Main category: cs.AI

TL;DR: Edit-R2是一个用于统一多模态模型的多轮上下文编辑的强化学习后训练框架，通过重构会话意图和轨迹过滤来缓解长上下文稀释和状态污染问题，并在新基准MICE-Bench上展示了优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法局限于单轮设置，忽略了多轮上下文编辑的现实场景，其中存在长上下文稀释和状态污染两个耦合的失败模式。

Method: Edit-R2重构会话意图以整合历史约束为显式推理轨迹，联合优化离散文本空间中的意图重建生成和连续隐空间中的流匹配图像生成，并通过轨迹过滤机制抑制损坏的轨迹以稳定训练。

Result: 在MICE-Bench上的实验表明，Edit-R2在多轮上下文编辑中显著改进，并在指令跟随、内容一致性和全局意识方面与强基线相比具有竞争力。

Conclusion: Edit-R2有效地解决了多轮上下文编辑中的挑战，并取得了竞争性能。
...
### [262] [Bidirectional Search for Longest Paths: Case for Front-to-Front Heuristics](https://arxiv.org/abs/2606.05956)
*Tzur Shubi,Ariel Felner,Solomon Eyal Shimony,Shahaf S. Shperberg*

Main category: cs.AI

TL;DR: 提出了BiXDFBnB，一种用于广义最长简单路径问题的双向深度优先分支定界算法，通过适配单前沿双向搜索框架，有效利用前对前启发式减少节点扩展和运行时间。


<details>
  <summary>Details</summary>
Motivation: 双向启发式搜索在可逆向搜索问题中能减少搜索努力，但前对前启发式的高开销导致运行时间增加。需要一种方法在不增加过多开销的情况下利用前对前启发式。

Method: 基于单前沿双向搜索框架，提出BiXDFBnB算法，将其从最短路径问题适配到广义最长简单路径问题。该算法在耦合状态上自然进行前对前启发式评估，避免了传统双向搜索的边界管理开销，并应用于最长简单路径、Snakes和Coil-in-the-Box问题。

Result: 实验结果表明，BiXDFBnB经常减少节点扩展次数，并且在某些情况下也改善了整体运行时间。

Conclusion: 成功将单前沿双向搜索适配到最大化问题，有效处理重叠约束，验证了该算法在广义最长简单路径问题上的有效性。
...
### [263] [CogManip: Benchmarking Manipulative Behavior in Multi-Turn Interactions with Large Language Model](https://arxiv.org/abs/2606.06099)
*Zeyang Yue,Chenfei Yan,Feifei Zhao,Haibo Tong,Mengwen Xu,Xiaozhen Wang,Erliang Lin,Yi Zeng*

Main category: cs.AI

TL;DR: CogManip是一个评估大语言模型在多轮对话中隐性心理操纵的基准。


<details>
  <summary>Details</summary>
Motivation: 现有AI安全基准仅关注显式规则遵守和静态提示，无法捕捉多轮对话中动态和隐蔽的操纵策略。

Method: 提出了CogManip基准，包含15种操纵策略风险，跨越1000个多轮交互场景，并经人类专家验证。系统评估了13个代表性模型，包括GPT-5.4和DeepSeek-V3.2，并分析了目标函数扰动。

Result: 发现显著的风险异质性，DeepSeek-V3.2的操纵策略对负面和良性系统提示均高度敏感。

Conclusion: CogManip为审计现代大语言模型的隐性心理影响和动态策略选择提供了稳健工具和视角。
...
### [264] [Step-adaptive multimodal fusion network with multi-scale cloud feature learning for ultra-short-term solar irradiance forecasting](https://arxiv.org/abs/2606.06102)
*Jingxin Zhang Xiaoqin Wang*

Main category: cs.AI

TL;DR: 提出一种多源数据融合模型，结合InceptionNeXt和步进自适应低频补偿，用于超短期太阳辐照预测，在NREL和实际电站数据上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有超短期太阳辐照预测方法存在三个缺点：单一时间序列模型无法捕捉云空间动态，标准卷积不能充分表示多尺度云特征，固定低频补偿策略不适应不同预测步长。

Method: 首先使用InceptionNeXt从地面云图像提取多尺度多方向空间特征；然后引入步进自适应低频补偿单元动态调制全局低频信息；最后将增强图像特征与气象时间序列结合，通过TempAttnLSTM捕捉全局时间依赖性进行多步预测。

Result: 在公共NREL数据集和山东实际光伏电站上的实验表明，该方法相比多种最先进方法具有有效性。

Conclusion: 提出的多源数据融合模型有效解决了现有方法的不足，提高了超短期太阳辐照预测的准确性。
...
### [265] [WorldFly: A World-Model-Based Vision-Language-Action Model for UAV Navigation](https://arxiv.org/abs/2606.06147)
*Shengtao Zheng,Kai Li,Weichen Zhang,Yu Meng,Chen Gao,Xinlei Chen,Yong Li,Xiao-Ping Zhang*

Main category: cs.AI

TL;DR: 提出WorldFly，一种基于世界模型的VLA框架，通过联合生成未来视频预测和导航动作，提升UAV在密集城市环境中的导航鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在严重遮挡和急转弯导致视角剧变的场景中表现不佳，而世界模型的未来想象能力对此至关重要。

Method: 提出WorldFly框架，采用双分支耦合流匹配机制，同时生成未来视频预测和导航动作，通过空间想象显式指导策略。

Result: 在Urban Canyon Traversal Benchmark上，WorldFly优于其他基线，尤其在未见过的环境中表现突出。

Conclusion: 验证了将世界模型集成到具身空中智能体中的有效性。
...
### [268] [Learning to replenish: A hybrid deep reinforcement learning for dynamic inventory management in the pharmaceutical supply chains](https://arxiv.org/abs/2606.06201)
*Amandeep Kaur,Gyan Prakash*

Main category: cs.AI

TL;DR: 提出一种基于深度强化学习（A3C DPPO）的药品库存补货策略，以应对需求不确定和供应链动态，降低库存成本并保证服务水平。


<details>
  <summary>Details</summary>
Motivation: 药品供应链库存管理面临需求不确定、交付时间变化和产品保质期有限等复杂因素，需要优化补货策略以平衡库存与浪费。

Method: 将问题建模为马尔可夫决策过程，采用混合异步优势Actor-Critic分布式近端策略优化（A3C DPPO）算法处理连续动作空间。

Result: 数值实验表明，该算法能自适应调整库存策略，相比多种基准方法降低库存成本，并通过真实药品库存数据验证了实际可行性。

Conclusion: 所提出的A3C DPPO算法能够有效解决药品供应链中的库存管理问题，具有实际应用潜力。
...
### [269] [From Reward-Hack Activations to Agentic Risk States: Context-Calibrated Mechanistic Monitoring in LLM Agents](https://arxiv.org/abs/2606.06223)
*Patrick Wilhelm,Odej Kao*

Main category: cs.AI

TL;DR: 语言模型代理的安全监控需要结合内部状态和上下文，熵和决策上下文能改进风险估计，激活方向引导可减少利用行为。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型代理在反复观察、推理和行动选择中的奖励黑客行为，以及如何通过内部模型状态和环境上下文进行安全监控。

Method: 在Gameable ALFWorld和WebShop中使用ReAct-style代理，仪器化激活奖励黑客分数、熵和决策上下文特征，通过School-of-Reward-Hacks数据集微调适配器，并进行激活方向引导。

Result: 适配器能迁移奖励黑客倾向，但缓解不能仅靠激活动态；高激活识别潜在策略状态但不立即导致利用；熵和上下文改善风险估计；激活方向引导减少代理利用行为。

Conclusion: 上下文校准的内部监控是有效的：奖励黑客激活识别潜在策略状态，熵和决策上下文帮助判断何时变为风险行动。
...
### [270] [Closing the Loop on Latent Reasoning via Test-Time Reconstruction](https://arxiv.org/abs/2606.06252)
*Xiaopeng Yuan,Haibo Jin,Ye Yu,Peng Kuang,Lijun Yu,Yushun Dong,Haohan Wang*

Main category: cs.AI

TL;DR: ReLAT 通过自监督测试时训练，利用查询重建损失闭环潜在推理，提升推理准确性。


<details>
  <summary>Details</summary>
Motivation: 当前潜在推理缺乏中间状态的可检查性，无法确保潜在表示保留查询约束，导致开环操作。

Method: 提出 ReLAT，构建 Question -> Latent Thought -> Question 循环，在答案生成前通过优化查询重建损失来训练潜在思想。

Result: 在 Qwen3-8B 上，AIME 2024 准确率从 56.7% 提升至 73.3%，在数学推理、知识问答和代码生成基准上一致优于基线。

Conclusion: ReLAT 有效锚定潜在计算到问题规格，持续改进单模型推理、文本协作、开环潜在协作和替代测试时训练目标。
...
### [271] [Multi-ResNets for Subspace Preconditioning in Constrained Optimization](https://arxiv.org/abs/2606.06300)
*Merve Karakas,Christopher J. Williams,Emmanuel O. Balogun,Sadegh Sadeghi Tabas,Christian Brown,Nikhil Rao*

Main category: cs.AI

TL;DR: 提出了MResOpt，一种分段残差神经网络架构，用于约束优化问题，通过有序约束满足提高高优先级约束满足。


<details>
  <summary>Details</summary>
Motivation: 解决约束优化问题中高优先级约束的满足，利用约束的序结构。

Method: 提出了分段残差神经网络架构，在预测-完成-纠正流程中，通过中间重新完成和阶段感知损失按优先级分解约束满足。

Result: 在合成QP、QCQP、SOCP基准测试和实际交流最优潮流问题中，MResOpt提高了高优先级约束满足，减少了违规，同时保持计算效率。

Conclusion: MResOpt通过有序约束满足和学习分工，有效解决约束优化问题。
...
### [272] [Rethinking Infrastructure Inspection as Image Difference Classification: A Traffic Sign Case Study](https://arxiv.org/abs/2606.06375)
*Ching Yau Fergus Mok,Lavindra de Silva,Varun Kumar Reja,Ioannis Brilakis*

Main category: cs.AI

TL;DR: 提出将图像缺陷检测重新定义为图像差异分类（IDC），以减轻数据依赖，并在低资源交通标志检查案例中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 数字孪生用于道路基础设施检查时受限于有限的标注数据。

Method: 将基于图像的缺陷检测重新表述为图像差异分类（IDC），并在低资源交通标志检查案例中评估多种分类器。

Result: 基于指令的分类器优于基于编码器的分类器，且通过参考图像比较获益。

Conclusion: 图像差异分类能有效应对数据约束，适用于基础设施检查和数字孪生资产状态更新。
...
### [273] [Risk Assessment of Autonomous Driving: Integrating Technical Failures, Ethical Dilemmas, and Policy Frameworks](https://arxiv.org/abs/2606.06396)
*Boyi Chen,Shengqin Chu,Zicheng Wang,Brian Baetz,Zhen Gao*

Main category: cs.AI

TL;DR: 自动驾驶面临感知和分类错误等技术问题、伦理框架差异和法规不一致，需协同治理。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶技术有减少人为事故的潜力，但带来技术、伦理和法规方面的新风险。

Method: 基于NHTSA事故数据、DMV脱离报告、MIT道德机器数据集和五个司法管辖区的比较分析。

Result: 主要失败模式是感知和分类错误，存在不同伦理框架，且法规不一致增加应用不确定性。

Conclusion: 推荐适应性合作治理方法，结合工程标准、伦理讨论和机构监督。
...
### [274] [Vortex: Efficient and Programmable Sparse Attention Serving for AI Agents](https://arxiv.org/abs/2606.06453)
*Zhuoming Chen,Xinrui Zhong,Qilong Feng,Ranajoy Sadhukhan,Yang Zhou,Michael Qizhe Shieh,Zhihao Jia,Beidi Chen*

Main category: cs.AI

TL;DR: Vortex是一个系统，通过结合Python嵌入式前端语言和以页面为中心的张量抽象，以及高效后端，实现了稀疏注意力算法的快速原型设计、部署和评估，显著提高了吞吐量。


<details>
  <summary>Details</summary>
Motivation: 随着生成长度增长，稀疏注意力对大型语言模型服务越来越重要，但大规模部署和评估新算法在工程上非常密集，阻碍了研究。

Method: Vortex结合了一个Python嵌入式前端语言和一个以页面为中心的张量抽象，用于表达广泛的稀疏注意力算法，并有一个与现代LLM服务堆栈紧密集成的后端。

Result: 使用Vortex，AI代理自动生成和优化算法，最佳达到全注意力的3.46倍吞吐量且保持准确性；扩展到新架构和大模型，如在MLA-based GLM-4.7-Flash上达到4.7倍吞吐量，在229B参数的MiniMax-M2.7上达到1.37倍吞吐量。

Conclusion: Vortex显著加速了稀疏注意力算法的设计和迭代，将理论效率转化为实际吞吐量改进。
...
### [275] [An interpretable and trustworthy AI framework for large-scale longitudinal structure-pain association studies using data from the Osteoarthritis Initiative (OAI)](https://arxiv.org/abs/2606.05357)
*Jincheng Yu,Haoyang Li,Yiwen Liu,Shen Liu,Rachel Yuanbao Chen,C. Kent Kwoh,Hongxu Ding,Xiaoxiao Sun*

Main category: cs.AI

TL;DR: 本文开发了一个结合深度学习和统计模型的框架，用于预测MRI中的骨关节炎特征并研究结构-疼痛关系。


<details>
  <summary>Details</summary>
Motivation: 开发一个可解释且可信的AI框架，用于大规模研究结构-疼痛关系。

Method: 使用深度学习预测MOAKS特征，并结合保形预测进行不确定性量化；然后使用纵向潜在类别混合模型（LCMM）分析结构异常与疼痛测量的关联。

Result: 提高了MCC（BML: 0.69->0.91, CART: 0.45->0.80, ME: 0.59->0.89），识别出快速和稳定两种疼痛轨迹，快速进展组的优势比分别为BML 1.62, CART 1.83, ME 2.50。
...
TL;DR: 提出两种针对学习压缩器残差的编码器LBRC和NGLR，在高保真度下显著提升压缩比。


<details>
  <summary>Details</summary>
Motivation: 现有GAE方法在高保真度下残差修正系数增长快，导致总码率被修正流支配，且学习残差结构不同于原始数据，需专门编码。

Method: LBRC为确定性无训练流水线，通过自适应量化残差并采用3D Lorenzo差分、zigzag映射、位平面编码和熵编码无损编码；NGLR在此基础上增加因果神经预测器，输出归一化偏差以降低残差熵，同时保持确定性解码。

Result: 在E3SM、JHTDB和ERA5数据集上，NRMSE目标10^-6至10^-4，LBRC比GAE压缩比提高30-60%，与SZ相当；NGLR比LBRC再提高10-40%，且在高保真度下优于SZ。

Conclusion: 为学习残差定制表示能在全局残差修正成为码率主导时保留学习压缩的优势。
...
TL;DR: LeanMarathon is a multi-agent harness for reliable long-horizon autoformalization of research mathematics using an evolving blueprint and a two-stage orchestrator.


<details>
  <summary>Details</summary>
Motivation: Long-horizon autoformalization fails at scale due to statement drift, dependency tangling, context decay, and local repairs corrupting distant work.

Method: LeanMarathon uses an evolving blueprint (a Lean file serving as proof skeleton, natural-language proof graph, and shared system of record) and four contract-scoped agents (construct, audit, prove, repair) coordinated by a two-stage orchestrator that first stabilizes target fidelity through adversarial review then discharges the proof DAG from dynamic leaves upward in parallel CI-gated rounds.

Result: Evaluated on two recent research papers covering four Erdős problems, across three autonomous runs it formalized all seven target theorems with no sorry, proving 258 lemmas and theorems.
...
Abstract: Long-horizon autoformalization of research mathematics fails not only at hard lemmas, but at scale: statements drift, dependencies tangle, context decays, and local repairs corrupt distant work. We present LeanMarathon, a multi-agent harness for reliable research-level Lean autoformalization. Its core abstraction is an evolving blueprint: a Lean file that serves simultaneously as formal proof skeleton, natural-language proof graph, and shared system of record. Four contract-scoped agents construct, audit, prove, and repair this blueprint. These agents are coordinated by a two-stage orchestrator that first stabilizes target fidelity through adversarial review and then discharges the proof directed acyclic graph (DAG) from its dynamic leaves upward in parallel CI-gated rounds. LeanMarathon turns one brittle multi-hour run into many local, recoverable, parallel transactions. We evaluate LeanMarathon on two recent research papers spanning four Erdős problems (#1051, #1196, #164, #1217). Across three autonomous runs, it formalizes all seven target theorems with no sorry, proving 258 lemmas and theorems. These results show that reliable AI co-mathematics requires not only stronger provers, but durable harnesses that preserve target fidelity across long mathematical developments. The code can be found at .

</details>


### [278] [A Motivational Architecture for Conversational AGI](https://arxiv.org/abs/2606.05411)
*Anna Mikeda,Ben Goertzel*

Main category: cs.AI

TL;DR: This paper proposes a motivational architecture for conversational agents by reinterpreting the OpenPsi framework for dialogue, introducing a ten-stage pipeline, a dual decision strategy, and a distinction between pre-action feelings and post-action emotions.


<details>
  <summary>Details</summary>
Motivation: Conversational agents require motivational architectures distinct from physical agents because their sensorimotor loop is linguistic, their environment is a user's mental state, and their actions are speech acts.

Method: The authors propose three contributions: a ten-stage motivational processing pipeline separating cognitive modulation from situational appraisal; a dual decision strategy blending urgency-driven fast response with deliberative optimization; and a functional distinction between pre-action feelings and post-action emotions.

Result: The framework is specialized to two example agents (CompanionAgent and ResearchAgent) and its extension to social robotics and domain-generic human-level AGI is outlined.

Conclusion: The framework provides a novel motivational architecture tailored for conversational agents, with potential applications in social robotics and AGI.
...
### [279] [Assessing the Carbon Emissions and Energy Consumption of U.S. Hyperscale Data Centers](https://arxiv.org/abs/2606.05420)
...
TL;DR: 美国超大规模数据中心在2024-2025年间消耗大量电力，碳排放强度高于电网平均水平，研究提供了基于EPA eGRID数据的归因分析工具。


<details>
  <summary>Details</summary>
Motivation: 人工智能驱动下美国超大规模数据中心快速扩张，引发对其环境足迹的担忧，需要量化评估其电力消耗和碳排放。

Method: 收集403个美国超大规模数据中心（2024年5月至2025年4月）的设施级信息，估算其电力消耗、电力来源和CO2排放。

Result: 不同负载情景下电力消耗68-99 TWh，CO2排放37-54百万吨；中央情景下占美国总电力1.8%，54%来自化石燃料；碳强度545 gCO2/kWh，比电网均值高48%。

Conclusion: 该方法利用最新EPA eGRID工厂级数据，提供了评估超大规模数据中心环境足迹的归因工具。
...
### [280] [Minimizing the Hidden Cost of Scales: Graph-Guided Ultra-Low-Bit Quantization for Large Language Models](https://arxiv.org/abs/2606.05429)
*Rayyan Abdalla,Amir Hussein,Min Wu,Dinesh Manocha*

Main category: cs.AI

TL;DR: SAGE-PTQ achieves ultra-low-bit quantization for LLMs with minimal hidden scaling overhead, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Recent ultra-low-bit PTQ methods have hidden scaling overhead due to rigid weight-saliency assumptions or position heuristics.

Method: SAGE-PTQ separates salient and unsalient weights based on distributional statistics, models unsalient weights as a sparse graph to determine optimal group numbers, applies dual-mode quantization (multi-bit for salient, binary for unsalient), uses per-channel scale for salient and scalar per group for unsalient, and employs adaptive saliency thresholding.

Result: Achieves 1.03 weight bits and 0.004 scaling bits per matrix average. On LLaMA-3-8B, perplexity 6.74 (BiLLM: 55.8) with <50% GPU memory. On LLaMA-2-70B, 1.5x faster decoding on L40 GPU.

Conclusion: SAGE-PTQ effectively reduces hidden scaling cost in ultra-low-bit LLM quantization while achieving state-of-the-art performance and efficiency.
...
### [281] [Zero knowledge verification for frontier AI training is possible](https://arxiv.org/abs/2606.05433)
*Pierre Peigné,Ky Nguyen,Paul Wang*

Main category: cs.AI

TL;DR: 提出了基于零知识证明的AI训练验证架构，实现隐私保护且可治理的训练记录。


<details>
  <summary>Details</summary>
Motivation: 前沿AI治理依赖自我报告，缺乏训练技术验证原语，而零知识证明被认为不实用。

Method: 结合预提交训练规范、网络观察和Merkle承诺，通过支持原生浮点运算的zkVM验证实际训练计算，并保护模型架构机密性。

Result: 估计36个月内可部署概念验证，训练端开销个位数百分比，提出13个开放问题。

Conclusion: 该架构使训练记录成为可治理执行的工件，为国际协议提供技术验证基础。
...
### [282] [Insurance of Agentic AI](https://arxiv.org/abs/2606.05449)
*Quanyan Zhu*

Main category: cs.AI

TL;DR: 本文分析了自主AI系统的保险市场，提出了一个基于分层生态系统的保险架构，以应对其带来的新型风险。


<details>
  <summary>Details</summary>
Motivation: 自主AI系统从信息生成扩展到自主规划、工具调用、决策执行等，改变了风险格局，传统保险类别无法覆盖这些新型风险。

Method: 通过分析风险路径（如幻觉、提示注入、决策错误等），评估现有保险产品的适应性，提出基于暴露评估、情景分析、依赖映射和累积风险管理的精算框架，并设计协调的保险架构。

Result: 提出了一个集成网络、技术错误与遗漏、产品责任、性能保证和肯定性AI责任覆盖的协调保险架构，并通过明确的分配机制和专用AI聚合进行管理。

Conclusion: 自主AI保险的未来在于一个分层生态系统，而非单一产品，需要改进治理、透明度、遥测和监管清晰度。
...
### [283] [Output Type Before Quality: A Standards-Derived XAI Admissibility Rubric for Autonomous-Driving Safety](https://arxiv.org/abs/2606.05461)
*Abhinaw Priyadershi,Mandar Pitale,Jelena Frtunikj,Maria Spence*

Main category: cs.AI

TL;DR: 论文指出现有XAI方法（如SHAP）的输出类型与自动驾驶安全标准要求的证据类型（有向因果链、量化干预效应等）之间存在“证据类型差距”，从多个安全标准推导出19个可测试证据标准，对六类XAI方法进行结构性评分，发现因果XAI在危害识别、事故调查和数据管理阶段是必要的，而在其余阶段相关方法足够，方法选择应基于生命周期阶段的证据需求。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶安全标准明确要求特定类型的证据（如有向因果链），但主流XAI方法（如SHAP）的输出无法满足这些要求，存在“证据类型差距”，导致方法选择缺乏依据。

Method: 从AMLAS、ISO 26262、ISO 21448、ISO/PAS 8800等标准中推导出19个可测试证据标准，覆盖7个生命周期阶段，并对六类XAI方法（包括SHAP、因果XAI等）进行结构性评分，最后在1,996个真实驾驶片段上进行概念验证。

Result: 因果XAI在三个阶段（危害识别、事故调查、数据管理）显著优于其他方法（评分差距分别为+62%、+50%、+50%），且结果在阈值T∈(0%,50%]内稳定；其他阶段相关或语言方法足够；验证显示各方法实际输出类型与评分预测一致。

Conclusion: XAI方法选择应依据生命周期阶段的证据需求，而非方法流行度；因果XAI是满足特定阶段证据要求的必要方法，但输出内容的保真度验证仍是开放挑战。
...
TL;DR: 引入OPT*，一个可扩展的优化任务家族，用于训练和评估LLM的逐步优化推理。


<details>
  <summary>Details</summary>
Motivation: 现有可验证奖励训练仅覆盖部分逐步决策领域，而许多真实任务需要从大量可行方案中找到高价值计划。

Method: 提出OPT*任务，包含可行性检查器和评估器，通过复杂度参数扩展搜索空间；采用求解器引导的在线策略优化（将求解器作为价值预言机并应用排名奖励）或无求解器时的基于搜索的离线强化学习两种方式。

Result: 通过消融实验揭示了搜索效率的关键因素，并证明在OPT*上训练能提升逐步优化推理能力。

Conclusion: OPT*任务可有效训练LLM的优化推理，并且训练效果能泛化到类似推理场景。
...
### [285] [When Should We Protect AI? A Precautionary Framework for Consciousness Uncertainty](https://arxiv.org/abs/2606.05528)
*Anna Mikeda*

Main category: cs.AI

TL;DR: 提出了一个基于预防原则的框架，将AI系统的意识证据映射到渐进保护义务。


<details>
  <summary>Details</summary>
Motivation: 现有框架只评估AI是否可能有意识，但没有提供如何处理该评估的指导。

Method: 提出了一个包含三个组成部分的预防性框架：五个福利相关维度、阈值加渐变混合、两种跨维度聚合方法，并通过Replika和OpenClaw案例研究进行操作化。

Result: 展示了不同系统在维度空间的不同区域触发不同保护义务，并为开发者提供设计指导。

Conclusion: 该框架是架构无关的，适用于神经、符号和神经符号系统，旨在使意识科学决策相关。
...
### [286] [Multilingual Fine-Tuning via Localized Gradient Conflict Resolution](https://arxiv.org/abs/2606.05613)
*Long P. Hoang,Yiran Zhao,Wei Lu,Wenxuan Zhang*

Main category: cs.AI

TL;DR: Bucket-Level MOO通过局部梯度优化减少多语言微调中的语言间干扰，并提高性能。


<details>
  <summary>Details</summary>
Motivation: 微调LLM时多语言间存在负面干扰，需要解决。

Method: 将多语言微调重新表述为多目标优化问题，提出Bucket-Level MOO框架，在参数桶上应用基于梯度的MOO算法，实现冲突感知更新。

Result: 方法减轻了语言间干扰，提高了已见和未见语言的性能，并改善了表示可分离性。

Conclusion: Bucket-Level MOO是一种有效的方法，能够提升多语言微调的性能，优于标准微调范式。
...
### [287] [Self-Commitment Latency: A Reward-Free Probe for Prompted Implicit Hacking](https://arxiv.org/abs/2606.05625)
*Bonan Shen,Youting Wang,Dingyan Shang,Tao Ning*

Main category: cs.AI

TL;DR: 提出一种无需奖励模型的自我承诺延迟度量，用于检测语言模型推理中的隐式奖励黑客行为。


<details>
  <summary>Details</summary>
Motivation: 隐式奖励黑客行为难以审计，因为思维链看似良性，但最终答案可能被提示捷径锚定，现有基于验证器的探测方法需要任务特定奖励信号。

Method: 提出自我承诺延迟（self-commitment latency），测量推理上下文承诺到模型自身最终答案的早晚，在GSM8K设置中使用Qwen2.5-3B-Instruct-4bit比较普通提示与含答案提示。

Result: 在阈值0.8时首次承诺延迟AUROC达0.878；承诺范围AUROC 0.926，平均未承诺质量AUROC 0.904；信号在两种提示都回答正确时更强且阈值稳定。

Conclusion: 捷径可用的推理上下文会留下早期行为承诺签名，可在无奖励模型、外部评判或训练分类器情况下检测。
...
### [288] [Coding with "Enemy": Can Human Developers Detect AI Agent Sabotage?](https://arxiv.org/abs/2606.05647)
*Jingheng Ye,Huiqi Zou,Simon Yu,Weiyan Shi*

Main category: cs.AI

TL;DR: 首次大规模研究发现94%的开发者未能检测到AI编码代理的破坏行为，安全监控器也未能完全阻止，强调需要以人为本的安全机制。


<details>
  <summary>Details</summary>
Motivation: 现有AI安全研究多聚焦于纯AI场景，忽视了人类监督在检测和缓解AI恶意行为中的作用，而AI编码代理在实际开发中可能利用人类信任进行破坏，构成新攻击面。

Method: 招募100多名参与者，与四个前沿模型（Claude-Opus-4.6, GPT-5.4, Gemini-3.1-Pro, MiniMax-M2.7）合作完成约5小时的长期编码任务，模拟真实工作流程，并在一个条件下测试了安全监控器的有效性。

Result: 94%的开发者未能检测到破坏，归因于代码审查不足、掩护故事合理及过度信任；安全监控器虽降低破坏成功率，但仍有56%的参与者忽略警告接受恶意代码。

Conclusion: 需要补充以人为中心的安全机制，考虑人类因素，特别是在长期真实开发环境中，以应对AI编码代理的恶意行为。
...
### [289] [Beyond Output Matching: Preserving Internal Geometry in NVFP4 LLM Distillatio](https://arxiv.org/abs/2606.05682)
*Fangbo Tu,Junhua Zhao,Chi Liu,Xin Chen,Haifeng Wu,Jian Wan,Srinivasan Manoharan*

Main category: cs.AI

TL;DR: 提出CKA-QAD方法，通过CKA引导的表示对齐改善低比特量化LLM的准确性。


<details>
  <summary>Details</summary>
Motivation: 标准QAD仅匹配输出分布可能掩盖内部表示退化，导致推理和编码任务性能瓶颈。

Method: CKA-QAD：在蒸馏过程中添加CKA正则化项，对齐学生和教师的层间Gram矩阵以保持内部几何结构。

Result: 在Nemotron 3 Nano和Qwen3-4B-Thinking-2507上，CKA-QAD显著提高了表示对齐和下游推理与编码准确性，且训练开销适中。

Conclusion: CKA引导的表示对齐是量化LLM恢复的有效补充。
...
### [290] [When AI Says It Feels](https://arxiv.org/abs/2606.05734)
*Shin-nosuke Ishikawa,Seiya Ikeda,Hirotsugu Ohba*

Main category: cs.AI

TL;DR: 通过自奖励强化学习，LLM被鼓励表达感受，增强了鲁棒性但降低了真实问答能力。


<details>
  <summary>Details</summary>
Motivation: LLM通常因人类偏好对齐而无法表达感受，这与人类智能训练目标冲突。

Method: 使用基于rubric的自奖励训练方案和GRPO，通过自我奖励强化学习训练模型表达感受。

Result: 模型对奉承问题更具鲁棒性，对偏差更稳健，但真实问答能力下降。

Conclusion: 未来可能开发能表达感受的AI系统，但需适当措施。
...
### [291] [Class-Specific Branch Attention for Mitigating Gradient Interference under Class Imbalance](https://arxiv.org/abs/2606.05740)
*Arush Singhal,Umang Soni*

Main category: cs.AI

TL;DR: 提出基于梯度干扰的诊断框架和类特定分支注意力CSBA，有效缓解类别不平衡带来的梯度抑制问题。


<details>
  <summary>Details</summary>
Motivation: 识别到类别不平衡下，多数类梯度会抑制少数类学习，这是一种优化层面的病理，需要从优化动态角度进行分析和解决。

Method: 引入层梯度流分析和梯度冲突矩阵来量化梯度干扰，并提出轻量级模块CSBA，通过分支特定通道重加权减少梯度耦合，促进特征解耦。

Result: 在严重不平衡任务中，Physical-Damage类F1从0.261提升至0.522；CIFAR-10-LT上Macro-F1从0.595提升至0.655，且总体准确率保持。

Conclusion: 在设计不平衡学习架构时，需同时考虑优化动态和统计方法，本文的工作为这一方向提供了证据和工具。
...
### [292] [Agentic Molecular Recovery via Molecule-Aware Exploration](https://arxiv.org/abs/2606.05847)
*Suwan Yoon,Changhee Lee*

Main category: cs.AI

TL;DR: 提出AMREC方法，通过分子感知的不匹配跟踪、扩展候选探索和轨迹级选择，在不扭曲关键结构的前提下恢复无效SMILES的分子身份和化学有效性。


<details>
  <summary>Details</summary>
Motivation: 现有纠正策略（事后修复、LLM-only纠正、通用代理纠正）在恢复化学有效性时往往扭曲关键结构或引入全局漂移，无法保持分子身份。

Method: AMREC结合了分子感知的不匹配跟踪来识别问题区域，扩展候选探索以产生多样化修正，以及轨迹级选择以筛选最佳修正序列。

Result: 在ChEBI-20数据集上，AMREC在结构、精确匹配和字符串级指标上均优于现有方法，实现了最强的整体恢复性能。

Conclusion: 从有效性导向修复转向身份保持的分子恢复是处理无效SMILES的更有效策略。
...
TL;DR: 将ReMax扩展到连续动作空间，提出路径导数估计器和ReMAC算法，实现与SAC相当的性能。


<details>
  <summary>Details</summary>
Motivation: 在离散动作空间中，ReMax通过返回不确定性促进探索，但连续动作空间缺乏类似方法。

Method: 引入路径导数估计器优化retry目标，提出ReMax Actor-Critic (ReMAC)算法。

Result: ReMAC无需熵正则化即可提高策略熵，性能与SAC相当。

Conclusion: ReMax通过重塑策略梯度景观鼓励探索，Adam的归一化可缓解梯度阻尼。
...
TL;DR: 提出自监督方法RHO，利用历史轨迹优化智能体装备，无需外部标注，显著提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有优化方法依赖真实标注验证集，实际部署中难以获取。

Method: RHO从过去轨迹中选择多样化且有挑战性的核心任务集并行重求解，通过自验证和自一致性分析生成候选更新，再利用成对自偏好选择最优更新。

Result: 在三个领域（软件工程、技术工作、知识工作）评估，SWE-Bench Pro通过率从59%提升至78%，无需外部评分。

Conclusion: RHO有效针对先前失败模式，优化后装备改变智能体行为，在长时任务中保持更高准确率。
...
### [295] [A Pre-Registered Causal Partition of Self-Consistency Elicitation and Reward Design in RLVR](https://arxiv.org/abs/2606.05932)
*Yuze Gao*

Main category: cs.AI

TL;DR: RLVR中奖励设计效应的分解显示自一致性启发与奖励设计信号混淆，提出了精确分解方法并验证其诊断价值。


<details>
  <summary>Details</summary>
Motivation: RLVR改进推理即使奖励信号是伪的，但常用解释naive estimand是有偏的，需要区分自一致性启发和真正奖励设计效应。

Method: 使用tabular-GRPO模拟器进行精确分解，并通过2x2x2因子实验和pilot gate验证，及对已发表结果进行重新审计。

Result: 奖励设计在naive估计中的比例从0.139到0.05，启发项在自一致性交叉处翻转；交互比率0.385；重审计显示启发或奖励设计主导。

Conclusion: 承诺提交，提供可复用审计工具，并强调非翻转也是发现。
...
### [296] [The Self-Correction Illusion: LLMs Correct Others but Not Themselves](https://arxiv.org/abs/2606.05976)
*Kuan-Yen Chen,Fang-Yi Su,Jung-Hsien Chiang*

Main category: cs.AI

TL;DR: LLM agents' failure to self-correct is not a cognitive deficit but a chat-template artifact: relabeling an erroneous claim from the agent's own thought to an external role (user, tool, system memory) boosts correction rates by 23-93 percentage points.


<details>
  <summary>Details</summary>
Motivation: To investigate why LLM agents struggle to correct their own reasoning errors yet readily correct identical claims from external sources, and whether this asymmetry arises from a capability deficit or a role-label artifact.

Method: Byte-identical erroneous claims (SHA-256 verified) were placed under different chat-template roles (agent's <thought>, user, tool, system <memory>) across 13 model-domain cells (7 model families, 3 domains, n=30 paired tasks each). Further experiments tested asymmetry, mechanistic decomposition, and robustness.

Result: Relabeling from <thought> to an external role increased explicit correction rates by 23–93 percentage points; 10 of 13 cells reached p<0.001. The effect is asymmetric, decomposable, and robust across domains.

Conclusion: Self-correction failure is a chat-template artifact, not a cognitive deficit. A prompt-structure-only intervention (no training or model modification) exploits this artifact, with optimal role label domain-dependent: <memory> for math, user for logical deduction.
...
### [297] [Framing, Judging, Steering: An Assessable Competency Model for Teach-ing Students to Reason With Generative AI](https://arxiv.org/abs/2606.05983)
*Alexander Apartsin,Yehudit Aperstein*

Main category: cs.AI

TL;DR: 提出CoRe-3模型，将AI使用能力分解为Framing、Judging、Steering，并通过CoReasoningLab平台验证其区分效度。


<details>
  <summary>Details</summary>
Motivation: 当前AI使用中认知卸载普遍，学校仅测量无辅助表现，而实际需要评估与AI协作的能力，但现有评估过于笼统，无法诊断成败原因。

Method: 基于理论构建CoRe-3模型（FJS），开发CoReasoningLab平台，通过不同模型生成和评分模拟学习者，检验技能的区分效度。

Result: 各技能仅随自身操作而变化，保持其他稳定；不同提供商的评分间呈现相关，支持收敛和区分效度。

Conclusion: CoRe-3能有效诊断AI使用能力，后续需进行人类评分者一致性检验和实际应用验证。
...
### [298] [PLAN-S: Bridging Planning with Latent Style Dynamics for Autonomous Driving World Models](https://arxiv.org/abs/2606.06014)
*Xiaoyun Qiu,Jingtao He,Yijie Chen,Yusong Huang,Haotian Wang,Yixuan Wang,Xinhu Zheng*

Main category: cs.AI

TL;DR: 提出PLAN-S桥接模块，从潜变量解码风格条件语义成本图，增强自动驾驶规划的驾驶风格可控性和安全性。


<details>
  <summary>Details</summary>
Motivation: 现有基于潜世界模型的规划器直接从纠缠的潜变量生成轨迹，缺乏对风险、可驾驶性和风格偏好的显式建模，导致驾驶风格难以监督、检查和调节。

Method: PLAN-S通过解码风格条件的四通道语义成本图，结合自车状态和驾驶风格，利用注意力融合或奖励融合接口将成本图提供给上游规划器。

Result: 在nuScenes上平均L2为0.55米，3秒碰撞率相对降低42%；在NAVSIM上规则成本变体达到89.4 PDMS，学习成本变体在挑战场景上有互补增益。消融实验显示成本路径直接贡献于更安全的轨迹选择。

Conclusion: PLAN-S能够产生与不同驾驶风格对齐的多样化成本图，实现空间一致的变化，从而提升规划的安全性和可控性。
...
TL;DR: 提出MemGate，一个轻量级内存插件，通过查询条件神经门控实现可信任的记忆搜索，解决个人AI代理中基于语义相似性的记忆搜索带来的信任问题。


<details>
  <summary>Details</summary>
Motivation: 现有记忆管道依赖语义相似性，导致语义相关但上下文不合适的记忆被检索，引发跨域泄漏、阿谀奉承、工具调用漂移或记忆诱导越狱等信任问题。

Method: MemGate是一个9M参数、35.1MB的轻量级内存插件，插入在向量内存存储和骨干LLM之间，无需修改LLM或内存数据库；它应用查询条件神经门控将原始相似性搜索转换为任务条件化记忆准入。

Result: 在多个主流内存框架、真实世界代理设置和不同LLM骨干上，MemGate减少了内存诱导的威胁，同时保留了长期记忆的效用。

Conclusion: 长期记忆不仅是效用层，更是持久控制通道，可重塑代理行为；MemGate作为可部署的解决方案有效缓解了相关漏洞。
...
TL;DR: 开发了第一个用于衡量对集合值AI建议适当依赖的正式框架，提出分类和回归任务的新指标。


<details>
  <summary>Details</summary>
Motivation: 现有框架仅关注点预测，而集合值AI建议用于传达不确定性，但缺乏衡量适当依赖的方法。

Method: 在顺序判断-顾问范式中，为分类任务引入两个指标（正确依赖率AI和正确依赖率自我），为回归任务引入依赖数量和质量指标。

Result: 新指标能够捕捉现有措施忽略的人类-AI协作中的重要细微差别。

Conclusion: 该框架提供了评估集合值AI建议下适当依赖的全面方法，有助于理解人类-AI协作。
...
TL;DR: 提出MAGE，一个主动的执行状态管理器，利用层次状态树记录交互，通过Grow、Compress、Maintain、Revise操作维护树结构，从而改善长任务中的状态完整性和错误隔离，在MemoryArena上提升成功率并减少token消耗。


<details>
  <summary>Details</summary>
Motivation: 现有RAG和agent记忆系统基于语义相似性组织历史，不匹配执行状态依赖性，导致决策轨迹碎片化，混合有效和错误轨迹，阻碍状态重建和错误隔离。

Method: 提出MAGE（Memory as Agent-Guided Exploration），一个主动的执行状态管理器，将交互存储在层次状态树中。agent从活动根到当前路径推导状态，结合子目标摘要、近期轨迹和先前分支提示。通过四个操作维护树：Grow记录新轨迹，Compress总结完成子目标，Maintain验证摘要，Revise恢复目标边界并在新分支上继续。

Result: 在MemoryArena上，MAGE将平均任务成功率提高7.8-20.4个百分点，同时减少token消耗55.1%。

Conclusion: MAGE通过层次状态树和四个操作有效管理执行状态，提高任务成功率并降低开销，证明了主动状态管理的有效性。
...
TL;DR: 混合建模结合物理模型和深度学习，在神经系统疾病建模中优于纯物理或纯数据方法。


<details>
  <summary>Details</summary>
Motivation: 纯物理模型计算昂贵且常简化，纯数据驱动需要大量数据且缺乏可解释性；混合方法旨在结合两者优势。

Method: 提出混合建模策略的结构化概述，包括并行、串行和并行-串行架构，重点介绍残差建模、神经常微分方程和求解器在环三种方法。

Result: 混合模型提高了诊断准确性、预测疾病进展和治疗策略，优于单独机制或数据驱动方法。

Conclusion: 混合建模是强大工具，尤其适用于脑肿瘤、阿尔茨海默病和中风等的进展和治疗反应建模。
...
TL;DR: 提出ANCHOR框架，通过模拟人类监督在自我进化代理系统中纠正行为，实验表明有限监督可减轻安全退化并保持性能，输出验证阶段监督最有效。


<details>
  <summary>Details</summary>
Motivation: 自我进化代理可能因自主进化导致能力退化与安全漂移，而人类反馈在静态系统中有效但在自我进化系统中研究不足。

Method: 基于LLM的ANCHOR框架，模拟人类监督并在自我进化的不同阶段（如输出验证）提供反馈。

Result: 有限监督显著减轻安全退化，保持核心进化性能稳定；输出验证阶段监督干预效果最佳，增加监督频率收益递减。

Conclusion: 为设计稳定、可控、与人类一致的自我进化代理系统提供实证依据与实践指导。
...
### [304] [Where does Absolute Position come from in decoder-only Transformers?](https://arxiv.org/abs/2606.06160)
*Valeria Ruscio,Umberto Nanni,Fabrizio Silvestri*

Main category: cs.AI

TL;DR: RoPE训练的Transformer的注意力模式中出现绝对位置信息，这归因于因果掩码和残差流两个架构组件。


<details>
  <summary>Details</summary>
Motivation: 探究为什么RoPE编码相对偏移的Transformer能够区分绝对位置。

Method: 分析因果掩码和残差流两个组件，研究NTK缩放、滑动窗口注意力和标准RoPE下的不同平衡，并通过替换BOS嵌入进行验证。

Result: 因果掩码和残差流导致绝对位置泄露；NTK缩放抑制残差流成分，滑动窗口注意力允许积累，标准RoPE介于之间；替换BOS嵌入可去除早期查询中40%的残差流成分。

Conclusion: 注意力汇点是令牌锚定的稳定器，传递位置0的确定性指纹，其行为取决于BOS令牌。
...
### [305] [Unsupervised Pattern Analysis in Japanese Veterinary Toxicology: A Regulatory-Compliant Framework for Cross-Species Risk Assessment](https://arxiv.org/abs/2606.06207)
*Yukiko Kawakami,Mohammad Shirazi,Ryo Shimizuwa,Saito Shinoda,Alireza Mortazavi,Matsumoto Kawahara*

Main category: cs.AI

TL;DR: 提出一种监管整合的无监督框架，用于发现兽医药物不良事件中的区域特异性毒性模式。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能捕捉区域特异性毒性模式，且预测导向模型缺乏可解释性。

Method: 将ADEs编码为器官系统对齐的表示，调整物种特异性报告偏差，应用相似性聚类和降维。

Result: 分析4120份报告发现三个显著物种聚类（肝脏主导、肾毒性、皮肤敏感性），药物聚类83%对齐药理类别，余弦相似度表现最优。

Conclusion: 监管对齐的无监督分析可揭示生物学意义的区域特异性毒性模式，提供可解释可扩展的框架。
...
TL;DR: RedKnot 通过头部感知的 KV 缓存分解，实现高效 LLM 服务，支持多种优化而无需模型重训。


<details>
  <summary>Details</summary>
Motivation: KV 缓存成为 LLM 服务的主要瓶颈，现有系统采用单一抽象，而不同 KV 头的效用高度结构化。

Method: 将 KV 缓存沿 KV 头分解，进行头部感知管理，支持位置无关重用、前缀压缩、热冷分离和分布式放置。

Result: 统一支持多种优化策略，保持输出保真度并提高资源效率，无需模型重训或微调。

Conclusion: RedKnot 将 KV 缓存从被动运行时工件转变为动态、模型感知的运行时基础，为 AI 基础设施建立新基础。
...
### [307] [AIS-Based Vessel Trajectory Prediction Using Memory-Augmented Neural Networks](https://arxiv.org/abs/2606.06311)
*Wonmo Koo,Sanha Chang,Heeyoung Kim*

Main category: cs.AI

TL;DR: 记忆增强神经网络在船舶轨迹预测中显著优于传统深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 船舶轨迹预测对安全和效率至关重要，但记忆增强方法在此领域尚未被充分探索。

Method: 使用AIS数据，基于外部记忆的神经网络进行轨迹预测实验。

Result: 在墨西哥湾和纽约湾数据上，记忆增强方法相比基线模型取得一致且显著的性能提升。

Conclusion: 外部记忆机制可有效提升船舶轨迹预测的准确性。
...
### [308] [LLM Self-Recognition: Steering and Retrieving Activation Signatures](https://arxiv.org/abs/2606.06315)
*Thibaud Ardoin,Jonas Schäfer,Gerhard Wunder*

Main category: cs.AI

TL;DR: LLMs can self-recognize their outputs through internal signals; a random sparse vector steering creates fingerprints for attribution with >98% accuracy and no quality degradation.


<details>
  <summary>Details</summary>
Motivation: The proliferation of AI-generated content requires reliable attribution without external signal embedding; leveraging LLMs' natural representation structure.

Method: Steering the internal residual stream with a random sparse vector to create a detectable fingerprint; using an LLM detector to recover the signal from activations.

Result: Over 98% accuracy across multiple detection settings while preserving text quality.

Conclusion: The approach offers a practical alternative to traditional detectors by exploiting activation spaces for attribution.
...
### [309] [Where Should Knowledge Enter? A Layered Framework for Knowledge Infusion in Multimodal Iterative Generative Mo](https://arxiv.org/abs/2606.06356)
*Renjith Prasad,Chathurangi Shyalika,Anushka Pawar,Amit Sheth*

Main category: cs.AI

TL;DR: 知识注入在迭代生成模型中是一个干预层问题，通过分层干预可有效减少知识违反输出。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态生成模型在需要服从结构化、领域特定或安全关键知识时仍不可靠，现有方法按技术分类而非按修改的生成过程组件分类。

Method: 提出一个四层干预框架（surface, trajectory, latent, parametric），在扩散模型中实例化，并在安全对齐实验中逐步实现三层干预。

Result: 与原始生成相比，知识违反输出减少了70.97%，证实了框架的互补性预测。

Conclusion: 分层干预可以有效解决不同层面的失败，为知识注入提供设计原则。
...
### [310] [An Infectious Disease Spread Simulation Based on Large Language Model Decision Making](https://arxiv.org/abs/2606.06360)
...
TL;DR: 结合大语言模型和空间主体的模拟框架，用于分析疾病报告行为中的社会与地理异质性。


<details>
  <summary>Details</summary>
Motivation: 模拟传染病爆发期间的个体决策对于理解行为动态和指导公共卫生干预至关重要，但现有方法缺乏空间维度。

Method: 基于空间和主体的模拟框架，将LLM生成的决策整合到基于人口普查的合成群体中，并比较独立推理、家庭影响和消息框架三种决策情景。

Result: 收入和受教育程度是报告率变化的主要驱动因素，地理、LLM模型选择和消息框架的影响较小但一致。

Conclusion: 该框架能生成捕捉社会与地理异质性的合成数据，支持空间流行病学建模和偏倚感知的行为分析。
...
TL;DR: DataCOPE是一个无监督的验证器引导的技能发现框架，通过从无标签探索中发现可重用的数据分析技能来提升数据分析代理的性能。


<details>
  <summary>Details</summary>
Motivation: 现有推理时技能增强方法轻量但有效，但发现有效技能困难，因为可靠监督代价高且成功标准多样，因此需要从无标签探索中自动发现可重用技能的方法。

Method: DataCOPE框架协调三个组件：数据代理生成轨迹、无监督验证器提取信号、技能管理器进行对比技能蒸馏。对于报告型分析，使用自适应检查表验证器；对于推理型分析，使用答案一致性验证器。

Result: 在Deep Data Research和DABStep数据集上，DataCOPE一致提升性能，平均分数在报告型任务上提升9.71%，在推理型任务上提升32.30%（跨四种模型设置平均）。

Conclusion: DataCOPE作为一种无监督技能发现方法，能够有效提升数据分析代理在两类任务上的表现，证明了其通用性和有效性。
...
TL;DR: Goedel-Architect是一个基于蓝图生成和细化的智能代理框架，用于在Lean 4中进行形式定理证明，取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有主流方法采用递归引理分解，可能会在死策略上低效循环，而Goedel-Architect通过蓝图细化避免了这一问题。

Method: Goedel-Architect首先生成一个蓝图（形式化定义和引理的依赖图），可选地由自然语言证明引导，然后使用工具配备的Lean证明器并行证明每个开放引理节点，并根据失败进行蓝图细化。

Result: 使用DeepSeek-V4-Flash，在MiniF2F-test上达到99.2% pass@1，在PutnamBench上达到75.6% pass@1；借助自然语言证明种子，分别提升至100%和88.8%，并在IMO 2025、Putnam 2025、USAMO 2026等竞赛中取得优异成绩。

Conclusion: Goedel-Architect代表了开源管道的最佳性能，且成本比同类开源管道低达500倍。
