...
### [1] [Never Seen Before: Benchmarking Genuine Zero-Shot Composed Image Retrieval with Consistent Video-Sourced Datasets](https://arxiv.org/abs/2606.07032)
*Zhenyu Yang,Zemin Du,Shengsheng Qian,Changsheng Xu*

Main category: cs.CV

TL;DR: 提出了ZeroSight基准和SC4CIR方法，用于更真实地评估零样本组合图像检索性能，并揭示了现有数据集和指标的性能膨胀。


<details>
  <summary>Details</summary>
Motivation: 现有ZS-CIR数据集存在参考与目标图像不相关的问题，且使用CLIP已训练过的公共图像数据集，并非真正的零样本场景。

Method: 构建了从视频中提取一致性参考-目标对的ZeroSight数据集，通过LLM生成相对描述，并采用考虑多正多负目标排序的评估方法；同时提出训练无关的MLLM驱动方法SC4CIR，通过3个对称一致性检查识别难负样本。

Result: 对27种方法的实验表明，当前ZS-CIR数据集和评估指标导致检索性能被夸大。

Conclusion: ZeroSight和SC4CIR能更准确地评估ZS-CIR性能，且SC4CIR可即插即用并提升现有方法的性能。
...
### [2] [MemDreamer: Decoupling Perception and Reasoning for Long Video Understanding via Hierarchical Graph Memory and Agentic Retrieval Mechanism](https://arxiv.org/abs/2606.07512)
*Cong Chen,Guo Gan,Kaixiang Ji,ChaoYang Zhang,Zhen Yang,Guangming Yao,Hao Chen,Jingdong Chen,Yi Yuan,Chunhua Shen*

Main category: cs.CV

TL;DR: MemDreamer通过将长视频理解转化为智能体探索过程，构建层次图记忆，实现SOTA性能且仅用2%上下文窗口。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型在处理数小时长视频时，由于全长度视觉序列导致令牌爆炸和注意力稀释，面临困难。

Method: 提出MemDreamer，一个即插即用框架，通过增量流式处理视频构建层次图记忆（三层架构，捕捉时空和因果关系），推理时采用观察-推理-行动循环的智能体工具增强检索。

Result: 在四个主流基准上达到SOTA，与人类专家差距仅3.7分，推理上下文窗口仅占全上下文摄入的2%，同时带来12.5分绝对准确率提升。

Conclusion: 统计分析发现VLM在逻辑推理和长视频理解基准性能之间呈强正线性相关，将智能体能力扩展确立为多模态理解的新范式。
...
### [3] [Spatial-Temporal Decoupled Adapter for Micro-gesture Online Recognition](https://arxiv.org/abs/2606.07355)
*Xucheng Shen,Kun Li,Fei Wang,Wei Qian,Jin Jiang,Dan Guo*

Main category: cs.CV

TL;DR: 提出时空解耦适配器和自适应软平衡增强，用于微手势在线识别，在挑战赛中取得第一。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效适配器使用单分支联合建模时空线索，难以捕捉微手势的细粒度模式，且数据集存在长尾分布问题。

Method: 提出时空解耦适配器，通过轻量级深度可分离卷积将视频适配分解为独立的时间和空间分支；引入自适应软平衡增强，根据类稀有性和学习难度动态分配增强强度。

Result: 在第四届EI-MiGA-IJCAI挑战赛Track 2中取得F1分数0.43808，排名第一。

Conclusion: 该方法有效提升了微手势在线识别的性能，尤其处理了长尾分布问题。
...
TL;DR: GOPAgen integrates video codec and structural memory for efficient motion comprehension in long videos.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack detailed motion comprehension coupled with an efficient memory architecture.

Method: Proposes GOPAgen with a motion agent trained on GOPs, GOP tree reasoning algorithm, structural memory mechanism, coarse-to-fine zoom-in algorithm, and motion vector database.

Result: Achieves superior VQA performance on MotionBench and Egoschema benchmarks.

Conclusion: Demonstrates the superiority of the proposed framework.
...
TL;DR: SGBR-HC通过监督光谱排名初始化稀疏门，实现自适应波段选择，在约20个波段时达到最优分类性能。


<details>
  <summary>Details</summary>
Motivation: 基于可微选择器的方法对初始化和离散子集提取敏感，且预设波段数限制灵活性。

Method: 两阶段方法：阶段1利用分类可分辨性和光谱多样性对候选波段排序；阶段2将排序结果初始化为稀疏门logits，并与空间分类器联合训练。

Result: 在帕维亚大学和休斯顿2013数据集上，SGBR-HC使用约20个波段获得最高平均OA和Cohen's kappa；无阶段1时OA分别下降8.84和22.15个百分点；随机分割导致OA虚高30.56个百分点。

Conclusion: 监督排名先验对性能至关重要，且空间泄漏是评估中的关键混淆因素。
...
### [6] [CULTURESCORE: Evaluating Cultural Faithfulness in Video Generation Models](https://arxiv.org/abs/2606.07311)
*Anku Rani,Wei Dai,Shravan Nayak,Pattie Maes,Mahdi M. Kalayeh,Paul Pu Liang*

Main category: cs.CV

TL;DR: 提出了CultureScore，一个用于评估视频生成模型文化忠实度的组合框架，将文化忠实度分解为身份、背景和行为三个维度。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在文化表现方面存在不足，而当前评估指标如VideoScore仅关注视觉质量，无法评估文化忠实度。

Method: 通过将文化忠实度分解为身份、背景和行为三个维度，构建CultureScore框架，并基于10个国家、6,180个视频和三个先进模型进行评估。

Result: 最佳模型总体CultureScore仅56.8%，其中行为维度得分低于52%；人类偏好与CultureScore一致，但与VideoScore相反，表明文化忠实度是重要但被忽视的标准。

Conclusion: 文化忠实度是公平视频生成的关键标准，当前模型在文化忠实方面仍有很大提升空间，尤其是行为维度。
...
### [7] [TEVI: Text-Conditioned Editing of Visual Representations via Sparse Autoencoders for Improved Vision-Language Alignment](https://arxiv.org/abs/2606.07451)
*Sweta Mahajan,Sukrut Rao,Jiahao Xie,Alexander Koller,Bernt Schiele*

Main category: cs.CV

TL;DR: 提出TEVI框架，利用稀疏自编码器根据标题选择性重建图像嵌入，改善CLIP嵌入对齐，提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型如图像和文本嵌入对齐不佳，源于图像信息多于标题描述的信息不平衡。

Method: 使用稀疏自编码器解缠图像嵌入，训练掩码模块根据给定标题选择性重建嵌入。

Result: 在合成标题设置中有效；在多个基准上（MS COCO, Flickr, IIW, DOCCI, RoCOCO）取得改进的检索性能和鲁棒性，特别是对于丰富标题。

Conclusion: TEVI通过以标题为信号保留描述属性并丢弃无关信息，有效改善了CLIP模型的嵌入对齐和下游性能。
...
### [8] [RPC-GS: Gaussian Splatting with native RPC Rendering for Satellite Imagery](https://arxiv.org/abs/2606.06690)
*Valentin Wagner,Sebastian Bullinger,Christoph Bodensteiner,Michael Arens*

Main category: cs.CV

TL;DR: 首次提出直接使用RPC模型进行卫星图像高斯泼溅的框架，避免近似误差，提升重建精度。


<details>
  <summary>Details</summary>
Motivation: 现有卫星高斯泼溅方法使用透视或仿射相机模型近似RPC，引入几何误差。

Method: 将高斯均值和协方差直接通过RPC模型投影，推导数值稳健的雅可比协方差变换，并集成度量射线深度。

Result: 在DFC2019和IARPA2016数据集上，RPC-GS相比透视和仿射近似，平均海拔误差分别降低29.6%/63.8%和9.9%/37.9%。

Conclusion: 开源代码以促进卫星成像领域的高斯泼溅研究。
...
### [9] [GuideCAD: A Lightweight Multimodal Framework for 3D CAD Model Generation via Prefix Embedding](https://arxiv.org/abs/2606.07024)
*Minseong Kim,Jinyeong Park,Sungho Park,Jibum Kim*

Main category: cs.CV

TL;DR: 提出GuideCAD，一种轻量级的3D CAD生成方法，利用视觉-文本表示和少量可训练参数，通过映射网络将图像嵌入转换为前缀嵌入，使预训练大语言模型融合视觉和文本信息，然后由Transformer解码器预测构建序列生成3D CAD模型。实验表明，与微调方法相比，参数减少约4倍，训练效率提高2倍，生成质量相当。


<details>
  <summary>Details</summary>
Motivation: 多模态3D CAD生成需要大量计算资源，需要高效训练。

Method: GuideCAD使用映射网络将图像嵌入转换为前缀嵌入，使预训练大语言模型整合视觉和文本信息，然后Transformer解码器预测构建序列生成3D CAD模型。

Result: 参数减少4倍，训练效率提高2倍，生成质量相当。

Conclusion: GuideCAD实现了高效且高质量的3D CAD生成。
...
### [10] [Detecting Temporally Localized Manipulations in Authentic Video Streams](https://arxiv.org/abs/2606.07090)
*Okan Umur,Ali Emre Güşlü,Ibrahim Delibasoglu*

Main category: cs.CV

TL;DR: 针对现有数据集在检测短插入操纵片段方面的不足，提出了一个专门用于部分操纵视频检测的新数据集，并评估了两种基于DINOv3特征的方法作为初始基准。


<details>
  <summary>Details</summary>
Motivation: 现有数据集未能充分模拟短操纵片段插入真实视频且原始视频继续的场景，需要专门针对此类检测的新数据集。

Method: 基于DINOv3特征，采用线性探针（三种阈值策略）和连续帧相似性方法检测时间操纵边界。

Result: 实验建立了部分操纵视频检测的初始基准，表明需要内容自适应阈值机制。

Conclusion: 所提出的数据集、代码和补充材料已公开，为后续研究提供了基础。

Abstract: The rapid advancement of video editing and generative artificial intelligence technologies has made realistic video manipulation increasingly accessible. Although existing datasets have significantly advanced research in deepfake detection, object removal, and video inpainting, they do not adequately model scenarios in which a short manipulated segment is inserted into an otherwise authentic video and the original video continues afterward. In this study, we review representative datasets from the literature, analyze their characteristics, and discuss their limitations with respect to temporally localized realistic manipulation detection. Based on this analysis, we motivate the need for a new dataset specifically designed for authentic videos containing short and highly realistic manipulated intervals. Finally, we evaluate two complementary approaches on our custom-curated test set to establish an initial benchmark for this challenging scenario. The first employs a linear probe on DINOv3 features, assessed under three thresholding strategies. The second leverages DINOv3 features with a consecutive frame similarity-based method to detect temporal manipulation boundaries. Together, these experiments provide an initial benchmark for partially manipulated video detection and highlight the need for content-adaptive thresholding mechanisms. The dataset, code, and supplementary materials are publicly available at .
...
### [16] [STREAM: Stochastic Riemannian Flow Matching with Anisotropic Decoder for Digital Histopathology Image Generation](https://arxiv.org/abs/2606.07036)
*Won June Cho,Daeky Jeong,Hyeongyeol Lim,Hongjun Yoon*

Main category: cs.CV

TL;DR: STREAM是首个在病理学领域应用黎曼流形匹配的生成框架，通过使用预训练病理视觉基础模型的令牌特征作为潜在空间并设计两阶段训练方法，实现了最先进的图像生成与重建性能。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型使用预训练视觉基础模型作为条件信号时出现“条件崩塌”，导致生成样本质量与多样性下降。

Method: 使用预训练病理学视觉基础模型的补丁令牌特征作为潜在空间，这些特征在单位超球面上具有角度主导性，适合黎曼公式。提出STREAM框架，包括桥型随机扰动在单位超球面上建立每个令牌的可整流性以训练扩散变压器，以及各向异性解码器对速度场雅可比矩阵的低能量方向分配鲁棒性同时保留高能量方向的保真度。

Result: 在乳腺癌和结直肠癌数据集上实现了最先进的重建和生成性能。

Conclusion: STREAM框架证明了在病理学中使用黎曼流形匹配的有效性，并展示出在合成高质量病理图像方面的优越性。
...
TL;DR: WorldBench是一个视觉更多样的推理基准，揭示多模态大模型在视觉理解上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏视觉多样性，无法充分评估模型在开放视觉输入下的可靠性。

Method: 构建视觉概念分类法，据此收集多样图像，并手动设计前沿模型难以回答的问题。

Result: WorldBench比现有基准视觉多样性更高；最强模型准确率仅64.0%，部分模型接近随机。

Conclusion: 视觉多样性对构建多模态基准至关重要，当前模型在视觉理解上仍有显著弱点。
...
### [19] [Synthetic Benchmarks Overstate Forward-Forward Scaling: Real-Data Limits of Layer-Local Training](https://arxiv.org/abs/2606.06539)
*Yucheng Chen*

Main category: cs.CV

TL;DR: 层局部训练（Forward-Forward）在现实规模上仍显著落后于反向传播，存在可扩展性天花板。


<details>
  <summary>Details</summary>
Motivation: 探图层局部训练在现实数据和大规模场景下的可行性，质疑其是否成为反向传播的替代方案。

Method: 开发DTG-FF（动态温度goodness、解耦归一化、多层融合）作为工具，在多个真实数据基准和合成任务上进行审计，并与反向传播基线对比。

Result: DTG-FF在CIFAR-10/100上低于匹配的BP基线2.40/5.93pp；在ImageNet-100 224x224上仅49.4% vs BP>75%；合成任务中FF优于BP但真实数据相反；内存优势不成立（DTG-FF内存和速度均劣于BP）。

Conclusion: 层局部训练在现实规模不是反向传播的可行替代，存在严重扩展局限。
...
### [20] [Architecture-Adaptive Uncertainty Fusion for Deepfake Detection](https://arxiv.org/abs/2606.06666)
*Ritesh Sharma,Mohammad Ghasemigol,Yuichi Motai*

Main category: cs.CV

TL;DR: 本文提出COF框架，融合五种不确定性源，通过最大化与预测误差的相关性来优化权重，在分布偏移下表现优于现有方法，但跨数据集时所有方法均出现泛化失败。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性量化方法依赖单一源，忽略架构差异，而深度伪造检测的取证部署需要可靠的不确定性估计。

Method: 提出Correlation-Optimized Fusion (COF)，一个架构自适应框架，通过约束优化在概率单纯形上最大化融合不确定性分数与预测误差的皮尔逊相关，融合认知、偶然、校准、共形和分布五种不确定性源。

Result: 在域内相关上，非线性方法比COF高5-6%，但在分布偏移下COF表现更好；在CelebDF上，COF在9/11架构中优于随机森林，相关性最高提升7.3倍；跨数据集评估时，所有方法的域内相关从0.41-0.47骤降至接近零，平均下降90.7%，七种架构出现不确定性反转。

Conclusion: COF是一个实用、可解释的框架，适用于受控分布部署，并指出域自适应不确定性量化是取证部署的核心挑战。
...
### [21] [SVHighlights: Towards Extremely Long Sport Video Highlight Detection](https://arxiv.org/abs/2606.06926)
*Donggyu Lee,Youngbin Ki,Jeonghun Kang,Taehwan Kim*

Main category: cs.CV

TL;DR: Introduces SVHighlights, the first benchmark for highlight detection in hour-long sports videos, and presents TF-SELECTOR, a training-free segment-based method that uses a multimodal LLM to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing highlight detection methods are limited to short-form content due to the lack of a suitable benchmark for long-form videos.

Method: Constructed SVHighlights from full-length sports videos and official highlights using a scalable pipeline; proposed TF-SELECTOR which segments videos into context-aware semantic groups and predicts segment-level saliency via a multimodal LLM (visual captions, transcripts, audio volume), without training.

Result: TF-SELECTOR outperforms VTG-tuned baselines with improvements of +3.12 in HIT@1, +4.06 in HIT@K, and +2.95 in IoU.

Conclusion: SVHighlights is a challenging testbed for long-form detection, and a simple segment-based strategy can effectively scale to hour-long videos.
...
### [22] [From Vision to Text: A Compact Multimodal Approach for Robust, Cross-Domain Presentation Attack Detection on ID Cards](https://arxiv.org/abs/2606.06966)
*Qingwen Zeng,Juan E. Tapia,Sneha Das,Christoph Busch*

Main category: cs.CV

TL;DR: This paper proposes a compact multimodal model for ID card presentation attack detection, finds that multimodal models fail in zero-shot settings, and emphasizes the need for real-world data and re-evaluation of synthetic benchmarks.


<details>
  <summary>Details</summary>
Motivation: Cross-domain shifts challenge ID card PAD due to privacy-restricted data availability.

Method: A compact multimodal model with new generative and discriminative blocks, combining visual and textual data.

Result: Multimodal models generalize well after supervised fine-tuning but fail in zero-shot settings.

Conclusion: Model capacity and real-world data are essential; existing synthetic datasets may not reflect real-world challenges, necessitating more realistic and diverse datasets.
...
### [23] [LARA: Latent Action Representation Alignment for Vision-Language-Action Models](https://arxiv.org/abs/2606.07100)
*Mengya Liu,Baoxiong Jia,Jiangyong Huang,Jingze Zhang,Siyuan Huang*

Main category: cs.CV

TL;DR: 提出潜在动作表示对齐（LARA）框架，通过表示对齐联合优化LAM和VLA，提升机器人操作性能。


<details>
  <summary>Details</summary>
Motivation: 现有LAM和VLA分开训练导致LAM未接地且VLA受限于冻结表示，限制了性能。

Method: LARA是一个即插即用框架，通过表示对齐联合训练LAM和VLA，使LAM学习动作轨迹避免虚假变化，VLA通过前向动态正则化减少幻觉。

Result: 在三个模拟和一个真实世界基准上，预训练、后训练增强和LAM细化分别平均提升约10%、5%和15%。

Conclusion: LARA在多种场景下有效，能显著提升VLA模型性能。
...
### [24] [Reconstructing Multi-Decadal Forest Disturbances: A Spatio-Temporal Transformer Approach](https://arxiv.org/abs/2606.07249)
...
### [25] [Dash2Sim: Closed-Loop Driving Simulation from in-the-wild Dashcam Videos](https://arxiv.org/abs/2606.07366)
*Anurag Ghosh,Francesco Pittaluga,Khiem Vuong,Angela Chen,Juan Alvarez-Padilla,Manmohan Chandraker,Srinivasa Narasimhan*

Main category: cs.CV

TL;DR: Dash2Sim converts monocular dashcam videos into metric 4D driving logs, creating the ROADWork4D dataset to benchmark work zone scenarios, finding planners struggle with lane changes and depth recovery aids novel-view synthesis.


<details>
  <summary>Details</summary>
Motivation: Self-driving simulations rely on limited data; dashcams cover more but are underutilized due to difficulty in recovering accurate 4D scenes from monocular in-the-wild videos, especially for work zones.

Method: Dash2Sim framework extracts metric, geo-referenced 4D driving logs from monocular dashcam videos, verified against an independent map without annotations.

Result: Created ROADWork4D dataset with 4,244 scenes across 17 cities. Evaluated closed-loop planners on work zones; rule-based/hybrid generalize better than learning-based but all fail lane changes. Dense depth from Dash2Sim improves novel-view synthesis perceptual metrics by up to 19%.

Conclusion: Dash2Sim shows potential to provide rich conditioning for closed-loop sensor simulation from monocular videos.
...
### [26] [Mitosis Detection in the Wild: Multi-Tumor and Context-Aware Generalization in the MIDOG 2025 Challenge](https://arxiv.org/abs/2606.07368)
...
TL;DR: MIDOG 2025挑战评估有丝分裂检测在多种肿瘤类型和扫描平台上的泛化能力，发现模型在非热点区域性能显著下降，存在盲点，集成可微弱提升性能。


<details>
  <summary>Details</summary>
Motivation: 先前基准仅关注扫描仪域偏移，而临床需要模型对组织学变异鲁棒，因此设计挑战评估算法在生物和上下文多样性上的性能。

Method: 构建包含365例、12种肿瘤类型、多扫描平台的数据集，要求检测随机和挑战性区域，并引入非典型有丝分裂像分类轨道，评估集成和测试时增强。

Result: 检测轨道F1最高0.740，AMF轨道平衡准确率最高0.908；挑战性区域假阳性率翻三倍，不同肿瘤类型性能差异大；集成提升F1 1.5%、平衡准确率1.3%，TTA无效。

Conclusion: 在真实世界场景下有丝分裂检测仍是重大挑战，多环境框架比仅热点评估更贴近临床可靠性需求。
...
### [27] [RealDocBench: A Benchmark for Field-Level QA and Layout Understanding on Real-World Regulated Documents](https://arxiv.org/abs/2606.07401)
*Ameya Joshi,Joon Kim,Gus Eggert,Joseph Bajor,Cindy Hao,Jing Reyhan,Kushal Byatnal,Eli Badgio*

Main category: cs.CV

TL;DR: 本文提出RealDocBench，一个基于真实受监管文档的文档解析基准，包含QA和布局两个轨道，评估18个系统，揭示了单一指标掩盖的性能差异和成本/延迟权衡。


<details>
  <summary>Details</summary>
Motivation: 现有文档解析基准多使用干净学术文档或合成文本，评估指标单一，无法反映下游任务实际需求（如特定字段的正确值）。

Method: 构建RealDocBench基准，包括QA轨道（1356个字段级问题，581个文档，按字段和问题准确性评分）和布局轨道（1500个页面图像，COCO风格标注，九类，匈牙利匹配器评分）。评估18个系统（商业API、VLMs、OCR模型）的准确性、成本和延迟。

Result: RealDocBench揭示了广泛的性能差异，医疗子领域表现持续困难，不同系统在成本和延迟上存在显著权衡。

Conclusion: 作者发布了数据集、解析器适配器和评估工具，以支持可重复的、字段级的文档解析系统比较。
...
TL;DR: DisPOSE是一个自监督框架，通过扩散过程建模多视角人员分配，结合可微Sinkhorn投影和超图卷积解码器，实现多人3D姿态估计，在标准数据集和遮挡场景中性能领先。


<details>
  <summary>Details</summary>
Motivation: 现有自监督方法依赖合成3D姿态目录，导致真实场景泛化差，需要更鲁棒的多视角分配方法。

Method: 将多视角人员分配问题看作生成扩散过程，使用可微Sinkhorn投影引导有效分配，并通过超图卷积解码器跨视角回归完整3D骨架。

Result: 在标准数据集上超越当前自监督方法，在手术室高度遮挡新基准上表现强劲，仅用10%伪标签即可保持99%性能。

Conclusion: 扩散定位实现高标签效率，且框架对不同相机排列几乎无关，具有强鲁棒性。
...
TL;DR: 本文提出从人类视角（观看、记忆、推理）统一分析基于LLM的视频理解，并围绕感知、记忆、推理和预测构建公式化描述。


<details>
  <summary>Details</summary>
Motivation: 视频理解研究正从短片段转向长视频、多模态和知识密集型场景，现有方法缺乏统一视角来分析模型如何获取证据、保留上下文和产生输出。

Method: 提出三位一体的功能分析框架（观看、记忆、推理），并形式化定义为感知表示、记忆状态、推理轨迹和最终预测，进而按此组织代表性方法（如感知、记忆、推理模块）。

Result: 识别出时空感知、高效长视频处理、记忆建模、流式理解和忠实推理等关键挑战，并系统分类了各类方法、应用领域、数据集和基准。

Conclusion: 未来需发展可扩展、记忆感知和证据驱动的视频智能，并指出开放问题如长视频高效处理、多模态对齐和可靠推理。
...
### [30] [The Lipreading Gap: Do VSR Models Perceive Visual Speech Like Human Lipreaders?](https://arxiv.org/abs/2606.07435)
*Rishabh Jain,Naomi Harte*

Main category: cs.CV

TL;DR: VSR系统在单词准确率上超过人类，但错误模式与人类不同，更依赖于训练词频而非视觉信息，表明它们主要依赖语言线索而非视觉感知。


<details>
  <summary>Details</summary>
Motivation: 探究VSR模型超越人类唇读者是否意味着像人类一样感知视觉语音。

Method: 在MaFI数据集上比较三个VSR系统与人类基线，使用词、字符、音素和视位级别指标，分析错误模式、与词频和视觉信息性的关系，以及视位准确率和混淆矩阵。

Result: VSR模型总体准确率更高，但在不同单词上成功/失败；文本n-gram基线仅靠初始音素就与人类相当；词级错误由训练词频解释更好；模型在人类最难视位上增益最大；对视觉清晰度依赖性弱。
...
### [31] [Breaking the Lock-in: Diversifying Text-to-Image Generation via Representation Modulation](https://arxiv.org/abs/2606.06813)
*Dahee Kwon,Haeun Lee,Jaesik Choi*

Main category: cs.CV

TL;DR: DAVE通过衰减早期生成中的DC分量来增强文本到图像模型的多样性，无需额外训练或开销。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在固定提示下产生过于相似的样本，现有多样性增强方法成本高。

Method: DAVE是一种训练无关的表示级干预，在早期生成阶段选择性衰减DC分量。

Result: DAVE在不影响采样管道的情况下，以可忽略的开销提高了提示一致的多样性，同时保持图像质量。

Conclusion: 通过观察DC分量快速收敛导致轨迹锁定，DAVE有效解决了多样性问题，且保持效率。
...
### [32] [CFRNet: Cycle-Consistent Fixed-Point Training for Real-Time Blind Face Restoration on Consumer Embedded NPUs](https://arxiv.org/abs/2606.06850)
*Fuchen Li,Xinyang Wang,Yahui Zhang,Yuhan Chen,Jiahong Guo,Zhuohan Qin,Wenbo Ma*

Main category: cs.CV

TL;DR: 提出了一种轻量级盲脸修复网络CFRNet，通过循环一致固定点训练实现在消费级NPU上的高效高质量修复。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如GFPGAN、CodeFormer）依赖大型预训练生成先验和难以编译量化的操作，不适合消费级NPU；而小型卷积修复器运行快但容易过度平滑并在眼部等区域留下伪影。

Method: 提出CFRNet，2.0M参数ResNet风格修复器，采用循环一致固定点训练（CCFP），包括渐进多循环监督、幂等损失和再退化循环损失，训练后网络作为固定点算子，多次应用不改变结果。

Result: 在300张测试集上，三循环LPIPS 0.250（比单循环低31%），两循环PSNR和SSIM最佳；每循环23ms（INT8）在HiSilicon Hi3402 NPU上运行，其他基线无法编译；循环次数k作为质量旋钮无需重训练。

Conclusion: CCFP思想同样适用于更易部署的普通CNN，并在车载驾驶员监控板上实时运行。
...
### [33] [Unified Safe In-context Image Generation in Multimodal Diffusion Transformers via Restricting Unsafe Information Flows](https://arxiv.org/abs/2606.06875)
*Xiang Yang,Feifei Li,Mi Zhang,Geng Hong,Xiaoyu You,Mi Wen,Min Yang*

Main category: cs.CV

TL;DR: A training-free safe generation framework for DiTs that regulates unsafe semantics by analyzing attention dynamics and modulating attention on identified unsafe patches.


<details>
  <summary>Details</summary>
Motivation: Existing safety mechanisms designed for T2I or U-Net architectures are ineffective for DiT-based frameworks, especially in I2I editing tasks.

Method: UVR analyzes information flow in MM-Attn, identifies task-independent start-up stage where unsafe semantics emerge and can be localized, then applies targeted attention modulation and restricts harmful information flow on those patches.

Result: UVR achieves 91% erase rate in image synthesis and 77% in editing tasks, preserving visual quality with minimal degradation.

Conclusion: UVR sets state-of-the-art safety performance in DiT-based image generation and editing.
...
TL;DR: MVSegNet is a lightweight segmentation network for fetal ventriculomegaly assessment that achieves high boundary and measurement accuracy with low computational cost.


<details>
  <summary>Details</summary>
Motivation: Fetal ventriculomegaly assessment requires accurate segmentation of the lateral ventricle from ultrasound images, which is challenging due to acoustic shadowing, speckle noise, and poor contrast.

Method: MVSegNet uses a lightweight encoder-decoder architecture with multi-scale feature extraction and boundary-aware refinement, trained on 584 annotated transventricular ultrasound frames.

Result: MVSegNet achieves Dice=80.79%, IoU=68.47%, Hausdorff distance=4.07mm, atrial width MAE=3.40mm, with 2.31M parameters and 165.6 FPS on T4 GPU, outperforming six baselines on boundary and measurement metrics.

Conclusion: MVSegNet supports automated fetal ultrasound analysis by providing accurate segmentation with low computational cost.
...
### [36] [Differences in Detection: Explainability Where it Matters](https://arxiv.org/abs/2606.07503)
*Johannes Theodoridis,Johannes Maucher,Andreas Schilling*

Main category: cs.CV

TL;DR: 提出Differences in Detection (DnD)方法，通过比较两个目标检测模型识别和遗漏的ground truth标签的集合关系，直观揭示个别和共同错误，并可与错误类型结合用混淆矩阵分析。


<details>
  <summary>Details</summary>
Motivation: 标准指标如mAP和TIDE不能直接比较两个模型，缺乏直观性和详细的错误分析，因此需要更直接的比较方法。

Method: 基于相同匹配算法，计算两个模型都识别到的ground truth标签交集、各自独有的差集以及都遗漏的补集，结合错误类型用混淆矩阵分析检测错误的差异。

Result: 该方法能更直接直观地比较两个模型，揭示个别和共同错误，与错误类型结合时可自然分析检测错误差异。

Conclusion: DnD方法本身有价值，最佳应用之一是指导可解释性方法（如ODAM）关注基于结构化子集的度量相关例子。
...
TL;DR: AE-YOLO框架融合注意力引导自编码器和YOLO，在绝缘子缺陷检测上达到95.10% mAP，显著超越基线。


<details>
  <summary>Details</summary>
Motivation: 针对无人机图像中绝缘子缺陷检测面临的类别不均衡、尺度变化大和缺陷目标小等挑战。

Method: 在FPN-PAN颈部集成轻量瓶颈自编码器，主干使用CBAM注意力，引入方差最大化自编码器正则化，联合focal loss、CIoU loss和自编码器正则化训练，推理时采用WBF融合和自编码器置信度提升。

Result: 在Insulator-Defect Detection数据集上，AE-YOLO（EfficientNetV2主干）达到95.10% mAP@0.5、96.40%精度、93.80%召回，比最强YOLO基线分别高出5.0 mAP和6.7召回。

Conclusion: 该框架有效且适应性强，是无人机输电线路巡检中实用且可扩展的缺陷检测方案。
...
### [38] [JA-SIREN: Deterministic Initialization for Sinusoidal Networks via Spectral Matching](https://arxiv.org/abs/2606.06671)
*Mohammed Alsakabi,Kejia Hu,John M. Dolan,Ozan K. Tonguz*

Main category: cs.CV

TL;DR: JA-SIREN是一种确定性初始化方案，通过谱分析提高正弦INR的可重复性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有INR方法随机初始化导致性能不一致（变化超过2.5 dB），影响科学计算中的可重复性。

Method: 基于目标信号的离散正弦变换（DST）和Jacobi-Anger展开，推导两层正弦MLP的封闭形式权重，无需随机种子或超参数调整。

Result: 在Kodak数据集上平均PSNR达67.18 dB，比最佳基线提高21.30 dB，且零运行方差。

Conclusion: 谱信息初始化是比随机初始化更有效且可重复的替代方案。
...
### [39] [MMBU: A Massive Multi-modal Biomedical Understanding Benchmark to Probe the Perception Capabilities of Vision-Language Models](https://arxiv.org/abs/2606.06696)
...
TL;DR: MMBU是最大的生物医学视觉语言基准，覆盖35个子模态，评估发现高准确率可能掩盖视觉感知和领域泛化的缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有基准有限，无法充分评估视觉语言模型在生物医学图像上的细粒度感知和跨模态泛化能力。

Method: 构建MMBU基准，包含开放和封闭版本的未接地分类、接地分类和物体检测任务，覆盖35个子模态，并评估15个开源和2个前沿视觉语言模型。

Result: 医学适应带来性能提升，但高准确率可能掩盖模型在视觉感知和领域泛化上的不足。

Conclusion: 需要更全面的基准来揭示模型的真正能力，避免被高准确率误导。
...
TL;DR: AdaGRPO通过在线课程过滤和跨级别优势融合，改进了GRPO在流模型中的训练稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO在提示选择上随机且优势估计依赖组内统计，缺乏全局视角，导致学习循环与能力脱节。

Method: 提出AdaGRPO，包括在线课程过滤策略（动态追踪模型熟练度并选择匹配的提示）和跨级别优势融合（整合组内与全局优势）。

Result: 实验表明AdaGRPO持续提升性能并显著稳定GRPO训练。

Conclusion: AdaGRPO作为轻量即插即用模块，可有效集成到现有框架，推动性能提升并稳定训练。
...
### [43] [MotionEnhancer: Leveraging Video Diffusion for Motion-Enhanced Vision-Language Models](https://arxiv.org/abs/2606.06853)
*Yifan Xu,Chao Zhang,Ruifei Ma,Fei Gao,Zhifei Yang,Jiaxing Qi,Zhipeng Chen*

Main category: cs.CV

TL;DR: 提出MotionEnhancer，利用视频扩散模型的运动先验增强视觉语言模型的运动理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在细粒度运动理解上能力有限，因为它们主要关注高级静态语义结构和宏观事件逻辑，而视频扩散模型擅长建模动态运动模式。

Method: MotionEnhancer包含两个无参数模块：运动敏感头选择（MHS）和运动显著文本令牌识别（MTTI），通过注意力对齐从视频扩散模型中提取运动先验，无需额外训练参数或修改架构。

Result: 在两个运动级视频理解基准上，MotionEnhancer对最先进的视觉语言模型实现了一致改进，特别是在运动相关指标上。

Conclusion: MotionEnhancer提供了一种可扩展的解决方案，有效增强了VLM的运动理解能力，且无需额外参数或架构修改。
...
### [44] [LRMIL: Efficient Low-Resolution Multiple Instance Learning via High-Resolution Knowledge Distillation for Whole Slide Image Classification](https://arxiv.org/abs/2606.06864)
*Yonghan Shin,Won-Ki Jeong*

Main category: cs.CV

TL;DR: 本文提出高效低分辨率多实例学习（LRMIL）框架，通过两阶段知识蒸馏将高分辨率知识转移到低分辨率表示，在保持性能的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有MIL方法依赖高分辨率补丁的穷举提取和编码，难以捕获全局视觉线索且计算开销大。

Method: LRMIL采用两阶段蒸馏策略：补丁级跨分辨率蒸馏对齐低分辨率补丁嵌入，幻灯片级知识蒸馏训练低分辨率学生MIL模型，推理时仅使用低分辨率补丁。

Result: 在多个WSI基准测试中，LRMIL在实现更高效推理的同时持续优于最先进的MIL方法。
...
TL;DR: 提出Multi-FRuGaL框架，通过分解感知、自适应门控融合处理缺失模态数据，在头颈癌预后任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现实临床数据常缺失或稀疏获取某些模态，限制了标准多模态融合的效果。

Method: 基于逐模态编码器、信号分解层、输入条件门控网络和信息感知融合目标的分解感知自适应门控中间融合框架，能分离冗余与互补信号，选择性加权信息模态，在多重缺失下仍保持定义良好。

Result: 在HANCOCK和HECKTOR数据集上，AUC提升至0.8496（生存）、0.8102（复发），HPV分类AUC达0.975；C-index最高达0.7421（无复发生存）。

Conclusion: Multi-FRuGaL能学习判别且鲁棒的多模态表示，即使严重缺失模态时仍有效。
...
### [46] [FreeAnimate: Training-Free Human Image Animation with Preview-Guided Denoising](https://arxiv.org/abs/2606.06885)
*Yuan Zeng,Yujia Shi,Zongqing Lu,QingMin Liao*

Main category: cs.CV

TL;DR: FreeAnimate是一种无需训练的人类图像动画框架，通过利用扩散模型的内在能力实现时间一致性、身份保留和背景稳定。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要大量训练数据和资源，限制了泛化能力和可访问性。

Method: 采用训练无关框架，通过预览生成策略提供时间和结构先验，并引入Inversion-Boosted Attention和Reference-Anchored Self-Attention模块。

Result: 在生成质量上优于现有训练无关和训练方法，与最先进方法相当，且泛化能力强。

Conclusion: FreeAnimate有效降低了资源需求，同时保持了高生成质量和泛化能力。
...
### [47] [Diagnosing Visual Ignorance in Vision-Language Models](https://arxiv.org/abs/2606.06890)
*Runyu Zhou,Qi Zhang,Qixun Wang,Yisen Wang*

Main category: cs.CV

TL;DR: 本研究从内部机制和外部行为两方面分析了视觉语言模型的语言先验依赖问题，发现中间层无法有效检索视觉信息，后续层进一步抑制视觉信号，且大量基准测试示例在视觉严重损坏下仍可回答，表明现有基准存在视觉忽视现象。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型频繁依赖语言先验，但其内部机制和对基准评估的影响尚不充分了解。

Method: 内部机制采用反事实层替换和监督层MLP探测；外部行为引入基于多步高斯模糊的渐进视觉衰减度量。

Result: 中间层无法有效检索视觉信息，后层进一步抑制视觉信号；大量示例在严重或完全视觉模糊下仍可回答，表明基准可能奖励视觉忽视。

Conclusion: 语言先验依赖是系统性的路由失败，影响模型内部和基准有效性；未来需设计基于结构隔离或反事实数据的训练与评估协议以强制跨模态接地。
...
### [48] [Stream3D-VLM: Online 3D Spatial Understanding with Incremental Geometry Priors](https://arxiv.org/abs/2606.06891)
*Hanxun Yu,Xuan Qu,Lei Ke,Boqiang Zhang,Yuxin Wang,Jianke Zhu,Dong Yu*

Main category: cs.CV

TL;DR: 提出第一个在线3D视觉语言模型，实现实时空间理解。


<details>
  <summary>Details</summary>
Motivation: 现有3D大语言模型需完整场景或预定义视频，无法实时在线理解。

Method: 基于LLM的自回归流控制决定响应时机；轻量级视觉-空间特征融合模块注入时间对齐几何先验；即插即用的几何自适应体素压缩模块高效压缩视觉令牌；构建超百万在线时空3D问答对及29任务基准。

Result: 在在线和离线3D空间理解、推理和定位任务上显著优于专有和开源模型。

Conclusion: 该方法实现了高效实时在线3D空间理解，性能优越。
...
### [49] [Lighting-Aware Representation Learning under Controllable Lighting Variation](https://arxiv.org/abs/2606.06899)
*Lizhen Zhu,Charantej Reddy Pochimireddy,James Z Wang,Brad Wyble*

Main category: cs.CV

TL;DR: 提出光照感知表示学习框架，通过对比学习和辅助目标显式建模光照变化，提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 光照变化导致外观显著变化，现有方法仅通过数据增强实现不变性，未显式建模光照信息；受人类视觉理论启发，希望将光照变化作为训练信号而非干扰。

Method: 扩展对比学习，引入辅助目标捕捉光照依赖变化，联合学习语义一致且对光照敏感的表征。

Result: 在ImageNet、ExDark、PASCAL VOC上，图像分类和检测任务中，光照感知训练持续优于标准对比学习基线，且相同架构和预算；在监督学习和简单光照设置下也表现良好。

Conclusion: 表明方法能增强模型在复杂视觉环境和常规图像处理任务中的鲁棒性和适应性。
...
### [50] [Don't Pause: Streaming Video-Language Synchrony for Online Video Understanding](https://arxiv.org/abs/2606.06991)
*Zhenyu Yang,Kairui Zhang,Shengsheng Qian,Weiming Dong,Changsheng Xu*

Main category: cs.CV

TL;DR: LyraV is a live streaming assistant for online video understanding that achieves fine-grained video-language synchrony via a hierarchical control framework with FDTC and SToP.


<details>
  <summary>Details</summary>
Motivation: Existing Video-LLMs pause video perception while generating responses, breaking real-time synchrony and causing stutters.

Method: LyraV uses Frame-Driven Transition Controller (FDTC) for high-level semantic decisions on speaking timing and Streaming Token Pacer (SToP) for adaptive language generation rate, enabling per-frame incremental sub-budget decoding.

Result: LyraV preserves general understanding, achieves 98.29% synchrony with video playback and 3.89 FPS processing speed, and demonstrates dynamic reasoning over streaming tokens.

Conclusion: LyraV effectively interleaves incoming video frames with generated tokens, achieving fine-grained synchrony in streaming scenarios.
...
TL;DR: 提出ForensicConcept框架，通过提取可解释概念并跨骨干迁移，提高AI图像检测器的泛化性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有AI图像检测器对未见过的生成器泛化失败，且黑盒性质阻碍理解失败原因。

Method: 利用Transformer归因定位关键区域，聚类为概念码本，通过概念对齐投影产生可审计证据；引入CleanDIFT扩散特征并量化骨干对齐度（CKNNA），通过概念码本注入迁移知识。

Result: 在GenImage、GAN-family和Chameleon基准上一致优于先前方法，且CKNNA对齐度可预测迁移效果。

Conclusion: 该框架不仅提升了检测性能，还提供了可解释的取证证据，并解释了不同骨干网络的迁移性差异。
...
TL;DR: 提出TrioPose，一种基于扩散变换器的姿态驱动框架，通过三重流姿态感知DiT、可学习关系偏差掩码和姿态引导空间损失加权策略，实现复杂多人物文本到图像生成的高保真度和语义对齐。


<details>
  <summary>Details</summary>
Motivation: 现有UNet适配器难以处理长程空间依赖，多模态扩散变换器中简单信号拼接会破坏预训练潜在分布，导致复杂多人物场景中出现肢体扭曲和特征串扰。

Method: 提出TSPA-DiT将姿态作为独立模态，采用逐层激活和零初始化的双重残差注入；设计可学习关系偏差掩码将拓扑连接分类为细粒度物理状态，映射为连续注意力软约束；引入姿态引导空间损失加权，利用热图导出的误差图聚焦解剖监督于易扭曲区域。

Result: 在Human-Art上AP达64.33，比之前方法提升30%；在CrowdPose和OCHuman上均取得最优结果，显著提升视觉保真度和文本-图像语义对齐。

Conclusion: TrioPose在复杂多人物生成任务中达到新高度，为姿态引导文本到图像生成设立了新标准。
...
TL;DR: 提出了一种自适应的多度量数据清洗框架，无需手动阈值或噪声先验，在多个数据集上实现高召回率和精度提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手动阈值、噪声先验或单一度量，在复杂数据场景下不稳定。

Method: 将样本映射到低维特征空间，通过多度量聚类（如2D度量结合KNN局部不一致和k-means全局质心距离，或3D度量额外加入z归一化得分）自适应划分干净和噪声样本。

Result: 在CIFAR-10、MNIST、ImageNet-100上，对5%-40%对称噪声取得高召回率（ImageNet-100上40%噪声时召回率≥98%），后续训练精度显著提升。
...
### [54] [3DMorph: Single-Image-Guided Local 3D Shape Editing and Morphing](https://arxiv.org/abs/2606.07115)
*Tobias Preintner,Yunfei Deng,Phillip Müller,Sebastian Illing,Adrian König,Thomas Bäck,Elena Raponi,Niki van Stein*

Main category: cs.CV

TL;DR: 3DMorph是一个无需训练的框架，用于单图像引导的局部3D形状编辑和变形，能够将2D编辑转移到3D并保留未修改区域，还能生成中间形状。


<details>
  <summary>Details</summary>
Motivation: 现有3D编辑方法在局部编辑方面存在局限，多为全局、领域特定或需要复杂交互，且关注外观而非几何。

Method: 提出3DMorph，输入编辑后的2D图像，自动定位相关3D区域并将2D修改转移到3D，同时保留未修改区域；还支持生成原始和编辑对象之间的中间形状。

Result: 在引入的Delta3D基准上，3DMorph优于最先进的生成和编辑方法。

Conclusion: 3DMorph实现了将直观的2D编辑有效转换为3D，便于形状探索。
...
### [55] [When Recovery Matters: The Blind Spot of Surrogate Privacy in MLLM Editing](https://arxiv.org/abs/2606.07171)
*Siyuan Xu,Yibing Liu,Peilin Chen,Yung-Hui LI,Shiqi Wang,Sam Kwong*

Main category: cs.CV

TL;DR: 提出SPPE基准和两种方法，用于评估和恢复隐私保护图像编辑中的替代效果。


<details>
  <summary>Details</summary>
Motivation: 现有隐私保护方法输出的是编辑后的替代图像而非源图像，忽略本地恢复，导致隐私风险。

Method: 构建SPPE基准，包含36个隐私类别和65条编辑指令，定义两个任务：可编辑性评估（ERMA方法）和替代到源编辑恢复（C2E-S2SER方法）。

Result: ERMA在SRCC和PLCC上分别提升13.9%和12.3%；C2E-S2SER在SPPE上全部8个指标优于SOER。

Conclusion: SPPE和所提方法有效提升了隐私保护编辑的评估与恢复性能。
...
### [56] [Seeing Without Exposing: Adaptive Privacy Control for Open-World, Context-Hungry MLLMs](https://arxiv.org/abs/2606.07175)
*Siyuan Xu,Yibing Liu,Peilin Chen,Yung-Hui Li,Shiqi Wang,Sam Kwong*

Main category: cs.CV

TL;DR: 提出一种训练-free的隐私漂移方法APD，可在保护隐私的同时保留上下文，并在新基准AdaptShield上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型隐私保护方法依赖预定义敏感类别和固定策略，无法处理用户输入不可预测的敏感信息和隐私敏感的视觉上下文。

Method: 提出Anchored Privacy Drifting (APD)，一种无需训练的方法，将隐私敏感元素漂移到语义等价的替代，同时将上下文线索锚定到源图像。并引入AdaptShield基准，覆盖22个隐私类别，结合传统隐私指标和基于MLLM的上下文效用评估。

Result: 在Qwen2.5、Qwen3、InternVL3和InternVL3.5四个MLLM系列上，文本类别隐私提升10.4%，基于MLLM的评估提升8.5%。

Conclusion: APD在隐私净化和内容保留之间实现了平衡的改进，为多模态隐私保护提供了有效方案。
...
### [57] [OPTIMUS-Prime: Minimal and Sufficient Concept Explanations for Deep Vision Models](https://arxiv.org/abs/2606.07180)
*Arthur Hoarau,Chenrui Zhu,Vu Linh Nguyen*

Main category: cs.CV

TL;DR: OPTIMUS框架生成基于质蕴含理论的概念性视觉热图解释，同时满足充分性和最小性形式保证。


<details>
  <summary>Details</summary>
Motivation: 现有XAI方法在计算机视觉中缺乏形式化保证，导致实际效用与理论严谨性之间存在差距。

Method: 提出OPTIMUS框架，利用质蕴含理论生成视觉热图，确保解释的充分性（突出概念保证预测）和最小性（无冗余子集）。

Result: 在视觉分类基准上验证，OPTIMUS热图能自然且忠实地呈现模型决策相关的概念。

Conclusion: OPTIMUS解释满足充分性和最小性，实现了逻辑严密与视觉连贯的统一。
...
TL;DR: 提出DualGate-Net，通过可学习的前门控融合机制自适应整合组织先验，在OCELOT上提升细胞检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有组织感知方法使用静态融合策略可能传播噪声，需自适应整合上下文。

Method: 提出双编码器框架，结合ConvNeXtV2局部编码器和SegFormer全局编码器，通过可学习前门控融合调节组织先验，并引入前景重建分支和细胞度线索。

Result: 在OCELOT基准上，验证集macro F1为0.7722，测试集为0.7345，优于其他方法。

Conclusion: 自适应先验整合能有效提升组织病理学细胞检测的鲁棒性和准确性。
...
### [59] [Does Appearance Help? A Systematic Study of Image-Based Re-Identification in Online 3D Multi-Pedestrian Tracking](https://arxiv.org/abs/2606.07233)
*Eduardo Borges,Luís Garrote,Urbano J. Nunes*

Main category: cs.CV

TL;DR: 本文提出一种轻量级投影框架，将图像ReID与LiDAR 3D MOT结合，通过级联匹配策略在保持低延迟的同时恢复遮挡轨迹，防止身份切换。


<details>
  <summary>Details</summary>
Motivation: LiDAR 3D MOT仅靠几何信息在长时间遮挡或拥挤人群中难以区分目标，而现有RGB ReID方法计算量大，影响实时性。

Method: 采用轻量级投影框架解耦几何和外观建模，分析轻量级CNN和ViT的特征提取架构，并评估包括级联匹配在内的多模态数据关联策略。

Result: 在KITTI数据集上，简单的线性融合会因视觉噪声降低性能，而级联匹配策略成功恢复被遮挡轨迹，防止身份切换，精度不变。

Conclusion: 轻量级架构能够在导航所需的低延迟和社交意识所需的区分能力之间达到最优平衡。
...
### [60] [Geometric-Aware Hypergraph Reasoning for Novel Class Discovery in Point Cloud Segmentation](https://arxiv.org/abs/2606.07280)
*Zihao Zhang,Aming Wu,Yang Li,Yahong Han,Jialie Shen*

Main category: cs.CV

TL;DR: 提出一种基于超图的新颖类发现方法，通过高阶关联和几何感知原型提升点云分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖成对关联，难以捕获复杂关系且忽视几何信息，导致分割不准确。

Method: 引入超图框架建模高阶关联，并提出几何感知原型增强几何信息表示。

Result: 在SemanticKITTI和SemanticPOSS数据集上验证了有效性和优越性。

Conclusion: 该方法通过超图和几何原型实现了更精确的点云语义分割。
...
### [61] [Varifold Moment Invariants for Sustainable and Explainable Contour Feature Extraction](https://arxiv.org/abs/2606.07333)
*G. Longari,J.-C. Alvarez Paiva,A.B. Tumpach*

Main category: cs.CV

TL;DR: Varifold Moments Invariants (VMI) 是一种统一的矩不变量框架，结合区域、边界和切线几何，通过轻量分类器实现高效高精度形状分类。


<details>
  <summary>Details</summary>
Motivation: 现有矩不变量难以结合几何与边界信息且计算成本高，VMI旨在提供高辨别力、几何可解释且计算高效的特征框架。

Method: 提出Varifold Moments Invariants特征，融合区域、边界和切线几何，并与随机森林或多层感知器分类器结合。

Result: 在叶子、物体、细胞等多数据集上，以少量几何特征超越基于轮廓的最新方法，同时大幅降低计算成本，可运行于轻量设备。

Conclusion: VMI框架能生成几何可解释的高辨别力特征，适用于资源受限设备上的实时分类任务。
...
### [62] [Mind the Gap: Disentangling Performance Bottlenecks in Video Instance Segmentation](https://arxiv.org/abs/2606.07394)
*Danial Hamdi,Fardin Ayar,Mahdi Javanmardi*

Main category: cs.CV

TL;DR: 提出一个基于整数线性规划（ILP）的诊断框架，以隔离视频实例分割（VIS）中分类、分割和跟踪的错误来源，并发现跟踪不稳定是性能下降的主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前VIS中分类、分割和跟踪目标的联合评估中，各自对性能损失的贡献不透明，需要一种诊断方法来揭示错误来源。

Method: 提出诊断框架，将身份和类分配建模为整数线性规划（ILP），生成模型无关的oracle以分层隔离每个错误源；并引入TrackLens可视化工具，将差距大小转化为可观察的查询级故障模式。

Result: 对七种VIS方法的分析表明，对在线方法而言，跟踪不稳定性是关键瓶颈，在严重遮挡下AP差距超过20，且随视频长度和实例密度急剧增加；语义分类在标准基准上有意义，但在跟踪失败最严重的地方影响可忽略；更强的骨干网络提升默认分数，但AP跟踪差距基本不变，证实时间脆弱性是算法性的而非表征性的。

Conclusion: 这些工具为针对VIS的核心挑战——鲁棒的长期时间关联提供了系统基础。
...
### [63] [OpenGlass: Open-Source Smart Glasses for On-Device Event-Based Gesture Recognition](https://arxiv.org/abs/2606.07431)
*Pietro Bonazzi,Julian Moosmann,Ahmet Celik,Philipp Mayer,Michele Magno*

Main category: cs.CV

TL;DR: 提出并实现了一个开源模块化智能眼镜平台，支持事件和帧相机，功耗优化，在GAP9上运行ML推理，在手势识别中达到83.94%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有智能眼镜受限于功耗、内存和计算，缺乏支持事件视觉和嵌入式ML的开源硬件平台。

Method: 设计模块化智能眼镜，使用FPC interposer兼容事件/帧相机；硬件软件协同电源管理（nRF5340唤醒、GAP9 SoC按需供电）；采用R(2+1)D模型和极性分离事件直方图进行手势识别。

Result: 200mAh电池持续11.8小时ML推理，手势识别准确率83.94%（F1=0.781），延迟33.9ms，数据增强和去歧义提升8.9个百分点。

Conclusion: 该开源平台降低了实验门槛，推动了紧凑型智能眼镜的快速发展。
...
### [64] [Implicit Data Synthesis for Contrastive Unsupervised Data Augmentation](https://arxiv.org/abs/2606.07498)
*Patrick Kage,Trevor Hedges,N. Siddharth,Pavlos Andreadis*

Main category: cs.CV

TL;DR: 提出通过扰动网络权重而非数据生成对比样本的方法，在雷达流星观测上提升性能。


<details>
  <summary>Details</summary>
Motivation: 科学观测数据标注困难，对比学习有用但数据增强可能改变数据本质结构。

Method: 在SimCLR框架中，通过扰动网络权重生成对比样本，而非进行数据空间增强。

Result: 在雷达流星观测数据集上，该方法在匹配协议下表现出性能提升。

Conclusion: 权重扰动对比学习能更好地保留科学数据结构，适用于科学观测场景。
...
### [65] [UniSHARP: Universal Sharp Monocular View Synthesis](https://arxiv.org/abs/2606.07514)
*Meixi Song,Dizhe Zhang,Hao Ren,Ruiyang Zhang,Bo Du,Ming-Hsuan Yang,Lu Qi*

Main category: cs.CV

TL;DR: UniSHARP extends SHARP to universal monocular rendering across diverse camera systems by aligning images in a unified omnidirectional latent space.


<details>
  <summary>Details</summary>
Motivation: To overcome the pinhole-specific assumptions of SHARP and enable rendering for conventional perspective, wide-FOV, fisheye, and omnidirectional cameras.

Method: UniSHARP performs implicit alignment in feature and Gaussian spaces using a ray-based universal representation with Gaussian primitives along rays and radial distances, and jointly decodes 2D semantic and 3D spatial features from UniK3D-inspired encoders.

Result: UniSHARP outperforms alternative methods by a large margin on a newly constructed benchmark stratified by field of view across diverse scenes.

Conclusion: UniSHARP effectively achieves universal monocular rendering for a continuum of camera systems.

Abstract: In this work, we focus on extending SHARP, the popular photorealistic view synthesis method, for universal monocular rendering across a continuum of camera systems, from conventional perspective cameras to wide-field-of-view, fisheye and omnidirectional panoramic settings. To overcome the pinhole-specific assumptions of SHARP, our key idea is to align various images in a unified omnidirectional latent space. Thus, we propose UniSHARP, which performs implicit alignment in both feature and Gaussian spaces. Specifically, Gaussian primitives are arranged along rays and radial distances in a ray-based universal representation, while 2D semantic and 3D spatial features extracted from UniK3D-inspired encoders are jointly decoded to generate the complete Gaussian cloud. To comprehensively evaluate our method, we construct a benchmark covering diverse imaging systems across various scenes. The benchmark is further stratified by field of view (FoV) to enable fine-grained assessment of the universal monocular rendering task. Extensive experiments on the proposed benchmark demonstrate the effectiveness of UniSHARP, outperforming alternative methods by a large margin. The project page can be found at:

</details>


### [66] [Applying Deep Learning for cockpit segmentation in the context of mixed reality](https://arxiv.org/abs/2606.06520)
...
### [68] [From Pixels to Newtons: Predicting In Vivo Joint Contact Forces from Monocular Video](https://arxiv.org/abs/2606.06631)
*Jessy Lauer*

Main category: cs.CV

TL;DR: 提出一种无需物理的管线，从未校准单目视频预测3D髋膝接触力，与肌肉骨骼模拟精度相当。


<details>
  <summary>Details</summary>
Motivation: 关节接触力对植入物寿命、软骨健康和康复结果至关重要，但之前只能通过有创测量在少数患者中获取。

Method: 每帧恢复参数化身体网格，编码为运动特征，通过自适应调制变压器解码为力，统一髋和膝模型，无需标记、力板等。

Result: 在26名患者25类活动中，RMSE髋0.32±0.08 BW、膝0.23±0.03 BW，匹配模拟精度，零样本迁移优于先前方法，纯视频特征保持精度。

Conclusion: 建立了单目视频估计关节负荷的可行性，可追溯分析临床记录、初级保健筛查和家庭康复跟踪。
...
### [71] [S23DR 2026 Winning Solution](https://arxiv.org/abs/2606.06695)
*Jan Skvrna,Miroslav Purkrabek,Lukas Neumann*

Main category: cs.CV

TL;DR: A winning solution for structured 3D wireframe reconstruction using flow-matching DiT with conditional vertex tokens.


<details>
  <summary>Details</summary>
Motivation: To achieve high-quality structured 3D wireframe reconstruction from sparse SfM data.

Method: Treats vertices as conditional set, denoises 64 vertex tokens with flow-matching DiT conditioned on Perceiver-style scene tokens; uses global pass for coarse structure, hull-cropped second pass for refinement, and multi-sample consensus step.

Result: Ranked first on private leaderboard with HSS = 0.654.

Conclusion: The proposed method achieves state-of-the-art performance on the S23DR 2026 challenge.
...
### [72] [MedSIGHT: Towards Grounded Visual Comprehension in Medical Large Vision-Language Models](https://arxiv.org/abs/2606.06760)
*Aofei Chang,Le Huang,Alex James Boyd,Parminder Bhatia,Taha Kass-Hout,Fenglong Ma,Cao Xiao*

Main category: cs.CV

TL;DR: MedSIGHT是一个统一医学大视觉语言模型框架，通过Region Perceiver和区域码本实现结构化像素级理解，在医学理解和分割任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型难以统一视觉理解和医学图像分割，缺乏连接视觉发现与语义解释的临床推理能力。

Method: 提出Region Perceiver模块编码空间信息到语言模型表示空间，引入医学区域码本作为符号表示，通过渐进式训练策略对齐模块，实现端到端空间定位。

Result: 仅用72K多模态指令对训练，在多种成像模态的医学理解和分割任务上均达到最先进性能。

Conclusion: MedSIGHT通过结构化空间理解有效统一了视觉发现与语义解释，为临床推理提供了框架。
...
### [73] [FS-DVS: A Frequency-Selective Dynamic Visual Sensing Paradigm for Enhancing Information Completeness](https://arxiv.org/abs/2606.06856)
*Feiyu Ji,Xiaokang Yang,Xiaoyun Yuan*

Main category: cs.CV

TL;DR: 提出FS-DVS，通过可学习空间滤波器模拟视网膜细胞聚合，实现频率选择性事件触发，提升性能并符合人类对比敏感度函数。


<details>
  <summary>Details</summary>
Motivation: 传统DVS缺乏空间整合，没有对比敏感度函数，导致亚阈值信号损失和信息不完整。

Method: 提出FS-DVS，在事件触发前集成可学习空间滤波器，通过可微分事件仿真框架端到端优化。

Result: 学习到的滤波器自发演化为中心-环绕模式，强调中频成分，符合人类CSF，在目标检测和动作识别上取得性能提升。

Conclusion: 相比增加灵敏度或后处理，该方法实现选择性信息增强和高噪声鲁棒性，为下一代神经形态传感器提供生物合理蓝图。
...
### [74] [EgoPressDiff: Multimodal Video Diffusion for Egocentric UV-Domain Hand-Pressure Estimation](https://arxiv.org/abs/2606.06872)
*Yuan Zeng,Zilue Gao,Yujia Shi,Zongqing Lu,Wenming Yang,QingMin Liao*

Main category: cs.CV

TL;DR: EgoPressDiff is a conditional video diffusion framework that generates UV-pressure maps from egocentric views, achieving state-of-the-art results in hand-surface contact pressure estimation.


<details>
  <summary>Details</summary>
Motivation: Existing methods discretize pressure signals and process frames independently, leading to quantization errors and temporal inconsistencies.

Method: The approach uses a multi-modal conditioning strategy with PoseNet and Vertex Encoder to extract features from hand pose and 3D mesh vertices, combined with depth information, and a Distribution-Calibrated Spatial Layer to fuse heterogeneous features.

Result: EgoPressDiff achieves state-of-the-art on EgoPressure, improving Volumetric IoU by over 34% relative to prior baseline, reducing MAE, and maintaining high temporal accuracy.

Conclusion: The framework addresses quantization errors and temporal inconsistencies, advancing egocentric pressure estimation for AR/VR, robotic imitation, and ergonomic analysis.
...
### [75] [ARAPDiffusion: ARAP Regularization for Diffusion-Based Deformable Shape Space Learning](https://arxiv.org/abs/2606.06887)
*Haibo Liu,Jinghan Ke,Haitao Yang,Xiangru Huang,Georgios Pavlakos,Qixing Huang*

Main category: cs.CV

TL;DR: 提出ARAPDiffusion，将ARAP变形模型作为正则化损失注入潜在扩散，减少对大量3D数据的需求，改进形状生成。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型依赖大量3D训练数据，而ARAPDiffusion通过注入ARAP正则化减少数据需求。

Method: 利用ARAP模型改进编码器/解码器和LD模型，训练过程交替使用合成分布和形状解码器来优化。

Result: 无条件与条件形状生成实验显示优于基线方法。

Conclusion: ARAPDiffusion能有效学习变形形状空间，减少数据需求并提升生成质量。
...
### [76] [LUCID: Learning Unified Control for Image Deflaring and Exposure Mastery in Nighttime Photography](https://arxiv.org/abs/2606.06901)
*Tingyu Yang,Yuan Cheng,Xiaoyun Yuan*

Main category: cs.CV

TL;DR: LUCID是一个统一的夜间图像恢复框架，将恢复过程重新定义为连续可控的过程，通过flare disentanglement和扩散模型实现真实夜间场景的优异性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法孤立处理夜间场景中的flare和噪声退化，忽视了它们之间的纠缠；LUCID旨在通过统一框架填补这一空白。

Method: LUCID包含flare disentanglement模块和扩散驱动模块，并采用四模式训练策略，结合classifier-free guidance实现用户可控的恢复和HDR重建。

Result: 在多样化的真实夜间场景中，LUCID始终优于现有方法。

Conclusion: LUCID通过将夜间恢复视为连续可控过程，有效处理纠缠退化，并提供了灵活的用户控制能力。
...
### [77] [Beyond Skeletons: Learning Animation Directly from Driving Videos with Same2X Training Strategy](https://arxiv.org/abs/2606.06903)
*Yuan Zeng,Yujia Shi,Yuhao Yang,Dongxia Liu,Zongqing Lu,Wenming Yang,Qingmin Liao*

Main category: cs.CV

TL;DR: DirectAnimator是一种无需姿态提取的人体图像动画框架，通过引入驱动线索三元组和CueFusion DiT块实现鲁棒控制，并采用Same2X训练策略处理跨身份场景，在视觉质量和身份保持上达到SOTA，且计算资源更少。
...
Method: DirectAnimator框架绕过姿态提取，直接从原始驱动视频学习。它引入Driving Cue Triplet（姿态、面部、位置线索）以捕获运动、表情和对齐，并通过CueFusion DiT块融合这些线索进行可靠控制。此外，采用Same2X训练策略，当驱动和参考身份不同时，对齐跨ID特征与同ID数据，以正则化优化并加速收敛。

Result: 实验表明DirectAnimator在视觉质量和身份保持上达到SOTA，对遮挡和复杂关节鲁棒，且计算资源更少。

Conclusion: DirectAnimator通过绕过姿态提取和设计有效的线索融合与训练策略，实现了高效鲁棒的人体图像动画，具有优越的性能和资源效率。
...
### [78] [DRIFT: From Robustness Gaps to Invariance Manifolds for AI-Generated Image Detection](https://arxiv.org/abs/2606.06918)
*Abhishek Ameta,Sayan Banerjee,Shreyas Pandith,Harshit,Ankita Chatterjee,Akshay Janardan Bankar,Amit Satish Unde*

Main category: cs.CV

TL;DR: 通过学习真实图像的结构化不变性流形，并在冻结的视觉基础模型上添加轻量级投影头，实现开放世界中的AI生成图像检测。


<details>
  <summary>Details</summary>
Motivation: 现有检测方法在开放世界中对未知生成器泛化能力不足，且训练-free方法缺乏对检测任务的原则性适应。

Method: 基于冻结的视觉基础模型，引入轻量级投影头将表示空间分解为鲁棒和脆弱子空间；训练鲁棒子空间抑制物理变换，脆弱子空间保留对编辑的敏感性；使用结构化排序边距实现层次分离，并通过多尺度块级漂移生成双通道不变性签名。

Result: 在未知生成器和分辨率上表现出强开放世界泛化，一致优于训练-free鲁棒性基线，并提供可解释的不变性违规定位图。

Conclusion: 通过结构化不变性流形学习，该方法有效提升了开放世界AI生成图像检测性能并提供了可解释性。
...
### [79] [When is 3D Worth It? A Resource-Performance Frontier for CNNs and Transformers in Lung CT](https://arxiv.org/abs/2606.06950)
...
TL;DR: 2D and 2.5D inputs provide a more reliable trade-off between performance, stability, and computational efficiency than full 3D representations for lung cancer screening classification.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether performance gains from 3D models justify their computational cost and complexity by systematically comparing input dimensionality (2D, 2.5D, 3D) under a fixed protocol.

Method: Compare CNNs and Vision Transformers with 2D, 2.5D, and 3D inputs on a leakage-free NLST cohort (n=1,977) and LIDC-IDRI data using a fixed training protocol.

Result: 2.5D CNN achieves best ROC-AUC of 0.682 (95% CI [0.546, 0.799]), but 3D CNNs show threshold instability and transformers exhibit degenerate predictions; all confidence intervals overlap.

Conclusion: 2D and 2.5D inputs offer more reliable trade-offs among performance, stability, and efficiency than 3D for imbalanced lung cancer screening classification.
...
### [80] [CL-CLIP: CLIP-Based Continual Learning Framework with Cost-Volume Category Decoupling for Object Detection](https://arxiv.org/abs/2606.06978)
*Zihan Liu,Yuguang Yang,Shengjie Su,Jianing Pang,Linlin Yang,Chunyu Xie,Nikolai Yu. Zolotykh,Baochang Zhang*

Main category: cs.CV

TL;DR: CL-CLIP是一种基于CLIP的持续目标检测框架，通过成本体积引导的类别解耦与多专家RoI头，在持续微调中缓解灾难性遗忘，在PASCAL VOC和MS-COCO上取得先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇检测器在持续更新时会严重遗忘旧知识，失去零样本检测能力，亟需一种能同时适应新类别并保持旧知识的持续学习方法。

Method: 遵循CAT-Seg，计算CLIP图像-文本相似度成本体积以生成类别响应图，将共享区域特征解耦为类别特定路径，并通过多专家RoI头分别处理。

Result: 在PASCAL VOC和MS-COCO上，CL-CLIP显著优于F-ViT基线，与现有持续目标检测器性能相当，特别是在新类别适应性和基类性能保持上表现突出。

Conclusion: CL-CLIP成功赋予开放词汇检测器更强的持续学习能力，实现了新类别学习与旧知识保持的有效平衡。
...
### [81] [AsyncPatch Diffusion: spatially-flexible image generation](https://arxiv.org/abs/2606.07079)
*Samuele Papa,Valentin De Bortoli,Guillaume Couairon,Daniel Sýkora,Romuald Elie,Klaus Greff*

Main category: cs.CV

TL;DR: 异步贴片扩散（AsyncPatch Diffusion）通过为不同区域分配不同的噪声水平，实现了空间自适应生成，并在无需微调的情况下适用于图像修复。


<details>
  <summary>Details</summary>
Motivation: 标准扩散模型对整个样本使用单一噪声水平，强制所有空间区域遵循相同的去噪轨迹，限制了灵活性。

Method: 提出AsyncPatch Diffusion框架，为不同输入维度（如图像像素或潜在标记）分配不同噪声水平；设计受控噪声采样器以平衡噪声配置；引入输入指导以提高局部一致性。

Result: 在ImageNet 256和LSUN上达到与常规扩散相当的生成质量，同时天然适合修复任务，无需特定任务微调。

Conclusion: 异步贴片扩散支持更丰富的异质去噪轨迹，并实现了自适应生成策略，如不确定性引导加速和自回归采样。
...
### [82] [Native3D: End-to-End 3D Scene Generation via Unified Mesh-Texture Modeling and Semantic Alignment](https://arxiv.org/abs/2606.07117)
*Yibo Liu,Ziwei Zhang,Haozhou Pang,Menghao Li,Lanshan He,Gan Qi*

Main category: cs.CV

TL;DR: Native3D是首个完全绕过2D中间表示的端到端3D场景生成框架，通过统一的网格-纹理联合表示和3D REPA Loss，实现了高质量的3D场景生成与编辑。


<details>
  <summary>Details</summary>
Motivation: 传统方法将3D表示适应到2D领域以利用预训练扩散模型，导致几何结构变形和纹理细节退化等域适应问题。

Method: 提出统一的网格-纹理联合表示，通过Transformer场景编码器同时建模几何和纹理；并设计3D表示对齐损失（3D REPA Loss），用对比学习对齐潜在空间的多层语义表示。

Result: Native3D在生成质量和编辑灵活性上均优于现有方法。

Conclusion: 为3D场景编辑提供了一种无需2D中间表示的端到端解决方案。
...
### [83] [Consistent-Inversion: Reverse Consistency Guidance for Structure-Preserving Visual Editing](https://arxiv.org/abs/2606.07145)
*Xiaocheng Lu,Jingcai Guo,Song Guo*

Main category: cs.CV

TL;DR: Consistent-Inversion是一种训练无关的反向一致性指导框架，用于结构保持的视觉编辑，通过检查目标轨迹是否能反转回源轨迹来提供校正信号，从而提高背景和结构保真度。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关的编辑器依赖于反转，但反转会产生轨迹不匹配，可能损害背景/布局细节或过度约束编辑。

Method: Consistent-Inversion构造辅助目标侧噪声表示，执行源引导的反向去噪，利用反向一致性差异作为早期目标去噪步骤的校正信号。

Result: 在PIE-Bench上，Consistent-Inversion在统一SD3.5协议下提高了背景和结构保真度，同时保持目标提示对齐。

Conclusion: 该方法无需更新模型参数，与基于反转的编辑器兼容，且稀疏应用时仅引入少量推理开销。
...
### [84] [Textual Supervision Enhances Geospatial Representations in Vision-Language Models](https://arxiv.org/abs/2606.07172)
...
TL;DR: 本文分析了视觉-only、视觉语言和多模态基础模型的地理空间表示，发现文本监督提升了空间准确性，表明语言是多模态地理空间AI的关键。


<details>
  <summary>Details</summary>
Motivation: 地理空间理解在机器学习系统中至关重要但未被充分探索，特别是在图像地理定位和空间推理任务中。

Method: 分析三种模型家族（ViT、CLIP、LLaVA等）的地理空间表示，通过评估按可定位程度分组的图像聚类（人物、地标、日常物品）来揭示空间准确性差距。

Result: 文本监督增强了地理空间表示的学习，并揭示了空间准确性的系统性差距。
...
### [85] [EvoGS: Constructing Continuous-Layered Gaussian Splatting with Evolution Tree for Scalable 3D Streaming](https://arxiv.org/abs/2606.07179)
*Yuang Shi,Simone Gasparini,Géraldine Morin,Wei Tsang Ooi*

Main category: cs.CV

TL;DR: EvoGS 是一种基于演化树的连续分层表示，用于流式 3D 高斯泼溅，能消除冗余并实现平滑质量过渡。


<details>
  <summary>Details</summary>
Motivation: 现有渐进式方法依赖离散分层，层间结构独立导致误差累积、严重冗余和质量波动。

Method: 提出 EvoGS，采用演化树结构，通过类似小波的父子细化产生更精细细节，子节点可结构性地纠正祖先误差，从而得到稀疏且高压缩性的层间信号。

Result: 实验表明，EvoGS 将冗余从超过 65% 降至 25% 以下；与最先进基线相比，传输负载最多降低 2.4 倍，GPU VRAM 占用最多降低 5.5 倍，并实现适合实时自适应流媒体的平滑质量过渡。

Conclusion: EvoGS 作为首个连续分层表示，有效解决了流式 3D 高斯泼溅中的冗余和质量控制问题，显著优于现有方法。
...
### [86] [AdaTok: Self-Budgeting Image Tokenization with Quality-Preserving Dynamic Tokens](https://arxiv.org/abs/2606.07185)
*Xiaocheng Lu,Yuxi Chen,Jie Zhang,Jian Liu,Jingcai Guo,Fangqi Zhu,Tao Han,Song Guo*

Main category: cs.CV

TL;DR: 提出AdaTok，一种自预算离散1D图像分词器，通过表示-分配协同设计，在单次前向传播中根据图像内容动态调整令牌数量，平衡了保真度和效率。


<details>
  <summary>Details</summary>
Motivation: 现有图像分词器使用固定令牌数，无法适应视觉复杂度的异构性，导致简单图像浪费令牌而复杂图像信息不足。弹性分词器虽可调整长度，但通常将令牌数作为外部输入而非分词器自身输出。

Method: AdaTok结合优先表示学习（嵌套尾部掩码排序令牌+多头LoRA解码器头解决语义偏移）与自适应令牌分配（轻量级确定性GRPO策略在候选预算上选择），并通过动态Pareto加权平衡保真度与效率。

Result: 在ImageNet-1K上，AdaTok-Full（256令牌）rFID 1.31；AdaTok-Adaptive平均使用118令牌，rFID 1.50，优于离散1D基线。自回归生成中吞吐量提升约2.1倍。

Conclusion: 视觉令牌计数可以作为内容条件输出学习，而非固定超参数，实现自适应预算的分词器能在保持高性能的同时显著提升效率。
...
### [87] [ExMesh: EXplicit Mesh Reconstruction with Topology Adaptation](https://arxiv.org/abs/2606.07288)
*Chuanjin Fan,Lifan Wu,Wenjie Chang,Hanzhi Chang,Wenfei Yang,Tianzhu Zhang*

Main category: cs.CV

TL;DR: ExMesh通过可微优化与离散拓扑更新直接优化显式网格，实现从粗到细的重建，在精度、效率和简洁性上取得平衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖中间表示和后处理，导致伪影和碎片几何；直接优化显式网格面临自适应拓扑细化和UV一致性的挑战。

Method: 提出ExMesh框架，集成可微优化与离散拓扑更新，包括自适应顶点分裂合并和实时UV维护，实现粗到细优化。

Result: 实验表明ExMesh在精度、计算效率和网格简洁性上取得平衡。

Conclusion: ExMesh成功解决直接优化显式网格的挑战，为高质量网格重建提供了有效方案。
...
### [88] [AnchorWorld: Embodied Egocentric World Simulation with View-based Evolution Customization](https://arxiv.org/abs/2606.07326)
*Yu Li,Menghan Xia,Gongye Liu,Xintao Wang,Conglang Zhang,Lei Ke,Yuxuan Lin,Ruihang Chu,Pengfei Wan,Kun Gai,Yujiu Yang*

Main category: cs.CV

TL;DR: 提出了AnchorWorld框架，通过3D人体运动和外部视点辅助训练增强交互完整性，并结合锚定视图与文本描述实现世界定制，在实验中显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 当前交互式世界建模缺乏实践所需的多功能可控性，特别是自我中心模拟中的交互完整性和世界定制能力不足。

Method: 利用3D人体运动作为主要交互模态，引入解耦的外部视点进行辅助训练以补充自我中心视图中缺失的身体部位信息；同时提出在统一世界坐标系中定义锚定视图并配合文本描述来定制自我演化的世界。

Result: AnchorWorld在多个指标上显著超越最先进基线，消融实验验证了关键设计的有效性，定制方案展现出良好的时空几何一致性并严格遵循预设演化动态。

Conclusion: AnchorWorld框架通过增强交互完整性和灵活定制机制，有效提升了交互式世界建模的性能和可控性。
...
### [89] [Skill-3D: Evolving Scene-Aware Skills for Agentic 3D Spatial Reasoning](https://arxiv.org/abs/2606.07436)
*Haoyuan Li,Zhengdong Hu,Jun Wang,Hehe Fan,Yi Yang*

Main category: cs.CV

TL;DR: 提出Skill-3D框架，通过场景记忆和场景感知技能实现自我进化的工具使用策略，显著提升3D空间推理中工具利用率。


<details>
  <summary>Details</summary>
Motivation: 现有3D推理代理的工具使用存在偏见和误用，且对所有场景采用统一策略，忽略了场景异质性，导致代理范式提升有限。

Method: Skill-3D框架包含场景记忆存储轨迹、从成功轨迹蒸馏场景感知技能并附加失败教训，当类似场景出现时注入技能引导代理，新轨迹的成败进一步更新技能和记忆，形成共同进化。

Result: 在VSI-Bench上工具利用率从39%提升至78%，在MMSI-Bench上提升Gemini-3-Flash 67%，在VSI-Bench上后训练提升Qwen3-VL-8B 43%。

Conclusion: Skill-3D通过场景感知的技能学习有效改善了代理的工具使用，推动了3D空间推理性能的大幅提升，并具有通用性。
...
### [90] [Streaming Video Generation with Streaming Force Control](https://arxiv.org/abs/2606.07508)
*Hanhui Wang,Yiming Xie,Haiwen Feng,Zhaoyang Lv,Shenlong Wang,Huaizu Jiang*

Main category: cs.CV

TL;DR: StreamForce是一个因果统一的流式视频生成框架，通过连续力输入实现物理可控的视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有模型对不同类型的力训练单独模型，假设固定力或依赖非因果处理，缺乏统一性和因果性。

Method: 设计了统一的力表示作为控制信号，并开发了力可控视频生成的蒸馏流程，结合自回归效率和力响应性。

Result: 在单GPU上以高达16.6 FPS运行，在力遵循和运动真实性方面达到最先进性能。

Conclusion: StreamForce实现了物理可控的视频生成，具有稳定的光度和动态真实性，提供了一个因果统一的框架。
...
### [91] [TA-RAG: Tone-Aware Retrieval-Augmented Generation for Peer-Support Health Communication](https://arxiv.org/abs/2606.06794)
*Yong-Bin Kang,Anthony McCosker*

Main category: cs.CL

TL;DR: TA-RAG is a prompt-based tone-aware RAG framework that improves communication quality in HIV peer support by adding tone control components.


<details>
  <summary>Details</summary>
Motivation: Factual grounding alone is insufficient for sensitive peer-support health communication; responses must be accessible, stigma-free, empathetic, and tailored to the recipient.

Method: TA-RAG is a lightweight, prompt-based tone-aware RAG framework that embeds explicit tone control via four components: stigma-free rewriting, readability adjustment, recipient adaptation, and empathy rephrasing, without model fine-tuning.

Result: TA-RAG's components improve targeted communication quality while preserving key content.

Conclusion: Prompt-based tone control is a potential direction for making RAG outputs suitable for sensitive peer-support health communication.
...
### [92] [Evidence Graph Consistency in Retrieval-Augmented Generation: A Model-Dependent Analysis of Hallucination Detection](https://arxiv.org/abs/2606.06748)
*Jianru Shen*

Main category: cs.CL

TL;DR: 提出证据图一致性（EGC）框架，通过构建局部证据图并计算五个结构一致性度量来检测RAG中的幻觉，但在不同模型家族中表现方向相反，表明不能作为模型独立的检测信号。


<details>
  <summary>Details</summary>
Motivation: 现有检测方法依赖生成答案与检索段落之间的平面相似度，忽略了证据片段与答案主张之间的结构关系。

Method: EGC框架构建每个回答的局部证据图，并计算五个结构一致性度量作为幻觉指标。

Result: 在RAGTruth的完整问答子集上评估六个LLM（5,767个回答），发现模型家族分裂：图一致性特征在Llama-2模型中呈现预期的诊断方向，但在GPT-4、GPT-3.5和Mistral-7B中系统性地反转。

Conclusion: 这种反转表明不同模型家族存在定性不同的幻觉模式，并且基于嵌入的图一致性不能作为模型独立的幻觉检测信号。
...
### [93] [A Four-Condition Diagnostic Protocol for Evidence Utilization in Long-Context and Retrieval-Augmented Language Models](https://arxiv.org/abs/2606.06758)
*Haizhou Xia*

Main category: cs.CL

TL;DR: 提出一种匹配的四条件证据可用性协议（ONCU），用于诊断长上下文或检索增强语言模型是否真正利用了提供的证据，而不是仅依赖参数记忆或表面指标。


<details>
  <summary>Details</summary>
Motivation: 现有指标（如答案准确率、检索召回率、引用重叠）无法区分模型是否实际使用了证据，导致无法识别模型从参数记忆回答、收到正确段落却失败，或引用证据但不转换答案等问题。

Method: 设计四条件协议（无证据、完整上下文、检索证据、oracle证据参考），在固定示例、提示、评分、检索设置和有效性检查下评估证据利用。使用ONCU作为协议绑定的估计器，仅对分母有效组计算，并单独报告分母自由答案、证据、检索和失败审计指标。在Qwen、Gemma、Llama、Mistral家族的5个模型上，对Controlled-ONCU-safe16K、HotpotQA-ONCU、2WikiMultiHopQA-ONCU进行实证，收集18000个ONCU兼容预测。

Result: 发现任务依赖的瓶颈：受控合成设置主要暴露完整上下文利用失败，而现实多跳设置主要暴露检索链覆盖失败（反映在分母自由答案和证据指标上），ONCU在oracle改进组上支持相同方向。

Conclusion: 贡献是一个诊断协议，用于分离无证据可回答性、oracle证据可恢复性、完整上下文利用和检索条件利用，而非提供单一分数排行榜。
...
### [94] [Principles of Concept Representation in Sentence Encoders](https://arxiv.org/abs/2606.06994)
*Isabelle Mohr,John Dujany,Jonathan Souquet,Andre Freitas*

Main category: cs.CL

TL;DR: 通过表示组合性视角，分析句子编码器产生概念表示的原理，通过消融实验识别四个原则，并发布两个新数据集。


<details>
  <summary>Details</summary>
Motivation: 探究句子编码器产生良好概念表示的原因，通过表示组合性来预测成功和失败。

Method: 在330万对同义词和定义上训练编码器，通过控制消融实验评估不同训练条件，使用三个去污染分割和一个修饰词标注的名词短语基准。

Result: 识别四个原则：微调重新校准潜在几何而非扩展（P1）；语义信号集中在最终层，跨层池化冗余（P2）；硬负样本改善判别和鲁棒性但不改善排序（P3）；监督有效性取决于组合类型，外延训练有助于交集和子集类别但损害关系和内涵类别（P4）。

Conclusion: 当前训练范式在概念表示上存在结构性限制，并发布了DBpedia语义间隔基准和修饰词标注NP释义套件两个新数据集。
...
### [95] [ThinkBooster: A Unified Framework for Seamless Test-Time Scaling of LLM Reasoning](https://arxiv.org/abs/2606.06915)
...
TL;DR: ThinkBooster是一个统一的测试时计算缩放框架，通过模块化库、联合评估基准和可部署服务，提升LLM推理性能与效率。


<details>
  <summary>Details</summary>
Motivation: 现有TTC缩放策略和推理评分器方法碎片化，评估协议不一致，且缺乏质量-成本权衡分析。

Method: ThinkBooster包含三部分：(i) 模块化Python库，实现SOTA TTC策略和评分器；(ii) 联合评估性能和计算效率的基准；(iii) 兼容OpenAI的代理服务，支持自适应推理的即插即用集成。还提供可视化调试器以检查推理轨迹。

Result: 在数学和编程任务上的实验揭示了TTC缩放策略的性能-计算权衡，并证明ThinkBooster在实际任务中带来实用收益。

Conclusion: ThinkBooster有效整合并评估了TTC缩放方法，实现了性能与效率的平衡，代码已开源。
...
### [98] [Didact: A Cross-Domain Capability Discovery System for Defence](https://arxiv.org/abs/2606.06942)
*Aarya Bodhankar,Aditya Joshi,Bao Gia Doan,Thomas Marchant,Oscar Leslie,Flora Salim*

Main category: cs.CL

TL;DR: Didact是一个原型系统，通过集成国防报告和政策文件与知识图谱，基于检索增强生成实现自然语言对话，帮助国防决策者高效发现和审计碎片化信息。


<details>
  <summary>Details</summary>
Motivation: 国防领域的研究和信息分散在异构格式和孤立存储库中，导致能力发现缓慢且难以审计。

Method: Didact集成公开的澳大利亚国防报告与政策文件，并构建知识图谱；采用复合检索增强生成（RAG）流水线，提供自然语言对话和交互式证据轨。

Result: 评估表明Didact的输出质量和运行时具有实用性。

Conclusion: 虽然为澳大利亚背景共同开发，但Didact可适应其他知识碎片化的领域。
...
### [99] [Style or Content? Evaluating Style Classifiers with Controlled Content Overlap](https://arxiv.org/abs/2606.07103)
*Zhuo Liu,Haozheng Du,Xiangxiang Xu,Hangfeng He*

Main category: cs.CL

TL;DR: 本文通过平行圣经翻译构建受控内容重叠设置，提出重叠参数α测量风格分类器对内容线索的依赖，发现低重叠模型在去除内容线索时性能下降，高重叠模型更稳健，内容检索探针显示内容可恢复性随α增加而降低，表明受控重叠可诊断风格学习与内容捷径。


<details>
  <summary>Details</summary>
Motivation: 风格分类器可能利用与风格标签相关的内容线索，但缺少系统方法衡量这种依赖。

Method: 基于平行圣经翻译构建受控内容重叠设置，定义重叠参数α为内容身份与风格标签之间互信息的归一化残差，通过交叉重叠评估RoBERTa分类器，并使用跨风格内容检索探针分析。

Result: 低重叠模型在去除内容线索时性能下降，高重叠模型迁移更鲁棒；随着α增加，内容可恢复性降低，训练动态显示内容去除是渐进的。

Conclusion: 受控内容重叠为区分风格学习和内容捷径提供了简单诊断工具。
...
### [100] [EASE-TTT: Evidence-Aligned Selective Test-Time Training for Long-Context Question Answering](https://arxiv.org/abs/2606.06906)
*Xiaopeng Yuan,Zebin Wang,Suwen Wang,Zongxin Yang,Haohan Wang,Yushun Dong*

Main category: cs.CL

TL;DR: 提出EASE-TTT，一种通过将检索到的证据块转换为注意力监督目标来指导查询侧测试时训练的方法，在长上下文问答任务上提升了小模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法如检索仅暴露证据而未调整注意力，轻量级测试时训练未解决证据定位，导致小模型在长上下文问答中仍面临挑战。

Method: 从上下文中选择证据块，将其标记位置转换为软注意力监督目标，用于查询侧测试时训练，最后用适应后的模型从完整上下文中生成答案。

Result: 在六个LongBench QA任务和三个小模型上，EASE-TTT的宏平均性能优于全上下文推理、检索基线及qTTT。

Conclusion: 证据对齐的测试时适应能有效提升长上下文问答中模型的表现。
...
### [101] [Contrastive Training with LLM-generated Near-Misses for Robust Code-Switching Speech Recognition](https://arxiv.org/abs/2606.06985)
*Tung X. Nguyen,Hieu Minh Truong,Giang-Son Nguyen,Nhu Vo,Wray Buntine,Dung D. Le*

Main category: cs.CL

TL;DR: 提出POI感知对比训练框架，通过扰动POI构造声学合理的近误假设，并联合POI加权交叉熵和多负对比排名损失微调Whisper-small，在CS-FLEURS和ViMedCSS上相比标准LoRA微调一致降低超过2%的错误率。


<details>
  <summary>Details</summary>
Motivation: 语码切换（CS）语音识别仍具挑战，现有方法对CS关键区域识别不足。

Method: 采用POI检测方法识别CS跨度，通过扰动ASR N-best输出中的POI并利用大语言模型扩展候选，经声学、音素和文本约束过滤保留硬负样本；使用LoRA微调Whisper-small，联合POI加权交叉熵锚目标与多负对比排名损失训练。

Result: 在CS-FLEURS（cmn-eng）和ViMedCSS（vie-eng）上，相比标准LoRA微调，通用错误率和CS感知错误率均一致降低超过2%。

Conclusion: 所提出的POI感知对比训练框架有效提升了CS场景的ASR性能。
...
### [102] [HKJudge: A Legal Discourse-Annotated Corpus for Interpreting What Courts Find, How They Reason, and What They Rule](https://arxiv.org/abs/2606.06679)
*Xi Xuan,Wenxin Zhang,Yufei Zhou,King-kui Sin,Chunyu Kit*

Main category: cs.CL

TL;DR: 构建了首个句子级专家标注的香港判决话语数据集HKJudge，并提出了两个任务及多种模型的基准评估。


<details>
  <summary>Details</summary>
Motivation: 法院判决对法律实践至关重要，但香港判决的话语分析因缺乏专家标注语料库而受到限制。

Method: 创建HKJudge数据集，涵盖香港所有法院级别的刑事判决，共约29万句子和650万token。设计两层话语模式：句子级26种修辞角色和跨度级三个判决要素（指控、监禁期、罚款）。由10位法律语言学专家标注，一致性κ=0.8。定义修辞角色分类和法律元素抽取两个任务，评估四种BERT模型、两种开源LLM（零样本和微调）和四种商业LLM。

Result: 提供了第一个基准评估，展示了句子级话语标注对建模香港判决结构的价值，并为未来法律判决预测提供了丰富数据基础。

Conclusion: 该工作证明了句子级话语标注对于理解香港判决结构的重要性，并为后续研究奠定了数据基础。
...
### [103] [OpenHalDet: A Unified Benchmark for Hallucination Detection across Diverse Generation Scenarios](https://arxiv.org/abs/2606.06959)
...
TL;DR: OpenHalDet是一个统一的幻觉检测基准，通过标准化评估流程和支持多种检测方法，解决了现有评估不一致和覆盖不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM幻觉检测评估面临两个核心挑战：不一致的推理配置和评估，以及下游领域和任务覆盖有限，导致性能难以比较、重现和泛化。

Method: 提出OpenHalDet基准，标准化从提示构建、响应生成、真实性标注到检测器评分和指标计算的整个评估管道，支持黑盒（仅输出）、灰盒（概率信号）和白盒（内部模型信号）检测方法。

Result: 通过将多样化的任务、模型和检测器整合到统一框架中，实现了受控比较，并系统性地展示了不同检测范式在LLM应用中的行为。

Conclusion: OpenHalDet作为开放可扩展的代码库发布，有助于可重复评估和未来幻觉检测方法的开发。
...
### [104] [Improving Cross-Lingual Factual Recall via Consistency-Driven Reinforcement Learning](https://arxiv.org/abs/2606.06586)
...
TL;DR: GRPO通过减少语言专门化有效提升跨语言事实一致性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型主要用英语训练，包含世界知识但在其他语言中表达不可靠，存在跨语言事实不一致问题。

Method: 构建PolyFact数据集（12种语言10万事实），在Qwen-2.5-7B和OLMo-2-1124-7B上比较CPT、SFT和GRPO方法。

Result: GRPO在跨语言一致性和泛化到未见语言上持续优于SFT和CPT。

Conclusion: GRPO通过减少MLP层和注意力头中的语言专门化，促进共享跨语言表示，从而提升性能。
...
### [105] [CRAFT: A Unified Counterfactual Reasoning Framework for Tabular Question Answering and Fact Verification](https://arxiv.org/abs/2606.06842)
*Chenshuo Pan,Yu Zhao,Jie Zhang,Changzai Pan,Zhenhe Wu,Jiayi Liang,Yujie Mao,Shuangyong Song,Yongxiang Li,Zhongjiang He*

Main category: cs.CL

TL;DR: CRAFT通过反事实推理统一表格问答和事实验证，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖单方向推理，限制了多步推理中替代假设的探索。

Method: 构建声明及其反事实变体，从原始和反事实路径提取证据，通过加权机制整合。

Result: 在WikiTQ和TabFact上超越基线，尤其复杂问答提升大，缩小不同LLM性能差距。

Conclusion: 反事实推理克服单方向推理限制，引导更辨别的推理，为结构化推理建立更原则性范式。
...
### [108] [Auditing Training Data in Domain-adapted LLMs: LoRA-MINT](https://arxiv.org/abs/2606.06946)
*Gonzalo Mancera,Daniel DeAlcala,Aythami Morales,Julian Fierrez,Ruben Tolosana,Francisco Jurado*

Main category: cs.CL

TL;DR: 提出LoRA-MINT，一种基于LoRA微调的LLM成员推理测试方法，通过困惑度分析实现高精度的训练数据审计。


<details>
  <summary>Details</summary>
Motivation: 评估LLM训练数据是否包含特定样本，以管理知识产权和敏感数据，提供审计工具。

Method: 利用模型困惑度与成员资格的关系，建立系统框架进行成员推理测试。

Result: 在四个模型和三个数据集上，精度达到0.77-0.92，优于现有基线方法。

Conclusion: LoRA-MINT是有效且可扩展的审计框架，可提高LLM透明度，促进AI伦理部署，且方法可推广到其他模型适配技术。
...
### [109] [Learning Perspectivist Social Meaning via Demographic-Conditioned Fusion Embeddings](https://arxiv.org/abs/2606.07123)
*Amanda Cercas Curry,Lucio La Cava,Luca Maria Aiello,Gianmarco De Francisci Morales*

Main category: cs.CL

TL;DR: 提出融合文本和人口统计表示的模型，以捕捉社会意义的视角性变化，在28k注释数据集上使用多种范式进行基准测试，融合模型在宏PR-AUC上取得5.9-6.5%相对提升。


<details>
  <summary>Details</summary>
Motivation: 当前NLP系统通常将社会意义注释简化为单一真实标签，忽略了不同人口群体间解释的多样性。

Method: 使用28k人类注释数据集，比较零样本、少样本和微调等方法，并提出融合文本和人口统计表示的嵌入模型。

Result: 融合模型在所有融合策略上相比纯文本基线获得一致且统计显著的提升（宏PR-AUC相对提升5.9-6.5%），shuffle消融实验确认人口统计特征携带真实预测信号。

Conclusion: 人口统计特征包含可用于预测社会意义的有效信号，建模视角性多样性有助于提升NLP系统的表现。
...
### [110] [How reliable are LLMs when it comes to playing dice?](https://arxiv.org/abs/2606.07515)
*Luca Avena,Gianmarco Bet,Bernardo Busoni*

Main category: cs.CL

TL;DR: 当前LLM在概率推理上仍存在不足，尤其在反直觉问题和有误导时表现显著下降。


<details>
  <summary>Details</summary>
Motivation: 探究LLM的概率推理能力，通过控制基准测试离散概率问题，并构建标准与反直觉数据集以触发启发式推理。

Method: 构建了标准习题和反直觉习题两个数据集，评估了8个最先进模型，每个模型在有/无链式思维提示下进行测试，并分析了token偏差和误导性提示的影响。

Result: 模型在标准问题上平均准确率为0.96，反直觉问题仅为0.59；规范表述被伪装变体替代时性能下降超20%；嵌入误导性建议导致性能下降高达34%，无模型免疫。

Conclusion: 尽管LLM在高级数学问题上表现成功，但当前模型并非真正的概率推理者。
...
### [111] [PromptPrint: Behavioral Biometrics Through Natural Language Prompting in LLMs](https://arxiv.org/abs/2606.06755)
*Shaiv Patel,Kartik Narayan,Vishal Patel*

Main category: cs.CL

TL;DR: PromptPrint 通过对 1034 名用户的 20680 个真实提示进行分析，验证了基于提示的作者身份识别是可行的，并发现词汇特征比语义特征更有效，且身份信号对语义改写脆弱。


<details>
  <summary>Details</summary>
Motivation: 传统作者归属研究主要针对长文本，而 LLM 交互中的提示简短且任务驱动，因此需要探究这些提示是否包含稳定的作者身份信号。

Method: 提出了 PromptPrint 方法，使用 20680 个真实提示（来自 1034 名用户），比较词汇表示和语义编码器，分析风格特征的独特性与一致性，并进行对抗攻击测试。

Result: 词汇表示显著优于语义编码器；风格特征存在独特性-一致性悖论；身份信号对词汇扰动的鲁棒性高于语义改写。

Conclusion: 建立了提示身份作为可行的行为生物特征，为 LLM 交互中的用户建模提供了新视角，对安全与隐私有重要意义。
...
### [112] [Re-Centering Humans in LLM Personalization](https://arxiv.org/abs/2606.06614)
...
### [113] [UnpredictaBench: A Benchmark for Evaluating Distributional Randomness in LLMs](https://arxiv.org/abs/2606.06622)
*Amirhossein Abaskohi,Amirhossein Dabiriaghdam,Liang Luo,Ellie Dingqiao Wen,Lele Wang,Giuseppe Carenini,Peter West*

Main category: cs.CL

TL;DR: UnpredictaBench评估LLM捕获真实分布的能力，发现即使简单分布采样仍具挑战，模型表现差异大且远未完美。


<details>
  <summary>Details</summary>
Motivation: LLM被用作其他实体替代品时倾向于输出单一答案，无法模拟真实系统的不可预测性，现有输出多样性工作不足以满足需要校准到目标分布的采样。

Method: 创建448个问题，涵盖统计分布、随机程序、自然语言随机场景；使用KS@N指标（基于Kolmogorov-Smirnov检验）量化模型输出与目标分布的一致性。

Result: 模型在KS@100上得分从接近0%到20%以上，无模型超过40%；添加推理可略微提升但无根本解决方案。

Conclusion: 分布性模拟即使对简单问题仍具挑战，UnpredictaBench是迈向使用LLM作为复杂系统代理的第一步。
...
### [114] [How Language Models Fail: Token-Level Signatures of Committed and Persistent Reasoning Failures](https://arxiv.org/abs/2606.06635)
*Tanvi Thoria,Kiana Jafari,Marc R. Schlichting,Mykel J. Kochenderfer*

Main category: cs.CL

TL;DR: 本文通过token级不确定性信号识别了语言模型推理失败的两种过程：早期锁定错误路径的'承诺失败'和不确定性持续累积的'持续不确定性'，并展示了框架对自我一致性的影响。


<details>
  <summary>Details</summary>
Motivation: 语言模型推理失败有不同的过程，留下可识别的签名，但现有工作缺乏对失败模式的区分和理解。本文旨在通过不确定性信号表征这些失败过程，并探索其对检测策略的影响。

Method: 使用token级不确定性信号，从推理轨迹中识别'承诺点'等特征，区分两种失败模式。在23个模型-数据集配置上验证，并通过自一致性实验展示框架的应用。

Result: 在23个配置中，20个验证了框架的可证伪预测。对于自一致性，框架指出了何时不确定性信号可以补充它，何时可以跳过。

Conclusion: 为理解LLM推理失败何时可检测提供了基础，并为调整检测策略提供了指导。
...
### [115] [The Piggyback Hypothesis of Generalization: Explaining and Mitigating Emergent Misalignment](https://arxiv.org/abs/2606.06667)
*Jiachen Zhao,Zhengxuan Wu,Aryaman Arora,Yiyou Sun,David Bau,Weiyan Shi*

Main category: cs.CL

TL;DR: 本文提出Piggyback Hypothesis来解释大型语言模型在窄微调后出现的紧急错位现象，即聊天模板前缀令牌会将微调行为携带到无关领域的查询中。基于此发现，提出Token-Regularized Finetuning (TReFT)方法，通过正则化特定令牌表示来减轻紧急错位，实验证明该方法在减少紧急错位的同时保留了领域内学习能力。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型在训练范例之外的广泛过度泛化机制，并通过紧急错位这一具体案例来探究其背后原因。

Method: 首先提出Piggyback Hypothesis，并通过扰动前缀令牌或修补前缀表示进行验证。然后提出Token-Regularized Finetuning (TReFT)，在训练中正则化特定令牌表示以抑制紧急错位。

Result: TReFT在多种模型和数据集上有效减少紧急错位，例如在Llama-3.1-8B法律领域微调中比数据交错方法多减少33.5%的紧急错位；在其它窄微调设置（如拒绝、工具使用）中，平均减少54.3%的离题泛化。

Conclusion: 大型语言模型可能以非预期的方式学习和泛化，本研究为更受约束的微调提供了方向，并呼吁进一步研究共享输入特征如何将模型行为携带到不同领域。
...
### [116] [What Do People Actually Want From AI? Mapping Preference Plurality](https://arxiv.org/abs/2606.06674)
*Julia Sepúlveda Coelho,Scott A. Hale*

Main category: cs.CL

TL;DR: 分析1500份全球回应发现，人们对AI偏好高度分歧，甚至连“真实性”的定义都相互冲突，暴露了当前RLHF对齐方法将矛盾信号简化为统一模型的问题。


<details>
  <summary>Details</summary>
Motivation: 当前RLHF方法在聚合偏好时存在局限，如忽略偏好冲突、依赖非代表性样本和仅用二元比较，因此需要了解人们真实、多样的需求。

Method: 对PRISM数据集中来自75个国家的1500份开放式回答进行定性分析。

Result: 不同受访者需求差异大，仅“真实性”被49%的人提及，但对其定义存在分歧（如来源、专家意见、不受欢迎观点）；部分能力和特征（如类人行为、防护措施）具有争议；人们常使用“默认”与“按需”的语境区分，而二元比较无法捕获。

Conclusion: 当前对齐实践存在根本缺陷，单一奖励模型无法反映真实偏好，高幻觉率表明方法未能识别实际需求，将情境化、有争议的信号压平为通用模型可能构成“认知暴力”。
...
### [117] [Does Topic Sentiment Cause Perceived Ideology? Comparing Human and LLM Annotations in Political News Articles](https://arxiv.org/abs/2606.06715)
*Upasana Chatterjee*

Main category: cs.CL

TL;DR: 微调后的GPT-4o-mini在意识形态标注中表现出情感-意识形态的捷径学习，而人类标注无此效应。


<details>
  <summary>Details</summary>
Motivation: 探究主题情感是否对感知政治意识形态有因果影响，以及该影响是否依赖标注者身份。

Method: 使用AllSides文章和Llama-3.3-70b-versatile的情感标注，比较人类专家、GPT-4o-mini（基线与微调）和Llama-3.3-70B的意识形态标签，应用双机器学习和社区层面中介分析。

Result: 人类标注无显著社区级效应；微调GPT-4o-mini准确率最高（F1=72.48），且唯一产生显著社区级处理效应和自然直接效应。

Conclusion: 微调导致模型内化人类判断中不存在的虚假情感-意识形态耦合，即捷径学习，该耦合在F1评估中不可见，影响LLM标注作为银标签和人类判断代理的使用。
...
### [118] [Modular Monolingual Adaptation using Pretrained Language Models](https://arxiv.org/abs/2606.06738)
*Nalin Kumar,Ondřej Dušek*

Main category: cs.CL

TL;DR: 提出一种模块化方法，通过替换标记、冻结嵌入并微调模型其他部分，提升低资源语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于对整个预训练语言模型进行微调，但可能不必要的；本文假设全模型调优不是必须的。

Method: 替换目标语言的标记，冻结对应的嵌入，然后微调模型的其余部分。

Result: 在苏格兰盖尔语、爱尔兰语和克丘亚语（8.5k训练样本）上，在掩码填充、命名实体识别和词性标注任务上，所提方法提升了性能。

Conclusion: 通过实验验证了所提模块化方法对低资源语言适配的有效性，并提供了对训练策略、预训练嵌入选择和模型的全面分析。
...
### [119] [Explain Like I'm 5 or Whatever I Choose: Evaluating the Interactive Potential of Language Model Responses](https://arxiv.org/abs/2606.06788)
*Indu Panigrahi,Tal August*

Main category: cs.CL

TL;DR: 提出基于语言复杂度的LLM评估框架，发现多数模型调整复杂度不一致。


<details>
  <summary>Details</summary>
Motivation: 现有评估忽略界面特性，需要纳入界面特定标准，受直接操作界面启发。

Method: 基于16人形成性研究，评估4个模型为98个科学查询生成5个不同语言复杂度的响应。

Result: 模型能调整复杂度但变化不一致，最佳模型仅46%正确移动指标。

Conclusion: 增加样本量和复杂度水平后结果稳健。
...
### [120] [Korean Culture into LLM Alignment: Toward Cultural Coherence](https://arxiv.org/abs/2606.06797)
*MinJae Jung,Minwoo Kim*

Main category: cs.CL

TL;DR: 本文提出了一种构建性方法，为韩国文化定义了文化连贯响应的概念，并通过DPO微调提高了韩国文化安全率，同时保持通用能力。


<details>
  <summary>Details</summary>
Motivation: 当前文化方面的工作主要关注抑制负面输出，但缺乏对文化连贯响应的定义，因此需要一种构建性的工作定义。

Method: 设计了一个对齐数据流水线，包括基于提示的LLM种子生成器扩展韩国伤害分类法，以韩国文化适应的安全响应政策为中心，让三个前沿模型各生成一个候选响应，并使用DPO对结果三元组进行微调。

Result: DPO微调提高了六个开放权重LLM的韩国文化安全率，在韩国通用能力基准上没有大幅下降，且定性输出显示微调模型能够引用韩国法规和程序，并在拒绝时提供建设性的韩国上下文信息。

Conclusion: 研究表明，这种构建性方法可以有效地提升LLM在特定文化背景下的安全性和文化连贯性，为多语言模型的文化适应提供了新的方向。
...
### [121] [Progress-SQL: Improving Reinforcement Learning for Text-to-SQL via Progressive Rewards](https://arxiv.org/abs/2606.06825)
*Shihao Zhang,Xiaoman Wang,Yuan Liu,Yunshi Lan,Weining Qian*

Main category: cs.CL

TL;DR: Progress-SQL is a multi-turn reinforcement learning framework with progressive rewards that leverages Oracle-guided Diagnostic Trees to improve Text-to-SQL generation through iterative SQL refinement.


<details>
  <summary>Details</summary>
Motivation: Existing reinforcement learning methods for Text-to-SQL typically use one-shot rewards over a single SQL state, which provide limited guidance for iterative correction and fail to capture multi-turn SQL improvement.

Method: Progress-SQL introduces an Oracle-guided Diagnostic Tree (ODT) to abstract SQL queries into clause-level structural profiles and provide diagnostic feedback. It combines ODT-based structural alignment with lexical alignment to define a progressive reward measuring improvement from initial to final SQL, plus a progression latency reward and an execution status reward.

Result: Experiments on BIRD, Spider, and Spider robustness variants show consistent improvement in Text-to-SQL performance on both primary and robustness evaluations.

Conclusion: The proposed multi-turn reinforcement learning framework effectively enhances Text-to-SQL generation by providing progressive rewards and diagnostic feedback for iterative refinement.
...
### [122] [Characterize Then Distill: Mechanistic Reasoning in Large Output Spaces](https://arxiv.org/abs/2606.06840)
*Debjyoti Saha Roy,Byron C. Wallace,Javed A. Aslam*

Main category: cs.CL

TL;DR: 现代推理模型通过两阶段过程（候选短名单和细粒度推理）实现强大的零样本多标签性能，基于此开发的机械蒸馏策略优于标准蒸馏。


<details>
  <summary>Details</summary>
Motivation: 理解推理模型在零样本多标签任务中的工作机制，并改进蒸馏方法。

Method: 将推理过程表征为两个互补的阶段：广泛的候选短名单筛选和细粒度推理，并基于此设计机械蒸馏策略。

Result: 机械蒸馏策略在多个数据集上始终优于标准蒸馏。

Conclusion: 揭示推理模型的工作机制有助于设计更有效的蒸馏方法。
...
### [123] [Interpreting Brain Responses to Language with Sparse Features from Language Models](https://arxiv.org/abs/2606.06857)
*Michael A. Lepori,Kendrick Kay,Greta Tuckute*

Main category: cs.CL

TL;DR: 通过增强稀疏编码模型，本研究发现人脑语言皮层与人工语言模型之间存在非平凡对应，大脑响应最佳由捕捉LM一般信息的特征解释。


<details>
  <summary>Details</summary>
Motivation: 为解决将生物和人工语言表征联系起来时黑箱与黑箱关联的批评，需要更可解释的编码模型。

Method: 使用增强稀疏编码模型，以分层稀疏自动编码器特征替代密集LM隐藏状态，并加入惊奇度预测因子，分析7T fMRI数据（8名参与者听200句语言多样句子）。

Result: 模型恢复了已知体素调谐（处理难度、意义抽象性），发现新体素调谐于人相关内容；额叶区域相对更好地由惊奇度解释；大脑响应最佳由捕捉LM一般信息的特征预测。

Conclusion: 大脑与LM的语言表征存在非平凡对应，支持模型与大脑的潜在共享表征。
...
### [124] [Are Large Language Models Suitable for Graph Computation? Progress and Prospects](https://arxiv.org/abs/2606.06865)
*Yuting Zhang,Yi Han,Kai Wang,Wei Ni,Angela Bonifati,Wenjie Zhang*

Main category: cs.CL

TL;DR: 本文综述了大语言模型在图计算中的两种角色：作为执行者直接求解和作为规划者分解步骤并调用工具，分析表明LLMs在小规模简单任务上表现良好但在大规模精确任务上仍不可靠。


<details>
  <summary>Details</summary>
Motivation: 现有调查主要关注图学习、文本属性图或图语言建模，缺乏对LLMs在图计算中角色的全面综述，且不清楚LLMs何时能可靠支持此类计算以及如何整合到图解决管道中。

Method: 通过基于角色的分类法进行全面回顾，识别两个主要范式：LLMs作为执行者和LLMs作为规划者。

Result: 分析表明LLMs对于简单、小规模任务有前途，但在大规模和需要精确性的任务上仍不可靠。

Conclusion: 总结了可用数据集并提出了四个未来方向。
...
TL;DR: 提出FinEvolveBench基准和Tree-of-Experience方法，用于隐式奖励环境下LLM agent的经验演化。


<details>
  <summary>Details</summary>
Motivation: 现有基准假设明确目标和清晰反馈，但实际中任务重复率低、奖励隐式且延迟，需要更难的评估环境。

Method: 构建FinEvolveBench金融情感预测基准，并设计Tree-of-Experience（ToE）结构化经验管理方法，包括组织、检索、验证和更新经验。

Result: 通用经验机制不优于无经验基线，而ToE在整体性能上更强。

Conclusion: 在隐式奖励环境中，结构化经验管理对自我演化agent至关重要。
...
### [126] [MADE: Beyond Scoring via a Multilingual Agentic Diagnosing Engine for Fine-Grained Evaluation Insights](https://arxiv.org/abs/2606.07020)
...
TL;DR: MADE是一个多语言智能诊断引擎，通过分解后评估分析并配备诊断集，显著提高了诊断报告质量，并被人类专家偏好。


<details>
  <summary>Details</summary>
Motivation: 单LLM和开放代理难以处理长噪声诊断输入，且没有可重用的分类法。

Method: MADE将后评估分析分解为规划、聚合分析、实例级案例检查、多语言和文化反思、以及有依据的报告合成，并配备专家主导的54查询和15语言诊断集，在大型多语言评估基座上评估。

Result: 在诊断报告质量上比最强共享基线高出47%，在87.9%的成对比较中被人类多语言专家偏好。

Conclusion: 与多语言专家一起应用，MADE进一步发现四个可操作的发现，涉及部署、迭代和跨文化陷阱，将基准评分表转化为模型选择和修复指导。
...
### [127] [mmPISA-bench: Do LLMs Reason Equally Well Across 43 Languages?](https://arxiv.org/abs/2606.07069)
*Yerzhan Sapenov,Jaromir Savelka*

Main category: cs.CL

TL;DR: mmPISA-bench是一个多语言推理基准，评估LLM在43种语言上的表现，发现机器翻译不影响准确性，但不同语言成本存在差异。


<details>
  <summary>Details</summary>
Motivation: 评估现代LLM在多语言推理任务上的能力，并探究高质量机器翻译是否足以替代人工翻译进行大规模评估。

Method: 构建基于PISA的25道多选题基准，提供人工和机器翻译版本，评估两个专有LLM在不同语言和翻译类型下的准确率及推理成本。

Result: LLM在所有语言上均能有效推理，准确率接近人类；机器翻译的题目未导致性能下降；部分语言的推理成本更高且准确率更低。

Conclusion: 高质量机器翻译可能足够用于大规模多语言推理评估，但需注意语言间的成本和准确性差异。
...
### [128] [SigmaScale: LLM Compression with SVD-based Low-Rank Decomposition and Learned Scaling Matrices](https://arxiv.org/abs/2606.07098)
*Ernests Lavrinovics,Marco Letizia,Roy Janco,Shai Segal,Johannes Bjerva,Maurizio Pierini*

Main category: cs.CL

TL;DR: SigmaScale通过学习激活感知的缩放矩阵优化SVD裁剪的LLM压缩，有效降低权重矩阵的内在秩，性能与最先进方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有的基于SVD的LLM压缩方法通常使用解析推导的缩放矩阵，缺乏对权重结构的自适应能力，导致压缩效率受限。SigmaScale旨在通过可学习的缩放变换更好地适应模型权重，从而降低推理计算成本。

Method: SigmaScale优化两组向量，分别定义对角行和列缩放变换，在激活感知的压缩损失下进行学习，替代解析推导的缩放矩阵。

Result: 在Llama 3.1 8B Instruct和Qwen3-8B上，SigmaScale在困惑度和零样本基准测试中与最先进的SVD压缩方法竞争力相当，且学习到的缩放降低了有效秩熵，与压缩损失强相关。

Conclusion: SigmaScale通过激活感知的变换提供了一种更灵活的低秩压缩途径，能够适应个体权重的结构，对于需要降低LLM推理计算成本的应用是一个有效选择。
...
### [129] [Explicit Evidence Grounding via Structured Inline Citation Generation](https://arxiv.org/abs/2606.07130)
*Anar Yeginbergen,Amelie Wührl,Anna Rogers,Rodrigo Agerri*

Main category: cs.CL

TL;DR: FullCite是一个生成结构化内联引文的框架，通过三种策略提升引文质量，并发现LLMs在精确证据跨度识别上存在不足。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统广泛应用，对事实性和忠实性生成的需求增加，通过引文进行信息归因变得至关重要。

Method: FullCite提出了三种内联引文生成策略：基于提示的生成、基于引文语法的约束解码和事后跨度对齐，并在三个QA基准上评估。

Result: 评估显示，LLMs能有效识别相关文档，但难以精确识别支持性证据跨度。

Conclusion: 实现忠实归因的问答需要更注重精确证据跨度的识别。
...
### [130] [UrduMMLU: A Massive Multitask Benchmark for Urdu Language Understanding](https://arxiv.org/abs/2606.07167)
*Ahmer Tabassum,Sarfraz Ahmad,Hasan Iqbal,Owais Aijaz,Momina Ahsan,Preslav Nakov*

Main category: cs.CL

TL;DR: 我们引入了UrduMMLU，一个包含26,431道Urdu多选题、覆盖26个学科和5个领域的基准，评估了30个LLM，发现Gemini-3.5-Flash表现最佳（约90%），但Urdu知识仍不均衡，尤其在地区性内容上。


<details>
  <summary>Details</summary>
Motivation: Urdu有超过2.3亿使用者，但缺乏基于本地教育资源的MMLU风格基准，现有翻译型资源不足以反映目标语言和教育背景。

Method: 从本地Urdu MCQ题库和公共考试PDF收集题目，通过双重人工标注和严格一致性过滤构建基准；在英文和Urdu提示下对30个LLM进行零样本评估，并对4个开源LLM进行多种少样本设置评估。

Result: Gemini-3.5-Flash达到90.20%和90.34%准确率，其他模型均低于85%；最强开源模型落后7.79和8.92个百分点；许多模型在Urdu中心组人文学科上比STEM低25-40分；少样本提示仅带来微小提升。

Conclusion: UrduMMLU显示当前LLM对Urdu知识掌握不均，尤其缺乏对地区性内容的理解，提示需要更多关注语言和区域特定的评估资源。
...
### [131] [Adversarial Creation and Detection of AI-Generated Social Bot Content](https://arxiv.org/abs/2606.07219)
*Mykola Trokhymovych,Ricardo Baeza-Yates,Alessandro Flammini,Diego Saez-Trumper,Filippo Menczer*

Main category: cs.CL

TL;DR: 通过对抗性方法建模恶意用户冒充行为，创建多语言跨平台数据集，训练检测模型，显著优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有检测模型因缺乏真实数据而在实际中失效，需要解决数据稀缺问题。

Method: 采用对抗性方法论模拟恶意用户冒充真实用户，策划多语言跨平台的人机消息配对数据集，并训练检测模型。

Result: 训练的模型在真实世界分布外数据上显著优于现有内容型机器人检测模型。

Conclusion: 对抗性方法和数据集有效提升了AI生成内容检测的鲁棒性和实际表现。
...
### [132] [When Large Language Models Fail in Healthcare: Evaluating Sensitivity to Prompt Variations](https://arxiv.org/abs/2606.07237)
*Mahdi Alkaeed*

Main category: cs.CL

TL;DR: 大型语言模型在医疗应用中对抗动高度敏感，可能导致临床建议改变或有害输出，缺乏安全性。


<details>
  <summary>Details</summary>
Motivation: LLMs在医疗中的广泛应用及其对提示扰动的敏感性引发了安全风险，需要进行系统性敏感性分析以评估其鲁棒性。

Method: 使用MedMCQA基准，将扰动分为自然和对抗类型，对通用和医学特定LLMs进行系统性敏感性分析，评估一致性、准确性和可靠性。

Result: 医学LLMs并非本质安全；微小措辞变化可改变临床建议，对抗性提示可导致危险输出；模型对词汇替换有弹性但容易因句法重排或误导性线索而崩溃。

Conclusion: 在医疗等高风险环境中，LLMs的不可预测性不可接受，需要提高其鲁棒性以确保安全可靠。
...
### [133] [Phun-Bench: Evaluating LLMs on Phonological Understanding in Chinese](https://arxiv.org/abs/2606.07300)
*Xing Yue,Yongliang Shen,Weiming Lu*

Main category: cs.CL

TL;DR: Presents Phun-Bench, a Chinese benchmark for evaluating LLMs' phonological understanding across homophony, rhyme, and phonetic similarity, finding that LLMs excel at recalling pronunciations but lack flexible phonological reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for LLMs' phonological abilities are either solvable through rote memorization or intertwined with other abilities, making them inadequate to measure genuine phonological understanding.

Method: Phun-Bench is a purpose-built Chinese benchmark with diverse tasks across three dimensions: Homophony, Rhyme, and Phonetic Similarity, designed to systematically evaluate LLMs' phonological understanding.

Result: LLMs excel at recalling correct pronunciations but generally struggle to leverage phonological knowledge in the flexible and intuitive way human speakers do.

Conclusion: The study highlights an underexplored frontier for future research and proposes a hypothesis regarding the underlying mechanism of LLMs' phonological understanding and 'perception'.
...
### [134] [M$^3$Exam: Benchmarking Multimodal Memory for Realistic User-Agent Interactions](https://arxiv.org/abs/2606.07402)
*Zhengjun Huang,Wenxuan Liu,Zhoujin Tian,Wei Chen,Junle Chen,Yuqian Wu,Fangyuan Zhang,Qintian Guo,Xiaofang Zhou*

Main category: cs.CL

TL;DR: 提出多模态对话记忆基准M^3Exam和改进方法M^3Proctor，显著提升准确率并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对真实多模态交互和隐含信息推断的评估。

Method: 构建M^3Exam基准，并设计M^3Proctor方法，通过检测查询模态偏差和按需消耗视觉源来优化记忆。

Result: M^3Proctor在准确率上提升13%，索引构建时间和检索令牌减少超70%。

Conclusion: 多模态记忆系统存在跨模态和效率差距，M^3Proctor有效缓解了这些问题。
...
### [135] [The Masked Advantage: Uncovering Local-Language Access to Cultural Knowledge in LLMs](https://arxiv.org/abs/2606.07422)
...
TL;DR: 当地语言能更好地获取文化知识，但原始准确率被语言熟练度差异掩盖。


<details>
  <summary>Details</summary>
Motivation: 现有评估中模板问题不自然，原始准确率混淆了语言熟练度与文化知识获取。

Method: 利用真实文化问题，通过问题类型和查询语言交叉设计，结合1PL项目反应理论模型分离熟练度与知识获取。

Result: 英语在文化无关问题上占优势；控制熟练度后，当地语言在几乎所有设置中表现出知识获取优势，该优势在原始准确率中被掩盖。

Conclusion: 当地语言表现差不一定意味着文化知识弱；当地文化知识可能更易通过当地语言获取，但受限于语言熟练度。
...
### [136] [Agentopia: Long-Term Life Simulation and Learning in Agent Societies](https://arxiv.org/abs/2606.07513)
...
TL;DR: 提出Agentopia框架，通过长期多代理生活模拟训练LLM，提升社交智能和角色扮演能力。


<details>
  <summary>Details</summary>
Motivation: 探索LLM是否能从模拟社交经验中学习理解和复制人类行为，并研究长期模拟中的社交行为和LLM能力提升。

Method: 构建Agentopia框架，100个代理在10年模拟中追求个人成长和社交，定义生活奖励，通过拒绝采样训练LLM。

Result: 代理展现丰富的涌现社交行为，生活奖励训练提高代理福祉，并在下游基准上提升15.6%。

Conclusion: 长期生活模拟和社交经验训练能有效增强LLM的社交智能和角色扮演性能。
...
### [137] [CAF-Gen: A Multi-Agent System for Enriching Argumentation Structures](https://arxiv.org/abs/2606.06646)
*Jakub Bąba,Jarosław Chudziak*

Main category: cs.CL

TL;DR: An automated multi-agent framework, CAF-Gen, enriches shallow argument structures into CAF-compliant models using an iterative Creator-Reviewer pipeline.


<details>
  <summary>Details</summary>
Motivation: Current Argument Mining techniques fail to capture richer structural information required by advanced schemas like the Carneades Argumentation Framework (CAF), which includes premise types, proof standards, and argument schemes.

Method: CAF-Gen employs a multi-agent system with an iterative Creator-Reviewer pipeline where a creator agent's output is validated by a critical agent to ensure structural integrity.

Result: Experiments demonstrate that the iterative feedback loop improves data quality and achieves strong alignment with original annotations, producing structurally richer models.

Conclusion: The multi-agent system overcomes limitations of single-pass generation, providing a robust methodology for automated modeling of formal argumentation.
...
### [138] [Signal-Driven Observation for Long-Horizon Web Agents](https://arxiv.org/abs/2606.06708)
*Shubham Gaur,Ian Lane*

Main category: cs.CL

TL;DR: 提出信号驱动观察（SDO）方法，通过仅在需要时重新获取DOM来减少上下文退化，提高长期任务性能。


<details>
  <summary>Details</summary>
Motivation: 长任务中代理频繁读取完整DOM导致上下文退化，耦合观察与行动频率是架构错误。

Method: 受递归语言模型启发，提出SDO：专用子调用读取完整DOM但只返回相关元素和选择器，仅在信号检测器触发时重新调用。

Result: 本文无实验结果，但指出了SDO引入的开放问题。

Conclusion: 呼吁社区将观察压缩作为web代理设计的核心架构决策。
...
### [139] [Data-Efficient Autoregressive-to-Diffusion Language Models via On-Policy Distillation](https://arxiv.org/abs/2606.06712)
*Xingyu Su,Jacob Helwig,Shubham Parashar,Atharv Chagi,Lakshmi Jotsna,Degui Zhi,James Caverlee,Dileep Kalathil,Shuiwang Ji*

Main category: cs.CL

TL;DR: 提出OPDLM，通过在线蒸馏将自回归模型转换为扩散语言模型，大幅减少训练需求并消除分布偏移。


<details>
  <summary>Details</summary>
Motivation: 现有ARLM转DLM方法存在两种分布偏移：目标切换导致知识丢失，以及训练-推理不匹配。

Method: 使用在线蒸馏（OPD），学生模型生成自身轨迹，教师模型提供目标logits，以在线方式训练双向注意力ARLM。

Result: OPDLM仅需15x到7000x更少的训练token，且在各种任务上表现强劲。

Conclusion: OPDLM避免了DLM的昂贵预训练，将DLM转换定位为ARLM的后训练形式。
...
### [140] [When to Think Deeply: Inhibitory Deliberation for LLM Reasoning](https://arxiv.org/abs/2606.06745)
*Zhixuan He,Yue Feng*

Main category: cs.CL

TL;DR: IDPR使用响应条件抑制，通过快速答案和置信度等特征决定是否调用慢推理，仅用8.20%的慢推理调用将准确率从47.90%提升至48.92%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过深思推理提升性能，但慢推理计算昂贵且通常不必要。

Method: 提出IDPR框架，首先生成快速答案，然后使用抑制控制器基于快速答案的置信度、logit边际、可解析性和生成成本等证据，决定是否释放或抑制该答案以进行慢推理。控制器通过配对快速-慢速结果训练，并在验证集上选择抑制阈值。

Result: 在5000个数学推理测试集上，IDPR仅对8.20%的示例调用慢推理，准确率从47.90%提升至48.92%，在相同慢调用预算下优于随机路由（46.76%）和最强置信度基线（48.22%），并实现了最高纠正精度。

Conclusion: 响应条件抑制能更好地识别从慢推理中受益的快速答案，从而在保持准确率的同时降低计算成本。
...
### [141] [When Better Codebooks Are Not Enough: Predictive Performance and Behavioral Reliability in LLM Political Event Coding](https://arxiv.org/abs/2606.06781)
*Zixian He,Bharath Raahul Murugesan,Patrick Brandt,Yibo Hu*

Main category: cs.CL

TL;DR: 清晰的代码本提高LLM分类性能，但未必提高行为可靠性；应同时评估准确性和编码逻辑一致性。


<details>
  <summary>Details</summary>
Motivation: 高准确率并不一定意味着LLM是忠实编码器，这对依赖专家代码本的社会科学研究至关重要。

Method: 通过操作化专家代码本（更清晰的定义、示例、检索上下文和规则），并在标签名称、代码本顺序和映射的受控变化下评估行为可靠性。

Result: 更清晰的代码本显著提高分类性能，尤其是细粒度事件，但预测增益未完全转化为行为可靠性；模型可能通过准确率测试但在可靠性测试中失败。

Conclusion: 评估代码本引导的LLM系统不仅应关注准确率，还应确保其保留编码逻辑，使输出对社会科学研究有意义。
...
### [142] [Quantifying Media Representation Dynamics Across 25 Years of News Reporting on Policing-related Deaths](https://arxiv.org/abs/2606.06812)
*Farhan Samir,Jappun Dhillon,Meghna Ravikumar,Syed Ishtiaque Ahmed,Vered Shwartz*

Main category: cs.CL

TL;DR: 加拿大警察致死事件新闻报道中，国家官僚观点频率是平民的三倍，并开发了PerspectiveGap模型。


<details>
  <summary>Details</summary>
Motivation: 基于社会学研究，分析媒体中警察代表的视角不平衡，并开发可扩展的计算方法。

Method: 对25年间4000篇新闻文章进行最大规模计算分析，开发PerspectiveGap模型，结合定性和定量分析。

Result: 国家官僚观点出现率是平民的近三倍；部分文章无平民观点，但近年平民代表增加；官僚叙述临床程序性，平民情感化。

Conclusion: PerspectiveGap框架可推广至其他司法管辖区，提供分析媒体系统构建警察问责叙事的方法。
...
### [143] [The Dark Regulome: Disentangling Predictability from Regulation in Genomic Foundation Models](https://arxiv.org/abs/2606.06834)
*Chahat Baranwal,Aadtya Baranwal,Lakshya Nitin Tandon*

Main category: cs.CL

TL;DR: 引入残差化和置换诊断方法，使用三种序列基础模型分析胶质瘤暗基因组元件，区分序列可预测性与调控驱动效应，揭示10kb调控范围，并提供通用ISM诊断工具。


<details>
  <summary>Details</summary>
Motivation: 胶质瘤通过功能性突触整合入神经回路，但非编码元件如何塑造突触基因表达未知；暗基因组中的调控程序（暗调控组）是天然研究对象，但基于似然的ISM评分与局部序列可预测性耦合，导致调控解读不确定。

Method: 使用三种架构基础模型（Caduceus-Ph, HyenaDNA, Enformer）对92个胶质瘤相关基因座的30,448个暗基因组元件进行ISM，引入残差化和置换诊断分离预测驱动与调控驱动的RIS方差，并通过跨架构分解、保守性、脑cis-eQTL和STRING-PPI进行验证。

Result: 发现10kb近端调控窗口在不同控制下稳健；语言模型导出的元件层级被六特征线性基线匹配（AUC=0.985）；跨架构分解显示语言模型共同排序长片段可预测转座元件，而Enformer保留cCRE区分信号，两组top-100零重叠；top-100元素在脑eQTL中富集3.3倍（p<5e-3），但转座元件调控层和NRXN1+NLGN1蛋白汇聚未通过置换检验。

Conclusion: 该诊断可推广为任何基于ISM的调控研究的通用方法工具。
...
### [144] [Translate-R1: Cost-Aware Translation Tool Use via Reinforcement Learning](https://arxiv.org/abs/2606.06835)
...
### [145] [TRACE: Trajectory Reasoning through Adaptive Cross-Step Evidence Aggregation for LLM Agents](https://arxiv.org/abs/2606.07054)
...
TL;DR: TRACE是一个用于长程LLM代理轨迹的监控框架，通过Triage-Inspect-Judge循环实现高效检测，在多个任务上取得了优于基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法有效检测通过一系列良性行为隐藏的恶意目标，因为它们要么一次性评估整个轨迹，要么分割成独立窗口，无法连接时间上相隔较远的证据。

Method: TRACE框架，通过TIJ（Triage-Inspect-Judge）循环：识别高信号区域，进行针对性检查并维护累积证据，合成轨迹级判决。

Result: 在10个任务域上评估，TRACE实现了0.713的F1和0.844的召回率，尤其在需要长程证据连接的任务上改进最大。

Conclusion: TRACE能够有效检测长程隐藏恶意行为，通过连接远端证据显著提升了监控性能。
...
### [146] [Modeling semantic association in self-paced reading with language model embeddings](https://arxiv.org/abs/2606.07066)
*Sara Møller Østergaard,Kenneth Enevoldsen,Afra Alishahi,Bruno Nicenboim*

Main category: cs.CL

TL;DR: 本研究使用语言模型嵌入的不同实现来估计语义关联，并发现句子嵌入在捕捉语义关联方面表现最佳，且方法论选择对结果有显著影响。


<details>
  <summary>Details</summary>
Motivation: 语义关联是阅读理解的重要组成部分，但基于嵌入的量化方法多样，需系统比较不同方法的影响。

Method: 使用联合EEG和自定进度阅读的荷兰语文本语料库，从语言模型获取嵌入，以十种不同实现（嵌入模型和上下文长度不同）计算语义关联，采用贝叶斯分层模型和贝叶斯因子分析。

Result: 嵌入模型的选择显著影响语义关联对N400和阅读时间的估计效果；仅依赖句子嵌入的实现方式在神经和行为测量上显示出超越词可预测性的可靠结果。

Conclusion: 量化语义关联时，方法论选择至关重要，句子嵌入具有捕捉语义关联的潜力。
...
### [147] [Geometry of Semantic Space: Comparative Study of Discrete and Continuous Models](https://arxiv.org/abs/2606.07183)
*Gabriel Bounias,Sabine Ploux*

Main category: cs.CL

TL;DR: 本文比较了监督向量嵌入和图模型的语义几何，发现局部拓扑相似但整体结构不同，提出两者互补可引导神经网络更稳定可解释。


<details>
  <summary>Details</summary>
Motivation: 理解NLP模型中语义几何的差异，探索更清晰可读的语义组织方式。

Method: 基于图的结构和embeddings的拓扑进行对比分析，应用于法国'Great National Debate'语料库。

Result: 局部拓扑相似，但整体结构和拓扑差异显著。

Conclusion: 深度监督模型与图模型具有互补性，可为引导神经网络向更稳定和可解释的收敛提供新路径。
...
### [148] [KIT's Submission to Cross-Lingual Voice Cloning in IWSLT 2026](https://arxiv.org/abs/2606.07240)
*Seymanur Akti,Alexander Waibel*

Main category: cs.CL

TL;DR: 跨语言语音克隆中通过语言标签提示和参考条件词汇匹配提高可懂度和自然性。


<details>
  <summary>Details</summary>
Motivation: 跨语言语音克隆面临保持可懂度和自然性的挑战，特别是在口音变化和领域特定词汇的情况下。

Method: 基于FishAudio-S2-Pro模型，引入语言标签提示，应用强化学习微调，并采用参考条件词汇匹配方法。

Result: 语言提示提供最大增益，词汇匹配在重叠子集中一致改善，RL微调改善可懂度。

Conclusion: 语言标签提示和参考条件词汇匹配有效提升了跨语言语音克隆的性能。
...
### [149] [SV-Detect: AI-generated Text Detection with Steering Vectors](https://arxiv.org/abs/2606.07313)
*Mikhail Vishnyakov,Tatiana Gaintseva*

Main category: cs.CL

TL;DR: 提出基于引导向量的机器生成文本检测方法，在分布偏移下表现优异。


<details>
  <summary>Details</summary>
Motivation: 机器生成文本检测在分布偏移（如跨领域、源模型和编辑攻击）下尤其困难。

Method: 从冻结语言模型的隐藏表示中提取引导向量，每层构建区分人写与机器生成文本的方向，通过层对齐表示输入，并用轻量分类器训练得到检测分数。

Result: 方法在分布内和分布偏移下均表现强劲，包括跨领域、源模型和编辑变换（如润色和改写），且学习到的方向对齐可识别风格线索并捕获额外信号。

Conclusion: 将假文本检测定位为表示空间探测问题，引导向量提供了简单有效的解决方案。
...
### [152] [Supervision versus Demonstration-Based In-Context Learning for Multiword Expression Classification](https://arxiv.org/abs/2606.07479)
*Sercan Karakaş,Yusuf Şimşek*

Main category: cs.CL

TL;DR: 研究土耳其语习语轻动词结构检测，比较BERTurk和指令微调LLMs在不同提示下的表现，发现提示敏感性和演示的重要性。


<details>
  <summary>Details</summary>
Motivation: 土耳其语习语轻动词结构表面形式与字面组合相同，难以区分，需要有效的检测方法。

Method: 采用二分类，使用人工受控集，对比BERTurk基线和三种LLMs在零样本、单样本、少样本提示下的性能。

Result: 零样本LLMs召回率低，单样本提高但引入偏差，少样本改善校准，GPT-OSS-20B和Qwen 2.5-14B表现稳健。

Conclusion: 检测对提示敏感，监督基线仍有竞争力，精心设计的提示可使LLMs达到或超越基线。
...
### [153] [TOPSIS-RAD: Ranking According to Desires](https://arxiv.org/abs/2606.07253)
*Leonardo Fernandes Costa,Helder Gomes Costa,Diogo Lima,Brunno Rodrigues*

Main category: cs.AI

TL;DR: 提出TOPSIS-RAD，通过决策者定义的拒绝性能水平和期望性能水平解决传统TOPSIS的排名问题。


<details>
  <summary>Details</summary>
Motivation: 传统TOPSIS的参考点来自数据集，导致排名与决策者需求不一致、对异常值敏感和排名反转。

Method: 引入VPL（拒绝性能水平）在归一化前排除不可行方案，DPL（期望性能水平）在归一化前将性能上限设为决策者期望水平，从而稳定参考点。

Result: 三个玩具示例表明：VPL通过移除不可行方案重塑归一化边界；DPL限制过高性能的影响，稳定排名。

Conclusion: 方法保留TOPSIS的距离结构，但将排名基于稳定的决策者指定边界；同时讨论了局限性和未来工作。
...
### [156] [Declarative Skills for AI Agents in Knowledge-Grounded Tool-Use Workflows](https://arxiv.org/abs/2606.06923)
*M. Danish Lim,I. Danial Bin Sharudin,Wen Han Chen,Cedric Lim,Laura Wynter*

Main category: cs.AI

TL;DR: 声明式代理通过自然语言技能文件在高质量检索下提升准确率和减少编排错误，但检索质量是关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 为工具使用的AI代理在真实客服工作流中寻找有效的编排机制。

Method: 将三种代理形式化为DEC-POMDP策略类，分析信息论和结构性质，并在5种语言模型和2种检索机制下进行实证比较。

Result: 检索质量是主导瓶颈；在高质量检索时声明式技能提升程序任务准确率并减少编排错误，而命令式状态机未可靠改善效果。

Conclusion: 声明式代理是有效的编排范式，其性能受限于检索质量。
...
### [157] [DuMate-DeepResearch: An Auditable Multi-Agent System with Recursive Search and Rubric-Grounded Reasoning](https://arxiv.org/abs/2606.07299)
...
TL;DR: 提出DuMate-DeepResearch多智能体深度研究框架，通过解耦Agent Core与工具生态系统，并引入图动态规划、递归两级执行和基于评分标准的测试时优化，在DeepResearch Bench和DeepResearch Bench II上取得最佳总体得分。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究系统受限于长程规划、单智能体分解调度、长文合成幻觉和有限过程可审计性等四个相互关联的局限。

Method: 框架将Agent Core（任务理解、规划、调度）与可扩展工具生态系统解耦，使中间决策和工具调用可追溯。引入三种机制：①图动态规划策略，从粗到细扩展研究路线图并通过反思、重规划、回溯和并行分支持续修订；②递归两级执行设计，将复杂搜索子任务委托给内部搜索智能体，隔离噪音检索并稳定长程执行；③基于评分标准的测试时优化，动态生成任务特定质量标准并用作证据驱动合成和自适应停止的实时推理支架。

Result: 在DeepResearch Bench上取得最佳总体得分58.03%，在DeepResearch Bench II上取得最佳总体得分61.95%，并排名第一在信息召回和分析上。

Conclusion: DuMate-DeepResearch通过多智能体架构和三种创新机制有效克服了现有DR系统的局限，在深度研究任务上建立了新的最佳性能。
...
### [158] [DyCon: Dynamic Reasoning Control via Evolving Difficulty Modeling](https://arxiv.org/abs/2606.07108)
*Tengyao Tu,Yulin Li,Hui-Ling Zhen,Libo Qin,Zhoujun Wei,Jinghua Piao,Zhuotao Tian,Yong Li,Min Zhang*

Main category: cs.AI

TL;DR: 提出DyCon，一种无需训练的方法，利用潜在步骤级表示动态控制推理深度，缓解推理模型的过度思考问题。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型存在过度思考问题，现有方法依赖静态难度估计或任务特定训练，无法适应推理过程中的动态复杂度；且问题难度动态变化并线性编码在步骤级嵌入中。

Method: DyCon框架，利用潜在步骤级表示显式建模任务难度的演化，动态控制推理深度，无需额外训练。

Result: 在4B至32B的四个模型、十二个基准（数学推理、问答、编码）上，DyCon显著减少冗余步骤，提升效率，且不牺牲准确率或泛化能力。

Conclusion: DyCon有效提升推理效率，为解决过度思考问题提供了一种灵活、通用的方案。
...
### [159] [Detecting and Mitigating Bias by Treating Fairness as a Symmetry Operation](https://arxiv.org/abs/2606.06514)
*Nishit Singh*

Main category: cs.AI

TL;DR: 提出一种基于对称性恢复的正则化方法，在不需因果图知识的情况下有效减少机器学习分类器的偏见，准确度损失小。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统在高风险社会经济场景中常显示偏见，需要有效的公平性干预方法。

Method: 将偏见形式化为敏感属性反事实切换下的对称性破坏，通过损失正则化恢复对称性，适用于任何可定义为位翻转的敏感属性。

Result: 在四个合成数据集上实现高达90%的违规减少，准确度成本约为5%。

Conclusion: 该方法计算轻量、通用，无需因果图，适合处理主流基准外的本地歧视源。
...
### [160] [CrowdMath: A Dataset of Crowdsourced Mathematical Research Discussions](https://arxiv.org/abs/2606.06526)
*Sherin Muckatira,Jesse Geneson,Slava Gerovitch,Pavel Etingof,Mikhail Gronas,Anna Rumshisky*

Main category: cs.AI

TL;DR: CrowdMath, a dataset of collaborative mathematical problem-solving discussions, shows that LLMs can follow local discussion flow but struggle to identify functional roles of contributions.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks evaluate well-specified problems with final answers, but do not capture collaborative open-problem solving.

Method: We introduce CrowdMath, a dataset of 164 expert-annotated progress chains from the MIT PRIMES–AoPS program, with posts labeled by functional roles. We define evaluation tasks and benchmark six frontier models.

Result: Models achieve 83–88% accuracy on next-post prediction but only 0.42 macro-F1 on post-role classification.

Conclusion: CrowdMath exposes a gap between solving well-specified mathematical problems and understanding collaborative mathematical progress.
...
### [161] [Attack Selection in Agentic AI Control Evaluations Meaningfully Decreases Safety](https://arxiv.org/abs/2606.06529)
*Catherine Ge-Wang,Tyler Crosse,Benjamin Hadad IV,Joachim Schaeffer,Ram Potham,Tyler Tracy*

Main category: cs.AI

TL;DR: 攻击选择显著降低AI控制评估的安全估计，当前评估可能过于乐观。


<details>
  <summary>Details</summary>
Motivation: 现有AI控制评估假设攻击者不策略性地选择攻击时间，可能低估实际风险。

Method: 将攻击决策分解为开始策略和停止策略，并在BashArena和LinuxArena中进行实验。

Result: 在1%审计预算下，开始策略降低安全性20pp，停止策略降低20-28pp。

Conclusion: 攻击选择是重要的安全因素，现有评估应纳入以得到更现实的估计。
...
### [162] [OpenSkill: Open-World Self-Evolution for LLM Agents](https://arxiv.org/abs/2606.06741)
...
TL;DR: OpenSkill enables agents to self-evolve from scratch without target-task supervision by bootstrapping skills and verification from open-world resources.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for self-evolving agents assume a usable learning loop (e.g., curated skills, successful trajectories, or verifier signals), but real open-world deployments may provide only a task prompt, necessitating a method that builds both skills and verification signals from scratch.

Method: OpenSkill bootstraps the self-evolution loop by acquiring grounded knowledge and verification anchors from documentation, repositories, and the web, synthesizing them into transferable skills, and refining those skills against self-built virtual tasks grounded in the anchors, without target-task supervision.

Result: Across three benchmarks and two target agents, OpenSkill achieves the best automated pass rate while satisfying the no-supervision constraint; its skills transfer across models without model-specific adaptation, and its self-built verifier aligns with ground-truth outcomes.

Conclusion: Open-world self-evolution is feasible without target-task supervision, as the open world can supply both knowledge to be learned and a supervision-independent practice environment.
...
### [164] [Quantum-Inspired Trace-Augmented Evidence Selection for Reasoning over Structured Hypothesis Spaces](https://arxiv.org/abs/2606.06941)
*Laura Wynter,Nirvik Sahoo,Paul Griffin*

Main category: cs.AI

TL;DR: 提出EP-HUBO方法，将CoT推理片段选择视为组合优化问题，通过证据池优化聚合推理，在法律基准上优于多数投票，尤其低污染领域。


<details>
  <summary>Details</summary>
Motivation: 现有多数投票方法忽略证据强度，导致在法律等证据密集型任务中错误，需要更好的推理聚合方式。

Method: 生成多个CoT痕迹，解析为每个假设的证据池，使用高阶无约束二元优化选择最强证据，最后用前沿模型裁决。

Result: 在证据密集型法律基准上，EP-HUBO优于多数投票，在低污染领域表现最佳。

Conclusion: HUBO优化提供原则性聚合方式，保留少数但正确假设，适用于低污染领域。
...
### [165] [Accounting for Context: Shaping Moral Credences for Value Alignment](https://arxiv.org/abs/2606.06972)
*Jazon Szabo,Sanjay Modgil*

Main category: cs.AI

TL;DR: 在道德不确定性下考虑语境因素时，弱帕累托原则被违反，这类似于辛普森悖论，揭示了忽略语境因素的聚合机制的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有道德不确定性聚合机制忽略了语境因素，如结果主义对行动后果的准确了解这一不现实假设。

Method: 形式化了考虑语境因素的道德不确定性下代理决策模型。

Result: 证明了弱帕累托原则的违反，并将其解释为辛普森悖论的变体。

Conclusion: 揭示了忽略语境因素的聚合机制的局限性。
...
### [166] [Exploring Agentic Tool-Calling Decisions via Uncertainty-Aligned Reinforcement Learning](https://arxiv.org/abs/2606.06976)
*Yijin Zhou,Linqian Zeng,Xiaoya Lu,Wenyuan Xie,Dongrui Liu,Junchi Yan,Jing Shao*

Main category: cs.AI

TL;DR: 提出TRUST方法，利用不确定性量化改进奖励设计，提升LLM代理的工具使用决策质量和性能。


<details>
  <summary>Details</summary>
Motivation: LLM代理常做出次优工具使用决策，现有方法忽略不确定性特征，导致过度自信错误和弱探索信号。

Method: TRUST将不确定性量化融入奖励设计作为排斥力以保持不确定性分离，并通过轻量级关键轮注释进行多轮轨迹统一后训练。

Result: 在多种工具使用基准上，TRUST一致提升决策质量和代理性能，并在优化过程中保持更可靠的不确定性估计。

Conclusion: TRUST能有效增强代理决策能力并维持不确定性估计的可靠性。
...
### [167] [The Sim-to-Real Gap of Foundation Model Agents: A Unified MDP Perspective](https://arxiv.org/abs/2606.07017)
*Xiaoou Liu,Tiejin Chen,Weibo Li,Xiyang Hu,Hua Wei*

Main category: cs.AI

TL;DR: 本文提出将基础模型代理的模拟-现实差距形式化为MDP问题，并倡导采用领域随机化等经典解决方案，以提升代理的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 基础模型代理在现实部署中存在模拟到现实的差距，而当前社区对此问题的处理不够系统化，需要构建统一的理论框架。

Method: 围绕MDP的四个要素（观察、动作、转移、奖励）分析差距，并倡导采用领域随机化等已有方法。

Result: 通过多语言工具调用等例子展示了观察空间差距导致的操作无效问题。

Conclusion: 旨在推动范式转变，建立统一词汇和标准化压力测试基准，以开发高可信代理。
...
### [168] [StainFlow: Entity-Stain Tracking and Evidence Linking for Process Rewards in GUI Agents](https://arxiv.org/abs/2606.07027)
...
TL;DR: StainFlow通过实体污点流机制改进过程奖励模型，解决GUI代理训练中奖励稀疏和现有PRMs的局限性。


<details>
  <summary>Details</summary>
Motivation: 强化学习中的轨迹级成功反馈过于稀疏，现有过程奖励模型存在全局里程碑分解主观单一和局部判断窗口固定导致信号稀释的问题。

Method: 受网络流分析中污点追踪机制启发，提出StainFlow，包含全局实体污点追踪模块和局部污点证据链接模块，分别通过追踪实体污点浓度变化客观分离任务阶段，以及动态构建高密度证据窗口验证关键节点。

Result: 在AndroidWorld和OGRBench上，在线RL成功相对提升3.2%，轨迹完成判断准确率提升1.8%。

Conclusion: StainFlow有效缓解了现有PRMs的局限性，为GUI代理提供了更准确的训练信号，提升了性能。
...
### [169] [Hierarchical Semantic-Constrained Heterogeneous Graph for Audio-Visual Event Localization](https://arxiv.org/abs/2606.07033)
*Zhe Yang,Ruyi Zhang,Hongtao Chen,Wenrui Li,Hengyu Man,Wangmeng Zuo,Xiaopeng Fan*

Main category: cs.AI

TL;DR: 提出层次语义约束异构图框架，结合欧几里得和双曲空间建模，解决开放词汇音频-视觉事件定位中的跨模态和多级别语义一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在欧几里得空间学习联合表示，但缺乏对未见类别的监督导致多时间尺度音频-视觉一致性难以保持，且缺乏片段级和视频级语义之间的层次约束。

Method: 构建包含音频、视觉片段节点及视频级节点的异质层次图，使用多方向时序边和双阈值滤波门控融合策略，引入双向语义约束，并将多级表示与文本原型映射到双曲空间，通过层次蕴涵正则化损失学习。

Result: 在OV-AVEL基准上优于现有方法，消融研究验证了各组件的有效性。

Conclusion: 该方法有效解决了跨模态和多级别语义一致性问题，提升了开放词汇音频-视觉事件定位性能。
...
### [170] [Front-to-Attractors: Modifying the Front-to-Front Heuristic in Bidirectional Search](https://arxiv.org/abs/2606.07047)
*Alvin Zou,Muhammad Suhail Saleem,Maxim Likhachev*

Main category: cs.AI

TL;DR: 提出了一种新的双向搜索启发式类F2A，平衡了信息性和计算成本。


<details>
  <summary>Details</summary>
Motivation: F2F启发式虽然信息丰富但计算开销大，F2E启发式计算快但信息不足。

Method: 提出了前向吸引子（F2A）启发式，通过评估到对搜索方向中少量动态维护的吸引子的距离来近似整个前沿。

Result: F2A相比F2F减少了多达11.2倍的成对评估，相比F2E平均减少了4.8倍的节点扩展。

Conclusion: F2A在保持最优性保证的同时，显著降低了计算成本，提高了效率。
...
### [171] [Think Fast: Estimating No-CoT Task-Completion Time Horizons of Frontier AI Models](https://arxiv.org/abs/2606.07157)
...
TL;DR: 前沿AI模型在没有显式思维链推理的情况下，其任务完成时间快速增加，预计到2030年可超过25分钟，这对依赖监控思维链的安全策略构成挑战。


<details>
  <summary>Details</summary>
Motivation: AI安全依赖对思维链推理的监控，但模型可能进行内部复杂推理而不产生显式思考标记，削弱监督，因此需要评估模型无思维链推理的能力及其增长趋势。

Method: 在43个基准测试、超过30,000个问题上测量前沿模型无思维链推理的表现，通过50%任务完成时间（TH）和推理标记数（以o3-mini为基准）量化，并与人类对比，分析历史趋势。

Result: 无思维链的50% TH每约一年翻倍，GPT-5.5的TH超过3分钟，推理标记超过1500；中位预测TH在2028年超过7分钟，2030年超过25分钟。

Conclusion: 建议前沿AI开发者明确跟踪无思维链推理能力，因其快速增长可能影响安全监控。
...
### [172] [Off-Policy Evaluation with Strategic Agents via Local Disclosure](https://arxiv.org/abs/2606.07308)
*Kiet Q. H. Vo,Abbavaram Gowtham Reddy,Julian Rodemann,Siu Lun Chau,Krikamol Muandet*

Main category: cs.AI

TL;DR: 通过事后解释披露局部信息，提出双重稳健估计器进行策略行为下的离线策略评估。


<details>
  <summary>Details</summary>
Motivation: 现有离线策略评估方法在代理人战略行为导致的协变量偏移下失效，且相关工作假设过强，缺乏实用性。

Method: 利用事后解释披露局部信息以恢复策略前协变量，估计代理人响应模型（假设成本敏感性服从条件对数正态分布），并构建双重稳健估计器。

Result: 证明了估计量的一致性，并通过实证验证了方法的有效性。

Conclusion: 交互设计通过揭示代理人战略响应中的隐藏结构，能有效减轻信息不对称。
...
### [173] [Act As a Real Researcher: A Suite of Benchmarks Evaluating Frontier LLMs and Agentic Harnesses in Research Lifecycle](https://arxiv.org/abs/2606.07462)
*Jiayu Wang,Weijiang Lv,Bowen Fu,Jing Fu,Jiayi Song,Lingyu Zhang,Lanxuan Xue,Luodi Chen,Zepeng Xin,Kaiyu Li,Xiangyong Cao*

Main category: cs.AI

TL;DR: 提出了AARR基准系列和首个基准AARRI-Bench，用于评估AI代理是否像真实研究人员一样进行细微推理，实验发现最佳配置成功率仅68.3%，表明需要关注研究行为而非复杂框架。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理在复杂编码和自主实验中表现突出，但缺乏现场敏感性、研究伦理和科学判断，无法完全替代人类研究员，因此需要新的评估基准。

Method: 构建AARR基准系列，专注于微观研究场景下代理的专业性、细致性和推理能力；基于AARRI-Bench在多个前沿模型和代理系统上进行实验。

Result: 最佳配置（Mini-SWE-Agent with Claude Opus 4.7）仅达到68.3%成功率，频繁忽略人类研究员容易发现的微妙关键细节。

Conclusion: 开发研究员级别的AI需要进一步探索研究行为，而非仅仅依赖复杂的代理框架。
...
TL;DR: SafeGene是一种可重用的安全适配器模块，通过从对齐-退化模型差异中提取可迁移安全向量，并在下游任务中通过少样本层系数重校准来恢复安全对齐，有效降低有害响应率同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 下游微调会削弱LLMs的安全对齐，即使训练数据无害，也使模型更易受恶意提示攻击，且需要频繁的安全恢复。

Method: 从对齐-退化模型差异中获得安全表示，通过数据感知层选择精炼为任务可迁移安全向量，并通过少样本层系数重校准表达于下游任务模型。

Result: 在多个模型族、下游任务和安全评判中，SafeGene增强的模型降低了有害响应率同时保持下游性能，优于代表性安全适应方法。

Conclusion: 安全能力可以解耦为独立可重用的适配器表示，从而有效解决跨任务的安全恢复问题。
...
TL;DR: 提出首个使用Lean4形式语言建模和验证LLM代理工作流的框架，有效提升工作流可靠性。


<details>
  <summary>Details</summary>
Motivation: LLM代理系统缺乏形式化方法来指定、验证和调试多步工作流，导致不可靠，类似自然语言在数学中的模糊性问题。

Method: 构建Lean4Agent框架，包含FormalAgentLib库用于形式化建模和验证工作流语义一致性，以及LeanEvolve用于利用验证结果自动改进工作流。

Result: 在SWE-Bench-Verified和ELAIP-Bench子集上，验证通过的工作流比未通过的性能平均高11.94%，LeanEvolve进一步将SWE性能提升7.47%。

Conclusion: 为使用依赖类型形式语言建模和验证代理行为奠定了基础，开辟了新的研究领域。
...
### [176] [CARVE-Q: Quantum-Proposed, Classically Certified Interactive Driving Repair](https://arxiv.org/abs/2606.06531)
*Yifan Wang*

Main category: cs.AI

TL;DR: 提出CARVE和CARVE-Q，用于驾驶否决操作的可认证修复，利用量子加速格搜索，保证规则合规和责任分配。


<details>
  <summary>Details</summary>
Motivation: 预测和博弈规划无法提供合法、可审计、责任有界的修复证明，需要一种证书架构确保修复遵守硬规则和成本分配。

Method: CARVE构建有限修复格并发出包含绑定规则、联合修复、合作包络、成本分配和后备计划的结构化证书；CARVE-Q用量子最小查找加速格搜索，保留经典安全权威。

Result: 在多达65,536个任务的格上实现量子最小查找，在INTERACTION数据集上达到100%路权尊重、100%责任一致性和零优先级误报。

Conclusion: 建立信任有界的量子AI模式：量子提出方案，CARVE负责认证，确保安全与责任。
...
### [177] [Position: Don't Just "Fix it in Post": A Science of AI Must Study Training Dynamics](https://arxiv.org/abs/2606.06533)
*Stella Biderman,Mohammad Aflah Khan,Niloofar Mireshghallah,Catherine Arnett,Fazl Barez,Naomi Saphra*

Main category: cs.AI

TL;DR: 主张AI科学必须研究训练动态，以实现对模型行为的预测、干预和设计。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究将模型视为静态产物，只关注后验行为分析，忽视了模型行为涌现的成因。

Method: 基于科学哲学与历史视角，审视机械可解释性、公平性、记忆与简单性偏差等领域的研究进展。

Result: 揭示缩放定律在损失预测上的成功尚未迁移至能力、偏见、鲁棒性等安全相关行为，并列出具体开放问题。

Conclusion: AI科学应超越事后修复，建立从早期信号预测、干预错误轨迹到设计可靠训练流程的渐进式理解框架。
...
### [178] [Accelerated Fourier SAT (AFSAT): Fully Realising a GPU-based Symmetric Pseudo-Boolean SAT Solver](https://arxiv.org/abs/2606.06641)
*Cody J Christopher,Charles Gretton*

Main category: cs.AI

TL;DR: 提出了AFSAT，一个GPU加速的伪布尔SAT求解器，基于连续局部搜索，改进了数值稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 解决FastFourierSAT概念验证中的内存延迟、浮点表示限制，提升稳定性和效率。

Method: 使用JAX编译器，利用纯函数组合、自动向量化、自动微分和JIT编译，进行大规模并行连续局部搜索，并通过定制DFT实现处理浮点限制。

Result: 显著改善数值稳定性、运行时性能和内存效率，并在多加速器下实现接近线性的吞吐量。

Conclusion: AFSAT通过自动并行化和紧凑表示克服了先前限制，是一种高效的伪布尔SAT求解器。
...
### [179] [A Study of Parallel Continuous Local Search](https://arxiv.org/abs/2606.06656)
*Cody J Christopher,Charles Gretton*

Main category: cs.AI

TL;DR: 研究并行连续局部搜索（CLS）解决具有对称伪布尔约束的SAT问题，发现冗余约束抑制收敛、CLS在混合设置中有效、局部搜索收益递减。


<details>
  <summary>Details</summary>
Motivation: 探索CLS作为布尔可满足性问题（特别是含对称伪布尔约束）的求解方法。

Method: 将n变量PB可满足性松弛为n维超立方体上的连续优化问题，目标函数可微，全局最小化对应满足赋值，并通过实证实验分析。

Result: 冗余约束抑制收敛；CLS在混合设置中快速完成部分分配；局部搜索因鞍点密集快速达到解质量稳定分布，附加步骤收益递减。

Conclusion: 研究结果为在现代加速硬件上实际应用CLS求解SAT问题提供指导。
...
### [180] [AEGIS: A Backup Reflex for Physical AI](https://arxiv.org/abs/2606.06660)
*Josef Chen*

Main category: cs.AI

TL;DR: AEGIS通过在弱策略的冻结激活上使用轻量级探针检测高风险步骤，并仅在需要时切换到更强的策略，在LIBERO-Spatial上恢复了10.1%的丢失轨迹，仅激活了38%的步骤。


<details>
  <summary>Details</summary>
Motivation: 长时程机器人操作倾向于逐渐失败：一步错误会降低状态，策略陷入无法恢复的盆地。失败通常在发生之前可见。

Method: AEGIS使用轻量级探针从弱策略的冻结激活中检测高风险步骤，当探针标记某步骤时，切换到单独更强的策略，但仅针对需要它的步骤。

Result: 在LIBERO-Spatial上，AEGIS恢复了弱策略单独丢失的轨迹的10.1%，而预算匹配的盲升级为4.6%，随机触发安慰剂为5.1%；探针早期窗口AUROC为0.764；强策略仅在38%的步骤上激活。

Conclusion: 选择性升级的时间点带来了显著的收益，并且通过预注册分析验证了该方法。
...
### [181] [A Geometric Account of Activation Steering through Angle-Norm Decomposition](https://arxiv.org/abs/2606.06735)
*Georgii Aparin,Tatiana Gaintseva*

Main category: cs.AI

TL;DR: 激活转向的效果主要来自角度变化而非范数，但范数影响稳定性，建议用角度和径向分量参数化。


<details>
  <summary>Details</summary>
Motivation: 重新审视球形转向核假设（隐藏状态范数不携带概念相关信息）并解释其动机，以解决线性转向的局限性。

Method: 通过控制实验分离角度和径向分量，比较转向方法如何耦合几何效应。

Result: 在七个语言模型上，概念主要在角度结构中表示，但范数对稳定性和下游效应重要。

Conclusion: 激活转向应参数化为可解释的角度和径向分量，而非单一系数。
...
TL;DR: 提出从异构交互证据自动构建技能的方法，通过工作流中间表示RWSA和W2S框架，提高了行为重放一致性。


<details>
  <summary>Details</summary>
Motivation: 高质量技能的手工编写成本高，且痕迹到技能构建不是简单摘要，因为痕迹碎片化、冗余且可能缺少安全关键行为。

Method: 提出RWSA中间表示和W2S框架，RWSA将技能分解为工作流结构、执行语义和运行时附件，W2S通过分割痕迹、诱导草稿、对齐结构、协调分支和压缩冗余来构建技能。

Result: 在70个技能上，W2S比基线提高10.5%的行为重放一致性。

Conclusion: 痕迹应被视为可执行运行时规范而非可压缩文本，自动技能构建需要专门方法。
...
### [183] [Teaching the Way, Not the Answer: Privileged Tutoring Distillation for Multimodal Policy Optimization](https://arxiv.org/abs/2606.07000)
*Shizhe Xiang,Ke An,Wenlong Yu,Yue Liu,Jian Luan,Pei Fu,Qilong Wang*

Main category: cs.AI

TL;DR: 提出PTD-PO框架，通过特权提示提供密集token分布监督，无需暴露答案，显著提升LVLMs的多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: RLVR的稀疏奖励导致探索效率低，外部教师蒸馏计算开销大，答案条件调优易引发捷径行为；需一种密集指导但不暴露答案的优化方法。

Method: 构建结构化特权提示（含空间注意力引导与中间推理步骤），通过上下文学习产生逐token分布监督；采用Top-K Jensen-Shannon散度目标，对齐学生失败rollouts与提示增强参考模型，稳定分布偏移。

Result: 在2B-8B参数LVLMs上，PTD-PO持续优于RLVR及蒸馏基线，有效减轻熵崩塌，提升复杂多模态推理准确率。

Conclusion: PTD-PO通过特权蒸馏策略优化，在不泄露答案前提下提供密集监督，实现了更高效稳定的多模态推理训练。
...
TL;DR: Proposes the Glassbox Framework, using Bayesian networks as transparent ante-hoc mediation layers for LLMs to enable auditable reasoning and accountability.


<details>
  <summary>Details</summary>
Motivation: Post-hoc explanations are unstable and non-contestable; opacity is untenable in high-stakes settings like public administration and healthcare.

Method: The Glassbox Framework integrates Bayesian networks before inference to encode domain knowledge and causal assumptions, providing auditable reasoning traces.

Result: Identifies foundational challenges in semantic alignment, dynamic model construction, probabilistic grounding, and human governance for scaling the framework.

Conclusion: Shifting from post-hoc to ante-hoc probabilistic mediation offers a principled path toward accountable AI systems.
...
TL;DR: 提出了一个在线上下文Pandora盒子模型用于自适应查询和选择LLM API，并设计了一个结合GMM估计和UCB置信边界的策略，实现了维度相关的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 受大语言模型（LLM）级联的启发，需要一种自适应查询和选择LLM API的方法，而现有模型不适用于输出介导的反馈结构。

Method: 直接建模上下文相关的保留指数，假设参数化结构，采用广义矩估计（GMM）估计保留指数，并结合UCB风格的置信界进行探索，同时维护共享的输出级奖励评估器。

Result: 在正则条件下，所提策略在T个周期上实现了维度相关的$\widetilde{O}(\sqrt{T})$累积遗憾。

Conclusion: 该工作为LLM API的自适应选择提供了理论保证，展示了在输出介导反馈结构下学习的有效性。
...
### [186] [How AI Agents Reshape Knowledge Work: Autonomy, Efficiency, and Scope](https://arxiv.org/abs/2606.07489)
*Jeremy Yang,Kate Zyskowski,Noah Yonack,Jerry Ma*

Main category: cs.AI

TL;DR: AI代理通过自主执行任务，显著加速工作流程，提高质量并降低成本。


<details>
  <summary>Details</summary>
Motivation: 研究AI系统从对话助手向自主代理转变如何加速和重塑知识工作。

Method: 利用Perplexity的Search和Computer产品的生产数据，以相似初始查询的会话作为自然实验，比较代理与搜索在执行相同任务时的差异。

Result: 三个主要发现：1) Computer每个会话执行26分钟自主工作，不满率低55%；2) Computer将完成时间从269分钟降至36分钟，成本降低94%；3) Computer扩展工作范围，涉及跨职业、高阶认知和复合任务。

Conclusion: AI代理能够加速工作流程，提升输出质量，降低成本，并扩大自动化工作的广度和深度。
