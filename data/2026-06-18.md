...
TL;DR: 提出检索增强图像字幕框架，利用层级多模态文章检索与LLM生成上下文丰富描述，在EVENTA挑战赛获第五。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖视觉线索，难以生成包含对象属性、事件上下文等深层信息的全面描述。

Method: 采用层级多模态文章检索，考虑文章结构特征（标题、正文权重）与视觉布局，结合内容-视觉、视觉-视觉及话语位置相似度；然后VLM生成初始描述，基于描述分割文章相关信息，LLM合并生成最终字幕。

Result: 在OpenEvent-V1私有测试集上得分为0.2824，排名ACM Multimedia EVENTA 2025第五名。

Conclusion: 该框架有效利用外部知识增强图像字幕的上下文丰富性，但性能仍有提升空间。
...
### [2] [LARE: Low-Attention Region Encoding for Text-Image Retrieval](https://arxiv.org/abs/2606.18885)
...
TL;DR: 提出LARE框架，通过双编码低注意力区域和完整图像，改善拥挤场景中的图像检索。


<details>
  <summary>Details</summary>
Motivation: 传统视觉编码器在拥挤场景中存在显著性偏差，忽略低注意力区域，影响细粒度检索。

Method: 提出LARE框架，采用双编码策略并行编码低注意力区域和完整图像，生成更多样和信息丰富的嵌入；同时引入Dense-Set数据集，重新标注以描述低注意力区域。

Result: 实验表明LARE通过保存微妙的非主导视觉线索，提高了检索性能。

Conclusion: LARE有效改善了拥挤场景中的图像检索，并提供了新的评估基准。
...
### [3] [DREAM: Extending Vision-Language Models with Dual-Objective Encoding for Cross-Modal Retrieval](https://arxiv.org/abs/2606.19062)
*Kaleem Ullah,Altaf Hussain,Muhammad Munsif,Sung Wook Baik*

Main category: cs.CV

TL;DR: 提出了DREAM模型，通过混合语言建模和层次视觉编码器提升视频检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频检索系统难以捕捉复杂语义和时间动态，需要更好的跨模态对齐。

Method: DREAM采用混合语言建模（masked和permuted）和层次视觉编码器（级联组注意力），实现双路径表示增强和对齐。

Result: 在MSRVTT、MSVD和LSMDC上分别取得49.4%、49.7%和27.3%的R1得分，达到新SOTA。

Conclusion: 层次注意力和双目标文本建模有效促进鲁棒、上下文感知的视频检索，为跨模态表示学习提供方向。
...
### [4] [Show, Don't Ask: Generative Visual Disambiguation for Composed Image Retrieval with Turn-Valid Coverage](https://arxiv.org/abs/2606.18992)
*Amsisan Tran,Baogh Le,Tuan Kiet Pham,Sui Yang Guang*

Main category: cs.CV

TL;DR: 提出CLARA框架，通过视觉替代面板和重新加权校准来解决组合图像检索中的歧义，并在多轮交互中保持覆盖保证。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用文本澄清问题，但覆盖保证仅在第一轮有效，且文本问题不足以解决细粒度视觉差异。

Method: CLARA通过展示视觉替代面板让用户选择最接近目标的原型图像，利用似然比重新加权校准以维持多轮覆盖保证，并约束原型表示当前候选集且从真实语料库中选取。

Result: 在开放域和时尚基准测试中，CLARA匹配了单轮最新检索性能，在多轮交互中保持名义覆盖，并以更少轮数找到目标，特别是在涉及视角或细粒度属性的歧义时表现出色。

Conclusion: CLARA通过视觉澄清有效解决了用户意图歧义，并在多轮交互中提供了有效的覆盖保证。
...
### [5] [Aerial-ground LiDAR place recognition with patch-level self-supervised learning and expanded reciprocal re-ranking](https://arxiv.org/abs/2606.18583)
*Yandi Yang,Xianghong Zou,Jianping Li,Haofeng Xie,Saurav Uprety,Hongzhou Yang,Naser El-Sheimy*

Main category: cs.CV

TL;DR: 提出一种航拍-地面LiDAR地点识别的检索与重排框架，通过多尺度自监督学习和扩展互惠重排算法，显著提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 传统地面LiDAR地点识别存在预访要求、覆盖不完全和视角有限等缺点，利用全覆盖ALS数据作为先验可克服这些缺陷，但面临域差距和假阳性挑战。

Method: 提出检索和重排框架：检索网络结合多尺度局部块自监督学习和场景级学习增强全局特征判别性；扩展互惠重排算法利用ALS结构化分布优化特征并更新相似度矩阵。

Result: 在CS-Urban-Scenes上，检索网络提升平均Recall@1 9.8%、Recall@1% 3.2%；在CS-Campus3D上也最优。ER重排进一步使平均Recall@1提升4.9%和10.2%。

Conclusion: 该方法有效解决航拍-地面LiDAR识别中的域差距和假阳性问题，显著优于现有方法。
...
### [6] [Physics-IQ Verified](https://arxiv.org/abs/2606.18943)
*Tim Rädsch,Yuki M Asano,Hilde Kuehne,Stefan Bauer,Priyank Jaini,Robert Geirhos,Carsten T. Lüth*

Main category: cs.CV

TL;DR: 对Physics-IQ基准进行系统审计，发现其缺陷并提出改进方案，得到Physics-IQ Verified基准，提升了测量视频生成模型物理理解能力的可靠性。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型需要理解物理现实，但现有Physics-IQ基准存在不足，需要更可靠的评估方法。

Method: 对Physics-IQ基准进行系统审计，改进提示和真实质量，引入样本级评分系统，重新计算样本和指标权重。

Result: 改进了57.6%的样本和34.8%的提示，在六个图像到视频生成模型的比较中观察到中度但显著的排名变化（Kendall's tau=0.46）。
...
### [7] [Mem-World: Memory-Augmented Action-Conditioned World Models for Persistent Robot Manipulation](https://arxiv.org/abs/2606.18960)
*Zirui Zheng,Jiaqian Yu,Xiongfeng Peng,jun shi,Mingyi Li,Chao Zhang,Weiming Li,Dong Wang,Huchuan Lu,Xu Jia*

Main category: cs.CV

TL;DR: 提出Mem-World，一种记忆增强的多视图动作条件世界模型，通过4D手腕视图中心surfel索引记忆解决遮挡和快速运动下的持久视频生成问题，在策略评估和长时任务策略学习中取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 在机器人操作中，世界模型面临频繁末端执行器遮挡和手腕相机快速运动导致当前观察不足以预测未来视图，模型遗忘或幻觉场景细节，现有记忆检索策略在动态场景中难以识别有用历史信息。

Method: 提出Mem-World，核心是W-VMem，一种4D手腕视图中心surfel索引记忆，将历史观察锚定到随时间演变的表面元素，通过显式建模场景元素被观察的时间和位置，实现几何感知的历史帧检索；生成时通过surfel渲染和评分选择信息丰富且非冗余的相关历史帧。

Result: 在复杂操作场景中生成持久视频rollout；相比Ctrl-World，策略评估与真实世界的Pearson相关性提高14.5%；通过合成数据生成支持策略改进，长时任务成功率从58%提升至72%。

Conclusion: Mem-World通过记忆增强的世界模型有效克服了操作中的遗忘和幻觉问题，提升了世界模型的持续生成能力和下游策略评估与学习性能。
...
### [8] [PEFT-MedSAM: Efficient Fine-Tuning of Medical Foundation Models for Explainable Skin Lesion Segmentation](https://arxiv.org/abs/2606.18707)
*Asad Channa,Abdullah Khan,Asghar Ali Chandio,Aamir Akbar,Shahzad Memon,Aqib Hussain,Ameer Hamza*

Main category: cs.CV

TL;DR: 提出PEFT-MedSAM方法，通过微调轻量级掩码解码器实现皮肤病变分割，在ISIC 2018上达到0.9411 Dice系数。


<details>
  <summary>Details</summary>
Motivation: 自动化皮肤病变分割有助于早期黑色素瘤检测，但现有深度学习方法性能不佳。

Method: 基于MedSAM，冻结图像编码器和提示编码器，仅训练轻量级掩码解码器。

Result: 在ISIC 2018上Dice为0.9411，IoU为0.8918；在PH2上Dice为0.9467±0.0310；优于U-Net和零样本MedSAM。

Conclusion: PEFT-MedSAM有效且通过Grad-CAM可解释性增强临床可信度。
...
### [9] [SP-TransientBench: A Real-Captured Single Photon Perception Benchmark](https://arxiv.org/abs/2606.18952)
...
TL;DR: 提出了一个真实捕获的多任务基准SP-TransientBench (STB)，用于单光子感知，涵盖深度估计、多视图重建和3D语义理解。


<details>
  <summary>Details</summary>
Motivation: 现有单光子感知研究受限于模拟数据或小规模控制捕获，缺乏真实场景下的系统评估，特别是多任务基准。

Method: 使用固态单光子LiDAR捕获10个场景的10297个视图，提供完整TOF直方图、多返回行为、标准元数据、校准姿态和语义标注。定义任务特定数据分割和评估协议。

Result: 建立了第一个用于真实单光子感知的多任务基准，支持可重复的评测，数据集和代码将公开。

Conclusion: STB填补了真实单光子感知评估的空白，将推动该领域研究。
...
### [10] [AMALIA-VL: A Native European Portuguese Open-Source Vision and Language Model](https://arxiv.org/abs/2606.19100)
...
TL;DR: AMALIA-VL是第一个开源的欧洲葡萄牙语指令微调大型视觉语言模型，采用三阶段训练方法和专门的数据混搭，为pt-PT建立了强基线。


<details>
  <summary>Details</summary>
Motivation: 欧洲葡萄牙语在现有开源多模态模型中系统性地被忽视，缺乏专门为其构建的模型。

Method: 使用高分辨率视觉编码器与动态图像平铺，结合pt-PT优化的语言模型，通过三阶段训练：视觉-语言对齐、通用视觉指令微调和偏好优化，并构建以pt-PT为中心的多模态数据混搭。

Result: 评估显示AMALIA-VL为开源pt-PT建立了强基线。

Conclusion: 将发布模型权重、训练数据和构建管道，以及机器翻译的pt-PT评估基准，以促进pt-PT LVLM开发的民主化。
...
### [11] [ProductConsistency: Improving Product Identity Preservation in Instruction-Based Image Editing via SFT and RL](https://arxiv.org/abs/2606.19103)
*Mukund Khanna,Raj Singh Yadav,Kunal Singh*

Main category: cs.CV

TL;DR: 提出ProductConsistency数据集和循环一致性奖励，用于提升产品为中心的指令图像编辑中的产品一致性。


<details>
  <summary>Details</summary>
Motivation: 现有指令图像编辑模型在产品中心场景中难以保持产品特征、品牌和文本元素，且缺乏相关数据集。

Method: 构建ProductConsistency数据集（87k SFT样本、869 RL图像及基准），提出循环一致性奖励指导RL训练，微调Qwen-Image-Edit-2511和Flux.1-Kontext-dev。

Result: 在OCR、Perceptual指标和MLLM评估中一致改进，Qwen模型字符错误率降低5倍。

Conclusion: 所提数据集和训练方法有效提升产品一致性、文本渲染和视觉质量。
...
### [12] [Hallucination Detection and Correction in Medical VLMs via Counter-Evidence Verification](https://arxiv.org/abs/2606.18609)
*Nan Zhou,Ke Zou,Meng Liu,Linchao He,Jiaqi Zhu,Yi Zhang,Hu Chen,Huazhu Fu*

Main category: cs.CV

TL;DR: CoEV是一个无需训练的即插即用框架，通过双向证据验证检测和纠正医学视觉语言模型中的幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注文本事实不一致，很少验证模型注意力是否真正反映视觉证据，导致医学VLM的可靠性受幻觉影响。

Method: CoEV通过双向验证文本断言与视觉证据的一致性，并将每个断言分配到四象限诊断图中，实现无需重新训练的幻觉检测与纠正。

Result: 在四个医学数据集上，幻觉检测的PR-AUC和ROC-AUC平均提升3.0%和3.9%，特定VQA场景提升18.5%；幻觉纠正的Micro-F1提升12.5%，幻觉率降低11.9%，同时提升医学VQA准确率。

Conclusion: CoEV能够可靠地检测和纠正幻觉，为临床诊断提供基于证据的可信线索。
...
### [13] [Rethinking Air-Ground Collaboration: A Progressive Cross-Task Benchmark and Socialized Learning Framework](https://arxiv.org/abs/2606.18841)
*Zhoupeng Guo,Yunqi Zhu,Zhihe Fan,Xinjie Yao,Ruipu Zhao,Boan Tao,Yiming Sun,Zhen Wang,Pengfei Zhu*

Main category: cs.CV

TL;DR: 提出了AGPC基准和SCP框架，通过渐进式跨任务协作与Dual-Layer Router模块实现异构空中地面感知，提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究将协作视为单任务跨视图融合，忽略了定位、关联和解析的功能依赖，且异质性导致负迁移。

Method: 构建AGPC基准，提出SCP框架，其核心模块DLR解耦输入侧多尺度专家选择和输出侧任务条件调制。

Result: SCP实现3.73%协同进化增益和7.86%平均下游性能改进。

Conclusion: 任务条件协作比统一融合更有效于异构空中地面感知。
...
### [14] [Quantification of Uncertainty with Adversarial Models in Medical Image Segmentation](https://arxiv.org/abs/2606.18860)
*Hana Jebril,Thomas Pinetz,Günter Klambauer,Hrvoje Bogunović*

Main category: cs.CV

TL;DR: QUAM-SM通过对抗搜索识别脆弱像素，提升不确定性校准和边界敏感性。


<details>
  <summary>Details</summary>
Motivation: 标准深度学习模型在病理边界处存在校准误差，导致过度自信的预测，掩盖了潜在弱点。

Method: 提出QUAM-SM后处理框架，利用有针对性的对抗搜索暴露预测不稳定性，并区分认知不确定性和偶然不确定性。

Result: 在两个公开数据集上，QUAM-SM在可靠性和边界敏感性上优于标准及近期不确定性估计方法。

Conclusion: QUAM-SM能提升临床工作流的可靠性，有助于治疗规划和手术干预。
...
### [15] [URDF Synthesis from RGB-D Sequences via Differentiable Joint Inference and Energy-Consistent Verification](https://arxiv.org/abs/2606.18861)
*Xinze Zhang*

Main category: cs.CV

TL;DR: KinemaForge从短RGB-D序列联合推断部件形状、关节拓扑和参数，通过能量一致验证器生成仿真就绪的数字孪生。


<details>
  <summary>Details</summary>
Motivation: 现有重建方法存在部件几何与运动学解耦以及模型违反能量守恒导致模拟漂移的问题。

Method: 提出包含运动学约束图、可微螺旋轴求解器和能量残差损失的约束驱动流水线。

Result: 在多个基准上，关节轴误差降低37.4%至46.6%，长时模拟漂移减少64%，闭环操作成功率提升14.6个百分点。

Conclusion: KinemaForge有效生成仿真就绪的数字孪生，代码和数据将公开。
...
### [16] [DINO-Med3D: Bridging Dimension and Domain Gaps in Volumetric Segmentation via Progressive Adaptation](https://arxiv.org/abs/2606.18886)
*Haoyu Hu,Xiyao Ma,Shiqi Liu,Linsen Zhang,Xiaoliang Xie,Xiaohu Zhou,Zeng-Guang Hou*

Main category: cs.CV

TL;DR: Adapting DINOv3 to 3D medical image segmentation via a two-stage progressive framework with multi-slice embedding, 3D adapters, and a parallel detail recovery stream.


<details>
  <summary>Details</summary>
Motivation: DINOv3 excels in natural images but fails in volumetric medical segmentation due to dimension (2D vs. 3D) and domain gaps.

Method: A two-stage framework: (1) Multi-slice embedding module with pseudo-3D context and a segmentation proxy task for domain adaptation; (2) Lightweight 3D adapters in the frozen backbone for inter-slice continuity. A parallel detail recovery stream preserves high-frequency boundary cues.

Result: Outperforms state-of-the-art baselines on five public datasets.

Conclusion: The proposed DINO-Med3D successfully adapts DINOv3 to 3D medical segmentation, achieving superior performance.
...
### [17] [Benchmarking Large Vision-Language Models on Fine-Grained Image Tasks: From Evaluation to Diagnosis](https://arxiv.org/abs/2606.19053)
*Hong-Tao Yu,Chen-Wei Xie,Yuxin Peng,Serge Belongie,Xiu-Shen Wei*

Main category: cs.CV

TL;DR: 提出了FG-BMK基准测试，用于细粒度评估LVLMs，发现当前模型在细粒度识别上存在视觉表示、语义基础等多重瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准从整体或任务特定角度评估LVLMs，对其在细粒度图像任务上的能力理解不足。

Method: 构建包含1.01M问题和0.28M图像的FG-BMK，通过人类导向和机器导向范式联合评估语义识别与视觉可区分性，诊断失败来源。

Result: 当前LVLMs在细粒度识别上不足，失败源于视觉表示、语义基础、模态对齐和类别知识的瓶颈；训练因素和扰动影响预测。

Conclusion: 为开发更可靠的LVLMs提供了诊断见解和未来数据构建及模型设计的指导。
...
### [18] [Taming I2V models for Image HOI Editing: A Cognitive Benchmark and Agentic Self-Correcting Framework](https://arxiv.org/abs/2606.19073)
*Jiayi Gao,Qingchao Chen,Yuxin Peng,Yang Liu*

Main category: cs.CV

TL;DR: 提出HOI-Edit基准和自纠正SCPE框架，利用I2V模型实现复杂人-物交互编辑。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑方法无法处理复杂的人-物交互，已有基准混淆HOI与静态属性，缺乏动态交互评估指标。

Method: 构建包含三个认知级别的HOI-Edit基准和自动化指标HOI-Eval；利用I2V模型进行动态编辑，并进一步提出SCPE自纠正框架，通过迭代优化提示约束I2V生成以准确呈现目标HOI。

Result: SCPE在HOI-Edit上达到与Nano Banana等SOTA模型竞争的性能。

Conclusion: SCPE框架通过I2V模型的时序能力和自纠正机制，有效实现了复杂HOI编辑，并提供了错误诊断能力。
...
### [19] [CAOA -- Completion-Assisted Object-CAD Alignment](https://arxiv.org/abs/2606.18429)
*Hiranya Garbha Kumar,Minhas Kamal,Balakrishnan Prabhakaran*

Main category: cs.CV

TL;DR: 提出了一种结合点云补全和对称感知姿态估计的CAD模型对齐方法，在Scan2CAD上提升17%准确率。


<details>
  <summary>Details</summary>
Motivation: 由于室内RGB-D扫描的噪声、不完整和分割误差，CAD模型对齐具有挑战性，现有补全方法无法泛化到真实场景。

Method: CAOA集成了语义和上下文感知的点云补全模块与对称感知相对姿态估计算法，并采用针对室内场景的合成数据生成策略。

Result: 在Scan2CAD基准上，CAOA相比最先进方法实现了17%的准确率提升。

Conclusion: CAOA有效提高了对齐精度，同时发布的S2C-Completion数据集为单对象补全提供了新基准。
...
### [20] [Domain Generalizable Adaptation of 3D Vision-Language Models via Regularized Fine-Tuning](https://arxiv.org/abs/2606.18472)
*Sneha Paul,Zachary Patterson,Nizar Bouguila*

Main category: cs.CV

TL;DR: 提出ReFine3D，一种正则化微调框架，用于3D大语言模型的域泛化调优，通过选择性层调优和多种正则化策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 域适应是3D视觉多模态基础模型的挑战，直接微调会导致过拟合和灾难性遗忘。

Method: 结合选择性层调优与多视角一致性、同义词文本多样性正则化，并加入点渲染视觉监督和测试时增强与置信度聚合。

Result: 在多个3D域泛化基准上，相比现有方法，base-to-novel提升1.36%，跨数据集提升2.43%，鲁棒性提升1.80%，少样本提升最多3.11%，计算开销小。

Conclusion: ReFine3D有效提高了3D大语言模型的域泛化能力，为多模态3D模型的微调提供了新思路。
...
### [21] [Vines-DB: An RGB image dataset for multi-species ornamental vine segmentation](https://arxiv.org/abs/2606.18484)
*Saroj Burlakoti,Utsav Bhandari,Aaron Etienne,Shital Poudyal*

Main category: cs.CV

TL;DR: Vines-DB是一个包含7种藤蔓植物实例分割标注的高分辨率RGB图像数据集，用于精准园艺和城市生态学的深度学习模型开发。


<details>
  <summary>Details</summary>
Motivation: 为多类实例分割在精准园艺和城市生态学中的应用提供基准数据集，支持自动冠层覆盖估计、物种识别和田间表型分析。

Method: 在田间条件下使用iPhone 16 Pro拍摄1218张原始图像，由训练有素的标注人员在Roboflow中手动标注多边形实例分割掩码，经过数据增强后扩展至2307张图像，并按分层采样划分为训练、验证和测试集。

Result: 包含7种藤蔓植物、1218张原始图像（增强后2307张），提供8类实例分割标注，覆盖田间条件和时间变化。

Conclusion: 该数据集能够支持深度学习模型开发，实现自动冠层覆盖估计、物种识别和可扩展的田间表型分析，并可用于现实条件下的分割基准测试。
...
### [22] [Forged Calamity: Benchmark for Cross-Domain Synthetic Disaster Detection in the Age of Diffusion](https://arxiv.org/abs/2606.18554)
...
TL;DR: 提出Forged Calamity基准数据集，评估合成灾难图像检测，发现现有方法普遍存在泛化能力差的问题，亟需领域与模型无关的检测方法。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型生成的照片级合成图像难以与真实照片区分，对网络安全、数字取证和灾害响应构成挑战，可能传播虚假信息或干扰应急操作。

Method: 构建包含30,000张图像（6,000张真实、24,000张由四种扩散模型生成的合成样本）的Forged Calamity数据集，并在微调和零样本设置下进行综合实验。

Result: 微调检测器在分布内表现良好，但对未见过的生成器或灾难类型准确率下降高达50%，呈现过拟合；零样本检测器难以保持稳定准确率，仅少数表示鲁棒模型有有限韧性。

Conclusion: 现有检测方法存在持续的泛化缺陷，迫切需要领域与模型无关的检测方法以确保视觉真实性。
...
### [23] [Experimental Analysis of Neural Network-Based Image Classification on the CIFAR-10 Dataset](https://arxiv.org/abs/2606.18565)
*Necati Kagan Erkek,Emre Balci,Berkin Halay*

Main category: cs.CV

TL;DR: 在CIFAR-10上实验比较了全连接和卷积神经网络，发现卷积架构虽达到74.77%验证准确率，但验证损失在中后期上升，体现了表示学习与记忆化的差异。


<details>
  <summary>Details</summary>
Motivation: 通过完整学习流程的实验分析，揭示神经网络中表示学习与记忆化的实际差异，并为后续研究提供可重复的基线。

Method: 采用6层卷积+3层最大池化的卷积网络，使用Adam优化器（lr=0.001）、批量大小128，训练10个周期，并进行图像向量化、归一化、one-hot编码等预处理。

Result: 验证准确率约74.77%，验证损失在训练中期后开始上升，而训练损失持续下降。

Conclusion: 该行为表明网络从表示学习转向记忆化，实验为未来关于正则化、数据增强、更深架构等研究提供了紧凑的基线。
...
### [24] [Multi-Modal Hyper-Graph Fusion for Low-Light Crowd Counting](https://arxiv.org/abs/2606.18566)
*Hao-Yuan Ma,Li Zhang,Yushi Qiu,Jie Gao,Yan Zhang,Bangjun Wang*

Main category: cs.CV

TL;DR: 提出基于多模态超图融合和可变形稀疏注意力的低光群体计数网络LCNet，在三个新基准上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 低光环境下的群体计数未被充分探索，现有方法依赖RGB在黑暗中不可靠。

Method: 构建三个低光群体计数基准；受Retinex启发引入深度和Canny边缘先验；提出多模态超图融合模块捕获高阶互补关系；提出可变形矩形稀疏注意力自适应分配计算；最终形成LCNet。

Result: 在三个基准上均超越现有SOTA方法。

Conclusion: LCNet在低光下鲁棒且有效，数据集将公开。
...
### [25] [Multi-Class Brain Tumor Classification Using Advanced Deep Learning Models: A Comparative Study](https://arxiv.org/abs/2606.18682)
*Asad Channa,Asghar Ali Chandio,Akhtar Hussain Jalbani,Mehwish Leghari,Shahzad Memon*

Main category: cs.CV

TL;DR: EfficientNetB0在脑肿瘤MRI分类中达到最佳准确率95%，特别是在脑膜瘤检测中表现出色。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习取得了进展，但准确分类脑肿瘤MRI仍然具有挑战性。

Method: 评估了五种CNN架构（定制基线、VGG16、VGG19、DenseNet121、EfficientNetB0）在约10,000张MRI图像上的性能，使用准确率和召回率作为指标。

Result: EfficientNetB0总体准确率95%，脑膜瘤召回率89%，显著优于其他模型；VGG19比VGG16差。

Conclusion: EfficientNetB0提供了分类准确率、参数效率和临床性能的最佳平衡。
...
### [26] [Spatially Stratified Distillation for Heterogeneous Radar Place Recognition](https://arxiv.org/abs/2606.18687)
*Sagun Singh Shrestha,Samuel Harding,Abdelwahed Khamis,Saimunur Rahman,Peyman Moghadam*

Main category: cs.CV

TL;DR: 提出空间分层蒸馏（SSD）方法，通过非对称空间对齐解决4D雷达与旋转雷达之间的稀疏性不匹配问题，在雷达地点识别中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 4D雷达极度稀疏且视场窄，与密集旋转雷达构建的参考地图存在显著结构密度差异，现有统一表示方法在多会话环境中性能下降。

Method: 采用空间分层蒸馏（SSD），基于物理雷达返回导出非对称空间对齐，在重叠区域强特征对齐，在仅教师有返回的稀疏区域大幅折扣蒸馏权重。

Result: 在HeRCULES数据集上显著优于先前方法，在动态序列上取得最先进结果。

Conclusion: SSD有效解决了跨雷达模态的表示差异，提升了异构雷达地点识别性能。
...
### [27] [SAMA: Semantic Anchor-aligned Augmentation for Unified Low-Resource Multimodal Information Extraction](https://arxiv.org/abs/2606.18780)
*Quanjiang Guo,Chong Mu,Jiazhou Pan,Ming Jia,Ling Tian,Hui Gao,Zhao Kang*

Main category: cs.CV

TL;DR: 提出语义锚定对齐的多模态数据增强框架SAMA，用于缓解多模态信息抽取中的数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 多模态信息抽取面临严重的数据稀缺问题，现有数据增强方法存在粗粒度的跨模态对齐和碎片化任务特定设计，无法充分利用共享语义知识。

Method: SAMA框架通过结构化语义锚定指导协作多专家多模态大语言模型生成文本样本，采用锚定保持扩散机制生成图像，并利用双约束过滤模块确保样本质量。

Result: 在MNER、MRE和MEE基准数据集上，SAMA在全监督和低资源设置下均一致优于最先进的增强基线。

Conclusion: SAMA展示了其通用性、鲁棒性和有效性，为多模态信息抽取提供了一种统一的数据增强解决方案。
...
### [28] [Urdu Katib Handwritten Dataset: A Historical Document Dataset for Offline Urdu Handwritten Text Recognition with CRNN-Based Baseline Evaluation](https://arxiv.org/abs/2606.19139)
*Ramza Basharat,Muhammad Usman Ali*

Main category: cs.CV

TL;DR: 本文提出了离线乌尔都语手写文本行数据集UKHD，评估了CRNN混合模型，其中CNN-BGRU-CTC表现最佳，具有低CER和WER。


<details>
  <summary>Details</summary>
Motivation: 乌尔都语手写文本识别研究有限，主要由于脚本独特挑战和缺乏基准数据集。

Method: 创建UKHD数据集，包含历史卡提布鲁写纳斯塔利克风格文本行，并评估不同CRNN混合模型。

Result: CNN-BGRU-CTC模型表现出稳健性能，字符错误率和词错误率较低。

Conclusion: 旨在支持研究社区开发鲁棒识别系统，以保护乌尔都语手写文献。
...
### [29] [FlowObject: Flow Steering for Bridging Generative Priors and Reconstruction Fidelity](https://arxiv.org/abs/2606.19019)
*Yuchen Rao,Xuqian Ren,Yinyu Nie,Sayan Deb Sarkar,Biao Zhang,Vincent Lepetit,Friedrich Fraundorfer*

Main category: cs.CV

TL;DR: FlowObject combines flow-matching generative priors with 3D Gaussian Splatting refinement via a dual-space guidance strategy for training-free sparse-view 3D reconstruction.


<details>
  <summary>Details</summary>
Motivation: Generative models suffer from synthetic bias and lack instance alignment, while optimization-based methods fail on unobserved geometry.

Method: Reformulates sparse-view reconstruction as a training-free guided inverse problem, using dual-space guidance to steer the ODE trajectory of a flow-matching model, followed by 3DGS refinement.

Result: Significantly outperforms state-of-the-art generative and optimization methods in geometric completeness and view-dependent appearance fidelity.

Conclusion: FlowObject effectively bridges generative priors and observational consistency for high-quality 3D reconstruction from sparse views.
...
### [30] [When AUC Misleads: Polarization-Aware Evaluation of Deepfake Detectors under Domain Shift](https://arxiv.org/abs/2606.19184)
*Dat Nguyen,Cosmin Radoi,Romain Hermary,Marcella Astrid,Nesryne Mejri,Enjie Ghorbel,Djamila Aouada*

Main category: cs.CV

TL;DR: 提出Cross-AUC指标以更真实评估深度伪造检测器在域偏移下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法（AUC分别在不同数据集上测量）不能反映真实场景中混合数据源和变化伪影类型的情况，需新指标纠正。

Method: 引入Cross-dataset AUC (Cross-AUC)，平均每个域的AUC并加上预测极化度量（通过Wasserstein距离量化类分数分布差异）来考虑域偏移鲁棒性。

Result: 在七个基准数据集上实验证明了Cross-AUC的实用相关性。

Conclusion: Cross-AUC更真实评估泛化能力且可解释性更好，能解释性能下降原因。
...
### [31] [A Multi-Domain Benchmark for Detecting AI-Generated Text-Rich Images from GPT-Image-2](https://arxiv.org/abs/2606.19259)
*Yijin Wang,Shuyi Wang,Wenhan Zhang,Yuqi Ouyang*

Main category: cs.CV

TL;DR: 提出了一个包含8602张文本丰富图像的多领域基准，用于检测AI生成的图像，评估发现现有检测器性能高度依赖领域且对JPEG压缩敏感，需要文本和布局感知的检测方法。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测基准主要关注对象中心图像，对文本丰富图像覆盖不足，而这类图像常包含隐私或决策相关信息，亟需专门基准。

Method: 构建包含商业海报、信息图表、学术海报、收据、表格和UI截图六类8602张图像的基准，在零样本设置下评估五个代表性检测器，并探索多模态视觉语言模型。

Result: 检测器性能领域依赖性强，最强传统检测器对JPEG压缩敏感；多模态模型有潜力但在结构化格式上存在局限。

Conclusion: 现有方法缺乏文本和布局感知能力，需开发专门针对文本丰富图像的检测方法。
...
TL;DR: Geometry-Aware Pointer (GAP) Loss reweights cross-entropy by spatial proximity to reduce adjacent-cell errors in table structure recognition, achieving state-of-the-art on PubTabNet and SynthTabNet.


<details>
  <summary>Details</summary>
Motivation: Standard cross-entropy loss weights all negative candidates equally, but 79.6% of pointer network errors occur between spatially adjacent cells (Manhattan distance ≤ 2), necessitating geometry-aware weighting.

Method: GAP Loss applies inverse distance weighting to the cross-entropy objective based on spatial distance from ground truth, focusing gradient flow on immediate neighbors with zero additional inference cost.

Result: Extensive experiments on PubTabNet and SynthTabNet show consistent reduction in adjacent-cell errors and new state-of-the-art performance.

Conclusion: Incorporating geometric inductive biases at the loss level provides a simple yet effective approach to robust table structure recognition.
...
### [33] [Toward Training-Free Zero-Shot Anomaly Detection in 3D Medical Images: A Batch-Based Approach Using 2D Foundation Models](https://arxiv.org/abs/2606.18749)
*Tai Le-Gia*

Main category: cs.CV

TL;DR: 提出CS3F，一种利用2D基础模型的训练无关框架，用于3D医学图像的零样本异常检测。


<details>
  <summary>Details</summary>
Motivation: 零样本异常检测对医学成像很重要，但现有2D方法扩展至3D受限于缺乏大规模3D基础模型和难以利用体积上下文。

Method: 将3D体积沿多解剖轴分解，逐切片用2D ViT编码，池化为局部体积令牌，通过跨主题相似度计算异常分数，并引入粗到细令牌化策略避免深度池化导致的信号衰减。

Result: 在脑MRI（转移瘤、胶质瘤、中风）和肺CT上评估，显示冻结的2D基础模型可支持3D异常定位，且精细令牌化的效果取决于病灶对比度和成像模态。

Conclusion: 冻结的2D基础模型可用于3D医学图像异常检测，但精细令牌化的获益因条件而异。
...
### [34] [SCR-Guided Difficulty-Aware Optimization for Infrared Small Target Detection](https://arxiv.org/abs/2606.18783)
*Yunus Sevim,Behçet Uğur Töreyin*

Main category: cs.CV

TL;DR: 提出REEM，一种SCR引导的困难感知优化框架，通过调制软IoU损失来提升红外小目标检测性能，无需额外参数或推理开销。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测因背景杂波、低对比度、弱空间响应而困难，几何重叠不足以表征检测质量。

Method: REEM从输入图像计算局部SCR作为可见性先验，对软IoU学习信号进行可微调制，强调低可见性目标，集成到U-Net中无需修改架构或增加参数。

Result: 在MSHNet上集成后，取得更高IoU和检测概率，大幅降低虚警，尤其在低可见性条件下表现优异。

Conclusion: SCR引导的困难感知优化是一种有效且物理基础的补充，适用于红外小目标检测。
...
### [35] [From Bounding Boxes to Visual Reasoning: An On-Policy Data Annotation Tool for Vision-Language Models](https://arxiv.org/abs/2606.18846)
*Like Zhang,Runliang Niu,Shiqi Wang,Xiyu Hu,Qianli Xing,Pan Wang,Qingzu He,Qi Wang*

Main category: cs.CV

TL;DR: ScreenAnnotator is an open-source annotation tool that unifies spatial, semantic, and structural primitives with an on-policy loop and template-driven synthesis to enable efficient annotation for vision-language models.


<details>
  <summary>Details</summary>
Motivation: Existing annotation tools fail to meet the demands of advanced grounded structured visual reasoning due to limited expressiveness, severe annotation-training decoupling, and poor data reusability.

Method: We define a unified annotation atom schema binding spatial, semantic, and structural primitives; implement an on-policy annotation loop with a Bayesian Annotation Verifier; and design a template-driven multi-task data synthesis process.

Result: The on-policy loop achieves nearly 100% accept rate on flowcharts and 77% on GUI screenshots, reducing annotation time. Fine-tuning a VLM on flowchart data yields 76.1% average accuracy, a 35.1% absolute gain.

Conclusion: ScreenAnnotator effectively bridges the infrastructure gap for generating high-quality data for structured visual reasoning tasks.
...
### [36] [Test-Time Adaptation in Optical Coherence Tomography Using Trajectory-Aligned Time-Independent Flow](https://arxiv.org/abs/2606.18876)
*Veit Hucke,Thomas Pinetz,Gregor Reiter,Ursula Schmidt-Erfurth,Hrvoje Bogunović*

Main category: cs.CV

TL;DR: 提出基于流匹配的测试时自适应方法，通过直方图匹配和去除时间条件，将低质量OCT图像转换为高质量图像，在AMD分割任务上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: OCT在眼科中至关重要，但低成本设备导致的图像质量不一致阻碍了自动化分析。

Method: 采用流匹配的测试时自适应方法，通过直方图匹配将测试图像分布对齐到合成参考轨迹，并移除网络时间条件以处理真实噪声分布偏差。

Result: 在AMD两个阶段的关键生物标志物分割上达到最先进性能。

Conclusion: 该方法有效克服域间隙，提升低质量OCT图像的分析能力。
...
### [37] [Budget-Aware Adaptive Adversarial Patches for Black-Box Object Detection](https://arxiv.org/abs/2606.18318)
*Pedram MohajerAnsari,Amir Salarpour,David Fernandez,Mert D. Pesé*

Main category: cs.CV

TL;DR: 提出了一种查询高效、预算自适应的黑盒对抗补丁攻击，联合优化位置、纹理和大小，通过严格的纯图像抑制评估揭示了视觉足迹与攻击成功的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒补丁攻击很少联合优化位置、纹理和大小，成功与视觉足迹关联不足，且评估混淆了EOT鲁棒性与纯视图抑制。

Method: 结合轻量级Contextual Thompson-Sampling放置器和NES风格像素更新，仅在进展停滞时增大补丁，并采用严格的纯图像抑制测试和可选的外观/可打印性权重。

Result: 在YOLOv5、Faster R-CNN和YOLOS上，对CNN检测器实现强抑制，对Transformer检测器实现显著抑制，使用紧凑补丁并暴露清晰的查询-足迹权衡，打印-捕获实验显示跨物体和视点迁移。

Conclusion: 该方法有效，揭示了视觉足迹与攻击成功的权衡，并展示了物理世界迁移的潜力。
...
### [38] [Reasoning as Intersection: Consensus-Frame Alignment for Visual Focus in Video-MLLMs](https://arxiv.org/abs/2606.18441)
*Chengwen Liu,Zhe Huang,Jisheng Dang,Hong Peng,Qi Tian,Tat-Seng Chua*

Main category: cs.CV

TL;DR: CF-GRPO是一种无需时间注释的进程级奖励框架，通过共识帧先验和帧使用分数的一致性优化，提高了视频推理的奖励信号质量和可解释性。


<details>
  <summary>Details</summary>
Motivation: 应用结果奖励到视频多模态大语言模型时，无法提供足够的指导来指示哪些视觉证据应支持答案。

Method: 提出CF-GRPO框架，构建共识帧先验（包括时间覆盖、场景转换线索和查询条件视觉相关性），计算模型端帧使用分数，并通过共识帧奖励优化它们的一致性，使用显著性感知的稀疏聚合和分布锐化。

Result: 在复杂视频推理基准上取得了有竞争力的性能，多个指标优于代表性Video-MLLM和RL基线。

Conclusion: 共识先验提供了训练期间强调的证据帧的可解释视图，并且CF-GRPO框架有效。
...
### [39] [Neural Phase Correlation](https://arxiv.org/abs/2606.18496)
*Cole Reynolds*

Main category: cs.CV

TL;DR: 提出一种学习相位相关的一般化方法，通过学习基直接表示图像间变换，适用于刚性和非刚性配准，在医学图像和量子物理中取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法未将变换作为一等对象，而是隐含学习；相位相关直接但限于全局平移；希望获得可学习的直接表示变换的方法。

Method: 学习一个可分解变换的基，使用代数原语扩展到非刚体变形和酉动力学，可能通过神经网络实现。

Result: 在ACDC和CAMUS数据集上匹配或超过基线，在量子谐振子中恢复本征态和能级。

Conclusion: 该框架是相位相关的一般化，有效处理多种变换，具有广泛适用性。
...
### [40] [Architectural Bias in Face Presentation Attack Detection: A Comparative Study of Vision Transformers and Convolutional Neural Networks](https://arxiv.org/abs/2606.18510)
...
### [41] [Rethinking Text-to-Image as Semantic-Aware Data Augmentation for Indoor Scene Recognition](https://arxiv.org/abs/2606.18555)
*Trong-Vu Hoang,Quang-Binh Nguyen,Dinh-Khoi Vo,Hoai-Danh Vo,Minh-Triet Tran,Trung-Nghia Le*

Main category: cs.CV

TL;DR: 利用Stable Diffusion生成合成室内图像进行数据增强，并采用DIRE防御方法实现100%检测合成图像。


<details>
  <summary>Details</summary>
Motivation: 室内图像识别因光照、遮挡和物体布局复杂而困难，且缺乏训练图像。

Method: 使用Stable Diffusion生成多样化逼真的室内场景图像作为数据增强，并利用DIRE（扩散重建误差）作为防御机制来识别合成图像。

Result: 在MIT Indoor Scene数据集上，该方法增强了深度模型的训练；DIRE方法使用MobilenetV3实现了100%的准确率识别Stable Diffusion生成的图像。

Conclusion: 该方法能有效利用合成图像提升室内识别性能，并能完美检测合成图像，防止滥用。
...
### [42] [MolmoMotion: Forecasting Point Trajectories in 3D with Language Instruction](https://arxiv.org/abs/2606.18558)
...
TL;DR: 提出目标条件3D点运动预测任务，构建大规模数据集和基准，训练模型优于基线，并展示其在下游任务中的迁移能力。


<details>
  <summary>Details</summary>
Motivation: 运动预测是视觉智能的核心，现有方法缺乏通用表示；3D点表示具有类无关、视图稳定、紧凑且直接有用的特性，因此形式化目标条件3D点运动预测任务。

Method: 构建MolmoMotion-1M大规模数据集（1.16M视频），创建PointMotionBench基准（111类物体、61种运动），训练MolmoMotion模型（支持自回归坐标预测和流匹配轨迹生成）。

Result: MolmoMotion准确预测多样运动模式，显著优于现有基线，且学习到的3D运动先验提升机器人操作训练效率与泛化性，并为视频生成提供有效运动引导。

Conclusion: 3D点运动预测是有效的通用运动表示，其先验可迁移至下游任务，推动视觉智能发展。
...
### [43] [Bridging Creative Intent and Visual Quality: Creator-Driven Recurrent Video Generation with Agentic Feedback Loops](https://arxiv.org/abs/2606.18591)
*Denis Savytski,Aiden Lei,Heding Liu,Warren Yang,Sihan Liang,Alexander Liu,Zhe Zhao*

Main category: cs.CV

TL;DR: 提出CHIEF框架，通过人类-AI协同迭代和自动主观反馈，使无经验用户也能创作连贯叙事的长视频。


<details>
  <summary>Details</summary>
Motivation: AI生成视频缺乏叙事连贯性和创意方向，且视频生成需要主观反馈，现有方法无法满足长视频需求。

Method: 基于人类在环迭代的视频生成框架，包含人类创意驱动、精细化智能体、以及基于角色条件多模态LLM的自动观众视角批评。

Result: 与高中生和大学生合作，成功制作了从1分钟短片到10分钟复杂剧情片。

Conclusion: CHIEF框架使无电影制作经验的用户也能创作叙事连贯的长视频，展示了人类-AI协同在视频创作中的潜力。
...
### [44] [Intrinsic 4D Gaussian Segmentation from Scene Cues](https://arxiv.org/abs/2606.18623)
*Hasan Yazar,Mohamed Rayan Barhdadi,Erchin Serpedin,Mehmet Tuncel,Hasan Kurban*

Main category: cs.CV

TL;DR: Intrinsic-GS是一种无需训练和掩码的4D高斯分割方法，通过构建基元亲和图并使用社区检测实现快速高质量分割。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部2D掩码，成本高且受限于掩码质量，本文探索从高斯本身恢复对象结构。

Method: 从高斯基元的外观、方向、尺度、变形轨迹和渲染边界线索构建稀疏亲和图，然后使用Leiden社区检测进行分割。

Result: 在Neu3D上达到0.746 mIoU，几何变体达0.902；在HyperNeRF上快12.5倍。

Conclusion: 分割信号已编码在高斯中，该方法提供快速无掩码方向，可能更泛化鲁棒。
...
### [45] [Spiking Pyramid Wavelet Transformation for High-efficient and Low-energy Image Restoration](https://arxiv.org/abs/2606.18644)
*Chen Zhao,Xiantao Hu,Song Wu,Qian Wang,Chen Wu,Rui Xie,Jian Yang,Ying Tai*

Main category: cs.CV

TL;DR: 提出基于离散小波变换的尖峰金字塔小波模型（SPWM），在保持图像质量的同时显著降低计算成本和能耗。


<details>
  <summary>Details</summary>
Motivation: 尖峰CNN在图像恢复中受限于感受野，需要更高效的方法。

Method: 构建尖峰双金字塔小波（SDPW）模块，建模长程依赖并利用小波域退化特性。

Result: 多个基准测试表明，SPWM显著降低计算成本和能耗，同时维持图像质量。

Conclusion: 展示了SNN在图像恢复中的潜力，为资源受限设备提供新方向。
...
### [46] [On-Manifold Variational Learning with Heat-Kernel Priors](https://arxiv.org/abs/2606.18658)
*Jiarui Xing,Tal Zeevi,Nian Wu,Jian Wang*

Main category: cs.CV

TL;DR: 提出流形锚定变分框架，通过几何感知EM算法学习无监督医学影像表示，确保原型在流形上，避免退化。


<details>
  <summary>Details</summary>
Motivation: 现有深度潜变量模型用欧几里得平均估计高斯混合先验，导致原型偏离数据流形，子群增多时退化。

Method: 基于几何感知EM算法的流形锚定变分框架，M步用热核加权潜图上扩散中心性最高的图中心点选择原型，加上Dirichlet能量正则化和子群不确定性评分。

Result: 在心脏瘢痕和脑MRI基准上达到最高准确度，生成最清晰原型，大子群数时保持稳定而基线退化。

Conclusion: 流形锚定EM是通用几何工具，可扩展标准EM并应用于其他潜变量模型。
...
### [47] [LandslideAgent with Multimodal LandslideBench: A Domain-Rule-Augmented Agent for Autonomous Landslide Identification and Analysis](https://arxiv.org/abs/2606.18661)
*Chengfu Liu,Dongyang Hou,Junwu Xiang,Cheng Yang,Xuezhi Cui,Zeyuan Wang,Liangtian Liu,Zelang Miao*

Main category: cs.CV

TL;DR: 提出了一种指令驱动的智能体框架，通过构建专用数据集LandslideBench、微调模型LandslideVLM和领域规则增强智能体LandslideAgent，显著提升了滑坡灾害解译的准确性和自动化水平。


<details>
  <summary>Details</summary>
Motivation: 智能滑坡灾害解译对于防灾减灾至关重要，然而现有范式难以同时提取视觉特征和高层次地学语义，通用视觉语言模型在复杂地质场景中存在感知局限和领域幻觉。

Method: 为此，提出了一种指令驱动的智能体框架，包含三个组件：首先，通过多VLM交叉验证和交互式标注构建了多模态细粒度数据集LandslideBench；然后，在LandslideBench上通过LoRA微调得到了面向滑坡的视觉语言模型LandslideVLM；最后，设计了一个领域规则增强的智能体LandslideAgent，以LandslideVLM为认知骨干，采用双重规则控制器调控自动化工具调用。

Result: 实验表明，LandslideBench在五种主流模型上为细粒度分类和语义分割提供了有效基准。LandslideVLM在滑坡判别、细粒度分类和语义描述质量上分别提升了10.96%、32.87%和15.91%。LandslideAgent进一步实现了自主多源空间数据推理。

Conclusion: 该框架实现了滑坡识别与分析的全流程智能化。
...
### [48] [BrainFusionNet: a deep learning and XAI model to understand local, global, and sequential features of MRI images for improved brain tumour detection](https://arxiv.org/abs/2606.18675)
*Md Taimur Ahad,Bo Song,Yan Li*

Main category: cs.CV

TL;DR: BrainFusionNet combines CNN, ViT, and GRU with explainable AI to achieve 98% accuracy in brain tumor classification, outperforming state-of-the-art CNNs.


<details>
  <summary>Details</summary>
Motivation: MRI noise obscures tumor boundaries and complicates tumor location and appearance, posing challenges for deep learning in brain tumor classification.

Method: The model integrates Convolutional Neural Networks (CNNs), Vision Transformers (ViT), and Gated Recurrent Units (GRUs) to extract spatial, contextual, and sequential features. Explainable AI techniques (SHAP, LIME, GradCAM) are used for visualization. Pixel intensity analysis is also performed.

Result: The model achieves 98% accuracy on two public MRI datasets via K-fold validation, outperforming six state-of-the-art CNNs (best at 96%). Pixel intensity distribution affects classification performance.

Conclusion: BrainFusionNet effectively extracts local and global features even for small tumors, with a balanced sequential CNN and customized ViT that stabilizes gradient flow. The hybrid architecture and explainability are novel contributions.
...
### [49] [Moving Beyond Diversity: Visual Token Pruning as Subspace Reconstruction for Efficient VLMs](https://arxiv.org/abs/2606.18681)
*Jaeyeon Lee,Shunjie Wen,Dong-Wan Choi*

Main category: cs.CV

TL;DR: 提出SPARE，一种基于子空间重建的视觉token剪枝方法，通过最小化重建误差迭代选择token，并利用反相关现象提升上下文感知，在去除94%视觉token时仍保留95%性能。


<details>
  <summary>Details</summary>
Motivation: 现有token剪枝方法依赖余弦归一化相似性，丢失幅度信息，无法忠实近似原始特征表示，导致在组合多技能推理任务上性能次优。

Method: 将token剪枝建模为列子集选择问题，通过迭代选择具有大投影残差的token进行重建驱动剪枝，并基于反相关现象将低图像-文本相关性的token作为额外选择标准以增强上下文感知。

Result: 在多个VLM和基准上取得一致最优性能，尤其在组合任务上表现突出；在LLaVA上可去除94%视觉token同时保持95%基线性能，且完全无需训练。

Conclusion: SPARE通过重建驱动和反相关机制实现了高效且有效的视觉token剪枝，验证了幅度信息与低相关性token在保留上下文中的重要性。
...
### [50] [Clinically Aligned Geometry Constraints for Robust IVUS Vessel Boundary Segmentation](https://arxiv.org/abs/2606.18723)
...
TL;DR: 提出GeoCat，一种几何一致的网络，用于IVUS分割，显著提高了几何保真度和临床测量准确性。


<details>
  <summary>Details</summary>
Motivation: 标准方法优先考虑重叠分数但遭受边界漂移和拓扑错误，导致临床测量不准确。

Method: 使用双笛卡尔-极坐标编码器、跨域注意力和时间融合处理5帧IVUS剪辑，并采用可微几何一致性损失直接监督临床相关描述符。

Result: Dice 0.93，95HD 0.14 mm，拓扑违规率1.0%，直径误差0.13-0.16 mm，角度误差约8度。

Conclusion: 支持可靠的斑块负担量化，提高了临床几何测量的准确性。
...
### [51] [HandwritingAgent: Language-Driven Handwriting Synthesis in Scalable Vector Space](https://arxiv.org/abs/2606.18788)
*Jaward Sesay,Yue Yu,Börje F. Karlsson*

Main category: cs.CV

TL;DR: HandwritingAgent是一个语言驱动的手写合成代理，无需风格特定训练即可直接生成SVG格式的自然手写序列，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法受限于风格特定的架构选择、对大数据集的高度依赖、高计算成本以及缺乏通过自然语言灵活控制书写风格的能力。

Method: HandwritingAgent利用大推理模型，在离散网格画布环境中几何分析和自回归生成笔划序列，以SVG格式输出，生成过程以文本（对话或非对话模式）和参考手写风格图像为条件。

Result: 在模仿、识别、多语言手写合成以及复杂数学和科学表达式生成等任务上，HandwritingAgent匹配或超越了最先进的生成手写模型，性能显著提升。

Conclusion: HandwritingAgent提供了一种更高效、可控且泛化能力更强的合成方法。
...
### [52] [SMART: A Flexible, Interpretable, and Scalable Spatio-temporal Brain Atlas from High-Resolution Imaging Data](https://arxiv.org/abs/2606.18753)
*John Kalkhof,Boris Gutman,Emile d'Angremont,Daniel C. Alexander,Marco Lorenzi*

Main category: cs.CV

TL;DR: SMART是一个用于学习时空脑图谱的框架，通过解耦全局疾病动态与患者特异性解剖，实现了可解释和准确的疾病进展预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用黑盒生成模型，缺乏灵活性、可解释性，且难以扩展到高维数据。

Method: 学习连续疾病时间图谱，解耦全局疾病动态与患者特异性解剖；使用区域特异性微分方程建模可解释的全局轨迹；通过多尺度神经细胞自动机参数化的密集微分同胚位移进行个性化。

Result: 在五个纵向MRI数据集（>1300名受试者）上，SMART产生了解剖学上有意义的预测，实现了最先进的预测准确性和改进的时间一致性，优于对抗和扩散基线。

Conclusion: 建立了一种新的范式，用于灵活、可解释和可扩展的时空变化建模在高维医学图像时间序列中。
...
### [53] [Fuzzy-Geometric Branch-Point Modeling for Structure-Aware Augmentation of Handwritten Chinese Characters](https://arxiv.org/abs/2606.18793)
*Dongbin Jiao,Yibo Lyu,Qiulu Wei,Fuxiang Lu,Shengcai Liu,Shi Yan*

Main category: cs.CV

TL;DR: 提出模糊几何驱动的结构感知增强框架（FGSA），通过模糊集建模分支点并优化成员场，结合参数化贝塞尔重建和多策略扰动实现鲁棒手写增强，显著降低词错误率。


<details>
  <summary>Details</summary>
Motivation: 数据稀缺和结构失真限制手写识别，现有增强方法导致拓扑形态损伤，尤其复杂汉字中分支点检测不可靠。

Method: 将分支点建模为骨架空间中的模糊集，融合拓扑邻域证据与方向场散度构建连续成员场，通过无监督代理目标优化实现笔画解耦；再通过参数化三次贝塞尔重建和多策略扰动合成运动学对齐样本。

Result: 在CASIA-HWDB1.1、ChiSig和LZUSig数据集上，FGSA显著降低词错误率（ΔWER），识别增益优于对比基线，且保持结构保真度和判别特征。

Conclusion: FGSA是一种高度可控的手写增强方法，能有效处理结构退化，提升识别性能，并实现任务增益与保真度的平衡。
...
### [54] [Where Will They Go? Modelling Multimodal Pedestrian Manoeuvres from Ego-centric Videos](https://arxiv.org/abs/2606.18824)
*Yuxuan Xie,Nicolas Pugeault,Chongfeng Wei,Hubert P. H. Shum,Edmond S. L. Ho*

Main category: cs.CV

TL;DR: 提出MMPM框架，通过分别建模跨路和非跨路两种模式的行人轨迹分布，实现更准确的轨迹预测。


<details>
  <summary>Details</summary>
Motivation: 现有随机预测器从单峰分布采样多未来，导致混合模式轨迹不真实，需多模态预测。

Method: 提出MMPM，包含行为感知的PIM模块（捕捉行人-车辆和行人-环境交互通过注视、头部和手势）和基于CVAE的MTP模块（分别建模跨路和非跨路模式），使用查询解码器保持模式一致性。

Result: 在PIE和JAAD数据集上超越SOTA，MTP可集成到BiTrap-NP和SGNet-ED等框架中进一步提高性能，新验证协议改进帧位移误差。

Conclusion: 提出的方法有效处理多模态轨迹预测，模式感知框架提升真实性和准确性。
...
### [55] [Native Active Perception as Reasoning for Omni-Modal Understanding](https://arxiv.org/abs/2606.19341)
...
TL;DR: 提出OmniAgent，一种基于POMDP迭代Observation-Thought-Action循环的原生全模态代理，通过主动感知和选择性记忆实现长视频高效理解，并在多个基准上达到开源最优。


<details>
  <summary>Details</summary>
Motivation: 现有长视频被动模型计算成本随视频长度线性增长，而交互式框架依赖全局预扫描，上下文成本仍与视频长度相关。需要一种能解耦推理复杂性与视频时长的主动感知方法。

Method: 将视频理解建模为POMDP，通过Agentic Supervised Fine-Tuning和Agentic Reinforcement Learning with TAURA训练代理，使其能根据查询按需执行动作，选择性地将视听线索提取到文本记忆中。

Result: 在VideoMME、LVBench等10个基准上达到开源模型最优性能，特别是LVBench上7B模型以50.5%超越72B模型的47.3%，并表现出正向测试时缩放。

Conclusion: 验证了主动感知在长视频理解中的有效性，实现了推理复杂度与视频时长的解耦，且性能随推理轮次增加而提升。
...
### [56] [DreamReg: Belief-Driven World Model for 2D-3D Ultrasound Registration](https://arxiv.org/abs/2606.18825)
*Luoyao Kang,Yuelin Zhang,Jiwei Shan,Haifan Gong,Qingpeng Ding,Shing Shin Cheng*

Main category: cs.CV

TL;DR: 提出DreamReg，一个信念驱动世界模型框架，用于2D-3D超声配准，通过信念更新和内部想象提高鲁棒性和精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法一次性或短视，难以随时间收集证据或捕捉医生根据反馈调整探头的行为。

Method: DreamReg维护信念状态，通过学习的动态更新，并在推理时通过内部想象模拟候选探头运动来细化配准。

Result: 在CAMUS和u-RegPro数据集上，鲁棒性和配准精度优于现有方法。

Conclusion: DreamReg为实时超声导航提供了改进的鲁棒性和竞争性的配准精度。
...
### [57] [Learning to Distort: Weakly-Supervised Image Quality Transfer for Prostate DWI Correction](https://arxiv.org/abs/2606.18869)
...
TL;DR: 提出弱监督图像质量迁移框架，通过原型流匹配生成真实畸变，训练单次EPI前列腺DWI畸变矫正模型。


<details>
  <summary>Details</summary>
Motivation: 单次EPI前列腺DWI常有几何畸变，缺乏配对的畸变-无畸变临床图像，传统矫正方法受限。

Method: 使用图像级质量标签（畸变vs无畸变）建立潜在质量原型，设计弱监督原型流匹配算法生成真实畸变，然后训练前向IQT模型进行畸变矫正。

Result: 生成的图像成功模拟临床诊断干扰，矫正模型在PI-RADS和Gleason评分分类下游任务中性能优于CycleGAN等非配对方法。

Conclusion: 弱监督图像质量转移可用于合成真实畸变，有效训练畸变矫正模型，提高临床诊断可靠性。
...
### [58] [Bridging Single Distortion Artifacts and Mmultifactorial Clinical Quality: Few-shot Biparametric MRI Quality Assessment via Distortion-trained Prototypical Networks](https://arxiv.org/abs/2606.18872)
...
TL;DR: 提出一种少样本双原型网络，利用双分支3D ResNet和特征调制技术，从少量样本中自动评估前列腺MRI质量，有效处理类别不平衡并显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 临床前列腺MRI质量评估依赖主观且耗时的PI-QUAL评分，且低质量案例稀缺多样，需要数据高效的自动化方法。

Method: 构建双分支3D ResNet融合T2和DWI特征，引入特征线性调制（FiLM）和梯度反转层（GRL）对齐b值分布、抑制采集偏差，基于失真标签元训练后适应PI-QUAL评分。

Result: 在两个数据集上，该方法在少样本IQA任务中显著优于基准，仅需5个代表样本即可有效预测PI-QUAL分数。

Conclusion: 该方法为临床前列腺MRI质量控制提供了一种可行且数据高效的标准化方案。
...
### [59] [Performance Gap Analysis between Latin and Arabic Scripts HTR](https://arxiv.org/abs/2606.18884)
*Sana Al-azzawi,Elisa Barney,Marcus Liwicki*

Main category: cs.CV

TL;DR: 该研究使用统一CRNN模型比较阿拉伯语和拉丁语手写文本识别，发现性能差距持续存在，注释质量和高视觉变异性是关键因素。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示阿拉伯语HTR性能较差，但原因不明，缺乏受控比较。

Method: 使用统一CRNN模型在九种数据集上（包括阿拉伯语、拉丁语等）进行行级HTR，变化训练规模（100到全量），并进行误差分析和数据特性分析。

Result: 性能差距在低资源时大，全量时仍有5-7 CER差异；注释错误导致性能下降；阿拉伯语字符分布更重尾，视觉相似字符混淆错误率更高（30% vs 15%）；固定训练样本覆盖不足。

Conclusion: 注释清理可缩小但不能消除差距；阿拉伯语的高视觉变异性需要更多数据；需改进数据标注和减少混淆错误以缩小性能差距。
...
### [60] [BindEdit: Taming Attention Leakage for Precise Multi-Object Image Editing](https://arxiv.org/abs/2606.18906)
*Chaewon Park,Soyoon Lee,Naeun Lee,Minjung Shin,Seogkyu Jeon,Kibeom Hong*

Main category: cs.CV

TL;DR: 提出BindEdit方法，通过在单扩散轨迹中施加注意力约束解决多对象编辑中的注意力泄漏问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑方法在复杂多对象场景中因注意力泄漏导致语义混合、对象重复或编辑不完整。

Method: BindEdit通过联合正则化交叉/自注意力抑制Edit-Token Leakage，通过交叉注意力重平衡抑制Source Dominance Leakage，并加入区域保真项。

Result: 在单对象和多对象编辑任务中均优于现有方法，单扩散轨迹内保持稳健性能。

Conclusion: BindEdit有效解决注意力泄漏，实现高质量多对象图像编辑。
...
TL;DR: 提出一种从无标签人类视频中学习动作先验的框架，通过混合解缠VQ-VAE和意图-感知解耦策略，仅需少量数据即可达到有监督VLA性能。


<details>
  <summary>Details</summary>
Motivation: 通用VLA模型需要大量带标注的机器人数据，而人类视频虽丰富但缺乏动作标签，因此希望利用无标签人类视频提取动作先验。

Method: 使用混合解缠VQ-VAE解耦运动动态与背景，构建跨实体动作码本；预训练VLM学习动作意图；适应阶段采用意图-感知解耦策略，由VLM预测动作意图，冻结的视觉编码器提供状态特征给动作专家，减少动作幻觉。

Result: 仅用无标签人类视频预训练，在模拟和真实环境中，与使用大量标注数据的SOTA VLA模型性能相当，且下游适应仅需50条轨迹。

Conclusion: 该方法有效利用了无标签人类视频学习通用动作先验，并通过解耦策略高效适应具体实体，显著降低对标注数据的依赖。
...
TL;DR: 通过视觉思维蒸馏，Visual-OPSD在保持推理速度大幅提升的同时，显著提高了多模态模型的性能。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型使用视觉思维（VT）提升空间任务但推理成本高，且VT的直接像素收益有限，但生成路径编码了有用推理，因此需要利用该推理而不增加成本。

Method: 提出Visual-OPSD，教师和学生共享权重，教师使用特权VT，学生仅看问题，通过token级JSD蒸馏将教师推理转移到纯文本学生。

Result: 在九个基准上，Visual-OPSD比教师提升3.40pp，速度提升14.3倍，在VSP上比同规模VLM提升63.83pp，且噪声控制实验证明收益来自生成路径的语义内容。

Conclusion: 生成路径的语义内容有效，通过蒸馏可在低成本下获得相同收益。
...
TL;DR: 提出新的非凸替代张量核范数与张量Ky Fan p-k范数之比（TNPK）用于低秩张量补全，开发ADMM算法，实验证明优越性能。


<details>
  <summary>Details</summary>
Motivation: 传统核范数近似张量tubal秩不够准确，需要更精确的非凸代理。

Method: 提出TNPK替代张量核范数，建立LRTC模型，证明低秩张量是局部极小值，推导Ky Fan p-k逆范数的近端算子，开发ADMM算法。

Result: 在合成和真实数据集上，所提方法优于现有最先进方法。

Conclusion: TNPK代理和ADMM算法能有效进行低秩张量补全，具有尺度不变性、参数灵活性和收敛保证。
...
### [64] [PorTEXTO: A European Portuguese Benchmark for Visual Text Extraction](https://arxiv.org/abs/2606.19096)
*João Cardeira,Diogo Glória-Silva,Manuel Letras da Luz,Rafael Ferreira,Diogo Tavares,David Semedo,João Magalhães*

Main category: cs.CV

TL;DR: 提出PorTEXTO基准，用于现代葡萄牙语OCR，发现专用多语言数据比模型规模更重要。


<details>
  <summary>Details</summary>
Motivation: 欧洲葡萄牙语在OCR基准测试中缺失，现有基准侧重于历史文献，需要现代OCR评估。

Method: 使用前沿LVLM转录和母语者审核的注释流程来构建基准。

Result: 大多数模型在真实样本上性能显著下降；专用多语言数据比模型大小或分辨率更能提升表现。

Conclusion: 应发布开放的葡萄牙语OCR资源。
...
TL;DR: Hand-4DGS是一个前馈框架，用于从自我中心视频重建动态4D手部，使用网格引导表示和时间卷积，实现快速推理和强泛化。


<details>
  <summary>Details</summary>
Motivation: 现有方法多关注多视角3D手部重建或4D人体重建，而自我中心4D手部重建面临快速头部运动、手部动态、严重遮挡和单视角模糊等挑战，需要新的解决方案。

Method: 提出Hand-4DGS框架，采用网格引导表示提供结构先验，结合时间卷积建模动态运动，通过高斯泼溅进行2D图像监督，无需3D手部姿态真值。

Result: 在H2O和ARCTIC数据集上显著优于基线，实现约60 FPS的快速推理，并展现出强泛化能力。

Conclusion: 前馈网络的泛化能力与有效2D监督相结合，避免了昂贵3D标注，为自我中心4D手部重建提供了高效且实用的方法。
...
### [66] [Moebius: 0.2B Lightweight Image Inpainting Framework with 10B-Level Performance](https://arxiv.org/abs/2606.19195)
*Kangsheng Duan,Ziyang Xu,Wenyu Liu,Xiaohu Ruan,Xiaoxin Chen,Xinggang Wang*

Main category: cs.CV

TL;DR: Moebius是一个高效的轻量级图像修复框架，通过Local-λ Mix Interaction块和自适应多粒度蒸馏策略，在极低参数(0.22B)和推理时间(>15x加速)下达到甚至超越10B级模型FLUX.1-Fill-Dev的性能。


<details>
  <summary>Details</summary>
Motivation: 10B级工业基础模型计算成本高昂，难以实际部署；极端结构压缩导致表示瓶颈，需要高效轻量级方案。

Method: Moebius重构扩散骨干，引入Local-λ Mix Interaction (LλMI)块（包含Local-λ和Interactive-λ模块）以压缩参数保持交互；同时采用自适应多粒度蒸馏策略，在潜在空间内动态平衡多种梯度损失以实现高保真对齐。

Result: 在自然和肖像基准上，Moebius与10B级FLUX.1-Fill-Dev相当或更优，使用仅0.22B参数（2%），推理加速超15倍。

Conclusion: Moebius为高保真图像修复设立了新的效率标准。
...
### [67] [ROSA-TFormer: A Radar-Optical Sensor-Aware Temporal Transformer for Pinus sylvestris Plantation Classification in Northern Shaanxi Using GEE-Derived Sentinel-1/2 Time Series](https://arxiv.org/abs/2606.19204)
*Nengbo Zhang,Chang sheng*

Main category: cs.CV

TL;DR: 提出ROSA-TFormer，一种雷达-光学传感器感知的时间Transformer，利用Sentinel-1/2时间序列数据对樟子松进行分类，在点级数据集上达到99.67%的总体准确率。


<details>
  <summary>Details</summary>
Motivation: Accurate identification of Pinus sylvestris var. mongolica plantations is important for monitoring afforestation quality and ecological restoration in northern Shaanxi.

Method: This paper proposes ROSA-TFormer, a radar-optical sensor-aware temporal Transformer for P. sylvestris classification using Sentinel-1/2 time-series data generated on Google Earth Engine. The model integrates separate SAR and optical embedding branches, a sensor-aware gate, and temporal attention pooling to capture multi-source seasonal features.

Result: Experiments on monthly and half-month point-level datasets show that ROSA-TFormer achieves strong classification performance, with 99.67% overall accuracy, 99.56% macro F1, and 98.91% P. sylvestris F1 on the HalfMonth-dataBig dataset. Spatial block validation and ablation results further indicate the effectiveness of radar-optical temporal fusion and sensor-aware modeling.

Conclusion: The results demonstrate the potential of ROSA-TFormer for point-level P. sylvestris plantation classification, while broader wall-to-wall validation remains necessary.
...
### [68] [GUMP-Net: An interpretable model-data-driven intelligent algorithm for multi-class pelvic segmentation](https://arxiv.org/abs/2606.19215)
*Liheng Wang,Yinghui Zhang,Licheng Zhang,Hailin Xu,Qiyong Cao,Chong Chen*

Main category: cs.CV

TL;DR: GUMP-Net结合改进的测地线活动轮廓模型和深度神经网络，实现多类骨盆分割，在小数据集上表现更优且具有可解释性。


<details>
  <summary>Details</summary>
Motivation: 骨盆分割对精准诊疗和手术规划至关重要，现有方法在小数据集和可解释性方面存在不足。

Method: 提出GUMP-Net，包含三个模块：目标检测模块（自动水平集初始化）、边缘检测器模块（学习解剖感知边缘）和迭代模块（深度水平集演化）。

Result: 在骨盆数据集上比现有方法更准确、鲁棒、一致，特别在小训练数据下，并在踝关节数据集上验证了泛化性。

Conclusion: 不仅提供高效分割方法，还从可解释几何视角理解深度学习分割。
...
### [69] [CABLE: Cloud-Assisted Bandwidth-efficient LMM-based Encoding for V2X Systems](https://arxiv.org/abs/2606.19258)
*Haohua Que,Zhipeng Bao,Qianyi Wu,Handong Yao*

Main category: cs.CV

TL;DR: CABLE是一种带宽高效的LMM编码框架，通过反馈循环仅传输ROI掩码，减少通信开销和预填充延迟。


<details>
  <summary>Details</summary>
Motivation: 全分辨率帧传输导致高通信开销和云侧预填充延迟，需要高效的边缘-云编码方案。

Method: 利用自我运动补偿传播先验云分割掩码，用残差运动线索精炼，通过走廊包络整合断开区域形成稳健ROI，仅上传ROI掩码，云分割输出作为下一帧先验，构成mask-to-ROI-to-LMM反馈循环。

Result: 在五个数据集上，ROI像素覆盖减少73-87%，预填充速度提升5-8倍，检测质量有适度权衡。

Conclusion: CABLE有效平衡带宽效率和感知性能，适用于V2X边缘-云协同感知。
...
### [70] [A Unified Framework for Efficient Remote Sensing Visual Question Answering: Adapting Dual, Hybrid, and Encoder-Decoder Architectures](https://arxiv.org/abs/2606.19277)
*Timothy Agboada,Shikha Chandel,Yadav Raj Ghimire,Leila Hashemi-Beni*

Main category: cs.CV

TL;DR: 针对遥感视觉问答，提出参数高效微调策略RS Adapter，比较三种视觉语言模型架构，发现混合FLAVA架构效果最佳。


<details>
  <summary>Details</summary>
Motivation: 通用基础模型直接应用于遥感VQA存在领域漂移和计算开销大，需要参数高效的微调方法。

Method: 提出统一架构手术管道，在冻结骨干的注意力与MLP层注入轻量瓶颈适配器，仅训练少于5%的参数。

Result: 所有适配模型均收敛，混合FLAVA架构在多模态推理与检索能力上优于其他架构，建立了资源高效VQA的新基线。

Conclusion: RS Adapter策略能有效适应遥感VQA任务，其中FLAVA架构在平衡推理与检索方面表现最优。
...
### [71] [Confidence is Not Reliability: Rethinking MC Dropout in Brain Tumour Segmentation](https://arxiv.org/abs/2606.19300)
...
### [72] [NeuMesh++: Towards Versatile and Efficient Volumetric Editing with Disentangled Neural Mesh-based Implicit Field](https://arxiv.org/abs/2606.19316)
*Chong Bao,Yuan Li,Bangbang Yang,Yujun Shen,Hujun Bao,Zhaopeng Cui,Yinda Zhang,Guofeng Zhang*

Main category: cs.CV

TL;DR: 提出一种基于网格的神经辐射场表示，实现高效全面的3D场景编辑。


<details>
  <summary>Details</summary>
Motivation: 现有神经渲染编辑方法功能有限，仅支持刚性变换和类别特定编辑。

Method: 基于网格顶点编码解耦的几何、纹理和语义码，配合局部空间参数化、可学习修改颜色、空间感知优化和语义辅助区域选择等技术。

Result: 在真实和合成数据集上的实验表明，该方法在表示质量和编辑能力上优于现有方法。

Conclusion: 该方法实现了高效全面的3D场景编辑，具有优越的表示质量和编辑能力。
...
### [73] [Beyond the Current Observation: Evaluating Multimodal Large Language Models in Controllable Non-Markov Games](https://arxiv.org/abs/2606.19338)
*Shengyuan Ding,Xilin Wei,Xinyu Fang,Haodong Duan,Dahua Lin,Jiaqi Wang,Yuhang Zang*

Main category: cs.CV

TL;DR: 提出RNG-Bench基准，用于评估多模态模型在非马尔可夫游戏中的记忆与推理能力，包含两个游戏和记忆间隙指标，实验表明遗忘是主要错误来源，微调可提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法独立评估模型在交互过程中重建过去观测并据此行动的能力，往往暴露完整状态或混淆相关技能。

Method: 设计RNG-Bench，包含Matching Pairs和3D Maze两个游戏，通过三个难度轴（网格大小、视觉模式、观测模态）控制难度，采用头对头决斗协议控制实例方差，并引入记忆间隙指标区分遗忘与决策错误。最后对Qwen3.5-9B进行微调。

Result: 最难配置需约128K token和350图像输入，现有MLLMs未饱和；记忆间隙分析表明大多数错误源于遗忘；微调Qwen3.5-9B在RNG-Bench上提升性能并泛化至其他基准。

Conclusion: RNG-Bench有效评估了模型的记忆与推理能力，微调是提升性能的有效手段。
...
### [74] [RegimeVGGT: Layer-Wise Spatially Preserving Redundancy Removal for Visual Geometry Grounded Transformer](https://arxiv.org/abs/2606.18439)
*Jinhao You,Shuo Lyu,Zhuohang Lyu,Tanxuan Li,Zibo Zhao,Jiaxiang Hu,Kai Tang,Yichen Guo*

Main category: cs.CV

TL;DR: 提出RegimeVGGT，一种训练无关的加速方法，通过层感知U型压缩实现6.7倍加速且保持重建质量。


<details>
  <summary>Details</summary>
Motivation: VGGT的二次交叉帧注意力限制可扩展性，现有训练无关加速器忽略层异质性。

Method: 通过谱、探针和因果分析揭示三个层间状态，然后应用层维U型压缩：显著性引导带状合并保护几何和边缘显著令牌，选择性保护K/V下采样通过相移空间网格、参考帧锚点及未压缩相机/注册令牌保留交叉帧空间覆盖和姿态关键路径。

Result: 在匹配重建质量下，实现6.7倍加速。

Conclusion: 层感知压缩有效提升VGGT效率，且无需训练。
...
### [75] [Data-Forcing Distillation: Restoring Diversity and Fidelity in Few-Step Video Generation](https://arxiv.org/abs/2606.18478)
*Siyi Chen,Shaowei Liu,Yixuan Jia,Zian Wang,Huan Ling,Qing Qu,Jun Gao*

Main category: cs.CV

TL;DR: 提出数据强迫蒸馏方法恢复视频生成中的多样性和保真度


<details>
  <summary>Details</summary>
Motivation: 现有分布匹配蒸馏方法因逆向KL目标导致样本多样性下降和过饱和

Method: 通过教师分数差异引导学生接近真实数据分布，仅需一行代码修改

Result: 在100-300步微调内有效恢复多样性和保真度，解决过饱和，性能超越教师模型

Conclusion: DFD是简单有效的后训练框架，显著提升视频生成质量
...
### [76] [A Prototypical Signature Approach for Writer-Independent Offline Signature Verification](https://arxiv.org/abs/2606.18528)
*Kecia G. de Moura,Robert Sabourin,Rafael M. O. Cruz*

Main category: cs.CV

TL;DR: A data-driven strategy using prototypical signatures improves offline handwritten signature verification by generating diverse, informative negative samples, enhancing detection of skilled forgeries and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Real forgeries are rare, and random negative sampling lacks diversity, increases redundancy, and escalates computational cost, leading to inefficient training.

Method: Propose a data-driven strategy to generate diverse, informative negative samples using prototypical signatures, combined with a primal-form linear SVM.

Result: Prototypical signatures yield more informative negative samples, improve skilled forgery detection, are backbone-agnostic, and when combined with linear SVM, offer an alternative to RBF models with better scalability and efficiency.

Conclusion: The approach effectively addresses the limitations of random negative sampling and improves both performance and efficiency.
...
### [77] [Technical Report for ICRA 2026 GOOSE 2D Fine-Grained Semantic Segmentation Challenge: Leveraging DINOv3 for Robust Outdoor Scene Understanding in Field Robotics](https://arxiv.org/abs/2606.18582)
*Jaeil Park,Hyobin Choi,Sangjin Lee,Hyungtae Lim,Sung-Hoon Yoon*

Main category: cs.CV

TL;DR: 提出结合自监督ViT和推理聚合策略的语义分割方法，在ICRA 2026挑战中取得第一名。


<details>
  <summary>Details</summary>
Motivation: 针对越野图像细粒度语义分割的挑战，改进网络设计和推理策略以提升分类精度。

Method: 网络设计采用自监督DINOv3 ViT-L/16骨干、ViT-Adapter和Mask2Former解码器，并加入粗类别辅助损失；推理时采用多尺度、水平翻转测试增强以及三个最佳检查点集成。

Result: 官方复合得分76.57%，其中细类mIoU 69.32%，类别级mIoU 83.81%，排名第一。

Conclusion: 该方法有效提升了细粒度分割性能，最终获得竞赛第一名。
...
### [78] [APT: Atomic Physical Transitions for Causal Video-Language Understanding](https://arxiv.org/abs/2606.18586)
*Shang Wu,Haoran Lu,Songling Liu,Chenwei Xu,Lie Lu,Pranav Maneriker,Fan Du,Manling Li,Zhaoran Wang,Han Liu*

Main category: cs.CV

TL;DR: 引入原子物理转移（APTs）作为最小因果状态变化，使物理事件过程明确，并提出APT-Tune参数高效微调方法，使VLMs在不遗忘事件级理解的情况下学习因果转移。


<details>
  <summary>Details</summary>
Motivation: 事件标签如“弹跳”隐藏了组成事件的因果状态变化过程，需要显式建模物理转移以理解事件为何发生。

Method: 定义原子物理转移（APTs）为最小、时间局部的状态变化，链接可见线索与主动物理机制及前后动态状态；构建包含14种转移类型、27,303个实例的混合来源数据集；提出APT-Tune，结合图像填充感知监督、格式条件协同训练和机制条件域到类型解码，通过仅11M LoRA参数微调Qwen3-VL-2B。

Result: 当前VLMs在零样本下APT召回率最多14%，直接微调导致事件级遗忘；APT-Tune显著提高APT召回率并改善事件级视频转移。

Conclusion: APTs不是新的答案格式，而是与人类对齐的因果监督信号，用于物理视频理解。
...
### [79] [UniTemp: Unlocking Video Generation in Any Temporal Order via Bidirectional Distillation](https://arxiv.org/abs/2606.18702)
*Lin Zhang,Sicheng Mo,Zefan Cai,Jinhong Lin,Zihao Lin,Jiuxiang Gu,Krishna Kumar Singh,Yuheng Li,Yin Li*

Main category: cs.CV

TL;DR: 提出UniTemp，一种基于自回归视频扩散模型的双向蒸馏框架，通过引入块状锚点潜在变量，实现任意方向（前向、后向、中间）的视频生成，性能与仅前向方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视频扩散模型仅限于前向时间生成，但实际视频创作需要灵活生成顺序，如基于未来上下文向后扩展或基于过去和未来上下文进行中间生成。

Method: 提出UniTemp，采用双向蒸馏框架，训练一个自回归学生模型；引入块状锚点潜在变量，解决因果3D VAE在反向生成时因缺失过去上下文导致的间段不连续问题，从而支持任意方向视频生成。

Result: UniTemp在短和长视频生成上与仅前向方法保持竞争力，同时支持双向视频扩展、中间生成、循环生成、场景过渡和视觉故事生成等多种工作流程。

Conclusion: UniTemp通过单一模型实现了灵活的任意方向视频生成，且性能不低于前向专用方法，为实际视频创作提供了更丰富的控制能力。
...
### [80] [SpectralDiT: Timestep-Conditioned Spectral Residual Correction for Flow-Matching DiTs](https://arxiv.org/abs/2606.18765)
*Jiayu Tian*

Main category: cs.CV

TL;DR: 提出 SpectralDiT，一种轻量级扩散变压器改进，通过时间步条件频谱校正提升生成质量，以极小额外开销实现显著 FID 降低。


<details>
  <summary>Details</summary>
Motivation: 改进 Diffusion Transformer 的 MLP 残差分支，通过频谱分解和门控机制增强模型表达能力。

Method: 在 MLP 残差分支添加时间步条件频谱校正模块，将残差更新分解为低频和高频分量，并学习零初始化的加法门控，使模型初始匹配基线 DiT。

Result: 在 CIFAR-10 像素空间生成中，FID 从 20.78 降至 19.71；在 ImageNet-100 潜在扩散中，以 0.6% 额外 FLOPs 和 1.36% 额外参数实现 8.7% 相对 FID 降低。

Conclusion: SpectralDiT 有效提升生成质量，门控可视化显示稳定的块特定频谱校正模式。
...
### [81] [Learned Radius Estimation for UDF-Based Point Cloud Reconstruction](https://arxiv.org/abs/2606.18787)
*Eito Ogawa,Hiroshi Watanabe*

Main category: cs.CV

TL;DR: 提出学习半径选择器改进UDF表面重建的精细尺度精度。


<details>
  <summary>Details</summary>
Motivation: 传统固定或曲率启发式半径无法捕获异质局部几何，限制精度。

Method: 学习逐查询半径选择器，预测连续支撑半径并插入冻结的LoSF-UDF，用抛物线插值缓存误差曲线得到离网格目标半径训练。

Result: 实验显示精细尺度重建精度提升。

Conclusion: 学习方法有效改善表面重建质量。
...
### [82] [Automatic ply-specific analyses of CFRP micrographs using shortest-path-based ply distinction](https://arxiv.org/abs/2606.18894)
*Jonas Naumann,Jonas P. Appels,Julius Biermann,Christopher Gorsky,Timo de Wolff,Christoph Brauer*

Main category: cs.CV

TL;DR: 提出一种基于图的最短路径算法，从语义分割掩膜中自动分离层实例，实现碳纤维复合材料的定量微观结构分析。


<details>
  <summary>Details</summary>
Motivation: 为了从高分辨率显微图像的语义分割结果中区分各个层，进而进行全面的定量分析，以揭示制造不均匀性和关联机械性能。

Method: 将分割掩膜视为以像素为顶点的图，利用最短路径算法计算层分离路径，并根据路径将纤维像素分配给对应层。

Result: 成功应用于多种特征的高分辨率显微图像，实现了局部纤维体积分数、层厚度等微观结构参数的定量测量。

Conclusion: 该方法有助于揭示制造引起的不均匀性，优化制造参数，并将机械性能与微观结构缺陷联系起来。
...
### [83] [DVANet: Degradation-aware Visual-prior Alignment Network for Image Restoration](https://arxiv.org/abs/2606.19097)
*Yanjie Tu,Qingsen Yan,Axi Niu,Tao Hu,Haokui Zhang,Jiantao Zhou*

Main category: cs.CV

TL;DR: 提出DVANet，一种基于半二次分裂优化的深度展开网络，通过退化感知观测一致性和视觉先验引导重建的协同展开实现全能图像恢复，在多种退化场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有端到端方法缺乏可解释性，深度展开方法依赖固定退化假设或预定义信息，难以适应复杂退化和局部损坏，导致退化抑制和细节恢复受限。

Method: 基于半二次分裂优化算法构建深度展开网络，包含退化感知观测一致性分支（使用退化表示模块提取全局/局部退化线索并进行退化条件映射）和视觉先验引导重建分支（引入DINOv3提供层次视觉先验以补充损坏区域结构）。

Result: 在多种场景退化和跨域图像恢复任务上取得优越或竞争性能，具有良好的退化适应性和泛化能力。

Conclusion: DVANet通过协同展开有效统一了复杂退化下的图像恢复，性能优越。
...
### [84] [Transformer Geometry Observatory TGO-I: Spectral Geometry Observatory](https://arxiv.org/abs/2606.19249)
*Kaustubh Kapil,Kishor P. Upla*

Main category: cs.CV

TL;DR: 提出了Transformer几何观测站（TGO）框架，用于研究Vision Transformer的表示几何，发现训练过程中维度利用增加，各向异性降低，方差重新分布，CLS token有效维度最高。


<details>
  <summary>Details</summary>
Motivation: 尽管Vision Transformers被广泛采用并取得成功，对其维度和表示几何的基本理解仍不足。

Method: 引入TGO框架，其中TGO-I关注ViT表示的谱几何，使用ViT-Small/16在ImageNet-100上训练，分析有效秩、稳定秩、参与比、谱熵、谱平坦度、谱各向异性、协方差结构、特征谱和奇异值谱。

Result: 观察到维度利用持续增加，各向异性降低，谱熵增加，参与比增加，特征谱变得更平坦。与直觉相反，方差在表示维度上重新分布，而非集中于少数主导方向，CLS token表示具有最高有效维度和最低各向异性。

Conclusion: 这些发现挑战了传统直觉，揭示了ViT表示几何的新特性，有助于更深入理解ViT。
...
### [85] [OneCanvas: 3D Scene Understanding via Panoramic Reprojection](https://arxiv.org/abs/2606.19253)
*Bartłomiej Baranowski,Dave Zhenyu Chen,Matthias Nießner*

Main category: cs.CV

TL;DR: OneCanvas通过将多视图补丁特征聚合到全景画布上，配合空间预训练课程，实现了高效3D场景理解。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景理解方法依赖复杂几何编码器或大训练预算，OneCanvas旨在简化。

Method: 将每个补丁反投影到3D世界坐标，根据经纬度连续放置到等距柱状画布，并添加3D位置嵌入；预训练VLM直接处理该表示；引入空间预训练课程，通过程序化放置物体补丁生成监督信号。

Result: 在SQA3D和VSI-Bench上达到最先进水平，在SPBench上泛化到分布外数据，训练计算量比最强方法少一个数量级。

Conclusion: OneCanvas是一种高效、通用的3D场景理解方法，减少了计算开销同时保持高性能。
...
### [86] [MCompassRAG: Topic Metadata as a Semantic Compass for Paragraph-Level Retrieval](https://arxiv.org/abs/2606.18508)
*Amirhossein Abaskohi,Raymond Li,Gaetano Cimino,Peter West,Giuseppe Carenini,Issam H. Laradji*

Main category: cs.CL

TL;DR: MCompassRAG是一种元数据引导的检索框架，通过主题级信号和LLM蒸馏提升RAG系统的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在文档分块与检索之间存在权衡：细粒度块提高精度但增加延迟和成本，大块降低语义可靠性。深度研究任务需要快速且精确的检索。

Method: 利用主题元数据丰富块嵌入，通过LLM教师蒸馏训练轻量级检索器，实现主题感知检索，无需额外LLM调用。

Result: 在六个复杂检索基准上，信息效率平均提升8.24%，延迟比最强基线降低5倍以上。

Conclusion: MCompassRAG通过主题感知检索有效平衡了精度与效率，适用于大规模异质语料库的深度研究任务。
...
### [87] [Lost in a Single Vector: Improving Long-Document Retrieval with Chunk Evidence Aggregation](https://arxiv.org/abs/2606.18781)
*Shanshan Lyu,Yiwei Wang,Yujun Cai,Jiafeng Guo,Shenghua Liu*

Main category: cs.CL

TL;DR: 提出DICE方法，通过分块编码和聚合改善长文档检索中的证据稀释问题。


<details>
  <summary>Details</summary>
Motivation: 长文档中关键片段在文档编码时被弱化，导致检索失败。

Method: DICE：将文档分割为块，独立编码后聚合为单个向量，保持查询-文档接口不变。

Result: 在LongEmbed上，检索性能提升，尤其超过4k tokens的片段；EDI在92.8%案例中低于基线。
...
### [88] [CoreMem: Riemannian Retrieval and Fisher-Guided Distillation for Long-Term Memory in Dialogue Agents](https://arxiv.org/abs/2606.18406)
*Jiaqi Chen,Yongqin Zeng,Shaoshen Chen,Yijian Zhang,Hai-Tao Zheng,Chunxia Ma,XiuTeng Zhou*

Main category: cs.CL

TL;DR: 本文提出CoreMem，一种基于信息几何的资源高效边云记忆架构，通过黎曼检索和Fisher引导的离散令牌蒸馏，在8GB VRAM设备上实现长期记忆对话。


<details>
  <summary>Details</summary>
Motivation: 现有对话代理长期记忆方法在边缘设备上存在内存和计算瓶颈，且检索中余弦相似度导致hubness问题，压缩中启发式规则导致语法碎片化，缺乏统一理论基础。

Method: CoreMem架构包含两个核心组件：1) Riemannian retrieval，使用Fisher-Rao度量进行局部自适应检索，通过Mahalanobis距离和Woodbury加速实现实时搜索；2) Fisher-guided discrete token distillation (FDTD)，通过Fisher信息灵敏度分数进行层次化句子到令牌压缩，平衡压缩率和KL散度并保护语法结构。

Result: 在LOCOMO和LongMemEval-S基准上，CoreMem在开放域和时序推理上分别提升4.51和4.17个百分点，且在8GB VRAM预算内运行。

Conclusion: CoreMem成功弥合了资源受限边缘设备与理论化终生记忆代理之间的差距，提供了在消费级硬件上部署长期记忆对话代理的可能性。
...
### [89] [SproutRAG: Attention-Guided Tree Search with Progressive Embeddings for Long-Document RAG](https://arxiv.org/abs/2606.18381)
*Amirhossein Abaskohi,Issam H. Laradji,Peter West,Giuseppe Carenini*

Main category: cs.CL

TL;DR: SproutRAG是一种注意力引导的分层RAG框架，通过构建二叉注意树实现多粒度检索而无需额外LLM调用。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法在平衡检索粒度与上下文连贯性时依赖LLM、固定上下文扩展或有损摘要，导致效率低或信息损失。

Method: SproutRAG利用学习到的句子间注意力构建二叉块树，将句子组织成层次化的语义单元，并在检索时使用分层波束搜索进行多粒度检索，模型端到端训练。

Result: 在四个基准测试中，SproutRAG的平均信息效率（IE）比最强基线提高6.1%。

Conclusion: SproutRAG通过注意力引导的分层架构有效解决了检索粒度与上下文连贯性的权衡问题，避免了额外LLM调用和信息损失。
...
### [90] [Beyond Reward Engineering: A Data Recipe for Long-Context Reinforcement Learning](https://arxiv.org/abs/2606.18831)
*Xiaoyue Xu,Sikui Zhang,Xiaorong Wang,Xu Han,Chaojun Xiao*

Main category: cs.CL

TL;DR: 一个简单有效的数据配方结合最小化结果导向GRPO显著提升长上下文推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有RL方法多关注奖励工程，但训练数据稀缺，本文从数据为中心的角度改进长上下文推理。

Method: 构建包含检索、多证据合成和推理三个任务家族共8个数据集（约14K例子），采用最小化结果导向GRPO进行训练。

Result: 在Qwen3-4B/8B/30B-A3B模型上，七个长上下文基准平均提升+7.2/+3.2/+6.4分，超越先前RL训练集；在智能体任务中GAIA提升4.8分，BrowseComp提升7.0分。

Conclusion: 数据配方有效且可迁移至智能体任务，将公开数据集以促进未来研究。
...
### [91] [ScholarSum: Student-Teacher Abstractive Summarization via Knowledge Graph Reasoning and Reflective Refinement](https://arxiv.org/abs/2606.18850)
*Bohou Zhang,Xiaoyu Tao,Mingyue Cheng,Huijie Liu,Qi Liu*

Main category: cs.CL

TL;DR: A hierarchical reflective graph-based framework that emulates a student-teacher writing process achieves both fluency and faithfulness in scientific summarization.


<details>
  <summary>Details</summary>
Motivation: Existing abstractive summarization methods fail to simultaneously achieve linguistic fluency and factual faithfulness.

Method: ScholarSum constructs a hierarchical knowledge graph from the document, uses a 'student' to generate an initial draft, and a 'teacher' reviewer to iteratively examine and refine the summary via evidence retrieval and rewriting.

Result: ScholarSum significantly outperforms previous baselines in both completeness and faithfulness.

Conclusion: The proposed student-teacher framework effectively reconciles fluency and faithfulness in scientific summarization.
...
### [92] [Morpheus: A Morphology-Aware Neural Tokenizer and Word Embedder for Turkish](https://arxiv.org/abs/2606.18717)
*Tolga Şakar*

Main category: cs.CL

TL;DR: Morpheus is a neural morpheme-boundary model for Turkish that serves as a lossless, morphology-aware tokenizer and word-embedding producer, achieving strong performance on reversible tokenization and lexical retrieval tasks.


<details>
  <summary>Details</summary>
Motivation: Turkish is agglutinative, with meaning carried by morphemes, but subword tokenizers split words by corpus statistics, fragmenting semantically loaded suffixes and sometimes failing to decode back to original text.

Method: A differentiable Poisson-binomial dynamic program converts per-character boundary probabilities into soft morpheme memberships during training and exact segments at inference, with no string normalization, ensuring decode(encode(w)) = w. The same forward pass also produces a structured word embedding.

Result: Among reversible tokenizers, Morpheus achieves the lowest bits-per-character (1.425), roughly doubles morphological alignment (MorphScore macro-F1 0.61 vs. ~0.32), and uses ~19% less GPU memory than 64K-vocabulary subword tokenizers. As an embedder, it leads on lexical retrieval (root-family MAP 0.85) and same-root verification (ROC-AUC 1.00), surpassing BGE-M3 and BERTurk, but lags on context-dependent tasks (NER, case/number probing).

Conclusion: Morpheus offers a strong trade-off for morphology-intensive tasks with its root-centric geometry, achieving high performance on lexical and morphologically aligned tasks while using less memory, though heavier contextual encoders remain superior for context-dependent tasks.
...
### [93] [Which Sections of a Research Paper Best Reveal Its Research Methods? Evidence from Library and Information Science](https://arxiv.org/abs/2606.19051)
*Qiuyu Fang,Jiayi Hao,Chengzhi Zhang*

Main category: cs.CL

TL;DR: 提出基于全文分段组合策略的研究方法自动分类方法，有效提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究依赖标题摘要但信息有限，全文过长且冗余，需更好利用全文内容。

Method: 将全文按物理位置分段，构建标注语料库，评估不同分段及组合的分类性能。

Result: 方法信息分布不均匀，中后段和末尾段区分力更强；书目元数据与跨段组合可提升性能。

Conclusion: 分段组合策略有效，方法信息分布在全文不同部分，联合元数据可增强分类。
...
### [94] [JetFlow: Breaking the Scaling Ceiling of Speculative Decoding with Parallel Tree Drafting](https://arxiv.org/abs/2606.18394)
...
TL;DR: JetFlow is a head-based speculative decoding framework that combines one-forward drafting efficiency with branch-wise causal conditioning, achieving up to 9.64x speedup on MATH-500 and consistently outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Prior head-based speculative decoding methods face a causality-efficiency dilemma: autoregressive drafters are effective but costly, while bidirectional drafters produce inconsistent trees, limiting the scaling of draft budget into speedup.

Method: JetFlow trains a causal parallel draft head over fused hidden states from the frozen target model, generating candidate trees aligned with the target model's autoregressive factorization, enabling efficient one-forward drafting with branch-wise causality.

Result: On dense and MoE Qwen3 models across math, coding, and chat benchmarks, JetFlow achieves up to 9.64x speedup on MATH-500 and 4.58x on open-ended conversational workloads on H100 GPUs, consistently outperforming bidirectional-head and tree-based SD baselines.

Conclusion: JetFlow effectively breaks the scaling limitation of speculative decoding by converting larger draft budgets into longer accepted prefixes and higher end-to-end speedup, demonstrating significant latency gains in realistic serving.
...
### [95] [VISUALSKILL: Multimodal Skills for Computer-Use Agents](https://arxiv.org/abs/2606.18448)
*Ziyan Jiang,Li An,Yujian Liu,Jiabao Ji,Qiucheng Wu,Jacob Andreas,Yang Zhang,Shiyu Chang*

Main category: cs.CL

TL;DR: VISUALSKILL is a hierarchical multimodal skill for computer-use agents that uses a central index over per-topic files with text and figures, constructed via authored documentation and live UI exploration, achieving a +15.3 point absolute gain over no-skill baseline and +8.3 over text-only skill on CUA benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing skill libraries represent skills as text only, ignoring the visual nature of GUI interaction, which limits performance on long-horizon tasks and unseen software.

Method: A two-stage pipeline that combines authored documentation with live-application UI exploration to construct hierarchical multimodal skills, organized as a central index over per-topic files, consumed by the agent via a load_topic MCP tool to fetch relevant text and figures on demand.

Result: On CUA-World and OSExpert-Eval, a Claude Code CLI agent with Claude Opus 4.6 using VISUALSKILL achieves an average score of 0.456, compared to 0.303 (no-skill) and 0.373 (matched text-only skill), representing +15.3 and +8.3 point absolute gains respectively.

Conclusion: Retaining visual figures in the skill artifact, rather than verbalizing them away, helps the agent identify UI elements and verify workflow state after each action, providing direct evidence that multimodal representation outperforms text-only representation for GUI agents.
...
### [96] [Steerable Cultural Preference Optimization of Reward Models](https://arxiv.org/abs/2606.18606)
*Minsik Oh,Advit Deepak,Sophie Wu,Douwe Kiela,Ekaterina Shutova*

Main category: cs.CL

TL;DR: 提出SCPO算法，通过加权训练平衡多元文化偏好，提升奖励模型性能和数据效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对齐研究主要关注统一偏好，缺乏对多元文化子社区的准确表达。

Method: 提出SCPO算法，在奖励模型训练中通过加权方法平衡不同文化偏好。

Result: 在PRISM和GlobalOpinionQA数据集上，少数群体奖励模型性能提升达7点，数据效率提高280%，并减轻了过度偏见。

Conclusion: SCPO有效且高效，能够平衡多元文化偏好，减少偏见。
...
### [97] [Dango: A Strictly L1-Only Large Language Model for Studying Second Language Acquisition](https://arxiv.org/abs/2606.19170)
*Shiho Matta,Yin Jou Huang,Fei Cheng,Takashi Kodama,Hirokazu Kiyomaru,Yugo Murawaki*

Main category: cs.CL

TL;DR: 提出了一个1.8B参数的LLM Dango，通过过滤L2污染和微调来模拟第二语言习得。


<details>
  <summary>Details</summary>
Motivation: 之前的研究使用较小或非解码器模型，限制了生成开放式文本的能力，不适合作为L2模拟器；且存在L2污染问题。

Method: 提出过滤方法减少L2污染，然后在LLM生成的L2学习课程上微调模型。

Result: Dango产生类似人类的L2生产模式，优于未过滤和标准多语言基线。

Conclusion: 发布模型、数据和代码以促进可重复的计算SLA研究和面向学习者的应用。
...
### [98] [DreamReasoner-8B: Block-Size Curriculum Learning for Diffusion Reasoning Models](https://arxiv.org/abs/2606.19257)
*Zirui Wu,Lin Zheng,Jiacheng Ye,Shansan Gong,Xueliang Zhao,Yansong Feng,Wei Bi,Lingpeng Kong*

Main category: cs.CL

TL;DR: 提出DreamReasoner-8B，采用块大小课程学习，实现高效的块扩散长链推理，性能与领先的自回归模型相当。


<details>
  <summary>Details</summary>
Motivation: 块扩散语言模型通过并行块去噪加速解码，但其在长链推理中的可扩展性尚未明确。

Method: 提出块大小课程学习，训练时从细粒度逐渐过渡到粗粒度的块大小，以弥合训练与推理块大小的粒度差距。

Result: 在数学和代码推理基准上，DreamReasoner-8B取得了与Qwen3-8B等领先开源自回归模型竞争的性能。

Conclusion: 为高效、推理能力的扩散语言模型建立了实践基础，并开源了模型。
...
### [99] [Montreal Forced Aligner and the state of speech-to-text alignment in 2026](https://arxiv.org/abs/2606.18466)
*Michael McAuliffe,Kaylynn Gunter,Michael Wagner,Morgan Sonderegger*

Main category: cs.CL

TL;DR: MFA 3.0通过扩展覆盖和模型改进，在多个语言上实现了接近或超越现有技术的强制对齐性能，平均边界误差低于15毫秒。


<details>
  <summary>Details</summary>
Motivation: MFA已成为最广泛使用的强制对齐工具，但自1.0版本以来经历了重大发展，需要系统记录其进展并评估其最新性能。

Method: 文档化了MFA 3.0的发展，并在英语、日语和韩语的四个基准数据集上将其与经典和神经强制对齐器进行性能对比。

Result: MFA 3.0在所有数据集上达到或接近最先进水平，平均边界误差低于15毫秒；适应和跨语言重映射对训练分布外的语言有效，发音概率建模和音系规则在特定条件下带来提升。

Conclusion: MFA 3.0表现优异，适应和跨语言重映射方法有效，发音概率建模和音系规则可在特定条件下提供额外增益。
...
### [100] [Beyond Scalar Scores: Exploring LLM-based Metrics for Clinical Significance Evaluation in Radiology Reports](https://arxiv.org/abs/2606.18797)
*Qingyu Lu,Ruochen Li,Liang Ding,Yufei Xia,Youxiang Zhu,Dacheng Tao*

Main category: cs.CL

TL;DR: 提出ReEvalMed基准和轻量级可解释指标，通过训练Qwen3-8B和MedGemma-4B模型，在临床误差检测上超越大模型，并建议单遍设置用于成本敏感部署。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标忽略临床准确性，LLM无法可靠区分临床错误和无害变化。

Method: 利用ReEvalMed基准，合成4k报告对，在Qwen3-8B和MedGemma-4B上训练轻量级可解释指标。

Result: 训练的指标在辨别和鲁棒性上超越32B级医疗LLM，与专有模型竞争力相当；两遍设置未一致提升性能。

Conclusion: 单遍训练指标是成本敏感部署的实用选择，两遍推理保留用于需平衡D-R的场景。
...
### [101] [Aligning Implied Statements for Implicit Hate Speech Generalizability with Context-Bounded Semi-hard Negative Mining](https://arxiv.org/abs/2606.18852)
*Wicaksono Leksono Muhamad,Yunita Sari*

Main category: cs.CL

TL;DR: 提出 ImpSH，一种通过将帖子与隐含声明对齐并利用上下文有界半硬负例进行对比学习的框架，用于隐式仇恨言论检测，改善跨域泛化。


<details>
  <summary>Details</summary>
Motivation: 隐式仇恨言论分类因意图常被暗示和语境掩盖而具有挑战性；现有监督对比方法过拟合表面线索且难以跨数据集迁移。

Method: 提出 ImpSH（三元组框架，对齐帖子与隐含声明，使用上下文有界半硬负例）和 AugSH（数据增强形成正例），在 BERT 和 HateBERT 上对 IHC、SBIC 和 DynaHate 进行受控评估。

Result: ImpSH 是标准监督对比基线 viable 的替代方案，通常提升跨域性能；表示分析显示更紧的正对和均匀全局分布，案例研究揭示域偏移下的典型假负例。

Conclusion: 通过上下文有界挖掘将帖子与隐含声明对齐，能提供更稳定的双射式映射到相关暗示，克服传统聚类表示学习的波动性。
...
### [102] [Beyond Tokenization: Direct Timestep Embedding and Contrastive Alignment for Time-Series Question Answering](https://arxiv.org/abs/2606.18986)
*Yafeng Wu,Huu Hiep Nguyen,Thin Nguyen,Hung Le*

Main category: cs.CL

TL;DR: CADE框架通过直接时间步嵌入和语义对齐，解决了LLM在时间序列问答中的标记化瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用基于块的编码器导致粒度固定和跨数据集迁移问题，且直接喂入数值序列会因标记化损失信息。

Method: 提出CADE框架，包含直接时间步嵌入（点式线性编码器和MLP投影器）和单向监督对比损失，对齐时间序列与语言表示。

Result: 在Time-MQA基准上，CADE在六个TSQA任务中一致提升性能，优于开源和专有LLM基线。

Conclusion: CADE通过直接时间步嵌入和语义对齐有效提升了TSQA性能。
...
TL;DR: 自动指标在评估LLM生成文本时面临有效性和区分力的权衡，引入RECOM数据集后发现没有指标能同时做好，建议报告两个轴并包含随机基线。


<details>
  <summary>Details</summary>
Motivation: 自动指标被默认使用，但需要同时完成两件事：区分真实内容对齐与表面巧合（有效性），以及区分更好或更差的系统（区分力）。在开放型、观点驱动的问答中，两者存在冲突。

Method: 构建RECOM数据集，包含15,000个2025年9月的r/AskReddit问题和真实社区回复，所有回复均晚于评估模型的训练截止日期。使用五个开源LLM（7-10B）生成答案，将每个答案与每个回复配对，采用余弦相似度、BERTScore等指标，并引入随机排列噪声基线，控制响应长度等因素。

Result: 余弦相似度能有效区分真假答案（Cohen's d≈2），但无法排名模型（|d|<0.1）；BERTScore精度在未控制长度时能排名模型（|d| up to 0.63），但控制长度后区分力消失（|d|=0.09），且有效性较弱（d≈0.8，对比余弦的≈2）。三个独立LLM法官也重现了有效性差距，但仅能弱分离模型。

Conclusion: 没有指标能同时兼顾有效性和区分力，建议在两条轴上报告指标，并包含明确的随机基线。RECOM数据集已公开。
...
### [104] [Freeing the Law with LOCUS: A Local Ordinance Corpus for the United States](https://arxiv.org/abs/2606.19334)
*Denis Peskoff,Joe Barrow,Christopher Vu,Diag Davenport*

Main category: cs.CL

TL;DR: 介绍LOCUS，一个包含美国绝大多数地方条例的语料库和县级协调访问层，并训练了基于ModernBERT的分类器用于分析。


<details>
  <summary>Details</summary>
Motivation: 地方条例对法律AI至关重要，但现有语料库缺乏此类数据，且这些条例分散在不同平台，难以批量获取。

Method: 通过OCR处理多种文档格式，收集了9,239个城市和县的公开条例；构建了覆盖2,309个县的协调访问层；并训练了基于ModernBERT的分类器和评分器。

Result: 原始语料库包含9,239个城市和县的代码，县级协调层覆盖美国大部分人口，并提供了覆盖元数据以支持可重复性。

Conclusion: LOCUS及其衍生模型已发布，旨在促进法律AI研究和地方条例的机器可读访问扩展。
...
### [105] [Continuous Audio Thinking for Large Audio Language Models](https://arxiv.org/abs/2606.18273)
*Gyojin Han,Dong-Jae Lee,Changho Choi,Jongsuk Kim,Junmo Kim*

Main category: cs.CL

TL;DR: 提出CoAT框架，通过连续潜在工作空间和专家蒸馏，在响应生成前组织声学信息，提升音频语言模型性能而不增加解码成本。


<details>
  <summary>Details</summary>
Motivation: 现有音频语言模型训练目标导致丢失声学信息，无法充分利用音频中的丰富内容。

Method: 引入连续音频思考空间，基于音频专家知识蒸馏，在单个预填充中处理，无需额外自回归解码。

Result: 在Qwen2-Audio、Qwen2.5-Omni-7B和Audio Flamingo 3三个模型上，音频推理、理解、分类、情感、转录等任务均取得性能提升。

Conclusion: CoAT有效辅助监督从思考位置传播到模型的文本响应。
...
### [106] [LLM Parameters for Math Across Languages: Shared or Separate?](https://arxiv.org/abs/2606.18453)
*Behzad Shomali,Luisa Victor,Tim Selbach,Ali Hamza Bashir,David Berghaus,Joachim Koehler,Mehdi Ali,Markus Frey*

Main category: cs.CL

TL;DR: 数学推理在LLM中表现出部分跨语言参数重叠，且英语参数集最大。
...
Method: 进行跨语言机制分析，定位和比较支持数学推理的模型参数，提取数学相关参数并比较跨语言重叠。

Result: 数学相关参数有部分跨语言重叠，主要集中在中间层；英语参数集最大，低资源语言参数集较小。

Conclusion: 数学行为既非完全语言不变也非完全语言特定，而是部分重叠且存在系统性语言依赖差异。
...
### [107] [Possible or Definite? A Benchmark for Evaluating Diagnostic Uncertainty Preservation in Clinical Text](https://arxiv.org/abs/2606.18471)
*Hongbo Du,Zixin Lu,Jiaming Qu*

Main category: cs.CL

TL;DR: LLMs在临床文本任务中难以保留诊断不确定性，经常改变不确定性表达，对临床意义有重大影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要评估LLMs生成文本的流畅性和连贯性，但忽略了它们是否正确保留了诊断不确定性，而这种不确定性对于临床决策至关重要。

Method: 构建包含1,200个临床文档和9,184个不确定性标注（五个级别）的基准，评估三个LLMs在保留不确定性方面的表现。

Result: LLMs保留原始不确定性线索的比例不足一半，且难以区分相邻不确定性级别之间的细微差别。

Conclusion: 揭示了标准评估指标未捕获的失败模式，为LLMs在临床工作流程中的安全部署提供了重要启示。
...
### [108] [PreUnlearn: Auditing Collateral Knowledge Damage Before Large Language Model Unlearning](https://arxiv.org/abs/2606.18473)
*Bo Su,Ankit Shah,Thai Le*

Main category: cs.CL

TL;DR: 从数据中心的视角研究LLM遗忘中附带损害的传播模式，发现衰减模式并通过遗忘集审计实现早期预警。


<details>
  <summary>Details</summary>
Motivation: 遗忘边界不清晰，相关知识甚至遥远信息在模型中纠缠，导致附带损害难以控制。

Method: 通过测量遗忘集到同域和远域知识的效应传播，并设计预遗忘预测任务分析数据特征，特别是交互特征。

Result: 附带损害随语义距离衰减但不消失，交互特征对下游损害预测力最强。

Conclusion: 遗忘集审计可作为早期预警工具，帮助识别风险遗忘运行并设计更可靠遗忘程序。
...
### [109] [Speech-Driven End-to-End Language Discrimination towards Chinese Dialects](https://arxiv.org/abs/2606.18584)
*Fan Xu,Jian Luo,MingWen Wang,GuoDong Zhou*

Main category: cs.CL

TL;DR: A speech-driven approach using MFCC features and an HMM-DNN model with attention and CNN achieves state-of-the-art fine-grained Chinese dialect discrimination.


<details>
  <summary>Details</summary>
Motivation: Traditional text-driven methods perform poorly for language discrimination among similar dialects, necessitating speech-driven features.

Method: First, explore speech-driven MFCC features for CNN-based discrimination; then, design an end-to-end HMM-DNN speech recognition model to predict dialect words with attention; finally, combine word embeddings and MFCC features via CNN.

Result: Evaluation on two benchmark Chinese dialect corpora demonstrates appropriateness and effectiveness, outperforming state-of-the-art methods.

Conclusion: The proposed speech-driven approach is effective and appropriate for fine-grained Chinese dialect discrimination.
...
### [110] [Low-resource Language Discrimination Towards Chinese Dialects with Transfer learning and Data Augmentation](https://arxiv.org/abs/2606.18597)
*Fan Xu,Yangjie Dan,Keyu Yan,Yong Ma,Mingwen Wang*

Main category: cs.CL

TL;DR: 提出一种结合迁移学习与数据增强的中文方言识别框架CDDTLDA。


<details>
  <summary>Details</summary>
Motivation: 中文方言识别因标注资源稀缺而面临挑战。

Method: 先在较大方言语料库上训练源ASR模型，再通过速度、音调、噪声扰动增强目标方言数据，基于源模型微调目标ASR模型，并用自注意力机制捕获共同语义特征，最后提取目标模型隐藏表示进行识别。

Result: 在两个基准中文方言语料库上显著优于现有方法。

Conclusion: CDDTLDA有效解决了低资源中文方言识别问题。
...
### [111] [Are LLMs Ready to Assist Physicians? PhysAssistBench for Interactive Doctor-Patient-EHR Assistance](https://arxiv.org/abs/2606.18613)
...
TL;DR: 提出PhysAssistBench基准，评估医学LLM在医生-患者-EHR交互中的协助能力，发现当前模型不可靠，需协调知识、沟通和系统。


<details>
  <summary>Details</summary>
Motivation: 现有评估测试孤立能力（临床知识、EHR交互、患者沟通），而实际协助需要这些能力的协调。

Method: 基于MIMIC-IV构建可交互的agentic患者，将静态记录转为多轮临床场景，创建1,296个双语评估回合。

Result: 领先LLM在该基准中表现不可靠，表明当前模型在协调能力上存在瓶颈。

Conclusion: 可靠协助需要跨知识、沟通和系统的协调，而非孤立提升任一能力。
...
### [112] [PragReST: Self-Reinforcing Counterfactual Reasoning for Pragmatic Language Understanding](https://arxiv.org/abs/2606.18624)
*Jihyung Park,Minchao Huang,Leqi Liu,Elias Stengel-Eskin*

Main category: cs.CL

TL;DR: PragReST improves LLM pragmatic reasoning via self-supervised counterfactual reasoning without human annotations or distillation.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with pragmatic inferences despite strong math/logic performance, often choosing literal interpretations.

Method: A self-supervised framework that constructs pragmatic QA data, generates counterfactual reasoning traces, and trains models with SFT and RL.

Result: Outperforms baselines on four benchmarks; improves Qwen3-8B/14B by 5.37% and 5.50% absolute accuracy; errors primarily due to lack of counterfactual reasoning.

Conclusion: Counterfactual reasoning is crucial for pragmatic inference; training preserves out-of-domain performance on general and math reasoning.
...
### [113] [PEC-Home: Interpretation of Progressively Elliptical Commands in Smart Homes](https://arxiv.org/abs/2606.18636)
*Yingyu Shan,Zeming Liu,Silin Li,Boao Qian,Jiashu Yao,Yuhang Guo,Haifeng Wang*

Main category: cs.CL

TL;DR: PEC-Home数据集揭示现有LLM在智能家居省略命令上的不足。


<details>
  <summary>Details</summary>
Motivation: 当前助手忽略人类对话中的渐进省略现象，导致对省略表达解释不准确，限制了实际应用。

Method: 构建PEC-Home模拟数据集，并在包括GPT-4o在内的多种LLM上进行实验评估。

Result: 现有助手在省略命令上执行准确率低，即使配备对话历史工具，仍不如完整命令。

Conclusion: 需要更好的方法来处理智能家居中的省略命令。
...
### [114] [The Wrong Kind of Right: Quantifying and Localizing Misfired Alignment in LLMs](https://arxiv.org/abs/2606.18656)
*Naihao Deng,Yiming Feng,Chimaobi Okite,Kaijian Zou,Lu Wang,Rada Mihalcea,Yulong Chen*

Main category: cs.CL

TL;DR: 对齐可能导致LLM忽略明确证据拒绝正确结论，称为misfired alignment，引入VETO基准和MAR指标展示该问题。


<details>
  <summary>Details</summary>
Motivation: 安全对齐可能导致模型过度泛化，拒绝有上下文支持的结论，需要量化该现象。

Method: 构建VETO基准（2032个对比对），定义MAR指标，测试25个LLM，进行控制启动实验和机制分析。

Result: 所有LLM有4.7-18.9% MAR，人类0%；安全启动放大MAR；后期层抑制证据答案，指令训练后出现。

Conclusion: 当前对齐方法过度泛化安全线索覆盖证据，需改进对齐目标以保持上下文基础。
...
### [115] [RegMix-D: Dynamic Data Mixing via Proxy Training Trajectories](https://arxiv.org/abs/2606.18663)
*Kaiyan Zhao,Zhongtao Miao,Akiko Aizawa,Yoshimasa Tsuruoka*

Main category: cs.CL

TL;DR: RegMix-D extends RegMix to dynamic data mixture selection by using full loss trajectories from proxy runs to predict optimal mixtures at multiple stages, improving over static mixtures.


<details>
  <summary>Details</summary>
Motivation: Existing data mixture methods like RegMix select a single static mixture, but proxy runs produce full loss trajectories that can be leveraged for better dynamic mixing.

Method: RegMix-D trains a regression model on loss trajectories from proxy runs to predict optimal mixtures at different training stages, with offline (pre-scheduled) and online (adaptive) deployment modes.

Result: On 25B tokens of Pile with a 1B model, RegMix-D improves over RegMix and DoReMi across 13 downstream tasks, and surpasses RegMix using only 128 proxy models (25% of RegMix's proxy compute).

Conclusion: RegMix-D achieves superior performance with high proxy efficiency, offering a practical dynamic mixture approach for LLM pretraining.
...
### [116] [TW-LegalBench: Measuring Taiwanese Legal Understanding](https://arxiv.org/abs/2606.18699)
*Fei-Yueh Chen,Chun Huang Lin,Chan Wei Hsu,Kuan Hsuan Yeh,Zih-Ching Chen,Kuan-Ming Chen,Patrick Chung-Chia Huang*

Main category: cs.CL

TL;DR: TW-LegalBench是评估LLM在台湾法律推理能力的新基准，包含多项任务，结果显示模型在律师资格考试中接近人类水平，但在法律判决预测和引用准确法律条文方面仍有困难。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注英语普通法和简体中文大陆法，而台湾法律体系未被充分评估，因此需要填补这一空白。

Method: 构建TW-LegalBench，包含超过16,000道多选题、117道开放作文题和超过14,000个法律判决预测实例。评估13个LLM，使用准确率、分解的LLM-as-Judge框架以及量刑准确性和法条引用指标。

Result: 顶级模型超过律师资格考试通过率（11%），但低于法官和检察官通过率（1-2%）。在LJP上，模型有合理的判决类型准确性和量刑预测能力，但难以引用确切法条。

Conclusion: 尽管模型在资格考试上接近人类水平，可靠的法律文本生成仍具挑战。
...
### [117] [LLMs Struggle to Measure What Distinguishes Students of Different Proficiency Levels: A Study of Item Discrimination in Reading Comprehension Assessment](https://arxiv.org/abs/2606.18709)
*Han Chen,Ming Li,Chenguang Wang,Yijun Liang,Dawei Zhou,Hong jiao,Tianyi Zhou*

Main category: cs.CL

TL;DR: 大型语言模型在捕捉项目区分度方面能力有限，与人类校准的区分度相关性较弱。


<details>
  <summary>Details</summary>
Motivation: 现有工作已经探讨了大型语言模型估计项目难度的能力，但不清楚它们是否能捕捉项目区分度。

Method: 我们采用两种互补方法在零样本设置下评估了42个专有和开源大型语言模型：直接区分度预测和基于响应的经典测试理论校准。

Result: 直接预测与人类校准区分度的最佳模型Spearman相关系数仅为0.152；基于响应的CTT校准中，全角色合成答题者池的Spearman相关系数为0.241。

Conclusion: 项目区分度对基于大型语言模型的心理测量评估仍是一个开放挑战；当前模型包含非随机的区分度相关信号，但尚未可靠地捕捉评估项目如何区分人类学生。
...
### [118] [LegalWorld: A Life-Cycle Interactive Environment for Legal Agents](https://arxiv.org/abs/2606.18728)
*Songhan Zuo,Shengbin Yue,Tao Chiang,Guanying Li,Yun Song,Xuanjing Huang,Zhongyu Wei*

Main category: cs.CL

TL;DR: LegalWorld 是一个模拟中国民事诉讼全生命周期的交互环境，用于评估法律代理在因果关联的各个阶段中的连贯能力。


<details>
  <summary>Details</summary>
Motivation: 现有的法律基准评估孤立子任务，先前的法律代理模拟器每次重新初始化场景，缺乏跨阶段因果依赖的建模。

Method: LegalWorld 将中国民事诉讼建模为五个阶段（七个子场景）的因果连接状态链，基于75,309对民事判决书，并配有可重用基础设施（本地内存、全局案例内存、技能/工具库）。在此基础上构建 LongJud-Bench 以评估代理在五个连续阶段中的能力。

Result: 来自217名法律背景评估者的18,992次评分证实了 LegalWorld 轨迹的程序忠实性和角色一致性；跨模型评估揭示了聚合分数无法暴露的尖锐分歧，没有单一骨干在咨询、起草和法庭辩护中全面领先。

Conclusion: 该环境能有效模拟生命周期依赖关系并揭示模型能力差异；详细资源将公开发布。
...
TL;DR: 介绍RedactionBench基准和R-Score指标，评估多种模型在上下文PII删节上的表现，发现上下文删节仍是未解决问题，人类在上下文删节上共识低。


<details>
  <summary>Details</summary>
Motivation: 现有基准混淆了PII提取与隐私语义，忽视上下文完整性，需要新的基准来区分删节与实体识别。

Method: 构建包含200个文档11个领域的手动注释基准RedactionBench，提出字符级指标R-Score，评估NER模型、小语言模型和带工具的前沿模型，并进行80多用户的人类评估。

Result: 人类在强制性删节和保留上共识高（89.4%和94.1%），但在上下文删节上共识低（47.7%）；35个模型比较显示上下文删节仍是挑战。

Conclusion: RedactionBench为隐私保护系统建立基线，R-Score解耦上下文歧义与严格精度，推动标准化评估和高效模型设计。
...
### [120] [Learning Robust Pair Confidence for Multimodal Emotion-Cause Pair Extraction](https://arxiv.org/abs/2606.18893)
...
### [121] [SAGE: Stochastic Prompt Optimization via Agent-Guided Exploration](https://arxiv.org/abs/2606.18902)
*Ziyi Zhu,Luka Smyth,Saki Shinoda,Jinghong Chen*

Main category: cs.CL

TL;DR: SPO是一个随机提示优化框架，比较了三种策略，SAGE通过多智能体诊断代码执行在心理健康聊天机器人中显著提升次日留存。


<details>
  <summary>Details</summary>
Motivation: 上下文工程是改进AI系统的主要手段，但文本梯度并非真实梯度，因此自动提示优化需被视为黑盒搜索。

Method: 提出SPO框架，包含错误信息随机搜索、遗传算法和SAGE（多智能体引导探索）三种策略。

Result: 在三个基准上无单一策略占优；SAGE在心理健康聊天机器人上经8轮A/B测试累积出统计稳健的次日留存提升。

Conclusion: 定性诊断与定量验证相结合是使智能体优化对开放任务对话有效的关键。
...
### [122] [As Easy as Rocket Science: Assessing the Ability of Large Language Models to Interpret Negation in Figurative Language](https://arxiv.org/abs/2606.18922)
*Jasmine Owers,Edwin Simpson,Martha Lewis*

Main category: cs.CL

TL;DR: 该研究通过新增注释测试了语言模型在否定和比喻语言组合上的表现，发现提示风格对性能影响很大。


<details>
  <summary>Details</summary>
Motivation: 比喻语言和否定是当前语言模型的挑战，且LLMs广泛使用且无法微调，因此需要理解它们正确解释包含否定和比喻语言文本的能力。

Method: 对现有比喻语言数据集新增否定注释，并在一系列语言模型上进行测试。

Result: 否定和比喻的结合特别具有挑战性，整体性能和不同否定类型的性能高度依赖于提示风格。

Conclusion: 处理否定和比喻语言时，需要谨慎选择提示风格以获得更好的模型表现。
...
### [123] [SenFlow: Inter-Sentence Flow Modeling for AI-Generated Text Detection in Hybrid Documents](https://arxiv.org/abs/2606.18946)
*Jingkun Luo,Yifan Sun,Da-Tian Peng,Guanxiong Pei*

Main category: cs.CL

TL;DR: 提出MOSAIC基准和SenFlow方法，用于混合文档中句子级AI生成文本检测，在跨域迁移上提升4.15个百分点的Macro-F1。


<details>
  <summary>Details</summary>
Motivation: 现有方法孤立分类句子，忽略句间依赖；现有基准缺少最新生成器。

Method: 构建MOSAIC基准，包含16,000混合文档，采用困惑度一致性过滤；提出SenFlow，将S-AGTD重构为结构化预测，整合基于图的句间传播和线性链CRF解码。

Result: 在MOSAIC上达到SOTA，跨域迁移Macro-F1平均提升4.15个百分点；即使经过困惑度过滤，AI插入仍存在句子长度差距，被句子级检测器利用。

Conclusion: 结构化预测方法有效利用句间依赖，显著提升混合文档检测性能，并揭示了隐藏的句子长度线索。
...
### [124] [GraphPO: Graph-based Policy Optimization for Reasoning Models](https://arxiv.org/abs/2606.18954)
*Yuliang Zhan,Xinyu Tang,Jian Li,Dandan Zheng,Weilong Chai,Jingdong Chen,Jun Zhou,Ge Wu,Wenyue Tang,Hao Sun*

Main category: cs.CL

TL;DR: 提出GraphPO，一种基于图的策略优化框架，通过将强化学习轨迹表示为有向无环图，合并语义等价路径并分配效率与正确性优势，减少冗余探索和估计方差，提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法独立采样导致冗余探索和计算浪费，稀疏奖励难以识别有用步骤；树方法虽改进但分支仍独立扩展且局部比较导致方差高。

Method: GraphPO将rollouts表示为有向无环图，以推理步骤为边、语义状态为节点；合并语义等价路径为等价类以共享后缀和重分配预算；对入边分配效率优势、出边分配正确性优势，从而从最终结果导出过程监督。

Result: 在三个LLM上，针对推理和智能体搜索基准，GraphPO在相同token或响应预算下持续优于链和树基线。

Conclusion: 理论分析表明GraphPO降低优势估计方差并提高推理效率，实验验证其有效性。
...
### [125] [G-IdiomAlign: A Gloss-Pivoted Benchmark for Cross-Lingual Idiom Alignment](https://arxiv.org/abs/2606.18989)
...
### [126] [Enhancing Multilingual Reasoning via Steerable Model Merging](https://arxiv.org/abs/2606.19002)
...
TL;DR: 提出ST-Merge框架，通过门控交叉注意力机制自适应调节源模型贡献，提升多语言推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法无法解决源模型之间的冲突，导致次优性能。

Method: 提出Steerable Model Merging (ST-Merge)框架，引入门控交叉注意力机制自适应加权或过滤源模型。

Result: 在21种语言的四个多语言推理基准上，ST-Merge持续优于多个强基线。

Conclusion: 门控交叉注意力有效缓解源模型冲突，实现更好的多语言推理性能。
...
### [127] [Sumi: Open Uniform Diffusion Language Model from Scratch](https://arxiv.org/abs/2606.19005)
*Mengyu Ye,Keito Kudo,Wataru Ikeda,Ryosuke Matsuda,Keisuke Sakaguchi,Jun Suzuki*

Main category: cs.CL

TL;DR: Sumi是一个7B参数的均匀扩散语言模型，从零预训练于1.5T tokens，在知识、推理和编码任务上与自回归模型竞争力相当，但在常识任务上表现较差。


<details>
  <summary>Details</summary>
Motivation: 缺乏大规模预训练的均匀扩散语言模型（UDLM）作为对比基准，而自回归和掩码扩散模型已有此类模型，因此需要从零预训练一个UDLM来研究其扩展行为、生成动态和可控性。

Method: 从零预训练一个7B参数的均匀扩散语言模型Sumi，使用1.5T tokens，数据混合侧重教育性文本，并完全开源模型权重、检查点和训练配方。

Result: 在知识、推理和编码基准上表现与类似token预算的自回归模型相当，但在常识基准上表现较差，可能归因于教育密集型数据混合。

Conclusion: 希望Sumi的发布能够使社区研究原生均匀扩散在规模上的特性，并促进对其尚未充分理解方面的研究。
...
### [128] [Trade-offs in Medical LLM Adaptation: An Empirical Study in French QA](https://arxiv.org/abs/2606.19266)
*Ikram Belmadani,Oumaima El Khettari,Carlos Ramisch,Frederic Bechet,Richard Dufour,Benoit Favre*

Main category: cs.CL

TL;DR: 在法语医学问答中比较持续预训练(CPT)、监督微调(SFT)及其组合，发现SFT对多项选择问答成本效益高，而CPT改善开放式问答生成质量。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在特定领域和语言上的适应策略效果，尤其是法语医学问答中，领域适应策略的有效性尚不明确。

Method: 以法语医学问答为案例，对比CPT、SFT及两者组合，在三个模型家族、多种大小和三种初始化下，评估多项选择(MCQA)和开放式(OEQA)问答，使用自动指标和LLM评判。

Result: MCQA中CPT+SFT最优但增益小且常不显著；OEQA中CPT提升重叠指标，SFT常降低生成质量；指令微调和CPT+SFT在LLM评估中更优；跨语言转移有效。

Conclusion: 提供在计算约束下选择适应策略的实用指南，推荐SFT作为MCQA的默认选择，CPT用于OEQA。
...
### [129] [Learning User Simulators with Turing Rewards](https://arxiv.org/abs/2606.19336)
*Yingshan Susan Wang,Cedegao E. Zhang,Linlu Qiu,Zexue He,Pengyuan Li,Alex Pentland,Roger P. Levy,Yoon Kim*

Main category: cs.CL

TL;DR: Turing-RL is a reinforcement learning approach using a Turing Test-based reward to train user simulator LLMs to generate responses indistinguishable from real users, outperforming baseline methods.


<details>
  <summary>Details</summary>
Motivation: Existing user simulators train LLMs to match a single ground truth response, which may not capture the diversity of real user behavior. There is a need for more realistic and indistinguishable responses.

Method: Turing-RL uses a discriminative Turing reward with an LLM judge to evaluate how indistinguishable the generated response is from the real user's response given history, and the user simulator LLM learns via RL to maximize this reward.
...
### [130] [Redact or Keep? A Fully Local AI Cascade for Educational Dialogue De-Identification](https://arxiv.org/abs/2606.18372)
*Haocheng Zhang,Zhuqian Zhou,Kirk Vanacore,Bakhtawar Ahtisham,René F. Kizilcec*

Main category: cs.CL

TL;DR: 提出全本地级联框架，将去标识重定义为约束性隐私分类，在数学辅导对话上达0.958 F1，优于LLM基线和商业API。


<details>
  <summary>Details</summary>
Motivation: 现有方法在治理与准确性间权衡：商业LLM需外传数据，本地NER系统过度删除课程术语。

Method: 级联框架：召回优先联合提议器结合轻量编码器与规则生成候选，上下文感知审查器二值化决策（Redact/Keep）。

Result: 最强本地配置达0.958 F1（LLM基线0.767，商业API 0.706），全在单笔记本运行；模糊性测试仅降0.03 F1。

Conclusion: 教育去标识中，问题表述比模型规模更关键。
...
### [131] [Want Better Synthetic Data? Steer It: Activation Steering for Low-Resource Language Generation](https://arxiv.org/abs/2606.18389)
*Jan Cegin,Daniil Gurgurov,Yusser Al Ghussin,Simon Ostermann*

Main category: cs.CL

TL;DR: 本文研究了激活引导作为低资源语言合成数据生成的替代方法，并展示了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前few-shot prompting方法成本高且降低多样性，因此探索激活引导。

Method: 提出Language Steering和Quality Steering两种策略，在多个LLM和语言上评估，生成数据并微调分类器。

Result: 早期层引导提高多样性，提升下游性能，尤其是低资源语言。

Conclusion: 激活引导是合成数据生成的有效替代方案。
...
### [132] [Towards Scalable Customization and Deployment of Multi-Agent Systems for Enterprise Applications](https://arxiv.org/abs/2606.18502)
...
TL;DR: 提出一个两阶段框架，用于定制和高效部署基于大语言模型的多智能体系统，实现了快速领域适配和4.48倍的吞吐量加速。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的多智能体系统虽性能强大，但生产部署面临领域定制需求和高延迟、高推理成本等挑战。

Method: 第一阶段进行智能体模型定制，结合连续预训练、监督微调和偏好优化，使紧凑模型适应专业领域；第二阶段进行推理优化，集成投机解码和FP8量化与目标校准，实现成本高效且质量损失最小的服务。

Result: 在企业工作负载上，实现快速领域适配，吞吐量提升4.48倍，同时保持性能并提升长尾场景的鲁棒性。

Conclusion: 该框架有效解决了生产部署中的定制化和效率问题，展示了在实际多智能体系统中的实用价值。
...
### [133] [Dual Dimensionality for Local and Global Attention](https://arxiv.org/abs/2606.18587)
*Zhiyuan Wang,Xuan Luo,Sirui Zeng,Xifeng Yan*

Main category: cs.CL

TL;DR: 距离自适应表示（DAR）对远距离token降低KV缓存维度，局部token保持全维度，在匹配全维性能的同时减少内存。


<details>
  <summary>Details</summary>
Motivation: 自然语言中局部token对预测更重要，而远距离token作为长期记忆，因此假设它们对表示维度的需求不对称。

Method: 提出距离自适应表示（DAR），在局部上下文窗口内保留全维表示，超出窗口的token使用降维表示（如1/4原始维度）。

Result: 在70M到410M参数预训练及1B参数继续微调下，DAR性能接近全维基线，而统一降维则性能更差。

Conclusion: 挑战了KV维度应均匀的假设，表明自适应分配表示能力可进一步减少推理时KV缓存。
...
### [134] [BCL: Bayesian In-Context Learning Framework for Information Extraction](https://arxiv.org/abs/2606.18620)
*Haoliang Liu,Chengkun Cai,Xu Zhao,Han Zhu,Shizhou Huang,Xinglin Zhang,Tao Chen,Jenq-Neng Hwang,Zhang Huaping,Lei Li*

Main category: cs.CL

TL;DR: BCL是一个使用贝叶斯更新和粒子滤波的上下文学习框架，用于信息抽取，通过四个步骤系统优化标签表示，在序列标注和关系分类上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有ICL方法在不同模型规模上表现不一致，且缺乏系统优化和泛化能力。

Method: 提出BCL框架，使用粒子滤波和贝叶斯更新，通过初始化、观察、权重更新和重采样四个步骤优化标签表示，适用于序列标注和关系分类。

Result: 大量实验表明，BCL在信息抽取任务上相比现有方法取得了显著且一致的改进。

Conclusion: BCL是一个有效的、泛化的ICL优化框架，能够显著提升信息抽取性能。
...
### [135] [Output Vector Editing for Memorization Mitigation in Large Language Models](https://arxiv.org/abs/2606.18767)
*Ahmad Dawar Hakimi,Kaiwei Lei,Isabelle Augenstein,Hinrich Schütze*

Main category: cs.CL

TL;DR: 提出输出向量编辑方法，通过优化修改MLP神经元输出向量引入干扰，抑制大语言模型中的记忆序列，实现高成功率且避免灾难性局部失败。


<details>
  <summary>Details</summary>
Motivation: 大语言模型记忆和复制训练数据带来隐私、版权和安全风险；现有神经元级方法仅归零激活，忽略了输出向量对残差流的直接影响。

Method: 定位负责记忆续写的MLP神经元，通过约束优化最小改变其输出向量，在词汇空间引入干扰，保持激活不变；在OLMo-7B等四个模型上挖掘6831个记忆序列进行评估。

Result: 最高抑制率87.9%，比零消融提升2.7倍；四种编辑模式覆盖96.5%序列，推荐单模式达81.5%且无灾难性失败；约14%序列需注意头辅助恢复。

Conclusion: 输出向量编辑有效抑制记忆且转移性好，注意头可作为互补机制，成功率随模型规模增加。
...
### [136] [Approximate Structured Diffusion for Sequence Labelling](https://arxiv.org/abs/2606.18856)
*Nicolas Floquet,Joseph Le Roux,Nadi Tomeh*

Main category: cs.CL

TL;DR: 利用扩散训练以整个标签序列为条件的CRF，结合近似推理，在词性标注中错误率降低16.5%。


<details>
  <summary>Details</summary>
Motivation: 传统CRF假设有限决策跨度（如标签bigrams），难以捕获长距离依赖，限制表达力并损害性能。

Method: 使用扩散训练条件CRF，条件为整个标签序列的噪声版本，并采用近似CRF推理。

Result: 在词性标注任务上实现16.5%的错误率降低。

Conclusion: 该方法有效提升序列标注性能，尤其对长距离依赖场景。
...
### [137] [Efficient Financial Language Understanding via Distillation with Synthetic Data](https://arxiv.org/abs/2606.18875)
*Wen-Fong,Huang,Edwin Simpson*

Main category: cs.CL

TL;DR: 通过聚类种子选择进行合成数据蒸馏，实现金融情感分析的高效模型压缩和领域适应。


<details>
  <summary>Details</summary>
Motivation: 大型指令遵循模型部署成本高，金融领域标注数据有限且获取昂贵，需要低资源条件下的高效情感分析。

Method: 收集少量手工标注示例，聚类后选择种子，通过结构化少样本提示生成合成数据，再利用蒸馏将教师知识迁移到紧凑学生模型。

Result: 聚类种子选择比随机采样产生更具代表性的合成数据，紧凑模型在复杂嘈杂文本上超越教师模型，在正式文本上保持竞争力。

Conclusion: 该框架为金融NLP中资源高效的领域适应提供了实用路径，显著减少了人工标注努力。
...
### [138] [Improving Medical Communication using Rubric-Guided Counterfactual Recommendations](https://arxiv.org/abs/2606.18889)
*Adrian Cosma,Nicoleta-Nina Basoc,Andrei Niculae,Cosmin Dumitrache,Emilian Radoi*

Main category: cs.CL

TL;DR: 提出LM引导的反事实推荐管道，通过优化可解释沟通特征提升远程医疗患者反馈，在独立审计下平均增益+6.41%，93.31%推荐非负。


<details>
  <summary>Details</summary>
Motivation: 文本远程医疗中患者反馈主要反映感知沟通质量而非医疗准确性，需要在不干扰医疗内容的前提下改善沟通以提升反馈。

Method: 利用语言模型发现并优化语气、个性化、可操作性和完整性等可解释沟通特征，结合医患交互元数据估计正面反馈；推理时搜索最小序数特征变化，推荐能增加正面反馈概率的沟通调整，并由独立审计模型验证泛化性。

Result: 跨交互推荐在独立审计下平均正面反馈概率增益+6.41%，93.31%的推荐非负。

Conclusion: 小的、可解释的沟通变化能捕获大部分预测增益，同时保留医生对医学推理和最终措辞的控制。
...
### [139] [Leadership as Coordination Control: Behavioral Signatures and the Recovery-Advantage Boundary in Multi-Agent LLM Teams](https://arxiv.org/abs/2606.19111)
*Haewoon Kwak*

Main category: cs.CL

TL;DR: 过程级协调控制仅在初始多数投票不可靠、任务可恢复且无指导交互未修复时才提升性能，与团队科学的权变理论预测一致。


<details>
  <summary>Details</summary>
Motivation: 类比团队科学中领导力的权变性，探究多智能体LLM团队中过程级协调控制增加价值的可测量条件。

Method: 定义行为特征和消融实验，将三种领导风格（交易型、变革型、情境型）作为控制器，共享动作词汇，在四个任务、三种模型上比较，并设计边界探测测试。

Result: 无控制器在准确性上占主导；交易型与初始投票差异≤1.3pp；仅在初始多数不可靠（llama-4-scout社会任务）时情境型控制器获+8pp增益；恢复优势账户预测控制器仅在特定条件下有效。

Conclusion: 零准确性结果符合权变理论预测，过程级协调控制应被视为需测量和理论映射的权变因素，而非追求排行榜表现。
...
### [140] [Language Models as Interfaces, Not Oracles: A Hybrid LLM-ML System for Pediatric Appendicitis](https://arxiv.org/abs/2606.19183)
*Soheyl Bateni,Maryam Abdolali*

Main category: cs.CL

TL;DR: ClaMPAPP hybrid system using LLM for feature extraction and XGBoost classifier for appendicitis diagnosis outperforms end-to-end LLMs, providing stable and auditable clinical decision support.


<details>
  <summary>Details</summary>
Motivation: LLMs are sensitive to prompts and produce plausible but incorrect outputs, while structured ML models require tabular inputs difficult to integrate with narrative workflows.

Method: ClaMPAPP uses an LLM to extract schema-constrained clinical features from narrative notes, applies deterministic plausibility checks, then passes validated features to an XGBoost classifier trained on clinical, laboratory, and ultrasound variables.

Result: ClaMPAPP achieved the strongest overall diagnostic performance in internal and external validation, minimizing missed appendicitis cases; end-to-end LLMs showed unstable sensitivity-specificity trade-offs and degradation under narrative reordering.

Conclusion: Supporting an LLM-as-interface, ML-as-predictor design that separates natural-language usability from predictive inference, providing a more auditable pathway for clinical decision support.
...
### [141] [Enhancing Decision-Making with Large Language Models through Multi-Agent Fictitious Play](https://arxiv.org/abs/2606.19308)
*Leyang Shen,Yang Zhang,Xiaoyan Zhao,Chun Kai Ling,Tat-Seng Chua*

Main category: cs.CL

TL;DR: 针对多智能体系统在决策任务中的立场纠缠问题，提出基于虚拟博弈的迭代框架MAFP，通过最佳响应历史混合策略提升决策质量与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM多智能体系统擅长处理执行复杂性任务，但无法应对决策任务中涉及多利益相关者立场相互依赖的立场纠缠问题。

Method: 提出多智能体虚拟博弈（MAFP），将利益相关者立场表示为智能体，通过迭代最佳响应其他智能体历史决策的经验混合来逼近均衡。

Result: 在竞争性策略决策任务中，MAFP在锦标赛强度和鲁棒性两个互补指标上均优于单轮和多轮基线方法。

Conclusion: MAFP通过博弈论视角有效解决了立场纠缠，显著提升了多智能体决策性能。
...
TL;DR: 提出人类-AI协同进化动力学框架(HACD-H)，用于建模长期人机交互中社会智能的出现。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过孤立组件建模社会行为，缺乏统一框架解释稳定社会关系和社会智能的出现。

Method: 构建HACD-H框架，整合情感适应、关系组织、社会记忆和人格一致性，引入多时间尺度社会认知等原则，并构建约14,700交互轮次的对话数据集进行理论驱动评估。

Result: 揭示社会认知的时间持久性层次、稳定关系吸引子、发展相变模式及结构化能量景观；社会智能与社会认知能量负相关(r=-0.391, p<0.001)，交互轨迹能量递减。

Conclusion: 社会智能源于长期社会认知协同进化，HACD-H为建模适应性人机社会交互提供统一理论基础。
...
### [143] [WorldLines: Benchmarking and Modeling Long-Horizon Stateful Embodied Agents](https://arxiv.org/abs/2606.18847)
*Yehang Zhang,Jianchong Su,Haojian Huang,Yifan Chang,Tianhao Zhou,Xinli Xu,Yingjie Xu,Yinchuan Li,Zexi Li,Ying-Cong Chen*

Main category: cs.AI

TL;DR: 提出了WorldLines基准和ObsMem框架，用于评估长期具身家庭助手在动态环境中的记忆和规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有长期记忆基准主要评估语言检索和问答，具身基准则聚焦短时间任务，缺乏对动态环境中长期记忆使用的测试。

Method: 构建WorldLines，包含长时间家庭轨迹（对话、动作、反馈等），转换为Memory QA和Embodied Task Planning样本；提出ObsMem框架，维护可见性感知记忆和动作原生状态轨迹。

Result: 实验揭示了部分可观察性、世界状态覆盖等挑战，ObsMem提供了更强的参考架构。

Conclusion: ObsMem是更优的架构，但仍需解决长期记忆用于具身规划中的难题。
...
### [144] [CaVe-VLM-CoT: An Interpretable Vision-Language Model Framework](https://arxiv.org/abs/2606.18385)
*Sneha Rao,Shaina Raza,Dhanesh Ramachandram*

Main category: cs.AI

TL;DR: 提出CaVe-VLM-CoT，一个基于反思的agentic-RAG框架，通过五阶段闭环管道强制执行基于证据的推理，并引入CaVeScore指标，在ScienceQA和MMMU上取得改进。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型容易产生幻觉，现有方法没有强制步骤级引用或路由验证失败回退到检索。

Method: 五阶段闭环管道：Extractor、Retriever、Solver、Citation Injector、Verifier，未接地声明触发结构化反馈以进行针对性重新检索，并引入23个组件级指标（含CaVeScore）。

Result: ScienceQA上准确率87.1%，CaVeScore 56.6%；MMMU上准确率55.2%，CaVeScore 35.7%。

Conclusion: 无需架构或提示修改即可显著提升性能，表明证据接地和反馈循环的有效性。
...
### [145] [Decoupling Search from Reasoning: A Vendor-Agnostic Grounding Architecture for LLM Agents](https://arxiv.org/abs/2606.18947)
*Emmanuel Aboah Boateng,Kyle MacDonald,Amardeep Kumar,Siddharth Kodwani,Sudeep Das*

Main category: cs.AI

TL;DR: DSG将搜索接地从推理模型中解耦，通过MCP兼容网关提供可配置控制，在保持准确性的同时大幅降低成本和延迟。


<details>
  <summary>Details</summary>
Motivation: 原生搜索接地将检索策略、提供商选择、证据注入、成本、延迟和生成行为捆绑在一起，难以检查、调优、复用或移植，并可能引发搜索诱导的冗长问题。

Method: 提出Decoupled Search Grounding (DSG)，一个与供应商无关的边界，通过MCP兼容网关将接地移出推理模型，暴露提供商路由、源感知上下文渲染、配置回退、检索深度控制以及精确和语义缓存作为一等控制。

Result: 在SimpleQA上DSG准确率86.1% vs 原生87.7%，但搜索成本降低91%，延迟降低68%，缓存命中率99.4%；在e-commerce QIU任务上搜索成本降低超98%，准确性匹配或略超原生。

Conclusion: 实时接地应被视为可优化的接口边界，而不是固定的模型特征。
...
### [146] [User as Engram: Internalizing Per-User Memory as Local Parametric Edits](https://arxiv.org/abs/2606.19172)
*Bojie Li*

Main category: cs.AI

TL;DR: User as Engram通过将用户内容存储为对Engram模型哈希键控记忆表的精确编辑，推理技能放在共享适配器中，实现了与大脑相似的内容-技能分离，比每用户LoRA在间接推理准确率上平均提高5.6倍，内存占用小33,000倍，且不降低推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有每用户LoRA将内容和技能混合在一个全局权重增量中，污染无关文本，而大脑将内容（海马体）和技能（新皮层）分离。需要一种个性化方法，既能直接回忆事实，又不会干扰其他文本或推理技能。

Method: 使用Engram模型，将用户内容作为精确编辑写入哈希键控记忆表，每个事实在触发时激活查找并添加答案值；推理技能则通过一个共享的LoRA适配器编码。编辑是‘玻璃盒’：精确触发、仅改变目标位置，且在不同层失败时停止。不同用户的事实落在不同哈希槽，可无损叠加。

Result: 相比每用户LoRA，间接推理准确率平均提升5.6倍，内存占用降低约33,000倍，且从未使单个用户的推理能力低于未修改的基础模型。当事实数超过约100个时，性能超越使用2.5倍规模模型的检索流水线。

Conclusion: User as Engram是一种高效、可扩展的个性化方法，通过分离内容与技能，避免了LoRA的污染问题，同时保持了直接回忆能力，并大幅提升了间接推理和内存效率。
...
TL;DR: DeFAb is a benchmark and generation pipeline for defeasible abduction, testing logical rigor with verifiable hypotheses; frontier models perform poorly, especially under rendering-robust evaluation.


<details>
  <summary>Details</summary>
Motivation: Current language models lack rigorous defeasible reasoning; existing benchmarks fail to enforce logical consistency. DeFAb uses publicly funded knowledge bases to formally ground instances.

Method: Pipeline combining taxonomic hierarchies (OpenCyc, YAGO, Wikidata) with behavioral property graphs (ConceptNet, UMLS) to generate 372,648+ instances across 33.75M materialized rules from 18 sources, with polynomial-time verifiable gold standards.

Result: Symbolic solver achieves 100% accuracy in <50μs; best frontier model 65% dropping to 23.5% under robust evaluation. Chain-of-thought variance exceeds inter-model gaps. Contamination control shows +19.4 pp gap on Level 3.

Conclusion: Defeasible reasoning is not reliably internalized by frontier models. DeFAb provides a rigorous instrument for measuring creativity and theory revision, and the verifier can serve as reward for preference optimization.
...
### [148] [R2D-RL: A RoboCup 2D Soccer Environment for Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2606.18786)
*Haobin Qin,Baofeng Zhang,Hidehisa Akiyama,Keisuke Fujii*

Main category: cs.AI

TL;DR: R2D-RL是一个将RCSS2D和HELIOS客户端连接到Python多智能体强化学习接口的环境。


<details>
  <summary>Details</summary>
Motivation: 机器人足球是多智能体强化学习的挑战性测试床，但现有竞赛导向的服务器-客户端架构难以与现代Python MARL工作流直接使用。

Method: 通过共享内存通信和周期级同步将RCSS2D和基于HELIOS的玩家客户端连接到Python MARL接口，支持全场和场景训练、可配置对手、动作空间、动作掩码、EPV奖励塑造和并行执行。

Result: 提供了前球门场景和11对11全场基准测试，以及基线结果。

Conclusion: R2D-RL为机器人足球领域提供了一个易于使用的MARL研究平台。
...
### [149] [SciRisk-Bench: A Risk-Dimension-Aware Benchmark for AI4Science Safety](https://arxiv.org/abs/2606.18936)
...
TL;DR: SciRisk-Bench is a benchmark for evaluating AI4Science safety across 7 disciplines, 31 subdisciplines and 10 risk dimensions, enabling fine-grained diagnosis of unsafe scientific models.


<details>
  <summary>Details</summary>
Motivation: There is an urgent need for safety benchmarks that evaluate not only scientific competence but also risk recognition in high-stakes scientific contexts, and existing datasets are underspecified in risk dimensions.

Method: SciRisk-Bench covers 7 disciplines, 31 subdisciplines and 10 risk dimensions. We evaluate both mainstream LLMs and science-oriented LLMs across risk dimensions, disciplines, and sub-disciplines.

Result: The evaluation enables fine-grained diagnosis of where scientific models remain unsafe.

Conclusion: SciRisk-Bench allows fine-grained diagnosis of safety issues in LLMs for science, highlighting areas where models are still unsafe.
...
### [150] [ThinkDeception: A Progressive Reinforcement Learning Framework for Interpretable Multimodal Deception Detection](https://arxiv.org/abs/2606.18988)
*Jinhao Song,Shan Liang,Yiqun Yue,Zhuhuayang Zhang,Tianqi Gao*

Main category: cs.AI

TL;DR: 提出ThinkDeception框架，利用多模态大语言模型和渐进式训练策略，将欺骗检测转变为可解释的认知推理过程，显著提升检测准确性和推理质量。


<details>
  <summary>Details</summary>
Motivation: 现有多模态欺骗检测方法缺乏可解释性，无法提供透明推理过程，难以捕捉跨模态不一致性。

Method: 引入多模态大语言模型构建认知推理框架，创建多模态思维链数据集，提出视觉-音频一致性组相对策略优化（VAC-GRPO）及渐进式训练策略。

Result: 在主流基准上达到新最优，检测准确性和推理质量显著优于现有方法。

Conclusion: 推动了欺骗检测向可解释的多模态认知推理发展。
...
TL;DR: 首次在轨实现视觉-语言模型自主多模态推理，通过语义压缩提升卫星数据利用效率。


<details>
  <summary>Details</summary>
Motivation: 地球观测数据生成远超下行带宽和人工处理能力，导致星上数据采集与地面情报之间存在鸿沟。

Method: 在LEO航天器部署NAVI-Orbital系统，利用Gemma 3视觉语言模型进行场景分类和描述，通过LangGraph图状态机协调检测与对话代理，支持自然语言重任务。

Result: 地面基准测试达88.16%准确率，经平板卫星验证及在轨实测（含未校正YAM-9图像，硬件加速GPU推理，无需微调）验证可行性。

Conclusion: 在卫星级边缘计算机上运行基础模型可行，通过语义压缩逆转变传统采集-全量下发带宽模式。
...
### [152] [ForecastBench-Sim: A Simulated-World Forecasting Benchmark](https://arxiv.org/abs/2606.18686)
*Jaeho Lee,Nick Merrill,Ezra Karger*

Main category: cs.AI

TL;DR: Introduces ForecastBench-Sim, a simulated-world forecasting benchmark built on Freeciv game rollouts to complement real-world benchmarks with controlled, immediately resolvable tasks.


<details>
  <summary>Details</summary>
Motivation: Real-world forecasting benchmarks have slow resolution, rare tail events, and difficulty scoring counterfactuals; a simulated world can overcome these limitations.

Method: Uses Freeciv game rollouts; forecasters receive a world report and answer questions about hidden future states, then the simulation continues for scoring. Supports continuous/binary questions, arbitrary time horizons, intervention worlds for causal queries, and rare outcomes.

Result: Describes the benchmark pipeline, question families, scoring protocol, release artifacts, and reports validation slices from model evaluations and an anonymized human pilot.

Conclusion: ForecastBench-Sim is intended to complement real-world benchmarks by providing controlled, immediately resolvable tasks for studying probabilistic reasoning under dynamic world states.
...
TL;DR: RTSGameBench is a benchmark for evaluating VLM strategic reasoning in RTS games, revealing that current VLMs struggle with coordination and scalability.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs lack strategic reasoning in competitive/cooperative settings, and current RTS benchmarks have limited scope, lack diagnostic capability, and are not extensible.

Method: We build RTSGameBench on Beyond All Reason with diverse evaluations, diagnostic mini-games, and a self-evolving generation framework. We also provide RTSGameAgent using FSM and agentic memory.

Result: State-of-the-art VLMs perform poorly in matchups requiring tighter coordination, multiagent coordination, and increased task scale.

Conclusion: RTSGameBench effectively diagnoses strategic reasoning limitations in VLMs, highlighting the need for improvement in coordination and scalability.
...
### [154] [RODS: Reward-Driven Online Data Synthesis for Multi-Turn Tool-Use Agents](https://arxiv.org/abs/2606.19047)
*Ruishan Fang,Siyuan Lu,Chenyi Zhuang,Tao Lin*

Main category: cs.AI

TL;DR: RODS通过动态合成边界样本，解决了多轮工具使用强化学习中静态数据集信息样本快速耗尽的问题。


<details>
  <summary>Details</summary>
Motivation: 多轮工具使用强化学习受限于静态数据集中信息样本的快速耗尽，且梯度信号集中在智能体能力边界附近的样本。

Method: RODS利用进度奖励方差作为零成本边界检测器，通过技能对齐的重采样流程合成与当前边界样本结构复杂性匹配的多轮变体，并维护与策略共同演化的动态回放缓冲区。

Result: 从400个人工种子开始并维持约800个活跃训练样本，RODS达到了与17K样本离线管道相当的性能，需要大约20倍更少的轨迹，且在控制设置中优于固定数据RL和环境增强。

Conclusion: RODS通过在线数据合成有效缓解了数据耗尽问题，显著提高了数据效率。
...
### [155] [TxBench-PP: Analyzing AI Agent Performance on Small-Molecule Preclinical Pharmacology](https://arxiv.org/abs/2606.19245)
*Hannah Le,Ramesh Ramasamy,Alex Urrutia,Mahsa Yazdani,Tim Proctor,Kenny Workman*

Main category: cs.AI

TL;DR: 提出TxBench-PP基准测试评估AI代理在临床前药理学决策上的表现，最佳模型仅通过59.3%。


<details>
  <summary>Details</summary>
Motivation: AI代理有望加速药物发现，但实际部署需要可信评估，现有基准缺乏真实性。

Method: 构建包含100个评估的基准，涵盖MoA、PD等任务，代理处理工作流快照并返回可验证答案。

Result: 16个模型配置中，无系统可靠恢复决策，最佳Claude Opus 4.8/Pi通过59.3%。

Conclusion: 当前AI系统在临床前药理学决策上表现不可靠，需要进一步改进。
...
TL;DR: X+Slides是一个针对受众条件幻灯片生成的基准，通过源接地探针和动态评估框架，揭示现有系统虽能覆盖部分受众必要信息，但视觉质量和广泛主题覆盖不能替代源接地评估。


<details>
  <summary>Details</summary>
Motivation: 现有幻灯片生成基准忽略目标受众这一关键真实世界因素，不同受众（如专家和决策者）需求不同，需要针对性的评估标准。

Method: 构建涵盖113个主题和七种演示场景的语料库，设计8133个去重源接地探针，并通过分配受众特定效用权重，计算四个互补指标：受众覆盖率、领域覆盖率、效率和正确性。

Result: 在tau_A=0.7下，DeepPresenter的最佳受众覆盖率为0.714，SlideTailor为0.594，NotebookLM消融版本为0.853，但存在明显的接地差异。

Conclusion: 现有系统能恢复大部分但非全部受众必要信息；视觉质量和广泛主题覆盖不能被视为证据支持，必须进行源接地评估。
...
TL;DR: 提出NeSyCat Torch，通过单子参数化统一神经符号语义，在MNIST加法上表现优异。


<details>
  <summary>Details</summary>
Motivation: 神经符号语义碎片化，NeSyCat缺乏对神经网络学习的谓词和函数的支持。

Method: 扩展NeSyCat为NeSyCat Torch，使用分布单子、惰性对数张量单子和批处理单子，在概率编程和张量后端实现。

Result: 在MNIST加法上，HaskTorch、JAX和PyTorch实现优于LTN和DeepProbLog，精度接近DeepStochLog。

Conclusion: 构造是单子参数化的，可扩展到连续概率，未来工作为神经表示。
...
### [158] [Rethinking Reward Supervision: Rubric-Conditioned Self-Distillation](https://arxiv.org/abs/2606.19327)
*Siyi Gu,Jialin Chen,Sophia Zhou,Arman Cohan,Rex Ying*

Main category: cs.AI

TL;DR: 论文提出了Rubric-Conditioned Self-Distillation框架，利用结构化细粒度反馈进行自我蒸馏，在科学推理任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有后训练方法依赖昂贵的链式思维注释或仅使用标量奖励信号，无法提供细粒度指导。

Method: 提出Rubric-Conditioned Self-Distillation，通过两阶段流程：先生成任务特定rubrics，再训练rubric引导的推理器，利用rubrics提供token级别的指导。

Result: 在多个科学推理基准上，平均超过GRPO 1.0分，超过OPSD 0.9分。

Conclusion: Rubric条件自我蒸馏能有效将标准级别反馈转化为token级别指导，提升推理性能。
...
TL;DR: 在共享工作空间人机团队中，协调和专业知识集成与能力同样重要；结合共享群体记忆和人在环门控的脚手架显著提升三人团队性能。


<details>
  <summary>Details</summary>
Motivation: 自动化AI智能体日益强大，但许多任务需要人类判断，研究人机团队中协调对性能的影响。

Method: 使用Collaborative Gym环境和DiscoveryBench任务，进行1482次会话，比较添加模拟人类协作者的效果，并评估结合共享群体记忆与模拟人在环门控的脚手架。

Result: 添加相关协作者在缺乏协调结构时降低性能；脚手架提升平均性能，尤其在三人团队中，责任信号和专业知识路由更清晰。

Conclusion: 人机团队的协调与集成方式与团队能力同样关键。
...
TL;DR: CEO-Bench是一个通过模拟经营初创公司500天来评估语言模型代理在长期、不确定、嘈杂和动态环境中综合能力的新基准，结果显示最先进模型表现不佳，无法持续盈利。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型代理在孤立短期任务上表现良好，但现实挑战需要长期规划、噪声中信息获取、适应变化和协调多个部分的能力，缺乏评估这些综合能力的基准。

Method: 通过模拟一家初创公司500天的运营，代理通过可编程Python接口管理定价、营销、预算等多个方面，需要分析嘈杂的数据库、挖掘隐藏信息并协调决策。

Result: 最强代理编写复杂代码（如模拟客户群预测现金流），但大多数模型表现挣扎；仅Claude Opus 4.8和GPT-5.5最终余额超过100万美元起始资金，但均未持续盈利。

Conclusion: CEO-Bench是衡量驱动持续适应进步所需智能的第一步，表明当前模型在复杂长期任务上仍有显著不足。
...
### [161] [Optimizing Lithium Production Decisions under Geological, Demand, and Pricing Uncertainties: A POMDP Framework for Multi-Objective Decision Making](https://arxiv.org/abs/2606.18598)
...
TL;DR: 该论文为通用智能体的记忆需求提供了形式化描述，证明了一致接近最优的策略必须使用记忆来区分领域，并且记忆可用于近似重建转移动态。


<details>
  <summary>Details</summary>
Motivation: 为了理解通用智能体在多个环境和目标中接近最优行动时需要在记忆中存储什么，这对于设计此类智能体至关重要。

Method: 通过形式化理论框架和分离定理的证明，分析了记忆在通用智能体中的作用。

Result: 主要结果包括：(1) 当两个领域共享观测瓶颈但需要不兼容的最优行动时，任何一致接近最优的策略必须在瓶颈处诱导出不同的记忆分布，从而得出分离定理：足够成功的智能体不能仅依赖当前观测，而必须保留领域相关信息在记忆中；(2) 如果记忆包含足够的信息来估计相关目标的价值，则记忆可用于近似重建局部转移动态。

Conclusion: 这些结果共同将记忆刻画为支持领域消歧、转移模型重建和通用智能体规划的基质。
...
TL;DR: ProfiLLM是一个针对工业级叫车调度的LLM数据管道，通过工具增强的全局知识挖掘和效用对齐的档案探索，有效解决大规模日志、长尾用户和档案效用问题，在滴滴实际部署中显著提升了预测AUC、GMV等指标。


<details>
  <summary>Details</summary>
Motivation: 生产调度系统依赖结构化数值特征，忽视上下文行为信号；利用LLM生成用户档案面临日志规模超大、长尾用户数据稀疏、以及档案不改善预测效用等挑战。

Method: 包括两个模块：(1) 工具增强的全局知识挖掘：LLM代理利用27个分析工具挖掘平台数据，生成全局知识、聚类规则和供需先验；(2) 效用对齐的档案探索：为聚类生成候选档案，通过下游效用代理评估，迭代优化并构造偏好对用于DPO微调。

Result: 在滴滴生产调度器上，AUC提升6.14%，调度模拟中GMV提升4.35%，在线A/B测试中GMV提升0.47%，完成率提升0.33%，接单前取消率降低0.82%。

Conclusion: ProfiLLM成功将LLM用于工业调度，处理了大规模数据和长尾用户，并带来了实际的业务提升，表明效用对齐的档案生成是有效的。
...
TL;DR: Xcientist通过将研究过程外部化为可检查的合同管理流程，解决了自动化研究中的推理不透明和声明漂移问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI科学工作流中，连接证据、想法、实验和声明的推理过程往往隐含在模型内部，导致不可检查和声明漂移等失败模式。

Method: Xcientist将文献证据、想法状态、实施计划、消融记录和修复痕迹作为持久研究工件，通过合同管理流程实现研究综合与实验验证的外部化。

Result: 在无训练记忆系统、图结构交通预测和多尺度物理信息神经网络三个领域，Xcientist保持了从问题制定到机制设计、验证和有限修订的可追踪轨迹。

Conclusion: AI科学家的评估不仅应基于最终工件，还应基于其合成和验证过程的可归因性、可检查性和科学负责任。
...
TL;DR: BeliefDiffusion结合扩散模型和模型预测控制，在部分可观察环境中显式建模多模态信念分布并进行规划，显著提升导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有信念方法无法捕捉多模态性，生成模型缺乏长期规划，需要更有效的导航框架。

Method: 提出BeliefDiffusion框架，利用扩散模型表征多模态信念分布，并通过模型预测控制进行规划，包括想象环境配置和规划导航策略两步。

Result: 在合成地图环境中，BeliefDiffusion在导航成功率和路径效率上显著优于模型强化学习和其他生成方法。

Conclusion: 显式多模态信念表示结合规划能实现更鲁棒的导航。
...
TL;DR: SGCD通过技能引导的延续蒸馏，在off-trajectory状态下提供监督，提升了GUI agent的成功率。


<details>
  <summary>Details</summary>
Motivation: 行为克隆在专家轨迹上训练，但策略偏离导致遇到off-trajectory状态缺乏监督，无法选择正确动作。

Method: 提出Skill-Guided Continuation Distillation (SGCD)框架：先运行普通策略到达off-trajectory状态，再用技能引导策略完成任务产生成功延续，与专家轨迹混合提供监督；技能从成功和失败rollout中提取，包括Continuation Plans、Critical Targets、Failure Traps和Success Criteria。

Result: 在OSWorld-Verified上，SGCD将三个基础模型的成功率从低30%提升至超过50%。

Conclusion: SGCD在GUI agent任务中有效且通用。
...
### [167] [ARIADNE: Agnostic Routing for Inference-time Adapter DyNamic sElection](https://arxiv.org/abs/2606.19079)
*Enrico Cassano,Michał Brzozowski,Zuzanna Dubanowska,Paolo Mandica,Neo Christopher Chung*

Main category: cs.AI

TL;DR: 提出ARIADNE，一种无需训练、与适配器无关的动态适配器选择框架，通过训练集嵌入质心进行路由，在多个任务上取得高性能。


<details>
  <summary>Details</summary>
Motivation: 现有路由方法依赖适配器内部信息或需要额外训练，限制了可扩展性和可移植性。

Method: ARIADNE通过计算适配器训练集嵌入的质心表示每个适配器，根据未标记输入与质心的距离选择适配器。

Result: 在Llama 3.2 1B Instruct上，23个任务恢复97.44%的上限性能，44个任务达到89.7%平均选择准确率。

Conclusion: ARIADNE无需训练或访问适配器内部即可有效进行动态适配器选择，具有良好的可扩展性和兼容性。
...
### [168] [Towards an Agent-First Web: Redesigning the Web for AI Agents](https://arxiv.org/abs/2606.19116)
...
TL;DR: 本文提出了一种面向AI代理的互联网重新设计方案，涵盖访问、经济和内容三个层面，包括十项设计原则，旨在将代理视为一等公民并重新谈判网络的社会契约。


<details>
  <summary>Details</summary>
Motivation: 网络最初假设人类是主要消费者，但AI代理的兴起打破了这一假设；当前网络通过屏蔽、验证码等方式排斥代理，导致代理访问被视为非法提取而非合法交互，需要重新设计。

Method: 从访问层、经济层和内容层三个层面进行重新设计：访问层提出代理继承人类访问权限，结合速率限制和代理标识元数据，并采用双层层架构；经济层提出基于意图的层级框架、代币订阅模型和委托内容经济；内容层提出ATML（代理文本标记语言）、四级人类监督层级模型和密码学溯源链以对抗认知递归。

Result: 提出了十项设计原则，构成一个代理优先的互联网蓝图。

Conclusion: 需要重新谈判网络的基础社会契约，以将代理整合为第一等公民，涵盖访问、经济和内容三个层面。
...
### [169] [Analysing drivers and interdependencies in European electricity markets using XAI](https://arxiv.org/abs/2606.19118)
*Antoine Pesenti,Aidan O'Sullivan*

Main category: cs.AI

TL;DR: This paper combines deep neural networks with explainable AI to analyze electricity price determinants across 39 European bidding zones, revealing that renewables (especially solar), gas prices, and interconnections are key drivers, and exploring a counterfactual fully integrated EU market.


<details>
  <summary>Details</summary>
Motivation: To address the lack of interpretability of deep neural networks for understanding the underlying drivers of electricity price formation in complex, nonlinear, and interdependent market systems.

Method: Deep neural networks combined with SHAP (SHapley Additive ExPlanations) and an enhanced SSHAP aggregation framework for interpretability in high-dimensional settings.

Result: Renewable energy sources, particularly solar, have disproportionate influence on prices; gas prices are a dominant and consistent driver; interconnections significantly shape price dynamics, indicating strong interdependence across European electricity systems.

Conclusion: The analysis provides insights into electricity price determinants, supports understanding of market integration, and a synthetic EU-wide market scenario suggests potential effects of full integration.
...
### [170] [Beyond Safe Data: Pretraining-Stage Alignment with Regular Safety Reflection](https://arxiv.org/abs/2606.19168)
*Jinhan Li,Kexian Tang,Yihan Xu,Zhuorui Ye,Kaifeng Lyu*

Main category: cs.AI

TL;DR: 提出Safety Reflection Pretraining，通过在预训练语料中插入安全反思来集成自我监控，从而提升LLM的安全对齐。


<details>
  <summary>Details</summary>
Motivation: 预训练阶段的对齐不应仅仅使数据安全，因为模型可能从看似安全的知识组合出危险行为。

Method: 在预训练语料中定期插入简短的安全反思。

Result: 在1.7B模型上提高了安全分类准确率，降低了推理和微调攻击的成功率；在MedSafetyWorld合成环境中也显示优于数据过滤和重写。

Conclusion: 预训练对齐不仅要确保训练数据安全，还要塑造模型从安全数据中可能获得的行为。
