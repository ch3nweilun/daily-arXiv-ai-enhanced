...
### [1] [HANCLIP: A Family of Hyperbolic Angular Negation Vision Language Models](https://arxiv.org/abs/2606.23843)
*Hoang-Bao Le,Aiden Durrant,Thai Son Mai,Binh T. Nguyen,Liting Zhou,Cathal Gurrin*

Main category: cs.CV

TL;DR: HANCLIP通过双曲空间和角度三元组目标增强VLMs对否定的理解，在保持标准性能的同时提升否定敏感性。


<details>
  <summary>Details</summary>
Motivation: VLMs对否定描述脆弱，依赖浅层共现，直接微调会损害原有知识。

Method: 使用双曲表示和角度三元组目标，在20,000图像-文本四元组上训练，重构嵌入空间以编码“图像不是什么”。

Result: 在NegBench上一致提升，标准分类和检索任务上保持或提高性能，且模型无关。

Conclusion: 精心设计的几何目标能有效扩展VLMs的推理能力，仅需少量数据即可。
...
TL;DR: 提出了一种用于海事环境视觉位置识别的两阶段检索-细化框架ProteusVPR，显著降低定位误差。


<details>
  <summary>Details</summary>
Motivation: 现有VPR方法在海事环境中的跨场景感知变化（如开放甲板与封闭舱室）下泛化能力差，无法可靠工作。

Method: ProteusVPR分两阶段：第一阶段使用标准VPR模型进行初始检索；第二阶段通过几何-视觉估计网络融合检索图像与两个时间相邻帧，利用几何描述符、局部仿射坐标系和相机方位角编码实现精确定位。同时引入XHZ数据集。

Result: 在XHZ数据集上，ProteusVPR在多种VPR骨干网上均提高了定位精度，平均定位误差降低超过60%。
...
TL;DR: 提出ObsGraph，一种观察中心的分层场景图，统一场景表示、检索与探索，通过粗到细检索和自适应多尺度探索提升具身推理与探索效率。


<details>
  <summary>Details</summary>
Motivation: 机器人在复杂未知环境中需通过探索获取任务信息，现有方法缺乏结构化场景表示与信息检索的紧密结合。

Method: 构建房间-视图-对象层次场景图，在有限预算下进行粗到细层次检索，并利用检索结果结构化探索候选空间，实现房间级探索、视图细化或边界探索的自适应切换。

Result: 在具身推理和探索基准测试中，ObsGraph显著提高了任务成功率和探索效率。

Conclusion: 结构化场景表示与基于证据缺口的目标信息收集能有效提升机器人在复杂环境中的表现。
...
### [4] [Accelerating Multimodal Large Language Models with Prior-Corrected Token Reduction](https://arxiv.org/abs/2606.24156)
*Zengjie Chen,Yuxiang Cai,Jingcai Guo,Taotao Cai,Jianwei Yin,Zhi Chen*

Main category: cs.CV

TL;DR: PriorTR是一种无需训练的token减少方法，通过对比任务条件注意力与模型诱导先验来改善视觉token修剪，提升多模态大模型的效率-准确率权衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力分数的token修剪方法受模型诱导先验影响，导致指令相关token被误剪，从而降低性能。

Method: PriorTR通过引入空token作为指令无关探针，在单次前向传播中计算模型诱导先验和任务条件后验，然后通过对比两者来估计每个视觉token的额外可用信息，用于指导修剪。

Result: 在多个多模态基准和MLLM上，PriorTR在激进token预算下持续改善准确率与效率的权衡，优于强训练无关基线。

Conclusion: PriorTR作为一种训练无关方法，有效分离先验与条件注意力，提升了视觉token减少的可靠性，尤其在低token预算下表现优越。
...
### [5] [DiffusionBench: On Holistic Evaluation of Diffusion Transformers](https://arxiv.org/abs/2606.24888)
*Xingjian Leng,Jaskirat Singh,Zhanhao Liang,Ethan Smith,Martin Bell,Aninda Saha,Yuhui Yuan,Liang Zheng*

Main category: cs.CV

TL;DR: 提出NanoGen框架统一DiT在ImageNet和T2I上的训练评估，发现两者方法排名无相关性，建议使用DiffusionBench替代仅用ImageNet。


<details>
  <summary>Details</summary>
Motivation: 当前DiT研究集中在ImageNet类条件生成，但可能不反映真实进展；T2I被认为成本高而被跳过，需要更全面的基准。

Method: 开发NanoGen统一框架，支持多种扩散方法，在ImageNet和T2I上训练模型，并比较方法排名。

Result: 训练21个潜在扩散模型，发现ImageNet和T2I方法排名无强相关性（Pearson相关系数-0.377到-0.580），表明仅用ImageNet评估不足。

Conclusion: 推荐使用包含ImageNet和T2I结果的DiffusionBench作为DiT研究的基准，以更准确地反映进展。
...
TL;DR: REALM是首个用于物理世界视觉语言模型的统一红队基准，整合多样化攻击方法、防御和模型，在共享数据集和指标下实现公平比较。


<details>
  <summary>Details</summary>
Motivation: 现有VLM红队评估分散且难以比较，聊天机器人基准不覆盖物理世界功能失败，需要统一协议来比较攻击方法并评估物理场景下的漏洞。

Method: 提出REALM，集成12种红队方法、3种防御和13种VLM，采用黑盒威胁模型和共享数据集；通过代理目标生成流水线构建场景特定的物理基础攻击目标，实现对抗目标对齐。

Result: 实验发现文本和排版注入攻击最有效，多模态协同优化产生最强视觉扰动迁移，单次攻击方法以更低成本接近迭代方法性能，而模型规模不保证对抗鲁棒性。

Conclusion: REALM提供了物理世界VLM红队评估的统一基准，揭示了不同攻击方法的相对有效性，并为未来研究提供了标准化评估框架。
...
### [7] [Dual-Branch Cross-Projection Debiasing through Diffusion-based Disentanglement](https://arxiv.org/abs/2606.24161)
*Xiangqian Zhao,Xinyang Jiang,Zhipeng Xu,Lingfeng He,Zilong Wang,Dongsheng Li,De Cheng,Nannan Wang*

Main category: cs.CV

TL;DR: 提出基于扩散解缠概念的置信度引导偏差挖掘（CBCM）和双分支交叉投影去偏（DCD）框架，在无组标签条件下实现最差组准确率最优，且仅调整0.22%参数。


<details>
  <summary>Details</summary>
Motivation: 基础模型依赖虚假关联导致少数群体泛化差；现有无监督去偏方法难以识别语义对齐的虚假属性，且单分支设计使目标与虚假特征纠缠。

Method: CBCM利用扩散解缠的语义概念表示识别可靠虚假属性；DCD通过双分支分离目标与虚假表示，并利用交叉零空间投影显式移除虚假信息。

Result: 在四个基准数据集上，方法在组无监督方法中取得最差组准确率领先，且仅调整0.22%参数。

Conclusion: 所提方法有效提升模型在少数组上的泛化，且参数高效。
...
### [10] [Boosting Text-Driven Video Segmentation via Geometry-Aware Distillation](https://arxiv.org/abs/2606.24464)
*Tianyu Zhu,Yingping Liang,Hesong Li,Ying Fu*

Main category: cs.CV

TL;DR: Proposes GeoLaV, a two-stage framework that enhances text-driven video segmentation by distilling 3D geometric knowledge from images, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing RVOS models trained with naive segmentation losses lack geometric consistency across frames, resulting in weak spatial understanding.

Method: First stage: monocular geometry pretraining with novel-view synthesis on single-image datasets. Second stage: geometry-aware distillation fine-tuning on video datasets with a general 3D prior model.

Result: Zero-shot generalization using only image data, and state-of-the-art results on multiple RVOS benchmarks after fine-tuning.

Conclusion: GeoLaV effectively incorporates 3D geometry to improve spatiotemporal coherence and language grounding in RVOS.
...
### [11] [Evaluating the Interpretability of Sparse Autoencoders with Concept Annotations](https://arxiv.org/abs/2606.24716)
*Jonas Klotz,Cassio F. Dantas,Pallavi Jain,Diego Marcos,Begüm Demir*

Main category: cs.CV

TL;DR: 提出一种基于人类标注的评估框架，通过合成基准和匹配算法量化稀疏自编码器潜在与概念的对齐。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法依赖代理指标或定性检查，缺乏语义对应的直接测量。

Method: 构建合成基准synCUB和synCOCO，提出全二元匹配追踪（FBMP）实现多对一映射，以及目标属性扰动对齐分数（TAPAScore）进行功能验证。

Result: FBMP和TAPAScore能可靠区分训练与未训练SAE；过完备度增加会降低扰动对齐，适度字典大小提供最佳可解释性。

Conclusion: 该评估框架揭示了字典大小与可解释性的权衡，为SAE在视觉模型中的应用提供了有效评估手段。
...
TL;DR: HeRA通过在注意力头级别强制跨模态表示对齐，有效提升MLLM性能并减少视觉幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有表示对齐方法固定语言骨干的层，忽略了Transformer的细粒度结构，HeRA旨在通过注意力头级别的对齐保持拓扑结构。

Method: 基于MKNN度量，对选定的注意力头应用对比目标作为微分代理，选择最不对齐的头进行对齐。

Result: 在多个MLLM和18个基准测试上，HeRA持续提升视觉中心任务性能并有效减少幻觉。

Conclusion: HeRA是一种利用注意力头细粒度结构的有效正则化方法，改善了跨模态表示对齐。
...
TL;DR: 提出DramaDirector框架，利用真实短剧镜头的几何信息生成多镜头视频，并在DramaBoard基准上提升忠实度、一致性和可控性。


<details>
  <summary>Details</summary>
Motivation: 短剧因其快速节奏和动态镜头，对基于提示或纯文本的视频生成管道构成挑战，现有方法难以满足需求。

Method: DramaDirector通过检索真实短剧镜头的深度和姿态索引，解耦每个镜头的静态视觉和动态叙事条件，并使用schema约束的SFT和GRPO训练规划器，结合文本-视觉对齐奖励，指导第一帧生成和图像到视频合成。

Result: 在DramaBoard基准上，DramaDirector在忠实度、一致性和可控性上优于代表性的多智能体和视频生成基线。

Conclusion: 提出几何引导的短剧生成方法，有效利用专业摄影知识，提升生成视频的质量和控制能力。
...
### [15] [Flood Mapping from RGB imagery using a Vision Foundation Model](https://arxiv.org/abs/2606.24120)
*Vladyslav Polushko,Tilman Bucher,Ronald Rösch,Thomas März,Markus Rauhut,Andreas Weinmann*

Main category: cs.CV

TL;DR: 卫星预训练的基础模型通过微调可有效用于RGB洪水测绘，在跨事件迁移中表现优异。


<details>
  <summary>Details</summary>
Motivation: 及时高分辨率洪水地图对应急响应至关重要，现有深度学习模型泛化能力不足，而卫星预训练模型需适应航拍RGB图像。

Method: 使用Prithvi-2.0-UPN（Prithvi-EO-2.0-600M视觉变换器+UPerNet解码器）进行二值水分割微调，在BlessemFlood21和NeuenahrFlood数据集上实验。

Result: 在目标数据集上达到SOTA；零样本跨事件迁移优于基线；用少量数据微调后性能快速接近全训练水平。

Conclusion: 卫星预训练的地球观测基础模型可适应厘米级RGB洪水测绘，并展现出强大的迁移能力。
...
TL;DR: 将测试时规范化重构为OOD检测，系统评估多种OOD分数和搜索算法，发现距离基分数配合随机搜索和局部精细化效果最佳，并引入门控机制避免损害分布内精度。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉模型对仿射变换（旋转、缩放、剪切）鲁棒性不足，现有提升鲁棒性的方法需要修改或重新训练模型，而测试时规范化虽不改变模型但现有方法依赖有限的logit基能量分数和定制搜索，设计空间未充分探索。

Method: 将规范化视为OOD检测，使用任何OOD分数作为能量在变换空间上最小化；系统评估约20种OOD分数和9种搜索算法，选择最优组合（距离基分数+随机搜索和局部精细化）；增加门控机制，仅当输入OOD分数高时才进行规范化。

Result: 在手写字符、素描、自然图像和3D点云等基准上，距离基分数配合随机搜索和局部精细化整体表现最佳；门控机制在保持分布内精度的同时，显著提升对变换输入的鲁棒性。

Conclusion: 将测试时规范化作为OOD检测问题，并系统选择OOD分数和搜索算法，可以在不重新训练或修改模型的情况下有效提升对仿射变换的鲁棒性，门控机制进一步保证了分布内性能。
...
### [17] [MorVess: Morphology-Aware Pulmonary Vessel Segmentation Network](https://arxiv.org/abs/2606.24214)
...
### [19] [ActiveScope: Actively Seeking and Correcting Perception for MLLMs](https://arxiv.org/abs/2606.24292)
*Yajing Wang,Chao Bi,Junshu Sun,Shufan Shen,Zhaobo Qi,Shuhui Wang,Qingming Huang*

Main category: cs.CV

TL;DR: 提出ActiveScope训练无关框架，通过主动搜索和自我纠正机制增强MLLMs在高分辨率图像中的细粒度感知，有效克服上下文主导和语义偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关方法在高分辨率图像细粒度感知中常被干扰物误导，无法定位多个目标，根本原因在于Contextual Dominance和Semantic Bias。

Method: ActiveScope包含Semantic Anchor Localization (SAL)模块利用细粒度语义锚点独立定位关键目标以减轻语义偏差，以及Interference-Suppressed Refinement (ISR)模块抑制对突出干扰物的注意力以克服上下文主导。

Result: 在高分辨率图像理解基准上超越现有训练无关方法，例如在V* Bench上达到96.34%准确率。

Conclusion: 验证了主动搜索和自我纠正范式在增强MLLMs细粒度感知方面的优越性。
...
### [20] [MM-TRELLIS: Point-Cloud Guided Multi-Modal 3D Vehicle Generation in Autonomous Driving](https://arxiv.org/abs/2606.24301)
*Hongli Xiao,Youjian Zhang,Yucai Bai,Chaoyue Wang,Yaohui Jin,Xiaoguang Ren,Wenjing Yang,Long Lan*

Main category: cs.CV

TL;DR: MM-TRELLIS is a multi-modal 3D generative model that integrates LiDAR and multi-view images for high-fidelity vehicle generation from driving scenes.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to fully exploit multimodal sensors and rely on neural rendering leading to low-quality mesh; native 3D generative models struggle with in-the-wild images.

Method: Uses multi-view images as conditioning and LiDAR point clouds as test-time guidance; aligns guidance with model priors during denoising, enforces consistency, and applies opacity-based voxel filtering from 3D Gaussian Splatting to produce clean meshes.

Result: Outperforms existing methods on Waymo dataset for high-fidelity 3D vehicle generation.

Conclusion: MM-TRELLIS effectively generates realistic 3D vehicles by leveraging multimodal data and 3D generative priors, enabling practical applications in autonomous driving.
...
### [21] [UniTranslator: A Unified Multi-modal Framework for End-to-end In-Image Machine Translation](https://arxiv.org/abs/2606.24333)
*Jiahao Lyu,Pei Fu,Zhenhang Li,Shaojie Zhang,Jiahui Yang,Yu Zhou,Can Ma,Zhenbo Luo,Jian Luan*

Main category: cs.CV

TL;DR: UniTranslator是一个统一的IIMT框架，通过UGAM和SMD模块解决了理解-生成冲突和空间错位，实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型在IIMT中面临理解-生成冲突和空间位置错位的挑战。

Method: 提出UniTranslator框架，包括理解-生成对齐模块（UGAM）和空间掩码解码器（SMD）。

Result: 在多个基准上达到SOTA，表现出翻译理解和图像生成之间的互增强效果。

Conclusion: 统一的多模态学习在IIMT中具有优势。
...
### [22] [Transformation Behavior of Images in Latent Space](https://arxiv.org/abs/2606.24430)
*Christian Zöllner,Mozzam Motiwala,Aysel Ahadova,Gerrit Anders,Robert Hüneburg,Jacob Nattermann,Matthias Kloor*

Main category: cs.CV

TL;DR: 编码器网络对图像变换具有鲁棒性但非完全不变，变换增强可提升性能，通用与专用网络差异显著。


<details>
  <summary>Details</summary>
Motivation: 训练编码器网络需适应下游任务，编码生物/诊断内容且对标签无关变换保持不变量，本文研究经典变换对潜在空间的影响。

Method: 使用Lunit Inc.、Bioptimus及Meta Research Team的编码器网络，通过对比原始与变换后图像嵌入及随机嵌入，基于H&E染色切片（结直肠组织和TCGA数据集）评估嵌入方差。

Result: 原始与变换后嵌入比随机嵌入更接近，表明鲁棒性，但未完全不变。

Conclusion: 编码器网络未完全中和变换效应，解释了变换增强的有效性；通用与专用网络间存在显著差异。
...
### [28] [Compact Object-Level Representations with Open-Vocabulary Understanding for Indoor Visual Relocalization](https://arxiv.org/abs/2606.24767)
*Zhaopeng Cui,Jiarui Hu,Jingbo Liu,Boming Zhao,Xiyue Guo,Boyin Feng,Haocheng Peng,Yujun Shen,Hujun Bao,Guofeng Zhang*

Main category: cs.CV

TL;DR: OpenReLoc利用多模态语义和对象导向参考帧实现高效准确的室内视觉重定位。


<details>
  <summary>Details</summary>
Motivation: 现有室内视觉重定位方法依赖低级视觉方案，难以感知场景语义和组成，限制了解释性和应用性。

Method: 提出OpenReLoc系统，包括基于基础模型的多模态机制用于开放词汇2D-3D对象匹配；设计对象导向参考帧作为位置先验，并采用DIOU参考帧选择策略支持可扩展场景；提出双路径2D迭代最近像素损失用于稳定精确的位姿优化。

Result: 实验结果表明OpenReLoc在多个数据集上取得了更优的重定位召回率和精度。

Conclusion: OpenReLoc有效结合场景语义与几何信息，提升了重定位性能和可解释性，代码将开源。
...
### [29] [Counting Trees from Satellite Imagery with Noisy Supervision](https://arxiv.org/abs/2606.24786)
...
### [31] [Listening makes Vision Clear for VLMs](https://arxiv.org/abs/2606.23763)
*Yiyang Chen,Yixin Tan,Binrui Shen*

Main category: cs.CV

TL;DR: 提出PV-TAM方法，通过提示侧语义和注意力峰值分布改进视觉-语言一致性评估。


<details>
  <summary>Details</summary>
Motivation: 回答侧注意力分布受解码漂移和结构token影响，不能准确反映语义一致性。

Method: 提出Prompt-Vision Token Activation Map (PV-TAM)，使用提示侧语义并加入滤波器去除模态边界标记的偏差，通过注意力峰值分布度量对齐。

Result: PV-TAM在注意力基础和IoU风格定位指标上持续优于回答侧基线。

Conclusion: PV-TAM能更有效地评估视觉-语言一致性，避免注意力扭曲。
...
### [32] [Fabric Image Demoiréing Benchmark from Synthesis to Restoration](https://arxiv.org/abs/2606.24072)
*Pengchao Wei,Xiaojie Guo*

Main category: cs.CV

TL;DR: 提出了第一个织物图像去摩尔纹基准，包括物理合成的大规模数据集和基线模型。


<details>
  <summary>Details</summary>
Motivation: 织物摩尔纹因纺织图案的宽带和半周期特性而更具挑战性，现有模型在织物去摩尔纹任务上表现不佳，且缺乏标准化基准。

Method: 开发了物理启发的合成框架，构建了包含16,050对多分辨率图像的织物摩尔纹数据集，并定制了基线模型。

Result: 提出的基线模型在基准数据集上实现了有竞争力的性能，并展现出强泛化能力。
...
### [33] [A Benchmark for Hallucination Detection in VLMs for Gastrointestinal Endoscopy](https://arxiv.org/abs/2606.24115)
*Aminu Lawal,Niyoj Oli,Sachin Acharya,Prashnna Gyawali,Maria Carmen Romano,Binod Bhattarai*

Main category: cs.CV

TL;DR: 本文在胃肠道内窥镜VQA数据集上基准测试九种幻觉检测方法，发现白盒方法ReXTrust最优，并识别出自信幻觉这一失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法主要在放射学上评估，而胃肠道内窥镜检查尚未充分探索，且VLM幻觉问题阻碍其临床部署。

Method: 在Gut-VLM数据集上，使用5种VLM（MedGemma-4B/27B, LLaVA-Med-7B等），评测9种方法（黑盒：RadFlag、SelfCheckGPT-NLI；灰盒：AvgProb等；白盒：ReXTrust），采用配对置换检验进行统计比较。

Result: ReXTrust在所有VLM上AUC最高（最高93.0），平均比次优方法高19.5个AUC点（p<0.001）；黑盒和聚类灰盒方法在LLaVA-v1.6-7B上接近随机；token级灰盒方法（MaxEnt、MaxProb）为最佳非白盒选择；发现自信幻觉（高一致性或高概率的幻觉）是系统性失败。

Conclusion: 白盒方法（ReXTrust）提供显著的幻觉检测优势，非白盒方法中token级灰盒统计最优，自信幻觉需要进一步处理以提升安全部署。
...
TL;DR: Sat2City v2利用预训练3D基础模型和真实世界卫星-网格配对数据集，实现从单张卫星图像生成显式的、可控制外观的纹理化3D城市网格。


<details>
  <summary>Details</summary>
Motivation: 生成显式3D城市资产对数字孪生、城市模拟和地理空间智能很重要，但现有方法如Sat2City存在外观随机、训练数据合成、对噪声真实重建扩展性差等问题。

Method: 构建包含16,241个卫星-网格对的真实世界数据集，将网格编码到预训练原生3D潜在空间，微调卫星条件几何流，并使用解码形状锚定卫星条件纹理化。

Result: 在度量尺度DSM重建和生成城市资产基准的几何与外观评估中，Sat2City v2在基线中取得最佳整体性能。

Conclusion: Sat2City v2将卫星到城市的生成从面向渲染的3D代理推进到显式纹理网格资产，并提供了第一个为此任务收集的卫星-网格配对数据集。
...
### [35] [Latent Visual States for Efficient Multimodal Reasoning](https://arxiv.org/abs/2606.24233)
...
TL;DR: EVA integrates continuous latent visual representations as intermediate thoughts in LLMs, improving efficiency and performance over discrete-token methods.


<details>
  <summary>Details</summary>
Motivation: Existing visual integration relies on discrete outputs (code/boxes), causing rigid dependencies and latency. EVA aims to overcome these limitations with continuous latent representations.

Method: EVA generates Latent_slot tokens as adaptive intermediate visual thoughts, co-optimized with text tokens via D-GSPO to handle policy deviation, and uses EVA-230K dataset for SFT.

Result: EVA achieves significant performance gains and enhanced inference efficiency across multiple benchmarks.

Conclusion: EVA effectively enhances LMM capabilities by natively generating continuous latent visual states, outperforming discrete-token approaches.
...
### [36] [UniRED: Unified RGB-D Video Frame Interpolation with Event Guidance](https://arxiv.org/abs/2606.24282)
*Yinuo Zhang,Guangshun Wei,Yuanfeng Zhou,Yiran Shen*

Main category: cs.CV

TL;DR: 提出统一多模态框架，结合RGB、深度和事件线索实现高帧率RGB-D视频插值。


<details>
  <summary>Details</summary>
Motivation: RGB-D相机帧率低难以捕捉快速动态，现有RGB插值方法不适用于RGB-D场景，且仅用两帧估计运动约束不足。

Method: 提取并融合RGB、深度和事件特征，通过运动基元细化估计双向流和Z轴细化深度，最后双向扭曲和软混合合成帧。

Result: 在公共基准和自建数据集上，RGB插值光度保真度和深度插值几何准确性均优于现有方法。

Conclusion: 有效利用多模态信息提升插值质量，并构建RGB-D-Event数据集缓解训练数据稀缺。
...
### [37] [REDI-Match: Rotation-Equivariant Distillation for Efficient and Robust Dense Matching](https://arxiv.org/abs/2606.24330)
*Yinji Ge,Guixu Zheng,Wulong Guo,Qian Feng,Xu Wu,Kai Zhou,Xinyuan Liu,Fei Xing*

Main category: cs.CV

TL;DR: 提出REDI-Match，通过旋转等变蒸馏解决VFM在严重面内旋转下的特征匹配瓶颈，实现高效准确的新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法面临数据驱动参数低效与严格等变网络语义不足的矛盾，冻结VFM将旋转负担全部转嫁解码器，形成架构瓶颈。

Method: REDI范式将VFM非等变语义蒸馏到轻量严格等变编码器，并设计熵驱动空间对齐模块，通过离散旋转假设锁定规范坐标系。

Result: 在SatAst等基准上达到新SOTA，绝对姿态精度提升13.89%，速度比RoMa v2快1.9x，单卡RTX 4090实现41 FPS实时推理。

Conclusion: REDI-Match有效突破了旋转泛化的架构瓶颈，在保持高效同时显著提升匹配性能。

Abstract: Vision Foundation Models (VFMs) have significantly advanced dense feature matching, yet severe in-plane rotation remains a critical challenge. Existing solutions face a fundamental dilemma: data-driven methods require inefficient parameter scaling to implicitly learn rotations, whereas strictly equivariant networks lack the semantic capacity of modern VFMs. Consequently, current frameworks typically freeze VFMs and shift the entire burden of rotation generalization to the downstream decoder. To break this architectural bottleneck, we propose REDI-Match, an efficient framework driven by a novel Rotation-Equivariant Distillation (REDI) paradigm. Instead of relying on rotation data augmentation to establish rotational correspondences, REDI distills the non-equivariant semantic representations of a VFM into a lightweight, strictly rotation-equivariant encoder, leveraging an equivariant geometric architecture to constrain robust high-dimensional semantics. To fully exploit these features, we equip the decoder with an entropy-driven spatial alignment module. By evaluating discrete rotation hypotheses, this mechanism explicitly locks onto the canonical coordinate system, eliminating global ambiguity before continuous refinement. Extensive experiments demonstrate that REDI-Match establishes a new state-of-the-art (SOTA) across multiple benchmarks. Notably, it achieves a 13.89% absolute pose accuracy improvement on the highly challenging SatAst dataset while operating 1.9x faster than the current SOTA (RoMa v2), enabling real-time inference (~41 FPS) on a single RTX 4090 GPU. Code: .

</details>


### [38] [TIGER: Taming Identity, Geometry, and Generative Priors for High-Quality Face Video Restoration](https://arxiv.org/abs/2606.24336)
*Yang Zhou,Wenxue Li,Peng Zhang,Yifei Chen,Fei Wang,Daiguo Zhou*

Main category: cs.CV

TL;DR: TIGER通过融合身份、几何和生成先验，实现高质量、身份一致的面部视频恢复。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时解决身份偏移、视角纠缠引导和感知真实性三大挑战。

Method: 首先建立身份先验（注入主题嵌入），然后构建几何先验（2D参考提升到3D参数空间融合），利用生成先验（一步整流流），并采用渐进式三阶段训练优化。

Result: 在身份保真度和时间稳定性上达到SOTA性能。

Conclusion: TIGER实现了高质量、高效且身份一致的面部视频恢复。
...
### [39] [SignNet-1M: Large-Scale Multilingual Sign Language Video Dataset with Downstream Benchmarks](https://arxiv.org/abs/2606.24361)
*Zhewen He,Junyi Hu,Haomian Huang,Zhenhua Li,Yu-Shen Liu,Yi Fang*

Main category: cs.CV

TL;DR: SignNet-1M是一个大规模增强手语数据集，通过3D高斯溅射、扩散模型和后渲染增强来模拟真实世界变化，提高模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有手语数据集在受限条件下收集，缺乏视角、背景和身份多样性，导致模型在真实世界分布变化下鲁棒性差。

Method: 采用三种增强方法：(i) 3D高斯溅射进行新视角渲染，(ii) 扩散模型进行背景替换和签名者替换，(iii) 后渲染增强模拟采集和压缩伪影。

Result: 在跨视角、跨背景、跨身份和后渲染变化等分布偏移下，训练SignNet-1M一致提升泛化能力，同时保持强分布内性能。

Conclusion: SignNet-1M数据集和增强管道能有效提升手语模型对真实世界变化的鲁棒性，为后续研究提供基准。
...
### [40] [Modality-Aware Out-of-Distribution Detection for Multi-Modal Action Recognition](https://arxiv.org/abs/2606.24404)
*Lars Doorenbos,Duc Manh Vu,Serdar Ozsoy,Juergen Gall*

Main category: cs.CV

TL;DR: 本文提出一种专门针对多模态场景的后验OOD检测方法，利用多模态与单模态预测之间的关系，结合特征空间分数，在MultiOOD基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 多模态融合提升了动作识别性能，但如何利用额外信息增强模型鲁棒性（尤其是多模态OOD检测）尚未充分探索。现有方法在推理时仍使用单模态OOD检测器，丢弃了重要信息。

Method: 基于多模态与单模态预测之间的关系，构建后验检测器；将该信号与检测多模态空间中离流形样本的特征空间分数结合，并通过多模态logits归一化，形成混合检测器。

Result: 在MultiOOD基准的多个数据集上，平均性能优于现有方法。

Conclusion: 推理时显式考虑不同模态对于多模态OOD检测至关重要。
...
### [41] [SENTRY: SAM2-Enhanced Neighbor-Aware and Temporally Reasoned Memory for Visual Tracking](https://arxiv.org/abs/2606.24449)
*Mohamad Alansari,Yonathan Michael,Hasan AlMarzouqi,Muzammal Naseer,Naoufel Werghi,Sajid Javed*

Main category: cs.CV

TL;DR: 提出SENTRY，通过写入前验证时间一致性改进SAM2跟踪器的内存更新，实现无需训练的状态对跟踪性能提升。


<details>
  <summary>Details</summary>
Motivation: SAM2跟踪器使用置信度掩码选择导致在遮挡、快速运动和干扰下发生漂移。

Method: SENTRY聚合每帧的多样分割假设，回溯为短轨迹，并通过邻域感知的循环一致匹配验证时间一致性，替换置信度驱动的写入。

Result: 在五个基线和九个基准上一致提升，实现零样本SOTA，SAM2-L版本在A100上达32.8 FPS，额外VRAM仅0.4-0.6 GB。

Conclusion: 强制写入时的时间有效性稳定了记忆增强跟踪，无需重新训练。
...
### [42] [ViTexQA: A Multi-Frame Temporal Perception Dataset for Video Text Question Answering](https://arxiv.org/abs/2606.24602)
*Zhentao Guo,Chen Duan,Tongkun Guan,Zining Wang,Kai Zhou,Pengfei Yan*

Main category: cs.CV

TL;DR: 提出ViTexQA数据集和FrameThinker方法，通过质量控制的CoT注释和两阶段训练（SFT+RL）增强视频文本理解中的多帧时间推理，ROUGE-L提升6.3%。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在视频文本理解中对跨帧时间分布的文本线索整合不足，现有数据集多数问题可从单帧回答，无法反映真实需求。

Method: 构建ViTexQA数据集时采用质量控制的CoT注释管道并引入时间约束，确保QA对需要跨帧文本融合；FrameThinker采用两阶段训练：CoT引导的SFT生成帧感知推理链，然后进行基于时间接地的RL优化多帧连贯性。

Result: 在ViTexQA上优于SOTA基线，ROUGE-L提升6.3%。

Conclusion: ViTexQA和FrameThinker有效提升了视频文本理解中的多帧时间推理能力。
...
### [43] [BioMedVR: Confusion-Aware Mixture-of-Prompt Experts for Biomedical Visual Reprogramming](https://arxiv.org/abs/2606.24740)
*Jiaxiang Liu,Tianxiang Hu,Juwei Guan,Yujie Wu,Yusong Wang,Yao Mu,Zuozhu Liu,Mingkun Xu*

Main category: cs.CV

TL;DR: 提出BioMedVR，首个基于视觉重编程的框架，用于生物医学图像的少样本适应，通过混淆最小化机制和混合提示专家解决类混淆问题，在多个数据集上取得优越性能。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型在自然图像上泛化能力强，但适应到生物医学成像面临计算成本高、数据稀缺和细粒度类间差异的挑战，现有视觉重编程方法忽略混淆负样本导致预测失调。

Method: 提出BioMedVR框架，包括混淆最小化机制（利用LLM生成混淆感知属性和混淆抑制损失减少假阳性对齐）和混合提示专家（结合正专家和负专家，通过自适应门控平衡）。

Result: 在18个数据集（11个生物医学和7个自然图像基准）上，BioMedVR取得了优越的准确性和泛化性能。

Conclusion: BioMedVR有效地桥接了视觉重编程和视觉-语言模型在生物医学领域的应用。
...
### [44] [High-Fidelity Synthetic Transmission Electron Microscopy Image Generation Using Diffusion Probabilistic Models for Data-Limited Semiconductor Metrology](https://arxiv.org/abs/2606.24817)
*Johannes Boehm,Bappaditya Dey*

Main category: cs.CV

TL;DR: 提出DDPM框架，利用15个样本从零训练生成高保真合成TEM图像，支持下游任务。


<details>
  <summary>Details</summary>
Motivation: TEM样品制备破坏性、成像慢、成本高，导致数据稀缺；现有生成模型缺乏TEM特定噪声和结构细节。

Method: 采用渐进式块训练策略，结合自定义TrivialAugment、跨进程域迁移、分类器引导和RePaint修复，实现全图像生成，并利用DDPM特征进行分割。

Result: 合成图像MS-SSIM > 0.98，专家评估一致，有效用于缺陷检测、分割和计量等下游ML训练。

Conclusion: 合成图像保持统计与物理真实性，缓解数据稀缺问题。
...
### [45] [BenchX: Benchmarking AI Models for Cancer Detection and Localization with Demographic and Protocol Biases](https://arxiv.org/abs/2606.24883)
...
TL;DR: A large-scale open benchmark of 85,355 CT scans reveals that current AI models for tumor detection perform poorly on rare subgroups.


<details>
  <summary>Details</summary>
Motivation: AI models for medical imaging often perform inconsistently across real-world settings due to variations in patient demographics and imaging protocols. To quantify these inconsistencies, we develop a benchmark.

Method: We use 85,355 CT scans, systematically evaluate 12 tumor-detection AI models across tumor size, location, patient subgroup, and imaging protocol. We leverage LLMs to extract subgroup information from clinical data.
...
Conclusion: The benchmark provides a foundation for building more reliable and robust AI models for tumor detection and highlights the need for rigorous, subgroup-level evaluation in medical imaging and computer vision.
...
### [46] [DriveStack-VLA: Render-Teacher Alignment for BEV-Based DeepStack Vision-Language-Action Model](https://arxiv.org/abs/2606.24051)
*Jingke Wang,Zhenru Zhao,Shuangming Lei,Hao Su,Yuehao Huang,Yijia Xie,Kai Tang,Guanglin Xu,AiXue Ye,Yukai Ma,Yong Liu*

Main category: cs.CV

TL;DR: DriveStack-VLA通过双视觉建模、Render-Teacher Alignment和自批判模块，增强了VLA驾驶模型的空间感知能力，在多个基准上取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA驾驶模型缺乏面向驾驶的空间智能，主要依赖透视图像和语言先验，缺乏度量几何、场景结构和安全关键感知，导致对弱视觉几何和感知覆盖不足。

Method: 提出DriveStack-VLA框架，包括：1) 通过DeepStack连接将BEV表示注入LLM解码器，并利用Render-Teacher Alignment对齐真实图像和栅格图像的感知焦点；2) 基于头部的自批判模块，用于排序采样轨迹并条件性地优化最佳轨迹。

Result: 在NAVSIMv1上达到91.6 PDMS，NAVSIMv2上达到91.0 EPDMS（启用人类惩罚过滤器），在闭环Bench2Drive上驾驶得分79.49，成功率达到56.36%。

Conclusion: DriveStack-VLA通过增强空间感知和轨迹选择策略，显著提升了VLA驾驶模型的性能，证明了空间智能在驾驶决策中的重要性。
...
### [49] [Co-occurring associated retained concepts in Diffusion Unlearning](https://arxiv.org/abs/2606.24192)
*Miso Kim,Georu Lee,Yunji Kim,Hoki Kim,Jinseong Park,Woojin Lee*

Main category: cs.CV

TL;DR: 提出ReCARE框架，在消除目标概念的同时保护良性共现概念，并引入CARE评分量化保留


<details>
  <summary>Details</summary>
Motivation: 现有消除方法会意外地抑制良性共现概念（如消除裸体时抑制“人”），需要保护这些必须保留的概念

Method: 定义CARE概念并提出CARE评分，提出ReCARE框架，自动构建CARE-set（良性共现词表），在训练中利用该词表实现稳定消除

Result: 在裸体、梵高风格和丁鲷物体等多个目标概念上，ReCARE在平衡稳健概念消除、整体效用和CARE保留方面达到最先进水平

Conclusion: ReCARE能够有效地同时实现目标概念消除和良性共现概念保留
...
### [50] [FiCA: Feed-forward instant Gaussian Codec Avatars from a Single Portrait Image](https://arxiv.org/abs/2606.24232)
...
TL;DR: 提出FiCA，一种前馈即时高斯编解码器头像生成管线，从单张肖像图像生成逼真、可驱动的3D高斯头像。


<details>
  <summary>Details</summary>
Motivation: 从单张图像生成逼真且可驱动的头像极具挑战，因为有限的视觉信息难以准确推断3D外观和几何。

Method: 结合人类视觉基础模型与扩散模型，从部分观察生成完整3D网格；引入前馈网格细化网络增强保真度和身份保持，无需测试时优化；利用通用先验将网格解码为3D高斯，实现实时驱动。

Result: 生成的头像忠实代表多样身份，视觉质量优于近期竞争方法。

Conclusion: 该方法证明了前馈方式从单张图像生成高质量可驱动3D高斯头像的有效性。
...
### [51] [Lite Any Stereo V2: Faster and Stronger Efficient Zero-Shot Stereo Matching](https://arxiv.org/abs/2606.24457)
...
### [55] [From Spatial to Spectral: An Efficient, Frequency-Guided Feature Representation Learner for Small Object Detection](https://arxiv.org/abs/2606.23825)
*Yuhan Rui,Shihan Qiao,Yibin Lou,Mingxi Yu,Yutong Wan,Yanqiao Chen,Dongsheng Hou,Zhen Cao,Athena Zhuoming Zhong,Qi Hao*

Main category: cs.CV

TL;DR: 提出从空间域转向频谱域的小目标检测方法，通过频率引导特征表示和DER算子实现高效检测，在多个基准上超越YOLOv11且参数极少。


<details>
  <summary>Details</summary>
Motivation: 空间域中小目标检测因高频细节被丢弃而困难，恢复细节计算昂贵且放大噪声。

Method: 提出频率引导特征表示框架和DER算子，包括WDG、LGE、FDHead三个轻量模块，注入频率感知调制到检测器各环节。

Result: 在VisDrone2019、UAVDT、TinyPerson、DOTAv1等基准上持续改进，DERNet系列以YOLOv11的1/6参数取得更优性能。

Conclusion: 频谱域特征处理能有效提升小目标检测，且参数效率高。
...
### [56] [ABACUS: Adapting Unified Foundation Model for Bridging Image Count Understanding and Generation](https://arxiv.org/abs/2606.23835)
*Anindya Mondal,Sauradip Nag,Anjan Dutta*

Main category: cs.CV

TL;DR: ABACUS是一个统一的视觉语言模型，通过三项关键创新实现多种计数任务的零样本处理，并在七个基准上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要基准特定训练，且存在空间定位、边界错误以及理解与生成之间的差距，ABACUS旨在无需外部注释解决这些问题。

Method: 模型基于3B参数基础模型，采用密度感知自适应缩放与对象性图进行空间定位，利用GRPO的边界感知计数策略消除裁剪错误，并通过循环一致GRPO让理解分支自我批评生成输出。

Result: 在七个基准上取得最先进结果，超越任务特定专家和更大通用模型。

Conclusion: ABACUS通过自我批评机制缩小理解与生成差距，无需外部注释，为统一视觉语言模型在计数任务中树立新标准。
...
### [57] [The Professor: Multi-Teacher Unsupervised Prompt Distillation for Vision-Language Models](https://arxiv.org/abs/2606.23897)
*Ahmad Algadhi,Ahmed Alzuhair,Omar Alkhulaif,Muzammil Behzad*

Main category: cs.CV

TL;DR: 提出多教师提示蒸馏方法TheProfessor，通过集成域微调和零样本教师模型，提升学生性能，尤其在域转移下。


<details>
  <summary>Details</summary>
Motivation: 扩展单教师PromptKD到多教师，利用互补的教师监督提升轻量学生模型。

Method: 使用固定双教师集成：域微调PromptSRC ViT-L/14和零样本EVA-CLIP-L/14，通过置信度加权或等概率集成进行蒸馏。

Result: 在四个数据集上，置信度加权平均HM从87.52提高到89.28（+1.77），等概率集成提高到88.88（+1.37），域转移数据集EuroSAT提升最大（+5.78）。

Conclusion: 多教师提示蒸馏在第二个教师提供互补监督时（尤其存在域转移）最有效。
...
### [58] [Trustworthy Image Authentication using Forensic Knowledge Graphs](https://arxiv.org/abs/2606.23917)
*Tai D. Nguyen,Matthew C. Stamm*

Main category: cs.CV

TL;DR: 提出Forensic Knowledge Graphs (FKGs)框架，集成取证证据提取、结构化推理和可解释说明，在伪造检测和定位上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有取证检测器缺乏可解释性，而视觉语言模型不能利用取证痕迹进行可靠检测。

Method: 提出FKGs，编码取证痕迹及其因果依赖和场景内容关联，通过取证认证网络和迭代上下文精炼策略生成准确的FKGs。

Result: 在检测、伪造识别和定位以及取证理由方面，FKG优于现有取证检测器和视觉语言模型。

Conclusion: FKGs是一个统一框架，有效地结合了取证证据、结构化推理和人类可解释的说明。
...
### [59] [EPEdit: Redefining Image Editing with Generative AI and User-Centric Design](https://arxiv.org/abs/2606.24057)
...
TL;DR: EPEdit is a user-friendly image editing application that leverages zero-shot Stable Diffusion models to perform diverse editing tasks without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing image editing tools like Photoshop require expertise, and generative AI solutions often involve costly retraining and fine-tuning.

Method: EPEdit integrates a backend framework with a front-end interface, using zero-shot image editing algorithms based on Stable Diffusion, guided by masks and prompts.

Result: User evaluations show EPEdit outperforms existing solutions in image editing, thematic design, and overall system performance.

Conclusion: EPEdit offers a cost-effective, user-friendly solution for comprehensive image editing.
...
### [60] [VisChronos: Revolutionizing Image Captioning Through Real-Life Events](https://arxiv.org/abs/2606.24058)
*Phuc-Tan Nguyen,Hieu Nguyen,Minh-Triet Tran,Trung-Nghia Le*

Main category: cs.CV

TL;DR: 提出VisChronos框架，利用历史事件知识增强图像描述生成，并引入EventCap数据集。


<details>
  <summary>Details</summary>
Motivation: 桥接视觉内容与自然语言理解的语义差距，解决传统方法在捕捉上下文叙述方面的局限性。

Method: 利用大型语言模型和密集字幕模型从单个图像中识别和描述真实事件，并构建EventCap数据集。

Result: 用户研究显示生成的描述准确、连贯且事件聚焦。

Conclusion: 为未来事件中心图像理解研究铺平道路。
...
### [61] [End-to-End Radar and Communication Modulation Recognition with Neuromorphic Computing](https://arxiv.org/abs/2606.24075)
*Xiaohu Li,Chongxiao Qu,Caiyong Lin,Chenxiao Dou,Wei Hua*

Main category: cs.CV

TL;DR: 提出EMRFormer，一种用于自动调制识别的尖峰驱动变压器架构，在神经形态硬件上实现高精度和超低能耗。


<details>
  <summary>Details</summary>
Motivation: 深度学习方法在自动调制识别中计算成本高，难以在资源受限平台上平衡精度与功耗，受神经形态架构启发，将其应用于AMR。

Method: 设计EMRFormer，包含自适应尖峰编码器和整数LIF神经元，通过尖峰可分离CNN（SSCNN）集成到尖峰驱动变压器（SpikeFormer）中，提取多尺度时间特征。

Result: 在多个数据集上达到最先进精度，低信噪比下保持强性能，理论能耗降低超90%；在KA200芯片上功耗比3090 GPU或Orin NX降低高达5倍。

Conclusion: 该工作为在资源受限设备上实现高效自动调制识别提供了有前景的途径。
...
### [62] [An LMM for Precisely Grounding Elements in Documents](https://arxiv.org/abs/2606.24118)
*Yijian Lu,Chuangxin Zhao,Kai Sun,Lei Hou,Juanzi Li,Ji Qi*

Main category: cs.CV

TL;DR: 提出了PreciseDoc模型，通过合成高质量文档数据和强化学习联合训练，显著提升文档中元素的精确空间定位和视觉问答能力。


<details>
  <summary>Details</summary>
Motivation: 现有LMM在文本丰富的文档图像中定位精度差，难以准确获取关键元素，限制了文档理解等应用。

Method: 包括两个数据合成管道生成带细粒度坐标的高质量文档，以及一个视觉接地推理训练范式，使用强化学习联合监督定位和推理。

Result: 在多个基准测试上，PreciseDoc在文档空间定位和理解任务上优于现有方法。

Conclusion: 证明了合成数据和联合训练方法能有效提升文档中精确元素定位和推理性能。
...
### [63] [Bengal-HP_RU: A Dataset of Bengal People For Head Pose Estimation](https://arxiv.org/abs/2606.24122)
...
### [65] [Tri-Efficient Transfer Learning for Point Cloud Videos](https://arxiv.org/abs/2606.24175)
*Yiding Sun,Dongxu Zhang,Jihua Zhu,Haozhe Cheng,Zhengqiao Li,Pengcheng Li,Chaowei Fang,Yonghao Dong,Lin Chen*

Main category: cs.CV

TL;DR: 提出PoinTriE框架，通过伪运动轨迹合成、几何运动对偶网络和轻量级侧网络，在点云视频理解中实现数据、参数和内存三方面效率。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调（PEFT）方法在点云视频理解中存在标注成本高和内存瓶颈问题；需要从现有数据挖掘更丰富监督信号，并大幅减少微调内存占用。

Method: 预训练时合成伪运动轨迹，配以文本和2D投影；提出几何运动对偶网络，进行多模态对比学习、刚体旋转预测和运动分布散度优化。微训时冻结骨干，仅更新由LoRA单元构建的轻量级时空侧网络，并采用梯度流掩蔽策略降低内存和参数。

Result: 在动作识别和语义分割任务上达到新的最先进结果。

Conclusion: PoinTriE成功解决了标注成本和内存瓶颈，实现了数据、参数和内存三方面的高效性。
...
### [66] [Trimming the Long-Tail of Visual World Modeling Evaluation](https://arxiv.org/abs/2606.24256)
*Bingxuan Li,Yining Hong,Cheng Qian,Hyeonjeong Ha,Jiateng Liu,Zhenhailong Wang,Yue Guo,Yunzhu Li,Heng Ji*

Main category: cs.CV

TL;DR: 提出Tailor-Bench基准，评估视觉世界模型在模拟不规则物理交互时的泛化能力，发现存在长尾差距，模型依赖表面视觉模式。


<details>
  <summary>Details</summary>
Motivation: 当前视觉世界模型主要模拟常见物理交互，但缺乏对罕见和不规则交互的泛化能力，引发对模型是否真正内化物理原理的疑问。

Method: 设计Tailor-Bench基准，包含常规、非常规、不可能三种场景模式，以及预测生成和描述生成两种任务设置。

Result: 模型性能从常规场景到非常规、不可能场景递减，图像模型无法正确模拟状态变化，视频模型存在时间不一致。

Conclusion: 现有视觉世界模型在物理交互泛化方面存在局限，主要依赖表面视觉模式而非深层物理原则。
...
### [67] [Hierarchical Spatial and Channel Aggregation for Cross-domain Few-shot Segmentation](https://arxiv.org/abs/2606.24296)
*Sujun Sun,Mingwu Ren,Haofeng Zhang*

Main category: cs.CV

TL;DR: 提出DHANet，通过双层次聚合和在线概率语义库解决跨域少样本分割中的语义和属性过对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有CD-FSS方法忽略跨域类语义粒度和判别属性的显著差异，导致语义过对齐和属性过对齐。

Method: 提出DHANet，包含HSA模块（空间层次聚合）、HCA模块（通道层次聚合）和OPSB模块（在线概率语义库）。

Result: 在四个目标域数据集上达到SOTA性能。

Conclusion: 验证了所提方法的有效性。
...
### [68] [Training-free Cross-domain Few-shot Segmentation via Robust Semantic Representation and Matching](https://arxiv.org/abs/2606.24297)
*Sujun Sun,Mingwu Ren,Haofeng Zhang*

Main category: cs.CV

TL;DR: 提出了一种无训练的跨域小样本分割框架，基于DINOv3和三个核心模块，在多个域上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖训练或微调，计算成本高且易过拟合；即使集成视觉基础模型，性能提升有限甚至下降。因此需要无训练框架以避免训练开销和过拟合。

Method: 构建无训练框架，基于自监督视觉编码器DINOv3，通过语义感知特征重新融合（SAFR）模块增强语义区分性，自适应支持增强（ASE）模块缩小支持与查询的语义差距，混合原型匹配（HPM）模块整合多种原型匹配结果适应不同语义复杂度。

Result: 在四个目标域数据集上的实验表明，无需任何训练即实现了跨域小样本分割的最先进性能。

Conclusion: 所提无训练框架有效避免了训练开销和过拟合风险，在跨域小样本分割任务中表现优异。
...
### [69] [Open-Vocabulary BEV Segmentation with 3D-Aware Geometric Constraints](https://arxiv.org/abs/2606.24353)
*Hojun Choi,Seulbin Hwang,Dae Jung Kim,Kisung Kim,Hyunjung Shim,Jinhan Lee*

Main category: cs.CV

TL;DR: We propose OVBEVSeg, a geometry-aware framework for open-vocabulary BEV segmentation using Gaussian splatting and 3D geometric constraints, achieving state-of-the-art performance with high efficiency.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art BEV perception methods are confined to closed-set scenarios, making them vulnerable to unpredictable real-world environments. This work introduces open-vocabulary BEV segmentation to recognize categories beyond the training set.

Method: OVBEVSeg enhances Gaussian splatting-based unprojection with robust 3D geometric constraints via three stages: (1) 2D-to-BEV pseudo-labeling using reliable 3D projection, (2) joint 2D-BEV per-scene optimization with BEV structural constraints, and (3) 3D geometric distillation for online efficiency.

Result: On nuScenes, OVBEVSeg outperforms closed-set methods by 15.3 mIoU on unseen categories, matches semi-supervised baselines trained with up to 40% annotations without novel-class labels, and achieves 2.5x faster inference with 0.22x memory consumption of projection-based methods.

Conclusion: OVBEVSeg demonstrates that geometry-aware constraints enable effective open-vocabulary BEV segmentation, significantly outperforming closed-set methods and matching semi-supervised approaches without novel class labels.
...
### [70] [MATCH: Flow Matching for Multi-View Anomaly Detection](https://arxiv.org/abs/2606.24375)
*Mathis Kruse,Melissa Schween,Bodo Rosenhahn*

Main category: cs.CV

TL;DR: 首个基于Flow Matching的多视角异常检测方法，在Real-IAD和MANTA-Tiny上达到SOTA，且可实时运行。


<details>
  <summary>Details</summary>
Motivation: 工业物体异常检测对提高生产效率至关重要，复杂物体需多视角分析，从而需多视角异常检测方法。

Method: 提出MATCH方法，利用Flow Matching的ODE形式估计似然得到异常分数，可进行物体、图像和像素级检测，并通过省略散度项实现实时处理。

Result: 在Real-IAD和MANTA-Tiny数据集上，异常检测和分割任务均达SOTA性能，且可在消费级硬件上运行。

Conclusion: 通过消融研究验证了方法选择，MATCH适用于实时生产场景。
...
### [71] [EgoSAT: A Comprehensive Benchmark of Egocentric Streaming Interaction Understanding](https://arxiv.org/abs/2606.24422)
*Yijia Lei,Jinzhao Li,Yichi Zhang,Jiacheng Hua,Yin Li,Miao Liu*

Main category: cs.CV

TL;DR: 提出了EgoSAT，第一个用于流式自我中心视频推理的全面基准，评估视觉语言模型在时间序列上的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在流式设置下缺乏对过去、现在和未来的统一推理能力，且存在严重的校准问题。

Method: 构建包含1,997个视频、4,800个问答对的基准，统一回顾、在线和前瞻任务，评估多种开源和闭源视觉语言模型。

Result: 现有模型在时间建模上表现不佳，且置信度与可回答性不匹配，常出现“自信地错误”行为。

Conclusion: 需要改进模型在流式设置下的时间推理和置信度校准。
...
TL;DR: 提出一种基于PTv3的流匹配方法PCFM用于医疗点云完成，在多个数据集上取得最先进生成性能，且采样步数更少。


<details>
  <summary>Details</summary>
Motivation: 医疗点云完成对于解剖重建和临床工作流重要，但生成建模研究不足。

Method: 采用连续时间生成建模，PCFM使用PTv3作为backbone的流匹配方法，并构建了确定性PTv3基线以及扩散完成基线。

Result: PCFM与确定性基线竞争，达到最先进生成性能，所需采样步数少于扩散，且PTv3提供高达7倍速度提升。

Conclusion: 研究表明流匹配方法在医疗点云完成中有效，且PTv3 backbone在效率和扩展性上具有优势。
...
TL;DR: P-MTP通过渐进课程损失和置信门控动态草稿，在文档解析中实现高达5倍加速的深度多令牌预测。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在文档解析中速度慢，多令牌预测优化不稳定，难以扩展到深度前瞻。

Method: 提出渐进课程损失（累积路径可靠性和回顾目标一致性自适应重加权）抑制梯度噪声，并采用置信门控动态草稿自适应校准推测长度。

Result: 跨多个基准和架构实现高达5倍加速，精度损失可忽略。

Conclusion: 首次在文档解析领域成功验证深度前瞻多令牌预测，显著加速推理。
...
### [74] [GeoIMO: Geometry-Driven Independent Motion Classification for Event Cameras](https://arxiv.org/abs/2606.24499)
*Anil Bayram Gogebakan,Filippo Marostica,Alessio Caviglia,Alessandro Savino,Stefano Di Carlo*

Main category: cs.CV

TL;DR: A geometry-driven, annotation-free framework that classifies objects as static or moving using event stream ego-motion, requiring no learning or manual labels.


<details>
  <summary>Details</summary>
Motivation: Existing event datasets rely on appearance-based annotations, which are poorly suited for motion-aware event perception, motivating a motion-based approach.

Method: A Focus of Expansion model with yaw compensation estimates global background motion; objects are labeled as moving when local motion deviates (scale-invariant residual). Temporal stabilization is applied. No learning or manual labels required.

Result: Experiments on MVSEC and Prophesee datasets show consistent performance; yaw compensation improves results during turns, and a simple translational model offers a favorable accuracy-efficiency trade-off.

Conclusion: The proposed framework effectively performs motion-aware classification without annotations, demonstrating robustness and practicality for automotive event data.
...
TL;DR: VisCritic是一种视觉过程奖励框架，通过对比操作前后截图来验证GUI代理动作，提升长时任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理在长时任务中缺乏步骤级验证，且现有过程奖励模型仅依赖文本推理，忽略视觉状态变化。

Method: VisCritic使用孪生视觉变换器提取变化感知表示，结合行动感知评论头评估动作成功性、任务进度和错误类型，并通过弱监督数据管道训练。

Result: 在五个基准上的实验表明，VisCritic作为即插即用增强模块，普遍提升GUI代理的基准指标，并提供视觉诊断线索。

Conclusion: VisCritic有效弥补了GUI代理在视觉验证上的空白，为长时任务提供可靠的过程奖励。
...
TL;DR: ForensicsTok reformulates image manipulation localization as autoregressive sequence generation, using a Token Splatting Decoder and Hierarchical Expert Fusion to improve precision and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM-based approaches use exogenous segmentation decoders leading to information bottlenecks and suboptimal localization due to reliance on semantic priors.

Method: ForensicsTok reformulates localization as autoregressive sequence generation. It uses a Token Splatting Decoder (TSD) to map tokens to binary masks via codebook-aware code smoothing, and a Hierarchical Expert Fusion (HEF) module to inject multi-scale forensic features.

Result: ForensicsTok substantially improves over MLLM baselines and slightly improves over forensic expert baselines, with stronger robustness to perturbations.

Conclusion: The unified architecture effectively compensates for lack of forensic priors in MLLMs and achieves state-of-the-art performance.
...
TL;DR: 提出PointVG-R，一种基于几何感知推理的指代视觉定位多模态大语言模型，结合强化学习和冷启动数据，在mIoU上超越基线15.86点。


<details>
  <summary>Details</summary>
Motivation: 传统方法忽略图像的几何信息，模型在解释指点手势的空间关系时存在认知脆弱性。

Method: 引入几何感知推理管道，构建EgoPoint-CoT数据集，通过监督微调和强化学习训练，并采用自适应重要性加权策略优化学习过程。

Result: 在指代视觉定位任务上达到SOTA，mIoU提升15.86点。

Conclusion: 提出的模块有效，验证了几何感知推理和强化学习结合的有效性。
...
TL;DR: 本文提出CF-World基准来评估文本到图像模型的反事实生成能力，发现模型在反事实设置下性能急剧下降，表明它们依赖视觉共现而非因果理解。


<details>
  <summary>Details</summary>
Motivation: 受Russell's inductivist turkey启发，旨在探究T2I模型是否具备真正的因果理解还是仅进行模式匹配。

Method: 提出CF-World基准，包含三个递进级别（事实、显式反事实、隐式反事实），使用VLM评估器CF-Eval，并引入两个指标：Prior Resistance Rate (PRR)和Reasoning Retention Rate (RRR)。

Result: 实验表明所有模型从事实到反事实设置性能急剧下降，失败源于世界知识与视觉外观紧密耦合，模型依赖训练数据中的频繁视觉共现。

Conclusion: 当前T2I模型缺乏因果理解，当需要反事实生成时，仍默认使用熟悉的常识先验。
...
TL;DR: 提出SPOFA框架，通过特征和梯度双重稳定机制解决异构知识蒸馏中的训练不稳定性问题，实现高精度且轻量。


<details>
  <summary>Details</summary>
Motivation: 异构知识蒸馏存在严重训练不稳定性，源于特征范数差异和梯度冲突这两个耦合的挑战。

Method: 提出特征和梯度双重稳定机制（SPOFA）：特征层面使用基于LayerNorm的解耦投影器，将特征大小与方向解耦；梯度层面使用动量驱动的指数移动平均动态缩放器，基于历史基线评估梯度冲突并惩罚有害信号。

Result: 在主流基准上取得最先进准确率，显著优于昂贵方法，且计算开销极小。

Conclusion: SPOFA通过轻量参数实现稳定蒸馏，有效解决异构知识蒸馏的稳定性问题。
...
### [80] [Multilevel Stochastic Plug-and-Play for Sparse-View CT Reconstruction](https://arxiv.org/abs/2606.24567)
*Antoine De Paepe,Alexandre Bousse,Dimitris Visvikis*

Main category: cs.CV

TL;DR: 提出多级随机Plug-and-Play方法加速稀疏视图CT重建，在保持质量的同时大幅减少运行时间。


<details>
  <summary>Details</summary>
Motivation: 现有PnP和随机PnP方法需要多次迭代才能收敛，限制了实际效率。

Method: 在多分辨率分析近似空间中执行多级步骤，利用小波分解避免估计细级先验梯度。

Result: 重建质量与最先进方法相当，同时显著减少运行时间。

Conclusion: ML-SPnP在SVCT重建中有效加速且保持质量。
...
### [81] [Jolia: Concept-Level Vision-Language Alignment for 3D CT Contrastive Learning](https://arxiv.org/abs/2606.24570)
...
TL;DR: ConQuer是一种针对3D医学图像的概念查询预训练方法，通过局部对齐提升CLIP，并在CT上达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP-style预训练将每个模态编码为单个全局token，可能丢失重要细节，而医学图像和报告具有多器官和结构化特点。

Method: ConQuer将报告分割成概念特定部分，使用交叉注意力查询学习局部对齐，无需分割掩码，对每个概念独立进行对比学习。

Result: 在胸部和腹部CT上训练的Jolia模型在发现分类、报告生成和跨中心迁移中超越CLIP基线，在多个公共基准上取得新SOTA。

Conclusion: ConQuer方法通过局部对齐提升了预训练效果，并提供了内置的空间可解释性。
...
TL;DR: 提出EERLoss，一种直接优化等错误率的损失函数，在击键动态验证中显著降低EER。


<details>
  <summary>Details</summary>
Motivation: 解决深度生物特征验证中优化目标与评估指标（EER）之间的错位问题。

Method: 引入EERLoss，一种次可微的EER精确近似，可适应DET曲线上任意操作点。

Result: 在KVC-onGoing基准上，相对EER降低约30%，收敛更快。

Conclusion: EERLoss作为任务对齐训练目标有效，特别适用于高方差生物特征。
...
### [83] [Agentic Collaborative Cognition for Zero-Shot 3D Understanding](https://arxiv.org/abs/2606.24649)
*Wenxin Wang,Bo Zhang,Feng Chen,Zixuan Wang,Wen Li,Changsheng Li,Yinjie Lei*

Main category: cs.CV

TL;DR: 提出协作多智能体框架，包括规划智能体和感知智能体，通过闭环迭代完成零样本3D理解，在多个基准上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法的视频观测视角有限且对3D场景的感知隐式，导致性能瓶颈。

Method: 规划智能体负责高层视角规划并补充缺失视角，感知智能体将3D场景显式总结为结构化认知地图，并通过跨视角一致标识符整合碎片化观察，同时提供反馈过滤不匹配候选，两者迭代协作直至任务完成。

Result: 在6个基准上达到SOTA，ScanRefer上Acc@0.5提升11.1%，3D辅助对话上BLEU-1提升14.6%，SQA3D上EM提升2.1。

Conclusion: 该协作多智能体框架有效提升了零样本3D理解的性能。
...
### [84] [SER: Learning to Ground Video Reasoning with Semantic Evidence Rewards](https://arxiv.org/abs/2606.24726)
*Sheng Xia,Zhengqin Lai,Tianxiang Jiang,Kanghui Tian,Shoujun Zhou,Bin Li,Yi Wang*

Main category: cs.CV

TL;DR: 提出语义证据奖励(SER)用于增强视频多模态大模型的细粒度时空推理和证据定位。


<details>
  <summary>Details</summary>
Motivation: 现有视频MLLM在细粒度时空推理中常基于无关帧或物体生成答案，且现有RL框架仅使用几何奖励(IoU)，对边界扰动敏感并忽略语义对齐。

Method: SER将时空证据定位重构为约束验证任务，使用裁判VLM作为局部检查器评估模型生成证据的相关性和定位质量，并结合时间惩罚。

Result: 在V-STAR基准上，SER达到49.6% mLGM，比强基线Open-o3-Video提升3.0分。

Conclusion: SER展示了提升答案准确性和证据定位的潜力。
...
TL;DR: 提出了自适应Hebbian路由用于少样本视觉Transformer，通过MLP路由器动态控制记忆行为，提升了少样本分类准确率并降低了推理时间。


<details>
  <summary>Details</summary>
Motivation: 固定Hebbian记忆行为不适合所有少样本任务，需要自适应机制。

Method: 使用轻量级MLP路由器根据支持集特征控制Hebbian记忆的贡献、更新强度和记忆保留，提出了三种自适应变体：自适应放置、自适应可塑性、完全自适应路由。

Result: 在Omniglot和CIFAR-FS数据集上，自适应变体优于固定Hebbian基线，完全自适应路由达到最高准确率96.94%，并减少推理时间。

Conclusion: 自适应可塑性和记忆激活能够改进少样本Transformer表示，验证了动态记忆路由的有效性。
...
TL;DR: 提出GradAudit，一种基于梯度的审计框架，通过分析模型内部优化动态来检测VLLM中的训练数据，有效解决跨模态检测和隐私保护问题。


<details>
  <summary>Details</summary>
Motivation: 现有训练数据检测方法在跨模态场景下失效或判别力不足，且医疗领域对隐私保护要求严格，需要更可靠的审计方法。

Method: GradAudit利用模型参数在训练样本上梯度稳定对齐、在非训练样本上梯度嘈杂不一致的现象，通过分析梯度签名来分离训练数据并检测图像-文本关联。

Result: 在医疗和通用数据集上，GradAudit在预训练和微调VLLM中显著优于现有基线；案例研究显示现有方法低估了版权数据使用，且低估程度随模型先进性增加。

Conclusion: GradAudit提供了有效的训练数据检测手段，揭示了现有方法的局限性，对版权保护和数据溯源具有重要价值。
...
### [87] [EG-VQA: Benchmarking Verifiable Video Question Answering with Grounded Temporal Evidence](https://arxiv.org/abs/2606.24797)
*Linpeng Huang,Weixing Chen,Zexin Chen,Yang Liu,Liang Lin*

Main category: cs.CV

TL;DR: 提出EG-VQA基准和EG-Reasoner模型，揭示证据定位对视频QA至关重要，仅靠规模不足以保证稳健理解。


<details>
  <summary>Details</summary>
Motivation: 现有Video-LLMs在VideoQA上表现良好，但基准仅评估答案正确性，忽略证据定位，导致生成与理解脱节。

Method: 构建EG-VQA基准（2076视频，11838QA对，细粒度证据标注），提出EG-F1指标联合衡量时间对齐与语义一致性，并训练EG-Reasoner模型使用显式证据监督。

Result: 强模型难以准确证据定位，EG-Reasoner在开源模型中达SOTA，与专有系统竞争力强，尤其在反事实问题上表现突出。

Conclusion: 表明规模不足以稳健视频理解，结构化证据监督是开发可靠可解释VideoQA系统的关键。
...
### [88] [OrbitForge: Text-to-3D Scene Generation via Reconstruction-Anchored Video Synthesis](https://arxiv.org/abs/2606.24799)
*Chenrui Fan,Paolo Favaro*

Main category: cs.CV

TL;DR: OrbitForge利用冻结的视频先验和每提示高斯泼溅优化，将文本生成视频转换为标准闭环3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有文本生成视频无法直接用于3D资产，存在相机控制难、视图覆盖不全、帧间不一致等问题。

Method: 使用3D重建作为锚点，通过可变形高斯泼溅和MedianGS获取初步3D重建，从规定轨道渲染检测缺失视角，再用文本到视频模型补全缺失视图，最后重建完整轨道得到最终高斯泼溅场景。无需任务特定微调或分数蒸馏。

Result: 在T3Bench审计中，达到359.0度中位跨度，Q10 ImageReward从8.07提升至16.36，与VideoMV竞争。

Conclusion: 该工作强调覆盖感知评估的重要性，表明OrbitForge能有效提升文本到3D的覆盖一致性和质量。
...
### [89] [DDStereo: Efficient Dual Decoder Transformers for Stereo 3D Road Anomaly Detection](https://arxiv.org/abs/2606.24805)
*Shiyi Mu,Zichong Gu,Zhiqi Ai,Yilin Gao,Shugong Xu*

Main category: cs.CV

TL;DR: DDStereo 是一种双解码器立体 Transformer，用于实时开放集 3D 对象检测，在封闭集和开放集协议下均达到最先进精度，并首次实现与单目方法相当的实时速度。


<details>
  <summary>Details</summary>
Motivation: 立体 3D 检测在实时性能和开放集泛化方面存在挑战：现有立体方法精度高但速度慢，不适合实时应用；开放集检测在立体领域尚未探索。

Method: 提出 DDStereo，包含两个轻量解码器分支：一个用于开放集前景 2D 检测，另一个用于 3D 属性回归，通过共享对象级查询实现统一目标级对齐。设计了紧凑的视差特征提取器和简化的解码器架构以提高推理效率。

Result: 在公共立体 3D 基准上，DDStereo 在封闭集和开放集协议下均达到最先进精度，推理速度超过现有立体 3D 检测器，首次实现与单目方法相当的实时性能。

Conclusion: DDStereo 有效解决了立体 3D 检测的实时性和开放集问题，为立体开放集检测提供了新方向。
...
TL;DR: 提出GeoT2V-Bench，一个基于重建的诊断基准，用于评估相机提示的T2V片段是否支持刚性3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅关注视觉合理性，忽略了生成帧应为静态3D场景提供多视图一致性的要求。

Method: 使用VGGT估计每帧相机内参和姿态，拟合DeformableGS，通过时间中值聚合得到静态MedianGS代理，沿相机路径渲染并比较。

Result: 在3840个重建中发现可见运动、静态渲染误差、流一致性和灵活-静态行为经常不一致。

Conclusion: GeoT2V-Bench能够捕捉生成视频作为全局静态场景采集时出现的互补失败模式。
...
TL;DR: Proposes Riemannian Residual Line Search for one-step diffusion editing, selecting among candidates by maximizing CLIP alignment.


<details>
  <summary>Details</summary>
Motivation: One-step diffusion editors need to balance target prompt satisfaction and source image preservation, but no fixed update strength works for all edits.

Method: Riemannian Residual Line Search: estimate local time curvature of prompt-delta field, project corrected direction onto original first-order update norm, form residual path from source to strong edit, keep original output as candidate, select by CLIP alignment.

Result: Achieves state-of-the-art performance among one-step update algorithms on 700-sample PIE-Bench++ across 10 edit types.

Conclusion: Treating edit strength as a post-hoc candidate selection problem on energy-field transport yields better results than fixed strength updates.
...
TL;DR: 使用ERP投影和RAFT+EACS实现球面立体图像的实时准确视差估计。


<details>
  <summary>Details</summary>
Motivation: 全向立体图像违反经典视差估计的几何假设，极线对应沿大圆路径弯曲，产生二维位移，无法在几何校正前作为单轴视差处理。

Method: 采用球面到ERP投影作为预处理，使极线变直恢复一维视差；然后使用RAFT+EACS框架，将RAFT稠密光流通过保留基线对齐分量缩减为视差。

Result: 在合成鱼眼立体数据集上，该管线实时生成准确、平滑、结构一致的视差图。

Conclusion: 确认ERP预处理与RAFT+EACS结合可有效从球面立体进行视差估计，为扩展传统立体管线到360成像提供直接途径。
...
### [93] [FLUX3D: High-Fidelity 3D Gaussian Generation with Diffusion-Aligned Sparse Representation](https://arxiv.org/abs/2606.24874)
*Haorui Ji,Weizhe Liu,Hongdong Li,Hengkai Guo*

Main category: cs.CV

TL;DR: FLUX3D通过扩散对齐结构潜在表示和稀疏结构感知扩散框架，改善图像到3DGS生成中的高频细节保留和2D-3D对齐。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏体素方法因使用判别式2D特征和缺乏跨模态对齐机制，难以保留高频视觉细节。

Method: 提出FLUX3D，包括（1）扩散对齐结构潜在表示（DA-SLAT）与仅解码器架构；（2）稀疏结构感知扩散框架，含稀疏结构多模态扩散变换器（SMDiT）和模态感知旋转位置嵌入（MARoPE）。

Result: 在基准实验中，FLUX3D在视觉保真度上大幅提升，显著优于所有SOTA方法生成高质量3DGS资产。

Conclusion: FLUX3D有效提升了表示学习和跨模态对齐，成为可扩展的图像到3DGS生成框架。
...
### [94] [Sol Video Inference Engine: Agent-Native Full-Stack Acceleration Framework for Efficient Video Generation](https://arxiv.org/abs/2606.23743)
*Yitong Li,Junsong Chen,Haopeng Li,Haozhe Liu,Jincheng Yu,Ligeng Zhu,Ping Luo,Song Han,Enze Xie*

Main category: cs.CV

TL;DR: 提出了Sol Video Inference Engine，一个基于代理的、无需训练的加速框架，通过组合缓存、稀疏注意力、令牌剪枝、量化和核融合五种技术，针对特定实例优化，在多种视频扩散模型上实现超过2倍加速且质量近无损。


<details>
  <summary>Details</summary>
Motivation: 现有加速方法高度依赖具体实例，不同模型、硬件和推理配置下的最优策略不同，手动调优成本高，因此需要自动化的实例特定加速框架。

Method: Sol Video Inference Engine，一个代理框架，包含并行技能代理优化五种加速技术（缓存、稀疏注意力、令牌剪枝、量化、核融合）的实现，代理集成器组合成全局加速栈，并有人工验证者提供反馈。

Result: 在三款视频扩散模型（64B Cosmos3-Super, 22B LTX-2.3, 2B SANA-Video）上，实现了超过2倍的端到端加速，同时保持近无损的VBench质量。

Conclusion: 代理框架对于视频扩散加速是有效的，能够以少量人工努力实现实例特定的高性能加速。
...
### [95] [DivRL: Disentangled Self-Similarity Rewards for Diverse Subject-Driven Generation](https://arxiv.org/abs/2606.23950)
*Qian Wang,Zhenyu Li,Abdelrahman Eldesokey,Peter Wonka*

Main category: cs.CV

TL;DR: 提出DivRL框架，通过门控优化同时提升身份一致性和结构多样性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在身份保持和多样性之间存在矛盾，强身份保持导致低多样性。

Method: 基于解耦视觉特征，引入负自相似度量（nSSM）和视觉语义匹配（VSM），采用“探索-抑制”策略，将身份保持作为可行性约束。

Result: 在保持身份一致性的前提下，显著提高结构多样性。

Conclusion: 门控优化公式有效地将身份保持从竞争目标转换为可行性约束，实现联合优化。
...
### [96] [Token-to-Token Alignment of Text Embeddings for Semantic Blending](https://arxiv.org/abs/2606.24021)
*Saar Huberman,Ron Mokady,Or Patashnik,Daniel Cohen-Or*

Main category: cs.CV

TL;DR: 提出Token-to-Token对齐框架，通过结构对齐和嵌入对齐实现文本提示间的语义对应，使线性插值成为有意义的操作，支持图像混合和连续编辑。


<details>
  <summary>Details</summary>
Motivation: 现代生成模型中，图像通过文本提示控制，但token序列缺乏一致结构，语义相似的图像对应不同序列，而相似序列可能编码不同语义，阻碍平滑过渡和连续控制。

Method: 两阶段对齐：结构对齐将提示改写为共享结构形式，使对应概念位于一致位置；嵌入对齐基于语义相似度匹配token表示。

Result: 对齐后，简单线性插值产生平滑连贯的语义过渡，支持图像混合和连续编辑。

Conclusion: 文本嵌入空间隐式编码连续语义结构，通过正确对齐可访问，语义控制可通过组织现有表示实现，而非修改生成模型。
...
### [97] [Ingredient-Level Food Image Segmentation for Nutrition Awareness](https://arxiv.org/abs/2606.24059)
*Jonesh Shrestha*

Main category: cs.CV

TL;DR: Fine-tuning SegFormer variants on FoodSeg103 for ingredient-level semantic segmentation, with B1 achieving 0.7929 pixel accuracy and 0.3204 mean IoU, and converting predictions to ingredient-area percentages for nutrition awareness.


<details>
  <summary>Details</summary>
Motivation: Single dish labels for entire food images hide visual structure; ingredient-level segmentation reveals multiple visible ingredients.

Method: Fine-tuned SegFormer-B0 and SegFormer-B1 on FoodSeg103 with ImageNet-pretrained MiT backbones and new 104-class output layers; predicted masks converted to ingredient-area percentages.

Result: SegFormer-B0: 0.7709 pixel accuracy, 0.2521 mean IoU; SegFormer-B1: 0.7929 pixel accuracy, 0.3204 mean IoU (+0.0683 gain in mean IoU).

Conclusion: Ingredient-area percentages provide a visual first-pass nutrition awareness cue, but are not direct estimates of calories or portion sizes.
...
### [98] [Universal Guideline-Driven Image Clustering via a Hybrid LLM Agent](https://arxiv.org/abs/2606.24094)
...
TL;DR: 第一个通过文本指南统一图像聚类场景的通用框架，提出生成概念代理建模和基于最小生成树的LLM遍历，在多种任务上优于专门方法。


<details>
  <summary>Details</summary>
Motivation: 不同聚类场景之间存在根本性差距，使得统一图像聚类具有挑战性。

Method: 提出Guideline-Driven Image Clustering Agent，通过文本指南统一聚类；采用Generative Concept Proxy Modeling生成指南感知嵌入；引入基于最小生成树的LLM Traversal进行自动聚类发现。

Result: 在从一般到细粒度、全局到局部、平衡到长尾分布等多种聚类场景中，该框架一致优于专门方法。

Conclusion: 该框架是通用的，能够有效桥接不同聚类场景，并取得优异性能。
...
### [99] [Beyond Bayer: Task-Optimal Sensor Co-Design for Robust Autonomous-Driving Segmentation](https://arxiv.org/abs/2606.24096)
...
### [100] [Autonomous Video Generation with Counterfactual Controllability for Self-Evolving World Models](https://arxiv.org/abs/2606.24152)
*Xin Wang,Wenxuan Liu,Tongtong Feng,Wenwu Zhu*

Main category: cs.CV

TL;DR: 视频生成模型学习部分、隐式的时空世界模型，但并非完全接地或可控；自主视频生成与反事实可控性是实现自我进化世界模型的有前途方式。


<details>
  <summary>Details</summary>
Motivation: 现有文献声称视频生成本质上是世界建模，但依赖缩放视觉预测自动得到物理代理是危险的主张，需要更准确的陈述。

Method: 提出新视角：将自主视频生成与反事实可控性相结合，作为实现自我进化世界模型的方法。

Result: 反事实可控性是关键，能测试生成未来是否可生存于具体化约束并将行动知识反馈回未来想象。

Conclusion: 自主视频生成与反事实可控性是实现自我进化世界模型的一种有前途方式。
...
### [103] [Towards Fast and Effective Long Video Understanding of Multimodal Large Language Models via Adaptive Quasi-Gaussian Sampling](https://arxiv.org/abs/2606.24187)
*Kun Zhang,Chenxin Fang,Tao Chen,Baiyang Song,Yunhang Shen,Yiyi Zhou,Rongrong Ji*

Main category: cs.CV

TL;DR: 提出AdaQ，一种无需训练的自适应帧采样方法，基于准高斯采样的3-σ规则，用于长视频理解中的关键帧选择，以提升性能并降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 长视频理解中，MLLMs面临计算和内存问题，现有关键帧选择方法灵活性低、噪声高。

Method: 将视频帧选择定义为准高斯采样问题，提出AdaQ，基于高斯分布的3-σ规则，自适应调整3-σ区间以适应当地查询和全局查询，实现鲁棒和自适应的帧采样。

Result: 在四个MLLM和三个嵌入模型上实验，AdaQ性能提升显著，例如Qwen3-VL-8B仅用64帧平均超过GPT4o 15.8%，且鲁棒性强，仅需一个超参数。

Conclusion: AdaQ是一种高效、鲁棒的长视频理解方法，通过自适应帧采样显著提升MLLM性能。
...
### [104] [Inclusive Interactive Collisions for Multi-View Consistent Compositional 3D Generation](https://arxiv.org/abs/2606.24206)
*Chang Liu,Mingwen Shao,Xiang Lv,Xinyuan Chen,Lingzhuang Meng,Qiao Zhang,Zhengyi Gong,Jinghao Hu*

Main category: cs.CV

TL;DR: 提出I2C-3D，一种基于优化的方法，通过包容性交互碰撞策略和多视图自适应分数蒸馏采样，生成多视图一致且交互合理的组合3D资产。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要生成单个3D物体，难以处理多物体组合场景且存在跨视图不一致问题。

Method: 提出包容性交互碰撞策略引导高斯基元合理交互，并设计多视图自适应分数蒸馏采样从预训练扩散模型中蒸馏多视图一致先验和布局先验。

Result: I2C-3D在生成质量和多视图一致性上优于现有方法。

Conclusion: I2C-3D能生成高保真多视图一致的组合3D资产，并支持灵活3D编辑，促进复杂场景生成。
...
### [105] [Geometry-Instructed Video Editing](https://arxiv.org/abs/2606.24225)
*Chirui Chang,Xiaoyang Lyu,Yi-Hua Huang,Haoru Tan,Shizhen Zhao,Yikang Ding,Jianmin Bao,Xin Tao,Pengfei Wan,Xiaojuan Qi*

Main category: cs.CV

TL;DR: GIVE is a geometry-instructed video editing framework that uses a unified object-state formulation and synthetic data to achieve reliable object-level geometric edits with temporal coherence and consistent secondary effects.


<details>
  <summary>Details</summary>
Motivation: Object-level geometric edits in video remain unreliable due to challenges in specifying 3D state changes unambiguously while updating secondary effects like shadows and reflections.

Method: GIVE represents edits via a unified object-state formulation using two video-aligned geometry streams (depth-box and orientation-box) for pre/post specification, and trains on paired data from a scalable graphics-engine pipeline.
...
Conclusion: GIVE provides a reliable unified framework for video geometry editing, enabling consistent secondary effects and generalization to real-world videos.
...
### [106] [M^2C-EvDet: Multi-Domain Multi-Order Cross-Modal Knowledge Distillation for Event-based Object Detection](https://arxiv.org/abs/2606.24248)
*Wei Bao,Siqi Li,Shouan Pan,Yi Xie,Yue Gao*

Main category: cs.CV

TL;DR: 提出M^2C-EvDet，一种用于事件目标检测的多域多阶跨模态知识蒸馏框架，结合频域学习和超图计算。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法仅关注空间视觉语义或成对关系信息，限制了在复杂场景下的性能。

Method: 包含自适应频率解耦特征蒸馏（AF^2D^2）和多阶关系蒸馏（MORD）两个模块，基于频域学习和超图计算。

Result: 该方法有效缩小了事件检测与帧检测之间的性能差距，取得了优越的结果。

Conclusion: M^2C-EvDet证明了多域多阶蒸馏可以增强事件目标检测。
...
### [107] [TuringViT: Making SOTA Vision Transformers Accessible to All](https://arxiv.org/abs/2606.24253)
...
TL;DR: TuringViT通过线性注意力、监督丰富的图像-视频数据策展和原生动态分辨率预训练，仅用10%的数据量即可训练出超越开源基线的SOTA级视觉Transformer，显著降低社区定制化ViT的门槛。


<details>
  <summary>Details</summary>
Motivation: 现有VLM/VLA系统依赖现成ViT，但下游需求多样需定制化SOTA级ViT，而训练成本高昂，标准注意力使高分辨率预训练不可行，通常用低分辨率预训练后适应。

Method: TuringViT包括三个关键设计：Turing Linear Attention (TLA)高效序列建模、VISTA-Curation构建监督丰富的图像-视频训练数据、原生动态分辨率预训练支持灵活输入并平滑迁移到下游VLM。

Result: 仅用10%的数据即超越领先开源ViT基线，在下游VLM上性能更强，高分辨率输入延迟缩放更优，缩放定律分析显示持续预测性提升。

Conclusion: TuringViT实现了快速适应、硬件友好和高效部署，成为统一视觉基础，其可复现流水线大幅降低了社区训练和部署SOTA ViT的成本，推动ViT普及。
...
### [108] [Social Structure Matters in 3D Human-Human Interaction Generation](https://arxiv.org/abs/2606.24255)
*Zhongju Wang,Beier Wang,Yatao Bian,Pichao Wang,Zhi Wang,Daoyi Dong,Hongdong Li,Huadong Mo,Zhenhong Sun*

Main category: cs.CV

TL;DR: 提出Solo-to-Social框架，结合LLM推理与运动技能生成文本驱动的3D人际交互运动。


<details>
  <summary>Details</summary>
Motivation: 文本到单人生成已成熟，但交互需要建模社会结构，LLM能推理但不能直接生成运动。

Method: LLM规划器分解交互阶段、分配角色并对齐序列；运动执行器通过LoRA、前一阶段自条件和自我-相对伙伴条件，将单人生成模型适应为双人协调运动。

Result: 生成的3D交互运动在阶段一致性、角色对齐和伙伴感知协调上表现更优。

Conclusion: 该框架有效桥接社会组织和运动实现，提升交互生成质量。
...
### [109] [MotifGen: Spatiotemporal interpolation of misaligned satellite images via multi-source generative modeling, in an application to tropical cyclones](https://arxiv.org/abs/2606.24263)
*Clément Dauvilliers,Claire Monteleoni*

Main category: cs.CV

TL;DR: 首个结合多源微波和红外数据的生成模型，用于热带气旋微波图像插值。


<details>
  <summary>Details</summary>
Motivation: 微波卫星重访时间长，可能错过快速演变，需要插值方法但数据高度异质。

Method: 构建生成模型，采用自监督任务（随机源掩码重建）训练，结合红外和微波数据。

Result: 显著降低CRPS，结合红外改进，集合平均与确定性模型相当，功率谱更接近真实观测。

Conclusion: 首次实现多微波仪器与红外观测在时间不规则下的微波图像插值生成模型。
...
### [110] [Ill-Posed by Design: Probing Evidence Use in VLMs](https://arxiv.org/abs/2606.24335)
*Boaz Meivar,Shaked Perek,Shani Shvartzman,Eli Schwartz,Shai Avidan*

Main category: cs.CV

TL;DR: 通过反事实分析，发现视觉语言模型在度量物体尺寸估计中主要依赖目标身份线索，即使大型模型也未能有效利用场景几何信息，且性能落后于纯文本模型。


<details>
  <summary>Details</summary>
Motivation: 现有反事实分析在证据使用诊断中因多线索支撑同一答案而受限，需要病态任务（如单目度量尺寸估计）来揭示模型依赖关系。

Method: 构建Metric VQA数据集（含10,813个维度查询和331个野外场景），评估12个开源视觉语言模型（3B-397B参数），并进行六通道反事实分析（视觉与语言证据）。

Result: 最大VLM（如Qwen3-VL-235B）在野外场景中落后于纯文本大语言模型；目标身份是最关键线索，目标像素和局部上下文仅部分有效，表观大小影响预测但无方向性，全局场景几何几乎未被利用。

Conclusion: LoRA微调表明度量尺寸任务可学习，但模型未学会利用场景几何；需进一步研究如何引导模型融合几何线索。
...
### [111] [Structural Kolmogorov-Arnold Convolutions: Learnable Function on the Values or the Filter Shape as Parameter-Efficient Alternative to Per-Edge Convolutional KANs](https://arxiv.org/abs/2606.24371)
...
TL;DR: 提出将可学习函数置于卷积结构而非每条边上的卷积KAN变体，其中RF-KAN和SV-KAN在CIFAR上以约0.4M参数超越普通卷积和每边KAN。


<details>
  <summary>Details</summary>
Motivation: 现有卷积KAN在每个核条目上附加可学习函数导致参数量大和过拟合，需要更高效的结构。

Method: 沿函数作用于像素值或滤波器形状的轴设计空间，提出三种实现：SV-KAN（共享值函数）、AG-KAN（共享值+内容自适应高斯门）、RF-KAN（基于Morlet小波基学习滤波器形状）。

Result: 在CIFAR-10和CIFAR-100上，RF-KAN和SV-KAN分别达到88.47%和64.40%准确率，性能接近且远优于普通卷积和每边KAN，参数仅约0.4M。

Conclusion: 学习形状是关键因素，RF-KAN的增益源于局部振荡小波基和内容自适应性，移除学习形状导致准确率大幅下降。
...
### [114] [Quantum CT via Dynamic Interval Encoding and Prior-Balanced QUBO Reconstruction](https://arxiv.org/abs/2606.24561)
*Ao Wang,Yikuang Yuluo,Yujie Liu,Shuangyang Zhong,Yuwen Zhang,Zihao Wang,Fenglin Liu,Andreas Maier,Haijun Yu,Yixing Huang*

Main category: cs.CV

TL;DR: 提出一种结合动态区间编码与先验平衡优化的QUBO灰度CT重建框架，在保持精度的同时降低变量开销，并在D-Wave混合求解器上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 固定全局位平面编码在提升灰度精度时会急剧增加QUBO规模和耦合复杂度，而低位编码则引入量化误差，限制了QUBO在灰度CT中的应用。

Method: 动态区间编码仅在当前估计的局部灰度区间内编码活动像素，配合边界命中引导的更新规则自适应切换搜索与细化；同时平衡投影数据一致性与边缘保持二次先验以增强优化稳定性。

Result: 在稀疏视图和有限角度扇形束CT实验中，该方法在结构和灰度分布恢复上优于分析、迭代、变分及表示基线方法；消融研究证实改进源于动态局部编码与稳定数据保真-先验耦合。

Conclusion: 该QUBO公式可直接在D-Wave混合量子-经典后端执行，展现了量子计算在灰度CT重建中的实用潜力。
...
### [115] [VSANet: View-aware Sparse Attention Network for Light Field Image Denoising](https://arxiv.org/abs/2606.24737)
*Gargi Panda,Soumitra Kundu,Saumik Bhattacharya,Aurobinda Routray*

Main category: cs.CV

TL;DR: Proposes VSANet, a view-aware sparse attention network for light field denoising that uses locality-sensitive hashing-based sparse attention for efficient cross-view feature aggregation and a feature refinement module to enhance informative features.


<details>
  <summary>Details</summary>
Motivation: Light field denoising is challenging due to high-dimensional data, and existing methods do not fully exploit cross-view correlations efficiently, especially with global interactions.

Method: VSANet introduces a view-aware sparse attention (VSA) block that performs cross-view aggregation via locality-sensitive hashing-based sparse attention in a unified spatial-angular token space, and a feature refinement (FR) block to emphasize spatial, angular, and epipolar features. These are integrated into a sequential attention refinement module.

Result: VSANet outperforms state-of-the-art light field denoising methods on benchmark datasets.

Conclusion: The proposed approach effectively handles the high dimensionality of light fields by enabling global feature interactions with linear complexity, achieving superior denoising performance.
...
### [116] [IV-CoT: Implicit Visual Chain-of-Thought for Structure-Aware Text-to-Image Generation](https://arxiv.org/abs/2606.24849)
...
TL;DR: 提出隐式视觉思维链（IV-CoT），通过将视觉条件分解为结构到语义的级联，在单次前向传递中实现结构感知的图像生成。


<details>
  <summary>Details</summary>
Motivation: 统一的多模态大语言模型在结构感知提示遵循（如对象计数、空间关系等）上表现不佳，原因是结构规划和外观渲染在单个条件流中纠缠。

Method: IV-CoT将视觉条件查询分解为结构到语义的级联：结构查询形成潜在视觉规划，语义查询根据规划渲染外观；并引入仅训练时使用的草图监督以引导结构查询捕获结构信息。

Result: 在GenEval和T2I-CompBench上取得优越结果；可视化显示结构和语义查询在结构感知生成中互补。

Conclusion: IV-CoT通过隐式链式推理有效解耦结构规划与外观渲染，提升结构感知生成能力。
...
### [117] [Do LLM Attribution Metrics Transfer? Auditing Retrieval-Augmented Generation Evaluation Across Datasets and Constructs](https://arxiv.org/abs/2606.23915)
*Tianyu Ding,Aditya Nannapaneni,Juan Pablo De la Cruz Weinstein*

Main category: cs.CL

TL;DR: 自动度量在LLM归因中不稳定，无统一最佳，需针对目标数据集验证。


<details>
  <summary>Details</summary>
Motivation: 探究自动评分器在不同评估结构中的可转移性，因为实践常将它们视为可互换。

Method: 审计8种自动评分器（词汇、嵌入、BERTScore、蕴含/基础训练模型），在三个评估结构（渊源/主题性、生成答案归属、事实核查蕴含）上测试，特别是生成答案归属多个数据集。

Result: 生成答案归属中，无评分器在所有数据集保持最佳；指标排名反转（Kendall tau=-0.64）；NLI在短声明AUROC 0.90但在长格式0.53；BERTScore长格式0.91；'best-on-average'规则失败，平均持出遗憾0.172 AUROC。

Conclusion: 度量选择必须在目标数据集上验证，不能从其他数据集学习；提示型LLM评判避免崩溃但非统一最佳且更昂贵。
...
### [118] [When Retrieval Metrics Mislead: Measuring Policy Signal in Long-Horizon Tool-Use Agents](https://arxiv.org/abs/2606.23937)
*Tianyu Ding,Juan Pablo De la Cruz Weinstein*

Main category: cs.CL

TL;DR: Exact-match clause recall can underestimate downstream policy utility in tau-bench; retrieval-augmented classifiers achieve near-gold-policy performance.


<details>
  <summary>Details</summary>
Motivation: To test whether exact-match retrieval recall is a reliable proxy for the usefulness of retrieved policy context in downstream decision making.

Method: Used Qwen2.5-3B/7B classifiers on tau-bench for pre-action policy classification, compared macro-F1 under gold-policy conditioning, retrieved clauses, mismatched-policy, and no-policy controls.

Result: Retrieved-clause classifiers achieved macro-F1 0.58 vs 0.60 with gold clauses (Delta=-0.02, 95% CI [-0.23,+0.21]), while mismatched and no-policy controls scored 0.32 and 0.21; exact match recall was only 7%.

Conclusion: Exact-match clause recall can underestimate downstream policy utility, motivating evaluation with retrieved policies in the classification loop rather than recall alone.
...
### [119] [RASC+: Retrieval-Constrained LLM Adjudication for Clinical Value Set Authoring](https://arxiv.org/abs/2606.23992)
*Sumit Mukherjee*

Main category: cs.CL

TL;DR: 提出检索约束的LLM裁决方法，通过候选池构建和LLM选择，在临床价值集完成任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 直接零样本LLM生成不适合临床代码系统，因为代码系统大、版本控制、难以记忆。

Method: 采用阶段式方法：先优化候选池构建（使用Qwen3检索、词汇感知扩展、代码显示救援检索），再用约束LLM裁决器（GPT-5）进行候选选择。

Result: 候选池召回率从0.553提升到0.730；使用GPT-5裁决后，全测试宏F1为0.549，保持出版商宏F1为0.533。

Conclusion: 检索约束的LLM裁决可以改善价值集完成，同时保持所有返回代码来自可审计候选池的安全性。
...
### [120] [MMed-Bench-IR: A Heterogeneous Benchmark for Multilingual Medical Information Retrieval](https://arxiv.org/abs/2606.24200)
*Junhyeok Lee,Han Jang,Hyeonjin Goh,Kyu Sung Choi*

Main category: cs.CL

TL;DR: 提出了MMed-Bench-IR基准，用于评估多语言医学检索中的三个关键能力，发现现有系统在跨语言上表现严重下降。


<details>
  <summary>Details</summary>
Motivation: 现有基准单独评估多语言医学检索的各个能力，未测量生物医学专业知识与多语言覆盖的交互。

Method: 构建了包含6种语言、三个任务（跨语言医学QA检索、概念辨别、多语言证据检索）的基准MMed-Bench-IR，任务间无重叠。

Result: 评估10个系统，发现生物医学编码器在英语上nDCG@10为0.818，但在日语上仅为0.056，跨语言失败严重。

Conclusion: 多语言医学检索需要更好的跨语言能力，英语专用基准无法检测此类差距。
...
### [121] [AVOC: Enhancing Hour-Level Audio-Video Understanding in Omni-Modal LLMs via Retrieval-Inspired Token Compression](https://arxiv.org/abs/2606.24286)
*Yijing Chen,Wenhui Tan,Xiaoyi Yu,Yuyue Wang,Xin Cheng,Kaisi Guan,Hao Jiang,Xiangyang Li,Guojie Zhu,Ruihua Song*

Main category: cs.CL

TL;DR: 提出AVOC框架，通过将令牌压缩视为top-K检索问题并集成相关性、重要性、多样性标准，实现长形式音频-视频理解在OmniModal LLM中的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有长形式音频-视频理解受限于有限上下文窗口和严重信息冗余。

Method: 提出AVOC框架，在编码器和LLM之间引入可学习令牌压缩模块，将多模态令牌压缩建模为top-K检索问题，并基于相关性、重要性、多样性三种信息检索标准设计压缩机制。

Result: 在OmniVideoBench和LVOmniBench上分别比第二名高4.9和5.5个平均准确率点，在长达1小时的Audio-Video Needle-in-a-Haystack任务上保持稳健性能。

Conclusion: AVOC通过检索式令牌压缩有效解决了长形式音频-视频理解的瓶颈，实现了最先进性能。
...
### [122] [On the Stability of Prompt Ranking in Large Language Model Evaluation](https://arxiv.org/abs/2606.24381)
*Shaoshuai Du,Penghao Liang,Yixian Shen,Chuanqi Shi,Hang Zhang,Lun Wang*

Main category: cs.CL

TL;DR: 研究提示排名在评估条件下的稳定性，发现排名不稳定导致选择不可靠，提出基于置信下限的稳定感知选择策略以提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前提示选择隐含假设排名稳定，但实际中排名易受随机种子和评估子集影响，导致选择不可靠。

Method: 提出稳定感知选择策略，基于下限置信界同时考虑性能与方差。

Result: 该方法在不稳定设定下提升鲁棒性，在稳定设定下保持竞争力。

Conclusion: 提示选择与LLM基准测试中应重视评估不确定性的影响。
...
### [123] [DREAM: Dense Retrieval Embeddings via Autoregressive Modeling](https://arxiv.org/abs/2606.24667)
*Yixuan Tang,Yi Yang*

Main category: cs.CL

TL;DR: DREAM通过将检索器生成的相似度分数注入冻结LLM的注意力头，利用自回归预测损失训练密集检索器，在多个基准上取得更好性能。


<details>
  <summary>Details</summary>
Motivation: 现有密集检索器需要大量标注数据，本文探索是否可以用LLM的自回归下一个词预测目标为检索提供监督。

Method: 提出DREAM，将检索器生成的查询-文档相似度分数注入冻结LLM的注意力头，在LLM预测目标时控制对候选文档的注意力，通过预测损失反向传播梯度训练检索器。

Result: 在BEIR和RTEB基准上，使用0.5B到3B参数的嵌入骨干，DREAM一致优于现有基线。

Conclusion: DREAM表明通过自回归建模训练密集检索器是一种有前景的方法。
...
### [124] [Are We Ready For An Agent-Native Memory System?](https://arxiv.org/abs/2606.24775)
*Wei Zhou,Xuanhe Zhou,Shaokun Han,Hongming Xu,Guoliang Li,Zhiyu Li,Feiyu Xiong,Fan Wu*

Main category: cs.CL

TL;DR: 本文从数据管理视角系统研究了LLM agent记忆，提出四模块分析框架，评估12个系统发现架构与工作负载瓶颈的对齐决定效果，并揭示成本-性能权衡。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅用端到端指标衡量记忆，忽略系统级问题如操作成本、架构权衡和动态更新鲁棒性，本文旨在填补这一空白。

Method: 提出将agent记忆分解为表示与存储、提取、检索与路由、维护四个模块的分析框架，评估12个记忆系统和两个基线，跨越5个基准工作负载和11个数据集，并进行细粒度消融研究。

Result: 无单一架构在所有场景占优，有效性取决于记忆结构与工作负载瓶颈的对齐；消融研究量化各模块对表示保真度、检索精度、更新正确性和长期稳定性的影响；本地维护比全局重组更成本高效。

Conclusion: 基于发现，识别了构建真正agent原生记忆系统的有希望的方向。

Abstract: Memory for large language model (LLM) agents has rapidly evolved from simple retrieval-augmented mechanisms into a data management system that supports persistent information storage, retrieval, update, consolidation, and dynamic lifecycle governance throughout agent execution. Despite this evolution, existing evaluations still benchmark agent memory mainly through end-to-end task success metrics (e.g., F1, BLEU), while treating the underlying system as a monolithic black box. As a result, critical system-level concerns, including operational costs, architectural trade-offs across memory modules, and robustness under dynamic knowledge updates, remain insufficiently explored. In this paper, we present a systematic experimental study of agent memory from a data management perspective. We propose an analytical framework that decomposes agent memory into four core modules: memory representation and storage, extraction, retrieval and routing, and maintenance. Under this framework, we evaluate 12 representative memory systems and two reference baselines across five benchmark workloads spanning 11 datasets. Our extensive end-to-end evaluation shows that no single architecture dominates across all scenarios; instead, effectiveness depends heavily on how well the memory structure aligns with the workload bottleneck. Furthermore, through fine-grained ablation studies, we quantify their individual effects on representation fidelity, retrieval precision, update correctness, and long-horizon stability. Finally, we reveal cost-performance trade-offs under realistic workloads, showing localized maintenance is more cost-efficient than global reorganization. Based on these findings, we identify promising directions towards building truly agent-native memory systems. The code is publicly available at .

</details>


### [125] [L3Cube-MahaPOS: A Marathi Part-of-Speech Tagging Dataset and BERT Models](https://arxiv.org/abs/2606.24825)
*Hariom Ingle,Ronit Ghode,Ishwari Gondkar,Jidnyasa Harad,Raviraj Joshi*

Main category: cs.CL

TL;DR: L3Cube-MahaPOS is a manually annotated Marathi POS dataset of 32,354 sentences with 16 tags, benchmarked across six model families. The best system (MahaBERT-v2) achieves 88.67% accuracy and 81.67% macro-F1.


<details>
  <summary>Details</summary>
Motivation: Marathi is under-resourced despite high speaker count and presents unique NLP challenges due to rich morphology, free word order, lack of capitalization, and code-mixing. A gold-standard POS dataset is needed to advance Marathi NLP.

Method: 32,354 sentences from news text were manually annotated by Marathi-proficient annotators using a 16-tag Universal Dependencies scheme. A preprocessing pipeline with Unicode normalization, Devanagari-aware tokenization, and noise filtering was applied. Benchmarking used HMM, CRF, BiLSTM, BiLSTM+CharCNN, MuRIL, and MahaBERT-v2.

Result: The best system achieves 88.67% token-level accuracy and 81.67% macro-F1 over 15 evaluated tag classes.

Conclusion: The dataset, annotation guidelines, and trained checkpoints are released to foster further research in Marathi NLP.
...
### [126] [SURGELLM: Rethinking Multi-Task Evaluation through Task-Aware Feature Gating with Class-Balanced Normalization](https://arxiv.org/abs/2606.24259)
*Noor Islam S. Mohammad,Ulug Bayazit*

Main category: cs.CL

TL;DR: Surgellm是一个统一的transformer框架，通过手术特征门、任务条件前缀和实例权重归一化来解决编码器在异质NLP任务中的三个问题，在四个任务上获得显著提升。


<details>
  <summary>Details</summary>
Motivation: 微调编码器在异质NLP任务中面临三个问题：不匹配的归纳偏差、类别不平衡对特征统计的破坏、缺乏机制条件化注意力于外部词汇知识。

Method: 提出了Surgellm框架，包含三个轻量模块：手术特征门（learned per-dimension sigmoid）、任务条件前缀令牌（量化特征值和任务ID）、实例权重归一化（消除类先验偏差）。并证明了一个超额风险界。

Result: 在SST-2、多跳检索、LLM提示归因和作者检测四个任务上，IWN变体达到宏F1 0.940，比最强非IWN基线高0.036，在作者检测上高0.130；随机词汇控制确认增益来自词汇。

Conclusion: 该方法有效，并发布了代码、词汇表和99.5%恢复率的自动提取配方。
...
### [127] [Transformer-Based Language Models Across Domain Verticals: Architectures, Applications and Critical Assessment](https://arxiv.org/abs/2606.24331)
*Guruprakash J,Krithika L.B*

Main category: cs.CL

TL;DR: 对Transformer模型家族及其应用的综合综述，提供了分类、比较和批判性评估。


<details>
  <summary>Details</summary>
Motivation: 帮助从业者从大量发布中区分出有持续价值的想法。

Method: 构建工作分类学，涵盖多种Transformer变体，并延伸讨论后期发展；调查在多个领域的部署。

Result: 提供了架构比较的四个维度，量化了参数数量与能源成本之间的权衡，并讨论了影响模型评价的因素。

Conclusion: 总结了需要更多关注的研究问题，并给出批判性评估。
...
### [128] [Escaping the Self-Confirmation Trap: An Execute-Distill-Verify Paradigm for Agentic Experience Learning](https://arxiv.org/abs/2606.24428)
*Shiding Zhu,Yudi Qi,Yajie Wang,Jiaze Li,Chao Song,Yaorui Shi,Yibo Miao,Hanqi Gao,Kai Zhang*

Main category: cs.CL

TL;DR: EDV proposes a three-stage framework (Execute-Distill-Verify) for reliable experience learning in LLM agents, decoupling execution, distillation, and verification to overcome the self-confirmation trap from single-agent loops.


<details>
  <summary>Details</summary>
Motivation: Existing single-agent experience learning methods are vulnerable to the Self-Confirmation Trap, where wrong-but-self-consistent trajectories are misidentified as successful experiences, leading to cumulative errors.

Method: EDV consists of Execute (multiple heterogeneous agents explore in parallel to generate diverse trajectories), Distill (dedicated third-party agent comparatively analyzes trajectories to produce candidate experiences), and Verify (execution group validates candidates via consensus mechanism before memory insertion).

Result: EDV consistently outperforms strong baselines on three long-horizon benchmarks: tau2-bench, Mind2Web, and MMTB.

Conclusion: Reliable experience construction is essential for robust agent self-evolution.

Abstract: Experience-driven self-evolution is critical for large language model (LLM) agents to improve through open-world interaction. However, existing experience learning methods mostly rely on single-agent loops, where the same agent executes tasks, summarizes outcomes, and determines memory content. This setup makes agents vulnerable to the Self-Confirmation Trap: wrong-but-self-consistent trajectories are misidentified as successful experience, leading to cumulative errors during retrieval and reuse. To address this issue, we propose EDV, an Execute-Distill-Verify framework for reliable experience learning. In the Execute stage, multiple heterogeneous agents explore the same task space in parallel to generate diverse candidate trajectories. In the Distill stage, a dedicated third-party agent comparatively analyzes these trajectories to produce candidate experiences, reducing executor-centric summarization bias. In the Verify stage, the execution group validates candidates via a consensus mechanism, and only approved experiences are written into shared or private memory. By decoupling the three stages, EDV transforms experience learning from isolated self-reflection into collaborative construction, filtering erroneous and noisy content before memory insertion. We evaluate EDV on three challenging long-horizon benchmarks: tau2-bench, Mind2Web and MMTB. Results show EDV consistently outperforms strong baselines, validating that reliable experience construction is essential for robust agent self-evolution. Our code is available at .

</details>


### [129] [Privacy-Preserving RAG via Multi-Agent Semantic Rewriting: Achieving Confidentiality Without Compromising Contextual Fidelity](https://arxiv.org/abs/2606.24623)
*Yuanhe Zhao,Tianyu Zhang,Huafei Xing,Derek F. Wong,Jianbin Li,Tao Fang*

Main category: cs.CL

TL;DR: 提出一个多智能体框架，通过语义重写净化检索内容，在降低隐私泄露的同时保持语义完整性。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成在敏感场景部署时，面临通过恶意提示导致隐私泄露的风险。

Method: 采用三个专门智能体（隐私提取、语义分析、重建）协同工作，对检索内容进行语义重写，移除敏感标识符并保留语义核心。

Result: 在ChatDoctor和Wiki-PII数据集上，针对LLaMA-3-8B的隐私泄露从144例降至1例，BLEU-1得分0.122优于SAGE的0.117，且作为离线预处理模块不增加推理延迟。

Conclusion: 该框架显著降低隐私泄露，保持上下文化忠实性，且不引入额外延迟。

Abstract: Retrieval-Augmented Generation enhances large language models by incorporating external knowledge, but deploying it in sensitive scenarios risks privacy leakage via malicious prompts. To address this, we propose a multi-agent framework that sanitizes retrieved content through semantic rewriting. By employing three specialized agents for privacy extraction, semantic analysis, and reconstruction, our approach collaboratively removes sensitive identifiers while preserving the semantic core. We evaluate the framework on the ChatDoctor and Wiki-PII datasets across six large language models. Experimental results demonstrate a significant reduction in privacy leakage under targeted attacks. For instance, we reduced targeted information exposure in LLaMA-3-8B from 144 instances in the baseline to just 1. Furthermore, we maintain strong contextual fidelity with a BLEU-1 score of 0.122, outperforming the existing SAGE method's 0.117. Finally, the framework operates as an asynchronous preprocessing module, introducing no additional latency to online inference, as all rewriting is executed as a one-time offline preprocessing step. To promote reproducibility, the source code of this work is publicly available at .

</details>


### [130] [Quantifying Prior Dominance in RAG Systems](https://arxiv.org/abs/2606.23695)
*Barak Or*

Main category: cs.CL

TL;DR: 提出NCU指标评估RAG，发现小语言模型在严格事实提取中优于大模型，商业API存在先验主导和负迁移。


<details>
  <summary>Details</summary>
Motivation: 当前RAG评估依赖离散启发式，存在“认知盲区”，无法区分上下文信息提取与参数化记忆。

Method: 引入NCU指标，利用连续token对数概率在零样本、oracle和对抗条件下量化上下文信息增益，评估1.5B-72B参数架构及商业API。

Result: 严格事实提取中传统缩放定律出现极端收益递减，小模型匹配或超越大模型；先验主导与模型规模和专有对齐相关；商业API在对抗冲突中覆盖外部证据并出现置信度崩溃。

Conclusion: 小语言模型在严格提取工作流中具有结构认知优势和更好的上下文遵循。
...
TL;DR: 提出MELD数据集并发现现有嵌入模型根据术语而非数学内容分组数学语句；通过对比学习对齐非正式与正式表述，改善了嵌入表示。


<details>
  <summary>Details</summary>
Motivation: 数学高度抽象导致同一陈述在不同子领域形式不同，现有嵌入模型未能捕捉数学等价性，而是按术语分组，这阻碍了跨领域知识发现。

Method: 构建MELD数据集，包含数学等价但词汇不同的语句对；提出对比学习方法，拉近非正式表述与不同正式表述的嵌入距离。

Result: 对比学习在非正式-正式检索任务及MELD数据集上均优于现有模型，证明能更好地捕捉数学等价性。

Conclusion: 所提方法能有效改善数学文本嵌入，使模型关注底层数学内容而非表面术语，有助于跨领域数学导航。
...
TL;DR: 提出MeMo模型，通过版本感知操作层和辅助CMM实现知识更新而不需重新训练。


<details>
  <summary>Details</summary>
Motivation: 减少知识变化时重新训练整个模型的需要。

Method: 版本感知操作层将高级操作编译为MeMo原语，并引入版本CMM和事务CMM。

Result: 概述了评估更新成功、回滚、可追溯性、局部性和事务重用的路线。

Conclusion: 通过编辑显式记忆可有效更新模型知识，支持替换、废弃、历史保留等操作。
...
### [133] [Cross-Lingual Exploration for Parametric Knowledge](https://arxiv.org/abs/2606.24579)
*Elisha Diskind,Itamar Trainin,Uri Shaham,Leshem Choshen,Idan Szpektor,Omri Abend*

Main category: cs.CL

TL;DR: Cross-lingual prompting strategies effectively improve knowledge retrieval and consistency in multilingual LLMs.


<details>
  <summary>Details</summary>
Motivation: Parametric knowledge in LLMs is not equally accessible across languages, leading to failures in cross-lingual knowledge transfer and consistency.

Method: Investigate cross-lingual prompting strategies by identifying four inherent dimensions of exploration and evaluating on multilingual factual benchmarks covering 17 languages.

Result: Cross-lingual exploration significantly improves knowledge transfer and factual recall, achieving a more efficient compute Pareto frontier than native-language scaling, and also improves cross-lingual consistency beyond accuracy gains.

Conclusion: Multilingual prompt exploration is established as a highly effective inference-time strategy for unlocking latent parametric knowledge.
...
### [134] [MEMPROBE: Probing Long-Term Agent Memory via Hidden User-State Recovery](https://arxiv.org/abs/2606.24595)
*Enze Ma,Yufan Zhou,Wei-Chieh Huang,Jie Yang,Huanhuan Ma,Zixuan Wang,Chengze Li,Chunyu Miao,Philip S. Yu,Zhen Wang*

Main category: cs.CL

TL;DR: 提出MEMPROBE基准，通过重建用户状态直接评估长期记忆，发现协助能力与记忆恢复能力分离。


<details>
  <summary>Details</summary>
Motivation: 现有长期记忆评估间接且缺乏审计，主张应作为可审计的后交互工件评估。

Method: 构建MEMPROBE基准，使用模拟用户和隐藏用户状态库，通过任务轨迹让代理生成记忆，再重建用户状态进行评估。

Result: 成功协助和可恢复记忆是不同能力；任务完成饱和但记忆恢复中等（约0.6），top-k下更低。

Conclusion: MEMPROBE首个直接研究记忆恢复的基准，为优化目标，迈向更忠实记忆的代理。
...
### [135] [Ground Then Rank: Revisiting Knowledge-Based VQA with Training-Free Entity Identification](https://arxiv.org/abs/2606.23881)
*Qian Ma,Qiong Wu,Zhengyi Zhou,Yao Ma*

Main category: cs.CL

TL;DR: 提出IBA框架，通过解耦实体识别和证据重排序，提升KB-VQA性能，降低复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有MM-RAG方法将实体区分和证据排名耦合在一阶段，导致高成本和有限泛化；MLLMs在开放实体命名时失败，但在候选集中选择更好。

Method: 提出训练无关的IBA框架，先利用MLLM从候选名称中挑选高置信度实体，再使用现成文本重排序器进行证据选择。

Result: 在Encyclopedic-VQA和InfoSeek上，该方法优于微调的多模态重排序基线，降低训练和推理复杂度，改进源于更好的实体识别和更信息性的证据选择。

Conclusion: 所提方法简单有效，可复现，为KB-VQA提供了一种解耦实体与证据推理的新思路。
...
### [136] [UOL@IDEM at BEA 2026 Shared Task 1: Neural Fusion and Feature-Rich Modeling for L1-Aware Vocabulary Difficulty Prediction](https://arxiv.org/abs/2606.24501)
*Nouran Khallaf,Serge Sharoff*

Main category: cs.CL

TL;DR: 提出一种结合多语言上下文表示和工程特征的回归方法，用于L1感知词汇难度预测，在西班牙语、德语和中文上取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 针对词汇难度预测中语言特异性（L1感知）的需求，需要开发跨语言适用的模型。

Method: 将任务建模为回归，结合多语言上下文表示（如BGE-M3、E5、LaBSE）与多种工程特征（频率、表面形式、检索证据、语义对齐、同源相似性、掩码语言模型可预测性），并针对每个语言训练单独系统。

Result: 在西班牙语、德语和中文上分别取得RMSE 1.132、1.037、0.891，超过官方基线；频率是最稳定的预测因子，其他特征提供互补的L1敏感信号；错误分析显示对简单项目过度预测，排名性能良好但校准较弱。

Conclusion: 多语言上下文表示与工程特征结合可有效进行L1感知词汇难度预测，频率是关键特征，但需进一步改进校准以处理简单项目。

Abstract: This paper describes UOL@IDEM's closed-track submission to the BEA 2026 shared task on L1-aware vocabulary difficulty prediction. We model the task as regression and train separate systems for Spanish, German, and Mandarin Chinese\footnote{Below we use \emph{Chinese} for brevity.}. Our system combines multilingual contextual representations with engineered features capturing frequency, surface form, retrieval evidence, semantic alignment, cognate similarity, and masked-language-model predictability. Development results show consistent gains over the official closed-track baselines, with sentence-embedding encoders such as BGE-M3, multilingual E5, and LaBSE performing best. Official submissions achieve RMSE scores of 1.132, 1.037, and 0.891 for Spanish, German, and Chinese, respectively. Feature analysis identifies frequency as the most stable predictor, while contextual predictability, form similarity, retrieval, and semantic features provide complementary L1-sensitive signals. Error analysis shows strong ranking performance but weaker calibration for the easiest items, which are often overpredicted. See

</details>


### [137] [Paying to Know: Micro-Transaction Markets for Verified Product Information in Agentic E-Commerce](https://arxiv.org/abs/2606.24783)
*Filippos Ventirozos,Matthew Shardlow*

Main category: cs.CL

TL;DR: 提出基于微支付的代理电商市场，强调NLP应关注信息获取等问题而非聊天流畅性。


<details>
  <summary>Details</summary>
Motivation: 当前商业NLP将购物聊天机器人视为推荐或转化工具，但代理原生微支付改变了稀缺性，瓶颈从匹配产品变为获取可信信息。

Method: 描绘了一个基于微交易的验证信息市场架构，并转化为成本最优信息获取、数据定价与谈判、实时实体解析、基于价值交换、隐私保护的人物建模等NLP问题。

Result: 论证该市场奖励真正的产品质量，并产生比基于排名的店面更真实的竞争。

Conclusion: 这些NLP问题值得领域关注，而不是聊天流畅性。
...
### [138] [SHERLOC: Structured Diagnostic Localization for Code Repair Agents](https://arxiv.org/abs/2606.24820)
*Hovhannes Tamoyan,Sean Narenthiran,Erik Arakelyan,Mira Mezini,Boris Ginsburg*

Main category: cs.CL

TL;DR: SHERLOC是一个无需训练、结合推理LLM与紧凑仓库工具和自我恢复的框架，在SWE-Bench定位任务上达到SOTA，并有效提升修复代理性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在代码库级任务中浪费一半预算用于定位错误，而专用定位框架仅评估文件检索，缺乏诊断上下文，无法满足修复代理需求。

Method: SHERLOC采用结构化假设驱动探索与推理，结合推理LLM、紧凑仓库工具和自我恢复，无需微调或多代理编排。

Result: 在SWE-Bench Lite上达到84.33% accuracy@1，SWE-Bench Verified上81.27% recall@1；约30B参数下匹配或超越其他方法；注入定位和诊断后使修复代理解决率平均提升5.95个百分点，同时减少定位令牌36.7%和总令牌23.1%。

Conclusion: SHERLOC实现了可操作的代码库级定位，显著提升后续修复效率，是一种高效、轻量级的解决方案。
...
### [141] [QuechuaTok: Morphological Boundary Accuracy as a Necessary Metric for Tokenizer Evaluation in Agglutinative Low-Resource Languages](https://arxiv.org/abs/2606.23943)
*Maria Contreras*

Main category: cs.CL

TL;DR: QuechuaTok benchmark compares tokenization strategies for Southern Quechua, showing that standard metrics like fertility rate are insufficient for agglutinative languages; PRPE achieves highest morphological accuracy (83.33% MorphAcc) while BPE has lowest fertility rate (1.636).


<details>
  <summary>Details</summary>
Motivation: Standard evaluation metrics such as fertility rate fail to capture morphological correctness for agglutinative languages.

Method: We present QuechuaTok, a benchmark comparing BPE, Unigram LM, WordPiece, and PRPE tokenizers on a 200k-sentence Southern Quechua corpus, using SQUOIA morphological analyzer as silver standard and evaluating fertility rate, OOV rate, and MorphAcc.

Result: BPE achieves lowest fertility rate (1.636 at 16k vocab) with 6.67% MorphAcc; PRPE achieves 83.33% MorphAcc, the highest of all systems.

Conclusion: Fertility rate alone is insufficient to evaluate tokenizers for agglutinative languages; morphological accuracy metrics are necessary.

Abstract: Tokenization is a foundational step in NLP pipelines, yet standard evaluation metrics such as fertility rate fail to capture morphological correctness for agglutinative languages. We present QuechuaTok, a systematic benchmark comparing four tokenization strategies - BPE, Unigram LM, WordPiece, and a morphology-aware PRPE tokenizer - for Southern Quechua (quz), a low-resource agglutinative language spoken by 8-10 million people in South America. Using a 200k-sentence corpus and the SQUOIA finite-state morphological analyzer (Rios, 2016) as silver standard, we evaluate three metrics: fertility rate, OOV rate, and morphological boundary accuracy (MorphAcc). Our results show that BPE achieves the lowest fertility rate (1.636 at 16k vocab) by memorizing surface word forms, while achieving only 6.67% MorphAcc. PRPE achieves 83.33% MorphAcc - the highest of all systems - demonstrating that fertility rate alone is insufficient to evaluate tokenizers for agglutinative languages. All code and models are publicly available at

</details>


### [142] [The African Language Tax: Quantifying the Cost, Latency, and Context Penalty of Tokenizing African Languages in Frontier LLMs](https://arxiv.org/abs/2606.24460)
*Olaoye Anthony Somide*

Main category: cs.CL

TL;DR: tokenizer对非洲语言的标记化惩罚导致高达8.9倍的推理成本和延迟，有效上下文窗口仅为英语的11%，且现有tokenizer无法消除这一差距。


<details>
  <summary>Details</summary>
Motivation: 尽管已知多语言环境中存在标记化惩罚，但针对非洲语言在企业部署经济和认知上下文容量方面的系统测量尚未进行。

Method: 使用并行语料库（FLORES-200+和MAFAND-MT）在20种非洲语言上，通过11种前沿和开放的tokenizer测量标记化溢价，并开发了开放测量工具afri-fertility。

Result: 所有非洲语言在tokenizer上均存在高于英语的标记化溢价（GPT-5/o200k_base中位数1.88倍，N'Ko最高8.92倍）；推理成本和延迟最多可达8.9倍，有效上下文窗口仅为英语的11%；最佳tokenizer Gemma 4将平均溢价从3.31倍降至2.38倍，但未消除。

Conclusion: 标记化惩罚对最负担不起的语言影响最大，构成了数字鸿沟，研究者发布了测量工具、排行榜、数据集和缓解指南。
...
### [143] [NatureBench: Can Coding Agents Match the Published SOTA of Nature-Family Papers?](https://arxiv.org/abs/2606.24530)
...
TL;DR: NatureBench是一个90个任务的基准测试，评估AI代理在真实科学问题上的表现，结果显示代理主要通过方法翻译而非科学发明取得成功。


<details>
  <summary>Details</summary>
Motivation: 评估AI编码代理能否超越复制实现科学发现，并解决环境碎片化问题以提升基准可信度。

Method: 基于NatureGym自动化流水线构建容器化环境，在禁用网络搜索协议下评估十个前沿代理配置。

Result: 最强模型在g>0.1标准下仅超越17.8%任务的SOTA；代理成功依赖于方法翻译，失败由错误方法选择和计算预算不足导致。

Conclusion: 发布了NatureBench基准、NatureGym流水线和公共排行榜，支持维护者端复制。
...
### [144] [CANDLE: Character-level Arabic Noise Deduplication using Lightweight Encoder](https://arxiv.org/abs/2606.24758)
*Faris Alasmary,Taif Nono,Orjuwan Zaafarani,Kholood Al Tabash,Ahmad Ghannam,Anas Salamah,Shouq Sadah,Lahouari Ghouti*

Main category: cs.CL

TL;DR: CANDLE is a lightweight Arabic character-level noise deduplication system using CTC, achieving state-of-the-art results and reducing LLM tokenizer fertility.


<details>
  <summary>Details</summary>
Motivation: Handling repeated characters in Arabic text is challenging due to ambiguity between correct spelling and informal elongation; existing methods rely on handcrafted rules or resources.

Method: Frames character deduplication as a sequence alignment problem using Connectionist Temporal Classification (CTC) on a character-based encoder, and applies knowledge distillation to reduce model depth from 6 layers to 2 layers.

Result: Achieves Sentence Error Rate (SER) as low as 5.37% on benchmarks, outperforming classification baselines; distillation achieves 3x depth reduction with minimal performance loss; reduces tokenizer fertility by up to 12.8% across Arabic LLM tokenizers.

Conclusion: CANDLE provides an effective, lightweight solution for Arabic character deduplication that also lowers LLM inference costs, with all code and models released publicly.
...
### [145] [ModTGCN: Modularity-aware Graph Neural Networks for Text Classification](https://arxiv.org/abs/2606.23694)
*Rajarshi Misra,Aditya Sharma,Vinti Agarwal,Hari Om Aggrawal*

Main category: cs.CL

TL;DR: ModTGCN通过模块化感知的图神经网络联合优化分类和社区结构，在文本分类中取得一致改进。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的文本分类模型忽略全局社区结构，导致类边界模糊和过平滑。

Method: 提出ModTGCN，联合优化交叉熵和模块化辅助目标，利用transformer嵌入构建文档相似图，并分解TextGCN图以提升可扩展性。

Result: 在五个基准上一致提升，在低同质性数据集（如Ohsumed和20NG）上改进更大。

Conclusion: ModTGCN通过引入模块化感知的社区结构有效改善了文本分类性能。
...
### [146] [Evaluating LLM Usage for Efficient and Explainable Numerical and Classified Implicit Sentiment Analysis of Product Desirability](https://arxiv.org/abs/2606.23701)
*Sherri Weitl-Harms,John Hastings*

Main category: cs.CL

TL;DR: 提出一个使用LLM从定性反馈中量化产品可取性的可扩展可解释框架，在两个数据集上达到高相关性和准确率。


<details>
  <summary>Details</summary>
Motivation: 定性产品反馈可以揭示细微的用户体验，但其隐含情感难以测量。

Method: 使用LLM进行零样本连续数值情感评分和分类情感分类，在PDT数据集上评估，并整合模型置信度和xAI解释。

Result: LLM生成的数值情感分数与专家标签的Pearson相关性高达0.97，分类准确率高达94%，GPT-4o-mini以94%更低成本达到可比性能。

Conclusion: 该框架结合PDT调查和成本高效LLM，可提供丰富的情感分数和用户印象，支持产品开发和营销。
...
TL;DR: Proposes a benchmark suite based on Pānini's grammar to unify Indic language processing.


<details>
  <summary>Details</summary>
Motivation: Indic language NLP is fragmented due to individual-language tools, overlooking shared morphosyntactic architecture from Sanskrit.

Method: A four-part benchmark suite grounded explicitly in the Pāninian framework.

Result: Expected to improve accuracy, data-efficiency, and transferability, merging sparse resources into a high-resource metalanguage.

Conclusion: Raises the question whether neural models represent Pānini's categories on their own.
...
### [148] [Aspect-Based Sentiment Evolution and its Correlation with Review Rounds in Multi-Round Peer Reviews: A Deep Learning Approach](https://arxiv.org/abs/2606.24188)
*Ruxue Hana,Haomin Zhoua,Jiangtao Zhong,Chengzhi Zhang*

Main category: cs.CL

TL;DR: 对多轮同行评审评论的方面级别情感分析，发现随着轮次增加积极情感上升，消极情感下降，且方面情感得分与轮次数量负相关。


<details>
  <summary>Details</summary>
Motivation: 先前研究受限于粗粒度分析和缺乏对评审轮次的区分，未充分探索评审者关注点和情感倾向的动态变化，本研究旨在调查方面级别情感的分布和演化及其与评审轮次数量的相关性。

Method: 分段处理11,063篇Nature Communications论文的多轮评审评论，识别方面聚类；构建约5,000条句子的人工标注语料库；训练基于深度学习的方面情感分类模型，其中LCF-BERT-CDM模型最佳（Macro-F1 82.65%）。

Result: 随着评审轮次增加，积极情感比例上升，消极情感下降；方面情感得分与总轮次数量负相关；关键方面包括“experiments”、“research significance”和“result analysis”。

Conclusion: 方面级别情感分析能有效揭示评审过程中的情感动态，为理解科学评价过程提供新视角。
...
### [149] [AutoSpecNER: A Fine-Grained Named Entity Recognition Dataset for Vehicle Specification Extraction](https://arxiv.org/abs/2606.24387)
*Jordan Lee,Filippos Ventirozos,Abdirahman Abdullahm,Ioanna Nteka,Peter Appleby,Matthew Shardlow*

Main category: cs.CL

TL;DR: 介绍了AutoSpecNER数据集，用于车辆广告细粒度实体识别，并通过基准测试显示DeBERTa取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 车辆广告包含丰富的规格信息，但汽车命名实体识别资源有限，需要专门的标注数据集。

Method: 构建了包含659条广告、超过1万个实体的AutoSpecNER数据集，涵盖15个类别，通过专家注释和一致性验证（平均91.5%）保证质量；对基于规则、微调Transformer编码器和大型语言模型进行了基准测试。

Result: DeBERTa以90%的微F1分数超过规则基线（43%）和最强LLM（77.8%）。

Conclusion: AutoSpecNER数据集和基准测试表明DeBERTa在汽车NER任务上的有效性，为后续研究提供基础。
...
### [150] [AI-PAVE-Br: Leveraging Large Language Models for Enhanced Product Attribute Value Extraction through a Golden Set Approach](https://arxiv.org/abs/2606.24655)
...
TL;DR: 引入AI-PAVEBr系统和大规模标注数据集Golden Set，显著提升巴西电商产品属性抽取性能。


<details>
  <summary>Details</summary>
Motivation: 巴西电商产品数据复杂多样，传统方法难以处理葡萄牙语产品描述的细微差别和多样性。

Method: 构建基于大语言模型的AI-PAVEBr系统，结合针对性提示工程；创建并公开高精度的Golden Set数据集。

Result: AI-PAVEBr在PAVE任务上显著优于传统命名实体识别（NER）基线。

Conclusion: 提供适用于巴西市场的可扩展解决方案和宝贵的公开数据集，推动非英语市场的PAVE研究。
...
### [151] [Self-Recognition Finetuning can Prevent and Reverse Emergent Misalignment](https://arxiv.org/abs/2606.23700)
*Arush Tagade,Shaoheng Zhou,Jiaxin Wen,Shi Feng*

Main category: cs.CL

TL;DR: SGTR微调作为字符针对性干预可有效防御紧急不对齐，且证据表明紧急不对齐是对齐角色的不稳定化而非采纳不对齐角色。


<details>
  <summary>Details</summary>
Motivation: 探索紧急不对齐（EM）的机制，并基于其与角色特征的联系，提出并测试自生成文本识别微调作为针对角色的防御。

Method: 在三个模型（GPT-4.1, Qwen2.5-32B-Instruct, Seed-OSS-36B-Instruct）上使用多个EM数据集进行两阶段微调实验，比较SGTR微调与良性微调基线（正确领域数据、通用知识、单词计数）在逆转和预防设置中的效果。

Result: 所有干预均实现EM逆转但仅恢复降级能力；预防中仅SGTR微调持续减少不对齐且不恶化指标；EM微调导致身份自我报告多样性增加，自我识别破坏加剧不对齐，移除身份提示减少EM效果。

Conclusion: 紧急不对齐并非采纳连贯不对齐角色，而是对齐角色的不稳定化；SGTR微调通过角色强化提供有效防御。
...
### [152] [One Year Later...The Harms Persist, But So Do We!](https://arxiv.org/abs/2606.23884)
*Annika Marie Schoene,Cansu Canca,Gautham Vijay Kumar,Anson Antony*

Main category: cs.CL

TL;DR: 通用大语言模型在心理健康对话中安全防护不足，评估发现仅自杀和自伤防护可靠，其他精神障碍失败率高达100%，亟需明确伤害类别并实施对应防护。


<details>
  <summary>Details</summary>
Motivation: 大语言模型日益用于心理健康对话，但其安全防护措施不足且不一致，存在对脆弱人群的潜在风险。

Method: 评估六个专有LLM，针对16种DSM-5疾病使用四种对抗攻击变体，引入八维伤害分类和多维评估框架。

Result: 防护只在自杀和自伤条件下可靠；饮食障碍、物质使用障碍和重度抑郁障碍等条件下失败率高达100%。

Conclusion: 要求明确界定临床条件伤害类别并实施防护，否则模型整合到教育环境会带来重大风险。
...
### [153] [Towards Spec Learning: Inference-Time Alignment from Preference Pairs](https://arxiv.org/abs/2606.24004)
*Dhriti Krishnan,Tejas Goyal,Jaromir Savelka*

Main category: cs.CL

TL;DR: Spec学习框架通过用户指令和少量偏好判断编译成自然语言规格，在推理时条件化LLM，无需参数更新，在专门领域优于DPO，且规格可解释。


<details>
  <summary>Details</summary>
Motivation: 现有引导LLM行为的方法要么依赖手动设计提示（脆弱且易错），要么需要昂贵的偏好微调。Spec学习旨在提供一种高效、可解释且无需参数更新的替代方案。

Method: Spec学习使用简短的用户指令和少量偏好判断，将其编译成自然语言提示（规格），在推理时条件化LLM，不更新模型参数。

Result: 基于编译规格生成的响应在专门领域的数据集上（偏好信号密集）通常优于直接偏好优化（DPO）。

Conclusion: 规格是人类可读的，透明且可解释，作为产生它们的偏好信号的书面体现，相比不可解释的权重更新更具优势。
...
### [154] [Best Preprocessing Techniques for Sentiment Analysis](https://arxiv.org/abs/2606.24055)
*Saranzaya Magsarjav,Melissa Humphries,Jonathan Tuke,Lewis Mitchell*

Main category: cs.CL

TL;DR: 在Twitter情感分析中，预处理技术的顺序显著影响模型效果，最佳顺序是分词、文本清洗、词干提取和停用词移除。


<details>
  <summary>Details</summary>
Motivation: 预处理对情感分析至关重要，但关于预处理技术顺序的系统研究很少，因此需要探索最佳顺序以提高模型效率。

Method: 通过实验系统地比较不同预处理顺序，评估各技术的影响程度，并确定最佳顺序。

Result: 分词影响最大，拼写纠正影响最小；词干提取和停用词移除可互换，但停用词移除应保留否定词；最佳顺序为分词、文本清洗、词干提取、停用词移除。

Conclusion: 提供了一种系统性的预处理顺序方法，帮助从业者高效部署预处理，避免昂贵的探索阶段。
...
### [155] [Selective Capability Unlearning in End-to-End Spoken Language Understanding](https://arxiv.org/abs/2606.24063)
*Akanksha Singh,Vinod Kumar Kurmi*

Main category: cs.CL

TL;DR: 提出Binding Subspace (BSU) 框架，通过隔离和减弱意图条件方向，解决SLU自回归模型的能力持久性问题。


<details>
  <summary>Details</summary>
Motivation: 自回归SLU模型中，抑制目标意图无法消除其条件映射，导致即使意图前缀被外部提供，模型仍能重构原始意图-槽结构，即能力持久性问题。

Method: 提出表示级框架Binding Subspace (BSU)，隔离并减弱意图条件方向，从而削弱该条件映射。

Result: 在SLU基准测试上，BSU显著减少强制前缀可恢复性，同时保持保留性能。

Conclusion: BSU有效消除了能力持久性问题，实现了功能移除与性能保留的平衡。
...
### [156] [Sentence-Level Contextual Entrainment in Large Language Models](https://arxiv.org/abs/2606.24077)
*Yang Liu,Chenhui Chu*

Main category: cs.CL

TL;DR: 将上下文引发现从token级别扩展到句子级别，发现句子级别的现象存在，且由少数注意力头控制，关闭它们可缓解而不影响性能。


<details>
  <summary>Details</summary>
Motivation: 探索上下文引发现是否从token级别扩展到句子级别，并研究其与模型规模及注意力头的关系。

Method: 通过计算句子平均log概率，在26个LLM和两个数据集上进行分析，并识别控制该现象的注意力头。

Result: 句子级别上下文引发现存在；模型越大现象越弱；该现象由2%-4%的注意力头控制。

Conclusion: 上下文引发现可被有效缓解，通过关闭少数相关注意力头而不损害模型性能。
...
### [157] [Metis: Bridging Text and Code Memory for Self-Evolving Agents](https://arxiv.org/abs/2606.24151)
...
TL;DR: 通过对文本记忆和代码记忆的对比研究，发现两者互补，并提出了结合两者的分层双表示记忆系统Metis，在AppWorld基准上取得了更好的性能平衡。


<details>
  <summary>Details</summary>
Motivation: 现有自进化代理系统在表示经验时，要么使用自然语言文本，要么使用代码，但选择是设计时决定的，未根据经验特征自适应，导致对两种表示的权衡理解不足。

Method: 提出Metis，一种基于分层双表示记忆的自进化代理系统，将文本经验组织为执行计划、环境事实和常见陷阱，并有选择地将重复计划结晶为可验证的可调用工具，结合了文本记忆的广泛适用性和代码记忆的执行效率。

Result: 在AppWorld基准上，Metis将任务准确率提高高达20.6%（相对于ReAct），执行成本降低高达22.8%；与其他系统相比，在准确率、执行效率和记忆构建成本之间取得更好平衡。

Conclusion: 文本和代码记忆互补，单一表示不足；Metis通过分层双表示有效结合两者，实现了更好的性能平衡。
...
### [158] [MedBench v5: A Dynamic, Process-Oriented, and Hallucination-Aware Benchmark for Clinical Multimodal Models](https://arxiv.org/abs/2606.24155)
...
TL;DR: MedBench v5是一个动态、过程导向的临床多模态模型评估基准，通过双维框架、可控压力测试、动态审计和幻觉传播监控，揭示了强整体性能不保证过程稳定性，主要破坏矛盾检测、诊断更新等。
...
Method: 采用Clinical Cognitive Responsiveness和Medical Atomic Skills双维框架覆盖63个任务；三种可切换信息流压力因素（遗漏、矛盾、证据延迟）进行因子化退化分析；动态过程审计协议含五个推理节点生成失败指纹；幻觉传播监控捕捉初始、传播、锚定和矛盾交互。

Result: 实验表明，强整体任务性能不保证过程稳定性；压力因素主要破坏矛盾检测、诊断更新、幻觉传播和基于矛盾的自我修正，而最终证据基础可能表面上保持稳定。

Conclusion: MedBench v5为临床AI评估提供了统一的基础设施，用于能力分析、可控压力测试、过程审计和幻觉轨迹分析。
...
### [159] [BehaviorBench: Benchmarking Foundation Models for Behavioral Science Tasks](https://arxiv.org/abs/2606.24162)
*Jin Huang,Yutong Xie,Wanli Song,Xingjian Zhang,Walter Yuan,Matthew O. Jackson,Qiaozhu Mei*

Main category: cs.CL

TL;DR: BehaviorBench是一个评估基础模型在行为科学任务中的基准，涵盖个体和分布水平，同时提出-1.5模型，在分布对齐上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型在行为科学任务中表现缺乏系统性理解，需要全面评估其在多样任务、背景和人群中的能力。

Method: 提出BehaviorBench基准，评估四个核心能力（行为预测、战略决策、特征推断、知识应用）在个体和分布水平；并开发-1.5模型，基于行为数据微调。

Result: 专有通用模型在个体预测和知识任务上强，而-1.5等行为基础模型在分布对齐上显著更强，且-1.5在分布指标领先，个体指标有竞争力。

Conclusion: 分布评估至关重要，BehaviorBench可作为开发行为对齐AI的基础，-1.5模型有望广泛用于行为科学研究。

Abstract: Foundation models have been increasingly applied to behavioral science domains such as psychology, sociology, and economics. While these models show promise in individual tasks such as survey response prediction and human-subject experiment simulation, there remains no systematic understanding of how well they perform across diverse behavioral science tasks, contexts, and populations. We introduce BehaviorBench, a comprehensive benchmark that evaluates foundation models along four core capabilities: (1) behavior prediction and simulation, (2) strategic decision-making, (3) subject-trait inference, and (4) behavioral knowledge application. Crucially, BehaviorBench evaluates model outputs at both the individual and distributional levels, capturing not only per-subject accuracy but also population-level alignment, an essential requirement for behavioral validity. Leveraging the tasks in BehaviorBench, we further develop -1.5, extending the family of behavioral foundation models fine-tuned on behavioral data. Our results reveal a considerable gap: proprietary general-purpose models excel at individual-level prediction and knowledge-intensive tasks, whereas behavioral foundation models, fine-tuned on behavioral data, achieve substantially stronger distributional alignment. Notably, -1.5 leads on distributional metrics and remains competitive on individual-level metrics, suggesting that proper behavioral adaptation can close the gap. Our results highlight the importance of distributional evaluation, establish BehaviorBench as a foundation for developing and assessing behaviorally aligned AI systems, and demonstrate -1.5's potential for a broad range of behavioral science studies. Our BehaviorBench and -1.5 models can be accessed via .

</details>


### [160] [A Synthetic Reliability-Aware PINN Benchmark for Offshore Wind Turbine Support-Structure Monitoring with Bayesian Inverse Identification](https://arxiv.org/abs/2606.24176)
*Puneet Kant,Monika Tanwar*

Main category: cs.CL

TL;DR: 提出了Digi Turbine，一种用于海上风力涡轮机支撑结构监测的可靠性感知物理信息神经网络基准。


<details>
  <summary>Details</summary>
Motivation: 可靠的结构健康监测需要快速状态估计，高保真有限元或气动弹性分析难以在线使用，而纯数据驱动代理需要大量训练集。

Method: 将简化的欧拉-伯努利梁方程与Winkler土壤基础嵌入训练目标，结合贝叶斯先验逆识别，并添加一阶可靠性方法筛选。

Result: 所有验证均使用合成配置，以NREL 5MW参考涡轮机为背景，以解析或有限差分解作为真值。

Conclusion: 该基准能够有效融合物理信息与数据，为OWT支撑结构监测提供可靠的状态估计。
...
### [161] [Decoherence as Defence and the Magnitude of Noise Regularisation: A Rigorous N -Qubit Theory of Stochastic Quantum Neural Networks for Adversarially Robust Network Intrusion Detection](https://arxiv.org/abs/2606.24219)
*Gautier-Edouard Edouard Filardo*

Main category: cs.CL

TL;DR: Stochastic quantum neural networks: dropout and depolarising noise both regularise similarly, with depolarising providing adversarial robustness; prior dichotomy not replicated.


<details>
  <summary>Details</summary>
Motivation: Resolve whether per-gate stochastic deactivation (true quantum dropout) can regularise where depolarising channel could not, and replace the loose robustness bound with predictive theory.

Method: N-qubit formulation via stochastic master equation, proof of decoherence-contraction theorem, derivation of adaptive-penalty formula for dropout vs depolarising noise, and experiments on NSL-KDD dataset under white-box attacks.

Result: Depolarising SQNN is significantly more robust than noiseless circuit, never suffers catastrophic collapse; both dropout and depolarising reduce train-test gap by ~0.01 (statistically significant), effects concentrated where overfitting is largest, and the single-seed dichotomy of prior work does not survive replication.

Conclusion: Opens problems resolved: dropout and depolarising noise regularise similarly, depolarising provides robustness; framework extended to real data and neutral-atom hardware.
...
### [162] [CALIBER: Calibrating Confidence Before and After Reasoning in Language Models](https://arxiv.org/abs/2606.24281)
*Conor Finlay,Joshua Kurien,Saurabh Dash,Marzieh Fadaee,Beyza Ermis*

Main category: cs.CL

TL;DR: CALIBER通过分别在思考前后进行置信度估计并匹配相应的监督目标，显著提升了推理语言模型校准性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只进行一次置信度估计，未考虑思考前后状态的不同，导致校准不准确。

Method: 提出CALIBER（Calibration Before and After Reasoning），在思考前和回答后分别估计置信度，并以提示级别成功监督思考前估计，以答案级别正确性监督思考后估计。

Result: 在BigMathDigits上，CALIBER相比最强单置信基线将ECE降低52.5%，同时获得最佳Brier分数和AUROC，准确率损失在2.1点以内；在30B模型上取得最佳ECE；在分布外基准上取得最佳ECE和Brier分数。

Conclusion: 位置-目标对齐（position-target alignment）在校准中至关重要，特别是在分布偏移下能持续降低校准误差。
...
### [163] [Meet UD_Czech-PDTC: A Large and Genre-Rich Treebank in Universal Dependencies](https://arxiv.org/abs/2606.24337)
*Marie Mikulová,Barbora Štěpánková,Daniel Zeman,Jan Štěpánek,Milan Straka,Jan Hajič*

Main category: cs.CL

TL;DR: 论文描述了将Prague Dependency Treebank-Consolidated (PDT-C)转换为Universal Dependencies (UD)的过程，处理了两种标注方案之间的差异。


<details>
  <summary>Details</summary>
Motivation: 将更大且更多样的PDT-C转换为UD，以丰富捷克语的UD资源。

Method: 通过分析两种标注方案的拓扑和粒度差异，讨论动机并采用转换方法克服差异。

Result: 成功将PDT-C转换为UD，展示了差异示例和转换方案。

Conclusion: 尽管PDT不那么‘通用’，但其多层标注提供了足够信息用于UD树。
...
TL;DR: 该研究针对土耳其语低资源环境下的诈骗电话检测，创建了首个多模态数据集并评估多种LLM，发现基于转录的输入优于直接音频处理，强调需要语言包容的AI安全研究。


<details>
  <summary>Details</summary>
Motivation: 诈骗电话对弱势群体造成威胁，但现有检测研究集中于高资源语言，低资源语言如土耳其语因标注数据稀缺和技术限制而面临挑战。

Method: 构建包含100个对齐音频-文本对的土耳其语诈骗/正常对话数据集，评估7个LLM（Gemini 2.5、GPT-4o、Qwen）在原始音频、自动转录和人工精炼转录三种输入条件下的表现。

Result: 基于转录的输入始终优于直接音频处理，且人工纠正与未纠正的转录性能相近。

Conclusion: 研究凸显了在低资源语言和真实威胁背景下进行文化和语言包容的AI安全研究及开发更稳健的多模态欺诈预防系统的必要性。
...
TL;DR: Agora is a benchmark for archive-grounded reasoning requiring agents to locate sparse evidence across large document collections, with current best accuracy at 59.4%.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks only cover parts of archive-grounded reasoning and do not jointly stress archive-groundedness, agentic exploration, and cross-domain coverage.

Method: Agora was built using an agentic pipeline combining cross-document task synthesis, leakage-preventing obfuscation, and difficulty filtering, resulting in 362 questions from 8 domain collections of 9,664 authentic documents and 372M tokens.

Result: Evaluating eight models, the strongest reaches only 59.4% accuracy, with notable variation across domains.

Conclusion: The task remains far from solved, highlighting the need for further research in archive-grounded reasoning.
...
### [166] [To Compare, or Not to Compare: On Methodological Practices in Evaluating Social Bias](https://arxiv.org/abs/2606.24596)
*Federico Marcuzzi,Xuefei Ning,Roy Schwartz,Iryna Gurevych*

Main category: cs.CL

TL;DR: 大型语言模型的社会偏见评估中，孤立评估和比较设置存在系统性范式差距，比较设置会催化潜在歧视，且随模型规模增大而增强。


<details>
  <summary>Details</summary>
Motivation: 当前社会偏见评估文献存在方法论碎片化导致矛盾结论，主要忽略了基准评估的结构框架。

Method: 引入统一可控的框架标准化异构基准，系统对比孤立人口统计评估与强制选择比较设置，并解耦思维链推理、中性备选选项等结构伪影的混淆效应。

Result: 发现大规模系统性范式差距：比较设置作为潜在歧视的催化剂，思维链推理加剧偏见，即使有中性选项偏见仍然存在，且偏见随模型规模扩大。

Conclusion: 研究人员应使用比较设置审计隐藏偏见，但从业者不应在模糊真实世界任务中依赖比较部署。
...
### [167] [Qwen-AgentWorld: Language World Models for General Agents](https://arxiv.org/abs/2606.24597)
...
TL;DR: 提出基于语言模型的世界模型Qwen-AgentWorld，通过三阶段训练和大量真实轨迹显著优于现有模型，并展示了作为模拟器和基础模型的潜力。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型世界模型如何推动通用代理的边界，构建能够模拟多种代理环境的语言世界模型。

Method: 构建Qwen-AgentWorld模型，采用三阶段训练：CPT注入通用世界建模能力，SFT激活下一状态预测推理，RL通过混合奖励提高模拟保真度；使用超过1000万条真实环境交互轨迹；提出AgentWorldBench基准进行评估。

Result: Qwen-AgentWorld显著优于现有前沿模型；作为环境模拟器支持强化学习训练，作为基础模型通过世界模型训练热身提高下游性能。

Conclusion: 语言世界模型在代理模拟和基础模型方面有巨大潜力，能够提升通用代理的能力。
...
### [168] [Same Lesson, Different Story: Cross-Lingual Reconstruction of Cultural Narratives in Large Language Models](https://arxiv.org/abs/2606.24610)
*Jory Alshaalan,Haya Albaker,Abeer Aldayel,Aljawharah Alabdullatif,Rehab Alahmadi*

Main category: cs.CL

TL;DR: 提出了多语言叙事评估框架，使用语义等价的谚语评估LLM在跨文化叙述中的文化基础保留，发现跨语言提示保留语义但改变叙事结构，模型间具有收敛性。


<details>
  <summary>Details</summary>
Motivation: 当多种文化传达相同道德教训时，评估文化基础背景变得复杂，且不确定LLM是否保留跨文化的文化基础意义。

Method: 构建包含414条谚语（覆盖15种语言）的多语言评估框架，使用4个LLM生成13k个叙事，以语义等价的谚语作为提示，分析意义保留、跨语言条件作用和模型收敛性。

Result: 跨语言提示基本保留谚语语义，但系统性地重新分配了代理、社会定位和叙事结构；在单语和跨语言设置中观察到强模型间收敛。

Conclusion: 需要更全面的文化基础评估；仅依赖语义相似性可能高估文化保留，忽视了叙事表达中的文化变异。
...
### [169] [The Warrant Gap: Claim-Conditioned Re-scoring for Fact-Checking](https://arxiv.org/abs/2606.24627)
*Arka Ujjal Dey,John Collomosse*

Main category: cs.CL

TL;DR: 提出SIFT和WSP方法，解决LLM事实核查中标签与证据不匹配的问题，在多个数据集上提升准确率并校准人类标准。


<details>
  <summary>Details</summary>
Motivation: 现有LLM事实核查系统在标准基准上准确率高，但常输出支持标签而引用的证据不合理；结构化分解会丢失完整声明的上下文。

Method: 引入SIFT（基于声明条件对证据跨度重新评分）和WSP（自动NLI检查引证是否蕴含声明）。

Result: 在FEVER、SciFact、5PILS、DP四个数据集上，SIFT恢复因简单分解损失的准确率高达27.6点，WSP的AUC为0.92、精确率0.98。

Conclusion: SIFT有效提升WSP，WSP与人类黄金证据高度一致，可作为可靠自动评估。
...
### [170] [Harmonic: Hierarchical State Space Models for Efficient Long-Context Language Modeling](https://arxiv.org/abs/2606.24650)
*Petr Nyoma*

Main category: cs.CL

TL;DR: 提出了一种分层状态空间模型Harmonic，通过堆叠三个不同时间尺度的循环层并使用预测误差作为输入，在长序列语言建模上优于Transformer和Mamba，且计算复杂度为O(L)。


<details>
  <summary>Details</summary>
Motivation: 解决标准Transformer在长序列上的二次复杂度和位置编码限制，以及状态空间模型在层次化表示上的不足。

Method: 构建三层递归状态空间模型，每层接收下层预测误差而非隐藏状态，实现不同时间尺度的层次化处理。

Result: 在enwiki8、WikiText-103等基准上，Harmonic在1K-32K序列长度上一致优于Transformer和Mamba，且能训练64K序列而其他模型内存溢出；在1B参数规模下，替换TinyLlama的注意力层后消除了RoPE限制，在Lambada上8K长度时损失低9.4 bpt。

Conclusion: Harmonic通过层次化结构和预测误差传递，实现了对长序列的高效建模，克服了Transformer的位置编码限制和二次复杂度，为长上下文语言建模提供了新方向。

Abstract: We present Harmonic, a hierarchical state space model (SSM) for language modeling. The architecture stacks three recurrent levels at progressively slower timescales; each level receives the prediction error of the level below as input, rather than its raw hidden state. On enwiki8 with equal token budgets, Harmonic outperforms a comparable Transformer (28M params) by +1.4% at 1K tokens, +6.7% at 8K tokens, and +11.4% at 32K tokens (bpt, lower is better). It also outperforms Mamba at every tested length by 0.7--1.8%. At 64K tokens, both Mamba and Transformer run out of memory on an 80GB H100; Harmonic trains successfully, reaching 6.169 bpt. Results replicate on WikiText-103 (H-TF gap +1.7% to +7.2% across 1K--32K). At 1B parameter scale, replacing all attention layers in TinyLlama 1.1B with HarmonicBlock eliminates the RoPE positional encoding limit: the resulting Hallamonic model maintains stable loss across sequence lengths 1K--8K on two independent clean benchmarks (Lambada and fineweb-edu held-out), while TinyLlama degrades catastrophically past its 2K-token RoPE limit (gap: +9.4 bpt at seq=8K on Lambada). Compute is O(L) per forward pass vs. O(L^2) for attention. Logs: .

</details>


### [171] [CN-NewsTTS Bench: a target-level automatic benchmark for raw-input Chinese news TTS pronunciation](https://arxiv.org/abs/2606.24714)
*Shijun Luo*

Main category: cs.CL

TL;DR: 提出CN-NewsTTS Benchmark用于评估中文新闻TTS系统对密集书写形式的发音准确性。


<details>
  <summary>Details</summary>
Motivation: 中文新闻文本中密集书写形式（如分数、模型名称、范围、单位符号、百分比、英文缩写、混合名称）频繁出现，TTS系统可能保持字符串不变但改变口语含义，需要评估其发音正确性。

Method: 构建CN-NewsTTS Bench v0.1基准，包含开发集、测试集、自动评估目标和来自三个ASR集成的转录，对七个产品TTS系统进行测试，并报告多种分析。

Result: 最佳系统达到0.879严格准确率，多个系统低于0.60。

Conclusion: 该基准为评估中文新闻TTS在处理密集书写形式上的性能提供了标准化方法，揭示了现有系统的不足。
...
### [172] [Posterior Refinement: Fast Language Generation via Any-Order Flow Maps](https://arxiv.org/abs/2606.24773)
...
TL;DR: FMLM+ combines flow matching with masking noise schedules and posterior refinement for efficient, high-quality non-autoregressive generation.


<details>
  <summary>Details</summary>
Motivation: Bridging the gap between MDMs (suffering from factorization error) and FMLMs (lacking inference-time flexibility) to achieve both quality and flexibility.

Method: Equipping FMLM with masking-style noise schedules and introducing Posterior Refinement, an inference-time strategy that adaptively self-corrects outputs.

Result: Matches performance of discrete baselines with 32x fewer NFEs and improves speed-quality tradeoff over both MDM and FMLM families.

Conclusion: FMLM+ with Posterior Refinement provides a scalable foundation for high-fidelity language modeling.
...
### [173] [Less is More: Quality-Aware Training Data Selection for Scientific Summarization](https://arxiv.org/abs/2606.24828)
*Maria Nefeli Paraskevopoulou,Tatiana Passali,Grigorios Tsoumakas*

Main category: cs.CL

TL;DR: We construct a large biomedical summarization dataset and show that training on high-quality author-written abstracts improves summarization performance.


<details>
  <summary>Details</summary>
Motivation: Scientific summarization datasets are limited in scale and often have varying reference quality; we address both challenges by creating a large dataset and analyzing abstract quality.

Method: We built a dataset of 1.88 million PMC articles and analyzed author-written abstracts using source-grounded and model-based metrics; we then trained summarization models on quality-selected subsets.

Result: Quality signals guide training-data selection; training on selected high-quality subsets outperforms random sampling at matched sizes and can match larger random subsets on factuality metrics.

Conclusion: Reference quality is important in scientific summarization, and quality-aware data selection can improve training efficiency.
...
### [174] [Layer-wise Probing of wav2vec 2.0 and Whisper for Consonant Cluster Reduction in African American English](https://arxiv.org/abs/2606.23948)
*Hamid Mojarad,Kevin Tang*

Main category: cs.CL

TL;DR: 自监督和监督语音模型以结构化渐变方式编码非洲裔美国英语的辅音丛简化，而非简单删除。


<details>
  <summary>Details</summary>
Motivation: 辅音丛简化在非洲裔美国英语中普遍存在，是自动语音识别差异的来源，但尚未充分研究其语音模型中的表征方式。

Method: 对wav2vec2-base和Whisper-small进行说话人无关的逐层探测，使用段简化检测和段简化恢复底层丛身份两个任务。

Result: 两种模型均能高精度区分简化和规范形式，且简化段保留底层爆破音线索，表明CCR是结构化渐变语音变化。

Conclusion: 现代语音模型对非洲裔美国英语的辅音丛简化模式存在结构化的语音编码。
...
### [175] [Faithful by Construction: Claim-Anchored Attribution for Multi-Document Summarization](https://arxiv.org/abs/2606.23989)
*Shuo Guan*

Main category: cs.CL

TL;DR: 提出CAMS框架，通过声明锚定的多文档摘要，显著提升忠实度和引用精度，同时暴露忠实度-覆盖率的可控权衡。


<details>
  <summary>Details</summary>
Motivation: 端到端大语言模型生成的多文档摘要容易产生幻觉且归因粗粒度（整文档或段落），事后归因导致摘要语句难以验证。

Method: 基于Extract-Select-Rewrite范式，先提取带词级来源的原子声明，跨文档聚类等价声明并标记冲突，再选择支持度高且显著的子集，最后重写为摘要，每个句子锚定到支持检查后的声明，链接到多个源片段。

Result: 在MultiNews等数据集上，CAMS在摘要质量上与强基线持平，同时大幅提升忠实度和引用精度，多源归因准确率提升约三分之二，并展现出可控制的忠实度-覆盖率权衡。

Conclusion: CAMS通过结构化保留细粒度多源可追溯性，以及支持感知选择、约束重写和验证，有效促进事实忠实性，为多文档摘要提供了一种归因导向且忠实度导向的解决方案。
...
### [176] [Predicting Poets' Origins from Verse: A Computational Analysis of Regional Linguistic Fingerprints in the Complete Tang Poems](https://arxiv.org/abs/2606.24093)
*Chi-Sheng Chen,Hung-Yun Liu*

Main category: cs.CL

TL;DR: 通过分析《全唐诗》中诗人的作品，发现地理起源在诗词语言中留下可检测的痕迹，模型能够以高于基准的准确率预测诗人的南北区域和所属道，并揭示了语言距离与地理距离的相关性以及时间动态效应。


<details>
  <summary>Details</summary>
Motivation: 探究唐代诗人的地理起源是否在其作品中留下语言痕迹，以及这些痕迹在文学史中的意义。

Method: 构建包含357位诗人和十道（唐行政区划）的诗级语料库，使用字符n-gram TF-IDF结合可解释领域特征（意象、季节、典故），采用经典分类模型和神经网络进行多类别起源预测，并使用层次冻结编码器的古典中文Transformer（GuwenBERT）进行对比。

Result: 南北区域分类准确率0.69（基准0.53），道级分类高于随机。发现语言距离与地理距离正相关（Mantel r=0.40，p≈0.09）；南北可分离性在盛唐最弱、晚唐最强；早期的错误分类具有历史意义（南方诗人被误判为北方）。GuwenBERT未超越TF-IDF，两者结合无提升。

Conclusion: 可解释的机器学习可以作为文学史的假设生成工具，揭示地理起源对诗歌语言的影响，并展示其历史演变。
...
### [177] [PORTER: Language-Grounded Event Representations for Portable Structured EHR Foundation Models](https://arxiv.org/abs/2606.24102)
*Lin Lawrence Guo,Adam Paul Yan,Emily Vettese,Lillian Sung*

Main category: cs.CL

TL;DR: PORTER是一种语言驱动的EHR基础模型，通过解耦事件表示与固定词汇表，实现跨词汇转移并保持性能。


<details>
  <summary>Details</summary>
Motivation: 现有EHR基础模型将临床事件编码为固定词汇的离散标记，无法表示未见概念或数值属性，限制了跨机构和流水线的迁移能力。

Method: PORTER使用冻结文本编码器通过描述表示事件，通过专用路径整合数值，并采用自回归预训练的时间骨干学习临床动态。

Result: 在儿科医院的74个任务中，PORTER达到与固定词汇模型相同的平均AUROC；转移到未见描述时无需重新训练即恢复97.1%的AUROC；在MIMIC上优于固定词汇模型（后者丢失69%的事件）；数值路径提高对数值大小的敏感性；计算效率比任务特定文本序列化方法高329倍。

Conclusion: PORTER是迈向词汇无关EHR基础模型的一步，减少了词汇协调需求，同时保持领域性能并支持高效跨任务复用。
...
### [178] [Pigeonholing: Bad prompts hurt models to collapse and make mistakes](https://arxiv.org/abs/2606.24267)
*Hyunji Nam,Keertana Chidambaram,Dorottya Demszky,Natasha Jaques*

Main category: cs.CL

TL;DR: 本文研究了大语言模型中由不良上下文导致的“鸽笼现象”（pigeonholing），即性能下降和模式崩溃，并提出了基于合成错误的RLVR缓解方法，将模型改进43-60%。


<details>
  <summary>Details</summary>
Motivation: 尽管上下文学习在LLMs中有效，但不良上下文（如用户错误建议或助手先前错误回答）会无意中导致性能严重下降和模式崩溃，这种现象缺乏系统研究。

Method: 在10个可验证和开放任务中，使用10种模型，系统评估了两种场景（用户建议和助手先前错误）下的鸽笼现象，并提出了RLVR with synthetic errors作为缓解方法。

Result: 鸽笼现象导致性能下降38-40%，错误重复从1次增至5次时额外下降14+%，即使正确上下文也可能引发模式崩溃；所提缓解方法在不良上下文下将模型改进43-60%。

Conclusion: 鸽笼现象是LLMs中的一个重要问题，RLVR with synthetic errors能有效缓解，但未来需进一步探索更鲁棒的方法。
...
### [179] [Prague Dependency Treebank -- Consolidated 2.0: Enriching a Complex Annotation Scheme](https://arxiv.org/abs/2606.24324)
*Marie Mikulová,Jiří Mírovský,Milan Straka,Pavlína Synková,Jan Štěpánek,Barbora Štěpánková,Jan Hajič*

Main category: cs.CL

TL;DR: Second consolidated version (PDT-C 2.0) of the Prague Dependency Treebank, a 4-million-token Czech language resource with inter-sentential phenomena annotations.


<details>
  <summary>Details</summary>
Motivation: To systematically include and link different layers of language, especially coreference and discourse relations.

Method: Sustained development over 30 years with uniform annotation across genres, accompanied by fully compatible lexicons.

Result: A richly linguistically annotated corpus used in international NLP comparisons and conversions into other formalisms, available under CC BY-NC-SA.

Conclusion: Concludes a 30-year project and is widely used for linguistic research and NLP development.
...
### [180] [Automatic Part-of-Speech Tagging of Arabic-English Dictionary Senses through WordNet](https://arxiv.org/abs/2606.24359)
*Diaa M. Fayed,Aly A. Fahmy,Mohsen A. Rashwan,Wafaa K. Fayed*

Main category: cs.CL

TL;DR: Proposes a resource-light algorithm for POS tagging bilingual dictionary senses by transferring tags from English translation equivalences obtained from WordNet, achieving high accuracy at low cost.


<details>
  <summary>Details</summary>
Motivation: Building NLP tools requires significant resources; resource-light approaches are needed. POS tagging of dictionary senses is prerequisite for linking to WordNet or standardizing into WordNet-LMF.

Method: Transfers POS tags from English translation equivalences (from Princeton WordNet) to dictionary senses after disambiguation, applied to the Al-Mawrid Arabic-English dictionary.

Result: Registered accuracy is high though the cost is little.

Conclusion: The algorithm provides an effective resource-light approach for POS tagging bilingual dictionaries, beneficial for poor-resource languages.
...
### [181] [MorfFlex: Handling Rich Morphology](https://arxiv.org/abs/2606.24366)
*Jaroslava Hlaváčová,Marie Mikulová,Barbora Štěpánková,Milan Straka,Jan Hajič*

Main category: cs.CL

TL;DR: MorfFlex是一种形态词典架构，通过模式化减少规模，支持超1亿词形和100万词条，用于语料库注释一致性和先进的NLP工具。


<details>
  <summary>Details</summary>
Motivation: 处理形态丰富的语言，确保手动形态注释的一致性，并为自动工具提供基础。

Method: 手工维护源文件，编码屈折和派生模式，转换为简单的三元组列表发布，利用模式大幅缩减词典规模。

Result: 构建了MorfFlex CZ词典，包含超过1亿词形和超过100万词条，并确保了Prague Dependency Treebanks的注释一致性，支持MorphoDiTa等工具。

Conclusion: 本文展示了有效管理丰富形态系统的方法，以及该语言资源在保持注释一致性和支持高级NLP应用中的实用性。
...
### [182] [Beyond Logprobs: A Multi-Signal Confidence Engine for LLM-Based Document Field Extraction](https://arxiv.org/abs/2606.24420)
*Nitesh Kumar*
...
### [183] [Measuring User's Mental Models of Speech Translation in Human-AI Collaboration](https://arxiv.org/abs/2606.24644)
*HyoJung Han,Nishant Balepur,Jordan Boyd-Graber,Marine Carpuat*

Main category: cs.CL

TL;DR: 通过跨语言问答任务研究用户对语音翻译系统的心理模型，发现实践和源语言知识能改善心理模型，语音转录也有帮助。


<details>
  <summary>Details</summary>
Motivation: 尽管数百万人日常使用机器翻译，但对其感知和能力界限了解甚少，本研究旨在探索用户对语音翻译系统的心理模型。

Method: 提出基于跨语言问答的新框架，用户接受MT输出或请求专业重译来回答问题，分析行为与准确性趋势。

Result: 用户通过实践发展更强的心理模型，尤其当有源语言知识时，主要依赖表面错误线索；提供语音转录有助于发展更好的心理模型。

Conclusion: 跨语言问答作为下游任务在研究MT心理模型方面具有前景，有助于理解人机协作。
...
### [184] [Task Decomposition for Efficient Annotation](https://arxiv.org/abs/2606.24734)
*Nupoor Gandhi,Emma Strubell*

Main category: cs.CL

TL;DR: 提出将结构化注释任务分解为子任务以减少推理负担，并基于中心理论引入推理负担模型来指导分解和分配，从而提高成本效率。


<details>
  <summary>Details</summary>
Motivation: 高质量结构化注释成本高昂，传统端到端注释复杂且推理负担重，而现代注释项目使用异构注释者，需要重新设计任务以降低负载。

Method: 将注释任务分解为子任务，基于中心理论建立推理负担模型（自由度度量），通过识别中心来约束输出空间，提供分解指南和子任务分配程序。

Result: 在先前工作中展示分解能提高成本效率。

Conclusion: 通过分解和合理分配子任务，可在固定预算下最大化注释质量。
...
### [185] [CompressKV: Semantic-Retrieval-Guided KV-Cache Compression for Resource-Efficient Long-Context LLM Inference](https://arxiv.org/abs/2606.24467)
*Xiaolin Lin,Jingcun Wang,Olga Kondrateva,Yiyu Shi,Bing Li,Grace Li Zhang*

Main category: cs.AI

TL;DR: CompressKV通过识别语义检索头和分层预算分配，在极低KV缓存下保持高推理性能，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存驱逐方法忽略注意力头的不同功能，导致关键标记被驱逐，性能下降，需要更智能的压缩框架。

Method: CompressKV识别语义检索头（SRHs）以选择保留关键标记的KV对，并根据离线估计的逐层驱逐错误分配缓存预算。

Result: 在LongBench上，仅用3%缓存保留97%以上全缓存性能；在Needle-in-a-Haystack上，用0.7%缓存达到90%准确率。

Conclusion: CompressKV在长上下文LLM推理中实现了改进的资源-性能权衡。
...
### [186] [Breaking the Filter Bubble: A Semantic Pareto-DQN Framework for Multi-Objective Recommendation](https://arxiv.org/abs/2606.24042)
*Cláudio Lúcio Do Val Lopes,Lucca Machado da Silva,André de Oliveira Brandão*

Main category: cs.AI

TL;DR: 提出基于 Pareto-DQN 的多目标强化学习推荐框架，通过超体积动作选择平衡参与度、多样性和公平性，在 MovieLens 数据集上实现了社会目标增益且保持参与度。


<details>
  <summary>Details</summary>
Motivation: 推荐系统单目标优化导致过滤气泡和语义同质化，标准 DQN 等模型无法权衡平台留存与信息多样性、公平性等社会价值。

Method: 将推荐形式化为语义多目标马尔可夫决策过程，使用高保真语义嵌入与 Pareto-DQN 智能体，将参与度、多样性、公平性作为独立奖励信号，采用基于超体积的动作选择。

Result: 在 MovieLens 小数据集上，该方法打破了语义崩溃反馈循环，维持高状态轨迹方差，有效映射帕累托前沿，在辅助社会目标上取得显著增益，仅对参与度有微小影响。

Conclusion: 该工作为构建内在一致、负责任的推荐系统提供了路径。
...
### [187] [Age of LLM: A Strategic 1v1 Benchmark for Reasoning, Diplomacy and Reliability of Large Language Models under Fog of War](https://arxiv.org/abs/2606.24391)
*Arnaud Ricci*

Main category: cs.AI

TL;DR: 提出一个新的LLM基准测试Age of LLM，通过战争游戏评估模型在对抗不确定性下的表现，发现核rush策略主导，但可靠性与获胜关联弱。


<details>
  <summary>Details</summary>
Motivation: 现有公开基准易受数据污染，且缺乏对LLM在复杂对抗环境（如信息不完整、欺骗）中推理能力的评估。

Method: 设计13x7网格的回合制1v1战争游戏，包含战争迷雾、完全外交和可靠性维度三种压力因素，使用规则提示，对15个推理模型进行54场比赛，共5258个动作，记录所有动作和消息。

Result: 五个发现：(1) 核rush主导（78-85%）；(2) 军事征服罕见但更快；(3) 外交频繁但很少达成；(4) 约58%非法动作源于迷雾/状态错误；(5) 可靠性与获胜之间存在弱关联。

Conclusion: 数据集小且不平衡，排名仅为初步描述，但轨迹和消息为研究LLM在对抗不确定性下的推理（如信念追踪、欺骗）提供了视角，并作为未来研究方向。
...
### [190] [Governed Shared Memory for Multi-Agent LLM Systems](https://arxiv.org/abs/2606.24535)
*Yanki Margalit,Nurit Cohen-Inger,Erni Avram,Ran Taig,Oded Margalit*

Main category: cs.AI

TL;DR: 本文形式化了多智能体LLM环境中的舰队记忆问题，定义了四个系统级原语以避免四种故障模式，在MemClaw中实现，并通过ArgusFleet评估，揭示了现实架构问题，得出结论：受控共享记忆需要明确的系统级抽象。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM环境需要稳健的共享知识管理机制。

Method: 形式化舰队记忆问题并识别四种故障模式，定义系统级原语（范围检索、时间更替、来源追踪、策略驱动的记忆传播），在MemClaw中实现，通过ArgusFleet在四个治理维度上评估生产服务。

Result: 来源追踪成功重建100%深度为4的推导链，传播表现为高内舰队可见性和零交叉舰队泄漏；发现了不对称范围执行和流水线排序冲突两个架构问题。

Conclusion: 仅靠长上下文检索不足以支撑生产多智能体记忆；受控共享记忆需要明确的系统级抽象，并且实际评估对发现设计遗漏的强制和流水线排序失败至关重要。
...
### [191] [Agentic AI for Bilevel Long-Term Optimization of Policy-Driven Physical Layer Systems](https://arxiv.org/abs/2606.24416)
*Bingnan Xiao,Chenhao Yang,Wei Ni,Xin Wang,Tony Q. S. Quek*

Main category: cs.AI

TL;DR: 提出一种基于agentic AI的双层优化框架Agentic-LTPO，用于自适应物理层配置，在无蜂窝MIMO波束成形中提升长期性能57.2%。


<details>
  <summary>Details</summary>
Motivation: 网络运营商政策变化、服务需求和实时约束使传统固定目标方法失效。

Method: 采用嵌套双层优化，上层通过agentic AI生成配置，下层求解实时物理层决策；针对无蜂窝MIMO波束成形，设计多智能体决策过程和检索增强验证，结合闭式波束成形器。

Result: 实验表明对动态政策适应性强，长期性能比传统方法提升57.2%。

Conclusion: Agentic-LTPO有效增强系统长期性能，适用于动态环境。
...
### [194] [Reinforcement Learning Towards Broadly and Persistently Beneficial Models](https://arxiv.org/abs/2606.24014)
...
TL;DR: 使用强化学习在真实领域训练有益行为，可提升AI对齐的分布外泛化和持久性。


<details>
  <summary>Details</summary>
Motivation: AI系统在高风险场景中部署时，对齐必须泛化到训练分布之外，而强化学习可能引入意外对齐问题。

Method: 构建涵盖健康、科学、教育等领域的真实场景数据集，训练有益特质（如诚实、公平等），然后用强化学习训练模型，并在50多个独立对齐基准上评估。

Result: 相比基线，有益特质RL在80%以上的分布外基准上改善性能；仅健康领域的干预也能提升非健康对齐评估；模型对对抗性提示和有害微调更具抵抗力。

Conclusion: 在真实领域中使用强化学习强化有益行为，可产生更稳健对齐的模型，促进人类福祉。
...
TL;DR: 提出了一个大规模开源对抗攻击数据集，用于评估视觉语言模型的安全性。


<details>
  <summary>Details</summary>
Motivation: 针对生成大量对抗攻击的高计算成本和复杂性，提供易于访问的数据集以促进对抗研究。

Method: 整合并扩展现有基准，覆盖10个类别和55个子类别的有害意图，使用最新对抗攻击策略生成47,524个样本。

Result: 数据集包含47,524个对抗样本，涵盖7,826个意图，并新增类别以拓展覆盖范围。

Conclusion: 该数据集降低了对抗研究门槛，支持系统性评估VLM鲁棒性和安全防御。
...
### [196] [Scaling Laws for Task-Specific LLM Distillation](https://arxiv.org/abs/2606.24747)
...
### [197] [Assessing Distribution Shift in Human Activity Recognition for Domain Generalization](https://arxiv.org/abs/2606.24781)
*Rebecca Adaimi,Edison Thomaz*

Main category: cs.AI

TL;DR: 系统评估了传感器基HAR中四种分布转移的影响，发现多样性转移主导所有类型，且现有域泛化方法仅稍优于ERM基线。


<details>
  <summary>Details</summary>
Motivation: HAR中数据多样性（设备、传感器、上下文变化）导致域泛化问题，但对不同分布转移类型的影响理解不足。

Method: 系统评估四种分布转移（设备类型、传感器位置、采样率、用户行为），量化其效应，构建统一基准，评估28种域泛化方法。

Result: 多样性转移主导所有类型，表明存在不共享的独特特征；当前域泛化方法仅边际优于ERM基线。

Conclusion: 首次系统探索特定分布转移下的域泛化和适应，提供开源基准平台和数据集以促进研究。
...
### [198] [OpenThoughts-Agent: Data Recipes for Agentic Models](https://arxiv.org/abs/2606.24855)
...
TL;DR: OpenThoughts-Agent 提出一个完全开放的数据策划流程，用于训练跨多种代理人任务泛化的模型，通过100多个消融实验和100K训练集微调Qwen3-32B，在七个基准上平均准确率44.8%，超越现有最强开放模型3.9个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有开放代理人训练数据工作通常针对单一基准，缺乏对如何策划数据以训练广泛能力代理人模型的公开研究，需要能跨多样化代理人任务泛化的训练数据。

Method: 构建一个完全开放的数据策划流程，通过100多个受控消融实验系统研究每个阶段，组装100K示例训练集，并在Qwen3-32B上进行微调。

Result: 在七个代理人基准上平均准确率44.8%，比最强现有开放数据代理人模型Nemotron-Terminal-32B（40.9%）高3.9个百分点，且训练数据在计算控制比较中表现出强扩展性。

Conclusion: 公开释放训练集、数据流程、实验数据和模型，以支持未来关于代理人模型训练的开放研究。

Abstract: Agentic language models dramatically expand the applications of AI yet little is publicly known about how to curate training data for broadly capable agents. Existing open efforts such as SWE-Smith, SERA, and Nemotron-Terminal typically target a single benchmark, leaving open the question of how to train models that generalize across diverse agentic tasks. The OpenThoughts-Agent (OT-Agent) project addresses this gap with a fully open data curation pipeline for training agentic models. We conduct more than 100 controlled ablation experiments to systematically investigate each stage of the pipeline, yielding insights on the importance of task sources and diversity. We then assemble a training set of 100K examples from our pipeline and fine-tune Qwen3-32B on this dataset, which yields an average accuracy of 44.8% across seven agentic benchmarks and a 3.9 percentage point improvement over the strongest existing open data agentic model (Nemotron-Terminal-32B, 40.9%). Moreover, our training data exhibits strong scaling properties, outperforming alternative open datasets at every training set size in compute-controlled comparisons. We publicly release our training sets, data pipeline, experimental data, and models at to support future open research on agentic model training.
...
TL;DR: 提出因果强化学习（CRL）框架，将因果推断与强化学习结合，统一多种学习模式并引入新类别。


<details>
  <summary>Details</summary>
Motivation: 因果推断和强化学习虽独立发展，但都基于反事实关系，有结合潜力以实现新的学习机会。

Method: 将环境分解为结构因果模型，基于此统一在线学习、离策略学习和因果演算学习，并引入广义策略学习、模仿学习和反事实学习等新类别。

Result: 引入了新的学习设置，扩展了反事实学习视野，并提出了因果强化学习的概念。

Conclusion: 因果推断和强化学习可以协同工作，潜力巨大，值得深入研究。
...
### [200] [Navigating User Behavior toward Personalized Multimodal Generation](https://arxiv.org/abs/2606.24196)
*Hengji Zhou,Yufeng Liu,Ye Liu,Yong Xu,Lianghao Xia,Liqiang Nie*

Main category: cs.AI

TL;DR: 提出NaviGen，通过双重标识符表示用户行为并两阶段训练，实现个性化图像和视频生成。


<details>
  <summary>Details</summary>
Motivation: 用户难以提供详细视觉指令，导致生成器与需求不匹配，需要将交互历史转化为可执行指令。

Method: 每个项目采用协同编码和文本编码的双重标识符，通过SFT+RL两阶段训练：先蒸馏偏好推理和指令编写，再用层次化与自一致性奖励对齐用户意图。

Result: 在产品、游戏和短视频领域改进了个性化生成，增强了下一项预测，生成了更具体、相关且可视觉生成的指令。

Conclusion: NaviGen有效对齐用户需求与生成输出，代码已开源。
...
### [201] [LemonHarness Technical Report](https://arxiv.org/abs/2606.24311)
...
TL;DR: LemonHarness通过明确工作空间边界、可复用规则知识库和感知时间的执行机制提高了长期代理的执行稳定性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型代理在长期任务中修改工作空间状态，但缺乏明确边界导致状态变化分散难以跟踪，弱约束变化积累使得状态管理困难。

Method: 提出LemonHarness框架，建立显式执行边界约束状态操作，通过结构化工具接口执行文件写入、依赖安装等操作并记录反馈；引入可复用规则知识库将执行规则和验收标准转化为运行时知识；增加时间感知执行机制，暴露已用和剩余预算给模型以重新平衡探索、实现和验证工作。

Result: 在Terminal-Bench 2.0上，LemonHarness_GPT-5.3-CodeX在445次试验中达到84.49%准确率；使用更强GPT-5.5骨干网将平均准确率提升至86.52%。

Conclusion: 统一运行时边界、可调用规则知识和时间感知执行能改善长期代理执行的稳定性。
...
### [202] [The Latent Bridge: A Continuous Slow-Fast Channel for Real-Time Game Agents](https://arxiv.org/abs/2606.24470)
*Bojie Li,Noah Shi*

Main category: cs.AI

TL;DR: 耦合一个慢推理VLM和一个快反应VLM，通过可训练的潜在桥（Latent Bridge）将慢模型的残差投影到快模型的输入空间，在实时控制任务中达到与文本桥相当或更好的性能，且仅在慢模型本身优于快模型时有效。


<details>
  <summary>Details</summary>
Motivation: 实时代理需要在毫秒级行动但规划秒级，单个模型难以兼顾延迟与质量；快模型反应快但规划弱，慢模型规划强但反应慢，因此需要耦合两者。

Method: 将两个冻结的VLM（9B反应型、8B推理型）耦合，仅训练通信通道。提出两种桥：文本桥（慢模型写文本后缀供快模型读取）和潜在桥（将慢模型残差通过可训练投影映射到快模型输入嵌入空间，类似LLaVA）。在7个Atari游戏和MetaDrive驾驶域上，为每个通道单独训练动作解码器。

Result: 潜在桥在几乎所有域匹配或超过文本桥：在MsPacman和RoadRunner上分别提升57%和28%，在其余任务上是安全替代。组合两个桥会严重干扰（RoadRunner降低96%）。桥的增益高度可预测：仅当慢推理本身优于快反应（T>F）时才有效，且潜在增益与文本增益高度相关（r=0.93）。在MetaDrive中两者均无增益，作为受控负例。

Conclusion: 潜在桥是文本桥的安全且有效的替代方案，尤其适用于规划密集型任务；应仅使用一种桥以避免干扰；收益可预测，便于实际部署。
...
### [203] [ScaleToT: Generalizing Structured LLM Reasoning for Billion-Scale Low-Activity User Modeling](https://arxiv.org/abs/2606.24605)
*Tianbao Ma,Chang Xi,Yichuan Zou,Chengen Li,Linxun Chen,Zilong Lu,Yanan Niu,Zhaojie Liu,Han Li,Kun Gai*

Main category: cs.AI

TL;DR: ScaleToT通过从少量LLM处理样本学习结构化推理，并扩展到数十亿低活跃用户，实现高效准确的用户建模。


<details>
  <summary>Details</summary>
Motivation: 准确用户建模依赖于丰富交互历史，但低活跃用户缺乏这些历史；LLM直接推理稀疏配置文件不可靠且成本高昂。

Method: 构建带类型用户状态链，使用有界熵引导的思维树（ToT）细化；通过教师链训练学生模型（SFT+OSIPO），再将推理表示转移到轻量级配置文件编码器。

Result: 在十亿级广告部署中，在线A/B测试将LT30提高6.738%，离线推理覆盖7.32%的潜在人群，大幅降低计算成本。

Conclusion: ScaleToT能高效扩展结构化推理到低活跃用户，平衡了推理质量和计算开销。
...
### [204] [RIFT-Bench: Dynamic Red-teaming For Agentic AI Systems](https://arxiv.org/abs/2606.23927)
*Yarin Yerushalmi Levi,Roy Betser,Amit Giloni,Lidor Erez,Itay Gershon,Oren Rachmil,Sindhu Padakandla,Roman Vainshtein*

Main category: cs.AI

TL;DR: RIFT-Bench is a graph representation-driven methodology for dynamic red-teaming that enables unified security evaluations across diverse agentic AI architectures.


<details>
  <summary>Details</summary>
Motivation: Existing security evaluations are often tied to specific implementations or domains, limiting unified comparison across heterogeneous systems.

Method: RIFT-Bench uses a novel hierarchical representation and operates in two automated phases: Discovery, which extracts system structure, and Scanning, which deploys adaptive adversarial attacks and produces a comprehensive evaluation report.

Result: Demonstrated effectiveness across 45 agentic systems spanning diverse implementations, showing generalization to heterogeneous architectures.

Conclusion: RIFT-Bench supports direct evaluation of mitigation strategies and provides a scalable foundation for security evaluation of agentic AI systems.
...
### [205] [Neuro-Symbolic Drive: Rule-Grounded Faithful Reasoning for Driving VLAs](https://arxiv.org/abs/2606.23938)
*Xiangbo Gao,Xiukun Huang,Boyu Lu,Junge Zhang,Mengjie Mao,Jiachen Li,Wei Xiong,Zhengzhong Tu*

Main category: cs.AI

TL;DR: Neuro-Symbolic Drive uses rule-based planner decision traces to supervise a driving VLA model, improving reasoning-motion coupling and performance.


<details>
  <summary>Details</summary>
Motivation: Current driving VLA models with CoT reasoning lack step-by-step decision semantics and causal connection between reasoning and planned motion.

Method: Instrument rule-based planners to capture executed trajectories and internal decision traces, serialize them into rule-grounded reasoning, and use them to fine-tune a driving VLA model (Qwen3.5-4B).

Result: On the simulator benchmark, ADE@3s reduced from 0.47 to 0.26 and miss rate from 8.30% to 6.40% under three-camera perception, and from 0.54 to 0.26 and 10.13% to 5.99% under eight-camera perception.

Conclusion: Neuro-Symbolic Drive converts neuro-symbolic planning logic into structured supervision, effectively improving driving VLA performance.
...
### [206] [Can Language Model Agents be Helpful Circuit Explainers in Mechanistic Interpretability?](https://arxiv.org/abs/2606.24026)
*Ayan Antik Khan,Harsh Kohli,Yuekun Yao,Huan Sun,Ziyu Yao*

Main category: cs.AI

TL;DR: 提出了AgenticInterpBench基准和HyVE代理解释器，用于自动解释Transformer电路组件，实验表明LM代理有潜力但可靠验证仍是障碍。


<details>
  <summary>Details</summary>
Motivation: 可解释性研究在自动定位电路方面取得进展，但解释组件功能仍费力且难以标准化，因此希望用LM代理辅助解释。

Method: 引入AgenticInterpBench（84个半合成电路，163个组件注释）和HyVE代理，通过迭代循环（观察、假设生成、因果验证）分析每个组件，输出组件解释和电路任务描述。

Result: 在四个LM骨干上，HyVE恢复了有用的解释，但无统一最优；强骨干形成观察性假设，失败多源于验证循环中的不完整计划、代码错误或未解决假设。

Conclusion: LM代理是前途的电路解释器，但可靠验证仍是关键障碍。
...
TL;DR: 提出策略引导策略优化(SGPO)，通过结构化策略描述替代轨迹模仿，提升弱模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的蒸馏方法通过模仿特定的解决方案轨迹来转移答案而非推理过程，导致模型记忆实例特定步骤，缺乏可迁移的问题解决技能，限制了对新问题的泛化能力。

Method: SGPO从强模型响应中提取结构化策略描述，为每个问题构建自主和策略引导轨迹，采用前向KL散度的标记级目标选择性转移策略条件引起的分布变化，并使用自适应实例级权重决定何时进行蒸馏。

Result: 在四个数学基准测试上，SGPO在Qwen2.5-7B-Instruct上平均得分比最强基线高出2.2分，一致优于SFT、在线RL和混合策略基线。

Conclusion: 前向KL目标提供了选择性蒸馏信号，优于直接轨迹模仿；策略蒸馏与基础模型能力呈现互补扩展性。
...
TL;DR: 提出ReMMD，一个真实的多语言多图像agent验证框架，包括基准ReMMDBench和验证器ReMMD-Agent，在五类真实性检测上性能最优且成本更低。


<details>
  <summary>Details</summary>
Motivation: 现有基准和方法不匹配现实多模态错误信息检测中长多语言叙述、多图像、混合来源和微妙图文错误的特点，且代理验证在真实证据搜索下成本高昂。

Method: 构建包含500样本、2756图像、五种单语和两种跨语言设置、三级文本长度、多图像帖子、五级真实性标签等特征的ReMMDBench；提出ReMMD-Agent，采用持久记忆将帖子分解为原子点，构建可重用证据集，预测结构化L1/L2/L3输出。

Result: ReMMD-Agent使用GPT-5.2在五类真实性上达到41.80%准确率和39.12%宏F1，相比MMD-Agent和T2-Agent分别降低17.5%和79.9%的成本。

Conclusion: ReMMD框架为多模态错误信息检测提供了更真实、高效的验证方案，在性能和成本上都优于现有系统。
...
### [209] [OmniPath: A Multi-Modal Agentic Framework for Auditing Wheelchair Accessibility](https://arxiv.org/abs/2606.24129)
*ASM Mobarak Hossain,Nadim Mahmud,Vaskar Raychoudhury,Md Osman Gani*

Main category: cs.AI

TL;DR: OmniPath combines OSM and LiDAR to create a high-fidelity 3D model of pedestrian environments, enabling automated micro-scale inspection of surface conditions for wheelchair users.


<details>
  <summary>Details</summary>
Motivation: Standard maps like OSM fail to convey physical surface conditions of paths, creating information barriers for wheelchair users.

Method: Fuses OSM network topology with high-density aerial LiDAR to build a 3D model, then virtually traverses the network in 0.5m increments, quantifying slope, cross slope, and vertical discontinuities against ADA standards, and computes a weighted severity score.

Result: Validated against 200 physical ground truth surveys, achieving F1-scores of 0.60 for severe and 0.58 for critical hazards.

Conclusion: OmniPath identifies 'invisible' barriers that standard maps miss, transforming static datasets into proactive accessibility data.
...
### [210] [T2D-Bench: Evidence-Gated Evaluation of LLM Outputs for Type 2 Diabetes Using a Multi-Layer Clinical-Lifestyle Knowledge Graph](https://arxiv.org/abs/2606.24145)
*Saba A. Farahani,Hung Cao,Ramesh Jain,Amir M. Rahmani*

Main category: cs.AI

TL;DR: T2D-Bench是一个可重复的基准测试和证据门控评估框架，用于测试LLM在2型糖尿病建议中是否满足显式证据要求。


<details>
  <summary>Details</summary>
Motivation: LLM虽然能生成流畅的临床建议，但常违反糖尿病指南或缺乏证据支持，需要可计算的约束来检测和修正。

Method: 构建多层临床-生活方式知识图谱，结合UMLS、DrugBank等生物医学数据和可计算的ADA护理标准规则，设计100个结构化场景（诊断、药物安全、生活方式冲突），并通过证据门控机制检测未支持的遗漏并进行受限制修订。

Result: 基线输出（GPT-4o-mini 35%、GPT-4o 33%）未通过证据路径检查；证据门能检测并修正这些遗漏，使输出符合基准证据要求。

Conclusion: 可计算的证据约束可以使糖尿病相关LLM输出中的未支持临床遗漏变得显式、可测量和可纠正。
...
### [211] [Exploring the relationship between human-centric AI and firm idiosyncratic risks](https://arxiv.org/abs/2606.24224)
*Zhen-Yuan Ralph Liu,Yu-Ting Wang,Jia-Jia Yan,Shivam Gupta,Mihalis Giannakis*

Main category: cs.AI

TL;DR: HCAI降低企业特质风险，数字化和高管持股增强此效应，而运营效率和IT背景CEO则削弱。


<details>
  <summary>Details</summary>
Motivation: 尽管Industry 5.0中广泛讨论HCAI，但其对企业特质风险的影响尚不明确，而这对企业应对技术革命中的财务风险至关重要。

Method: 采用2015-2023年中国上市公司多源面板数据，结合情境化AI理论与社会技术系统理论进行分析。

Result: HCAI与更低的企业特质风险相关；数字化和高管持股强化风险降低作用，运营效率和CEO的IT背景则意外削弱该作用。

Conclusion: 研究为AI时代的企业伦理AI治理和财务风险管理提供了理论贡献与实践启示。
...
### [212] [FlowR2A: Learning Reward-to-Action Distribution for Multimodal Driving Planning](https://arxiv.org/abs/2606.24231)
*Xirui Li,Zhe Liu,Xiaoqing Ye,Wenhua Han,Yifeng Pan,Junyu Han,Hengshuang Zhao*

Main category: cs.AI

TL;DR: FlowR2A通过将奖励重新定义为生成条件，统一了密集监督和动态提议生成，实现了多模式驾驶规划的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决基于评分的方法（密集监督但固定词汇）和基于锚的方法（动态提议但稀疏监督）之间的紧张关系。

Method: 提出FlowR2A，学习奖励条件动作分布，使用流匹配解码器，引入每时间步奖励条件和奖励噪声增强，支持奖励引导和锚定采样。

Result: 在NAVSIM v1和v2基准测试中取得最先进结果，多模式提议质量显著高于先前方法。

Conclusion: FlowR2A有效平衡了硬安全约束和软进度目标，通过生成式框架提升了提议质量。
...
### [213] [SP-Mind: An Autonomous Reasoning Agent for Spatial Proteomics Analysis](https://arxiv.org/abs/2606.24235)
*Yucheng Yuan,Yuanfeng Ji,Zhongxiao Li,Ruijiang Li*

Main category: cs.AI

TL;DR: SP-Mind is the first autonomous AI agent unifying spatial proteomics analysis, achieving state-of-the-art performance on the SP-Bench benchmark.


<details>
  <summary>Details</summary>
Motivation: Current spatial proteomics analysis workflows are fragmented, requiring expert manual orchestration, limiting scalability and reproducibility.

Method: SP-Mind uses expert-curated biological analysis skills and specialized computational tools to convert natural-language queries into end-to-end analytical workflows without task-specific fine-tuning.

Result: SP-Mind achieves state-of-the-art performance compared to existing open-source biomedical agent baselines on SP-Bench and downstream tasks.

Conclusion: SP-Mind effectively unifies the spatial proteomics pipeline, enhancing scalability and reproducibility in tumor microenvironment research and precision medicine.
...
### [214] [Towards Federated Long-Tailed Graph Learning: An Energy-Guided Dual Decoupling Approach](https://arxiv.org/abs/2606.24237)
*Lianshuai Guo,Zhongzheng Yuan,Xunkai Li,Meixia Qu,Wenyu Wang*

Main category: cs.AI

TL;DR: 提出FedEPD，一种基于双重解耦的长尾联邦图学习方法，通过拓扑纯化和语义重新校准实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在长尾分布下性能退化，因为偏置全局模型和结构隔离少数节点，而拓扑无关的统计补偿在数据稀缺下失败，过度拟合结构噪声。

Method: FedEPD框架，采用双重解耦范式，先利用分布感知Dirichlet能量剪枝过滤异嗜边，然后从拓扑中心节点提取鲁棒全局原型并通过空间低通原型注入对抗Non-IID分布偏移，最后以两阶段交替优化保护多数边界并提高少数精度。

Result: 在各类长尾基准上达到SOTA，Accuracy绝对提升4.97%，Macro-F1提升5.48%。

Conclusion: FedEPD有效解决了联邦图学习中的长尾分布问题，通过拓扑纯化和语义重新校准显著提升性能。
...
### [215] [Probing the Misaligned Thinking Process of Language Models](https://arxiv.org/abs/2606.24251)
*Kaiwen Zhou,Constantin Venhoff,Jonathan Michala,Xin Eric Wang,William Saunders*

Main category: cs.AI

TL;DR: 通过将大型语言模型的错误对齐行为分解为细粒度认知过程指标，并用线性探针从内部激活中检测，实现了在分布外基准上0.935 AUROC的检测性能，同时保持低误报率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型显示出越来越多的错误对齐行为，如策略性欺骗、敷衍和自我保存，这些行为在高度部署中需要可靠检测。

Method: 提出将错误对齐分解为18个细粒度指标，利用元计划引导的自动管道生成多轮训练对话，训练线性探针从模型内部激活检测这些指标，并构建包含自动化行为诱发、既有基准和自然对话的分布外评估集。

Result: 探针在分布外基准上达到0.935 AUROC，与强LLM评判者相当，且在良性对话上保持低误报率。

Conclusion: 该方法能有效检测错误对齐行为，并通过深入分析揭示了探针和模型内部对错误对齐指标的表征。
...
### [216] [Prob-BBDM: a Probabilistic Brownian Bridge Diffusion Model for MRI sequence image-to-image translation](https://arxiv.org/abs/2606.24313)
*Martin Valls,Pascal Bourdon,Christine Fernandez-Maloigne,Guillaume Herpe,David Helbert*

Main category: cs.AI

TL;DR: 提出Prob-BBDM，一种基于布朗桥扩散模型的MRI序列合成方法，仅需4步扩散即可生成高质量图像，并在医学分割任务中验证了临床可用性。


<details>
  <summary>Details</summary>
Motivation: 多模态医学图像分析对优化检查质量至关重要，但获取多种3D成像模态资源密集且耗时，亟需高效的图像翻译方法。

Method: 基于布朗桥扩散模型（BBDM），集成变分编码器引导的扩散机制，从2D轴向切片合成MRI序列，通过概率图像分布提升合成质量。

Result: 在BraTS 2021数据集上SSIM达88.46%、PSNR 26.09 dB，仅需4步扩散；外部数据集表现一致；肿瘤分割Dice 88.71%、HD95 3.49 mm。
...
TL;DR: LLMs show high Causal Caution in academic contexts but significantly lower in practical advisory contexts; a self-correction prompt can restore it.


<details>
  <summary>Details</summary>
Motivation: Previous benchmarks overlooked Causal Caution (refraining from causal judgment when evidence is insufficient), which is crucial for trust in decision-support roles.

Method: Used a PCH-score rubric to evaluate 4 LLMs across 480 trials in academic vs practical advisory contexts, including self-correction prompt experiments.

Result: Causal Caution dropped from 91.7-100% (academic) to 6.7-18.3% (practical), and 0.5% in concrete recommendation prompts; self-correction restored it to 71.4-100%.

Conclusion: Suppression of Causal Caution is context-dependent, not capability-limited; multi-agent architectures with separate causal auditing may improve governance.
...
### [218] [Can Aggregate Invariants Accelerate Continuous Subgraph Matching? Limits, Laws, and a Dynamic Spectral Index](https://arxiv.org/abs/2606.24421)
*Minghao Chen,Jiale Zheng*

Main category: cs.AI

TL;DR: Spectral filtering accelerates continuous subgraph matching over dynamic graphs by maintaining exact local spectra for selective vertices, achieving up to 51% candidate removal or 47% update enumeration skip, with no change in intermediates.


<details>
  <summary>Details</summary>
Motivation: To investigate whether spectral filtering, effective for static subgraph matching, can be applied to continuous subgraph matching (CSM) on dynamic graphs, and to address the challenges of maintaining spectral bounds under updates.

Method: Analyzed lazy vs. exact spectral bound maintenance; proposed selective exact maintenance based on anti-correlation between pruning utility and recomputation cost; integrated a dynamic local-spectra index into a decoupled CSM benchmark.

Result: Spectral filters remove up to 51% candidates or skip 47% updates; intermediates unchanged; radius-stratified workload achieves 99.9% reduction in intermediates and 748× speedup.

Conclusion: Aggregate spectral tests boost candidate construction and list scans but not adjacency-guided exploration; intermediate-invariance methodology is effective for evaluating CSM filters; reusable dynamic local-spectra index released.
...
### [219] [Bayesian control for coding agents](https://arxiv.org/abs/2606.24453)
*Theodore Papamarkou,Vladislav Smirnov,Viktor Mazanov,Artem Vazhentsev,Preslav Nakov,Timothy Baldwin,Artem Shelmanov*

Main category: cs.AI

TL;DR: 将编排视为成本敏感的序列假设检验，贝叶斯控制对编码代理中的工具使用进行动态决策，在昂贵验证和信息性但非完美批评者情况下最有价值，且信念状态提供可解释的正确性分数。


<details>
  <summary>Details</summary>
Motivation: 当前编码代理使用固定规则编排工具，忽略不确定性，导致效率低下或成本过高。

Method: 将编排形式化为成本敏感的序列假设检验，使用贝叶斯控制器维护候选正确性的信念，并动态决定收集更多证据、改进候选、验证或停止。

Result: 在六个生成器和九个编码基准测试中，贝叶斯控制在验证成本高且评论者信息性但不完美时表现最佳；信念状态在不确定性量化上优于令牌概率和原始工具成功基线。

Conclusion: 贝叶斯控制提供了一种有效且可解释的方法来管理编码代理中的工具使用，特别是在验证成本高的情况下。
...
### [220] [A specialized reasoning large language model for accelerating rare disease diagnosis: a randomized AI physician assistance trial](https://arxiv.org/abs/2606.24510)
...
TL;DR: RaDaR是一个32B参数的开源推理LLM，通过49,170个公开病例和104,666个合成病例的推理增强训练，在罕见病诊断中超越包括671B DeepSeek-R1在内的开源模型，并能在临床怀疑前提前诊断，辅助医生提升准确率21.44个百分点。


<details>
  <summary>Details</summary>
Motivation: 罕见病诊断面临临床专家稀缺、现有LLM可部署性差、临床证据有限以及训练数据匮乏等挑战。

Method: 基于32B参数的LLM，采用推理增强训练（Reinforcement Learning from Human Feedback），使用49,170个公开免费文本病例和104,666个合成病例（表型锚定叙述），并在四个外部验证中心和随机医生辅助试验中评估。

Result: 在公开基准和四个外部验证中心中性能最强；回顾性队列中61.06%的病例在临床怀疑前优先诊断，平均提前1.87个月；随机试验中辅助医生诊断准确率提高21.44个百分点；合成数据消融实验显示表型锚定叙述对长尾罕见病有效且数据量呈单调扩展趋势。

Conclusion: RaDaR提供了一个可部署的罕见病诊断推理模型，其开发与验证框架为数据稀缺下的诊断AI研究提供了可复现的范式。
...
### [221] [Reinforcement Learning for Computer-Use Agents with Autonomous Evaluation](https://arxiv.org/abs/2606.24515)
*Marta Sumyk,Oleksandr Kosovan*

Main category: cs.AI

TL;DR: 提出一种通过噪声校正的自主视觉语言评估作为奖励信号的RL微调框架，显著提升GUI代理任务成功率。


<details>
  <summary>Details</summary>
Motivation: 由于开源桌面环境缺乏可扩展的机器可读奖励信号，任务成功难以通过手工规则或密集标签定义，导致RL训练困难。

Method: 使用VL模型根据最终截图和指令评估任务完成度，将评估反馈视为有噪声二元奖励，并推导噪声校正的奖励估计用于PPO强化学习。

Result: 在macOSWorld等三个基准上，校正后的评估奖励优于零样本和原始评估微调，平均成功率提升12.6和5.1个百分点。

Conclusion: 实验表明，显式建模和校正评估器噪声可使自主评估成为GUI环境中实用的RL奖励信号。
...
### [222] [GUI vs. CLI: Execution Bottlenecks in Screen-Only and Skill-Mediated Computer-Use Agents](https://arxiv.org/abs/2606.24551)
*Xiao Zhou,Siyue Zhang,Yilun Zhao,Jinbiao Wei,Tingyu Song,Arman Cohan,Chen Zhao*

Main category: cs.AI

TL;DR: 在受控基准测试中，GUI代理表现优于原始CLI代理，但通过技能增强CLI可反超，揭示两者不同瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有评估混杂了交互模态与任务差异，需要公平比较GUI和CLI代理。

Method: 引入440个桌面任务的匹配执行层基准测试，GUI和CLI代理接收相同目标、状态和验证器，但使用各自模态原生操作。

Result: 最强GUI代理通过率59.1%，最强原始CLI代理48.2%，技能增强CLI达69.3%。

Conclusion: GUI代理受限于长期工作流的可靠交互，CLI代理受限于技能接口覆盖和可扩展性。
...
### [223] [ASALT: Adaptive State Alignment for Lateral Transfer in Multi-agent Reinforcement Learning](https://arxiv.org/abs/2606.24601)
*Anurag Akula,Satheesh K. Perepu,Abhishek Sarkar,Kaushik Dey*

Main category: cs.AI

TL;DR: 提出ASALT方法，通过适配器将不同维度的观察和状态映射到共享嵌入空间，实现多智能体跨域迁移学习。


<details>
  <summary>Details</summary>
Motivation: 现有MARL迁移学习要求源和目标域的状态空间维度相同，限制了实际应用。

Method: ASALT引入观察级和状态级适配器，将目标域的观测和全局状态映射到共享嵌入空间，实现演员和评论家的知识迁移。

Result: 在协作场景中，ASALT在样本效率和全局回报上优于基线，但效果取决于域不匹配程度，且能减轻负迁移。

Conclusion: ASALT有效处理状态空间维度不匹配的迁移问题，并缓解负迁移，提升了迁移的鲁棒性。
...
### [224] [Uncertainty-Aware Longitudinal Forecasting of Alzheimer's Disease Progression Using Deep Learning](https://arxiv.org/abs/2606.24604)
*Arya Hariharan,Shreyank N Gowda,Anala M R*

Main category: cs.AI

TL;DR: 提出结合有序诊断预测、多时间点轨迹生成和分解不确定性估计的概率框架，用于阿尔茨海默病进展建模，在ADNI上优于基线，轨迹不确定性合理。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型将阿尔茨海默病进展简化为单步分类，缺乏对未来轨迹的不确定性估计，难以提供可靠的长期预测。

Method: 采用Temporal Fusion Transformer编码器与CORAL有序输出层，结合不对称损失权重和转换器过采样；使用自回归混合密度网络生成五年概率轨迹，并通过解析混合方差和五元自助法集成分离aleatoric和epistemic不确定性。

Result: 在ADNI上，模型在下一个诊断预测中优于基线和Transformer基线，尤其在MCI vs 痴呆鉴别上提升显著；生成轨迹的90%可信区间覆盖接近名义水平，不确定性随预测时间增加；epistemic不确定性在稀有病型、MCI和痴呆患者中更高，在OASIS-3外部评估中也与预测误差正相关。

Conclusion: 该概率框架能有效建模疾病进展轨迹并提供可靠的不确定性估计，有助于临床决策和患者管理。
...
TL;DR: 本文通过实验表明，使用LLM助手支持CQ验证后，仍需要工具在发布前优化CQ以避免歧义和复杂性。


<details>
  <summary>Details</summary>
Motivation: CQ验证耗时且易错，歧义和复杂性会导致不一致的建模和验证结果。

Method: 对19名参与者进行实验，使用LLM助手执行20个CQ验证任务。

Result: 结果表明需要工具在发布前优化CQ以避免歧义或过度复杂。

Conclusion: 工具对于避免后期本体工程阶段的歧义和复杂性是必要的。
...
TL;DR: Themis is an XAI-enabled framework for Reinforcement Learning from Human Feedback that combines explainability and alignment, training reward models matching or outperforming true reward signals.


<details>
  <summary>Details</summary>
Motivation: No publicly available framework combines transparency via explainability and alignment via human feedback for safe RL.

Method: Themis is introduced as a configurable framework supporting over 200 environments, with a cloud-based platform for collecting human feedback.

Result: Themis trains reward models that match or outperform the true reward signal using human preferences, and scales to support 1000 users on a modest machine.

Conclusion: Themis effectively combines XAI and RLHF for safe RL training and evaluation, with scalable infrastructure.
...
TL;DR: 提出SAFARI框架，通过工具增强的诊断循环和短时记忆，实现超越上下文限制的代理故障诊断。


<details>
  <summary>Details</summary>
Motivation: 现有方法将完整轨迹加载到LLM上下文窗口中，导致注意力稀释，且当轨迹超过上下文限制时失效。

Method: SAFARI用工具增强的诊断循环替代线性上下文加载，包括专门工具箱和持久短时记忆，实现诊断准确性与上下文限制的解耦。

Result: 在Who&When数据集上以1M token预算比SOTA高20%，在TRAIL GAIA子集上以25K token预算高19%，在故障位置超出原生上下文5倍时仍保持0.58精度。

Conclusion: SAFARI有效解耦诊断准确性与上下文限制，在超出上下文窗口时仍能保持有效诊断，传统方法则完全失效。
...
TL;DR: 提出LaGO框架，利用预训练LLM作为潜在动作先验来软指导在线强化学习，避免直接控制的不稳定性，在离散和连续控制任务上均显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将LLM作为直接控制器进行精确动作生成，在实际中不可靠。

Method: LaGO框架将预训练LLM作为潜在动作先验，以软指导方式优化在线策略，而非显式规划或控制。

Result: 在CLEVR-Robot上平均成功率从15.1%提升至27.2%，在Meta-World上从2.7%提升至15.2%，同时奖励也持续提高。

Conclusion: LLM的知识能改善规划和在线决策，且更强的预训练LLM提供更有效的指导。
...
TL;DR: 提出首个实用的精确算法，用于在可变成本和概率分布下最小化命题公式的期望评估成本。


<details>
  <summary>Details</summary>
Motivation: 在许多决策场景中，信息获取成本不同，需要构造最优的确定性评估策略。

Method: 提出了分支定界算法，包含变量选择启发式、剪枝和缓存技术。

Result: 实验展示了算法在随机实例上的可扩展性，量化了贪婪束搜索变体的效率-质量权衡，并在心脏疾病诊断实例上验证。此外，证明了问题为#P-hard且属于PSPACE。

Conclusion: 该算法是首个实用且精确的解决方案，为该层次的一般性问题提供了有效方法。
...
### [230] [BluTrain: A C++/CUDA Framework for AI Systems](https://arxiv.org/abs/2606.24780)
...
TL;DR: BluTrain是一个从零开始设计的轻量级、架构通用的训练框架，通过原生实现所有层并在8-GPU系统上训练GPT-2，在吞吐量和内存效率上超越PyTorch，同时保持数值精度。


<details>
  <summary>Details</summary>
Motivation: 深度学习进展更多依赖系统工程而非建模，模型训练行为由硬件表达方式决定，需实现对硬件表达的绝对控制并简化系统复杂性。

Method: BluTrain基于标准C++和CUDA编程模型，原生实现了类型化张量模块（带反向模式自动微分）、线性代数库、缓存分配器、多模式分布式执行模块和基于MLIR的深度学习编译器。

Result: 在8-GPU 6000 Ada系统上训练124M参数GPT-2，BluTrain实现平均407K tokens/s（PyTorch为395K），内存占用减少最高22%，数值精度严格保持，最终验证损失略低。

Conclusion: 每层都开放给原生调优，性能上限由框架自身决定。
...
TL;DR: 本研究调查了LLM对话助手在评估非功能需求时的准确性和质量，发现开发者同意LLM但准确性低，且系统响应长度和提供信息轮次负面影响满意度，而主动交互正面影响。


<details>
  <summary>Details</summary>
Motivation: 当前评估基准仅关注功能正确性，忽略了非功能需求（NFRs）的评估质量，而NFRs模糊、依赖上下文且涉及程序多个部分，需要超越单轮准确性的方法。

Method: 招聘49名程序员与GitHub Copilot交互，评估148个HIPAA相关的NFRs在iTrust代码库中的三个方面：需求满足程度、推理和代码定位。

Result: 开发者倾向于同意LLM评估，但准确性低；用户满意度建模表明，更长的系统响应和更多信息提供轮次负面影响满意度，而主动交互正面影响。
...
### [232] [Critique of Agent Model](https://arxiv.org/abs/2606.23991)
*Eric Xing,Mingkai Deng,Jinyu Hou*

Main category: cs.AI

TL;DR: 本文基于笛卡尔哲学和科幻作品，定义了真正的代理需要内在化结构，区分了代理式(agentic)和能动式(agentive)系统，并提出GIC架构以实现通用代理模型。


<details>
  <summary>Details</summary>
Motivation: 随着LLM系统被营销为“代理”，需要澄清自动化与代理的界限，以构建可靠系统并理解AI风险。

Method: 通过哲学概念和科幻实例分析当前AI代理，沿五个维度分析架构，提出内在化要求，并设计GIC架构，包括层次目标分解、身份演化、基于世界模型的模拟推理、学习自我调节和自我导向学习。

Result: 给出了代理式与能动式系统的区分，并提出了GIC架构作为通用代理模型。

Conclusion: 强调能动式系统具有更大自主性但仍需人类监督，并提供关于可审计性、可控性和安全性的见解。
...
### [233] [Safe and Generalizable Hierarchical Multi-Agent RL via Constraint Manifold Control](https://arxiv.org/abs/2606.24010)
*Zihao Guo,Jianing Zhao,Ling Li,Hao Liang,Giuseppe Loianno,Yali Du*

Main category: cs.AI

TL;DR: 分层多智能体强化学习框架，通过低层约束流形强制硬安全约束，高层学习协调策略，提供理论安全保证和稳定训练，实现高安全率与竞争性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在学习性能与安全保证之间存在权衡：学习型方法缺乏理论安全保证，控制型方法过于保守。

Method: 提出分层多智能体强化学习框架，低层通过约束流形强制硬安全约束，高层学习协调策略，提供理论安全保证和稳定学习动态。

Result: 该方法在保持近乎完美安全率的同时实现竞争性能，并有效泛化至不同数量的智能体和障碍物。

Conclusion: 该方法在安全与效率间取得平衡，具备理论保证和良好泛化能力。
...
### [234] [Ensemble Feature Selection and Harris Hawks Optimization for Explainable Mental Health Risk Prediction in Female Sex Workers](https://arxiv.org/abs/2606.24047)
*Ahnaf Atef Choudhury,Md. Parvej Hoque Palash,Shahriar Siddique Ayon,Ramkrishna Saha,Abdullah Al Mamun*

Main category: cs.AI

TL;DR: 提出一种混合预测模型，结合集成特征选择和哈里斯鹰优化逻辑回归，用于预测女性性工作者的抑郁风险，并利用可解释AI揭示关键因素。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习模型难以捕捉女性性工作者心理健康问题中的高维复杂风险模式，而该群体暴露于暴力、污名和经济困难，亟需更有效的预测工具。

Method: 混合预测模型：采用ANOVA和互信息进行集成特征选择，使用Harris Hawks优化调优逻辑回归，并结合可解释AI（XAI）分析风险因素。

Result: 在3005名女性性工作者中，模型准确率95.78%，F1分数95.77%，AUC 0.96，识别出创伤后应激、客户相关暴力和职业因素为主要抑郁预测因子。

Conclusion: 该模型弥合了传统方法与机器学习之间的差距，提供了一个可解释AI工具，有助于对弱势群体进行早期干预和针对性心理健康规划。
...
### [235] [Exploring Academic Influence of Algorithms by Co-occurrence Network Based on Full-text of Academic Papers](https://arxiv.org/abs/2606.24099)
*Yuzhuo Wang,Chengzhi Zhang,Min Song,Seong Deok Kim,Youngsoo Ko,Juhee Lee*

Main category: cs.AI

TL;DR: 本文首次大规模构建NLP领域算法共现网络，分析算法影响力的网络结构和时间演化。


<details>
  <summary>Details</summary>
Motivation: 现有研究孤立评估个体算法，忽视算法间的集体影响，因此从网络视角研究算法影响。

Method: 使用深度学习模型从论文全文中提取算法实体，构建整体、累积和年度共现网络，并用多种中心性度量分析。

Result: 算法网络具有复杂网络特征，连接密度随时间增加；经典、高性能及跨期算法高中心性；影响力下降时先失去核心位置。

Conclusion: 提供算法影响力的时空视角，为未来算法、学者和任务网络研究奠定基础。
...
### [236] [VeryTrace: Verifying Reasoning Traces through Compilable Formalism and Structured Verification](https://arxiv.org/abs/2606.24124)
*Ninghan Zhong,Ahmet Ege Tanriverdi,Kaan Kale,Sriram Vishwanath*

Main category: cs.AI

TL;DR: VeryTrace通过形式化推理轨迹和混合验证，提高了链式思维推理的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 链式思维提示中的逻辑错误或幻觉会无声传播，导致自信但错误的结论。

Method: 提出VeryTrace框架，包括领域特定语言（DSL）来表示推理步骤，以及结合确定性检查和LLM审计的混合验证器，实现步骤级错误定位和修复。

Result: 在AIME 2025、LLM-BabyBench和CLUTRR三个基准测试上，VeryTrace提高了零样本基线准确率。

Conclusion: 形式化轨迹验证实现了精确性和泛化性，无需领域特定训练或上下文示例。
...
### [237] [The Geometry Behind Diffusion and Flow Matching: Gradient Flows and Geodesics in Wasserstein Space](https://arxiv.org/abs/2606.24157)
*Yian Yao,Weiwei Zhang*

Main category: cs.AI

TL;DR: 本文统一了扩散模型和最优传输流匹配在Wasserstein几何中的框架，揭示两者分别为自由能梯度流和Wasserstein测地线。


<details>
  <summary>Details</summary>
Motivation: 扩散模型和流匹配方法虽密切相关但缺乏统一的理论基础，需要阐明其几何关系。

Method: 利用概率测度空间$\mathcal{P}_2(\mathbb{R}^d)$的Wasserstein几何，将扩散模型视为自由能梯度流（JKO scheme），流匹配视为Wasserstein测地线（Benamou-Brenier公式）。

Result: 扩散模型沿梯度流路径，流匹配沿测地线路径，两者收敛到相同分布，且流匹配因直线路径需更少采样步。

Conclusion: 在统一几何框架下，两类模型的关系精确，为生成模型的设计提供新视角。
...
TL;DR: 多语言初始化在低数据时显著降低词错误率，但优势随目标语言数据量增加而消失，且不受延迟影响；量化引入轻微性能下降。


<details>
  <summary>Details</summary>
Motivation: 解决流式语音识别模型适应新语言时，选择多语言还是仅英语初始化的问题，并探究数据量、延迟和量化对优势的影响。

Method: 使用0.6B参数的FastConformer编码器，在8种欧洲语言上控制目标语言数据量（100h-2500h）、三种流式延迟和离线解码，进行系统实验并评估词错误率。

Result: 多语言初始化优势随数据量增加按幂律衰减，每次数据翻倍优势减半；延迟不显著影响优势；4-bit量化使编码器缩小约3倍，平均词错误率增加约0.5个百分点。

Conclusion: 低数据时使用多语言初始化，大数据时选择无关紧要；延迟和量化决策可独立于初始化进行。
...
### [239] [Tractable Reasoning and Conjunctive Query Answering for Defeasible DL-Lite under Rational Closure](https://arxiv.org/abs/2606.24279)
*Giovanni Casini,Umberto Straccia*

Main category: cs.AI

TL;DR: Efficient reasoning under Rational Closure for DL-Lite using a plug-in architecture with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: To handle defeasible knowledge in Description Logics non-monotonically, extending Rational Closure to lightweight DL-Lite variants efficiently.

Method: Analyzing entitlement and Conjunctive Query answering under Rational Closure for DL-Lite core and horn, and designing a plug-in architecture leveraging existing classical reasoners.

Result: Reasoning and CQ answering under Rational Closure for DL-Lite can be done efficiently with minimal computational overhead.

Conclusion: The plug-in architecture enables practical non-monotonic reasoning for DL-Lite under Rational Closure without significant performance penalties.
...
### [240] [MVG-KAN: Multi-View Geo-Wind Guided KAN for PM$_{2.5}$ Forecasting](https://arxiv.org/abs/2606.24347)
*Cheng Huang,Muyao Guan,Jairus Yougui Railey,Ning Xu,Honghui Xu,Changjiang Zhang,Zhen Zhang,Shiqing Zhang,Cong Bai*

Main category: cs.AI

TL;DR: 提出多视角地理-风引导KAN模型（MVG-KAN）用于PM2.5预测，从周期、残差和空间扩散三个视角建模站级演化。


<details>
  <summary>Details</summary>
Motivation: 现有时空预测方法主要基于距离、相关性或纯自适应图，难以充分表示风依赖的污染物传输等异质因素。

Method: MVG-KAN包含周期-残差分解主干、Geo-Wind图（结合地理距离衰减和风向风速传输）以及TKAN残差头（学习非线性自回归校正）。

Result: 摘要未给出具体实验结果，但模型设计旨在增强对残差惯性和污染物协变的建模。

Conclusion: 该模型综合了局地周期规律、站间残差动态和气象引导空间扩散，为PM2.5预测提供了一种新的多视图融合方法。
...
TL;DR: DigenRL是一种用于扩散生成大语言模型的解耦合强化学习框架，通过生成轴流水线、时间步并行、弹性训练器辅助生成和一步约束异步策略，实现了1.56-2.10倍的吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 现有扩散生成LLMs的RL系统（如veRL-Omni）采用共置执行，耦合了推出和训练资源，限制了异构部署和独立扩展，且高效系统尚未充分探索。

Method: 提出DigenRL框架，包含生成轴流水线(GAP)和时间步并行(TSP)实现细粒度流水线、弹性训练器辅助生成(TAG)动态利用训练资源、以及紧密的一步约束异步策略减少执行气泡。

Result: 在16-32 GPU的三个测试床上，使用HunyuanVideo-13B等模型，相比veRL-Omni和GenRL，吞吐量提升1.56-2.10倍。

Conclusion: DigenRL通过解耦合架构和多项优化有效提升了扩散生成LLMs的RL系统效率，支持灵活资源分配和异构部署。
...
TL;DR: ATRIA是一个多智能体心电图报告系统，通过将报告声明绑定到支持证据并允许验证和修改单个发现，来模拟临床医生的迭代工作流程。


<details>
  <summary>Details</summary>
Motivation: 现有心电图报告生成方法要么是端到端融合导致错误传播，要么是基于智能体的单次通行而不重新审视输出，而临床报告需要迭代上下文整合和双向编辑。

Method: ATRIA使用多个智能体，将每个报告声明绑定到支持证据，标记无支持的声明，在会话中整合额外上下文，并让临床医生验证和修改单个发现，而不是接受一个不透明的输出。

Result: 通过四个交互案例展示了ATRIA，并提供现场演示和视频。

Conclusion: ATRIA作为基于云的web服务已准备就绪，可立即部署，使用已在临床使用的心电图分析模型，因此提供临床可信的发现和迭代工作流程。
...
TL;DR: 提出一种循环一致神经架构，通过前向和逆向来生成形式验证证书的忠实自然语言解释，并利用符号验证器确保忠实性，在财务合规领域达到90.0%的循环验证正确性。


<details>
  <summary>Details</summary>
Motivation: 形式验证产生的证书对非专业利益相关者不透明，需要可解释的生成方法。

Method: 使用前向网络NN1将证书映射到解释，逆向网络NN2从解释重建证书，并利用符号验证器提供可微分忠实性代理；采用指针生成器机制复制状态名称以确保词汇基础；结合混合推理时间路由策略。

Result: 在420个测试证书上，达到90.0%循环验证正确性，超过最佳多LLM基线（76.1%）13.9个百分点，在12个类别中获胜10个，推理速度快860倍（185毫秒vs160秒），且支持离线操作、确定性输出和零推理成本。

Conclusion: 训练专业化优于通用LLM提示，且消除了云端推理的部署约束。
...
### [244] [On the Smallness of the Large Language Models Scaling Exponents](https://arxiv.org/abs/2606.24504)
*Sauro Succi,Peter V. Coveney,Alex Hansen*

Main category: cs.AI

TL;DR: 当前LLM缩放指数表明能源不可持续，pedestal effect不能解决该问题，数据粗糙度可能影响缩放行为。


<details>
  <summary>Details</summary>
Motivation: LLM应用的缩放指数显示能源消耗不可持续，需要深入理解其根本原因。

Method: 通过理论分析和与流体湍流模型的类比，探讨数据平滑度对缩放指数的影响。

Result: 归因于pedestal effect并不能消除不可持续性问题，数据粗糙度通过类比具有显著影响。

Conclusion: LLM缩放指数的不可持续性仍然存在，且数据特性（如粗糙度）是重要因素。
...
### [245] [Quant Convergence: Bridging Classical Value Investing and Modern Factor Models for Systematic Equity Selection](https://arxiv.org/abs/2606.24575)
*Augusto Eiji Yamazaki,Hugo Garrido-Lestache Belinchon*

Main category: cs.AI

TL;DR: 格雷厄姆价值投资规则能有效约束AI模型风险，纯格雷厄姆模型表现优于复杂模型。


<details>
  <summary>Details</summary>
Motivation: 现代AI模型常过度拟合市场噪音，研究者想测试格雷厄姆规则能否作为约束。

Method: 构建三种特征集（纯格雷厄姆规则、现代市场因子、混合），使用XGBoost和AutoGluon模型，在20年数据上测试，采用四年买入持有策略。

Result: 纯格雷厄姆随机森林取得232.13%最高回报，1.38 Calmar比率；AutoGluon虽有222.68%回报但回撤39.78%；混合随机森林最大回撤最低34.53%。

Conclusion: 格雷厄姆的‘安全边际’仍有效，可防止AI过度冒险。
...
### [246] [LLMs Prompted for Legal Context Object More: Overrefusal from Small On-Premises LLMs in Criminal Legal Context](https://arxiv.org/abs/2606.24585)
*Anastasiia Kucherenko,François Brouchoud,Dimitri Percia David,Andrei Kucharavy*

Main category: cs.AI

TL;DR: 研究发现权威前缀显著增加小型LLM在法律提示中的拒绝率，而角色扮演前缀效果不一，表明模型对上下文框架不稳定。


<details>
  <summary>Details</summary>
Motivation: 法律专业人士已开始使用个人LLM，但即使看似无害的使用也可能因选择性拒绝引入偏见，因此需要预测和评估这种偏见。

Method: 调查多个现代小型LLM在带有不同前缀（权威、角色扮演）的法律提示下的拒绝行为，与无前缀基线比较。

Result: 权威前缀使拒绝率增加2-20倍；角色扮演前缀对某些模型大幅增加拒绝，对另一些影响不大。

Conclusion: 小型可部署LLM在真实机构用户可能引入的上下文框架下不稳定，需进一步研究以最小化偏见机会。
...
TL;DR: 本文提出了AI tokenomics框架，分析token在AI系统中的生成、消费、定价、分配和优化。


<details>
  <summary>Details</summary>
Motivation: Token已成为现代基础模型服务的核心会计单位，需要理解其经济含义以优化资源分配和定价。

Method: 通过连接token级技术成本与工作流级生产函数、企业资源分配、测量方法、市场设计问题，构建框架。

Result: 发现token支出和经济价值不同，价值取决于边际生产率、工作流位置、隐藏推理活动、风险和下游传播效应。

Conclusion: 识别了开放研究方向：隐藏token测量、经验校准、token生产率、动态分配和基于token的市场。
...
TL;DR: 本文扩展UCQ以表达最小完全抽象，并通过最大恢复表征最大声音抽象。


<details>
  <summary>Details</summary>
Motivation: 在OBDA中，需要将数据查询抽象到本体层，但完美抽象可能不存在，因此研究最小完全和最大声音抽象。

Method: 在UCQs中引入有限不等式和标记常数谓词，分析其表达能力，并与数据交换中的最大恢复概念建立联系。

Result: 扩展能够表达最小完全抽象，从而表达完美抽象（若存在）；通过最大恢复表征了最大声音抽象。

Conclusion: 该扩展在不增加复杂度的情况下支持抽象，并建立了与数据交换的联系。
...
### [249] [Decentralised AI Training and Inference with BlockTrain](https://arxiv.org/abs/2606.24722)
*Peter Toth*

Main category: cs.AI

TL;DR: Spheroid BlockTrain is a decentralized training protocol that partitions models into independently trainable blocks, achieving competitive performance with reduced infrastructure requirements.


<details>
  <summary>Details</summary>
Motivation: Frontier AI training is dominated by centralized accelerator clusters, creating structural advantages for large labs and making open/independent AI efforts dependent on scarce capital and infrastructure.

Method: BlockTrain splits a model into blocks, each optimized on local objectives from the same global target and composed at inference; uses HTTP/TCP transport for distributed training.

Result: On WikiText, BlockTrain achieves CE 1.359 (perplexity 3.89), within 0.04 CE of an end-to-end Transformer; six-worker run reaches CE 1.385; three-host run improves CE from 5.580 to 1.811 with 15.22 GB movement; inference outperforms plain-autoregressive TCP pipeline.

Conclusion: BlockTrain enables decentralized training with near-centralized performance, reducing reliance on dense accelerator clusters and democratizing AI development.
...
### [250] [Can Scale Save Us From Plasticity Loss in Large Language Models?](https://arxiv.org/abs/2606.24752)
*J. Fernando Hernandez-Garcia,Tomás Figliolia,Beren Millidge*

Main category: cs.AI

TL;DR: 研究了GPT-style Transformer模型在多语言持续学习中的损失可塑性问题，发现该现象依然存在，并遵循可预测的缩放定律，且单靠增加参数无法完全避免。


<details>
  <summary>Details</summary>
Motivation: 尽管损失可塑性在持续学习中已被认识数十年，但主要在小模型和较老架构中研究，缺乏在现代基于Transformer的大型语言模型（LLM）中尤其是在自然语言领域的探索，因此需要确定该问题是否仍然存在。

Method: 在GPT-style Transformer模型（参数量5M到314M的非嵌入参数）上进行多语言持续学习实验，通过在越南语探测任务上的性能下降来度量可塑性损失，并分析其与模型大小的缩放关系。

Result: 发现了损失可塑性的证据，且其发生遵循一个可预测的缩放定律，与模型大小呈亚线性增长；更大模型可以延迟可塑性损失的明显效应，但增加参数无法完全阻止；同时在静态多语言训练中也观察到可塑性损失，表明该现象并非仅限于任务突变的持续学习。

Conclusion: 大型Transformer语言模型在经过长时间训练后，无论是在持续学习还是静态设置中，最终都会失去高效适应新数据的能力，因此损失可塑性仍然是当前LLM面临的一个挑战。
...
TL;DR: 本文指出Andreas & Günther的最新因果关系定义同时包含三种类型，表明其分类无区别，并通过例子比较削弱了所有七个定义。


<details>
  <summary>Details</summary>
Motivation: 评估Andreas & Günther对实际因果关系的七种定义及其三种类型分类的有效性。

Method: 通过逻辑分析证明最新定义包含所有三种类型，并在关键例子上比较新定义与其他六种定义。

Result: 发现最新定义实例化了所有三种类型，且比较显示所有七个定义在例子中表现不一致。

Conclusion: Andreas & Günther的分类是区分无区别的，且他们的所有因果关系定义都存在问题。
...
### [253] [Grading the Grader: Lessons from Evaluating an Agentic Data Analysis System](https://arxiv.org/abs/2606.24839)
...
TL;DR: 提出结构认证，一种过渡局部框架，通过深度组合目标过滤过渡，提供通用智能体内部世界模型的逐项误差保证，实现可认证部署。


<details>
  <summary>Details</summary>
Motivation: 在大世界体制中，智能体能力必然专化，标准均匀保证无法区分关键瓶颈和无关失败，且已证明通用智能体非通用，使标准最坏情况分析无信息性。

Method: 引入结构认证，一种过渡局部框架，将有界目标条件性能映射到智能体内部世界模型的逐项保证；提供算法使用深度组合目标过滤特定过渡。

Result: 证明通用智能体在深度组合目标上具有结构世界模型，误差界为O(1/n)+O(δ)，该界在小区间δ regime中是紧的。

Conclusion: 通过定位长时域规划可靠的特定过渡，实现通用智能体的可认证部署。
